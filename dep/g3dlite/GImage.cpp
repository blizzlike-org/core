...
GImage::GImage(const std::string &filename, Format format, const MemoryManager::Ref &m)
...
GImage::GImage(const GImage &other, const MemoryManager::Ref &m) : m_memMan(m), m_byte(NULL) { _copy(other); }
...
bool GImage::pasteSubImage(GImage &dest, const GImage &src, int destX, int destY, int srcX, int srcY, int srcWidth,
                           int srcHeight) {

  if ((src.m_width < srcX + srcWidth) || (src.m_height < srcY + srcHeight) || (dest.m_width < destX + srcWidth) ||
      (dest.m_height < destY + srcHeight) || (srcY < 0) || (srcX < 0) || (destY < 0) || (destX < 0) ||
...
bool GImage::supportedFormat(const std::string &format) { return (stringToFormat(format) != UNKNOWN); }
...
int GImage::sizeInMemory() const { return sizeof(GImage) + m_width * m_height * m_channels; }
...
void GImage::computeNormalMap(int width, int height, int channels, const uint8 *src, GImage &normal,
...
#define ELEVATION(DX, DY) ((int)B[(((DX + x + w) % w) + ((DY + y + h) % h) * w) * stride])
      ...
      delta.y = -(ELEVATION(-1, -1) * 1 + ELEVATION(0, -1) * 2 + ELEVATION(1, -1) * 1 + -ELEVATION(-1, 1) * 1 +
                  -ELEVATION(0, 1) * 2 + -ELEVATION(1, 1) * 1);

      delta.x = -(-ELEVATION(-1, -1) * 1 + ELEVATION(1, -1) * 1 + -ELEVATION(-1, 0) * 2 + ELEVATION(1, 0) * 2 +
      ...
      if (lowPassBump) {
        H = (ELEVATION(-1, -1) + ELEVATION(0, -1) + ELEVATION(1, -1) + ELEVATION(-1, 0) + ELEVATION(0, 0) +
             ELEVATION(1, 0) + ELEVATION(-1, 1) + ELEVATION(0, 1) + ELEVATION(1, 1)) /
            (255.0f * 9.0f);
      }
#undef ELEVATION
