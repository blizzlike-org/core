...
/**
  ...
  G3D::Texture::Ref - Represents image on the graphics card (not directly readable on the CPU).  Supports 2D, 3D, and a
  variety of interpolation methods, loads file formats.
  ...
  const Storage &slowGet(int x, int y, WrapMode wrap) {
    ...
    case WrapMode::ERROR:
      alwaysAssertM(((uint32)x < w) && ((uint32)y < h),
                    format("Index out of bounds: (%d, %d), w = %d, h = %d", x, y, w, h));
...
protected:
  ...
  Compute bicubic(const Compute *ctrl, double s) const {
    ...
    static const double B[4][4] = {
        {0.0, -0.5, 1.0, -0.5}, {1.0, 0.0, -2.5, 1.5}, {0.0, 0.5, 2.0, -1.5}, {0.0, 0.0, -0.5, 0.5}};
...
public:
  ...
  */
  ...
  static Ref create(int w = 0, int h = 0, WrapMode wrap = WrapMode::ERROR) { return new Map2D(w, h, wrap); }
  ...
  virtual void crop(const Rect2D &rect) {
    crop(iRound(rect.x0()), iRound(rect.y0()), iRound(rect.x1()) - iRound(rect.x0()),
