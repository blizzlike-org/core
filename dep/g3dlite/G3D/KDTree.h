...
*/
template <class Type> class Indirector {
public:
  Type *handle;
    ...
    */
    ...
    void verifyNode(const Vector3 &lo, const Vector3 &hi) {
      ...
      debugAssertM(lo == splitBounds.low(),
                   format("lo = %s, splitBounds.lo = %s", lo.toString().c_str(), splitBounds.low().toString().c_str()));
    ...
    bool intersects(const Ray &ray, float distance) const {
      ...
      bool rayWillHitBounds = CollisionDetection::collisionLocationForMovingPointFixedAABox(
          ray.origin(), ray.direction(), splitBounds, location, alreadyInsideBounds);

      bool canHitThisNode =
          (alreadyInsideBounds || (rayWillHitBounds && ((location - ray.origin()).squaredLength() < square(distance))));

      return canHitThisNode;
    }

    template <typename RayCallback>
    void intersectRay(const Ray &ray, RayCallback &intersectCallback, float &distance,
...
public:
  ...
  private:
    ...
    BoxIntersectionIterator(const AABox &b, const Node *root)
        : isEnd(root == NULL), box(b), node(const_cast<Node *>(root)), nextValueArrayIndex(-1) {
