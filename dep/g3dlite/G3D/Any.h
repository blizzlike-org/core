...
private:
  ...
  */
  ...
  inline bool isPlaceholder() const { return !m_placeholderName.empty(); }
...
public:
  ...
  */
  ...
  Any &operator[](const char *key) { return operator[](std::string(key)); }
...
public:
  ...
  /** If key \s appears in the any, reads its value into \a v and
    ...
    */
  template <class ValueType> void get(const std::string &s, ValueType &v) {
    v = m_any[s];
  ...
  /** Get the value associated with a key only if the key is actually present.
    ...
    */
  template <class ValueType> bool getIfPresent(const std::string &s, ValueType &v) {
