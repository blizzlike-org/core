...
public:
  ...
  public:
    ...
    */
    ...
    Settings()
        : wordWrap(WRAP_WITHOUT_BREAKING), allowWordWrapInsideDoubleQuotes(false), numColumns(80), spacesPerIndent(4),
          convertNewlines(true), trueSymbol("true"), falseSymbol("false") {
#ifdef G3D_WIN32
...
public:
  ...
  /** Convenient idiom for writing multiple symbols in a row, e.g.
      ...
      */
  void writeSymbols(const std::string &a, const std::string &b = "", const std::string &c = "",
  ...
  void vprintf(const char *fmt, va_list argPtr) G3D_CHECK_VPRINTF_METHOD_ARGS;
};
