...
/**
 ...
 an enum would use:
  ...
  the same output.
 */
template <class Key, class Value, class HashFunc = HashTrait<Key>, class EqualsFunc = EqualsTrait<Key>> class Table {
public:
  /**
...
private:
  ...
  private:
    ...
    Node(const Key &k, const Value &v, size_t h, Node *n) : entry(k, v), hashCode(h), next(n) {}
...
public:
  ...
  */
   ...
   */
  double debugGetLoad() const { return debugGetDeepestBucketSize() / (double)size(); }
  ...
  private:
     ...
     */
    Iterator(const ThisType *table) : table(const_cast<ThisType *>(table)) { isDone = true; }

    Iterator(const ThisType *table, size_t m_numBuckets, Node **m_bucket)
        : table(const_cast<ThisType *>(table)), m_numBuckets(m_numBuckets), m_bucket(m_bucket) {
  ...
  public:
    ...
    Entry *operator->() const { return &(node->entry); }

    operator Entry *() const { return &(node->entry); }
   ...
   */
  Iterator begin() const { return Iterator(this, m_numBuckets, m_bucket); }
