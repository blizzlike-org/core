...
public:
  ...
  */
   ...
   */
  inline int64 getLength() const { return m_length; }

  inline int64 size() const { return getLength(); }

  /**
   Returns the current byte position in the file,
   where 0 is the beginning and getLength() - 1 is the end.
   */
  inline int64 getPosition() const { return m_pos + m_alreadyRead; }
   ...
   */
  inline void reset() { setPosition(0); }
  ...
  inline bool readBool8() { return (readInt8() != 0); }
  ...
  /**
   Skips ahead n bytes.
   */
  inline void skip(int64 n) { setPosition(m_pos + m_alreadyRead + n); }

  /**
    Returns true if the position is not at the end of the file
  */
  inline bool hasMore() const { return m_pos + m_alreadyRead < m_length; }
