...
public:
  ...
  template <typename T> inline static void multisend(const Array<ReliableConduitRef> &array, uint32 type, const T &m) {
...
private:
  ...
  template <typename T> void serializeMessage(uint32 type, const T &m, BinaryOutput &b) const {
    ...
    debugAssertM(b.size() < MTU, format("This LightweightConduit is limited to messages of "
