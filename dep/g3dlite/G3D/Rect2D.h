...
public:
  ...
  /** Min, min corner */
  Vector2 x0y0() const { return min; }

  Vector2 x1y0() const { return Vector2(max.x, min.y); }

  Vector2 x0y1() const { return Vector2(min.x, max.y); }

  /** Max,max corner */
  Vector2 x1y1() const { return max; }

  /** Width and height */
  Vector2 wh() const { return max - min; }

  Vector2 center() const { return (max + min) * 0.5; }
  ...
  static Rect2D xywh(const Vector2 &v, const Vector2 &w) { return xyxy(v.x, v.y, v.x + w.x, v.y + w.y); }
  ...
  bool contains(const Vector2 &v) const { return (v.x >= min.x) && (v.y >= min.y) && (v.x <= max.x) && (v.y <= max.y); }
   ...
   */
  template <class T> void clip(const Array<T> &inPoly, Array<T> &outPoly) const {
    ...
    bool entirelyClipped =
        clipSide2D(x0(), lessThan, X, inPoly, temp) || clipSide2D(x1(), greaterThan, X, temp, outPoly) ||
