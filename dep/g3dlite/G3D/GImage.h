/**
  ...
  @cite JPEG compress/decompressor is the <A HREF="http://www.ijg.org/files/">IJG library</A>, used in accordance with
  their license.
  @cite JPG code by John Chisholm, using the IJG Library
  @cite TGA code by Morgan McGuire
  @cite BMP code by John Chisholm, based on code by Edward "CGameProgrammer" Resnick <A
  HREF="mailto:cgp@gdnmail.net">mailto:cgp@gdnmail.net</A> at <A
  ...
  @cite PNG compress/decompressor is the <A HREF="http://www.libpng.org/pub/png/libpng.html">libpng library</A>, used in
...
public:
  class Error {
  public:
    Error(const std::string &reason, const std::string &filename = "") : reason(reason), filename(filename) {}
  ...
  */
  template <typename Type> inline const Type *rawData() const { return (Type *)m_byte; }

  /** \copybrief GImage::rawData() const */
  template <typename Type> inline Type *rawData() { return (Type *)m_byte; }
...
public:
  ...
  */
   ...
   */
  static bool pasteSubImage(GImage &dest, const GImage &src, int destX, int destY, int srcX, int srcY, int srcWidth,
                            int srcHeight);
  ...
  */
  ...
  <PRE>
    ...
    */
  static void computeNormalMap(const class GImage &bump, class GImage &normal,
                               const BumpMapPreprocess &preprocess = BumpMapPreprocess());

  static void computeNormalMap(int width, int height, int channels, const uint8 *src, GImage &normal,
                               const BumpMapPreprocess &preprocess = BumpMapPreprocess());
  ...
  */
  ...
  /** Overwrites every pixel with one of the two colors in a checkerboard pattern.
      ...
      */
  static void makeCheckerboard(GImage &im, int checkerSize = 1,
