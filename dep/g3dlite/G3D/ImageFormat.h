...
public:
  ...
  enum ColorSpace { COLOR_SPACE_NONE, COLOR_SPACE_RGB, COLOR_SPACE_HSV, COLOR_SPACE_YUV, COLOR_SPACE_SRGB };
...
private:
  ImageFormat(int numComponents, bool compressed, int glFormat, int glBaseFormat, int luminanceBits, int alphaBits,
              int redBits, int greenBits, int blueBits, int depthBits, int stencilBits, int hardwareBitsPerTexel,
              int packedBitsPerTexel, int glDataFormat, bool opaque, bool floatingPoint, Code code,
              ColorSpace colorSpace, BayerPattern bayerPattern = BAYER_PATTERN_NONE);
...
*/
  ...
  /** Converts between arbitrary formats on the CPU.  Not all format conversions are supported or directly supported.
      ...
      YUV422 expects data in YUY2 format (Y, U, Y2, v).  Most YUV formats require width and heights that are multiples
     of 2.

      Returns true if a conversion was available, false if none occurred.
  */
  static bool convert(const Array<const void *> &srcBytes, int srcWidth, int srcHeight, const ImageFormat *srcFormat,
  ...
  static bool conversionAvailable(const ImageFormat *srcFormat, int srcRowPadBits, const ImageFormat *dstFormat,
...
template <> struct HashTrait<const G3D::ImageFormat *> {
