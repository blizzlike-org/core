...
public:
   ...
   */
  inline explicit AABox(const Vector3 &v) { lo = hi = v; }
  ...
  */
  inline AABox(const Vector3 &low, const Vector3 &high) { set(low, high); }
  ...
  inline bool isFinite() const { return lo.isFinite() && hi.isFinite(); }

  inline const Vector3 &low() const { return lo; }

  inline const Vector3 &high() const { return hi; }
   ...
   */
  inline Vector3 center() const { return (lo + hi) * 0.5; }
  ...
  inline Vector3 extent() const { return hi - lo; }
...
@param childMask Test mask for the children of this volume.
  ...
  inline bool contains(const AABox &other) const {
    return (other.hi.x <= hi.x) && (other.hi.y <= hi.y) && (other.hi.z <= hi.z) && (other.lo.x >= lo.x) &&
           (other.lo.y >= lo.y) && (other.lo.z >= lo.z);
  }

  inline bool contains(const Vector3 &point) const {
    return (point.x >= lo.x) && (point.y >= lo.y) && (point.z >= lo.z) && (point.x <= hi.x) && (point.y <= hi.y) &&
           (point.z <= hi.z);
  }
  ...
  inline bool operator==(const AABox &b) const { return (lo == b.lo) && (hi == b.hi); }
