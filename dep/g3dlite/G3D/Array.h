...
/**
 ...
 \sa G3D::SmallArray
 */
template <class T, int MIN_ELEMENTS = 10, size_t MIN_BYTES = 32> class Array {
private:
  ...
  static bool compareGT(const T &a, const T &b) { return a > b; }
  ...
  */
  ...
  /**
     "The member function returns the storage currently allocated to hold the controlled
      ...
      For compatibility with std::vector.
  */
  int capacity() const { return numAllocated; }
...
return( lhs < rhs? true : false );
  ...
  //}
  template <class LessThan> void sort(const LessThan &lessThan) {
   ...
   */
  template <typename StrictWeakOrdering> void sortSubArray(int beginIndex, int endIndex, StrictWeakOrdering &lessThan) {
    std::sort(data + beginIndex, data + endIndex + 1, lessThan);
  }
  ...
  void partition(const T &partitionElement, Array<T> &ltArray, Array<T> &eqArray, Array<T> &gtArray,
  ...
  template <typename Comparator>
  void medianPartition(Array<T> &ltMedian, Array<T> &eqMedian, Array<T> &gtMedian, Array<T> &tempArray,
                       const Comparator &comparator) const {
