...
#define ThisType PointHashGrid<Value, PosFunc, EqualsFunc>
  ...
  /** Locate the cell and index within that cell containing v. Called by
      remove() and contains(). */
  bool find(const Value &v, Vector3int32 &foundCellCoord, Cell *&foundCell, int &index) {
...
public:
  ...
  */
  PointHashGrid(const Array<Value> &init, float radiusHint = -1.0f,
  ...
  */
  ...
  private:
    ...
    Iterator(const ThisType *grid)
        : m_isEnd(grid->size() == 0), m_grid(grid), m_tableIterator(grid->m_data.begin()), m_arrayIndex(0),
  ...
  public:
    inline bool operator!=(const Iterator &other) const {
      if (other.m_isEnd && m_isEnd) {
        ...
        return (m_isEnd != other.m_isEnd) || (m_tableIterator != other.m_tableIterator) ||
    ...
    Iterator &operator++() {
      debugAssert(!m_isEnd);
      debugAssertM(m_epoch == m_grid->m_epoch, "It is illegal to mutate the HashGrid "
  ...
  */
  ...
  private:
    ...
    /** End iterator */
    BoxIterator()
        : m_isEnd(true), m_grid(NULL), m_exact(true), m_current(0, 0, 0), m_cell(NULL), m_arrayIndex(0), m_epoch(0) {}

    /** Begin iterator */
    BoxIterator(const ThisType *grid, bool exact, const AABox &box)
        : m_isEnd(false), m_grid(grid), m_exact(exact), m_box(box), m_current(-1, 0, 0), m_cell(NULL), m_arrayIndex(0),
          m_epoch(grid->m_epoch) {
  ...
  */
  ...
  private:
    ...
    SphereIterator(const ThisType *grid, const Sphere &sphere)
        : m_isEnd(false), m_sphere(sphere), m_boxIterator(grid, false, getBoundingBox(sphere)) {
  ...
  */
  SphereIterator beginSphereIntersection(const Sphere &sphere) const { return SphereIterator(this, sphere); }
  ...
  */
  ...
  public:
    ...
    public:
      ...
      AABox bounds() const {
        ...
        return AABox(Vector3(k) * m_parent->m_cellWidth, Vector3(k + Vector3int32(1, 1, 1)) * m_parent->m_cellWidth);
      }
  ...
  private:
    ...
    CellIterator(const ThisType *grid)
        : m_isEnd(false), m_grid(grid), m_tableIterator(grid->m_data.begin()), m_epoch(grid->m_epoch) {
  ...
  public:
    ...
    inline bool operator!=(const CellIterator &other) const {
      ...
      return !((m_isEnd && other.m_isEnd) ||
               ((m_isEnd == other.m_isEnd) && (m_tableIterator != other.m_tableIterator)));
    }
    ...
    CellIterator &operator++() {
      debugAssertM(m_epoch == m_grid->m_epoch, "It is illegal to mutate the HashGrid while "
  ...
  */
  ...
  int debugGetDeepestBucketSize() const { return m_data.debugGetDeepestBucketSize(); }
