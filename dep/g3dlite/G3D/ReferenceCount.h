...
private:
  ...
  void zeroPointer() {
    if (m_pointer != NULL) {
      ...
      debugAssertM(pointer->ReferenceCountedObject_refCount > 0, "Dangling reference detected.");
  ...
  void setPointer(T *x) {
    if (x != m_pointer) {
      ...
      if (x != NULL) {
        ...
        // first pointer to it
        ReferenceCountedObject *pointer = (ReferenceCountedObject *)m_pointer;
        debugAssertM(pointer->ReferenceCountedObject_refCount >= 0, "Negative reference count detected.");
...
public:
   ...
   */
  template <class S> inline ReferenceCountedPointer(const ReferenceCountedPointer<S> &p) : m_pointer(NULL) {
  ...
  /**
    ...
    */
  template <class S> ReferenceCountedPointer<S> downcast() {
    return ReferenceCountedPointer<S>(dynamic_cast<S *>(m_pointer));
  }

  template <class S> const ReferenceCountedPointer<S> downcast() const {
  ...
  inline ReferenceCountedPointer(const ReferenceCountedPointer<T> &p) : m_pointer(NULL) { setPointer(p.m_pointer); }
  ...
  inline bool operator==(const ReferenceCountedPointer<T> &y) const { return (m_pointer == y.m_pointer); }

  inline bool operator!=(const ReferenceCountedPointer<T> &y) const { return (m_pointer != y.m_pointer); }

  bool operator<(const ReferenceCountedPointer<T> &y) const { return (m_pointer < y.m_pointer); }
  ...
  inline bool isNull() const { return (m_pointer == NULL); }

  inline bool notNull() const { return (m_pointer != NULL); }
   ...
   */
  inline int isLastReference() const { return (m_pointer->ReferenceCountedObject_refCount.value() == 1); }
};
 ...
 */
template <class T> class WeakReferenceCountedPointer : public _WeakPtr {
private:
...
private:
  ...
  void setPointer(T *p) {
    ...
    if (pointer != NULL) {
      ...
      _WeakPtrLinkedList *head = new _WeakPtrLinkedList(this, pointer->ReferenceCountedObject_weakPointer);
  ...
  void zeroPointer() {
    ...
    if (strong.notNull()) {
      ...
      while ((*node)->weakPtr != this) {
        node = &((*node)->next);
        debugAssertM(*node != NULL, "Weak pointer exists without a backpointer from the object (2).");
      }
...
public:
  ...
  /**
    ...
    RCP&lt;<I>T</I>&gt; &lt;: RCP&lt;<I>S</I>&gt; if <I>T</I> &lt;: <I>S</I>
   */
  template <class S> inline WeakReferenceCountedPointer(const WeakReferenceCountedPointer<S> &p) : pointer(0) {
    // Threadsafe: the object cannot be collected while the other pointer exists.
    setPointer(p.pointer);
  }

  template <class S> inline WeakReferenceCountedPointer(const ReferenceCountedPointer<S> &p) : pointer(0) {
  ...
  WeakReferenceCountedPointer(const WeakReferenceCountedPointer<T> &weakPtr) : pointer(0) {
    setPointer(weakPtr.pointer);
  }

  WeakReferenceCountedPointer(const ReferenceCountedPointer<T> &strongPtr) : pointer(0) {
    setPointer(strongPtr.pointer());
  }

  ~WeakReferenceCountedPointer() { zeroPointer(); }
  ...
  bool operator==(const WeakReferenceCountedPointer<T> &other) const { return pointer == other.pointer; }

  bool operator!=(const WeakReferenceCountedPointer<T> &other) const { return pointer != other.pointer; }

  bool operator<(const WeakReferenceCountedPointer<T> &y) const { return (pointer < y.pointer); }

  bool operator>(const WeakReferenceCountedPointer<T> &y) const { return (pointer > y.pointer); }

  bool operator<=(const WeakReferenceCountedPointer<T> &y) const { return (pointer <= y.pointer); }

  bool operator>=(const ReferenceCountedPointer<T> &y) const { return (pointer >= y.pointer); }

protected:
  /** Invoked by the destructor on ReferenceCountedPointer. */
  void objectCollected() {
    debugAssertM(pointer != NULL, "Removed a weak pointer twice.");
    pointer = NULL;
  }
};
