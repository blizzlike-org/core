...
void Any::beforeRead() const {
  if (isPlaceholder()) {
    ...
    e.message = "This exception may have been thrown later than "
...
Any &Any::operator=(int x) { return (*this = Any(x)); }
...
bool Any::operator==(const Any &x) const {
  ...
  case TABLE: {
    ...
    for (Table<std::string, Any>::Iterator it1 = cmptable.begin(), it2 = xcmptable.begin();
         it1 != cmptable.end() && it2 != xcmptable.end(); ++it1, ++it2) {
      if (*it1 != *it2) {
        return false;
      }
    }
  ...
  while (!((token.type() == Token::SYMBOL) && (token.string()[0] == closeSymbol))) {
    ...
    if (m_type == TABLE) {
      ...
      if ((token.type() != Token::SYMBOL) || (token.string() != "=")) {
        ...
        // Read the next token, which is the value (don't consume comments--we want the value pointed to by a to get
        // those).
        token = ti.read();
      }
    }
...
void Any::verifyName(const std::string &n, const std::string &m) const {
  ...
  verify(beginsWith(x, toUpper(n)) || beginsWith(x, toUpper(m)), "Name must begin with " + n + " or " + m);
}
