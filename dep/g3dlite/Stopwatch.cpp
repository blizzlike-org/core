...
Stopwatch::Stopwatch(const std::string &myName)
    : myName(myName), inBetween(false), lastTockTime(-1), lastDuration(0), lastCycleCount(0), m_fps(0), emwaFPS(0),
...
void Stopwatch::after(const std::string &s) {
  RealTime now = System::time();
  debugPrintf("%s: %10s - %8fs since %s (%fs since start)\n", myName.c_str(), s.c_str(), now - prevTime,
              prevMark.c_str(), now - startTime);
  prevTime = now;
  prevMark = s;
}
