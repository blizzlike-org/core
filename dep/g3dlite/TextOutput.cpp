...
TextOutput::TextOutput(const TextOutput::Settings &opt)
    : startingNewLine(true), currentColumn(0), inDQuote(false), filename(""), indentLevel(0) {
  setOptions(opt);
}

TextOutput::TextOutput(const std::string &fil, const TextOutput::Settings &opt)
    : startingNewLine(true), currentColumn(0), inDQuote(false), filename(fil), indentLevel(0) {
...
void TextOutput::writeNumber(double n) { this->printf("%f ", n); }
...
void TextOutput::writeSymbols(const std::string &a, const std::string &b, const std::string &c, const std::string &d,
...
void TextOutput::wordWrapIndentAppend(const std::string &str) {
  ...
  for (uint32 i = 0; i < str.size(); ++i) {
    ...
    if (currentColumn >= cols) {
      ...
      while ((k < maxLookBackward) && (lastSpace > 0) && (!((data[lastSpace] == ' ') && unquotedSpace))) {
        --lastSpace;
        ++k;
      ...
      if (k == maxLookBackward) {
        ...
        while ((k < maxLookBackward) && (firstSpace > 0) && (data[firstSpace] == ' ')) {
          --firstSpace;
          ++k;
        }
