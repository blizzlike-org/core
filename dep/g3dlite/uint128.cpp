...
uint128 &uint128::operator*=(const uint128 &x) {
  ...
  // Overflow doesn't matter here because the result is going into hi - any overflow will exceed the capacity of a
  // 128-bit number Note: hi * x.hi will always overflow, since (x * 2^64) * (y * 2^64) = x*y*(2^128). The largest
  // number expressable in 128 bits is 2^128 - 1.
