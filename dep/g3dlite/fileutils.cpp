  ...
  if (zipfileExists(file, zip, desiredFile)) {
    struct zip *z = zip_open(zip.c_str(), ZIP_CHECKCONS, NULL);
    {
      ...
      struct zip_file *zf = zip_fopen(z, desiredFile.c_str(), ZIP_FL_NOCASE);
      {
        ...
        debugAssertM((size_t)test == length, desiredFile + " was corrupt because it unzipped to the wrong size.");
        (void)test;
      }
      zip_fclose(zf);
    }
...
void zipClose(void *data) { System::alignedFree(data); }
...
static void _zip_resolveDirectory(std::string &completeDir, const std::string &drive, const Array<std::string> &path,
                                  const int length) {
  completeDir = drive;
  int tempLength;
...
void parseFilename(const std::string &filename, std::string &root, Array<std::string> &path, std::string &base,
                   std::string &ext) {
 ...
 */
static void getFileOrDirListNormal(const std::string &filespec, Array<std::string> &files, bool wantFiles,
                                   bool includePath) {
 ...
 */
static void _zip_addEntry(const std::string &path, const std::string &prefix, const std::string &file,
                          Set<std::string> &files, bool wantFiles, bool includePath) {
...
static void getFileOrDirListZip(const std::string &path, const std::string &prefix, Array<std::string> &files,
                                bool wantFiles, bool includePath) {
#if _HAVE_ZIP /* G3DFIX: Use ZIP-library only if defined */
...
static void determineFileOrDirList(const std::string &filespec, Array<std::string> &files, bool wantFiles,
                                   bool includePath) {
