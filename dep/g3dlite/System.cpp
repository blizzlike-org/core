...
System::System()
    : m_initialized(false), m_cpuSpeed(0), m_hasCPUID(false), m_hasRDTSC(false), m_hasMMX(false), m_hasSSE(false),
      m_hasSSE2(false), m_hasSSE3(false), m_has3DNOW(false), m_has3DNOW2(false), m_hasAMDMMX(false),
      m_cpuVendor("Uninitialized"), m_numCores(1), m_machineEndian(G3D_LITTLE_ENDIAN), m_cpuArch("Uninitialized"),
      m_operatingSystem("Uninitialized"), m_version("Uninitialized"), m_outOfMemoryCallback(NULL),
      m_realWorldGetTickTime0(0), m_highestCPUIDFunction(0) {
...
void System::init() {
  ...
  bool success = RegistryUtil::readInt32("HKEY_LOCAL_MACHINE\\HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",
                                         "~MHz", m_cpuSpeed);
  ...
  {
    char c[1024];
    sprintf(c, "%d x %d-bit %s processor", systemInfo.dwNumberOfProcessors,
            (int)(::log((double)maxAddr) / ::log(2.0) + 2.0), arch);
    m_cpuArch = c;
  }
  ...
  if (success) {
    char c[1000];
    sprintf(c, "Windows %d.%d build %d Platform %d %s", osVersionInfo.dwMajorVersion, osVersionInfo.dwMinorVersion,
            osVersionInfo.dwBuildNumber, osVersionInfo.dwPlatformId, osVersionInfo.szCSDVersion);
    m_operatingSystem = c;
  } else {
...
std::string System::findDataFile(const std::string &full, bool errorIfNotFound) {
  ...
  if (directoryArray.size() == 0) {
    ...
    static const std::string subdirs[] = {"font", "gui", "SuperShader", "cubemap", "icon", "material", "image",
                                          "md2",  "md3", "ifs",         "3ds",     "sky",  ""};
...
void System::setAppDataDir(const std::string &path) { instance().m_appDataDir = path; }
...
RealTime System::time() {
  ...
  return ((RealTime)(now.QuadPart - instance().m_start.QuadPart) / instance().m_counterFrequency.QuadPart) +
  ...
  return (now.tv_sec - instance().m_start.tv_sec) + (now.tv_usec - instance().m_start.tv_usec) / 1e6 +
         instance().m_realWorldGetTickTime0;
#endif
}
...
private:
  /** Pointer given to the program.  Unless in the tiny heap, the user size of the block is stored right in front of the
   * pointer as a uint32.*/
...
public:
  ...
  std::string performance() const {
    if (totalMallocs > 0) {
      ...
      return format("malloc performance: %5.1f%% <= %db, %5.1f%% <= %db, "
                    "%5.1f%% <= %db, %5.1f%% > %db",
                    100.0 * mallocsFromTinyPool / total, BufferPool::tinyBufferSize,
                    100.0 * mallocsFromSmallPool / total, BufferPool::smallBufferSize,
                    100.0 * mallocsFromMedPool / total, BufferPool::medBufferSize,
                    100.0 * (1.0 - (double)pooled / total), BufferPool::medBufferSize);
    } else {
  ...
  std::string status() const {
    return format("preallocated shared buffers: %5d/%d x %db", maxTinyBuffers - tinyPoolSize, maxTinyBuffers,
                  tinyBufferSize);
  }
};
