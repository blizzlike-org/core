...
static bool needSwapBytes(G3DEndian fileEndian) { return (fileEndian != System::machineEndian()); }
...
BinaryInput::BinaryInput(const uint8 *data, int64 dataLen, G3DEndian dataEndian, bool compressed, bool copyMemory)
    : m_filename("<memory>"), m_bitPos(0), m_bitString(0), m_beginEndBits(0), m_alreadyRead(0), m_bufferLength(0),
      m_pos(0) {
...
BinaryInput::BinaryInput(const std::string &filename, G3DEndian fileEndian, bool compressed)
    : m_filename(filename), m_bitPos(0), m_bitString(0), m_beginEndBits(0), m_alreadyRead(0), m_length(0),
      m_bufferLength(0), m_buffer(NULL), m_pos(0), m_freeBuffer(true) {
  ...
  if (FileSystem::inZipfile(m_filename, zipfile)) {
    ...
    struct zip *z = zip_open(zipfile.c_str(), ZIP_CHECKCONS, NULL);
    {
      ...
      struct zip_file *zf = zip_fopen(z, internalFile.c_str(), ZIP_FL_NOCASE);
      {
        ...
        debugAssertM(test == m_length, internalFile + " was corrupt because it unzipped to the wrong size.");
        (void)test;
      }
      zip_fclose(zf);
    }
