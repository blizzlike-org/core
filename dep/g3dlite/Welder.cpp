...
private:
  ...
  int getIndex(const Vector3 &v, const Vector3 &n, const Vector2 &t) {
    PointHashGrid<VNTi>::SphereIterator it = weldGrid.beginSphereIntersection(Sphere(v, vertexWeldRadius));
  ...
  /**
   ...
   Called from process()
   */
  void updateTriLists(Array<Array<int> *> &indexArrayArray, const Array<Vector3> &vertexArray,
                      const Array<Vector3> &normalArray, const Array<Vector2> &texCoordArray) {
...
#           endif
  ...
  /** Expands the indexed triangle lists into a triangle list.
      ...
      Called from process() */
  void unroll(const Array<Array<int> *> &indexArrayArray, const Array<Vector3> &vertexArray,
              const Array<Vector2> &texCoordArray, Array<Vector3> &unrolledVertexArray,
   ...
   */
  void smoothNormals(const Array<Vector3> &vertexArray, const Array<Vector3> &normalArray,
    ...
    for (int v = 0; v < normalArray.size(); ++v) {
      ...
      PointHashGrid<VN>::SphereIterator it = grid.beginSphereIntersection(Sphere(vertexArray[v], vertexWeldRadius));
...
public:
   ...
   */
  void process(Array<Vector3> &vertexArray, Array<Vector2> &texCoordArray, Array<Vector3> &normalArray,
               Array<Array<int> *> &indexArrayArray, float normAngle, float texRadius, float normRadius) {
  ...
  WeldHelper(float vertRadius) : weldGrid(vertRadius, AreaMemoryManager::create()), vertexWeldRadius(vertRadius) {}
};
} // namespace _internal

void Welder::weld(Array<Vector3> &vertexArray, Array<Vector2> &texCoordArray, Array<Vector3> &normalArray,
                  Array<Array<int> *> &indexArrayArray, const Welder::Settings &settings) {

  _internal::WeldHelper(settings.vertexWeldRadius)
      .process(vertexArray, texCoordArray, normalArray, indexArrayArray, settings.normalSmoothingAngle,
               settings.textureWeldRadius, settings.normalWeldRadius);
}
