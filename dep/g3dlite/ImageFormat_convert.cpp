...
typedef void (*ConvertFunc)(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
...
#define DECLARE_CONVERT_FUNC(name)                                                                                     \
  static void name(const Array<const void *> &srcBytes, int srcWidth, int srcHeight, const ImageFormat *srcFormat,     \
                   int srcRowPadBits, const Array<void *> &dstBytes, const ImageFormat *dstFormat, int dstRowPadBits,  \
...
static ConvertFunc findConverter(TextureFormat::Code sourceCode, TextureFormat::Code destCode, bool needsSourcePadding,
                                 bool needsDestPadding, bool needsInvertY) {
...
bool conversionAvailable(const ImageFormat *srcFormat, int srcRowPadBits, const ImageFormat *dstFormat,
                         int dstRowPadBits, bool invertY = false) {
  ...
  if ((srcFormat->code == dstFormat->code) && (srcRowPadBits == dstRowPadBits) && !invertY) {
    ...
    ConvertFunc directConverter =
        findConverter(srcFormat->code, dstFormat->code, srcRowPadBits > 0, dstRowPadBits > 0, invertY);
...
bool ImageFormat::convert(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                          const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                          const ImageFormat *dstFormat, int dstRowPadBits, bool invertY, BayerAlgorithm bayerAlg) {
  ...
  if ((srcFormat->code == dstFormat->code) && (srcRowPadBits == dstRowPadBits) && !invertY) {
    ...
    System::memcpy(dstBytes[0], srcBytes[0],
                   iCeil(((srcWidth * srcFormat->cpuBitsPerPixel + srcRowPadBits) * srcHeight) / 8.0f));
    conversionAvailable = true;
  } else {
    ...
    if (directConverter) {
      directConverter(srcBytes, srcWidth, srcHeight, srcFormat, srcRowPadBits, dstBytes, dstFormat, dstRowPadBits,
                      invertY, bayerAlg);
      conversionAvailable = true;
    } else {
      ConvertFunc toInterConverter =
          findConverter(srcFormat->code, ImageFormat::CODE_RGBA32F, srcRowPadBits > 0, false, false);
      ;
      ConvertFunc fromInterConverter =
          findConverter(ImageFormat::CODE_RGBA32F, dstFormat->code, false, dstRowPadBits > 0, invertY);
      ;
      ...
      if (toInterConverter && fromInterConverter) {
        ...
        toInterConverter(srcBytes, srcWidth, srcHeight, srcFormat, srcRowPadBits, tmp, ImageFormat::RGBA32F(), 0, false,
                         bayerAlg);
        fromInterConverter(reinterpret_cast<Array<const void *> &>(tmp), srcWidth, srcHeight, ImageFormat::RGBA32F(), 0,
                           dstBytes, dstFormat, dstRowPadBits, invertY, bayerAlg);
...
// L8 ->
static void l8_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight, const ImageFormat *srcFormat,
                       int srcRowPadBits, const Array<void *> &dstBytes, const ImageFormat *dstFormat,
                       int dstRowPadBits, bool invertY, ImageFormat::BayerAlgorithm bayerAlg) {
  (void)bayerAlg;
  (void)dstRowPadBits;
...
// L32F ->
static void l32f_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight, const ImageFormat *srcFormat,
                         int srcRowPadBits, const Array<void *> &dstBytes, const ImageFormat *dstFormat,
                         int dstRowPadBits, bool invertY, ImageFormat::BayerAlgorithm bayerAlg) {
  int srcIndex = 0;
  int dstByteOffset = 0;
...
// RGB8 ->
static void rgb8_to_rgba8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                          const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                          const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                          ImageFormat::BayerAlgorithm bayerAlg) {
  uint8 *dst = static_cast<uint8 *>(dstBytes[0]);
...
static void rgb8_to_bgr8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight, const ImageFormat *srcFormat,
                         int srcRowPadBits, const Array<void *> &dstBytes, const ImageFormat *dstFormat,
                         int dstRowPadBits, bool invertY, ImageFormat::BayerAlgorithm bayerAlg) {
  uint8 *dst = static_cast<uint8 *>(dstBytes[0]);
  const uint8 *src = static_cast<const uint8 *>(srcBytes[0]);
...
static void rgb8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                            ImageFormat::BayerAlgorithm bayerAlg) {
...
// BGR8 ->
static void bgr8_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight, const ImageFormat *srcFormat,
                         int srcRowPadBits, const Array<void *> &dstBytes, const ImageFormat *dstFormat,
                         int dstRowPadBits, bool invertY, ImageFormat::BayerAlgorithm bayerAlg) {
  uint8 *dst = static_cast<uint8 *>(dstBytes[0]);
  const uint8 *src = static_cast<const uint8 *>(srcBytes[0]);
...
static void bgr8_to_rgba8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                          const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                          const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                          ImageFormat::BayerAlgorithm bayerAlg) {
  uint8 *dst = static_cast<uint8 *>(dstBytes[0]);
...
static void bgr8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                            ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits % 8 == 0, "Source row padding must be a multiple of 8 bits for this format");
...
// RGBA8 ->
static void rgba8_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                          const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                          const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                          ImageFormat::BayerAlgorithm bayerAlg) {
  uint8 *dst = static_cast<uint8 *>(dstBytes[0]);
...
static void rgba8_to_bgr8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                          const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                          const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                          ImageFormat::BayerAlgorithm bayerAlg) {
  uint8 *dst = static_cast<uint8 *>(dstBytes[0]);
...
static void rgba8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                             const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                             const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                             ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits % 8 == 0, "Source row padding must be a multiple of 8 bits for this format");
...
// RGB32F ->
static void rgb32f_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                              const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                              const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                              ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits % 8 == 0, "Source row padding must be a multiple of 8 bits for this format");
...
// RGBA32F ->
static void rgba32f_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                            ImageFormat::BayerAlgorithm bayerAlg) {
...
static void rgba32f_to_rgba8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                             const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                             const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                             ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(dstRowPadBits % 8 == 0, "Destination row padding must be a multiple of 8 bits for this format");
...
static void rgba32f_to_bgr8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                            ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(dstRowPadBits % 8 == 0, "Destination row padding must be a multiple of 8 bits for this format");
...
static void rgba32f_to_rgb32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                              const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                              const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                              ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(dstRowPadBits % 8 == 0, "Destination row padding must be a multiple of 8 bits for this format");
...
static void rgb8_to_yuv420p(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                            ImageFormat::BayerAlgorithm bayerAlg) {
...
static void rgb8_to_yuv422(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                           const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                           const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                           ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits == 0, "Source row padding must be 0 for this format");
...
static void rgb8_to_yuv444(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                           const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                           const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                           ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits == 0, "Source row padding must be 0 for this format");
...
#define PIXEL_YUV_TO_RGB8_G(y, u, v)                                                                                   \
  static_cast<uint8>(iClamp((298 * (y - 16) - 100 * (u - 128) - 208 * (v - 128) + 128) >> 8, 0, 255))
#define PIXEL_YUV_TO_RGB8_B(y, u, v) static_cast<uint8>(iClamp((298 * (y - 16) + 516 * (u - 128) + 128) >> 8, 0, 255))

static void yuv420p_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                            const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                            const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                            ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits == 0, "Source row padding must be 0 for this format");
...
static void yuv422_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                           const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                           const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                           ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits == 0, "Source row padding must be 0 for this format");
...
static void yuv444_to_rgb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                           const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                           const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                           ImageFormat::BayerAlgorithm bayerAlg) {
  debugAssertM(srcRowPadBits == 0, "Source row padding must be 0 for this format");
...
static const float R_BGB[5][5] = {{0.0f, 0.0f, -3.0f / 2.0f, 0.0f, 0.0f},
...
// rgb8 --> bayer helpers
// =====================================================================
static void rgb8_to_bayer_rggb8(const int w, const int h, const uint8 *src, uint8 *dst) {
...
static void rgb8_to_bayer_grbg8(const int w, const int h, const uint8 *src, uint8 *dst) {
  Color3uint8 *srcColor = (Color3uint8 *)src;
  Color1uint8 *dstColor = (Color1uint8 *)dst;
...
static void rgb8_to_bayer_bggr8(const int w, const int h, const uint8 *src, uint8 *dst) {
  Color3uint8 *srcColor = (Color3uint8 *)src;
  Color1uint8 *dstColor = (Color1uint8 *)dst;
...
static void rgb8_to_bayer_gbrg8(const int w, const int h, const uint8 *src, uint8 *dst) {
  Color3uint8 *srcColor = (Color3uint8 *)src;
  Color1uint8 *dstColor = (Color1uint8 *)dst;
...
static void rgba32f_to_bayer_rggb8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
  ...
  rgba32f_to_rgb8(srcBytes, srcWidth, srcHeight, ImageFormat::RGBA32F(), 0, tmp, ImageFormat::RGB8(), 0, invertY,
                  bayerAlg);
...
static void rgba32f_to_bayer_gbrg8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                                   const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                                   const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                                   ImageFormat::BayerAlgorithm bayerAlg) {
  Array<void *> tmp;
...
static void rgba32f_to_bayer_grbg8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                                   const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                                   const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                                   ImageFormat::BayerAlgorithm bayerAlg) {
  Array<void *> tmp;
...
static void rgba32f_to_bayer_bggr8(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                                   const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                                   const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                                   ImageFormat::BayerAlgorithm bayerAlg) {
  Array<void *> tmp;
...
// bayer --> rgb8 helpers
// =====================================================================
static void bayer_rggb8_to_rgb8_mhc(int w, int h, const uint8 *in, uint8 *_out) {
  debugAssert(in != _out);
...
static void bayer_gbrg8_to_rgb8_mhc(int w, int h, const uint8 *in, uint8 *_out) {
...
static void bayer_grbg8_to_rgb8_mhc(int w, int h, const uint8 *in, uint8 *_out) {
...
static void bayer_bggr8_to_rgb8_mhc(int w, int h, const uint8 *in, uint8 *_out) {
  // Run the equivalent function for red
...
static void bayer_rggb8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
  ...
  rgb8_to_rgba32f(reinterpret_cast<Array<const void *> &>(tmp), srcWidth, srcHeight, ImageFormat::RGB8(), 0, dstBytes,
                  ImageFormat::RGBA32F(), 0, invertY, bayerAlg);

  System::free(tmp[0]);
}

static void bayer_gbrg8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                                   const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                                   const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                                   ImageFormat::BayerAlgorithm bayerAlg) {
  Array<void *> tmp;
...
static void bayer_grbg8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                                   const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                                   const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                                   ImageFormat::BayerAlgorithm bayerAlg) {
  Array<void *> tmp;
...
static void bayer_bggr8_to_rgba32f(const Array<const void *> &srcBytes, int srcWidth, int srcHeight,
                                   const ImageFormat *srcFormat, int srcRowPadBits, const Array<void *> &dstBytes,
                                   const ImageFormat *dstFormat, int dstRowPadBits, bool invertY,
                                   ImageFormat::BayerAlgorithm bayerAlg) {
  Array<void *> tmp;
