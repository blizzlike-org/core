...
#define setMany(i0, i1, i2, i3, field, extreme)                                                                        \
  _corner[i0].field = _corner[i1].field = _corner[i2].field = _corner[i3].field = (extreme).field

Box::Box() {}

Box::Box(const AABox &b) { init(b.low(), b.high()); }
...
void Box::getLocalFrame(CoordinateFrame &frame) const {
  ...
  frame.rotation = Matrix3(_axis[0][0], _axis[1][0], _axis[2][0], _axis[0][1], _axis[1][1], _axis[2][1], _axis[0][2],
                           _axis[1][2], _axis[2][2]);
...
bool Box::contains(const Vector3 &point) const {
  ...
  return (osPoint.x >= 0) && (osPoint.y >= 0) && (osPoint.z >= 0) && (osPoint.x <= 1) && (osPoint.y <= 1) &&
         (osPoint.z <= 1);
}
...
void Box::getRandomSurfacePoint(Vector3 &P, Vector3 &N) const {
  ...
  if (r < aXY) {
    P = _axis[0] * (float)uniformRandom(-0.5, 0.5) * _extent.x +
        _axis[1] * (float)uniformRandom(-0.5, 0.5) * _extent.y + _center + _axis[2] * d * _extent.z * 0.5f;
    N = _axis[2] * d;
  } else if (r < aYZ) {
    P = _axis[1] * (float)uniformRandom(-0.5, 0.5) * _extent.y +
        _axis[2] * (float)uniformRandom(-0.5, 0.5) * _extent.z + _center + _axis[0] * d * _extent.x * 0.5f;
    N = _axis[0] * d;
  } else {
    P = _axis[2] * (float)uniformRandom(-0.5, 0.5) * _extent.z +
        _axis[0] * (float)uniformRandom(-0.5, 0.5) * _extent.x + _center + _axis[1] * d * _extent.y * 0.5f;
    N = _axis[1] * d;
  }
}
