...
std::string NetworkDevice::formatIP(uint32 addr) {
  return format("%3d.%3d.%3d.%3d", (addr >> 24) & 0xFF, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF, addr & 0xFF);
}
...
bool NetworkDevice::init() {
  ...
  logPrintf("Network:\n"
            ...
            wsda.szDescription, wsda.szSystemStatus, wsda.wVersion, wsda.wHighVersion, wsda.iMaxSockets,
            wsda.iMaxUdpDg);
...
ReliableConduitRef ReliableConduit::create(const NetAddress &address) { return new ReliableConduit(address); }

ReliableConduit::ReliableConduit(const NetAddress &_addr)
    : state(NO_MESSAGE), receiveBuffer(NULL), receiveBufferTotalSize(0), receiveBufferUsedSize(0) {
...
///////////////////////////////////////////////////////////////////////////////
LightweightConduitRef LightweightConduit::create(uint16 receivePort, bool enableReceive, bool enableBroadcast) {
...
void LightweightConduit::sendBuffer(const NetAddress &a, BinaryOutput &b) {
  NetworkDevice *nd = NetworkDevice::instance();
  if (sendto(sock, (const char *)b.getCArray(), b.size(), 0, (struct sockaddr *)&(a.addr), sizeof(a.addr)) ==
      SOCKET_ERROR) {
...
uint32 LightweightConduit::waitingMessageType() {
  ...
  if (!alreadyReadMessage) {
    ...
    int ret = recvfrom(sock, (char *)messageBuffer.getCArray(), messageBuffer.size(), 0,
                       (struct sockaddr *)&remote_addr, (socklen_t *)&iRemoteAddrLen);
...
NetListenerRef NetListener::create(const uint16 port) { return new NetListener(port); }
...
#ifndef G3D_WIN32
  return new ReliableConduit(sClient, NetAddress(htonl(remote_addr.sin_addr.s_addr), ntohs(remote_addr.sin_port)));
#else
  return new ReliableConduit(sClient, NetAddress(ntohl(remote_addr.sin_addr.S_un.S_addr), ntohs(remote_addr.sin_port)));
#endif
}
