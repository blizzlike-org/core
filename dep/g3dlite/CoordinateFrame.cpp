...
std::string CoordinateFrame::toXYZYPRDegreesString() const {
  ...
  return format("CFrame::fromXYZYPRDegrees(% 5.1ff, % 5.1ff, % 5.1ff, % 5.1ff, % 5.1ff, % 5.1ff)", uframe.translation.x,
                uframe.translation.y, uframe.translation.z, toDegrees(uframe.yaw), toDegrees(uframe.pitch), 0.0f);
}
...
CoordinateFrame::CoordinateFrame(const Any &any) {
  *this = CFrame();
  ...
  if (beginsWith(n, "VECTOR3")) {
    ...
    *this = fromXYZYPRDegrees(any[0], any[1], any[2], (s > 3) ? any[3].number() : 0.0f,
                              (s > 4) ? any[4].number() : 0.0f, (s > 5) ? any[5].number() : 0.0f);
  }
}
...
std::string CoordinateFrame::toXML() const {
  return G3D::format("<COORDINATEFRAME>\n  %lf,%lf,%lf,%lf,\n  %lf,%lf,%lf,%lf,\n  %lf,%lf,%lf,%lf,\n  "
                     "%lf,%lf,%lf,%lf\n</COORDINATEFRAME>\n",
                     rotation[0][0], rotation[0][1], rotation[0][2], translation.x, rotation[1][0], rotation[1][1],
                     rotation[1][2], translation.y, rotation[2][0], rotation[2][1], rotation[2][2], translation.z, 0.0,
                     0.0, 0.0, 1.0);
}
