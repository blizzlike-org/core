...
void FileSystem::listHelper(const std::string &shortSpec, const std::string &parentPath, Array<std::string> &result,
  ...
  for (int i = 0; i < dir.nodeArray.size(); ++i) {
    ...
    if (FilePath::matches(entry.name, shortSpec, settings.caseSensitive)) {
      ...
      if ((settings.files && settings.directories) || (settings.files && (entry.type == FILE_TYPE)) ||
...
void FilePath::parse(const std::string &filename, std::string &root, Array<std::string> &path, std::string &base,
                     std::string &ext) {
