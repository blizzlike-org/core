...
AnyVal::AnyVal(G3D::TextInput &t) : m_type(NIL), m_value(NULL), m_referenceCount(NULL) { deserialize(t); }
...
AnyVal::AnyVal(double v) : m_type(NUMBER), m_referenceCount(NULL) { m_value = new double(v); }

AnyVal::AnyVal(int v) : m_type(NUMBER), m_referenceCount(NULL) { m_value = new double(v); }

AnyVal::AnyVal(const Rect2D &v) : m_type(RECT2D), m_referenceCount(NULL) { m_value = new Rect2D(v); }

AnyVal::AnyVal(const AABox &v) : m_type(AABOX), m_referenceCount(NULL) { m_value = new AABox(v); }

AnyVal::AnyVal(const Vector2 &v) : m_type(VECTOR2), m_referenceCount(NULL) { m_value = new Vector2(v); }

AnyVal::AnyVal(const Vector3 &v) : m_type(VECTOR3), m_referenceCount(NULL) { m_value = new Vector3(v); }

AnyVal::AnyVal(const Vector4 &v) : m_type(VECTOR4), m_referenceCount(NULL) { m_value = new Vector4(v); }

AnyVal::AnyVal(const Color1 &v) : m_type(COLOR1), m_referenceCount(NULL) { m_value = new Color1(v); }

AnyVal::AnyVal(const Color3 &v) : m_type(COLOR3), m_referenceCount(NULL) { m_value = new Color3(v); }

AnyVal::AnyVal(const Color4 &v) : m_type(COLOR4), m_referenceCount(NULL) { m_value = new Color4(v); }

AnyVal::AnyVal(const std::string &v) : m_type(STRING), m_referenceCount(NULL) { m_value = new std::string(v); }

AnyVal::AnyVal(const char *v) : m_type(STRING), m_referenceCount(NULL) { m_value = new std::string(v); }

AnyVal::AnyVal(const Quat &v) : m_type(QUAT), m_referenceCount(NULL) { m_value = new Quat(v); }

AnyVal::AnyVal(const CoordinateFrame &v) : m_type(COORDINATEFRAME), m_referenceCount(NULL) {
  m_value = new CoordinateFrame(v);
}

AnyVal::AnyVal(const Matrix2 &v) : m_type(MATRIX2), m_referenceCount(NULL) { m_value = new Matrix2(v); }

AnyVal::AnyVal(const Matrix3 &v) : m_type(MATRIX3), m_referenceCount(NULL) { m_value = new Matrix3(v); }

AnyVal::AnyVal(const Matrix4 &v) : m_type(MATRIX4), m_referenceCount(NULL) { m_value = new Matrix4(v); }

AnyVal::AnyVal(const AnyVal &c) : m_type(NIL), m_value(NULL), m_referenceCount(NULL) { *this = c; }
...
void AnyVal::serialize(G3D::TextOutput &t) const {
  ...
  case RECT2D:
    t.printf("R(%g, %g, %g, %g)", ((Rect2D *)m_value)->x0(), ((Rect2D *)m_value)->y0(), ((Rect2D *)m_value)->width(),
             ((Rect2D *)m_value)->height());
    break;

  case AABOX:
    t.printf("AAB(V3(%g, %g, %g), V3(%g, %g, %g))", aabox().low().x, aabox().low().y, aabox().low().z, aabox().high().x,
             aabox().high().y, aabox().high().z);
    break;
  ...
  case VECTOR4:
    t.printf("V4(%g, %g, %g, %g)", ((Vector4 *)m_value)->x, ((Vector4 *)m_value)->y, ((Vector4 *)m_value)->z,
             ((Vector4 *)m_value)->w);
    break;
  ...
  case MATRIX2: {
    ...
    t.printf("M2(\n");
    t.pushIndent();
    t.printf("%10.5f, %10.5f,\n%10.5f, %10.5f)", m[0][0], m[0][1], m[1][0], m[1][1]);
    t.popIndent();
  } break;
  ...
  case MATRIX3: {
    ...
    t.printf("M3(\n");
    t.pushIndent();
    t.printf("%10.5f, %10.5f, %10.5f,\n%10.5f, %10.5f, %10.5f,\n%10.5f, %10.5f, %10.5f)", m[0][0], m[0][1], m[0][2],
  ...
  case COLOR4:
    t.printf("C4(%g, %g, %g, %g)", ((Color4 *)m_value)->r, ((Color4 *)m_value)->g, ((Color4 *)m_value)->b,
             ((Color4 *)m_value)->a);
    break;
...
AnyVal &AnyVal::operator[](const char *key) { return this->operator[]((std::string)key); }

const AnyVal &AnyVal::operator[](const char *key) const { return this->operator[]((std::string)key); }
