...
void GImage::encodePNG(BinaryOutput &out) const {
  ...
  switch (m_channels) {
  case 1:
    png_set_IHDR(png_ptr, info_ptr, m_width, m_height, 8, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE,
  ...
  case 4:
    png_set_IHDR(png_ptr, info_ptr, m_width, m_height, 8, PNG_COLOR_TYPE_RGBA, PNG_INTERLACE_NONE,
                 PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
    sig_bit.red = 8;
    sig_bit.green = 8;
    sig_bit.blue = 8;
...
void GImage::decodePNG(BinaryInput &input) {
  ...
  if ((color_type == PNG_COLOR_TYPE_RGBA) || ((color_type == PNG_COLOR_TYPE_PALETTE) && (png_ptr->num_trans > 0))) {
