...
void MeshBuilder::commit(std::string &n, Array<int> &indexArray, Array<Vector3> &outvertexArray) {
  ...
  if (close == MeshBuilder::AUTO_WELD) {
    ...
    MeshAlg::computeAreaStatistics(triList, index, minEdgeLen, meanEdgeLen, medianEdgeLen, maxEdgeLen, minFaceArea,
                                   meanFaceArea, medianFaceArea, maxFaceArea);
...
void MeshBuilder::centerTriList() {
  ...
  // Matrix3 rot90 = Matrix3::fromAxisAngle(Vector3::UNIT_Y, toRadians(180)) * Matrix3::fromAxisAngle(Vector3::UNIT_X,
  // toRadians(90));
