...
void GCamera::getProjectPixelMatrix(const Rect2D &viewport, Matrix4 &P) const {
  ...
  P = Matrix4(sx, 0, 0, sx + viewport.x0() - m_pixelOffset.x, 0, -sy, 0, sy + viewport.y0() + m_pixelOffset.y, 0, 0, 1,
              0, 0, 0, 0, 1) *
      P;
}
...
void GCamera::frustum(const Rect2D &viewport, Frustum &fr) const {
  ...
  if (m_farPlaneZ == -finf()) {
    ...
    // Finite
    const float s = zf / zn;
    fr.vertexPos.append(Vector4(x * s, y * s, zf, 1), Vector4(-x * s, y * s, zf, 1), Vector4(-x * s, -y * s, zf, 1),
  ...
  // Left plane
  face.plane =
      Plane(Vector3(-fr.faceArray.last().plane.normal().x, 0, fr.faceArray.last().plane.normal().z), Vector3::zero());
  ...
  // Bottom plane
  face.plane =
      Plane(Vector3(0, -fr.faceArray.last().plane.normal().y, fr.faceArray.last().plane.normal().z), Vector3::zero());
...
void GCamera::getNearViewportCorners(const Rect2D &viewport, Vector3 &outUR, Vector3 &outUL, Vector3 &outLL,
                                     Vector3 &outLR) const {
...
void GCamera::getFarViewportCorners(const Rect2D &viewport, Vector3 &outUR, Vector3 &outUL, Vector3 &outLL,
                                    Vector3 &outLR) const {
...
void GCamera::lookAt(const Vector3 &position, const Vector3 &up) { m_cframe.lookAt(position, up); }
