...
Vector3 CollisionDetection::separatingAxisForSolidBoxSolidBox(const int separatingAxisIndex, const Box &box1,
                                                              const Box &box2) {
...
float CollisionDetection::projectedDistanceForSolidBoxSolidBox(const int separatingAxisIndex, const Vector3 &a,
                                                               const Vector3 &b, const Vector3 &D, const double *c,
                                                               const double *ca, const double *ad, const double *bd) {
  (void)D;
...
bool CollisionDetection::parallelAxisForSolidBoxSolidBox(const double *ca, const double epsilon, int &axis1,
                                                         int &axis2) {
...
void CollisionDetection::fillSolidBoxSolidBoxInfo(const Box &box1, const Box &box2, Vector3 &a, Vector3 &b, Vector3 &D,
                                                  double *c, double *ca, double *ad, double *bd) {
...
bool CollisionDetection::fixedSolidBoxIntersectsFixedSolidBox(const Box &box1, const Box &box2,
...
void CollisionDetection::closestPointsBetweenLineAndLine(const Line &line1, const Line &line2, Vector3 &closest1,
                                                         Vector3 &closest2) {
...
float CollisionDetection::penetrationDepthForFixedBoxFixedBox(const Box &box1, const Box &box2,
...
float CollisionDetection::penetrationDepthForFixedSphereFixedBox(const Sphere &sphere, const Box &box,
                                                                 Array<Vector3> &contactPoints,
                                                                 Array<Vector3> &contactNormals) {
...
float CollisionDetection::penetrationDepthForFixedSphereFixedSphere(const Sphere &sphereA, const Sphere &sphereB,
                                                                    Array<Vector3> &contactPoints,
                                                                    Array<Vector3> &contactNormals) {
...
float CollisionDetection::penetrationDepthForFixedSphereFixedPlane(const Sphere &sphereA, const Plane &planeB,
                                                                   Array<Vector3> &contactPoints,
                                                                   Array<Vector3> &contactNormals) {
...
float CollisionDetection::penetrationDepthForFixedBoxFixedPlane(const Box &box, const Plane &plane,
                                                                Array<Vector3> &contactPoints,
                                                                Array<Vector3> &contactNormals) {
...
float CollisionDetection::collisionTimeForMovingPointFixedPlane(const Vector3 &point, const Vector3 &velocity,
                                                                const Plane &plane, Vector3 &location,
                                                                Vector3 &outNormal) {
...
bool __fastcall CollisionDetection::rayAABox(const Ray &ray, const Vector3 &invDir, const AABox &box,
                                             const Vector3 &boxCenter, float boundingRadiusSquared, Vector3 &location,
                                             bool &inside) {
...
float CollisionDetection::collisionTimeForMovingPointFixedSphere(const Vector3 &point, const Vector3 &velocity,
                                                                 const Sphere &sphere, Vector3 &location,
                                                                 Vector3 &outNormal, bool solid) {
...
float CollisionDetection::collisionTimeForMovingSphereFixedSphere(const Sphere &movingSphere, const Vector3 &velocity,
                                                                  const Sphere &fixedSphere, Vector3 &location,
                                                                  Vector3 &outNormal) {
  ...
  float time = collisionTimeForMovingPointFixedSphere(
      movingSphere.center, velocity, Sphere(fixedSphere.center, fixedSphere.radius + movingSphere.radius), location,
      outNormal);
...
float CollisionDetection::collisionTimeForMovingPointFixedBox(const Vector3 &point, const Vector3 &velocity,
                                                              const Box &box, Vector3 &location, Vector3 &outNormal) {
...
float CollisionDetection::collisionTimeForMovingPointFixedAABox(const Vector3 &origin, const Vector3 &dir,
                                                                const AABox &box, Vector3 &location, bool &Inside,
                                                                Vector3 &normal) {
...
bool CollisionDetection::collisionLocationForMovingPointFixedAABox(const Vector3 &origin, const Vector3 &dir,
                                                                   const AABox &box, Vector3 &location, bool &Inside,
                                                                   Vector3 &normal) {
...
float CollisionDetection::collisionTimeForMovingPointFixedRectangle(const Vector3 &point, const Vector3 &velocity,
                                                                    const Vector3 &v0, const Vector3 &v1,
                                                                    const Vector3 &v2, const Vector3 &v3,
                                                                    Vector3 &location, Vector3 &outNormal) {
...
static int findRayCapsuleIntersectionAux(const Vector3 &rkOrigin, const Vector3 &rkDirection, const Capsule &rkCapsule,
                                         double afT[2]) {
...
/** Used by collisionTimeForMovingPointFixedCapsule.
    @cite From magic software http://www.magic-software.com/Source/Intersection3D/MgcIntr3DLinCap.cpp
        ...
        */
static bool findRayCapsuleIntersection(const Ray &rkRay, const Capsule &rkCapsule, int &riQuantity,
                                       Vector3 akPoint[2]) {
...
float CollisionDetection::collisionTimeForMovingPointFixedCapsule(const Vector3 &_point, const Vector3 &velocity,
                                                                  const Capsule &capsule, Vector3 &location,
                                                                  Vector3 &outNormal) {
...
float CollisionDetection::collisionTimeForMovingSphereFixedPlane(const Sphere &sphere, const Vector3 &velocity,
                                                                 const Plane &plane, Vector3 &location,
                                                                 Vector3 &outNormal) {
...
float CollisionDetection::collisionTimeForMovingSphereFixedTriangle(const class Sphere &sphere, const Vector3 &velocity,
  ...
  if (isPointInsideTriangle(triangle.vertex(0), triangle.vertex(1), triangle.vertex(2), triangle.normal(), outLocation,
                            b, triangle.primaryAxis())) {
  ...
  if (t < finf()) {
    ...
    b[next[edgeIndex]] = (outLocation - triangle._vertex[edgeIndex]).dot(triangle.edgeDirection[edgeIndex]) /
...
#ifdef G3D_DEBUG
    {
      ...
      Vector3 blend = b[0] * triangle.vertex(0) + b[1] * triangle.vertex(1) + b[2] * triangle.vertex(2);
      debugAssertM(blend.fuzzyEq(outLocation), format("Barycentric coords don't match intersection. %s != %s",
                                                      blend.toString().c_str(), outLocation.toString().c_str()));
...
float CollisionDetection::collisionTimeForMovingSphereFixedRectangle(const Sphere &sphere, const Vector3 &velocity,
                                                                     const Vector3 &v0, const Vector3 &v1,
                                                                     const Vector3 &v2, const Vector3 &v3,
                                                                     Vector3 &location, Vector3 &outNormal) {
...
float CollisionDetection::collisionTimeForMovingSphereFixedBox(const Sphere &sphere, const Vector3 &velocity,
                                                               const Box &box, Vector3 &location, Vector3 &outNormal) {
...
float CollisionDetection::collisionTimeForMovingSphereFixedCapsule(const Sphere &sphere, const Vector3 &velocity,
                                                                   const Capsule &capsule, Vector3 &location,
                                                                   Vector3 &outNormal) {
...
Vector3 CollisionDetection::bounceDirection(const Sphere &sphere, const Vector3 &velocity, const float collisionTime,
...
Vector3 CollisionDetection::slideDirection(const Sphere &sphere, const Vector3 &velocity, const float collisionTime,
...
Vector3 CollisionDetection::closestPointOnLineSegment(const Vector3 &v0, const Vector3 &v1,
                                                      const Vector3 &edgeDirection, const float edgeLength,
                                                      const Vector3 &point) {
...
Vector3 CollisionDetection::closestPointOnTrianglePerimeter(const Vector3 &v0, const Vector3 &v1, const Vector3 &v2,
                                                            const Vector3 &point) {
...
Vector3 CollisionDetection::closestPointOnTrianglePerimeter(const Vector3 v[3], const Vector3 edgeDirection[3],
                                                            const float edgeLength[3], const Vector3 &point,
                                                            int &edgeIndex) {
...
#ifdef G3D_DEBUG
  {
    ...
    debugAssertM(fuzzyEq(diff.direction().dot(edgeDirection[edgeIndex]), 1.0f) || diff.fuzzyEq(Vector3::zero()),
...
bool CollisionDetection::isPointInsideTriangle(const Vector3 &v0, const Vector3 &v1, const Vector3 &v2,
                                               const Vector3 &normal, const Vector3 &point, float b[3],
...
bool CollisionDetection::isPointInsideRectangle(const Vector3 &v0, const Vector3 &v1, const Vector3 &v2,
                                                const Vector3 &v3, const Vector3 &normal, const Vector3 &point) {

  return isPointInsideTriangle(v0, v1, v2, normal, point) || isPointInsideTriangle(v2, v3, v0, normal, point);
}

Vector3 CollisionDetection::closestPointToRectanglePerimeter(const Vector3 &v0, const Vector3 &v1, const Vector3 &v2,
...
Vector3 CollisionDetection::closestPointToRectangle(const Vector3 &v0, const Vector3 &v1, const Vector3 &v2,
                                                    const Vector3 &v3, const Vector3 &point) {
...
bool CollisionDetection::movingSpherePassesThroughFixedBox(const Sphere &sphere, const Vector3 &velocity,
                                                           const Box &box, double timeLimit) {
...
bool CollisionDetection::movingSpherePassesThroughFixedSphere(const Sphere &sphere, const Vector3 &velocity,
                                                              const Sphere &fixedSphere, double timeLimit) {
...
bool CollisionDetection::fixedSolidSphereIntersectsFixedTriangle(const Sphere &sphere, const Triangle &triangle) {
  ...
  if ((v - sphere.center).squaredLength() <= square(sphere.radius)) {
    ...
    float b[3];
    if (isPointInsideTriangle(triangle.vertex(0), triangle.vertex(1), triangle.vertex(2), triangle.normal(), v, b,
  ...
  v = closestPointOnTrianglePerimeter(triangle._vertex, triangle.edgeDirection, triangle.edgeMagnitude, sphere.center,
                                      edgeIndex);
