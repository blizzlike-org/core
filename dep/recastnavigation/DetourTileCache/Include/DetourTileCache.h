...
public:
  ...
  dtStatus queryTiles(const float *bmin, const float *bmax, dtCompressedTileRef *results, int *resultCount,
  ...
  ///  @param[out]	upToDate	Whether the tile cache is fully up to date with obstacle requests and tile
  ///  rebuilds. 							If the tile cache is up to date another (immediate) call to update will have no effect; 							otherwise
  ///  another call will continue processing obstacle requests and tile rebuilds.
