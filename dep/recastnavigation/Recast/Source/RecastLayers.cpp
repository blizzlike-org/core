...
bool rcBuildHeightfieldLayers(rcContext *ctx, rcCompactHeightfield &chf, const int borderSize, const int walkableHeight,
  ...
  rcScopedDelete<rcLayerSweepSpan> sweeps(
      (rcLayerSweepSpan *)rcAlloc(sizeof(rcLayerSweepSpan) * nsweeps, RC_ALLOC_TEMP));
  if (!sweeps) {
        ...
        for (int j = i + 1; j < nlregs; ++j) {
          if (lregs[i] != lregs[j]) {
            ...
            if (!addUnique(ri.layers, ri.nlayers, RC_MAX_LAYERS, lregs[j]) ||
              ...
              ctx->log(RC_LOG_ERROR, "rcBuildHeightfieldLayers: layer overflow (too many overlapping walkable "
                                     "platforms). Try increasing RC_MAX_LAYERS.");
              return false;
            }
          }
        }
