...
struct FastLZCompressor : public dtTileCacheCompressor {
  virtual int maxCompressedSize(const int bufferSize) { return (int)(bufferSize * 1.05f); }
...
struct LinearAllocator : public dtTileCacheAlloc {
  ...
  LinearAllocator(const size_t cap) : buffer(0), capacity(0), top(0), high(0) { resize(cap); }
...
int Sample_TempObstacles::rasterizeTileLayers(const int tx, const int ty, const rcConfig &cfg, TileCacheData *tiles,
                                              const int maxTiles) {
  ...
  for (int i = 0; i < m_geom->getConvexVolumeCount(); ++i) {
    rcMarkConvexPolyArea(m_ctx, vols[i].verts, vols[i].nverts, vols[i].hmin, vols[i].hmax, (unsigned char)vols[i].area,
                         *rc.chf);
  }
...
void drawTiles(duDebugDraw *dd, dtTileCache *tc) {
  ...
  for (int i = 0; i < tc->getTileCount(); ++i) {
    ...
    duDebugDrawBoxWire(dd, bmin[0] - pad, bmin[1] - pad, bmin[2] - pad, bmax[0] + pad, bmax[1] + pad, bmax[2] + pad,
                       col, 2.0f);
  }
}
...
public:
  TempObstacleHilightTool() : m_sample(0), m_hitPosSet(false), m_drawType(DRAWDETAIL_AREAS) {
  ...
  virtual void init(Sample *sample) { m_sample = (Sample_TempObstacles *)sample; }
...
void Sample_TempObstacles::handleSettings() {
  ...
  snprintf(msg, 64, "Memory  %.1f kB / %.1f kB (%.1f%%)", m_cacheCompressedSize / 1024.0f, m_cacheRawSize / 1024.0f,
...
void Sample_TempObstacles::handleRenderOverlay(double *proj, double *model, int *view) {
  ...
  /*	imguiDrawRect(280,10,300,100,imguiRGBA(0,0,0,64));
          ...
          snprintf(text,64,"Compressed: %.1fkB (%.1f%%)", m_tileCache->getCompressedSize()/1024.0f,
                           m_tileCache->getRawSize() > 0 ?
     100.0f*(float)m_tileCache->getCompressedSize()/(float)m_tileCache->getRawSize() : 0); imguiDrawText(300, y,
     IMGUI_ALIGN_LEFT, text, imguiRGBA(255,255,255,255)); y -= 20;
