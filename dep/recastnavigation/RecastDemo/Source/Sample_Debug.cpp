...
Sample_Debug::Sample_Debug() : m_chf(0), m_cset(0), m_pmesh(0) {
  resetCommonSettings();
  ...
  /*	const float orig[3] = {0,0,0};
          ...
          // Tile_-13_-14.bin is basically just the bytes that was output by Detour. It should be loaded at X: -13 and
     Y: -14.
...

  {
    ...
    if (m_cset) {
      ...
      if (io.openForRead("PathSet_TMP_NA_PathingTestAReg1_1_2_CS.rc")) {
        ...
        printf("bmin=(%f,%f,%f) bmax=(%f,%f,%f)\n", m_cset->bmin[0], m_cset->bmin[1], m_cset->bmin[2], m_cset->bmax[0],
               m_cset->bmax[1], m_cset->bmax[2]);
        printf("cs=%f ch=%f\n", m_cset->cs, m_cset->ch);
      } else {
...
void Sample_Debug::handleMeshChanged(InputGeom *geom) { m_geom = geom; }
