...
ConvexVolumeTool::ConvexVolumeTool()
    : m_sample(0), m_areaType(SAMPLE_POLYAREA_GRASS), m_polyOffset(0.0f), m_boxHeight(6.0f), m_boxDescent(1.0f),
      m_npts(0), m_nhull(0) {}
...
void ConvexVolumeTool::handleRenderOverlay(double * /*proj*/, double * /*model*/, int *view) {
  ...
  const int h = view[3];
  if (!m_npts) {
    imguiDrawText(280, h - 40, IMGUI_ALIGN_LEFT,
    ...
    imguiDrawText(280, h - 40, IMGUI_ALIGN_LEFT,
                  ...
                  imguiRGBA(255, 255, 255, 192));
    imguiDrawText(280, h - 60, IMGUI_ALIGN_LEFT, "The shape will be convex hull of all added points.",
                  imguiRGBA(255, 255, 255, 192));
  }
}
