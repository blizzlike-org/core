...
OffMeshConnectionTool::OffMeshConnectionTool() : m_sample(0), m_hitPosSet(0), m_bidir(true), m_oldFlags(0) {}
...
void OffMeshConnectionTool::handleRenderOverlay(double *proj, double *model, int *view) {
  ...
  if (m_hitPosSet &&
      gluProject((GLdouble)m_hitPos[0], (GLdouble)m_hitPos[1], (GLdouble)m_hitPos[2], model, proj, view, &x, &y, &z)) {
  ...
  if (!m_hitPosSet) {
    ...
    imguiDrawText(280, h - 40, IMGUI_ALIGN_LEFT, "LMB: Set connection end point and finish.",
                  imguiRGBA(255, 255, 255, 192));
  }
}
