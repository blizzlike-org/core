...
static bool overOffmeshConnection(const dtCrowdAgent *ag, const float radius) {
  ...
  const bool offMeshConnection =
      (ag->cornerFlags[ag->ncorners - 1] & DT_STRAIGHTPATH_OFFMESH_CONNECTION) ? true : false;
...
dtCrowd::dtCrowd()
    : m_maxAgents(0), m_agents(0), m_activeAgents(0), m_agentAnims(0), m_obstacleQuery(0), m_grid(0), m_pathResult(0),
      m_maxPathResult(0), m_maxAgentRadius(0), m_velocitySampleCount(0), m_navquery(0) {}
...
int dtCrowd::addAgent(const float *pos, const dtCrowdAgentParams *params) {
  ...
  dtPolyRef ref = 0;
  dtVcopy(nearest, pos);
  dtStatus status = m_navquery->findNearestPoly(pos, m_agentPlacementHalfExtents,
                                                &m_filters[ag->params.queryFilterType], &ref, nearest);
...
void dtCrowd::checkPathValidity(dtCrowdAgent **agents, const int nagents, const float dt) {
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    if (!m_navquery->isValidPolyRef(agentRef, &m_filters[ag->params.queryFilterType])) {
      ...
      agentRef = 0;
      m_navquery->findNearestPoly(ag->npos, m_agentPlacementHalfExtents, &m_filters[ag->params.queryFilterType],
                                  &agentRef, nearest);
    ...
    if (ag->targetState != DT_CROWDAGENT_TARGET_NONE && ag->targetState != DT_CROWDAGENT_TARGET_FAILED) {
      if (!m_navquery->isValidPolyRef(ag->targetRef, &m_filters[ag->params.queryFilterType])) {
        ...
        ag->targetRef = 0;
        m_navquery->findNearestPoly(ag->targetPos, m_agentPlacementHalfExtents, &m_filters[ag->params.queryFilterType],
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo *debug) {
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    ag->ncorners =
        ag->corridor.findCorners(ag->cornerVerts, ag->cornerFlags, ag->cornerPolys, DT_CROWDAGENT_MAX_CORNERS,
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    if (overOffmeshConnection(ag, triggerRadius)) {
      ...
      dtPolyRef refs[2];
      if (ag->corridor.moveOverOffmeshConnection(ag->cornerPolys[ag->ncorners - 1], refs, anim->startPos, anim->endPos,
                                                 m_navquery)) {
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    if (ag->params.updateFlags & DT_CROWD_OBSTACLE_AVOIDANCE) {
      ...
      if (adaptive) {
        ns = m_obstacleQuery->sampleVelocityAdaptive(ag->npos, ag->params.radius, ag->desiredSpeed, ag->vel, ag->dvel,
                                                     ag->nvel, params, vod);
      } else {
        ns = m_obstacleQuery->sampleVelocityGrid(ag->npos, ag->params.radius, ag->desiredSpeed, ag->vel, ag->dvel,
                                                 ag->nvel, params, vod);
      }
