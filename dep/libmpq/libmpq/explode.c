...
static const uint8_t pkzip_dist_bits[] = {0x02, 0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06,
...
/* tables used for data extraction. */
static const uint8_t pkzip_dist_code[] = {0x03, 0x0D, 0x05, 0x19, 0x09, 0x11, 0x01, 0x3E, 0x1E, 0x2E, 0x0E, 0x36, 0x16,
...
/* tables used for data extraction. */
static const uint16_t pkzip_len_base[] = {0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005, 0x0006, 0x0007,
...
static int32_t skip_bit(pkzip_cmp_s *mpq_pkzip, uint32_t bits) {
  ...
  if (mpq_pkzip->in_pos == mpq_pkzip->in_bytes) {
    ...
    if ((mpq_pkzip->in_bytes = mpq_pkzip->read_buf((char *)mpq_pkzip->in_buf, &mpq_pkzip->in_pos, mpq_pkzip->param)) ==
        0) {
      return 1;
    }
    mpq_pkzip->in_pos = 0;
