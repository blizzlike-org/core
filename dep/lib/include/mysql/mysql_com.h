...
#define CLIENT_ALL_FLAGS                                                                                               \
  (CLIENT_LONG_PASSWORD | CLIENT_FOUND_ROWS | CLIENT_LONG_FLAG | CLIENT_CONNECT_WITH_DB | CLIENT_NO_SCHEMA |           \
   ...
   CLIENT_SSL_VERIFY_SERVER_CERT | CLIENT_REMEMBER_OPTIONS | CLIENT_PLUGIN_AUTH | CLIENT_CONNECT_ATTRS |               \
   CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA | CLIENT_CAN_HANDLE_EXPIRED_PASSWORDS | CLIENT_SESSION_TRACK |                \
   CLIENT_DEPRECATE_EOF | CLIENT_OPTIONAL_RESULTSET_METADATA | CLIENT_ZSTD_COMPRESSION_ALGORITHM |                     \
...
#define CLIENT_BASIC_FLAGS                                                                                             \
  (CLIENT_ALL_FLAGS &                                                                                                  \
   ~(CLIENT_SSL | CLIENT_COMPRESS | CLIENT_SSL_VERIFY_SERVER_CERT | CLIENT_ZSTD_COMPRESSION_ALGORITHM))
...
#define SERVER_STATUS_CLEAR_SET                                                                                        \
  (SERVER_QUERY_NO_GOOD_INDEX_USED | SERVER_QUERY_NO_INDEX_USED | SERVER_MORE_RESULTS_EXISTS |                         \
   SERVER_STATUS_METADATA_CHANGED | SERVER_QUERY_WAS_SLOW | SERVER_STATUS_DB_DROPPED | SERVER_STATUS_CURSOR_EXISTS |   \
   SERVER_STATUS_LAST_ROW_SENT | SERVER_SESSION_STATE_CHANGED)
...
bool net_write_command(struct NET *net, unsigned char command, const unsigned char *header, size_t head_len,
...
bool generate_sha256_scramble(unsigned char *dst, size_t dst_size, const char *src, size_t src_size, const char *rnd,
                              size_t rnd_size);
...
unsigned long STDCALL net_field_length_checked(unsigned char **packet, unsigned long max_length);
#endif
