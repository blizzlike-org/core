...
public:
  ...
  inline MeshObjects const *GetMeshObjects(unsigned int tx, unsigned int ty) const {
    return m_GeomData.GetMeshObjects(tx, ty);
  };
  inline bool GetNavMeshTileBounds(unsigned int tx, unsigned int ty, float const *&bmin, float const *&bmax) {
    return m_MMapData.GetTileBounds(tx, ty, bmin, bmax);
  };
