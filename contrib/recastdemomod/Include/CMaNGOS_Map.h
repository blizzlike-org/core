...
enum ShowLevels { SHOW_LEVEL_NONE, SHOW_LEVEL_MAP, SHOW_LEVEL_NEIGHBOR_TILES, SHOW_LEVEL_TILES };

struct SelectedTile {
  SelectedTile(unsigned int x, unsigned int y, float const *bmin, float const *bmax) : tx(x), ty(y) {
    rcVcopy(bMin, bmin);
    rcVcopy(bMax, bmax);
  }
...
public:
  ...
  bool GeomChanged();
  inline void GetGeomBounds(float const *&bmin, float const *&bmax) {
    bmin = m_MapInfos->BMin();
    bmax = m_MapInfos->BMax();
  }
