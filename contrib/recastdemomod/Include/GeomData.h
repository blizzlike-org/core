...
class MeshDetails {
public:
  MeshDetails(float const *verts, int const *tris, unsigned int vCount, unsigned int tCount,
...
public:
  ...
  inline float const *BMin() const { return m_BMin; }
  inline float const *BMax() const { return m_BMax; }
  bool GetMeshData(G3D::Array<float> &sVerts, G3D::Array<int> &sTris, G3D::Array<float> &lVerts, G3D::Array<int> &lTris,
                   G3D::Array<unsigned char> &liquidFlags) const;
  static void GetTileBounds(unsigned int tileX, unsigned int tileY, float const *verts, int vertCount, float *bmin,
                            float *bmax);

private:
  void LoadMap();
  void LoadVMap();
  void MergeMeshArrays(G3D::Array<float> &dstVerts, G3D::Array<int> &dstTris, G3D::Array<float> const &srcVerts,
                       G3D::Array<int> const &srcTris) const;
