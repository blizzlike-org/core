...
MeshDetails::MeshDetails(float const *verts, int const *tris, unsigned int vCount, unsigned int tCount,
                         unsigned char *flags)
    : m_Verts(verts), m_Tris(tris), m_Normals(NULL), m_VertCount(vCount), m_TrisCount(tCount), m_Flags(flags),
      m_LocalFlags(false) {
  ComputeNormals();
...
void MeshObjects::GetTileBounds(unsigned int tileX, unsigned int tileY, float const *verts, int vertCount, float *bmin,
                                float *bmax) {
...
MeshInfos::MeshInfos(MeshDetails const *sMesh, MeshDetails const *lMesh, float const *bmin, float const *bmax)
    : m_SolidMesh(sMesh), m_LiquidMesh(lMesh) {
  rcVcopy(m_BMin, bmin);
  rcVcopy(m_BMax, bmax);
}

MeshObjects::MeshObjects(unsigned int mapId, unsigned int tileX, unsigned int tileY, BuildContext *ctx)
    : m_MapId(mapId), m_TileX(tileX), m_TileY(tileY), m_Ctx(ctx), m_MapInfos(NULL), m_VMapInfos(NULL) {
  ...
  if ((m_MapMesh.solidVerts.size() != 0) || (m_MapMesh.liquidVerts.size() != 0)) {
    ...
    if (m_MapMesh.liquidVerts.size() != 0) {
      liquid = new MeshDetails(m_MapMesh.liquidVerts.getCArray(), m_MapMesh.liquidTris.getCArray(),
                               m_MapMesh.liquidVerts.size() / 3, m_MapMesh.liquidTris.size() / 3,
                               m_MapMesh.liquidType.getCArray());
  ...
  if ((m_VMapMesh.solidVerts.size() != 0) || (m_VMapMesh.liquidVerts.size() != 0)) {
    ...
    if (m_VMapMesh.liquidVerts.size() != 0) {
      liquid = new MeshDetails(m_VMapMesh.liquidVerts.getCArray(), m_VMapMesh.liquidTris.getCArray(),
                               m_VMapMesh.liquidVerts.size() / 3, m_VMapMesh.liquidTris.size() / 3,
                               m_VMapMesh.liquidType.getCArray());
...
void MeshObjects::MergeMeshArrays(G3D::Array<float> &dstVerts, G3D::Array<int> &dstTris,
                                  G3D::Array<float> const &srcVerts, G3D::Array<int> const &srcTris) const {
...
bool MeshObjects::GetMeshData(G3D::Array<float> &sVerts, G3D::Array<int> &sTris, G3D::Array<float> &lVerts,
                              G3D::Array<int> &lTris, G3D::Array<unsigned char> &liquidFlags) const {
  bool res = false;
...
GeomData::GeomData() : m_MapId(0), m_Ctx(NULL), m_NoMapFile(false) {}

GeomData::~GeomData() { RemoveAllTiles(); }
...
static bool isectSegAABB(const float *sp, const float *sq, const float *amin, const float *amax, float &tmin,
                         float &tmax) {
...
static bool intersectSegmentTriangle(const float *sp, const float *sq, const float *a, const float *b, const float *c,
                                     float &t) {
  float v, w;
...
bool GeomData::RaycastMesh(MeshDetails const *mesh, float *src, float *dst, float &tmin) const {
  ...
  for (int i = 0; i < ncid; ++i) {
    ...
    for (int j = 0; j < ntris * 3; j += 3) {
      float t = 1;
      if (intersectSegmentTriangle(src, dst, &verts[tris[j] * 3], &verts[tris[j + 1] * 3], &verts[tris[j + 2] * 3],
                                   t)) {
        if (t < tmin)
          tmin = t;
        hit = true;
