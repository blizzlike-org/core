...
public:
  NavMeshTileTool() : m_sample(0), m_hitPosSet(false), m_agentRadius(0) { m_hitPos[0] = m_hitPos[1] = m_hitPos[2] = 0; }

  virtual ~NavMeshTileTool() {}

  virtual int type() { return TOOL_TILE_EDIT; }

  virtual void init(CMaNGOS_Map *sample) { m_sample = (CMaNGOS_Map *)sample; }
  ...
  virtual void handleRenderOverlay(double *proj, double *model, int *view) {
    GLdouble x, y, z;
    if (m_hitPosSet && gluProject((GLdouble)m_hitPos[0], (GLdouble)m_hitPos[1], (GLdouble)m_hitPos[2], model, proj,
                                  view, &x, &y, &z)) {
      int tx = 0, ty = 0;
    ...
    // Tool help
    const int h = view[3];
    imguiDrawText(280, h - 40, IMGUI_ALIGN_LEFT, "LMB: Rebuild hit tile.  Shift+LMB: Clear hit tile.",
                  imguiRGBA(255, 255, 255, 192));
  }
};

void duDebugDrawLiquidTriMesh(duDebugDraw *dd, const float *verts, int /*nverts*/, const int *tris,
                              const float *normals, int ntris, const float texScale) {
  if (!dd)
    return;
  if (!verts)
    return;
...
CMaNGOS_Map::CMaNGOS_Map()
    : m_drawMode(DRAWMODE_NAVMESH), m_navMeshDrawFlags(DU_DRAWNAVMESH_OFFMESHCONS | DU_DRAWNAVMESH_COLOR_TILES),
      m_maxTiles(0), m_maxPolysPerTile(0), m_tileSize(32), m_tileCol(duRGBA(11, 243, 229, 200)), m_tileBuildTime(0),
      m_tileMemUsage(0), m_tileTriCount(0), m_isBuilded(false), m_bigBaseUnit(false), m_mapID(-1),
      m_showLevelOld(false), m_DrawVMapMesh(true), m_DrawMapMesh(true), m_showLevel(SHOW_LEVEL_NONE),
      m_GeomChanged(false), m_tool(NULL), m_SelectedTile(NULL) {
...
CMaNGOS_Map::~CMaNGOS_Map() { ClearAllGeoms(); }
...
bool CMaNGOS_Map::ShowNeighborTiles(int height, int width) {
  ...
  if (imguiBeginScrollArea("Choose neighbor tile", width - 10 - 250 - 10 - winWSize, height - 10 - winHSize, winWSize,
                           winHSize, &levelScroll))
    mouseOverMenu = true;
...
bool CMaNGOS_Map::ShowTilesLevel(int height, int width) {
  ...
  const unsigned int winWSize = 180;
  if (imguiBeginScrollArea("Choose tile", width - 10 - 250 - 10 - winWSize, height - 10 - winHSize, winWSize, winHSize,
                           &levelScroll))
    mouseOverMenu = true;
...
bool CMaNGOS_Map::ShowLevel(int height, int width) {
  ...
  switch (m_showLevel) {
  case SHOW_LEVEL_MAP:
    return ShowMapLevel(height, width);
    break;
  case SHOW_LEVEL_NEIGHBOR_TILES:
    return ShowNeighborTiles(height, width);
    break;
  case SHOW_LEVEL_TILES:
    return ShowTilesLevel(height, width);
    break;
  default:
    return false;
    break;
...
void CMaNGOS_Map::handleExtraSettings() {
  ...
  if (!m_MapInfos->IsEmpty()) {
    ...
    while (itr != m_MapInfos->GetGeomsMap()->end()) {
      ...
      char buff[40];
      snprintf(buff, sizeof(buff), "%dx%d > Found %s%s%s", mo->GetTileX(), mo->GetTileY(), map.c_str(), vmap.c_str(),
               mmap.c_str());
      if (imguiItem(buff)) {
...
void CMaNGOS_Map::handleRenderOverlay(double *proj, double *model, int *view) {
  ...
  if (m_tileBuildTime > 0.0f &&
      gluProject((GLdouble)(m_tileBmin[0] + m_tileBmax[0]) / 2, (GLdouble)(m_tileBmin[1] + m_tileBmax[1]) / 2,
                 (GLdouble)(m_tileBmin[2] + m_tileBmax[2]) / 2, model, proj, view, &x, &y, &z)) {
    char text[32];
...
/*   const float* bmin = m_geom->getMeshBoundsMin();
   ...
   if (!m_solid || !rcCreateHeightfield(m_ctx, *m_solid, m_cfg.width, m_cfg.height, m_cfg.bmin, m_cfg.bmax, m_cfg.cs,
   m_cfg.ch))
   {
   ...
   if (!m_dmesh || !rcBuildPolyMeshDetail(m_ctx, *m_pmesh, *m_chf, m_cfg.detailSampleDist, m_cfg.detailSampleMaxError,
   *m_dmesh))
   {
...
void CMaNGOS_Map::handleRender() {
  ...
  for (MeshObjectsMap::const_iterator itr = m_MapInfos->GetGeomsMap()->begin(); itr != m_MapInfos->GetGeomsMap()->end();
       itr++) {
    if (m_drawMode != DRAWMODE_NAVMESH_TRANS) {
      ...
      if (m_DrawMapMesh && mmi) {
        ...
        if (mlmi)
          duDebugDrawLiquidTriMesh(&dd, mlmi->Verts(), mlmi->VertCount(), mlmi->Tris(), mlmi->Normals(),
                                   mlmi->TrisCount(), texScale);
      }
      ...
      if (m_DrawVMapMesh && vmi) {
        ...
        if (vlmi)
          duDebugDrawLiquidTriMesh(&dd, vlmi->Verts(), vlmi->VertCount(), vlmi->Tris(), vlmi->Normals(),
                                   vlmi->TrisCount(), texScale);
      }
    }
  }
  ...
  // Tiles grid
  duDebugDrawGridXZ(&dd, m_GridBMin[0], m_GridBMin[1], m_GridBMin[2], 3, 3, RecastDemo::BLOCK_SIZE,
                    duRGBA(255, 0, 0, 128), 2.0f);
