...
MMapData::MMapData()
    : m_NavMesh(NULL), m_NavQuery(NULL), m_Initialized(false), m_triareas(NULL), m_solid(NULL), m_chf(NULL),
      m_cset(NULL), m_pmesh(NULL), m_dmesh(NULL), m_MultipleTile(false) {}
...
bool MMapData::LoadTile(unsigned int tx, unsigned int ty) {
  ...
  snprintf(tileFilename, sizeof(tileFilename), "mmaps/%03d%02d%02d.mmtile", m_MapId, tx,
           ty); // Todo: check X,Y swap reason and get ride of it
  ...
  if (header.mmapVersion != MMAP_VERSION) {
    fclose(fp);
    m_Ctx->log(RC_LOG_ERROR, "LoadMMapTile: Mmap version is not correct! Found(%u), expected(%u)!", header.mmapVersion,
               unsigned int(MMAP_VERSION));
    return false;
  }
...
void MMapData::BuildNavMeshOfTile(unsigned int tx, unsigned int ty, MeshObjects const *meshObjects, rcConfig const *cfg,
                                  float agentHeight, float agentRadius, float agentClimb,
...
void MMapData::BuildMoveMapTile(unsigned int tileX, unsigned int tileY, MeshObjects const *meshObjects, rcConfig *cfg,
                                SamplePartitionType partitionType) {
  IntermediateValues iv;
  ...
  for (int y = 0; y < RecastDemo::TILES_PER_MAP; ++y) {
    for (int x = 0; x < RecastDemo::TILES_PER_MAP; ++x) {
      ...
      if (!tile.solid || !rcCreateHeightfield(m_Ctx, *tile.solid, tileCfg.width, tileCfg.height, tileCfg.bmin,
                                              tileCfg.bmax, tileCfg.cs, tileCfg.ch)) {
      ...
      if (!tile.chf ||
          !rcBuildCompactHeightfield(m_Ctx, tileCfg.walkableHeight, tileCfg.walkableClimb, *tile.solid, *tile.chf)) {
      ...
      if (!tile.cset ||
          !rcBuildContours(m_Ctx, *tile.chf, tileCfg.maxSimplificationError, tileCfg.maxEdgeLen, *tile.cset)) {
      ...
      if (!tile.dmesh || !rcBuildPolyMeshDetail(m_Ctx, *tile.pmesh, *tile.chf, tileCfg.detailSampleDist,
                                                tileCfg.detailSampleMaxError, *tile.dmesh)) {
  ...
  params.walkableClimb =
      RecastDemo::BASE_UNIT_DIM * cfg->walkableClimb; // keep less that walkableHeight (aka agent height)!
