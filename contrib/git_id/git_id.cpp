...
char remotes[NUM_REMOTES][MAX_REMOTE] = {"git@github.com:cmangos/mangos-tbc.git",
...
int last_sql_rev[NUM_DATABASES] = {2325, 2325, 2326};

int last_sql_nr[NUM_DATABASES] = {1, 2, 1};
...
int get_rev(const char *from_msg) {
  ...
  char nr_str[256];
  if (sscanf(from_msg, "[" REV_PREFIX "%[0123456789]]", nr_str) != 1)
    return 0;
  if (from_msg[strlen(nr_str) + strlen(REV_PREFIX) + 2 - 1] != ']')
    return 0;
...
bool convert_sql_updates() {
  ...
  for (std::set<std::string>::iterator itr = new_sql_updates.begin(); itr != new_sql_updates.end(); ++itr) {
    ...
    sql_update_info info;
    if (!get_sql_update_info(filename, info))
      return false;
    if (info.db_idx == NUM_DATABASES)
      return false;
    ...
    snprintf(new_name, MAX_PATH, REV_PRINT "_%0*d_%s%s%s", rev, 2, info.nr, info.db, info.has_table ? "_" : "",
             info.table);
    ...
    out_buff << "ALTER TABLE " << db_version_table[info.db_idx] << " CHANGE COLUMN required_"
...
bool change_sql_history() {
  ...
  for (std::list<std::string>::reverse_iterator next = hashes.rbegin(), itr = next++; next != hashes.rend();
       ++itr, ++next) {
