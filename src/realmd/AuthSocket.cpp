...
bool AuthSocket::_HandleLogonChallenge() {
  ...
  std::unique_ptr<QueryResult> ip_banned_result(
      LoginDatabase.PQuery("SELECT expires_at FROM ip_banned "
                           "WHERE (expires_at = banned_at OR expires_at > UNIX_TIMESTAMP()) AND ip = '%s'",
                           m_address.c_str()));
  ...
  if (ip_banned_result) {
    ...
    QueryResult *result = LoginDatabase.PQuery(
        "SELECT id,locked,lockedIp,gmlevel,v,s,token FROM account WHERE username = '%s'", _safelogin.c_str());
    if (result) {
      ...
      if (!locked && !broken) {
        ...
        QueryResult *banresult = LoginDatabase.PQuery(
            "SELECT banned_at,expires_at FROM account_banned WHERE "
            "account_id = %u AND active = 1 AND (expires_at > UNIX_TIMESTAMP() OR expires_at = banned_at)",
            fields[1].GetUInt32());
        if (banresult) {
  ...
  if (!srp.Proof(lp.M1, 20)) {
    if (lp.securityFlags & SECURITY_FLAG_AUTHENTICATOR || !_token.empty()) {
      ...
      if (ServerToken != clientToken) {
        BASIC_LOG("[AuthChallenge] Account %s tried to login with wrong pincode! Given %u Expected %u Pin Count: %u",
                  _login.c_str(), clientToken, ServerToken, pinCount);
    ...
    ///- Update the sessionkey, current ip and login time and reset number of failed logins in the account table for
    /// this account
    ...
    LoginDatabase.PExecute(
        "UPDATE account SET sessionkey = '%s', locale = '%s', failed_logins = 0 WHERE username = '%s'", K_hex,
        _safelocale.c_str(), _safelogin.c_str());
    if (QueryResult *loginfail =
            LoginDatabase.PQuery("SELECT id FROM account WHERE username = '%s'", _safelogin.c_str()))
      LoginDatabase.PExecute(
          "INSERT INTO account_logons(accountId,ip,loginTime,loginSource) VALUES('%u','%s',NOW(),'%u')",
          loginfail->Fetch()[0].GetUInt32(), m_address.c_str(), LOGIN_TYPE_REALMD);
    ...
    if (MaxWrongPassCount > 0) {
      ...
      LoginDatabase.PExecute("UPDATE account SET failed_logins = failed_logins + 1 WHERE username = '%s'",
                             _safelogin.c_str());

      if (QueryResult *loginfail =
              LoginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str())) {
        ...
        if (failed_logins >= MaxWrongPassCount) {
          ...
          if (WrongPassBanType) {
            ...
            LoginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','MaNGOS "
                                   "realmd','Failed login autoban')",
                                   current_ip.c_str(), WrongPassBanTime);
            BASIC_LOG("[AuthChallenge] IP %s got banned for '%u' seconds because account %s failed to authenticate "
                      "'%u' times",
...
bool AuthSocket::_HandleReconnectChallenge() {
  ...
  QueryResult *result =
      LoginDatabase.PQuery("SELECT sessionkey FROM account WHERE username = '%s'", _safelogin.c_str());
...
bool AuthSocket::_HandleRealmList() {
  ...
  ///- Circle through realms in the RealmList and construct the return packet (including # of user characters in each
  /// realm)
  ByteBuffer pkt;
...
void AuthSocket::LoadRealmlist(ByteBuffer &pkt, uint32 acctid) {
  ...
  case 6141: // 1.12.3
  {
    ...
    for (const auto &i : sRealmList) {
      ...
      QueryResult *result = LoginDatabase.PQuery(
          "SELECT numchars FROM realmcharacters WHERE realmid = '%d' AND acctid='%u'", i.second.m_ID, acctid);
      if (result) {
        Field *fields = result->Fetch();
      ...
      bool ok_build =
          std::find(i.second.realmbuilds.begin(), i.second.realmbuilds.end(), _build) != i.second.realmbuilds.end();
      ...
      // 1.x clients not support explicitly REALM_FLAG_SPECIFYBUILD, so manually form similar name as show in more
      // recent clients
...
int32 AuthSocket::generateToken(char const *b32key) {
  ...
  unsigned int truncHash = (hmac_result[offset] << 24) | (hmac_result[offset + 1] << 16) |
                           (hmac_result[offset + 2] << 8) | (hmac_result[offset + 3]);
