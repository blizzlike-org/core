...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()("config,c", boost::program_options::value<std::string>(&configFile)->default_value(_REALMD_CONFIG),
  ...
  sLog.outString("[%s Auth server v%s] port(%d)", _PACKAGENAME, VERSION, sConfig.GetIntDefault("RealmServerPort", -1));
  sLog.outString("\n\n"
  ...
  LoginDatabase.Execute(
      "UPDATE account_banned SET active = 0 WHERE expires_at<=UNIX_TIMESTAMP() AND expires_at<>banned_at");
...
#ifdef _WIN32
  {
    ...
    if (Aff > 0) {
      ...
      if (GetProcessAffinityMask(hProcess, &appAff, &sysAff)) {
        ...
        if (!curAff) {
          sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not accessible for realmd. Accessible "
