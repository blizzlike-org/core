...
/// RASocket destructor
RASocket::~RASocket() { sLog.outRALog("Connection was closed."); }
...
bool RASocket::HandleInput() {
  auto const minLevel =
      static_cast<AccountTypes>(sConfig.GetIntDefault("RA.MinLevel", AccountTypes::SEC_ADMINISTRATOR));
  ...
  case AuthLevel::Authenticated: {
    if (m_input.length()) {
      ...
      sWorld.QueueCliCommand(new CliCommandHolder(
          m_accountId, m_accountLevel, m_input.c_str(), [this](const char *buffer) { this->Send(buffer); },
...
void RASocket::Send(const std::string &message) { Write(message.c_str(), message.length()); }
