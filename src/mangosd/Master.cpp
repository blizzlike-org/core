...
int Master::Run() {
...
#ifdef _WIN32
  {
    ...
    if (Aff > 0) {
      ...
      if (GetProcessAffinityMask(hProcess, &appAff, &sysAff)) {
        ...
        if (!curAff) {
          sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not accessible for mangosd. Accessible "
                        "processors bitmask (hex): %x",
                        Aff, appAff);
        } else {
...

  {
    ...
    MaNGOS::Listener<WorldSocket> listener(sConfig.GetStringDefault("BindIP", "0.0.0.0"),
                                           int32(sWorld.getConfig(CONFIG_UINT32_PORT_WORLD)), networkThreadWorker);

    std::unique_ptr<MaNGOS::Listener<RASocket>> raListener;
    if (sConfig.GetBoolDefault("Ra.Enable", false))
      raListener.reset(new MaNGOS::Listener<RASocket>(sConfig.GetStringDefault("Ra.IP", "0.0.0.0"),
                                                      sConfig.GetIntDefault("Ra.Port", 3443), 1));

    std::unique_ptr<SOAPThread> soapThread;
    if (sConfig.GetBoolDefault("SOAP.Enabled", false))
      soapThread.reset(
          new SOAPThread(sConfig.GetStringDefault("SOAP.IP", "127.0.0.1"), sConfig.GetIntDefault("SOAP.Port", 7878)));
  ...
  LoginDatabase.DirectPExecute("UPDATE realmlist SET realmflags = realmflags | %u WHERE id = '%u'", REALM_FLAG_OFFLINE,
                               realmID);
...
void Master::clearOnlineAccounts() {
  ...
  /// \todo Only accounts with characters logged on *this* realm should have online status reset. Move the online column
  /// from 'account' to 'realmcharacters'?
