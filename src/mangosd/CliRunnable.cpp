...
/// \todo This function has to be enhanced to respect the login/realm split (delete char, delete account chars in realm,
/// delete account chars in realm then delete account
 ...
 * @return             returns false if there was a problem while selecting the characters (e.g. player name not
 * normalizeable)
 */
  ...
  if (!searchString.empty()) {
    ...
    if (isNumeric(searchString))
      resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate FROM "
                                            "characters WHERE deleteDate IS NOT NULL AND guid = %u",
...
/**
 * Generate WHERE guids list by deleted info in way preventing return too long where list for existed query string
 * length limit.
 *
 * @param itr          a reference to an deleted info list iterator, it updated in function for possible next function
 * call if list to long
...
void ChatHandler::HandleCharacterDeletedListHelper(DeletedInfoList const &foundList) {
  ...
  for (const auto &itr : foundList) {
    ...
    if (!m_session)
      PSendSysMessage(LANG_CHARACTER_DELETED_LIST_LINE_CONSOLE, itr.lowguid, itr.name.c_str(),
                      itr.accountName.empty() ? "<nonexistent>" : itr.accountName.c_str(), itr.accountId,
                      dateStr.c_str());
    else
      PSendSysMessage(LANG_CHARACTER_DELETED_LIST_LINE_CHAT, itr.lowguid, itr.name.c_str(),
                      itr.accountName.empty() ? "<nonexistent>" : itr.accountName.c_str(), itr.accountId,
                      dateStr.c_str());
  }
...
/**
 * Handles the '.character deleted list' command, which shows all deleted characters which matches the given search
 * string
 *
...
/**
 * Handles the '.character deleted restore' command, which restores all deleted characters which matches the given
 * search string
 *
 * The command automatically calls '.character deleted list' command with the search string to show all restored
 * characters.
 *
...
/**
 * Handles the '.character deleted delete' command, which completely deletes all deleted characters which matches the
 * given search string
 *
...
/**
 * Handles the '.character deleted old' command, which completely deletes all deleted characters deleted with some days
 * ago
 *
...
bool ChatHandler::HandleCharacterEraseCommand(char *args) {
  ...
  PSendSysMessage(LANG_CHARACTER_DELETED, target_name.c_str(), target_guid.GetCounter(), account_name.c_str(),
                  account_id);
  return true;
}
...
bool ChatHandler::HandleAccountOnlineListCommand(char *args) {
  ...
  //                                                 0            1         2        3        4
  QueryResult *result = LoginDatabase.PQuery("SELECT distinct a.id, username, ip, gmlevel, expansion FROM account a "
                                             "join account_logons b on(a.id=b.accountId) WHERE active_realm_id = %u",
                                             realmID);
