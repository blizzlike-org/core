...
inline uint32 secsToTimeBitFields(time_t secs) {
  ...
  return (lt->tm_year - 100) << 24 | lt->tm_mon << 20 | (lt->tm_mday - 1) << 14 | lt->tm_wday << 11 | lt->tm_hour << 6 |
         lt->tm_min;
}
...
template <class Side, Side Default, uint8 Sides> struct Die {
...
size_t
utf8length(std::string &utf8str); // returns string's length in utf8 chars, sets string to "" on invalid utf8 sequence
size_t utf8limit(std::string &utf8str,
                 size_t bytes); // returns string's new size in bytes, sets string to "" on invalid utf8 sequence
...
template <typename E> constexpr typename std::underlying_type<E>::type AsUnderlyingType(E enumValue) {
