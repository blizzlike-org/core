...
public:
  ...
  template <typename T> class SQLSIterator {
  ...
  template <typename T> SQLSIterator<T> getDataBegin() const { return SQLSIterator<T>(m_data, m_recordSize); }
  template <typename T> SQLSIterator<T> getDataEnd() const {
...
public:
  ...
  template <class T> T const *LookupEntry(uint32 id) const {
    if (id >= GetMaxEntry())
      return nullptr;
...
public:
  ...
  template <typename T> class SQLMultiSIterator {
  ...
  template <typename T> SQLMSIteratorBounds<T> getBounds(uint32 key) const {
...
template <class DerivedLoader, class StorageClass> class SQLStorageLoaderBase {
public:
  void Load(StorageClass &store, bool error_at_empty = true);

  template <class S, class D> void convert(uint32 field_pos, S src, D &dst);
  template <class S> void convert_to_bool(uint32 field_pos, S src, bool &dst);
  template <class S> void convert_to_str(uint32 field_pos, S src, char *&dst);
  ...
  // trap, no body
  template <class D> void convert_from_str(uint32 field_pos, char *src, D &dst);
  void convert_str_to_str(uint32 field_pos, char *src, char *&dst);

private:
  template <class V> void storeValue(V value, StorageClass &store, char *p, uint32 x, uint32 &offset);
