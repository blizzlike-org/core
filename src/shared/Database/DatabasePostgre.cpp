...
SqlConnection *DatabasePostgre::CreateConnection() { return new PostgreSQLConnection(*this); }
...
bool PostgreSQLConnection::Initialize(const char *infoString) {
  ...
  if (host == ".")
    mPGconn = PQsetdbLogin(nullptr, port_or_socket_dir == "." ? nullptr : port_or_socket_dir.c_str(), nullptr, nullptr,
                           database.c_str(), user.c_str(), password.c_str());
  else
    mPGconn = PQsetdbLogin(host.c_str(), port_or_socket_dir.c_str(), nullptr, nullptr, database.c_str(), user.c_str(),
                           password.c_str());

  /* check to see that the backend connection was successfully made */
  if (PQstatus(mPGconn) != CONNECTION_OK) {
    sLog.outError("Could not connect to Postgre database at %s: %s", host.c_str(), PQerrorMessage(mPGconn));
    PQfinish(mPGconn);
    mPGconn = nullptr;
    return false;
  }

  DETAIL_LOG("Connected to Postgre database %s@%s:%s/%s", user.c_str(), host.c_str(), port_or_socket_dir.c_str(),
             database.c_str());
