...
private:
  ...
  enum class ReadState { Idle, Reading };
...
public:
  ...
  template <typename T> std::shared_ptr<T> shared() { return std::static_pointer_cast<T>(shared_from_this()); }
...
private:
  ...
  public:
    ...
    template <typename... Args> void operator()(Args &&...args) { m_handler(std::forward<Args>(args)...); }
