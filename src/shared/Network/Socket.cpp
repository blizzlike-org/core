...
void Socket::StartAsyncRead() {
  ...
  m_socket.async_read_some(
      ...
      make_custom_alloc_handler(
          m_allocator, [ptr](const boost::system::error_code &error, size_t length) { ptr->OnRead(error, length); }));
}
...
void Socket::OnRead(const boost::system::error_code &error, size_t length) {
  ...
  while (m_inBuffer->m_readPosition < m_inBuffer->m_writePosition) {
    if (!ProcessIncomingData()) {
      // this errno is set when there is not enough buffer data available to either complete a header, or the packet
      // length specified in the header goes past what we've read.  in this case, we will reset the buffer with the
      // remaining data
      if (errno == EBADMSG) {
...
void Socket::ForceFlushOut() { m_outBufferFlushTimer.cancel(); }
