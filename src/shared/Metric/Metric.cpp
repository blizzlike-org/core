...
void metric::measurement::add_tag(std::string key, std::string value) { m_tags.insert({key, value}); }

void metric::measurement::add_field(std::string key, boost::any value) { m_fields.insert({key, value}); }
...
void metric::measurement::set_condition(std::function<bool()> condition) { m_condition = std::move(condition); }

metric::metric::metric() { initialize(); }
...
void metric::metric::initialize() {
  ...
  m_connectionInfo = {sConfig.GetStringDefault("Metric.Address", "127.0.0.1"),
                      sConfig.GetIntDefault("Metric.Port", 8086), sConfig.GetStringDefault("Metric.Database", "perfd"),
                      sConfig.GetStringDefault("Metric.Username", ""), sConfig.GetStringDefault("Metric.Password", "")};
