...
template <typename T> UnitAI *GetNewAIInstance(Creature *creature) { return new T(creature); }

template <typename T> GameObjectAI *GetNewAIInstance(GameObject *gameobject) { return new T(gameobject); }

template <typename T> InstanceData *GetNewInstanceScript(Map *map) { return new T(map); }

struct Script {
  Script()
      : pGossipHello(nullptr), pGossipHelloGO(nullptr), pGossipSelect(nullptr), pGossipSelectGO(nullptr),
        pGossipSelectWithCode(nullptr), pGossipSelectGOWithCode(nullptr), pDialogStatusNPC(nullptr),
        pDialogStatusGO(nullptr), pQuestAcceptNPC(nullptr), pQuestAcceptGO(nullptr), pQuestAcceptItem(nullptr),
        pQuestRewardedNPC(nullptr), pQuestRewardedGO(nullptr), pGOUse(nullptr), pItemUse(nullptr), pItemLoot(nullptr),
        pAreaTrigger(nullptr), pProcessEventId(nullptr), pEffectDummyNPC(nullptr), pEffectDummyGO(nullptr),
        pEffectDummyItem(nullptr), pEffectScriptEffectNPC(nullptr), pEffectAuraDummy(nullptr), pTrapSearching(nullptr),
        GetGameObjectAI(nullptr), GetAI(nullptr), GetInstanceData(nullptr) {}
