...
struct CombatTimer : public Timer {
  CombatTimer(uint32 id, std::function<void()> functor, bool combat, uint32 timerMin, uint32 timerMax,
              bool disabled = false)
      : Timer(id, functor, timerMin, timerMax, disabled), combat(combat) {}
...
public:
  ...
  // Adds a combat action which has no timer. It is reset to default value at start. Useful for one-off actions like
  // phase transition at HP level.
