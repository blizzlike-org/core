...
public:
  ...
  virtual void
  UpdateEscortAI(const uint32 diff); // used when it's needed to add code in update (abilities, scripted events, etc)
  ...
  void Start(bool run = false, const Player *player = nullptr, const Quest *quest = nullptr,
             bool instantRespawn = false, bool canLoopPath = false);
...
private:
  ...
  bool m_canInstantRespawn; // if creature should respawn instantly after escort over (if not, database respawntime are
                            // used)
