...
bool CombatAI::IsTargetingRestricted() { return m_storedTarget; }
...
void RangedCombatAI::DistancingStarted() { SetCombatScriptStatus(true); }
    ...
    if (m_rangedModeSetting == TYPE_PROXIMITY) {
      if (m_currentRangedMode && m_creature->CanReachWithMeleeAttack(m_creature->GetVictim()))
        SetCurrentRangedMode(false);
               ...
               m_mainSpellInfo && m_mainSpellCost * 2 < m_creature->GetPower(POWER_MANA) &&
               m_creature->IsSpellReady(*m_mainSpellInfo))
        SetCurrentRangedMode(true);
    }
  }
