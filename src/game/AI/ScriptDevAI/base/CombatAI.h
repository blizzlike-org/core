...
/*
    Small wrapper class that reduces the amount of code needed to use CombatActions and adds proper spell action reset
   functionality
*/
...
public:
  ...
  void Reset() override { ResetAllTimers(); }
...
public:
  RangedCombatAI(Creature *creature, uint32 combatActions)
      : CombatAI(creature, combatActions), m_rangedMode(false), m_rangedModeSetting(TYPE_NONE), m_chaseDistance(0.f),
        m_currentRangedMode(false), m_mainSpellId(0), m_mainSpellCost(0), m_mainSpellInfo(nullptr),
        m_mainSpellMinRange(0.f), m_mainAttackMask(SPELL_SCHOOL_MASK_NONE) {}
  ...
  SpellSchoolMask GetMainAttackSchoolMask() const override {
    return m_currentRangedMode ? m_mainAttackMask : CombatAI::GetMainAttackSchoolMask();
  }
