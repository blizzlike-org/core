...
bool npc_escortAI::AssistPlayerInCombat(Unit *who) {
  ...
  // unit state prevents (similar check is done in CanInitiateAttack which also include checking unit_flags. We skip
  // those here)
...
bool npc_escortAI::IsPlayerOrGroupInRange() {
  if (Player *player = GetPlayerForEscort())
    if (player->CheckForGroup(
            [&](Player const *player) -> bool { return m_creature->IsWithinDistInMap(player, MAX_PLAYER_DISTANCE); }))
      return true;
    ...
    if (m_playerCheckTimer < diff) {
      if (!HasEscortState(STATE_ESCORT_PAUSED) && !IsPlayerOrGroupInRange()) {
        ...
        if (m_canInstantRespawn) {
          ...
          if (m_creature->IsPet())
            static_cast<Pet *>(m_creature)
                ->Unsummon(PET_SAVE_AS_DELETED); // we assume escort AI pet to always be non-saved
          else
...
bool npc_escortAI::IsSD2EscortMovement(uint32 moveType) const { return moveType >= EXTERNAL_WAYPOINT_MOVE; }
...
void npc_escortAI::SetCurrentWaypoint(uint32 pointId) {
  ...
  {
    script_error_log("EscortAI for %s tried to set new waypoint %u, but not paused", m_creature->GetGuidStr().c_str(),
                     pointId);
    return;
  }

  if (!m_creature->GetMotionMaster()->SetNextWaypoint(pointId)) {
    script_error_log("EscortAI for %s current waypoint tried to set to id %u, but doesn't exist in this path",
...
void npc_escortAI::Start(bool run, const Player *player, const Quest *quest, bool instantRespawn, bool canLoopPath) {
  ...
  if (!sWaypointMgr.GetPathFromOrigin(m_creature->GetEntry(), m_creature->GetGUIDLow(), m_waypointPathID,
                                      PATH_FROM_EXTERNAL)) {
    script_error_log("EscortAI attempt to start escorting for %s, but has no waypoints loaded.",
  ...
  if (m_canReturnToStart && m_canInstantRespawn)
    debug_log("SD2: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature "
              "will never despawn.");
...
void npc_escortAI::SetEscortWaypoints(uint32 pathId) { m_waypointPathID = pathId; }
