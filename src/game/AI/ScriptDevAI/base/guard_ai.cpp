...
guardAI::guardAI(Creature *creature) : ScriptedAI(creature), m_globalCooldown(0), m_buffTimer(0) {}
...
void guardAI::Aggro(Unit *who) {
  if (m_creature->GetEntry() == NPC_CENARION_INFANTRY) {
    switch (urand(0, 2)) {
    case 0:
      DoScriptText(SAY_GUARD_SIL_AGGRO1, m_creature, who);
      break;
    case 1:
      DoScriptText(SAY_GUARD_SIL_AGGRO2, m_creature, who);
      break;
    case 2:
      DoScriptText(SAY_GUARD_SIL_AGGRO3, m_creature, who);
      break;
    }
  }
  ...
  if (m_creature->IsAlive() && !m_creature->IsInCombat()) {
    if (m_buffTimer < diff) {
      ...
      const SpellEntry *spellInfo =
          SelectSpell(m_creature, -1, -1, SELECT_TARGET_ANY_FRIEND, 0, 0, 0, 0, SELECT_EFFECT_AURA);
  ...
  if (!m_creature->IsNonMeleeSpellCasted(false)) {
    ...
    if (m_creature->CanReachWithMeleeAttack(m_creature->GetVictim())) {
      ...
      if (spellInfo)
        ...
        spellInfo =
            SelectSpell(m_creature->GetVictim(), -1, -1, SELECT_TARGET_ANY_ENEMY, 0, 0, 0, 0, SELECT_EFFECT_DONTCARE);
