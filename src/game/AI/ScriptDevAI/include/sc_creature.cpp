...
ScriptedAI::ScriptedAI(Creature *creature) : CreatureAI(creature), m_uiEvadeCheckCooldown(2500) {}
...
void ScriptedAI::DoPlaySoundToSet(WorldObject *source, uint32 soundId) {
  ...
  if (!GetSoundEntriesStore()->LookupEntry(soundId)) {
    script_error_log("Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", soundId,
                     source->GetTypeId(), source->GetGUIDLow());
    return;
  }
...
SpellEntry const *ScriptedAI::SelectSpell(Unit *target, int32 school, int32 mechanic, SelectTarget selectTargets,
                                          uint32 powerCostMin, uint32 powerCostMax, float rangeMin, float rangeMax,
  ...
  for (uint8 i = 0; i < 4; ++i) {
    ...
    if (m_creature->IsWithinDistInMap(target, tempRange->minRange) ||
        !m_creature->IsWithinDistInMap(target, tempRange->maxRange))
      continue;
...
void FillSpellSummary() {
  ...
  for (uint32 i = 0; i < GetSpellStore()->GetMaxEntry(); ++i) {
    ...
    for (uint8 j = 0; j < 3; ++j) {
      ...
      if (tempSpell->Effect[j] == SPELL_EFFECT_SCHOOL_DAMAGE || tempSpell->Effect[j] == SPELL_EFFECT_INSTAKILL ||
      ...
      if (tempSpell->Effect[j] == SPELL_EFFECT_HEAL || tempSpell->Effect[j] == SPELL_EFFECT_HEAL_MAX_HEALTH ||
...
void ScriptedAI::DoTeleportPlayer(Unit *unit, float x, float y, float z, float ori) {
  ...
  if (unit->GetTypeId() != TYPEID_PLAYER) {
    script_error_log("%s tried to teleport non-player (%s) to x: %f y:%f z: %f o: %f. Aborted.",
                     m_creature->GetGuidStr().c_str(), unit->GetGuidStr().c_str(), x, y, z, ori);
    return;
  }
...
// It is assumed the information is found elswehere and can be handled by mangos. So far no luck finding such
// information/way to extract it.
enum {
  NPC_BROODLORD = 12017,
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(const uint32 diff) {
  ...
  case NPC_MALACRASS:
    ...
    break;
  default:
    script_error_log("EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition.",
                     m_creature->GetEntry());
    return false;
  }
