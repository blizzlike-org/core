...
void ScriptedInstance::DoUseDoorOrButton(ObjectGuid guid, uint32 withRestoreTime, bool useAlternativeState) {
  ...
  if (GameObject *pGo = instance->GetGameObject(guid)) {
    ...
    if (pGo->GetGoType() == GAMEOBJECT_TYPE_DOOR || pGo->GetGoType() == GAMEOBJECT_TYPE_BUTTON ||
      ...
      script_error_log("Script call DoUseDoorOrButton, but gameobject entry %u is type %u.", pGo->GetEntry(),
                       pGo->GetGoType());
  }
}
...
void ScriptedInstance::DoUseDoorOrButton(uint32 entry, uint32 withRestoreTime /*= 0*/,
  ...
  if (find != m_goEntryGuidStore.end())
    ...
    debug_log("SD2: Script call DoUseDoorOrButton(by Entry), but no gameobject of entry %u was created yet, or it was "
              "not stored by script for map %u.",
...
void ScriptedInstance::DoToggleGameObjectFlags(uint32 entry, uint32 GOflags, bool apply) {
  ...
  if (find != m_goEntryGuidStore.end())
    ...
    // Output log, possible reason is not added GO to storage, or not yet loaded
    debug_log("SD2: Script call ToogleTameObjectFlags (by Entry), but no gameobject of entry %u was created yet, or it "
...
void ScriptedInstance::DoRespawnGameObject(uint32 entry, uint32 timeToDespawn) {
  ...
  if (find != m_goEntryGuidStore.end())
    ...
    // Output log, possible reason is not added GO to storage, or not yet loaded;
    debug_log("SD2: Script call DoRespawnGameObject(by Entry), but no gameobject of entry %u was created yet, or it "
              "was not stored by script for map %u.",
              entry, instance->GetId());
}
...
GameObject *ScriptedInstance::GetSingleGameObjectFromStorage(uint32 entry) const {
  ...
  script_error_log("Script requested gameobject with entry %u, but no gameobject of this entry was created yet, or it "
                   "was not stored by script for map %u.",
                   entry, instance->GetId());
...
Creature *ScriptedInstance::GetSingleCreatureFromStorage(uint32 entry, bool skipDebugLog /*=false*/) const {
  ...
  // Output log, possible reason is not added GO to map, or not yet loaded;
  if (!skipDebugLog)
    script_error_log("Script requested creature with entry %u, but no npc of this entry was created yet, or it was not "
                     "stored by script for map %u.",
                     entry, instance->GetId());
...
/**
   ...
   @param   pDialogueArray The static const array of DialogueEntry holding the information about the dialogue. This
...
/**
   ...
   @param   pDialogueTwoSideArray The static const array of DialogueEntryTwoSide holding the information about the
   dialogue. This array MUST be terminated by {0,0,0,0,0}
 */
...
void DialogueHelper::StartNextDialogueText(int32 textEntry) {
  ...
  if (!found) {
    script_error_log("Script call DialogueHelper::StartNextDialogueText, but textEntry %i is not in provided dialogue "
                     "(on map id %u)",
