...
#define OUT_SAVE_INST_DATA                                                                                             \
  debug_log("SD2: Saving Instance Data for Instance %s (Map %d, Instance Id %d)", instance->GetMapName(),              \
            instance->GetId(), instance->GetInstanceId())
#define OUT_SAVE_INST_DATA_COMPLETE                                                                                    \
  debug_log("SD2: Saving Instance Data for Instance %s (Map %d, Instance Id %d) completed.", instance->GetMapName(),   \
            instance->GetId(), instance->GetInstanceId())
#define OUT_LOAD_INST_DATA(a)                                                                                          \
  debug_log("SD2: Loading Instance Data for Instance %s (Map %d, Instance Id %d). Input is '%s'",                      \
            instance->GetMapName(), instance->GetId(), instance->GetInstanceId(), a)
#define OUT_LOAD_INST_DATA_COMPLETE                                                                                    \
  debug_log("SD2: Instance Data Load for Instance %s (Map %d, Instance Id: %d) is complete.", instance->GetMapName(),  \
            instance->GetId(), instance->GetInstanceId())
#define OUT_LOAD_INST_DATA_FAIL                                                                                        \
  script_error_log("Unable to load Instance Data for Instance %s (Map %d, Instance Id: %d).", instance->GetMapName(),  \
                   instance->GetId(), instance->GetInstanceId())
...
public:
  ...
  // Wrapper for simulating map-wide text in this instance. It is expected that the Creature is stored in
  // m_npcEntryGuidStore if loaded.
...
public:
  ...
  /// Function to initialize the dialogue helper for instances. If not used with instances, GetSpeakerByEntry MUST be
  /// overwritten to obtain the speakers
  void InitializeDialogueHelper(ScriptedInstance *instance, bool canSimulateText = false) {
    m_instance = instance;
    m_canSimulate = canSimulateText;
  }
