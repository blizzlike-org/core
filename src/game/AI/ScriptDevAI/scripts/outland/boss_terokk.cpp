...
struct boss_terokkAI : public CombatAI {
  boss_terokkAI(Creature *creature) : CombatAI(creature, TEROKK_COMBAT_ACTION_MAX) {
    AddCustomAction(TEROKK_ACTION_SPAWN, 0u,
                    [&] { m_creature->CastSpell(nullptr, SPELL_RED_BEAM, TRIGGERED_OLD_TRIGGERED); });
    ...
    AddCombatAction(TEROKK_COMBAT_ACTION_SHADOW_BOLT_VOLLEY,
                    GetInitialActionTimer(TEROKK_COMBAT_ACTION_SHADOW_BOLT_VOLLEY));
  ...
  uint32 GetSubsequentActionTimer(TerokkActions id) const {
    ...
    case TEROKK_COMBAT_ACTION_DIVINE_SHIELD:
      return 22000;
    case TEROKK_COMBAT_ACTION_SHADOW_BOLT_VOLLEY:
      return urand(4000, 15000);
    case TEROKK_COMBAT_ACTION_CHOSEN_ONE:
      return 30000;
    case TEROKK_COMBAT_ACTION_CLEAVE:
      return urand(7000, 15000);
    default:
      return 0;
    }
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case TEROKK_COMBAT_ACTION_SPAWN_ACE:
      if (m_creature->GetHealthPercent() > 50.f)
        return;
      m_creature->SummonCreature(NPC_ACE, -3335.223f, 3431.947f, 426.3864f, 0.f, TEMPSPAWN_MANUAL_DESPAWN, 0, true,
                                 true);
      m_creature->SummonCreature(NPC_ACE, -3335.223f, 3431.947f, 426.3864f, 0.f, TEMPSPAWN_MANUAL_DESPAWN, 0, true,
                                 true);
      m_creature->SummonCreature(NPC_ACE, -3335.223f, 3431.947f, 426.3864f, 0.f, TEMPSPAWN_MANUAL_DESPAWN, 0, true,
...
bool ProcessEventId_event_summon_terokk(uint32 /*eventId*/, Object *source, Object * /*pTarget*/, bool /*isStart*/) {
  ...
  if (player->GetMap()->SpawnedCountForEntry(NPC_TEROKK) == 0)
    player->SummonCreature(NPC_TEROKK, -3788.856f, 3507.526f, 286.8846f, 3.159046f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                           600000, true, false, 0, 0, 0, true);
  return true;
}
