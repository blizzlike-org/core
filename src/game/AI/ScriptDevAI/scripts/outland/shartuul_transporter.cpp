...
struct npc_doomguard_punisherAI : public CombatAI {
  ...
  void JustPreventedDeath(Unit *killer) override { SetActionReadyStatus(PUNISHER_POSSESSION_TRANSFER, true); }
...
struct npc_shivan_assassinAI : public CombatAI {
  ...
  void EnterEvadeMode() override { CombatAI::EnterEvadeMode(); }
  ...
  void JustPreventedDeath(Unit *killer) override { SetActionReadyStatus(SHIVAN_POSSESSION_TRANSFER, true); }
...
struct npc_shartuulAI : public RangedCombatAI {
  npc_shartuulAI(Creature *creature)
      : RangedCombatAI(creature, SHARTUUL_ACTION_MAX),
...
struct MadnessRift : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override {
    ...
    static_cast<Creature *>(target)->SetFactionTemporary(SHARTUUL_FACTION_HOSTILE,
                                                         TEMPFACTION_RESTORE_RESPAWN | TEMPFACTION_RESTORE_REACH_HOME);
  }
};
