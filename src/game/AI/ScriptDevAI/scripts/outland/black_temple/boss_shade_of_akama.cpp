...
static const uint32 auiRandSpawnEntry[] = {NPC_ASH_ELEMENTAL, NPC_ASH_ROGUE, NPC_ASH_SPIRITBIND};
...
static const Location brokenSpawnLocations[] = {
    // Broken
    {501.5699f, 337.8271f, 112.8673f}, {499.0937f, 337.9894f, 112.8673f}, {496.8722f, 338.0152f, 112.8673f},
    {532.5723f, 344.6454f, 112.8671f}, {532.5565f, 346.8792f, 112.8671f}, {532.5491f, 348.6840f, 112.8671f},
    {521.8465f, 393.1343f, 112.8671f}, {521.9014f, 395.6381f, 112.8671f}, {522.2610f, 397.7423f, 112.8671f},
    {522.2963f, 402.3578f, 112.8671f}, {521.9184f, 404.6848f, 112.8671f}, {522.4290f, 406.5160f, 112.8671f},
    {495.9472f, 463.3625f, 112.8673f}, {498.3421f, 463.8384f, 112.8673f},
};

static const Location brokenDestinationLocations[] = {
    {476.2499f, 369.0865f, 112.7838f}, {473.3224f, 368.4310f, 112.7838f}, {479.1774f, 369.7420f, 112.7838f},
    {491.9286f, 383.0433f, 112.7838f}, {490.0695f, 380.6888f, 112.7838f}, {493.1895f, 385.7655f, 112.7838f},
    {488.3535f, 395.3652f, 112.7838f}, {487.5005f, 392.4891f, 112.7838f}, {488.5529f, 398.3586f, 112.7838f},
    {491.8618f, 403.2035f, 112.7838f}, {491.7784f, 400.2046f, 112.7838f}, {491.9451f, 406.2023f, 112.7838f},
    {479.1884f, 434.8635f, 112.7839f}, {478.1014f, 433.3450f, 112.7839f},
};
...
struct npc_akamaAI : public ScriptedAI, public CombatActions, private DialogueHelper {
  npc_akamaAI(Creature *creature)
      : ScriptedAI(creature), CombatActions(AKAMA_ACTION_MAX), DialogueHelper(aOutroDialogue) {
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit *sender, Unit * /*invoker*/, uint32 /*miscValue*/) override {
    if (eventType == AI_EVENT_CUSTOM_A) {
      ...
      case NPC_SHADE_OF_AKAMA:
        ...
        m_creature->GetMotionMaster()->MovePoint(PHASE_EPILOGUE, akamaWaypoints[2].x, akamaWaypoints[2].y,
                                                 akamaWaypoints[2].z);
        break;
      case NPC_ASH_CHANNELER:
        // Move the shade to Akama when all channelers are dead
        // Note: the boss should be already slowly moving, but this isn't possible because of the missing stack for the
        // speed debuff
        break;
      }
  ...
  void DoStartEvent() {
    ...
    m_creature->GetMotionMaster()->MovePoint(PHASE_CHANNEL, akamaWaypoints[0].x, akamaWaypoints[0].y,
                                             akamaWaypoints[0].z);
  }
  ...
  void DoSummonBrokenAshtongue() {
    ...
    for (auto loc : brokenSpawnLocations) {
      if (Creature *pBroken = m_creature->SummonCreature(NPC_ASH_BROKEN, loc.x, loc.y, loc.z, 0,
                                                         TEMPSPAWN_TIMED_DESPAWN, 10 * MINUTE * IN_MILLISECONDS))
        pBroken->GetMotionMaster()->MovePoint(1, brokenDestinationLocations[i].x, brokenDestinationLocations[i].y,
                                              brokenDestinationLocations[i].z);
      ++i;
    }
  }
...
bool GossipHello_npc_akama(Player *pPlayer, Creature *pCreature) {
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if (pInstance->GetData(TYPE_SHADE) != DONE)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_START_ENCOUNTER, GOSSIP_SENDER_MAIN,
...
struct boss_shade_of_akamaAI : public ScriptedAI {
  boss_shade_of_akamaAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_black_temple *>(creature->GetInstanceData())) {
    Reset();
  }
  ...
  void Aggro(Unit * /*enemy*/) override { m_creature->HandleEmote(0); }
...
struct npc_creature_generatorAI : public ScriptedAI, public TimerManager {
  npc_creature_generatorAI(Creature *creature)
      : ScriptedAI(creature), m_spawn(false), m_left(creature->GetPositionY() > 400.f) {
    m_instance = static_cast<instance_black_temple *>(creature->GetInstanceData());
  ...
  void JustSummoned(Creature *summoned) override {
    ...
    case NPC_ASH_DEFENDER: {
      ...
      if (Creature *akama = m_instance->GetSingleCreatureFromStorage(NPC_AKAMA_SHADE)) {
        float x, y, z;
        akama->GetNearPoint(summoned, x, y, z, summoned->GetObjectBoundingRadius(),
                            akama->GetCombinedCombatReach(summoned, true), akama->GetAngle(summoned));
        summoned->GetMotionMaster()->MovePoint(POINT_DESTINATION, x, y, z);
      }
      break;
    }
...
UnitAI *GetAI_npc_akama_shade(Creature *creature) { return new npc_akamaAI(creature); }

UnitAI *GetAI_boss_shade_of_akama(Creature *creature) { return new boss_shade_of_akamaAI(creature); }

UnitAI *GetAI_mob_ashtongue_channeler(Creature *creature) { return new mob_ashtongue_channelerAI(creature); }

UnitAI *GetAI_mob_ashtongue_sorcerer(Creature *creature) { return new mob_ashtongue_sorcererAI(creature); }

UnitAI *GetAI_npc_creature_generator(Creature *creature) { return new npc_creature_generatorAI(creature); }
