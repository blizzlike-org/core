...
struct boss_teron_gorefiendAI : public ScriptedAI, public CombatActions {
  boss_teron_gorefiendAI(Creature *creature)
      : ScriptedAI(creature), CombatActions(GOREFIEND_ACTION_MAX), m_introDone(false) {
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float x, float y, float z) { return x < 516.8f && y > 402.7f; });
    Reset();
  }
  ...
  uint32 GetSubsequentActionTimer(GorefiendActions id) {
    ...
    case GOREFIEND_ACTION_DOOM_BLOSSOM:
      return 35000;
    case GOREFIEND_ACTION_INCINERATE:
      return urand(20000, 50000);
    case GOREFIEND_ACTION_SHADOW_OF_DEATH:
      return urand(30000, 50000);
    case GOREFIEND_ACTION_CRUSHING_SHADOWS:
      return urand(10000, 26000);
    default:
      return 0;
    }
  }
  ...
  void OnSpellCooldownAdded(SpellEntry const *spellInfo) override {
    if (spellInfo->Id == SPELL_SHADOW_OF_DEATH) {
      ResetTimer(GOREFIEND_ACTION_SHADOW_OF_DEATH,
                 GetSubsequentActionTimer(GorefiendActions(GOREFIEND_ACTION_SHADOW_OF_DEATH)));
  ...
  void ExecuteActions() {
    ...
    for (uint32 i = 0; i < GOREFIEND_ACTION_MAX; ++i) {
      if (GetActionReadyStatus(i)) {
        ...
        case GOREFIEND_ACTION_SHADOW_OF_DEATH: {
          if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOW_OF_DEATH,
                                                               SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_AURA |
                                                                   SELECT_FLAG_SKIP_TANK)) {
...
struct npc_doom_blossomAI : public ScriptedAI, public TimerManager {
  npc_doom_blossomAI(Creature *creature) : ScriptedAI(creature) {
    AddCustomAction(0, true, [&] {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOW_BOLT, SELECT_FLAG_PLAYER))
  ...
  void JustSummoned(Creature *summoned) override {
    m_creature->GetMotionMaster()->MovePoint(1, summoned->GetPositionX(), summoned->GetPositionY(),
...
struct npc_shadow_constructAI : public ScriptedAI, public TimerManager {
  npc_shadow_constructAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_black_temple *>(creature->GetMap()->GetInstanceData())) {
  ...
  void Reset() override { m_atrophyTimer = 2500; }
...
struct ShadowOfDeath : public AuraScript {
  void OnAbsorb(Aura * /*aura*/, int32 &currentAbsorb, uint32 & /*reflectedSpellId*/, int32 & /*reflectDamage*/,
                bool &preventedDeath) const override {
    preventedDeath = true;
    currentAbsorb = 0;
  }
...
struct SummonBlossomMoveTarget : public SpellScript {
  void OnDestTarget(Spell *spell) const override { spell->m_targets.m_destPos.z += 12.f; }
};
