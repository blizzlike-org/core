...
struct boss_gurtogg_bloodboilAI : public ScriptedAI, public CombatActions {
  boss_gurtogg_bloodboilAI(Creature *creature)
      : ScriptedAI(creature), CombatActions(GURTOGG_ACTION_MAX),
    ...
    m_creature->GetCombatManager().SetLeashingCheck([](Unit *, float, float y, float) { return y < 140.f; });
    Reset();
  }
    ...
    for (uint32 i = 0; i < GURTOGG_ACTION_MAX; ++i) {
      if (GetActionReadyStatus(i)) {
        ...
        case GURTOGG_ACTION_FEL_ACID: {
          ...
          if (m_creature->getThreatManager().getThreatList().size() >=
              2) // when at least 2 players skip 2 topaggro and main tank
          {
            flags |= SELECT_FLAG_SKIP_TANK;
            exclusionCount = 2;
          }
          Unit *target = m_phase1 ? m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, exclusionCount,
                                                                      SPELL_FEL_ACID_1, flags)
