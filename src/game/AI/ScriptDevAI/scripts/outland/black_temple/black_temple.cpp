...
void instance_black_temple::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }

void instance_black_temple::OnPlayerEnter(Player * /*pPlayer*/) { DoTeleportAkamaIfCan(); }
...
void instance_black_temple::OnObjectCreate(GameObject *go) {
  ...
  case GO_PRE_SHAHRAZ_DOOR: // Door leading to Mother Shahraz
    if (m_auiEncounter[TYPE_SHADE] == DONE && m_auiEncounter[TYPE_GOREFIEND] == DONE &&
        m_auiEncounter[TYPE_BLOODBOIL] == DONE && m_auiEncounter[TYPE_RELIQUIARY] == DONE)
      go->SetGoState(GO_STATE_ACTIVE);
    break;
...
void instance_black_temple::SetData(uint32 type, uint32 data) {
  ...
  case TYPE_ILLIDAN:
    ...
    break;
  default:
    script_error_log("Instance Black Temple: ERROR SetData = %u for type %u does not exist/not implemented.", type,
                     data);
    return;
  }
...
void instance_black_temple::DoOpenPreMotherDoor() {
  if (GetData(TYPE_SHADE) == DONE && GetData(TYPE_GOREFIEND) == DONE && GetData(TYPE_BLOODBOIL) == DONE &&
      GetData(TYPE_RELIQUIARY) == DONE) {
...
void instance_black_temple::Load(const char *chrIn) {
  ...
  loadStream >> m_auiEncounter[0] >> m_auiEncounter[1] >> m_auiEncounter[2] >> m_auiEncounter[3] >> m_auiEncounter[4] >>
      m_auiEncounter[5] >> m_auiEncounter[6] >> m_auiEncounter[7] >> m_auiEncounter[8];
...
InstanceData *GetInstanceData_instance_black_temple(Map *map) { return new instance_black_temple(map); }
