...
enum {
  ...
  SPELL_TARGET_RANDOM_LOCATION =
      40118, // Used to select a random location within 15.00 yards around the volcano - DUMMY - Is the visual
 ...
 * Statet at wowwiki: "If the gaze target is further away than 40 yards, he dashes at about five times the normal run
 * speed until the range is about 20 yards."
...
struct boss_supremusAI : public ScriptedAI, CombatActions {
  boss_supremusAI(Creature *creature)
      : ScriptedAI(creature), CombatActions(SUPREMUS_ACTION_MAX),
...
struct SupremusRandomTarget : public AuraScript {
  void OnApply(Aura *aura, bool apply) const override {
    ...
    if (apply) {
      ...
      caster->AI()->AttackStart(
          aura->GetTarget()); // visual change for immediate crosshair update - only safe with fixate
