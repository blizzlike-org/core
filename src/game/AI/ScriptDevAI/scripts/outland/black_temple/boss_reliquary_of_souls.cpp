...
struct boss_reliquary_of_soulsAI : public Scripted_NoMovementAI, public TimerManager {
  boss_reliquary_of_soulsAI(Creature *pCreature)
    ...
    AddCustomAction(RELIQUARY_ACTION_SUBMERGE, 0u, [&] {
      uint32 spellId = 0;
      switch (m_phase) {
      case PHASE_1_SUFFERING:
        spellId = SPELL_SUMMON_ESSENCE_SUFFERING;
        break;
      case PHASE_2_DESIRE:
        spellId = SPELL_SUMMON_ESSENCE_DESIRE;
        break;
      case PHASE_3_ANGER:
        spellId = SPELL_SUMMON_ESSENCE_ANGER;
        break;
      default:
        return;
      }
  ...
  void MoveInLineOfSight(Unit *who) override {
    if (m_phase == PHASE_0_NOT_BEGUN && who->GetTypeId() == TYPEID_PLAYER &&
        !static_cast<Player *>(who)->IsGameMaster() &&
...
struct essence_base_AI : public ScriptedAI, public CombatActions {
  essence_base_AI(Creature *creature, uint32 maxActions)
      : ScriptedAI(creature), CombatActions(maxActions),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    SetDeathPrevention(true);
    SetReactState(REACT_DEFENSIVE);
    m_bIsPhaseFinished = false;
    AddCustomAction(ESSENCE_GENERIC_ACTION_ATTACK, 3500u, [&]() { m_creature->SetInCombatWithZone(); });
  }
  ...
  void JustPreventedDeath(Unit * /*attacker*/) override {
    ...
    if (Creature *pReliquary = m_instance->GetSingleCreatureFromStorage(NPC_RELIQUARY_OF_SOULS))
      m_creature->GetMotionMaster()->MovePoint(1, pReliquary->GetPositionX(), pReliquary->GetPositionY(),
                                               pReliquary->GetPositionZ(), FORCED_MOVEMENT_RUN);
...
struct boss_essence_of_sufferingAI : public essence_base_AI {
  ...
  void OnPhaseFinished() { DoScriptText(SUFF_SAY_RECAP, m_creature); }
...
struct boss_essence_of_desireAI : public essence_base_AI {
  ...
  uint32 GetSubsequentActionTimer(DesireActions id) {
    ...
    case DESIRE_ACTION_DEADEN:
      return 30000;
    case DESIRE_ACTION_SPIRIT_SHOCK:
      return 2000; // chain cast during tbc
    default:
      return 0;
    }
  }
  ...
  void OnPhaseFinished() { DoScriptText(DESI_SAY_RECAP, m_creature); }

  void DamageTaken(Unit *dealer, uint32 &damage, DamageEffectType damagetype, SpellEntry const *spellInfo) override {
    int32 damageTaken = ((int32)damage) / 2;
    if (dealer)
      dealer->CastCustomSpell(dealer, SPELL_AURA_OF_DESIRE_SELF_DMG, &damageTaken, nullptr, nullptr,
...
struct boss_essence_of_angerAI : public ScriptedAI, public CombatActions {
  boss_essence_of_angerAI(Creature *pCreature)
      : ScriptedAI(pCreature), CombatActions(ANGER_ACTION_MAX),
  ...
  void JustDied(Unit * /*pKiller*/) override { DoScriptText(ANGER_SAY_DEATH, m_creature); }
...
struct npc_enslaved_soulAI : public ScriptedAI, TimerManager {
  ...
  void JustRespawned() override { DoCastSpellIfCan(m_creature, SPELL_ENSLAVED_SOUL_PASSIVE); }

  void JustDied(Unit * /*killer*/) override {
    DoCastSpellIfCan(m_creature, SPELL_SOUL_RELEASE, CAST_TRIGGERED);

    // Notify the main boss about the spirit death. Needs to be done here, because the spirit is summoned with triggered
    // spell
    if (m_instance) {
      if (Creature *reliquary = m_instance->GetSingleCreatureFromStorage(NPC_RELIQUARY_OF_SOULS)) {
...
struct npc_reliquary_LOS_aggro_triggerAI : ScriptedAI {
  npc_reliquary_LOS_aggro_triggerAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetMap()->GetInstanceData())) {
...
UnitAI *GetAI_boss_reliquary_of_souls(Creature *pCreature) { return new boss_reliquary_of_soulsAI(pCreature); }

UnitAI *GetAI_boss_essence_of_suffering(Creature *pCreature) { return new boss_essence_of_sufferingAI(pCreature); }

UnitAI *GetAI_boss_essence_of_desire(Creature *pCreature) { return new boss_essence_of_desireAI(pCreature); }

UnitAI *GetAI_boss_essence_of_anger(Creature *pCreature) { return new boss_essence_of_angerAI(pCreature); }

UnitAI *GetAI_npc_enslaved_soul(Creature *pCreature) { return new npc_enslaved_soulAI(pCreature); }
