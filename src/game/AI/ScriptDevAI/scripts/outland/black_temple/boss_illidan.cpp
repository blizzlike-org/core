...
enum {
  ...
  TEXT_ID_AKAMA_ILLIDAN_PREPARE =
      10465, // ToDo: fix text id - this entry is wrong -> "The time has come to face Illidan, $N. Are you ready?"
  ...
  SPELL_FLAME_CRASH =
      40832,               // Summons an invis/unselect passive mob that has an uiAura of flame in a circle around him.
  SPELL_DRAW_SOUL = 40904, // 5k Shadow Damage in front of him. Heals Illidan for 100k health (script effect)
  SPELL_PARASITIC_SHADOWFIEND = 41917, // DoT of 3k Shadow every 2 seconds. Lasts 10 seconds. (Script effect: Summon 2
                                       // parasites once the debuff has ticked off)
  // SPELL_SUMMON_PARASITICS       = 41915,               // Summons 2 Parasitic Shadowfiends on the target. Handled in
  // core.
  ...
  SPELL_SHADOW_BLAST = 41078, // 8k - 11k Shadow Damage. Targets highest threat. Has a splash effect, damaging anyone in
                              // 20 yards of the target.
  ...
  // SPELL_DEMON_FIRE             = 40029,                // Blue fire trail left by Eye Blast. Deals 2k per second if
  // players stand on it.
  ...
  // Blade of Azzinoth
  SPELL_RANGE_MARKER =
      41997, // Dummy effect used by the Blade of Azzinoth to check the range of the Azzinoth flame - needs core support
...
static const Locations aIllidariElitesPos[MAX_ILLIDARI_ELITES] = {
    {743.9686f, 289.6447f, 311.1807f}, {753.8425f, 286.562f, 310.9353f},  {745.2552f, 322.1574f, 310.4596f},
    {745.3237f, 283.986f, 309.2765f},  {750.0472f, 282.3274f, 309.4353f}, {747.0576f, 326.4268f, 309.0688f},
    {751.0878f, 327.6505f, 309.4576f}, {748.8422f, 288.062f, 310.9782f},  {750.0322f, 323.6064f, 310.2757f},
...
struct boss_illidan_stormrageAI : public CombatAI, private DialogueHelper {
  boss_illidan_stormrageAI(Creature *creature)
      : CombatAI(creature, ILLIDAN_ACTIONS_MAX), DialogueHelper(aEventDialogue),
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit * /*invoker*/, uint32 miscValue) override {
    ...
    } else if (eventType == AI_EVENT_CUSTOM_E) // End of Eye Blast
    {
      ...
      m_creature->GetMotionMaster()->MovePoint(POINT_ILLIDAN_FLIGHT_RANDOM, illidanFlightPos[m_curEyeBlastLoc].fX,
                                               illidanFlightPos[m_curEyeBlastLoc].fY,
                                               illidanFlightPos[m_curEyeBlastLoc].fZ, FORCED_MOVEMENT_RUN);
  ...
  void HandlePhaseTransition() {
    ...
    case PHASETRANSITION_LIFTOFF: {
      ...
      case 2: // picks closest trigger out of the 3 and flies towards it
      {
        ...
        for (ObjectGuid guid : lowerTriggers) {
          if (Creature *trigger = m_creature->GetMap()->GetCreature(guid)) {
            if (closestTrigger) {
              if (closestTrigger->GetDistance(m_creature, true, DIST_CALC_NONE) >
                  trigger->GetDistance(m_creature, true, DIST_CALC_NONE))
      ...
      case 4: {
        if (m_instance) {
          ...
          //{
          //    if (m_creature->GetDistance(illidanFlightPos[firstEyeBlastPos].fX,
          //    illidanFlightPos[firstEyeBlastPos].fY, illidanFlightPos[firstEyeBlastPos].fZ, DIST_CALC_NONE)
          //        > m_creature->GetDistance(illidanFlightPos[i].fX, illidanFlightPos[i].fY, illidanFlightPos[i].fZ,
          //        DIST_CALC_NONE))
          //    {
          //        if (m_creature->GetDistance(illidanFlightPos[secondEyeBlastPos].fX,
          //        illidanFlightPos[secondEyeBlastPos].fY, illidanFlightPos[secondEyeBlastPos].fZ, DIST_CALC_NONE)
          //            > m_creature->GetDistance(illidanFlightPos[firstEyeBlastPos].fX,
          //            illidanFlightPos[firstEyeBlastPos].fY, illidanFlightPos[firstEyeBlastPos].fZ, DIST_CALC_NONE))
          //        {
          ...
          //        firstEyeBlastPos = i;
          //    }
          //    else if (m_creature->GetDistance(illidanFlightPos[secondEyeBlastPos].fX,
          //    illidanFlightPos[secondEyeBlastPos].fY, illidanFlightPos[secondEyeBlastPos].fZ, DIST_CALC_NONE)
          //        > m_creature->GetDistance(illidanFlightPos[i].fX, illidanFlightPos[i].fY, illidanFlightPos[i].fZ,
          //        DIST_CALC_NONE))
          //    {
    ...
    case PHASETRANSITION_LAND: {
      ...
      case 0: {
        m_creature->SetImmobilizedState(false);
        m_creature->SetTarget(nullptr);
        m_creature->GetMotionMaster()->MovePoint(POINT_ILLIDAN_LANDING, aCenterLoc[0].fX, aCenterLoc[0].fY,
                                                 aCenterLoc[0].fZ, FORCED_MOVEMENT_RUN);
        break;
      }
      case 1: {
        // Despawn the blades
  ...
  bool DoCastEyeBlastIfCan() {
    ...
    m_creature->SummonCreature(NPC_ILLIDAN_TARGET, aEyeBlastPos[m_curEyeBlastLoc].fX, aEyeBlastPos[m_curEyeBlastLoc].fY,
                               aEyeBlastPos[m_curEyeBlastLoc].fZ, 0, TEMPSPAWN_TIMED_DESPAWN, 15000, true, true,
                               m_curEyeBlastLoc + 1);
    return true;
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ILLIDAN_ACTION_SHADOW_FIEND: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_PARASITIC_SHADOWFIEND,
                                                           SELECT_FLAG_PLAYER))
...
struct npc_akama_illidanAI : public CombatAI, private DialogueHelper {
  npc_akama_illidanAI(Creature *creature)
      : CombatAI(creature, AKAMA_ACTION_MAX), DialogueHelper(aIntroDialogue),
        m_instance(static_cast<instance_black_temple *>(creature->GetInstanceData())),
        m_akamaStage(AKAMA_STAGE_ILLIDAN), m_outroStage(0) {
    ...
    AddCustomAction(AKAMA_SUMMON_ILLIDARI, true, [&]() {
      for (const Locations &aIllidariElitesPo : aIllidariElitesPos)
        m_creature->SummonCreature(NPC_ILLIDARI_ELITE, aIllidariElitesPo.fX, aIllidariElitesPo.fY, aIllidariElitesPo.fZ,
                                   0, TEMPSPAWN_DEAD_DESPAWN, 0);
    ...
    AddCustomAction(AKAMA_OUTRO_ACTIONS, true, [&]() { HandleOutro(); });
    InitializeDialogueHelper(m_instance);
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case NPC_SPIRIT_OF_OLUM:
      m_creature->SummonCreature(NPC_SPIRIT_OF_OLUM, 751.6437f, 297.2233f, 312.2083f, 6.03f, TEMPSPAWN_TIMED_DESPAWN,
                                 50000);
      m_creature->SummonCreature(NPC_SPIRIT_OF_UDALO, 751.4565f, 311.0107f, 312.19f, 0.0f, TEMPSPAWN_TIMED_DESPAWN,
                                 50000);
      break;
...
bool GossipHello_npc_akama_illidan(Player *player, Creature *creature) {
  ...
  // Before starting combat
    ...
    player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_START_EVENT, GOSSIP_SENDER_MAIN,
...
struct boss_maievAI : public CombatAI, private DialogueHelper {
  boss_maievAI(Creature *creature)
      : CombatAI(creature, MAIEV_ACTION_MAX), DialogueHelper(aEpilogueDialogue),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())), m_outro(false) {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case MAIEV_ACTION_TAUNT: {
      switch (urand(0, 2)) {
      case 0:
        DoScriptText(SAY_MAIEV_TAUNT_1, m_creature);
        break;
      case 1:
        DoScriptText(SAY_MAIEV_TAUNT_2, m_creature);
        break;
      case 2:
        DoScriptText(SAY_MAIEV_TAUNT_3, m_creature);
        break;
      }
      ResetCombatAction(action, GetSubsequentActionTimer(MaievActions(action)));
      return;
...
struct npc_flame_of_azzinothAI : public CombatAI {
  npc_flame_of_azzinothAI(Creature *creature)
      : CombatAI(creature, FLAME_ACTION_MAX),
        m_instance(static_cast<instance_black_temple *>(creature->GetInstanceData())) {
  ...
  Unit *DoPickChargeTarget() {
    ...
    for (ThreatList::const_iterator itr = threatlist.begin(); itr != threatlist.end(); ++itr) {
      if (Unit *target = m_creature->GetMap()->GetUnit((*itr)->getUnitGuid())) {
        ...
        if ((!first || first->GetDistance(x, y, z, DIST_CALC_COMBAT_REACH) > 30.f) &&
            (!second || second->GetDistance(x, y, z, DIST_CALC_COMBAT_REACH) > 30.f))
          return target;
      }
    }
    return nullptr;
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case FLAME_ACTION_SUMMON_BLAZE: {
      if (Unit *target = m_creature->GetVictim())
        if (target->CastSpell(nullptr, SPELL_BLAZE, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                              m_creature->GetObjectGuid()) == SPELL_CAST_OK)
...
struct npc_shadow_demonAI : public ScriptedAI {
  ...
  void FindNewTarget() {
    ...
    if (illidan && illidan->GetTypeId() == TYPEID_UNIT) {
      if (Unit *target = static_cast<Creature *>(illidan)->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                                                 SPELL_PARALYZE, SELECT_FLAG_PLAYER)) {
...
struct npc_blade_of_azzinothAI : public ScriptedAI {
  npc_blade_of_azzinothAI(Creature *creature)
  ...
  void SummonedCreatureJustDied(Creature *summoned) override {
    ...
    if (summoned->GetEntry() == NPC_FLAME_OF_AZZINOTH) {
      ...
      // For some reason it doesn't work with Spell Hit for SPELL_GLAIVE_RETURNS script effect, so we need to inform him
      // manually
      if (Creature *illidan = m_instance->GetSingleCreatureFromStorage(NPC_ILLIDAN_STORMRAGE)) {
        if (boss_illidan_stormrageAI *illidanAI = dynamic_cast<boss_illidan_stormrageAI *>(illidan->AI()))
...
struct npc_parasitic_shadowfiendAI : public ScriptedAI, public TimerManager {
  npc_parasitic_shadowfiendAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    ...
    AddCustomAction(1, 2000u, [&]() {
      if (Creature *illidan = m_instance->GetSingleCreatureFromStorage(NPC_ILLIDAN_STORMRAGE)) {
        ...
        if (Unit *target = illidan->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
                                                          SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_CUSTOM, m_params)) {
...
bool GOUse_go_cage_trap(Player * /*player*/, GameObject *go) {
  ...
  if (trapTrigger) {
    ...
    if (illidan && static_cast<boss_illidan_stormrageAI *>(illidan->AI())->m_phase != PHASE_4_DEMON &&
        !static_cast<boss_illidan_stormrageAI *>(illidan->AI())->GetCombatScriptStatus()) {
...
UnitAI *GetAI_boss_illidan_stormrage(Creature *creature) { return new boss_illidan_stormrageAI(creature); }

UnitAI *GetAI_npc_akama_illidan(Creature *creature) { return new npc_akama_illidanAI(creature); }

UnitAI *GetAI_boss_maiev(Creature *creature) { return new boss_maievAI(creature); }

UnitAI *GetAI_mob_flame_of_azzinoth(Creature *creature) { return new npc_flame_of_azzinothAI(creature); }

UnitAI *GetAI_npc_cage_trap_trigger(Creature *creature) { return new npc_cage_trap_triggerAI(creature); }

UnitAI *GetAI_npc_shadow_demon(Creature *creature) { return new npc_shadow_demonAI(creature); }

UnitAI *GetAI_npc_blade_of_azzinoth(Creature *creature) { return new npc_blade_of_azzinothAI(creature); }

UnitAI *GetAI_npc_parasitic_shadowfiend(Creature *creature) { return new npc_parasitic_shadowfiendAI(creature); }
