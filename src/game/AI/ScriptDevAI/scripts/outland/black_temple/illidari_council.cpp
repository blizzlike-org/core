...
struct mob_blood_elf_council_voice_triggerAI : public ScriptedAI {
  mob_blood_elf_council_voice_triggerAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<ScriptedInstance *>(m_creature->GetInstanceData())),
  ...
  void Reset() override { m_uiEnrageTimer = 0; }
  ...
  void StartVoiceEvent() {
    ...
    case 0:
      ...
      break;
    case 1:
      m_councilDialogue.StartNextDialogueText(SAY_VERA_AGGRO);
      break;
    case 2:
      m_councilDialogue.StartNextDialogueText(SAY_MALA_AGGRO);
      break;
    case 3:
      m_councilDialogue.StartNextDialogueText(SAY_ZERE_AGGRO);
      break;
    }
...
struct mob_illidari_councilAI : public ScriptedAI, public TimerManager {
  mob_illidari_councilAI(Creature *creature)
  ...
  void DoStartEvent() {
    ...
    if (Creature *voiceTrigger = m_instance->GetSingleCreatureFromStorage(NPC_COUNCIL_VOICE)) {
      if (mob_blood_elf_council_voice_triggerAI *voiceAI =
              dynamic_cast<mob_blood_elf_council_voice_triggerAI *>(voiceTrigger->AI()))
...
struct boss_illidari_councilAI : public CombatAI {
  boss_illidari_councilAI(Creature *creature, uint32 combatActions)
      : CombatAI(creature, combatActions), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {}
  ...
  void DamageTaken(Unit * /*dealer*/, uint32 &damage, DamageEffectType /*damagetype*/,
    ...
    m_creature->CastCustomSpell(nullptr, SPELL_SHARED_RULE_DAM, &damageTaken, nullptr, nullptr,
...
struct boss_gathios_the_shattererAI : public boss_illidari_councilAI {
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(SAY_GATH_SLAY, m_creature); }
  ...
  Creature *DoSelectCouncilMember() {
    if (Creature *veras = m_instance->GetSingleCreatureFromStorage(NPC_VERAS)) {
      ...
      switch (random) {
      case 0:
        return m_instance->GetSingleCreatureFromStorage(NPC_ZEREVOR);
      case 1:
        return m_instance->GetSingleCreatureFromStorage(NPC_LADY_MALANDE);
      case 2:
        return veras;
      }
    }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case GATHIOS_ACTION_HAMMER_OF_JUSTICE: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_HAMMER_OF_JUSTICE,
...
struct boss_high_nethermancer_zerevorAI : public boss_illidari_councilAI {
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(SAY_ZERE_SLAY, m_creature); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ZEREVOR_ACTION_ARCANE_EXPLOSION: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_ARCANE_EXPLOSION,
...
struct boss_lady_malandeAI : public boss_illidari_councilAI {
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(SAY_MALA_SLAY, m_creature); }
...
struct boss_veras_darkshadowAI : public boss_illidari_councilAI {
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(SAY_VERA_SLAY, m_creature); }
...
UnitAI *GetAI_mob_illidari_council(Creature *creature) { return new mob_illidari_councilAI(creature); }

UnitAI *GetAI_boss_gathios_the_shatterer(Creature *creature) { return new boss_gathios_the_shattererAI(creature); }

UnitAI *GetAI_boss_lady_malande(Creature *creature) { return new boss_lady_malandeAI(creature); }

UnitAI *GetAI_boss_veras_darkshadow(Creature *creature) { return new boss_veras_darkshadowAI(creature); }
