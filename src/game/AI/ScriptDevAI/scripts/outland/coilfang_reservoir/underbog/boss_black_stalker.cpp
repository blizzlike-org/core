...
struct boss_black_stalkerAI : public CombatAI {
  boss_black_stalkerAI(Creature *creature)
      : CombatAI(creature, BLACK_STALKER_ACTION_MAX), m_isRegularMode(creature->GetMap()->IsRegularDifficulty()) {
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit * /*unit*/, float x, float y, float /*z*/) -> bool { return x < 100.0f || y < -30.0f; });
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case BLACK_STALKER_ACTION_CHAIN_LIGHTNING: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CHAIN_LIGHTNING, SELECT_FLAG_PLAYER))
