...
struct boss_hungarfenAI : public CombatAI {
  boss_hungarfenAI(Creature *creature)
      : CombatAI(creature, HUNGARFEN_ACTION_MAX), m_bIsRegularMode(creature->GetMap()->IsRegularDifficulty()) {
  ...
  void JustDied(Unit * /*killer*/) override { DoCastSpellIfCan(nullptr, SPELL_DESPAWN_MUSHROOMS, CAST_TRIGGERED); }

  void JustReachedHome() override { DoCastSpellIfCan(nullptr, SPELL_DESPAWN_MUSHROOMS, CAST_TRIGGERED); }
