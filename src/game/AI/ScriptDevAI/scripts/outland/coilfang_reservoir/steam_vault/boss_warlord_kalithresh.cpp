...
struct boss_warlord_kalithreshAI : public ScriptedAI, public CombatActions {
  boss_warlord_kalithreshAI(Creature *creature) : ScriptedAI(creature), CombatActions(WARLORD_KALITHRESH_ACTION_MAX) {
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float x, float y, float z) { return x < -95.7f && y > -439.6f; });
    Reset();
    Reset();
  }
  ...
  uint32 GetSubsequentActionTimer(const uint32 action) const {
    switch (action) {
    case WARLORD_KALITHRESH_ACTION_WARLORDS_RAGE:
      return urand(35000, 45000);
    case WARLORD_KALITHRESH_ACTION_REFLECTION:
      return urand(24000, 35000);
    case WARLORD_KALITHRESH_ACTION_IMPALE:
      return urand(7500, 12500);
    case WARLORD_KALITHRESH_ACTION_HEAD_CRACK:
      return urand(45000, 58000);
    default:
      return 0; // never occurs but for compiler
    }
  }
      ...
      if (GetActionReadyStatus(i)) {
        ...
        case WARLORD_KALITHRESH_ACTION_WARLORDS_RAGE: {
          ...
          if (Creature *Distiller = GetClosestCreatureWithEntry(m_creature, NPC_NAGA_DISTILLER, 100.0f)) {
            ...
            m_creature->SetWalk(
                false, true); // Prevent him from slowing down while meleehit/casted upon while starting to move
...
struct mob_naga_distillerAI : public Scripted_NoMovementAI {
  ...
  void Reset() override { m_creature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE); }
...
UnitAI *GetAI_boss_warlord_kalithresh(Creature *pCreature) { return new boss_warlord_kalithreshAI(pCreature); }

UnitAI *GetAI_mob_naga_distiller(Creature *pCreature) { return new mob_naga_distillerAI(pCreature); }
