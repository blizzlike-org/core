...
struct boss_the_lurker_belowAI : public CombatAI {
  ...
  void JustSummoned(Creature *summoned) override { summoned->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SWIMMING); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case LURKER_SUBMERGE_PHASE: {
      ...
      if (DoCastSpellIfCan(nullptr, SPELL_SUBMERGE) == CAST_OK) {
        ...
        m_creature->SetStandState(UNIT_STAND_STATE_CUSTOM); // TODO investigate Submerge visual spell to see if it
                                                            // shouldnt do this instead in aura
        m_submergePhases = 0;
    ...
    case LURKER_WATERBOLT: {
      ...
      // Spam Waterbolt spell when not tanked
        ...
        if (m_rangeCheckState > 1)
          if (Unit *target =
                  m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_WATERBOLT, SELECT_FLAG_PLAYER))
...
bool GOUse_go_strange_pool(Player * /*player*/, GameObject *go) {
  // There is some chance to fish The Lurker Below, sources are from 20s to 10minutes, average 5min => 20 tries, hence
  // 5%
  if (urand(0, 99) < 10) {
