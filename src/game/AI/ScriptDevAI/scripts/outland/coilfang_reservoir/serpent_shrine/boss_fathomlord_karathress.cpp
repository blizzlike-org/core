...
struct boss_fathomlord_karathressAI : public ScriptedAI {
  boss_fathomlord_karathressAI(Creature *creature)
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float /*x*/, float y, float /*z*/) { return y > -425.f || y < -624.f; });
    Reset();
  }
  ...
  void JustSummoned(Creature *summoned) override { m_spawns.push_back(summoned->GetObjectGuid()); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiCataclysmicBoltTimer < uiDiff) {
      ...
      Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, uint32(0),
...
struct boss_fathomguard_sharkkisAI : public CombatAI {
  boss_fathomguard_sharkkisAI(Creature *creature)
      : CombatAI(creature, SHARKKIS_ACTION_MAX),
  ...
  void JustDied(Unit * /*killer*/) override { DoCastSpellIfCan(m_creature, SPELL_POWER_OF_SHARKKIS, CAST_TRIGGERED); }
  ...
  void SummonedCreatureJustDied(Creature *summoned) override { ResetCombatAction(SHARKKIS_PET_SUMMON, 30000); }

  void ExecuteAction(uint32 action) override {
    switch (action) {
    case SHARKKIS_HURL_TRIDENT:
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_HURL_TRIDENT, SELECT_FLAG_PLAYER))
        if (DoCastSpellIfCan(target, SPELL_HURL_TRIDENT) == CAST_OK)
          ResetCombatAction(action, 5000);
      break;
    case SHARKKIS_LEECHING_THROW:
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_LEECHING_THROW,
                                                           SELECT_FLAG_PLAYER | SELECT_FLAG_POWER_MANA))
    ...
    case SHARKKIS_PET_SUMMON:
      if (DoCastSpellIfCan(m_creature, urand(0, 1) ? SPELL_SUMMON_FATHOM_LURKER : SPELL_SUMMON_FATHOM_SPOREBAT) ==
          CAST_OK)
...
struct boss_fathomguard_tidalvessAI : public CombatAI {
  boss_fathomguard_tidalvessAI(Creature *creature)
      : CombatAI(creature, TIDALVESS_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float /*x*/, float y, float /*z*/) { return y > -425.f || y < -624.f; });
  ...
  void JustDied(Unit * /*killer*/) override { DoCastSpellIfCan(m_creature, SPELL_POWER_OF_TIDALVESS, CAST_TRIGGERED); }
...
struct boss_fathomguard_caribdisAI : public ScriptedAI {
  boss_fathomguard_caribdisAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float /*x*/, float y, float /*z*/) { return y > -425.f || y < -624.f; });
    Reset();
  ...
  void JustDied(Unit * /*killer*/) override { DoCastSpellIfCan(m_creature, SPELL_POWER_OF_CARIBDIS, CAST_TRIGGERED); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiCycloneTimer < uiDiff) {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SUMMON_CYCLONE, SELECT_FLAG_PLAYER))
