...
SD%Complete: 80
SDComment: Some details are not very clear: the usage of Shoot and Multishot spells; the summons positions. Tainted Core
paralize NYI. Timers need improvements. SDCategory: Coilfang Resevoir, Serpent Shrine Cavern EndScriptData */
...
struct boss_lady_vashjAI : public RangedCombatAI {
  boss_lady_vashjAI(Creature *creature)
      : RangedCombatAI(creature, VASHJ_ACTION_MAX),
  ...
  void Aggro(Unit * /*who*/) override {
    ...
    case 3:
      ...
      break;
    case 4:
      DoScriptText(SAY_AGGRO5, m_creature);
      break;
    }
  ...
  void JustSummoned(Creature *summoned) override {
    ...
    case NPC_TAINTED_ELEMENTAL:
      ...
      summoned->ForcedDespawn(16000, true); // only despawn after 15 seconds if still alive, different rules on death,
                                            // one second more for appearing
  ...
  void DoInformGeneratorStopped() {
    ...
    // Check if all generators have been deactivated, or the creature doesn't have the spell barrier aura (in order to
    // avoid eventual aura stacking bugs)
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case VASHJ_ACTION_STATIC_CHARGE: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_STATIC_CHARGE, SELECT_FLAG_PLAYER)) {
...
struct mob_enchanted_elementalAI : public ScriptedAI {
  mob_enchanted_elementalAI(Creature *creature) : ScriptedAI(creature) { Reset(); }

  void Reset() override {}

  void MoveInLineOfSight(Unit *pWho) override {
    // Buff Lady Vashj on range check - spell has script target
    if (pWho->GetEntry() == NPC_LADYVASHJ && pWho->IsWithinDistInMap(m_creature, INTERACTION_DISTANCE) &&
