...
instance_serpentshrine_cavern::instance_serpentshrine_cavern(Map *pMap)
    : ScriptedInstance(pMap), m_uiSpellBinderCount(0) {
  Initialize();
}

void instance_serpentshrine_cavern::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_serpentshrine_cavern::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_THELURKER_EVENT:
    switch (uiData) {
    case IN_PROGRESS:
      WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_LURKER_BELOW, afLurkerSpawnPos[0], afLurkerSpawnPos[1],
                                                    afLurkerSpawnPos[2], afLurkerSpawnPos[3], TEMPSPAWN_DEAD_DESPAWN,
                                                    0),
                                  instance);
      break;
    case DONE:
  ...
  case TYPE_LEOTHERAS_EVENT:
    if (uiData == FAIL) {
      for (GuidList::const_iterator itr = m_lSpellBindersGUIDList.begin(); itr != m_lSpellBindersGUIDList.end();
           ++itr) {
...
void instance_serpentshrine_cavern::Load(const char *chrIn) {
  ...
  loadStream >> m_auiEncounter[TYPE_HYDROSS_EVENT] >> m_auiEncounter[TYPE_KARATHRESS_EVENT] >>
      m_auiEncounter[TYPE_LADYVASHJ_EVENT] >> m_auiEncounter[TYPE_LEOTHERAS_EVENT] >>
...
bool instance_serpentshrine_cavern::CheckConditionCriteriaMeet(Player const *pPlayer, uint32 uiInstanceConditionId,
  ...
  script_error_log("instance_serpentshrine_cavern::CheckConditionCriteriaMeet called with unsupported Id %u. Called "
...
void instance_serpentshrine_cavern::EngageBridgeConsole(GameObject *_console) {
  if (m_auiEncounter[TYPE_HYDROSS_EVENT] == SPECIAL && m_auiEncounter[TYPE_THELURKER_EVENT] == SPECIAL &&
      m_auiEncounter[TYPE_LEOTHERAS_EVENT] == SPECIAL && m_auiEncounter[TYPE_KARATHRESS_EVENT] == SPECIAL &&
      m_auiEncounter[TYPE_MOROGRIM_EVENT] == SPECIAL)
...
struct npc_serpentshrine_parasiteAI : public ScriptedAI {
  ...
  void DamageDeal(Unit * /*pDoneTo*/, uint32 & /*damage*/, DamageEffectType /*damagetype*/,
