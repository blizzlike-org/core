...
struct boss_hydross_the_unstableAI : public ScriptedAI {
  ...
  void DoSpawnAdds() {
    ...
    for (uint8 i = 0; i < MAX_HYDROSS_ADDS; ++i) {
      ...
      m_creature->SummonCreature(m_bCorruptedForm ? NPC_PURE_SPAWN : NPC_TAINTED_SPAWN, fX, fY, fZ, 0,
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiMarkTimer < uiDiff) {
      if (DoCastSpellIfCan(m_creature, m_bCorruptedForm ? aMarkCorruption[m_uiMarkCount]
                                                        : aMarkHydross[m_uiMarkCount]) == CAST_OK) {
        ++m_uiMarkCount;
        m_uiMarkTimer = 15000;
...
UnitAI *GetAI_boss_hydross_the_unstable(Creature *pCreature) { return new boss_hydross_the_unstableAI(pCreature); }
...
UnitAI *GetAI_npc_spawn_of_hydross(Creature *pCreature) { return new npc_spawn_of_hydrossAI(pCreature); }
