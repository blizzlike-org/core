...
static const uint32 m_auiSpellWateryGraveTeleport[] = {SPELL_WATERY_GRAVE_TELEPORT_1, SPELL_WATERY_GRAVE_TELEPORT_2,
                                                       SPELL_WATERY_GRAVE_TELEPORT_3, SPELL_WATERY_GRAVE_TELEPORT_4};
static const uint32 m_auiSpellSummonGlobule[] = {SPELL_SUMMON_GLOBULE_1, SPELL_SUMMON_GLOBULE_2, SPELL_SUMMON_GLOBULE_3,
                                                 SPELL_SUMMON_GLOBULE_4};
...
struct boss_morogrim_tidewalkerAI : public CombatAI {
  boss_morogrim_tidewalkerAI(Creature *creature)
      : CombatAI(creature, MOROGRIM_ACTION_MAX),
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [](Unit *, float x, float y, float z) { return x < 304.12f || x > 457.35f; });
  }
...
struct WateryGrave : public SpellScript {
  ...
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    if (effIdx == EFFECT_INDEX_0) {
      ...
      spell->GetCaster()->CastSpell(spell->GetUnitTarget(), m_auiSpellWateryGraveTeleport[targetIdx],
