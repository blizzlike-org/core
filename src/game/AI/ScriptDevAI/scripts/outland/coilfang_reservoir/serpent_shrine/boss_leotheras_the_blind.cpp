...
struct boss_leotheras_the_blindAI : public CombatAI {
  boss_leotheras_the_blindAI(Creature *creature)
      : CombatAI(creature, LEOTHERAS_ACTION_MAX),
  ...
  void KilledUnit(Unit *victim) override {
    ...
    switch (urand(0, 2)) {
    case 0:
      DoScriptText(m_bDemonForm ? SAY_DEMON_SLAY1 : SAY_NIGHTELF_SLAY1, m_creature);
      break;
    case 1:
      DoScriptText(m_bDemonForm ? SAY_DEMON_SLAY2 : SAY_NIGHTELF_SLAY2, m_creature);
      break;
    case 2:
      DoScriptText(m_bDemonForm ? SAY_DEMON_SLAY3 : SAY_NIGHTELF_SLAY3, m_creature);
      break;
    }
  }
...
struct npc_inner_demonAI : public ScriptedAI {
  ...
  void Reset() override { m_uiShadowBoltTimer = urand(6100, 8400); }
...
struct InsidiousWhisper : public SpellScript, public AuraScript {
  void OnInit(Spell *spell) const override { spell->SetMaxAffectedTargets(4); }
  ...
  void OnApply(Aura *aura, bool apply) const override {
    ...
    if ((apply && faction_rank >= REP_FRIENDLY) ||
        (!apply && player->GetReputationRank(aura->GetModifier()->m_amount) >= REP_FRIENDLY))
