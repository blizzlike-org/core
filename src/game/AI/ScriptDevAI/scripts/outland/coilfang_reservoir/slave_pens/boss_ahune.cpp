...
enum {
  ...
  SPELL_SPANKY_HANDS =
      46146, // procs on melee hit; should proc 46430 but currently is not used because of the invalid proc flag
...
struct boss_ahuneAI : public Scripted_NoMovementAI {
  ...
  void JustDied(Unit * /*pKiller*/) override {
    ...
    DoCastSpellIfCan(m_creature, m_creature->GetMap()->IsRegularDifficulty() ? SPELL_AHUNE_LOOT : SPELL_AHUNE_LOOT_H,
                     CAST_TRIGGERED);
  }
...
UnitAI *GetAI_boss_ahune(Creature *pCreature) { return new boss_ahuneAI(pCreature); }
...
UnitAI *GetAI_npc_frozen_core(Creature *pCreature) { return new npc_frozen_coreAI(pCreature); }
...
struct npc_ice_spear_bunnyAI : public Scripted_NoMovementAI {
  ...
  void Reset() override { m_uiEventCount = 0; }
...
UnitAI *GetAI_npc_ice_spear_bunny(Creature *pCreature) { return new npc_ice_spear_bunnyAI(pCreature); }
