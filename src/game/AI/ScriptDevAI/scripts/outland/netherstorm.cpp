...
SDName: Netherstorm
SD%Complete: 80
SDComment: Quest support: 10191, 10198, 10299, 10310, 10321, 10322, 10323, 10329, 10330, 10337, 10338, 10365(Shutting
Down Manaforge), 10406, 10425, 10438, 10924. SDCategory: Netherstorm EndScriptData */
...
struct npc_manaforge_spawnAI : public ScriptedAI {
  ...
  void Reset() override { m_creature->ClearInCombat(); }
        ...
        if (fDistance < 20) // If within cast range
        {
          ...
          if (uiManaforgeEntry == NPC_ARA_C_CONSOLE) {
            m_creature->GetMotionMaster()->MovePoint(
                0, m_creature->GetPositionX(), m_creature->GetPositionY(),
                m_creature->GetPositionZ()); // Why must I do this? They walk back to spawn point otherwise (and only
                                             // ara mobs do this)
          ...
          /* Given an origin (x,y) and line distance "d" which makes an angle "a" with the x-axis, the coordinates for
           * the other point are: x' = x + (d * cos(a)) y' = y + (d * sin(a)) */
...
UnitAI *GetAI_npc_manaforge_spawnAI(Creature *pCreature) { return new npc_manaforge_spawnAI(pCreature); }
...
struct npc_manaforge_control_consoleAI : public ScriptedAI {
  ...
  void ResetControlConsoleAndDespawn() {
    for (ObjectGuid &guid : m_vSummonGuids) {
      if (Creature *summon = m_creature->GetMap()->GetCreature(guid))
        summon->ForcedDespawn(100); // requires delay, to prevent calling spell::cancel, as the call can originate from
                                    // channeled spell finish
    }
  ...
  void DoWaveSpawnForCreature(Creature *pCreature) {
    ...
    case NPC_BNAAR_C_CONSOLE:
      if (urand(0, 1)) {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afBanaarTechCoords[0][0], m_afBanaarTechCoords[0][1],
                                               m_afBanaarTechCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(0, m_afBanaarTechCoords[1][0], m_afBanaarTechCoords[1][1],
                                             m_afBanaarTechCoords[1][2]);
        }
      } else {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afBanaarTechCoords[0][0], m_afBanaarTechCoords[0][1],
                                               m_afBanaarTechCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(2, m_afBanaarTechCoords[3][0], m_afBanaarTechCoords[3][1],
                                             m_afBanaarTechCoords[3][2]);
        }
      }
      m_uiWaveTimer = 30000;
      break;
    case NPC_CORUU_C_CONSOLE:
      route = urand(0, 2);
      if (route == 0) {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[0][0], m_afCoruuTechCoords[0][1],
                                               m_afCoruuTechCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(0, m_afCoruuTechCoords[1][0], m_afCoruuTechCoords[1][1],
                                             m_afCoruuTechCoords[1][2]);
        }
      } else if (route == 1) {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[2][0], m_afCoruuTechCoords[2][1],
                                               m_afCoruuTechCoords[2][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(0, m_afCoruuTechCoords[3][0], m_afCoruuTechCoords[3][1],
                                             m_afCoruuTechCoords[3][2]);
        }
      } else {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[4][0], m_afCoruuTechCoords[4][1],
                                               m_afCoruuTechCoords[4][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(0, m_afCoruuTechCoords[5][0], m_afCoruuTechCoords[5][1],
                                             m_afCoruuTechCoords[5][2]);
        }
      }
      m_uiWaveTimer = 20000;
      break;
    case NPC_DURO_C_CONSOLE:
      route = urand(0, 2);
      if (route == 0) {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afDuroTechCoords[0][0], m_afDuroTechCoords[0][1],
                                               m_afDuroTechCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(0, m_afDuroTechCoords[1][0], m_afDuroTechCoords[1][1],
                                             m_afDuroTechCoords[1][2]);
        }
      } else if (route == 1) {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afDuroTechCoords[0][0], m_afDuroTechCoords[0][1],
                                               m_afDuroTechCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(2, m_afDuroTechCoords[3][0], m_afDuroTechCoords[3][1],
                                             m_afDuroTechCoords[3][2]);
        }
      } else {
        if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afDuroTechCoords[5][0], m_afDuroTechCoords[5][1],
                                               m_afDuroTechCoords[5][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               120000))) {
          pAdd->SetWalk(false);
          pAdd->GetMotionMaster()->MovePoint(4, m_afDuroTechCoords[6][0], m_afDuroTechCoords[6][1],
                                             m_afDuroTechCoords[6][2]);
        }
      }
      m_uiWaveTimer = 15000;
      break;
    case NPC_ARA_C_CONSOLE:
      ...
      if (route == 0) {
        for (uint8 i = 0; i < 3; i++)
          if ((pAdd = m_creature->SummonCreature(NPC_ARA_TECH, m_afAraTechCoords[i][0], m_afAraTechCoords[i][1],
                                                 m_afAraTechCoords[i][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                                 120000))) {
            pAdd->SetWalk(false);
            pAdd->GetMotionMaster()->MovePoint(0, m_afAraTechCoords[i + 3][0], m_afAraTechCoords[i + 3][1],
                                               m_afAraTechCoords[i + 3][2]);
          }
        if (!urand(0, 2)) // 1 in 3 chance to spawn a engineer
          if ((pAdd = m_creature->SummonCreature(NPC_ARA_ENGI, m_afAraTechCoords[0][0], m_afAraTechCoords[0][1],
                                                 m_afAraTechCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                                 120000)))
            pAdd->GetMotionMaster()->MovePoint(0, m_afAraTechCoords[3][0], m_afAraTechCoords[3][1],
                                               m_afAraTechCoords[3][2]);
      } else if (route == 1) {
        for (uint8 i = 6; i < 9; i++)
          if ((pAdd = m_creature->SummonCreature(NPC_ARA_TECH, m_afAraTechCoords[i][0], m_afAraTechCoords[i][1],
                                                 m_afAraTechCoords[i][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
        ...
        if (!urand(0, 2)) // 1 in 3 chance to spawn a engineer
          if ((pAdd = m_creature->SummonCreature(NPC_ARA_ENGI, m_afAraTechCoords[6][0], m_afAraTechCoords[6][1],
                                                 m_afAraTechCoords[6][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                                 120000)))
            pAdd->GetMotionMaster()->MovePoint(0, m_afAraTechCoords[9][0], m_afAraTechCoords[9][1],
                                               m_afAraTechCoords[9][2]);
      } else {
        ...
        if (!urand(0, 2)) // 1 in 3 chance to spawn a engineer
          if ((pAdd = m_creature->SummonCreature(NPC_ARA_ENGI, m_afAraTechCoords[12][0], m_afAraTechCoords[12][1],
                                                 m_afAraTechCoords[12][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                                 120000)))
            pAdd->GetMotionMaster()->MovePoint(0, m_afAraTechCoords[15][0], m_afAraTechCoords[15][1],
                                               m_afAraTechCoords[15][2]);
      }
      m_uiWaveTimer = 15000;
      break;
    }
  ...
  void DoFinalSpawnForCreature(Creature *pCreature) {
    ...
    switch (pCreature->GetEntry()) {
    case NPC_BNAAR_C_CONSOLE:
      if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afBanaarTechCoords[5][0], m_afBanaarTechCoords[5][1],
                                             m_afBanaarTechCoords[5][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                             120000))) {
        pAdd->SetWalk(false);
        pAdd->GetMotionMaster()->MovePoint(3, m_afBanaarTechCoords[6][0], m_afBanaarTechCoords[6][1],
                                           m_afBanaarTechCoords[6][2]);
      }
      break;
    case NPC_CORUU_C_CONSOLE:
      if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[0][0], m_afCoruuTechCoords[0][1],
    ...
    case NPC_DURO_C_CONSOLE:
      ...
      break;
    case NPC_ARA_C_CONSOLE:
      if ((pAdd = m_creature->SummonCreature(NPC_ARA_GORKLONN, m_afAraTechCoords[12][0], m_afAraTechCoords[12][1],
                                             m_afAraTechCoords[12][2], 4.44f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                             120000))) {
        Player *pPlayer;
  ...
  void SummonedMovementInform(Creature *pSummoned, uint32 uiMotionType, uint32 uiData) override {
    ...
    case NPC_BNAAR_C_CONSOLE:
      if (pSummoned->GetEntry() == NPC_SUNFURY_TECH && uiMotionType == POINT_MOTION_TYPE && pSummoned->IsAlive())
        switch (uiData) {
        case 0:
          pSummoned->GetMotionMaster()->MovePoint(1, m_afBanaarTechCoords[2][0], m_afBanaarTechCoords[2][1],
                                                  m_afBanaarTechCoords[2][2]);
          break;
        case 2:
          pSummoned->GetMotionMaster()->MovePoint(3, m_afBanaarTechCoords[4][0], m_afBanaarTechCoords[4][1],
                                                  m_afBanaarTechCoords[4][2]);
          break;
        case 1:
        case 3:
          if (!m_bShutdownSaid) {
    ...
    case NPC_DURO_C_CONSOLE:
      if (pSummoned->GetEntry() == NPC_SUNFURY_TECH && uiMotionType == POINT_MOTION_TYPE && pSummoned->IsAlive())
        switch (uiData) {
        case 0:
          pSummoned->GetMotionMaster()->MovePoint(1, m_afDuroTechCoords[2][0], m_afDuroTechCoords[2][1],
                                                  m_afDuroTechCoords[2][2]);
          break;
        case 2:
          pSummoned->GetMotionMaster()->MovePoint(3, m_afDuroTechCoords[4][0], m_afDuroTechCoords[4][1],
                                                  m_afDuroTechCoords[4][2]);
          break;
        case 1:
        case 3:
        case 4:
      ...
      break;
    case NPC_ARA_C_CONSOLE:
      if ((pSummoned->GetEntry() == NPC_ARA_TECH || pSummoned->GetEntry() == NPC_ARA_ENGI ||
           pSummoned->GetEntry() == NPC_ARA_GORKLONN) &&
          uiMotionType == POINT_MOTION_TYPE && pSummoned->IsAlive())
        switch (uiData) {
        case 0:
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiHardResetTimer <= uiDiff) {
      ...
      sLog.outCustomLog("Manaforge invalid state, helper creature: %s (%u)", m_creature->GetName(),
                        m_creature->GetEntry());
  ...
  if (uiAction == GOSSIP_ACTION_INFO_DEF + 1) {
    ...
    case 3726: // b'naar
      if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_BNAAR_ALDOR) == QUEST_STATUS_INCOMPLETE ||
           pPlayer->GetQuestStatus(QUEST_SHUTDOWN_BNAAR_SCRYERS) == QUEST_STATUS_INCOMPLETE) &&
          pPlayer->HasItemCount(ITEM_BNAAR_ACESS_CRYSTAL, 1))
        pManaforge = pPlayer->SummonCreature(NPC_BNAAR_C_CONSOLE, 2918.95f, 4189.98f, 164.5f, 0.34f,
                                             TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
      break;
    case 3730: // coruu
      if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_CORUU_ALDOR) == QUEST_STATUS_INCOMPLETE ||
           pPlayer->GetQuestStatus(QUEST_SHUTDOWN_CORUU_SCRYERS) == QUEST_STATUS_INCOMPLETE) &&
          pPlayer->HasItemCount(ITEM_CORUU_ACESS_CRYSTAL, 1))
        pManaforge = pPlayer->SummonCreature(NPC_CORUU_C_CONSOLE, 2423.36f, 2755.72f, 135.5f, 2.14f,
                                             TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
      break;
    case 3734: // duro
      if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_DURO_ALDOR) == QUEST_STATUS_INCOMPLETE ||
           pPlayer->GetQuestStatus(QUEST_SHUTDOWN_DURO_SCRYERS) == QUEST_STATUS_INCOMPLETE) &&
          pPlayer->HasItemCount(ITEM_DURO_ACESS_CRYSTAL, 1))
        pManaforge = pPlayer->SummonCreature(NPC_DURO_C_CONSOLE, 2976.48f, 2183.29f, 166.00f, 1.85f,
                                             TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
      break;
    case 3722: // ara
      if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_ARA_ALDOR) == QUEST_STATUS_INCOMPLETE ||
           pPlayer->GetQuestStatus(QUEST_SHUTDOWN_ARA_SCRYERS) == QUEST_STATUS_INCOMPLETE) &&
          pPlayer->HasItemCount(ITEM_ARA_ACESS_CRYSTAL, 1))
        pManaforge = pPlayer->SummonCreature(NPC_ARA_C_CONSOLE, 4013.71f, 4028.76f, 195.00f, 1.25f,
                                             TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
      break;
    }

    if (pManaforge) {
      if (npc_manaforge_control_consoleAI *pManaforgeAI =
              dynamic_cast<npc_manaforge_control_consoleAI *>(pManaforge->AI())) {
        ...
        case 3726: // b'naar
          ...
          for (std::list<Creature *>::iterator itr = lNearbyWarpEngineers.begin(); itr != lNearbyWarpEngineers.end();
               ++itr)
            (*itr)->AI()->AttackStart(pPlayer);
...
struct npc_commander_dawnforgeAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    case 4:
      ...
      m_creature->SummonCreature(NPC_PATHALEON_THE_CALCULATOR_IMAGE, 2325.851563f, 2799.534668f, 133.084229f, 6.038996f,
...
UnitAI *GetAI_npc_commander_dawnforge(Creature *pCreature) { return new npc_commander_dawnforgeAI(pCreature); }
...
struct npc_bessyAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 4:
      ...
      if (Creature *pSpirit = m_creature->SummonCreature(NPC_SEVERED_SPIRIT, 2438.142f, 2200.449f, 101.5952f, 4.63704f,
                                                         TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 25000, false, true))
        pSpirit->GetMotionMaster()->MoveWaypoint(1);
      break;
    case 8:
      if (Creature *pSpirit = m_creature->SummonCreature(NPC_SEVERED_SPIRIT, 2279.846f, 2188.402f, 91.61183f, 6.22683f,
                                                         TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 25000, false, true))
  ...
  void Reset() override { m_creature->SetWalk(true); }
};

bool QuestAccept_npc_bessy(Player *pPlayer, Creature *pCreature, const Quest *pQuest) {
  if (pQuest->GetQuestId() == QUEST_COWS_COME_HOME) {
    pCreature->SetFactionTemporary(FACTION_ESCORT_N_NEUTRAL_ACTIVE,
                                   TEMPFACTION_RESTORE_RESPAWN | TEMPFACTION_TOGGLE_NON_ATTACKABLE);
...
UnitAI *GetAI_npc_bessy(Creature *pCreature) { return new npc_bessyAI(pCreature); }
      ...
      if (m_uiSubEventTimer) {
        if (m_uiSubEventTimer <= uiDiff) {
          ...
          case 1: // Wait time before Say
            if (Creature *pAlley =
                    GetClosestCreatureWithEntry(m_creature, NPC_BOT_SPECIALIST_ALLEY, INTERACTION_DISTANCE * 2)) {
...
UnitAI *GetAI_npc_maxx_a_million(Creature *pCreature) { return new npc_maxx_a_million_escortAI(pCreature); }
...
struct npc_zeppitAI : public ScriptedPetAI {
  ...
  void Reset() override { SetReactState(REACT_PASSIVE); }
...
UnitAI *GetAI_npc_zeppit(Creature *pCreature) { return new npc_zeppitAI(pCreature); }
...
struct npc_protectorate_demolitionistAI : public npc_escortAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(urand(0, 1) ? SAY_ATTACKED_1 : SAY_ATTACKED_2, m_creature); }
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    ...
    if (pWho->GetTypeId() == TYPEID_PLAYER) {
      if (pWho->HasAura(SPELL_PROTECTORATE) &&
          ((Player *)pWho)->GetQuestStatus(QUEST_ID_DELIVERING_MESSAGE) == QUEST_STATUS_INCOMPLETE) {
  ...
  void UpdateEscortAI(const uint32 uiDiff) override {
    ...
    if (m_uiEventTimer) {
      if (m_uiEventTimer <= uiDiff) {
        ...
        case 0:
          m_creature->SummonCreature(NPC_ARCHON, 3875.69f, 2308.72f, 115.80f, 1.48f,
                                     TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 30000);
          m_uiEventTimer = 4000;
          break;
        case 1:
          m_creature->SummonCreature(NPC_NEXUS_STALKER, 3884.06f, 2325.22f, 111.37f, 3.45f,
                                     TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 30000);
          m_creature->SummonCreature(NPC_NEXUS_STALKER, 3861.54f, 2320.44f, 111.48f, 0.32f,
                                     TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 30000);
          m_uiEventTimer = 9000;
          break;
        case 2:
...
UnitAI *GetAI_npc_captured_vanguard(Creature *pCreature) { return new npc_captured_vanguardAI(pCreature); }
...
struct npc_drijyaAI : public npc_escortAI {
  ...
  void DoSpawnCreature(uint32 uiEntry) {
    if (Creature *pTrigger = m_creature->GetMap()->GetCreature(m_explodeTriggerGuid))
      m_creature->SummonCreature(uiEntry, pTrigger->GetPositionX(), pTrigger->GetPositionY(), pTrigger->GetPositionZ(),
                                 pTrigger->GetOrientation(), TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
  }
...
UnitAI *GetAI_npc_drijya(Creature *pCreature) { return new npc_drijyaAI(pCreature); }
...
UnitAI *GetAI_npc_saeed(Creature *pCreature) { return new npc_saeed_escortAI(pCreature); }
...
struct npc_dimensiusAI : public Scripted_NoMovementAI {
  ...
  void Reset() override {
    ...
    m_creature->SetFlag(UNIT_FIELD_FLAGS,
                        UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PLAYER);
  }
...
UnitAI *GetAI_npc_dimensius(Creature *pCreature) { return new npc_dimensiusAI(pCreature); }
...
struct npc_salhadaarAI : public ScriptedAI {
  ...
  void JustRespawned() override { m_uiFoundBalls = false; }
    ...
    if (m_uiAttackTimer) {
      if (m_uiAttackTimer <= uiDiff) // on attack start cast spell that handles everything
      {
        ...
        if (!playerList.empty()) {
          ...
          m_creature->SetFactionTemporary(FACTION_SALHADAAR_HOSTILE,
                                          TEMPFACTION_RESTORE_COMBAT_STOP | TEMPFACTION_TOGGLE_NOT_SELECTABLE);
          return;
        }
        Reset();
        m_creature->ForcedDespawn();
      ...
      if (!m_uiUsedMirrorImage && m_creature->GetHealthPercent() < 25) // at 25% hp cast mirror image, its two spells
      {
        ...
        if (m_uiStasisTimer <= uiDiff) {
          if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_STASIS)) {
            if (m_creature->CastSpell(target, SPELL_STASIS, TRIGGERED_NONE) ==
                SPELL_CAST_OK) // cast stasis on random target every 22 seconds
...
UnitAI *GetAI_npc_salhadaar(Creature *pCreature) { return new npc_salhadaarAI(pCreature); }
...
UnitAI *GetAI_npc_energy_ball(Creature *pCreature) { return new npc_energy_ballAI(pCreature); }
...
struct npc_void_conduitAI : public Scripted_NoMovementAI {
  ...
  void Reset() override { m_uiEvadeTimer = 0; }
...
UnitAI *GetAI_npc_void_conduit(Creature *pCreature) { return new npc_void_conduitAI(pCreature); }
...
struct npc_scrap_reaverAI : ScriptedPetAI {
  ...
  void KilledUnit(Unit *victim) override { HandleKill(victim); }

  void OwnerKilledUnit(Unit *victim) override { HandleKill(victim); }
...
struct npc_scrapped_fel_reaverAI : ScriptedAI {
  ...
  void SummonedCreatureJustDied(Creature *summoned) override { ClearSpawn(summoned); }

  // has to be done in both, because spawns can despawn before event ends (blizzlike) without dying and then respawn
  void SummonedCreatureDespawn(Creature *summoned) override { ClearSpawn(summoned); }
  ...
  void ProcessEvent() {
    if (m_spawnTimer <= 0) {
      ...
      for (i = 0; i < COUNT_SPAWNING_LOCATIONS;
           i++, m_locationCounter = (m_locationCounter + 1) % COUNT_SPAWNING_LOCATIONS)
        if (!m_spawnsPerLocation[m_locationCounter])
          break;

      if (i != COUNT_SPAWNING_LOCATIONS)
        m_creature->SummonCreature(NPC_ZAXXIS_AMBUSHER, spawningLocations[m_locationCounter][0],
                                   spawningLocations[m_locationCounter][1], spawningLocations[m_locationCounter][2],
                                   spawningLocations[m_locationCounter][3], TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 240000,
                                   true, true, m_locationCounter + 1);
    }
    if (m_eventTimer <= 0) {
...
UnitAI *GetAI_npc_scrapped_fel_reaver(Creature *creature) { return new npc_scrapped_fel_reaverAI(creature); }
...
struct npc_adyen_the_lightwardenAI : public ScriptedAI {
  npc_adyen_the_lightwardenAI(Creature *creature)
      : ScriptedAI(creature), m_crusaderStrikeTimer(0), m_hammerOfJusticeTimer(0), m_eventStarted(false) {
  ...
  void AddTimer(const uint32 id, const uint32 timer) { m_timers[id] = timer; }
  ...
  void ExecuteTimer(const uint32 id) {
    ...
    case EVENT_ADYEN_SAY_2:
      ...
      m_creature->SummonCreature(NPC_KAYLAAN_THE_LOST, 4955.793f, 3926.349f, 208.391f, 4.570264f,
                                 TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true, false, 1);
      break;
  ...
  Player *GetPlayerTarget() const { return m_creature->GetMap()->GetPlayer(m_playerGuid); }

  void JustSummoned(Creature *summoned) override {
    switch (summoned->GetEntry()) {
    case NPC_EXARCH_ORELIS:
      m_orelisGuid = summoned->GetObjectGuid();
      break;
    case NPC_ANCHORITE_KARJA:
      m_karjaGuid = summoned->GetObjectGuid();
      break;
    case NPC_ISHANAH:
      m_ishanahGuid = summoned->GetObjectGuid();
      break;
    case NPC_KAYLAAN_THE_LOST:
      m_kaylaanGuid = summoned->GetObjectGuid();
      break;
    default:
      break;
    }
  ...
  void StartEvent(Player *player) {
    if (Creature *socrethar = ((ScriptedInstance *)m_creature->GetMap()->GetInstanceData())
                                  ->GetSingleCreatureFromStorage(NPC_SOCRETHAR)) {
    ...
    if (Creature *wrathLord = m_creature->GetMap()->GetCreature(
            ObjectGuid(HIGHGUID_UNIT, uint32(NPC_WRATH_LORD), uint32(DB_GUID_WRATH_LORD_1)))) {
      wrathLord->GetMotionMaster()->Clear(false, true);
      wrathLord->GetMotionMaster()->MoveWaypoint(PATH_ID_WRATH_LORD_1);
    }
    if (Creature *wrathLord = m_creature->GetMap()->GetCreature(
            ObjectGuid(HIGHGUID_UNIT, uint32(NPC_WRATH_LORD), uint32(DB_GUID_WRATH_LORD_2)))) {
      wrathLord->GetMotionMaster()->Clear(false, true);
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit *sender, Unit * /*invoker*/, uint32 /*miscValue*/) override {
    if (eventType == AI_EVENT_CUSTOM_A) {
      Creature *ishanah =
          m_creature->SummonCreature(NPC_ISHANAH, 4866.2f, 3799.016f, 199.141f, 0.4680258f,
                                     TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true, true, 1, FACTION_DEATHBLOW);
...
struct npc_kaylaan_the_lostAI : public ScriptedAI {
  npc_kaylaan_the_lostAI(Creature *creature)
      : ScriptedAI(creature), m_deathPrevented(false), m_avengersShieldTimer(0), m_burningLightTimer(0), m_healTimer(0),
        m_holySlamTimer(0) {
    m_creature->GetCombatManager().SetLeashingDisable(true);
  }
...
bool AreaTrigger_at_socrethar_seat(Player *player, AreaTriggerEntry const * /*at*/) {
  if (player->IsCurrentQuest(QUEST_DEATHBLOW_TO_THE_LEGION)) {
    if (player->GetMap()->SpawnedCountForEntry(NPC_ADYEN_THE_LIGHTWARDEN) == 0) {
      Creature *adyen = player->SummonCreature(NPC_ADYEN_THE_LIGHTWARDEN, 4805.796f, 3774.112f, 210.6172f, 5.550147f,
                                               TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true, false, 1,
                                               FACTION_DEATHBLOW, 0, true);
      adyen->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
      Creature *orelis =
          adyen->SummonCreature(NPC_EXARCH_ORELIS, 4804.807f, 3775.562f, 210.6171f, 5.532694f,
                                TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true, false, 1, FACTION_DEATHBLOW);
      orelis->SetUInt32Value(UNIT_NPC_FLAGS, 0);
      Creature *karja =
          adyen->SummonCreature(NPC_ANCHORITE_KARJA, 4803.637f, 3773.844f, 210.6176f, 5.602507f,
                                TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true, false, 1, FACTION_DEATHBLOW);
...
bool GossipHello_npc_adyen_the_lightwarden(Player *player, Creature *creature) {
  ...
  if (creature->getFaction() == FACTION_SHATTRATH)
    ...
    if (npc_adyen_the_lightwardenAI *ai = dynamic_cast<npc_adyen_the_lightwardenAI *>(creature->AI())) {
      Creature *socrethar =
          ((ScriptedInstance *)creature->GetMap()->GetInstanceData())->GetSingleCreatureFromStorage(NPC_SOCRETHAR);
