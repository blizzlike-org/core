...
std::map<uint32, std::vector<ObjectGuid>> secondPhaseSpawns = {
    ...
    {2, {}},
    {3,
     {ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161427u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161432u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161429u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161431u),
      ObjectGuid(HIGHGUID_UNIT, NPC_MOARG_TORMENTER, 161440u)}},
    {4, {}},
    {5,
     {ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161438u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161434u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161435u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161437u),
      ObjectGuid(HIGHGUID_UNIT, NPC_MOARG_TORMENTER, 161441u)}},
    {6, {}},
    {7,
     {ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161436u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161430u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161433u),
      ObjectGuid(HIGHGUID_UNIT, NPC_GANARG_UNDERLING, 161428u),
      ObjectGuid(HIGHGUID_UNIT, NPC_MOARG_TORMENTER, 161439u)}},
};
...
struct world_map_outland : public ScriptedMap, public TimerManager {
  world_map_outland(Map *pMap) : ScriptedMap(pMap), m_shadeData({AREAID_AZURE_WATCH, AREAID_FALCONWING_SQUARE}) {
    Initialize();
  }
  ...
  void SetData(uint32 type, uint32 data) override {
    ...
    case TYPE_DEATHS_DOOR_NORTH:
      ...
      sWorldState.ExecuteOnAreaPlayers(AREAID_DEATHS_DOOR, [=](Player *player) -> void {
        player->SendUpdateWorldState(WORLD_STATE_DEATHS_DOOR_NORTH_WARP_GATE_HEALTH, m_deathsDoorNorthHP);
      });
      break;
    case TYPE_DEATHS_DOOR_SOUTH:
      m_deathsDoorSouthHP = std::max(0, 100 - int32(data * 15));
      sWorldState.ExecuteOnAreaPlayers(AREAID_DEATHS_DOOR, [=](Player *player) -> void {
        player->SendUpdateWorldState(WORLD_STATE_DEATHS_DOOR_SOUTH_WARP_GATE_HEALTH, m_deathsDoorSouthHP);
      });
      break;
    case TYPE_SHARTUUL:
      if (data == EVENT_START) {
        ...
        if (m_shartuulShieldPercent == 750 || m_shartuulShieldPercent == 375) {
          if (Creature *overseer = GetSingleCreatureFromStorage(NPC_OVERSEER_SHARTUUL))
            if (Unit *demon = GetCurrentDemon())
              DoScriptText(m_shartuulShieldPercent == 750 ? SAY_SECOND_HAMMER : SAY_FIFTH_HAMMER, overseer,
                           demon->GetOwner());
        }
  ...
  bool CheckConditionCriteriaMeet(Player const *player, uint32 instanceConditionId, WorldObject const *conditionSource,
                                  uint32 conditionSourceType) const override {
    ...
    script_error_log("world_map_outland::CheckConditionCriteriaMeet called with unsupported Id %u. Called with param "
                     "plr %s, src %s, condition source type %u",
  ...
  uint32 CalculateBashirTimerValue() { return 60 - m_bashirTime.tm_min + 60 * (m_bashirTime.tm_hour % 2); }
  ...
  void StartBashirLandingEvent() {
    ...
    m_apprentice->SummonCreature(NPC_SKYGUARD_AETHER_TECH, bashirSpawnPositions[0][0], bashirSpawnPositions[0][1],
                                 bashirSpawnPositions[0][2], bashirSpawnPositions[0][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 1000, true);
    Creature *ranger = m_apprentice->SummonCreature(
        NPC_SKYGUARD_RANGER, bashirSpawnPositions[1][0], bashirSpawnPositions[1][1], bashirSpawnPositions[1][2],
        bashirSpawnPositions[1][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 2);
    ranger = m_apprentice->SummonCreature(NPC_SKYGUARD_RANGER, bashirSpawnPositions[2][0], bashirSpawnPositions[2][1],
                                          bashirSpawnPositions[2][2], bashirSpawnPositions[2][3],
                                          TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 3);
    m_apprentice->SummonCreature(NPC_SKYGUARD_LIEUTENANT, bashirSpawnPositions[3][0], bashirSpawnPositions[3][1],
                                 bashirSpawnPositions[3][2], bashirSpawnPositions[3][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 1000, true);
    ranger = m_apprentice->SummonCreature(NPC_SKYGUARD_RANGER, bashirSpawnPositions[4][0], bashirSpawnPositions[4][1],
                                          bashirSpawnPositions[4][2], bashirSpawnPositions[4][3],
                                          TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
  ...
  void SpawnAlliesAtCrystalforge(BashirPhases phaseId, Player *player) {
    player->SummonCreature(NPC_SKYGUARD_LIEUTENANT, bashirCustomSpawnPositions[0][0], bashirCustomSpawnPositions[0][1],
                           bashirCustomSpawnPositions[0][2], bashirCustomSpawnPositions[0][3],
                           TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    Creature *ranger = player->SummonCreature(
        NPC_SKYGUARD_RANGER, bashirCustomSpawnPositions[1][0], bashirCustomSpawnPositions[1][1],
        bashirCustomSpawnPositions[1][2], bashirCustomSpawnPositions[1][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 2);
    ranger = player->SummonCreature(NPC_SKYGUARD_RANGER, bashirCustomSpawnPositions[2][0],
                                    bashirCustomSpawnPositions[2][1], bashirCustomSpawnPositions[2][2],
                                    bashirCustomSpawnPositions[2][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 3);
    Creature *tech = player->SummonCreature(
        NPC_SKYGUARD_AETHER_TECH, bashirCustomSpawnPositions[3][0], bashirCustomSpawnPositions[3][1],
        bashirCustomSpawnPositions[3][2], bashirCustomSpawnPositions[3][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    tech->HandleEmote(EMOTE_STATE_USESTANDING_NOSHEATHE);
    ranger = player->SummonCreature(NPC_SKYGUARD_RANGER, bashirCustomSpawnPositions[4][0],
                                    bashirCustomSpawnPositions[4][1], bashirCustomSpawnPositions[4][2],
                                    bashirCustomSpawnPositions[4][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 4);
    Creature *vendor;
    switch (phaseId) {
    case BASHIR_PHASE_ALL_VENDORS_SPAWNED:
      vendor = player->SummonCreature(NPC_AETHER_TECH_MASTER, bashirCustomSpawnPositions[7][0],
                                      bashirCustomSpawnPositions[7][1], bashirCustomSpawnPositions[7][2],
                                      bashirCustomSpawnPositions[7][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
      vendor->HandleEmote(EMOTE_STATE_USESTANDING);
      vendor->SetCanFly(false);
      // no break
    case BASHIR_PHASE_TRANSITION_3:
    case BASHIR_PHASE_3:
      vendor = player->SummonCreature(NPC_AETHER_TECH_ADEPT, bashirCustomSpawnPositions[6][0],
                                      bashirCustomSpawnPositions[6][1], bashirCustomSpawnPositions[6][2],
                                      bashirCustomSpawnPositions[6][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
      vendor->HandleEmote(EMOTE_STATE_USESTANDING);
      vendor->SetCanFly(false);
      // no break
    case BASHIR_PHASE_TRANSITION_2:
    case BASHIR_PHASE_2:
      vendor = player->SummonCreature(NPC_AETHER_TECH_ASSISTANT, bashirCustomSpawnPositions[5][0],
                                      bashirCustomSpawnPositions[5][1], bashirCustomSpawnPositions[5][2],
                                      bashirCustomSpawnPositions[5][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
      vendor->HandleEmote(EMOTE_STATE_USESTANDING);
      vendor->SetCanFly(false);
      break;
    default:
  ...
  void HandleBashirMobDeath(Creature *creature) {
    ...
    case BASHIR_PHASE_1:
      bossSpawnCount = PHASE_1_KILL_COUNT;
      break;
    case BASHIR_PHASE_2:
      bossSpawnCount = PHASE_2_KILL_COUNT;
      break;
    case BASHIR_PHASE_3:
      bossSpawnCount = PHASE_3_KILL_COUNT;
      break;
    default:
  ...
  void SpawnRandomBashirMob() {
    ...
    Creature *tech = GetSingleCreatureFromStorage(
        NPC_SKYGUARD_AETHER_TECH); // Has to be alive during this so we can use him for summoning
    uint32 entry;
    switch (data->phaseId) {
    case BASHIR_PHASE_1:
      entry = NPC_SLAVERING_SLAVE;
      break;
    case BASHIR_PHASE_2:
      entry = NPC_BASHIR_SUBPRIMAL;
      break;
    case BASHIR_PHASE_3:
      entry = urand(0, 1) ? NPC_BASHIR_CONTROLLER : NPC_BASHIR_RECKONER;
      break;
    }
  ...
  void SpawnTowerDefenders() {
    ...
    for (BashirCombatSpawn &data : bashirCombatDisruptorSpawnLocations) {
      Creature *mob =
          tech->SummonCreature(NPC_BASHIR_SUBPRIMAL, data.x, data.y, data.z, data.ori, TEMPSPAWN_DEAD_DESPAWN, 0, true);
      mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
    }
  }
  ...
  uint32 GetActionTimer(OutlandActions action) {
    switch (action) {
    case BASHIR_ACTION_INTRO:
      return 0;
    case BASHIR_ACTION_SPAWN_ENEMY:
      return 20000;
  ...
  void StartTransitionPhase() {
    ...
    case BASHIR_PHASE_TRANSITION_1:
      vendor = tech->SummonCreature(NPC_AETHER_TECH_ASSISTANT, bashirSpawnPositions[5][0], bashirSpawnPositions[5][1],
                                    bashirSpawnPositions[5][2], bashirSpawnPositions[5][3],
                                    TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true, true, 1);
      break;
    case BASHIR_PHASE_TRANSITION_2:
      vendor = tech->SummonCreature(NPC_AETHER_TECH_ADEPT, bashirSpawnPositions[6][0], bashirSpawnPositions[6][1],
                                    bashirSpawnPositions[6][2], bashirSpawnPositions[6][3],
                                    TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true, true, 1);
      break;
    case BASHIR_PHASE_TRANSITION_3:
      vendor = tech->SummonCreature(NPC_AETHER_TECH_MASTER, bashirSpawnPositions[7][0], bashirSpawnPositions[7][1],
                                    bashirSpawnPositions[7][2], bashirSpawnPositions[7][3],
                                    TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true, true, 1);
      break;
    default:
      break;
  ...
  void SpawnBoss(BashirPhases phase) {
    ...
    switch (phase) {
    case BASHIR_PHASE_1:
      mob = tech->SummonCreature(NPC_BASHIR_FLESH_FIEND, bashirSpawnPositions[8][0], bashirSpawnPositions[8][1],
                                 bashirSpawnPositions[8][2], bashirSpawnPositions[8][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 2 * 60 * IN_MILLISECONDS, true);
      mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
      break;
    case BASHIR_PHASE_2:
      SpawnTowerDefenders();
      mob = tech->SummonCreature(NPC_DISRUPTOR_TOWER, bashirSpawnPositions[9][0], bashirSpawnPositions[9][1],
                                 bashirSpawnPositions[9][2], bashirSpawnPositions[9][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 2 * 60 * IN_MILLISECONDS, true);
      mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
      mob = tech->SummonCreature(NPC_DISRUPTOR_TOWER, bashirSpawnPositions[10][0], bashirSpawnPositions[10][1],
                                 bashirSpawnPositions[10][2], bashirSpawnPositions[10][3],
                                 TEMPSPAWN_CORPSE_TIMED_DESPAWN, 2 * 60 * IN_MILLISECONDS, true);
      mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
      mob = tech->SummonCreature(NPC_DISRUPTOR_TOWER, bashirSpawnPositions[11][0], bashirSpawnPositions[11][1],
                                 bashirSpawnPositions[11][2], bashirSpawnPositions[11][3],
                                 TEMPSPAWN_CORPSE_TIMED_DESPAWN, 2 * 60 * IN_MILLISECONDS, true);
      mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
      break;
    case BASHIR_PHASE_3:
      mob = tech->SummonCreature(NPC_GRAND_COLLECTOR, bashirSpawnPositions[12][0], bashirSpawnPositions[12][1],
                                 bashirSpawnPositions[12][2], bashirSpawnPositions[12][3],
                                 TEMPSPAWN_CORPSE_TIMED_DESPAWN, 2 * 60 * IN_MILLISECONDS, true);
      mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
      break;
    default:
  ...
  void ScriptAction(uint32 eventId) {
    ...
    case 5: // talk to lieutenant
      DoScriptText(SAY_RANGER_1, GetSingleCreatureFromStorage(NPC_SKYGUARD_RANGER),
                   GetSingleCreatureFromStorage(NPC_SKYGUARD_LIEUTENANT)); // uses last ranger
      break;
  ...
  Creature *GetShartuulManager() { return GetSingleCreatureFromStorage(NPC_FEL_PORTAL_ALARM); }
  ...
  Player *GetCurrentPlayer() { return instance->GetPlayer(m_playerGuid); }
  ...
  uint32 GetSpellIdForEntry(uint32 entry) {
    ...
    case NPC_FEL_IMP_DEFENDER:
      return SPELL_LEGION_RING_FEL_IMP_TRANSFORM;
    case NPC_FELHOUND_DEFENDER:
      return SPELL_LEGION_RING_FEL_HOUND_TRANSFORM;
    case NPC_GANARG_UNDERLING:
      return SPELL_LEGION_RING_INFERNAL_TRANSFORM;
    case NPC_MOARG_TORMENTER:
      return SPELL_LEGION_RING_MOARG_TRANSFORM;
    }
  }
  ...
  void HandleShartuulAddSpawns() {
    ...
    if (m_shartuulPhase == PHASE_3_DOOMGUARD_PUNISHER_ADDS) {
      ...
      if (m_shartuulWave == 0 || m_shartuulWave == 3 || m_shartuulWave == 7) {
        ...
        case 0:
          textId = SAY_FIRST_SPAWNS;
          break;
        case 3:
          textId = SAY_FIRST_MOARG;
          break;
        case 7: {
          if (Creature *demon = GetCurrentDemon())
            demon->SummonCreature(NPC_SHIVAN_ASSASSIN, m_spawnPosition.x, m_spawnPosition.y, m_spawnPosition.z,
                                  m_spawnPosition.o, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 300000, true);
  ...
  void HandleShartuulAddAttack() {
    for (auto guid : m_shartuulAttackAdds) {
      if (Creature *spawn = instance->GetCreature(guid)) {
        ...
        spawn->SetFactionTemporary(SHARTUUL_FACTION_HOSTILE,
  ...
  void HandleShartuulEventReset() {
    ...
    GuidVector spawns;
    std::vector<uint32> spawnEntries = {NPC_FELHOUND_DEFENDER, NPC_FEL_IMP_DEFENDER, NPC_GANARG_UNDERLING,
                                        NPC_MOARG_TORMENTER};
    ...
    for (auto &data : pillarTargeting)
      if (Creature *pillarSource =
              instance->GetCreature(ObjectGuid(HIGHGUID_UNIT, NPC_LEGION_RING_EVENT_INVISMAN, data.first)))
  ...
  void HandleShartuulSpawnSequence() {
    ...
    case 0: {
      WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_INVISIBLE_STALKER_LARGE_AOI, shartuulSpawnPosition.x,
                                                    shartuulSpawnPosition.y, shartuulSpawnPosition.z,
                                                    shartuulSpawnPosition.o, TEMPSPAWN_TIMED_DESPAWN, 27000),
                                  instance);
      WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_LEGION_RING_EVENT_INVISMAN_LG, shartuulSpawnPosition.x,
                                                    shartuulSpawnPosition.y, shartuulSpawnPosition.z + 13.f,
                                                    shartuulSpawnPosition.o, TEMPSPAWN_TIMED_DESPAWN, 27000),
                                  instance);
      timer = 2000;
      break;
    ...
    case 6: {
      WorldObject::SummonCreature(TempSpawnSettings(GetCurrentPlayer(), NPC_SHARTUUL, shartuulSpawnPosition.x,
                                                    shartuulSpawnPosition.y, shartuulSpawnPosition.z,
                                                    shartuulSpawnPosition.o, TEMPSPAWN_TIMED_DESPAWN, 1200000),
                                  instance);
      timer = 5000;
      break;
    }
    ...
    case 10: {
      if (Creature *shartuul = GetSingleCreatureFromStorage(NPC_SHARTUUL))
        if (Creature *cannon =
                instance->GetCreature(ObjectGuid(HIGHGUID_UNIT, uint32(NPC_PORTABLE_FEL_CANNON), 161399u)))
          shartuul->SummonCreature(NPC_DREADMAW, cannon->GetPositionX(), cannon->GetPositionY(), cannon->GetPositionZ(),
                                   cannon->GetOrientation(), TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000, true);
      timer = 3000;
      break;
    }
    case 11: {
  ...
  void OnCreatureCreate(Creature *creature) override {
    ...
    case NPC_FURYWING:
    case NPC_INSIDION:
      if (creature->IsTemporarySummon()) // Only dragons summoned by the player (by using the respective egg
                                         // gameobjects) should have UNIT_FLAG_IMMUNE_TO_PLAYER
  ...
  void OnCreatureRespawn(Creature *creature) override {
    ...
    case NPC_FEL_EYE_STALK:
      if (m_shartuulPhase == PHASE_7_SHIVAN_ASSASSIN_BOSS_3) {
        ...
        static_cast<Creature *>(creature)->SetFactionTemporary(
  ...
  void OnCreatureDeath(Creature *creature) override {
    ...
    case NPC_UNLEASHED_HELLION:
      if (!GetSingleCreatureFromStorage(NPC_EMISSARY_OF_HATE, true)) {
        ...
        if (m_uiEmissaryOfHate_KilledAddCount == 6) {
          creature->SummonCreature(NPC_EMISSARY_OF_HATE, aSpawnLocations[POS_IDX_EMISSARY_SPAWN][0],
                                   aSpawnLocations[POS_IDX_EMISSARY_SPAWN][1],
                                   aSpawnLocations[POS_IDX_EMISSARY_SPAWN][2],
                                   aSpawnLocations[POS_IDX_EMISSARY_SPAWN][3], TEMPSPAWN_DEAD_DESPAWN, 0);
          m_uiEmissaryOfHate_KilledAddCount = 0;
        }
      }
      break;
    ...
    case NPC_BASHIR_RECKONER:
      m_bashirEnemyWaveSpawns.erase(
          std::remove(m_bashirEnemyWaveSpawns.begin(), m_bashirEnemyWaveSpawns.end(), creature->GetObjectGuid()),
          m_bashirEnemyWaveSpawns.end());
    ...
    if (m_shartuulTimer <= diff) {
      ...
      if (m_shartuulPhase != PHASE_0_SHARTUUL_DISABLED || (demon && demon->GetCharmer())) {
        ...
        if (demon) {
          ...
          if (bottomLine.closestPoint(curPos).x > curPos.x || topLine.closestPoint(curPos).x < curPos.x ||
              leftLine.closestPoint(curPos).y < curPos.y || rightLine.closestPoint(curPos).y > curPos.y)
            fail = true;
        }
  ...
  void ShowChatCommands(ChatHandler *handler) override {
    handler->SendSysMessage("This instance supports the following commands:\n bashir (0,1,2,3,4,5,6,7) starts event at "
                            "stage respectively - start event, start phase 1, finish phase 1,"
  ...
  void ExecuteChatCommand(ChatHandler *handler, char *args) override {
    ...
    if (val == "bashir") {
      ...
      if (msg == EQUIP_ERR_OK) {
        Item *item = player->StoreNewItem(dest, ITEM_CRYSTALFORGED_DARKRUNE, true,
                                          Item::GenerateItemRandomPropertyId(ITEM_CRYSTALFORGED_DARKRUNE));
        if (item)
