...
struct mob_lumpAI : public ScriptedAI {
  ...
  void DamageTaken(Unit * /*dealer*/, uint32 &damage, DamageEffectType /*damagetype*/,
    ...
    if (m_creature->GetHealth() < damage ||
        (m_creature->GetHealth() - damage) * 100 / m_creature->GetMaxHealth() < 30) {
      damage = std::min(damage, m_creature->GetHealth() - 1);

      m_creature->RemoveAllAuras();
      m_creature->CombatStop(true);

      // should get unit_flags UNIT_FLAG_IMMUNE_TO_PLAYER | UNIT_FLAG_IMMUNE_TO_NPC at faction change, but unclear
      // why/for what reason, skipped (no flags expected as default)
...
UnitAI *GetAI_mob_lump(Creature *pCreature) { return new mob_lumpAI(pCreature); }
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit * /*pSender*/, Unit *pInvoker, uint32 uiMiscValue) override {
    if (eventType == AI_EVENT_START_ESCORT && pInvoker->GetTypeId() == TYPEID_PLAYER) {
      if (m_creature->GetEntry() == NPC_MAGHAR_CAPTIVE) {
        ...
        if (Creature *summoned = m_creature->SummonCreature(NPC_MURK_BRUTE, -1574.611f, 8547.862f, 2.084420f, 2.32f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000))
          DoScriptText(SAY_MURK_BRUTE_NO_ESCAPE, summoned);
        m_creature->SummonCreature(NPC_MURK_RAIDER, -1576.695f, 8546.507f, 2.0844190f, 1.902409f,
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000);
        ...
        if (Creature *summoned = m_creature->SummonCreature(NPC_MURK_BRUTE, -1530.8f, 8455.717f, -4.019422f, 0.084f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000))
          DoScriptText(SAY_MURK_BRUTE_NO_ESCAPE, summoned);
        m_creature->SummonCreature(NPC_MURK_RAIDER, -1514.549f, 8477.511f, -4.015618f, 4.43f,
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000);
    ...
    } else if (eventType == AI_EVENT_CUSTOM_EVENTAI_A) // sent by brutes 5 minutes after they die to remove the
                                                       // questgiver flag if nobody took up the quest
    {
  ...
  void WaypointReached(uint32 uiPointId) override {
    if (m_creature->GetEntry() == NPC_MAGHAR_CAPTIVE) {
      ...
      case 5:
        ...
        break;
      case 6:
        if (Creature *summoned = m_creature->SummonCreature(NPC_MURK_BRUTE, -1496.662f, 8508.388f, 1.015174f, 2.56f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000))
          DoScriptText(SAY_MURK_BRUTE_WHERE, summoned);
        m_creature->SummonCreature(NPC_MURK_PUTRIFIER, -1494.623f, 8505.492f, 1.173438f, 2.63f,
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000);
        m_creature->SummonCreature(NPC_MURK_SCAVENGER, -1497.349f, 8505.020f, 1.107700f, 2.56f,
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000);
        break;
      case 11:
      ...
      case 8:
        ...
        break;
      case 9:
        if (Creature *summoned = m_creature->SummonCreature(NPC_MURK_BRUTE, -1417.57f, 8516.55f, 8.593721f, 3.76f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000))
          DoScriptText(SAY_MURK_BRUTE_WHERE, summoned);
        m_creature->SummonCreature(NPC_MURK_PUTRIFIER, -1411.089f, 8507.651f, 8.976571f, 3.21f,
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000);
        m_creature->SummonCreature(NPC_MURK_SCAVENGER, -1440.539f, 8490.212f, 6.207497f, 1.03f,
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 10000);
        break;
      case 10:
...
UnitAI *GetAI_npc_nagrand_captive(Creature *pCreature) { return new npc_nagrand_captiveAI(pCreature); }

/*######
## npc_creditmarker_visist_with_ancestors (Quest 10085)
######*/

enum { QUEST_VISIT_WITH_ANCESTORS = 10085 };
    ...
    if (m_uiAbyssalTossTimer < uiDiff) {
      if (Unit *victim = m_creature->GetVictim()) {
        if (!m_creature->CanReachWithMeleeAttack(victim)) {
          Creature *rethedronTarget =
              m_creature->SummonCreature(NPC_RETHHEDRONS_TARGET, victim->GetPositionX(), victim->GetPositionY(),
                                         victim->GetPositionZ(), 0, TEMPSPAWN_TIMED_OR_CORPSE_DESPAWN, 30000);
          ;
