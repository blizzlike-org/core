...
struct npc_dirty_larryAI : public ScriptedAI {
  ...
  uint32 NextStep(uint32 uiStep) {
    ...
    case 2:
      ...
      return 2000;
    case 3:
      DoScriptText(SAY_COUNT, m_creature, pPlayer);
      return 5000;
    case 4:
      DoScriptText(SAY_COUNT_1, m_creature, pPlayer);
      return 2000;
    case 5:
      DoScriptText(SAY_COUNT_2, m_creature, pPlayer);
      return 2000;
    case 6:
      DoScriptText(SAY_ATTACK, m_creature, pPlayer);
      return 2000;
    case 7:
...
UnitAI *GetAI_npc_dirty_larry(Creature *pCreature) { return new npc_dirty_larryAI(pCreature); }
...
struct npc_salsalabimAI : public ScriptedAI {
  ...
  void Reset() override { m_uiMagneticPullTimer = 15000; }

  void DamageTaken(Unit *pDoneBy, uint32 &damage, DamageEffectType /*damagetype*/,
                   SpellEntry const * /*spellInfo*/) override {
...
UnitAI *GetAI_npc_salsalabim(Creature *pCreature) { return new npc_salsalabimAI(pCreature); }
...
bool QuestRewarded_npc_adal(Player *player, Creature *creature, Quest const *quest) {
  ...
  case QUEST_KAELTHAS_AND_THE_VERDANT_SPHERE:
    ...
    player->GetMap()->ScriptsStart(sRelayScripts, SCRIPT_RELAY_ID, creature, player,
                                   Map::SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE); // only once active per adal
    return true;                                                             // handled
  }
