...
enum {
  ...
  SPELL_GROUND_SLAM = 33525, // AoE Ground Slam applying Ground Slam to everyone with a script effect (most likely the
                             // knock back, we can code it to a set knockback)
...
struct boss_gruulAI : public CombatAI {
  boss_gruulAI(Creature *creature)
      : CombatAI(creature, GRUUL_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
