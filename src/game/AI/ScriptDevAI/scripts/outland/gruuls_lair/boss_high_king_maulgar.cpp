...
struct boss_high_king_maulgarAI : public CombatAI {
  boss_high_king_maulgarAI(Creature *creature)
      : CombatAI(creature, MAULGAR_ACTION_MAX),
  ...
  void EventCouncilDeath() {
    ...
    case 1:
      DoScriptText(SAY_OGRE_DEATH1, m_creature);
      break;
    case 2:
      DoScriptText(SAY_OGRE_DEATH2, m_creature);
      break;
    case 3:
      DoScriptText(SAY_OGRE_DEATH3, m_creature);
      break;
    case 4:
      DoScriptText(SAY_OGRE_DEATH4, m_creature);
      break;
    }
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case MAULGAR_CHARGE:
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_CHARGE, SELECT_FLAG_PLAYER))
...
struct Council_Base_AI : public RangedCombatAI {
  Council_Base_AI(Creature *creature, uint32 actions)
      : RangedCombatAI(creature, actions), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {}
...
struct boss_kiggler_the_crazedAI : public Council_Base_AI {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case KIGGLER_ACTION_ARCANE_EXPLOSION:
      if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_ARCANE_EXPLOSION,
...
struct boss_krosh_firehandAI : public Council_Base_AI {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case KROSH_BLAST_WAVE:
      if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, nullptr,
                                            SELECT_FLAG_PLAYER | SELECT_FLAG_USE_EFFECT_RADIUS))
