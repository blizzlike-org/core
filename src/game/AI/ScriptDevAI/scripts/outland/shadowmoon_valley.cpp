...
SD%Complete: 100
SDComment: Quest support: 10451, 10458, 10480, 10481, 10514, 10540, 10588, 10707, 10781, 10804, 10854, 10985, 11020,
11064, 11067, 11068, 11069, 11070, 11071. SDCategory: Shadowmoon Valley EndScriptData */
...
struct mob_mature_netherwing_drakeAI : public ScriptedAI {
  ...
  void SpellHit(Unit *pCaster, SpellEntry const *pSpell) override {
    ...
    if (pCaster->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_PLACE_CARCASS &&
        !m_creature->HasAura(SPELL_JUST_EATEN)) {
...
UnitAI *GetAI_mob_mature_netherwing_drake(Creature *pCreature) { return new mob_mature_netherwing_drakeAI(pCreature); }
          ...
          if (Player *pPlayer = m_creature->GetMap()->GetPlayer(m_playerGuid)) {
            ...
            if (Creature *pEscapeDummy = GetClosestCreatureWithEntry(m_creature, NPC_ESCAPE_DUMMY, 50.0f))
              ...
              m_creature->GetRandomPoint(m_creature->GetPositionX(), m_creature->GetPositionY(),
                                         m_creature->GetPositionZ(), 20.0f, fX, fY, fZ);
              fZ += 25;
            }
...
bool EffectDummyCreature_npc_dragonmaw_peon(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
struct npc_wildaAI : public npc_escortAI {
  ...
  void DoSpawnAssassin(uint8 uiCount = 1) {
    ...
    for (uint8 i = 0; i < uiCount; ++i) {
      m_creature->GetRandomPoint(m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(),
                                 10.0f, fX, fY, fZ);
    ...
    switch (urand(0, 6)) {
    case 0:
      DoScriptText(SAY_WIL_PROGRESS_1, m_creature);
      break;
    case 1:
      DoScriptText(SAY_WIL_PROGRESS_2, m_creature);
      break;
    case 2:
      DoScriptText(SAY_WIL_PROGRESS_3, m_creature);
      break;
    case 3:
      DoScriptText(SAY_WIL_PROGRESS_4, m_creature);
      break;
    case 4:
      DoScriptText(SAY_WIL_PROGRESS_5, m_creature);
      break;
    case 5:
      DoScriptText(SAY_WIL_AGGRO_1, m_creature);
      break;
    case 6:
      DoScriptText(SAY_WIL_AGGRO_2, m_creature);
      break;
    }
  }
  ...
  void DoFreeSpirits() {
    ...
    for (Creature *spirit : lSpiritsInRange) {
      ...
      spirit->StopMoving();
      spirit->GetMotionMaster()->MoveFollow(m_creature, m_creature->GetDistance(spirit) * 0.25f,
                                            M_PI_F / 2 + m_creature->GetAngle(spirit), true);
...
UnitAI *GetAI_npc_wilda(Creature *pCreature) { return new npc_wildaAI(pCreature); }
...
static TorlothCinematic TorlothAnim[] = {
    {SAY_TORLOTH_DIALOGUE1, TORLOTH, 2000}, {SAY_ILLIDAN_DIALOGUE, LORD_ILLIDAN, 7000},
    {SAY_TORLOTH_DIALOGUE2, TORLOTH, 3000}, {0, TORLOTH, 1000}, // Torloth stand
    {SAY_TORLOTH_DIALOGUE3, TORLOTH, 7000}, {0, TORLOTH, 0},    // Torloth attacks
};
...
struct npc_lord_illidan_stormrageAI : public Scripted_NoMovementAI {
  ...
  void SummonWave() {
    ...
    for (uint8 i = 0; i < uiCount; ++i) {
      ...
      if (Creature *pSpawn = m_creature->SummonCreature(WavesInfo[m_uiWaveCount].uiCreatureId, fLocX, fLocY, fLocZ,
                                                        fOrient, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 30000)) {
...
struct npc_totem_of_spiritsAI : public ScriptedAI {
  ...
  void JustSummoned(Creature *summoned) override {
    ...
    summoned->GetMotionMaster()->MovePoint(1, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                           m_creature->GetPositionZ() + 4);
  }
...
bool EffectDummyCreature_npc_totem_of_spirits(Unit * /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                              Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/) {
...
bool ProcessEventId_event_spell_soul_captured_credit(uint32 uiEventId, Object *pSource, Object * /*pTarget*/,
                                                     bool bIsStart) {
...
const static WaveCoords DeathwailDescentCoords[] = {
    {-3245.675f, 298.3688f, 171.9848f}, {-3251.500f, 301.2324f, 167.3459f}, {-3258.824f, 302.1381f, 162.9848f},
    {-3267.146f, 297.6195f, 157.9292f}, {-3266.891f, 283.9230f, 155.4848f}, {-3261.760f, 278.8767f, 150.5682f},
    {-3255.110f, 278.1188f, 147.8738f}, {-3248.274f, 278.3419f, 143.6453f}, {-3245.108f, 287.5689f, 142.7843f},
...
struct npc_shadowlord_deathwailAI : public ScriptedAI {
  npc_shadowlord_deathwailAI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
  ...
  void DoSummonWave(bool both = false) {
    if (both) {
      ...
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupOneSpawnCoords[0].xCoord,
                                 WaveGroupOneSpawnCoords[0].yCoord, WaveGroupOneSpawnCoords[0].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 1);
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupOneSpawnCoords[1].xCoord,
                                 WaveGroupOneSpawnCoords[1].yCoord, WaveGroupOneSpawnCoords[1].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 2);
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupOneSpawnCoords[2].xCoord,
                                 WaveGroupOneSpawnCoords[2].yCoord, WaveGroupOneSpawnCoords[2].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 3);

      // Wave starting outdoors
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupTwoSpawnCoords[0].xCoord,
                                 WaveGroupTwoSpawnCoords[0].yCoord, WaveGroupTwoSpawnCoords[0].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 4);
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupTwoSpawnCoords[1].xCoord,
                                 WaveGroupTwoSpawnCoords[1].yCoord, WaveGroupTwoSpawnCoords[1].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 5);
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupTwoSpawnCoords[2].xCoord,
                                 WaveGroupTwoSpawnCoords[2].yCoord, WaveGroupTwoSpawnCoords[2].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 6);
    } else {
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupThreeSpawnCoords[0].xCoord,
                                 WaveGroupThreeSpawnCoords[0].yCoord, WaveGroupThreeSpawnCoords[0].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 1);
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupThreeSpawnCoords[1].xCoord,
                                 WaveGroupThreeSpawnCoords[1].yCoord, WaveGroupThreeSpawnCoords[1].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 2);
      m_creature->SummonCreature(NPC_SHADOWMOON_RETAINER, WaveGroupThreeSpawnCoords[2].xCoord,
                                 WaveGroupThreeSpawnCoords[2].yCoord, WaveGroupThreeSpawnCoords[2].zCoord, 0.0f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, RETAINER_DESPAWN_TIME, true, true, 3);
    }
  }
  ...
  void DoBeginDescent() {
    ...
    m_creature->GetMotionMaster()->MovePoint(0, DeathwailDescentCoords[0].xCoord, DeathwailDescentCoords[0].yCoord,
                                             DeathwailDescentCoords[0].zCoord);
  ...
  void MovementInform(uint32 motionType, uint32 pointId) override {
    ...
    case 7:
    case 8:
      m_creature->GetMotionMaster()->MovePoint(pointId + 1, DeathwailDescentCoords[pointId + 1].xCoord,
                                               DeathwailDescentCoords[pointId + 1].yCoord,
                                               DeathwailDescentCoords[pointId + 1].zCoord);
      break;
    case 9:
          ...
          if (Player *player = m_creature->GetMap()->GetPlayer(attacker->GetObjectGuid()))
            if (Group *group = player->GetGroup())
              for (GroupReference *ref = group->GetFirstMember(); ref != nullptr; ref = ref->next()) {
                ...
                if (member && member->IsAlive() && m_cHOFVisualTrigger &&
                    m_cHOFVisualTrigger->IsWithinDistInMap(member, MAX_PLAYER_DISTANCE))
  ...
  bool IsPlayerOrGroupInRangeAndActiveInEvent() {
    ...
    if (Player *player = m_creature->GetMap()->GetPlayer(m_playerGuid)) // can find player
    {
      ...
      {
        if (player->IsAlive() && m_cHOFVisualTrigger &&
            m_cHOFVisualTrigger->IsWithinDistInMap(player, MAX_PLAYER_DISTANCE))
          return true;
      }
    }
      ...
      if (m_uiFelFireballTimer < uiDiff) {
        if (m_cDeathwailTrigger) {
          if (m_creature->CastSpell(m_cDeathwailTrigger, SPELL_FEL_FIREBALL, TRIGGERED_NONE) == SPELL_CAST_OK) {
            ...
            m_creature->SummonCreature(NPC_FELFIRE_SUMMONER, m_cDeathwailTrigger->GetPositionX(),
                                       m_cDeathwailTrigger->GetPositionY(), m_cDeathwailTrigger->GetPositionZ(), 0.0f,
                                       TEMPSPAWN_TIMED_DESPAWN, 6000, true);
          }
        }
...
UnitAI *GetAI_npc_shadowlord_deathwail(Creature *pCreature) { return new npc_shadowlord_deathwailAI(pCreature); }
...
struct mob_shadowmoon_soulstealerAI : public ScriptedAI {
  mob_shadowmoon_soulstealerAI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
...
struct npc_spawned_oronok_tornheartAI : public ScriptedAI, private DialogueHelper {
  ...
  void JustRespawned() override {
    ...
    Creature *borak = m_creature->SummonCreature(NPC_BORAK_SON_OF_ORONOK, -3419.458f, 1383.739f, 228.1865f, 5.532694f,
                                                 TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000, true, true);
    ...
    Creature *gromtor = m_creature->SummonCreature(NPC_GROMTOR_SON_OF_ORONOK, -3419.458f, 1383.739f, 228.1865f,
                                                   5.532694f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000, true, true);
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case NPC_CYRUKH_THE_FIRELORD: {
      Creature *cyrukh = m_creature->SummonCreature(NPC_CYRUKH_THE_FIRELORD, -3600.073f, 1799.803f, 39.70634f,
                                                    1.448623f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000, true, true);
    ...
    case NPC_REDEEMED_SPIRIT_OF_EARTH: {
      ...
      Creature *elemental = m_creature->SummonCreature(NPC_REDEEMED_SPIRIT_OF_AIR, aDamnationLocations[0].m_fX,
                                                       aDamnationLocations[0].m_fY, aDamnationLocations[0].m_fZ,
                                                       aDamnationLocations[0].m_fO, TEMPSPAWN_TIMED_DESPAWN, 32000);
      elemental->CastSpell(nullptr, SPELL_ELEMENTAL_SPAWN_IN, TRIGGERED_NONE);
      elemental = m_creature->SummonCreature(NPC_REDEEMED_SPIRIT_OF_EARTH, aDamnationLocations[1].m_fX,
                                             aDamnationLocations[1].m_fY, aDamnationLocations[1].m_fZ,
                                             aDamnationLocations[1].m_fO, TEMPSPAWN_TIMED_DESPAWN, 32000);
      elemental->CastSpell(nullptr, SPELL_ELEMENTAL_SPAWN_IN, TRIGGERED_NONE);
      elemental = m_creature->SummonCreature(NPC_REDEEMED_SPIRIT_OF_FIRE, aDamnationLocations[2].m_fX,
                                             aDamnationLocations[2].m_fY, aDamnationLocations[2].m_fZ,
                                             aDamnationLocations[2].m_fO, TEMPSPAWN_TIMED_DESPAWN, 32000);
      elemental->CastSpell(nullptr, SPELL_ELEMENTAL_SPAWN_IN, TRIGGERED_NONE);
      elemental = m_creature->SummonCreature(NPC_REDEEMED_SPIRIT_OF_WATER, aDamnationLocations[3].m_fX,
                                             aDamnationLocations[3].m_fY, aDamnationLocations[3].m_fZ,
                                             aDamnationLocations[3].m_fO, TEMPSPAWN_TIMED_DESPAWN, 32000);
      elemental->CastSpell(nullptr, SPELL_ELEMENTAL_SPAWN_IN, TRIGGERED_NONE);
      break;
    }
  ...
  Creature *GetSpeakerByEntry(uint32 uiEntry) override {
    ...
    case NPC_ORONOK_TORN_HEART:
      return m_creature;
    case NPC_EARTHMENDER_TORLOK:
      return m_creature->GetMap()->GetCreature(m_torlokGuid);
    case NPC_REDEEMED_SPIRIT_OF_EARTH:
      return m_creature->GetMap()->GetCreature(m_earthSpiritGuid);
    case NPC_REDEEMED_SPIRIT_OF_FIRE:
      return m_creature->GetMap()->GetCreature(m_fireSpiritGuid);
    case NPC_GROMTOR_SON_OF_ORONOK:
      return m_creature->GetMap()->GetCreature(m_gromtorGuid);
    case NPC_BORAK_SON_OF_ORONOK:
      return m_creature->GetMap()->GetCreature(m_borakGuid);
    case NPC_CYRUKH_THE_FIRELORD:
      return m_creature->GetMap()->GetCreature(m_cyrukhGuid);
    default:
      return nullptr;
    }
  }
  ...
  void JustSummoned(Creature *pSummoned) override {
    ...
    case NPC_REDEEMED_SPIRIT_OF_EARTH:
      ...
      break;
    case NPC_GROMTOR_SON_OF_ORONOK:
      m_gromtorGuid = pSummoned->GetObjectGuid();
      break;
    case NPC_BORAK_SON_OF_ORONOK:
      m_borakGuid = pSummoned->GetObjectGuid();
      break;
    case NPC_CYRUKH_THE_FIRELORD:
      m_cyrukhGuid = pSummoned->GetObjectGuid();
      break;
    }
  }
  ...
  void EnterEvadeMode() override {
    ...
    if (Creature *pCyrukh = GetSpeakerByEntry(NPC_CYRUKH_THE_FIRELORD)) {
      if (!pCyrukh->IsAlive()) {
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_EPILOGUE, aDamnationLocations[6].m_fX,
                                                 aDamnationLocations[6].m_fY, aDamnationLocations[6].m_fZ);
      ...
      script_error_log(
          "Npc %u couldn't be found or something really bad happened. Epilogue event for quest %u will stop.",
          NPC_CYRUKH_THE_FIRELORD, QUEST_CIPHER_OF_DAMNATION);
  ...
  void MovementInform(uint32 motionType, uint32 pointId) override {
    ...
    case POINT_MOTION_TYPE:
      PointMovementInform(motionType, pointId);
      break;
    case WAYPOINT_MOTION_TYPE:
      WaypointMotionType(motionType, pointId);
      break;
    default:
      break;
    }
  ...
  void WaypointMotionType(uint32 /*motionType*/, uint32 pointId) {
    ...
    case WAYPOINT_ID_INTRO_FINAL: {
      ...
      m_creature->GetMotionMaster()->Clear(false, true);
      m_creature->GetMotionMaster()->MoveIdle();
      m_creature->GetMotionMaster()->MovePoint(POINT_ID_ATTACK_READY, aDamnationLocations[4].m_fX,
                                               aDamnationLocations[4].m_fY, aDamnationLocations[4].m_fZ);
      if (Creature *borak = GetSpeakerByEntry(NPC_BORAK_SON_OF_ORONOK)) {
        borak->Mount(0);
...
bool GossipSelect_npc_spawned_oronok_tornheart(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/,
                                               uint32 uiAction) {
  if (uiAction == GOSSIP_ACTION_INFO_DEF + 1) {
    // Note: this movement expects MMaps.
    DoScriptText(SAY_ORONOK_ELEMENTS, pCreature);
    pCreature->GetMotionMaster()->MovePoint(POINT_ID_ELEMENTS, aDamnationLocations[5].m_fX, aDamnationLocations[5].m_fY,
                                            aDamnationLocations[5].m_fZ);
...
UnitAI *GetAI_npc_domesticated_felboar(Creature *pCreature) { return new npc_domesticated_felboarAI(pCreature); }

bool EffectDummyCreature_npc_shadowmoon_tuber_node(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                                   Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/) {
...
struct npc_veneratus_spawn_nodeAI : public Scripted_NoMovementAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    // Check for the spirit hunter in order to spawn Veneratus; this will replace missing spells 36614 (dummy periodic
    // spell) and 36616 (summon spell)
    if (pWho->GetEntry() == NPC_SPIRIT_HUNTER && m_creature->IsWithinDistInMap(pWho, 40.0f) &&
...
UnitAI *GetAI_npc_veneratus_spawn_node(Creature *pCreature) { return new npc_veneratus_spawn_nodeAI(pCreature); }
...
struct npc_disobedient_dragonmaw_peonAI : public ScriptedAI {
  npc_disobedient_dragonmaw_peonAI(Creature *creature) : ScriptedAI(creature) { Reset(); }
  ...
  void HandleBooterang(Unit *caster) {
    ...
    switch (urand(0, 5)) {
    case 0:
      DoScriptText(SAY_BOOTERANG1, m_creature, player);
      break;
    case 1:
      DoScriptText(SAY_BOOTERANG2, m_creature, player);
      break;
    case 2:
      DoScriptText(SAY_BOOTERANG3, m_creature, player);
      break;
    case 3:
      DoScriptText(SAY_BOOTERANG4, m_creature, player);
      break;
    case 4:
      DoScriptText(SAY_BOOTERANG5, m_creature, player);
      break;
    case 5:
      DoScriptText(SAY_BOOTERANG6, m_creature, player);
      break;
    }
    ...
    if (!m_creature->IsInCombat()) {
      ...
      if (m_booterangTimer) {
        if (m_booterangTimer <= diff) {
          ...
          if (node)
            node->GetNearPoint(m_creature, x, y, z, m_creature->GetObjectBoundingRadius(), 5.f,
                               node->GetAngle(m_creature));
          else // failsafe
            m_creature->GetPosition(x, y, z);
          m_creature->GetMotionMaster()->MovePoint(POINT_ARRIVED, x, y, z);
          m_booterangTimer = 0;
        } else
          m_booterangTimer -= diff;
      }
    } else {
      if (!m_creature->SelectHostileTarget() || !m_creature->GetVictim())
        return;
...
static SpawnData dragonmawSpawns[COMMANDER_COUNT][DRAGONMAW_SPAWN_COUNT] = { // orientations are not sniffed
    {
        // scryer
...
static DeadliestScriptInfo deadliestScriptInfo[COMMANDER_COUNT] = {
    ...
    {NPC_SCRYERS_DRAGONMAW_SKYBREAKER, NPC_SANCTUM_DEFENDER, LAST_POINT_HOBB, SAY_EVENT_ACCEPT_HOBB,
     SAY_EVENT_START_HOBB, SAY_EVENT_END_HOBB, SAY_EVENT_ACCEPT_ARCUS, QUEST_DEADLIEST_TRAP_SCRYERS},
    {NPC_ALDOR_DRAGONMAW_SKYBREAKER, NPC_ALTAR_DEFENDER, LAST_POINT_ARCUS, SAY_EVENT_ACCEPT_ARCUS,
     SAY_EVENT_START_ARCUS, SAY_EVENT_END_ARCUS, SAY_EVENT_ACCEPT_ARCUS, QUEST_DEADLIEST_TRAP_ALDOR}};

struct npc_commanderAI : public CombatAI {
  npc_commanderAI(Creature *creature, uint8 commanderId)
      : CombatAI(creature, COMMANDER_COMBAT_ACTION_MAX), m_defenderSpawns(DEFENDER_SPAWN_COUNT),
        m_dragonmawSpawns(DRAGONMAW_SPAWN_COUNT), m_killCounter(0), m_commanderId(commanderId) {
    m_attackDistance = 30.f;
    m_meleeEnabled = false;
    AddCombatAction(COMMANDER_COMBAT_ACTION_AIMED_SHOT,
                    GetInitialCombatActionTimer(COMMANDER_COMBAT_ACTION_AIMED_SHOT));
    AddCombatAction(COMMANDER_COMBAT_ACTION_MULTI_SHOT,
                    GetInitialCombatActionTimer(COMMANDER_COMBAT_ACTION_MULTI_SHOT));
    AddCombatAction(COMMANDER_COMBAT_ACTION_SHOOT, GetInitialCombatActionTimer(COMMANDER_COMBAT_ACTION_SHOOT));

    AddCustomAction(COMMANDER_ACTION_START_QUEST_FLAGS, true,
                    [&]() { m_creature->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER); });
    AddCustomAction(COMMANDER_ACTION_START_QUEST_TEXT, true, [&]() {
      DoScriptText(deadliestScriptInfo[m_commanderId].startText, m_creature,
                   m_creature->GetMap()->GetPlayer(m_startingPlayer));
    });
    AddCustomAction(COMMANDER_ACTION_START_QUEST_MOVEMENT, true,
                    [&]() { m_creature->GetMotionMaster()->MoveWaypoint(); });
    AddCustomAction(COMMANDER_ACTION_POST_MOVEMENT_TEXT, true, [&]() {
      DoScriptText(deadliestScriptInfo[m_commanderId].midText, m_creature,
                   m_creature->GetMap()->GetPlayer(m_startingPlayer));
    });
  ...
  void SpawnDefenders() {
    for (uint32 i = 0; i < DEFENDER_SPAWN_COUNT; ++i) {
      Creature *defender =
          m_creature->SummonCreature(deadliestScriptInfo[m_commanderId].defenderNpc, defenderSpawns[m_commanderId][i].x,
                                     defenderSpawns[m_commanderId][i].y, defenderSpawns[m_commanderId][i].z,
                                     defenderSpawns[m_commanderId][i].ori, TEMPSPAWN_CORPSE_DESPAWN, 1000, true);
  ...
  void SpawnSpecificDragonmaw(uint32 i) {
    Creature *dragonmaw = m_creature->SummonCreature(
        deadliestScriptInfo[m_commanderId].dragonmawNpc, dragonmawSpawns[m_commanderId][i].x,
        dragonmawSpawns[m_commanderId][i].y, dragonmawSpawns[m_commanderId][i].z, dragonmawSpawns[m_commanderId][i].ori,
        TEMPSPAWN_CORPSE_DESPAWN, 1000, true, true, i);
  ...
  virtual void StartAttackingEvent() { SpawnDragonmaw(); }
  ...
  void WinEvent() {
    ...
    DoScriptText(deadliestScriptInfo[m_commanderId].winText, m_creature,
                 m_creature->GetMap()->GetPlayer(m_startingPlayer));
  ...
  void FailEvent() {
    DoScriptText(deadliestScriptInfo[m_commanderId].deathText, m_creature,
                 m_creature->GetMap()->GetPlayer(m_startingPlayer));
    ResetEvent();
  }
  ...
  void JustDied(Unit *killer) override { FailEvent(); }
...
struct npc_commander_hobbAI : public npc_commanderAI {
  ...
  void StartAttackingEvent() override {
    ...
    m_creature->SetOrientation(5.340707f);
    m_creature->SetFacingTo(5.340707f);
...
struct npc_commander_arcusAI : public npc_commanderAI {
  npc_commander_arcusAI(Creature *creature) : npc_commanderAI(creature, COMMANDER_ARCUS) {
    AddCustomAction(COMMANDER_ACTION_POST_MOVEMENT_FACE_DIRECTION, true, [&]() {
      m_creature->SetOrientation(2.949606f);
      m_creature->SetFacingTo(2.949606f);
    });
  }
...
static DragonmawRacerScriptInfo dragonmawRacesScriptInfo[RACER_COUNT] = {
    {QUEST_MUCKJAW, SAY_START_MUCKJAW, 0, 0, SPELL_AGGRO_CHECK_MUCKJAW, SPELL_AGGRO_BURST_MUCKJAW,
     SAY_END_MUCKJAW}, // doesn't attack and has no target NPC
    {QUEST_TROPE, SAY_START_TROPE, SPELL_ATTACK_TROPE, NPC_TARGET_TROPE, SPELL_AGGRO_CHECK_TROPE,
     SPELL_AGGRO_BURST_TROPE, SAY_END_TROPE},
    {QUEST_CORLOK, SAY_START_CORLOK, SPELL_ATTACK_CORLOK, NPC_TARGET_CORLOK, SPELL_AGGRO_CHECK_CORLOK,
     SPELL_AGGRO_BURST_CORLOK, SAY_END_CORLOK},
    {QUEST_ICHMAN, SAY_START_ICHMAN, SPELL_ATTACK_ICHMAN, NPC_TARGET_ICHMAN, SPELL_AGGRO_CHECK_ICHMAN,
     SPELL_AGGRO_BURST_ICHMAN, SAY_END_ICHMAN},
    {QUEST_MULVERICK, SAY_START_MULVERICK, SPELL_ATTACK_MULVERICK, NPC_TARGET_MULVERICK, SPELL_AGGRO_CHECK_MULVERICK,
     SPELL_AGGRO_BURST_MULVERICK, SAY_END_MULVERICK},
    {QUEST_SKYSHATTER, SAY_START_SKYSHATTER, SPELL_ATTACK_SKYSHATTER, NPC_TARGET_SKYSHATTER,
     SPELL_AGGRO_CHECK_SKYSHATTER, SPELL_AGGRO_BURST_SKYSHATTER, SAY_END_SKYSHATTER}};
...
struct npc_dragonmaw_racerAI : public npc_escortAI {
  ...
  virtual void AttackPlayer() {
    if (Player *pPlayer = GetPlayerForEscort()) {
      ...
      m_creature->GetRandomPoint(m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(),
                                 15.0f, fX, fY, fZ);
      m_creature->SummonCreature(dragonmawRacesScriptInfo[m_racerId].targetNpc, fX, fY, fZ, 0, TEMPSPAWN_TIMED_DESPAWN,
                                 6000);
      m_creature->GetRandomPoint(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 10.0f, fX,
                                 fY, fZ);
      m_creature->SummonCreature(dragonmawRacesScriptInfo[m_racerId].targetNpc, fX, fY, fZ, 0, TEMPSPAWN_TIMED_DESPAWN,
                                 6000);
    }
...
struct npc_dragonmaw_racer_skyshatterAI : public npc_dragonmaw_racerAI {
  ...
  void AttackPlayer() override {
    if (Player *pPlayer = GetPlayerForEscort()) {
      float fX, fY, fZ;
      m_creature->GetRandomPoint(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 5.0f, fX,
                                 fY, fZ);
      m_creature->SummonCreature(dragonmawRacesScriptInfo[m_racerId].targetNpc, fX, fY, fZ, 0, TEMPSPAWN_TIMED_DESPAWN,
                                 6000);
    }
...
enum {
  ...
  // Spells
  SPELL_BATTLE_FLOW_REGULATOR = 39699, // The BT Battle Sensor NPC casts this spell/aura, presumably using it to control
                                       // the constant demon/aldor/scryer fight outside of BT, no clue how it works
...
struct mob_bt_battle_fighterAI : public ScriptedAI, public CombatActions {
  ...
  uint32 GetInitialActionTimer(const uint32 action) const {
    ...
    case NPC_SHADOWHOOF_ASSASSIN: {
      ...
      case ASSASSIN_DEBILITATING_STRIKE:
        return urand(3500, 4000);
      case ASSASSIN_SINISTER_STRIKE:
        return urand(6000, 8000);
      default:
        return 0;
      }
    }
    case NPC_ILLIDARI_SUCCUBUS: {
      ...
      case SUCCUBUS_LASH_OF_PAIN:
        return urand(2000, 4000);
      case SUCCUBUS_SEDUCTION:
        return urand(5000, 14000);
      default:
        return 0;
      }
    }
    case NPC_LIGHTSWORN_VINDICATOR: {
      ...
      case VINDICATOR_HAMMER:
        return urand(5000, 15000);
      case VINDICATOR_HOLY_LIGHT:
        return urand(2500, 4000);
      case VINDICATOR_SEAL_OF_SAC:
        return urand(1000, 4000);
      default:
        return 0;
      }
    }
    case NPC_ANCHORITE_CAALEN: {
      ...
      case CAALEN_HOLY_SMITE:
        return urand(3000, 9000);
      case CAALEN_PRAYER_OF_HEALING:
        return urand(9000, 12000);
      default:
        return 0;
      }
    }
  ...
  uint32 GetSubsequentActionTimer(const uint32 action) const {
    ...
    case NPC_ILLIDARI_RAVAGER: {
      switch (action) {
      case ILLIDARI_CLEAVE:
        return urand(9500, 12000);
      case ILLIDARI_CUTDOWN:
        return urand(14000, 16000);
      case ILLIDARI_DEMORALIZING_SHOUT:
        return urand(15000, 18000);
      default:
        return 0;
      }
    }
    case NPC_SHADOWHOOF_ASSASSIN: {
      switch (action) {
      case ASSASSIN_DEBILITATING_STRIKE:
        return urand(10000, 12000);
      case ASSASSIN_SINISTER_STRIKE:
        return urand(11000, 18000);
      default:
        return 0;
      }
    }
    ...
    case NPC_ANCHORITE_CAALEN: {
      switch (action) {
      case CAALEN_HOLY_SMITE:
        return urand(8000, 13000);
      case CAALEN_PRAYER_OF_HEALING:
        return urand(12000, 15000);
      default:
        return 0;
      }
    }
    ...
    case NPC_SHADOWLORD: {
      switch (action) {
      case SHADOWLORD_INFERNO:
        return urand(15000, 25000);
      case SHADOWLORD_CARRION_SWARM:
        return urand(10000, 12000);
      case SHADOWLORD_SLEEP:
        return urand(10000, 16000);
      default:
        return 0;
      }
    }
  ...
  void ExecuteActions() {
    ...
    switch (m_creature->GetEntry()) {
    case NPC_ILLIDARI_RAVAGER:
      ExecuteRavagerActions();
      break;
    case NPC_SHADOWHOOF_ASSASSIN:
      ExecuteAssassinActions();
      break;
    case NPC_ILLIDARI_SUCCUBUS:
      ExecuteSuccubusActions();
      break;
    case NPC_LIGHTSWORN_VINDICATOR:
      ExecuteVindicatorActions();
      break;
    case NPC_ANCHORITE_CAALEN:
      ExecuteCaalenActions();
      break;
    case NPC_SEASONED_MAGISTER:
      ExecuteMagisterActions();
      break;
    case NPC_SHADOWLORD:
      ExecuteShadowlordActions();
      break;
    }
  }
...
UnitAI *GetAI_mob_bt_battle_fighter(Creature *pCreature) { return new mob_bt_battle_fighterAI(pCreature); }
...
struct FormationMap {
  ...
  void registerFormationMarker(ObjectGuid marker) { markerToOccupant[marker] = ObjectGuid(); }
  ...
  ObjectGuid claimFormationMarker(ObjectGuid occupant) {
    ...
    for (std::map<ObjectGuid, ObjectGuid>::iterator itr = markerToOccupant.begin(); itr != markerToOccupant.end();
         ++itr)
...
struct npc_bt_battle_sensor : public ScriptedAI {
  npc_bt_battle_sensor(Creature *creature) : ScriptedAI(creature) { Reset(); }
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit *sender, Unit * /*invoker*/, uint32 /*miscValue*/) override {
    ...
    case NPC_ILLIDARI_RAVAGER:
      ...
      case AI_EVENT_CUSTOM_B: {
        if (Creature *senderCreature = dynamic_cast<Creature *>(sender))
          senderCreature->GetMotionMaster()->MoveRandomAroundPoint(sender->GetPositionX(), sender->GetPositionY(),
                                                                   sender->GetPositionZ(), 15.0f);
      ...
      case AI_EVENT_CUSTOM_EVENTAI_A: {
        ...
        if (!marker.IsEmpty())
          if (Creature *markerCreature = m_creature->GetMap()->GetCreature(marker)) {
            ...
            sender->GetMotionMaster()->MovePoint(0, markerCreature->GetPositionX(), markerCreature->GetPositionY(),
                                                 markerCreature->GetPositionZ());
    ...
    case NPC_BT_BATTLE_SENOR:
      ...
      if (eventType == AI_EVENT_CUSTOM_EVENTAI_A) {
        ...
        } else if (Creature *fyra = m_creature->GetMap()->GetCreature(m_fyraGuid))
  ...
  void ReinforceAldorScryerFrontline() {
    while ((m_uiNumLightswornForward + m_uiNumMagisterForward) < m_uiMaxNumTroopsForward) {
      ...
      if (urand(0, m_uiMaxNumTroopsForward + m_uiNumLightswornForward - m_uiNumMagisterForward) <
          m_uiMaxNumTroopsForward)
        ...
        if (Creature *fighterCreature = m_creature->GetMap()->GetCreature(fighter))
          if (!fighterCreature->IsInCombat())
            if (Creature *markerCreature = m_creature->GetMap()->GetCreature(marker)) {
              ...
              fighterCreature->GetMotionMaster()->MovePoint(
                  0, markerCreature->GetPositionX(), markerCreature->GetPositionY(), markerCreature->GetPositionZ());
...
UnitAI *GetAI_npc_bt_battle_sensor(Creature *pCreature) { return new npc_bt_battle_sensor(pCreature); }
