...
struct NetherCharge : public ScriptedAI, public TimerManager {
  ...
  void MoveToRandomPoint() {
    if (m_stopMoving)
      return;
    float x, y, z;
    m_creature->GetPosition(x, y, z); // do some urand radius shenanigans to spawn it further and make it walk to go
                                      // using doing X and Y yourself and using function in MAP to get proper Z
...
struct NetherChargePassive : public AuraScript {
  void OnApply(Aura *aura, bool /*apply*/) const { aura->ForcePeriodicity(2000); }
