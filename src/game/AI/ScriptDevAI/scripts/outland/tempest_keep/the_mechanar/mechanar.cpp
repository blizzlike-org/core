...
instance_mechanar::instance_mechanar(Map *pMap)
    : ScriptedInstance(pMap), m_uiBridgeEventTimer(2000), m_uiBridgeEventPhase(0), m_uiPathaleonEngageTimer(0) {
  Initialize();
}

void instance_mechanar::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_mechanar::OnPlayerEnter(Player *pPlayer) {
  ...
  pPlayer->SummonCreature(aBridgeEventLocs[6][0].m_uiSpawnEntry, aBridgeEventLocs[6][0].m_fX,
                          aBridgeEventLocs[6][0].m_fY, aBridgeEventLocs[6][0].m_fZ, aBridgeEventLocs[6][0].m_fO,
...
void instance_mechanar::OnCreatureDeath(Creature *pCreature) {
  switch (pCreature->GetEntry()) {
  case NPC_GYRO_KILL:
    SetData(TYPE_GYRO_KILL, DONE);
    break;
  case NPC_IRON_HAND:
    SetData(TYPE_IRON_HAND, DONE);
    break;
  case NPC_LORD_CAPACITUS:
    SetData(TYPE_CAPACITUS, DONE);
    break;
...
void instance_mechanar::DoSpawnBridgeWave() {
  if (Player *pPlayer = GetPlayerInMap(true, false)) {
    for (uint8 i = 0; i < MAX_BRIDGE_TRASH; ++i) {
      ...
      if (Creature *pTemp = pPlayer->SummonCreature(
              aBridgeEventLocs[m_uiBridgeEventPhase][i].m_uiSpawnEntry, aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fX,
              aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fY, aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fZ,
              aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
    ...
    if (m_uiBridgeEventTimer <= uiDiff) {
      ...
      for (const auto &data : instance->GetPlayers()) {
        if (m_uiBridgeEventPhase == 0) {
          if (data.getSource()->GetPositionZ() > 26.3f && data.getSource()->GetPositionY() < -12.f &&
              data.getSource()->GetPositionX() < 270.f) {
            DoSpawnBridgeWave();
          }
        } else if (m_uiBridgeEventPhase == 3) {
          if (data.getSource()->GetPositionZ() > 24.5f && data.getSource()->GetPositionY() > 2.5f &&
              data.getSource()->GetPositionX() < 155.f) {
            DoSpawnBridgeWave();
          }
        }
      }
...
InstanceData *GetInstanceData_instance_mechanar(Map *pMap) { return new instance_mechanar(pMap); }
