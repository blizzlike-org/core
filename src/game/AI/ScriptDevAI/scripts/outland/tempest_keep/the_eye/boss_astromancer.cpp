...
struct boss_high_astromancer_solarianAI : public CombatAI {
  boss_high_astromancer_solarianAI(Creature *creature)
      : CombatAI(creature, SOLARIAN_ACTION_MAX),
  ...
  void HandleSplitAgents() {
    for (uint8 i = 0; i < MAX_SPOTLIGHTS; ++i)
      if (Creature *spotlight = m_creature->GetMap()->GetCreature(m_vSpotLightsGuidVector[i]))
        spotlight->CastSpell(nullptr, SPELL_ASTROMANCER_ADDS, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
  ...
  void HandleSplitPriests() {
    ...
    for (uint8 i = 0; i < 2; ++i)
      if (Creature *spotlight = m_creature->GetMap()->GetCreature(m_vSpotLightsGuidVector[i]))
        spotlight->CastSpell(nullptr, SPELL_ASTROMANCER_PRIEST, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                             m_creature->GetObjectGuid());
    // Teleport the boss at the last portal
    if (Creature *spotlight = m_creature->GetMap()->GetCreature(m_vSpotLightsGuidVector[2]))
      m_creature->NearTeleportTo(spotlight->GetPositionX(), spotlight->GetPositionY(), spotlight->GetPositionZ(),
                                 spotlight->GetOrientation(), true);
  ...
  void ExecuteAction(uint32 action) override {
...
#ifdef PRENERF_2_0_3
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_WOTA_MAIN_CAST, SELECT_FLAG_PLAYER))
        if (DoCastSpellIfCan(target, SPELL_WOTA_MAIN_CAST) == CAST_OK)
          ResetCombatAction(action, urand(25000, 30000));
#else
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_WRATH_OF_THE_ASTROMANCER,
                                                           SELECT_FLAG_PLAYER))
    ...
    case SOLARIAN_ARCANE_MISSILES: {
      ...
      m_creature->SelectAttackingTargets(units, ATTACKING_TARGET_ALL_SUITABLE, 0, SPELL_ARCANE_MISSILES,
                                         SELECT_FLAG_PLAYER);
      if (units.size() > 0) {
...
#ifdef PRENERF_2_0_3
      // She casts this spell every 45 seconds. It is a kind of Moonfire spell, which she strikes down on the whole raid
      // simultaneously. It hits everyone in the raid for 2280 to 2520 arcane damage.
      if (DoCastSpellIfCan(nullptr, SPELL_BLINDING_LIGHT) == CAST_OK &&
          DoCastSpellIfCan(nullptr, SPELL_MARK_OF_SOLARIAN) == CAST_OK)
#else
      if (DoCastSpellIfCan(nullptr, SPELL_BLINDING_LIGHT) == CAST_OK)
#endif
  ...
  void OnApply(Aura *aura, bool apply) const override {
    if (!apply)
      if (aura->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
        aura->GetTarget()->CastSpell(
            aura->GetTarget(), aura->GetSpellProto()->CalculateSimpleValue(SpellEffectIndex(aura->GetEffIndex() + 1)),
