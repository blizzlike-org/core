...
enum {
  ...
  SPELL_FLOATING_DROWNED =
      36550, // guesswork based on video confirmation, now on retail one spell handles all - changed visual
...
static const uint32 m_auiSpellGravityLapseTeleport[] = {35966, 35967, 35968, 35969, 35970, 35971, 35972, 35973, 35974,
                                                        35975, 35976, 35977, 35978, 35979, 35980, 35981, 35982, 35983,
                                                        35984, 35985, 35986, 35987, 35988, 35989, 35990};
...
//                                              11     12
static const uint32 triggerSpellIds[13] = {36201, 36201, 36201, 36290, 36290, 36290, 36290,
                                           36290, 36290, 36290, 36291, 36291, 36291};
...
struct boss_kaelthasAI : public ScriptedAI {
  ...
  void MoveInLineOfSight(Unit *who) override {
    if (m_uiPhase == PHASE_0_NOT_BEGUN && who->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
        ...
        m_creature->CanAttackOnSight(who)) {
  ...
  void JustSummoned(Creature *summoned) override {
    ...
    case NPC_FLAME_STRIKE_TRIGGER: {
      DoCastSpellIfCan(summoned, SPELL_FLAME_STRIKE_DUMMY,
                       CAST_FORCE_TARGET_SELF | CAST_TRIGGERED | CAST_AURA_NOT_PRESENT);
      break;
    }
    ...
    case POINT_ID_CENTER: {
      ...
      if (m_worldTriggersFirstStage.empty()) {
        ...
        if (triggers.size() < TRIGGERS_TOTAL)
          sLog.outErrorDb(
              "Kaelthas Sunstrider script is missing World Trigger Large NPCs, current count %lu, expected count %u",
              triggers.size(), TRIGGERS_TOTAL);
  ...
  void ExecuteActions() {
    ...
    if (!m_unit->CanReactInCombat() ||
        m_unit->hasUnitState(UNIT_STAT_PROPELLED | UNIT_STAT_RETREATING | UNIT_STAT_CHANNELING) ||
        m_unit->IsNonMeleeSpellCasted(false))
      return;
      ...
      if (m_phaseTransitionGrowthTimer) {
        if (m_phaseTransitionGrowthTimer <= uiDiff) {
          ...
          case 8: {
            ...
            switch (m_phaseTransitionGrowthStage) {
            case 5:
              spellId = SPELL_EXPLODE_4;
              break;
            case 6:
              spellId = SPELL_EXPLODE_2;
              break;
            case 7:
              spellId = SPELL_EXPLODE_3;
              break;
            case 8:
              spellId = SPELL_EXPLODE_1;
              break;
            }
          ...
          m_phaseTransitionGrowthTimer -= uiDiff;
      }
        ...
        if (m_phaseTransitionPureNetherBeamTimer <= uiDiff) {
          ...
          case 0:
            for (uint32 i = 0; i < TRIGGERS_GROUP_ONE; ++i)
              if (urand(0, 1)) // 50% chance per trigger
                if (Creature *trigger =
                        m_creature->GetMap()->GetCreature(m_worldTriggersSecondStage[groupOneIndices[i]]))
          ...
          case 1:
            for (uint32 i = 0; i < TRIGGERS_GROUP_TWO; ++i)
              if (urand(0, 1)) // 50% chance per trigger
                if (Creature *trigger =
                        m_creature->GetMap()->GetCreature(m_worldTriggersSecondStage[groupTwoIndices[i]]))
          ...
          case 2:
            for (uint32 i = 0; i < TRIGGERS_GROUP_THREE; ++i)
              if (urand(0, 1)) // 50% chance per trigger
                if (Creature *trigger =
                        m_creature->GetMap()->GetCreature(m_worldTriggersSecondStage[groupThreeIndices[i]]))
          ...
          if (m_phaseTransitionPureNetherBeamStage < 30) // 30 pure nether beam cycles on retail
            m_phaseTransitionPureNetherBeamTimer =
                std::max(0, 400 - int32(uiDiff)); // need to roll over timer to achieve blizzlike behaviour
          else
            m_phaseTransitionPureNetherBeamTimer = 0;
        } else
          m_phaseTransitionPureNetherBeamTimer -= uiDiff;
      }
      break;
    }
    }
...
bool EffectDummyCreature_kael_phase_2(Unit *caster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                      Creature *creatureTarget, ObjectGuid /*originalCasterGuid*/) {
...
struct advisor_base_ai : public ScriptedAI {
  advisor_base_ai(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    Reset();
  }
...
struct boss_thaladred_the_darkenerAI : public advisor_base_ai {
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(SAY_THALADRED_AGGRO, m_creature); }

  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_THALADRED_DEATH, m_creature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiGazeTimer <= uiDiff) {
      ...
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
                                                           SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_RANGE, parameters)) {
        DoResetThreat();
...
struct boss_lord_sanguinarAI : public advisor_base_ai {
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(SAY_SANGUINAR_AGGRO, m_creature); }

  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_SANGUINAR_DEATH, m_creature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_fearActive) {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, nullptr,
                                                           SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_AOE_RANGE,
                                                           m_paramsBellowingRoar)) {
...
struct boss_grand_astromancer_capernianAI : public advisor_base_ai {
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(SAY_CAPERNIAN_AGGRO, m_creature); }

  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_CAPERNIAN_DEATH, m_creature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiConflagrationTimer <= uiDiff) {
      ...
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CONFLAGRATION, SELECT_FLAG_PLAYER)) {
...
struct boss_master_engineer_telonicusAI : public advisor_base_ai {
  boss_master_engineer_telonicusAI(Creature *creature) : advisor_base_ai(creature) {
    m_creature->SetModifierValue(
        UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT,
        1.f); // sniff evidence seems to point out that telonicus isnt subject to 50% offhand dmg modifier
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(SAY_TELONICUS_AGGRO, m_creature); }

  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_TELONICUS_DEATH, m_creature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiRemoteToyTimer <= uiDiff) {
      ...
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_REMOTE_TOY,
...
struct npc_nether_vaporAI : public ScriptedAI, public TimerManager {
  ...
  void UpdateAI(const uint32 diff) { UpdateTimers(diff); }
};
...
struct NetherVaporSummon : public SpellScript {
  void OnDestTarget(Spell *spell) const override { spell->m_targets.m_destPos.z += 5.f; }
};
