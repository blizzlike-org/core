...
struct boss_void_reaverAI : public CombatAI {
  boss_void_reaverAI(Creature *creature)
      : CombatAI(creature, VOID_REAVER_ACTION_MAX),
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit * /*unit*/, float /*x*/, float /*y*/, float /*z*/) -> bool {
      ...
      for (auto itr : threatList) {
        if (Unit *target = m_creature->GetMap()->GetUnit(itr->getUnitGuid())) {
          if (target->GetTypeId() == TYPEID_PLAYER &&
              !target->IsWithinDist3d(m_creature->GetPositionX(), m_creature->GetPositionY(),
                                      m_creature->GetPositionZ(), 18.0f))
