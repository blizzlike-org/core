...
SDName: boss_alar
SD%Complete: 99
SDComment: TODO: Make Alar Root and Hover during P1 platforms and dynamically enable flying and hover and use spell
31514 for p2 transition SDCategory: Tempest Keep, The Eye EndScriptData */

/* Pre-nerf Changes
Movement: Has been changed to have a random chance to either DoFlameQuills() or to move to a random platform. There is
even a small chance he will stay at the same platform for 30 seconds. Embers: Do not do % hp damage to Al'ar when
killed.

Patches
2.1.0 - Al'ar movement was updated to the current state (platform 1, 2, 3, 4), and embers were given the %hp burn
element.
*/
...
enum {
  ...
  SPELL_ASHTONGUE_RUSE = 42090, // Player can complete 10946 quest, only if has aura 42090. If kill Alar without this
                                // aura - quest not completed.
};
...
static const EventLocation aPlatformLocation[MAX_PLATFORMS] = {
    {335.5587f, 58.56138f, 18.30103f},  {387.8628f, 32.18992f, 20.23814f}, {388.777f, -32.10653f, 20.26319f},
    {333.6099f, -60.69928f, 18.01521f},
#ifdef PRENERF_2_0_3
    {264.36f, 40.78f, 20.21f},          {268.38f, -49.80f, 20.38f}
#endif
};

static const EventLocation aCenterLocation[] = {
    {330.3943f, -2.02203f, 42.4762f}, {333.589f, -0.768249f, -2.38949f}, {329.9976f, -0.2719078f, 42.42101f}};
...
struct boss_alarAI : public CombatAI {
  boss_alarAI(Creature *creature)
      : CombatAI(creature, ALAR_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
  ...
  void Aggro(Unit * /*who*/) override {
    ...
    // The boss will always move to the first platform from the left side; also set the movement to idle to stop the DB
    // movement
    m_creature->GetMotionMaster()->MoveIdle();
    m_creature->GetMotionMaster()->MovePoint(POINT_ID_PLATFORM, aPlatformLocation[m_uiCurrentPlatformId].m_fX,
                                             aPlatformLocation[m_uiCurrentPlatformId].m_fY,
                                             aPlatformLocation[m_uiCurrentPlatformId].m_fZ);
  }
    ...
    if (m_instance) {
      ...
      for (const auto &playerRef : players)
        if (Player *player = playerRef.getSource())
          if (player->GetQuestStatus(QUEST_RUSE_ASHTONGUE) == QUEST_STATUS_INCOMPLETE &&
  ...
  void DoFlameQuills() {
    ...
    m_creature->GetMotionMaster()->MovePoint(POINT_ID_QUILLS, aCenterLocation[0].m_fX, aCenterLocation[0].m_fY,
                                             aCenterLocation[0].m_fZ);
    SetCombatScriptStatus(true);
  ...
  void DoPlatformMove(bool summon) {
    ...
    m_creature->GetMotionMaster()->MovePoint(POINT_ID_PLATFORM, aPlatformLocation[m_uiFuturePlatformId].m_fX,
                                             aPlatformLocation[m_uiFuturePlatformId].m_fY,
                                             aPlatformLocation[m_uiFuturePlatformId].m_fZ);
  ...
  void StartPhaseTransition() {
    ...
    // We set the health to 1 in order to avoid the forced death stand flag - this way we can have the ressurrect
    // animation
    ...
    if (DoCastSpellIfCan(nullptr, SPELL_EMBER_BLAST) == CAST_OK) {
      ...
      m_creature->GetMotionMaster()->MovePoint(POINT_ID_RESSURRECT, aCenterLocation[1].m_fX, aCenterLocation[1].m_fY,
                                               aCenterLocation[1].m_fZ, FORCED_MOVEMENT_RUN);
    }
  }
  ...
  void HandleDiveBomb() {
    ...
    case 0: {
      ...
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_PLAYER)) {
        ...
        m_creature->NearTeleportTo(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(),
  ...
  void ExecuteAction(uint32 action) {
    ...
    case ALAR_DIVE_BOMB: {
      ...
      m_creature->SetHover(true);
      m_creature->SetLevitate(true);
      m_creature->GetMotionMaster()->MovePoint(POINT_ID_DIVE_BOMB, aCenterLocation[2].m_fX, aCenterLocation[2].m_fY,
                                               aCenterLocation[2].m_fZ);
      SetDeathPrevention(true);
