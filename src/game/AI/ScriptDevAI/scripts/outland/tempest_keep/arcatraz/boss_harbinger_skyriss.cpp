...
struct boss_harbinger_skyrissAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_creature->GetHealthPercent() < 100 - 33 * m_uiSplitPhase && m_creature->GetHealthPercent() > 5.0f) {
      DoCastSpellIfCan(m_creature, m_uiSplitPhase == 1 ? SPELL_66_ILLUSION : SPELL_33_ILLUSION,
    ...
    if (m_uiDominationTimer < uiDiff) {
      if (Unit *pTarget =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, uint32(0), SELECT_FLAG_PLAYER)) {
    ...
    if (!m_bIsRegularMode) {
      if (m_uiManaBurnTimer < uiDiff) {
        Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, nullptr,
...
UnitAI *GetAI_boss_harbinger_skyriss(Creature *pCreature) { return new boss_harbinger_skyrissAI(pCreature); }
