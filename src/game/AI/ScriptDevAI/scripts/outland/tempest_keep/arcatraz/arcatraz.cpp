...
instance_arcatraz::instance_arcatraz(Map *pMap)
    : ScriptedInstance(pMap), DialogueHelper(aArcatrazDialogue), m_uiResetDelayTimer(0), m_uiEntranceEventTimer(0),
      m_uiKilledWarders(0), m_uiKilledDefenders(0) {
  Initialize();
}
...
void instance_arcatraz::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_HARBINGERSKYRISS:
    if (uiData == FAIL) {
      ...
      for (GuidList::const_iterator itr = m_lSkyrissEventMobsGuidList.begin(); itr != m_lSkyrissEventMobsGuidList.end();
           ++itr) {
...
void instance_arcatraz::JustDidDialogueStep(int32 iEntry) {
  ...
  case YELL_MELLICHAR_RELEASE1:
    pMellichar->SummonCreature(urand(0, 1) ? NPC_BLAZING_TRICKSTER : NPC_PHASE_HUNTER, aSummonPosition[0].m_fX,
                               aSummonPosition[0].m_fY, aSummonPosition[0].m_fZ, aSummonPosition[0].m_fO,
  ...
  case SPELL_TARGET_BETA:
    ...
    break;
  case TYPE_WARDEN_2:
    pMellichar->SummonCreature(NPC_MILLHOUSE, aSummonPosition[1].m_fX, aSummonPosition[1].m_fY, aSummonPosition[1].m_fZ,
                               aSummonPosition[1].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
    break;
  case SPELL_TARGET_DELTA:
    ...
    break;
  case TYPE_WARDEN_3:
    pMellichar->SummonCreature(urand(0, 1) ? NPC_AKKIRIS : NPC_SULFURON, aSummonPosition[2].m_fX,
                               aSummonPosition[2].m_fY, aSummonPosition[2].m_fZ, aSummonPosition[2].m_fO,
                               TEMPSPAWN_DEAD_DESPAWN, 0);
  ...
  case SPELL_TARGET_GAMMA:
    ...
    break;
  case TYPE_WARDEN_4:
    pMellichar->SummonCreature(urand(0, 1) ? NPC_TW_DRAKONAAR : NPC_BL_DRAKONAAR, aSummonPosition[3].m_fX,
                               aSummonPosition[3].m_fY, aSummonPosition[3].m_fZ, aSummonPosition[3].m_fO,
                               TEMPSPAWN_DEAD_DESPAWN, 0);
    pMellichar->CastSpell(pMellichar, SPELL_TARGET_OMEGA, TRIGGERED_NONE);
    if (Creature *pTarget = GetSingleCreatureFromStorage(NPC_PRISON_BOSS_POD))
      pMellichar->SetFacingToObject(pTarget);
  ...
  case YELL_MELLICHAR_RELEASE5:
    ...
    break;
  case TYPE_WARDEN_5:
    if (Creature *pSkyriss =
            pMellichar->SummonCreature(NPC_SKYRISS, aSummonPosition[4].m_fX, aSummonPosition[4].m_fY,
                                       aSummonPosition[4].m_fZ, aSummonPosition[4].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
...
void instance_arcatraz::Update(uint32 uiDiff) {
  ...
  if (m_uiEntranceEventTimer) {
    if (m_uiEntranceEventTimer <= uiDiff) {
      ...
      if ((uiEntry == NPC_PROTEAN_HORROR && m_uiKilledWarders == MAX_WARDERS) ||
          (uiEntry == NPC_PROTEAN_NIGHTMARE && m_uiKilledDefenders == MAX_DEFENDERS)) {
      ...
      if (Creature *pTemp =
              pPlayer->SummonCreature(uiEntry, aEntranceSpawnLoc[0], aEntranceSpawnLoc[1], aEntranceSpawnLoc[2],
                                      aEntranceSpawnLoc[3], TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 30000)) {
...
InstanceData *GetInstanceData_instance_arcatraz(Map *pMap) { return new instance_arcatraz(pMap); }
