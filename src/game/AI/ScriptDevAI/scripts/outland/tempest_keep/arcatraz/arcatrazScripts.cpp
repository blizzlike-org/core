...
struct npc_millhouse_manastormAI : public ScriptedAI, private DialogueHelper {
  npc_millhouse_manastormAI(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(aIntroDialogue) {
...
UnitAI *GetAI_npc_millhouse_manastorm(Creature *pCreature) { return new npc_millhouse_manastormAI(pCreature); }
...
struct npc_warden_mellicharAI : public ScriptedAI {
  ...
  void Aggro(Unit *pWho) override {
    ...
    // In theory the Seal Sphere should protect the npc from being attacked, but because LoS isn't enabled for
    // Gameobjects we have to use this workaround
...
UnitAI *GetAI_npc_warden_mellichar(Creature *pCreature) { return new npc_warden_mellicharAI(pCreature); }
...
struct npc_arcatraz_defenderAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiFlamingWeaponTimer < uiDiff) {
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_bIsRegularMode ? SPELL_FLAMING_WEAPON : SPELL_FLAMING_WEAPON_H) ==
          CAST_OK)
        m_uiFlamingWeaponTimer = urand(3000, 6000);
    } else
    ...
    if (m_creature->GetVictim() && m_creature->GetVictim()->GetTypeId() != TYPEID_PLAYER) {
      if (m_uiProteanSubdualTimer < uiDiff) {
        if (DoCastSpellIfCan(m_creature->GetVictim(),
                             m_bIsRegularMode ? SPELL_PROTEAN_SUBDUAL : SPELL_PROTEAN_SUBDUAL_H) == CAST_OK)
...
UnitAI *GetAI_npc_arcatraz_defender(Creature *pCreature) { return new npc_arcatraz_defenderAI(pCreature); }
