...
struct boss_dalliahAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiGiftDoomsayerTimer < uiDiff) {
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_bIsRegularMode ? SPELL_GIFT_DOOMSAYER : SPELL_GIFT_DOOMSAYER_H) ==
          CAST_OK)
    ...
    if (!m_bIsRegularMode) {
      if (m_uiShadowWaveTimer < uiDiff) {
        if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_FARTHEST_AWAY, 0, SPELL_SHADOW_WAVE,
                                                              SELECT_FLAG_PLAYER)) {
...
UnitAI *GetAI_boss_dalliah(Creature *pCreature) { return new boss_dalliahAI(pCreature); }
