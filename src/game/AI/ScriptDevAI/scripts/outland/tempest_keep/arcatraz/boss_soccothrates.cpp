...
enum {
  ...
  SPELL_CHARGE =
      35754, // script target on 21030; also dummy effect area effect target on 20978 - makes the target cast 35769
...
static const DialogueEntry aIntroDialogue[] = {
    {SAY_SOCCOTHRATES_INTRO_1, NPC_SOCCOTHRATES, 3000}, {SAY_DALLIAH_INTRO_2, NPC_DALLIAH, 2000},
    {SAY_SOCCOTHRATES_INTRO_3, NPC_SOCCOTHRATES, 4000}, {SAY_DALLIAH_INTRO_4, NPC_DALLIAH, 5000},
    {SAY_SOCCOTHRATES_INTRO_5, NPC_SOCCOTHRATES, 3000}, {SAY_DALLIAH_INTRO_6, NPC_DALLIAH, 3000},
    {SAY_SOCCOTHRATES_INTRO_7, NPC_SOCCOTHRATES, 0},    {0, 0, 0},
};

struct boss_soccothratesAI : public ScriptedAI, private DialogueHelper {
  boss_soccothratesAI(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(aIntroDialogue) {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    if (!m_bHasYelledIntro && pWho->GetTypeId() == TYPEID_PLAYER && m_creature->IsWithinDistInMap(pWho, 75.0f) &&
  ...
  void EnterEvadeMode() override {
    ...
    if (m_creature->IsAlive())
      m_creature->GetMotionMaster()->MovePoint(1, aSoccotharesStartPos[0], aSoccotharesStartPos[1],
                                               aSoccotharesStartPos[2]);
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiFelfireShockTimer < uiDiff) {
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_bIsRegularMode ? SPELL_FELFIRE_SHOCK : SPELL_FELFIRE_SHOCK_H) ==
          CAST_OK)
