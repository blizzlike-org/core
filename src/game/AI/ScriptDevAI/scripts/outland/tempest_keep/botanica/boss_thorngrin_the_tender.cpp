  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_sacrificeTimer < diff) {
      ...
      if (!m_creature->IsNonMeleeSpellCasted(false))
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_SACRIFICE,
                                                             SELECT_FLAG_PLAYER | SELECT_FLAG_IN_LOS))
...
UnitAI *GetAI_boss_thorngrin(Creature *pCreature) { return new boss_thorngrinAI(pCreature); }
