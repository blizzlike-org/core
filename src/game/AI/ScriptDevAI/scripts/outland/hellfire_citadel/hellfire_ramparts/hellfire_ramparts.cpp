...
instance_ramparts::instance_ramparts(Map *pMap) : ScriptedInstance(pMap), m_uiSentryCounter(0) { Initialize(); }

void instance_ramparts::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_ramparts::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_NAZAN:
    ...
    if (uiData == DONE && m_auiEncounter[0] == DONE) {
      DoRespawnGameObject(instance->IsRegularDifficulty() ? GO_FEL_IRON_CHEST : GO_FEL_IRON_CHEST_H, HOUR);
      DoToggleGameObjectFlags(instance->IsRegularDifficulty() ? GO_FEL_IRON_CHEST : GO_FEL_IRON_CHEST_H,
                              GO_FLAG_NO_INTERACT, false);
    }
...
InstanceData *GetInstanceData_instance_ramparts(Map *pMap) { return new instance_ramparts(pMap); }
