...
struct boss_watchkeeper_gargolmarAI : public CombatAI {
  boss_watchkeeper_gargolmarAI(Creature *creature)
      : CombatAI(creature, GARGOLMAR_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) {
    ...
    case GARGOLMAR_ACTION_YELL_FOR_HEAL:
      ...
      break;
    case GARGOLMAR_ACTION_MORTAL_WOUND:
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_MORTAL_WOUND : SPELL_MORTAL_WOUND_H) ==
          CAST_OK) {
    ...
    case GARGOLMAR_ACTION_SURGE:
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_FARTHEST_AWAY, 0, SPELL_SURGE, SELECT_FLAG_PLAYER)) {
