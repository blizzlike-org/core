...
struct boss_vazruden_heraldAI : public CombatAI {
  boss_vazruden_heraldAI(Creature *creature)
      : CombatAI(creature, NAZAN_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
    ...
    m_creature->GetCombatManager().SetLeashingCheck([&](Unit *, float x, float, float) { return x >= -1336.0f; });
  ...
  void MovementInform(uint32 motionType, uint32 pointId) override {
    ...
    if (motionType == POINT_MOTION_TYPE) {
      ...
      case POINT_ID_LANDING: {
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_LANDED, landingPhasePos.x, landingPhasePos.y,
                                                 landingPhasePos.z);
        break;
      }
  ...
  void DoSplit() {
    ...
    m_lastFlightPoint = 0;
    m_creature->GetMotionMaster()->MovePoint(POINT_ID_FLYING, flyingPositions[0].x, flyingPositions[0].y,
                                             flyingPositions[0].z);
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case NAZAN_FLIGHT_MOVE: {
      ...
      m_creature->SetImmobilizedState(false);
      m_creature->GetMotionMaster()->MovePoint(POINT_ID_FLIGHT_MOVE, flyingPositions[m_lastFlightPoint].x,
                                               flyingPositions[m_lastFlightPoint].y,
                                               flyingPositions[m_lastFlightPoint].z);
      SetCombatScriptStatus(true);
    ...
    case NAZAN_CONE_OF_FIRE: {
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_inRegularMode ? SPELL_CONE_OF_FIRE : SPELL_CONE_OF_FIRE_H) ==
          CAST_OK)
        ResetCombatAction(action, urand(7300, 13200));
      break;
    }
...
struct boss_vazrudenAI : public CombatAI {
  boss_vazrudenAI(Creature *creature)
      : CombatAI(creature, VAZRUDEN_ACTION_MAX),
