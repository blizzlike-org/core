...
struct boss_omor_the_unscarredAI : public RangedCombatAI {
  boss_omor_the_unscarredAI(Creature *creature)
      : RangedCombatAI(creature, OMOR_ACTION_MAX), m_inRegularMode(creature->GetMap()->IsRegularDifficulty()) {
  ...
  void JustDied(Unit * /*killer*/) override { DoScriptText(SAY_DIE, m_creature); }
  ...
  void HandlePullBack() {
    if (Player *player = m_creature->GetMap()->GetPlayer(m_playerGuid)) {
      // if unit dosen't have this flag, then no pulling back (script will attempt cast, even if orbital strike was
      // resisted)
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case OMOR_TREACHERY: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_TREACHEROUS_AURA,
                                                           SELECT_FLAG_PLAYER)) {
    ...
    case OMOR_SHADOW_BOLT: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, SPELL_SHADOW_BOLT, SELECT_FLAG_PLAYER)) {
