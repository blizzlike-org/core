...
instance_shattered_halls::instance_shattered_halls(Map *pMap)
    : ScriptedInstance(pMap), m_uiExecutionTimer(55 * MINUTE * IN_MILLISECONDS), m_uiTeam(0), m_uiExecutionStage(0),
      m_uiPrisonersLeft(3) {
  Initialize();
}

void instance_shattered_halls::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_shattered_halls::OnPlayerEnter(Player *pPlayer) {
  ...
  if (m_uiTeam == ALLIANCE)
    pPlayer->SummonCreature(aSoldiersLocs[1].m_uiAllianceEntry, aSoldiersLocs[1].m_fX, aSoldiersLocs[1].m_fY,
                            aSoldiersLocs[1].m_fZ, aSoldiersLocs[1].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
  else
    pPlayer->SummonCreature(aSoldiersLocs[0].m_uiHordeEntry, aSoldiersLocs[0].m_fX, aSoldiersLocs[0].m_fY,
                            aSoldiersLocs[0].m_fZ, aSoldiersLocs[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
}
  ...
  case TYPE_EXECUTION:
    ...
    if (uiData == IN_PROGRESS && !GetSingleCreatureFromStorage(NPC_EXECUTIONER, true)) {
      if (Player *pPlayer = GetPlayerInMap()) {
        ...
        for (uint8 i = 2; i < 5; ++i)
          pPlayer->SummonCreature(m_uiTeam == ALLIANCE ? aSoldiersLocs[i].m_uiAllianceEntry
                                                       : aSoldiersLocs[i].m_uiHordeEntry,
                                  aSoldiersLocs[i].m_fX, aSoldiersLocs[i].m_fY, aSoldiersLocs[i].m_fZ,
                                  aSoldiersLocs[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);

        // Summon the executioner; Note: according to wowhead he shouldn't be targetable until Kargath encounter is
        // finished
        if (Creature *pExecutioner =
                pPlayer->SummonCreature(NPC_EXECUTIONER, afExecutionerLoc[0], afExecutionerLoc[1], afExecutionerLoc[2],
                                        afExecutionerLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0, true))
    ...
    if (uiData == DONE) {
      ...
      if (Creature *pOfficer =
              GetSingleCreatureFromStorage(m_uiTeam == ALLIANCE ? NPC_OFFICER_ALLIANCE : NPC_OFFICER_HORDE)) {
...
void instance_shattered_halls::OnCreatureEnterCombat(Creature *creature) {
  ...
  // This is according to the blizz comments which say that it is possible to complete the event if you engage the npc
  // while you have only a few seconds left
  if (creature->GetEntry() == NPC_EXECUTIONER)
...
bool instance_shattered_halls::CheckConditionCriteriaMeet(Player const *pPlayer, uint32 uiInstanceConditionId,
                                                          WorldObject const *pConditionSource,
                                                          uint32 conditionSourceType) const {
  ...
  script_error_log("instance_shattered_halls::CheckConditionCriteriaMeet called with unsupported Id %u. Called with "
                   "param plr %s, src %s, condition source type %u",
                   uiInstanceConditionId, pPlayer ? pPlayer->GetGuidStr().c_str() : "nullptr",
                   pConditionSource ? pConditionSource->GetGuidStr().c_str() : "nullptr", conditionSourceType);
  return false;
}
...
void instance_shattered_halls::Update(uint32 uiDiff) {
      ...
      for (auto &blaze : m_vBlazeTimers) {
        if (blaze.second <= uiDiff) {
          ...
          if (GameObject *blazeGo = instance->GetGameObject(blaze.first))
            blazeGo->CastSpell(gauntlet, nullptr, SPELL_FLAMES, TRIGGERED_NONE, nullptr, nullptr,
                               blazeGo->GetObjectGuid());
        } else
          blaze.second -= uiDiff;
      }
    }
  ...
  if (m_uiExecutionTimer < uiDiff) {
    ...
    case 0:
      // Kill the officer
      if (Creature *pSoldier =
              GetSingleCreatureFromStorage(m_uiTeam == ALLIANCE ? NPC_OFFICER_ALLIANCE : NPC_OFFICER_HORDE))
        pSoldier->Suicide();

      // Make Kargath yell
      DoOrSimulateScriptTextForThisInstance(m_uiTeam == ALLIANCE ? SAY_KARGATH_EXECUTE_ALLY : SAY_KARGATH_EXECUTE_HORDE,
    ...
    case 1:
      if (Creature *pSoldier =
              GetSingleCreatureFromStorage(m_uiTeam == ALLIANCE ? NPC_SOLDIER_ALLIANCE_2 : NPC_SOLDIER_HORDE_2))
        pSoldier->Suicide();
    ...
    case 2:
      if (Creature *pSoldier =
              GetSingleCreatureFromStorage(m_uiTeam == ALLIANCE ? NPC_SOLDIER_ALLIANCE_3 : NPC_SOLDIER_HORDE_3))
        pSoldier->Suicide();
...
InstanceData *GetInstanceData_instance_shattered_halls(Map *pMap) { return new instance_shattered_halls(pMap); }
...
static float gauntletSpawnCoords[1][3] = {{409.848f, 315.385f, 1.921f}};

static float scoutCoords[1][3] = {{494.015f, 316.213f, 1.945f}};
...
static float firstWaveWaypoints[2][3] = {{495.646f, 313.251f, 1.945f}, {497.516f, 319.176f, 1.945f}};
...
struct npc_Gauntlet_of_Fire : public ScriptedAI {
  ...
  void DoInitialGets() {
    m_porung = GetClosestCreatureWithEntry(
        m_creature, m_creature->GetMap()->IsRegularDifficulty() ? NPC_SHATTERED_HAND_BG : NPC_GUARD_PORUNG, 150.0f);
  }

  void DoSummonInitialWave() {
    if (Creature *pAdd = m_creature->SummonCreature(NPC_SHATTERED_HAND_ZEALOT, zealotSpawnCoords[1][0],
                                                    zealotSpawnCoords[1][1], zealotSpawnCoords[1][2], 0.0f,
                                                    TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 150000, true, true)) {
      pAdd->GetMotionMaster()->MovePoint(100 + m_uiNumInitialWaves, firstWaveWaypoints[0][0], firstWaveWaypoints[0][1],
                                         firstWaveWaypoints[0][2]);
    }
    if (Creature *pAdd = m_creature->SummonCreature(NPC_SHATTERED_HAND_ZEALOT, zealotSpawnCoords[2][0],
                                                    zealotSpawnCoords[2][1], zealotSpawnCoords[2][2], 0.0f,
                                                    TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 150000, true, true)) {
      pAdd->GetMotionMaster()->MovePoint(200 + m_uiNumInitialWaves, firstWaveWaypoints[1][0], firstWaveWaypoints[1][1],
                                         firstWaveWaypoints[1][2]);
    }
  }

  void DoSummonSHZealot() {
    if (Creature *pAdd = m_creature->SummonCreature(NPC_SHATTERED_HAND_ZEALOT, zealotSpawnCoords[0][0],
                                                    zealotSpawnCoords[0][1], zealotSpawnCoords[0][2], 0.0f,
                                                    TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 150000, true, true)) {
  ...
  void SummonedMovementInform(Creature *pSummoned, uint32 uiMotionType, uint32 uiData) override {
    if (pSummoned->GetEntry() == NPC_SHATTERED_HAND_ZEALOT && uiMotionType == POINT_MOTION_TYPE) // sanity check
    {
      ...
      // Left
      case 100:
        pSummoned->GetMotionMaster()->MovePoint(98, zealotDestinations[0][0], zealotDestinations[0][1],
                                                zealotDestinations[0][2]);
        break;
      case 101:
        pSummoned->GetMotionMaster()->MovePoint(98, zealotDestinations[2][0], zealotDestinations[2][1],
                                                zealotDestinations[2][2]);
        break;
      case 102:
        pSummoned->GetMotionMaster()->MovePoint(98, zealotDestinations[4][0], zealotDestinations[4][1],
                                                zealotDestinations[4][2]);
        break;
      case 103:
        pSummoned->GetMotionMaster()->MovePoint(98, zealotDestinations[6][0], zealotDestinations[6][1],
                                                zealotDestinations[6][2]);
        break;
      // Right
      case 200:
        pSummoned->GetMotionMaster()->MovePoint(99, zealotDestinations[1][0], zealotDestinations[1][1],
                                                zealotDestinations[1][2]);
        break;
      case 201:
        pSummoned->GetMotionMaster()->MovePoint(99, zealotDestinations[3][0], zealotDestinations[3][1],
                                                zealotDestinations[3][2]);
        break;
      case 202:
        pSummoned->GetMotionMaster()->MovePoint(99, zealotDestinations[5][0], zealotDestinations[5][1],
                                                zealotDestinations[5][2]);
        break;
      case 203:
        pSummoned->GetMotionMaster()->MovePoint(99, zealotDestinations[7][0], zealotDestinations[7][1],
                                                zealotDestinations[7][2]);
        break;
...
struct npc_Shattered_Hand_Scout : public ScriptedAI {
  ...
  void Reset() override { m_bRunning = false; }
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    if (pWho->GetTypeId() == TYPEID_PLAYER && !static_cast<Player *>(pWho)->IsGameMaster() &&
        pWho->GetDistance(m_creature) <= 50.f)
      if (!m_bRunning)
        DoStartRunning();
  }
  ...
  void MovementInform(uint32 uiMovementType, uint32 uiData) override {
    if (uiMovementType == POINT_MOTION_TYPE && m_creature->IsAlive()) {
      ...
      case 0:
        ...
        if (Creature *gauntlet = m_creature->SummonCreature(NPC_GAUNTLET_OF_FIRE, gauntletSpawnCoords[0][0],
                                                            gauntletSpawnCoords[0][1], gauntletSpawnCoords[0][2], 0.0f,
                                                            TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 600000)) {
