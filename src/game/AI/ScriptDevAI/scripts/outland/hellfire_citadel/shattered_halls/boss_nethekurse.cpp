...
struct boss_grand_warlock_nethekurseAI : public ScriptedAI {
  ...
  void DoYellForPeonAggro() {
    switch (urand(0, 3)) {
    case 0:
      DoScriptText(SAY_PEON_ATTACK_1, m_creature);
      break;
    case 1:
      DoScriptText(SAY_PEON_ATTACK_2, m_creature);
      break;
    case 2:
      DoScriptText(SAY_PEON_ATTACK_3, m_creature);
      break;
    case 3:
      DoScriptText(SAY_PEON_ATTACK_4, m_creature);
      break;
    }
  }
  ...
  void DoYellForPeonDeath(Unit *pKiller) {
    ...
    switch (urand(0, 2)) {
    case 0:
      DoScriptText(SAY_PEON_DIE_1, m_creature);
      break;
    case 1:
      DoScriptText(SAY_PEON_DIE_2, m_creature);
      break;
    case 2:
      DoScriptText(SAY_PEON_DIE_3, m_creature);
      break;
    }
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    if (!m_bIntroOnce && pWho->GetTypeId() == TYPEID_PLAYER && !((Player *)pWho)->IsGameMaster() &&
        m_creature->IsWithinDistInMap(pWho, 45.0f) && m_creature->IsWithinLOSInMap(pWho)) {
      m_bIntroOnce = true;
...
struct mob_fel_orc_convertAI : public ScriptedAI {
  ...
  void Reset() override { m_uiHemorrhageTimer = 3000; }

  void MoveInLineOfSight(Unit *pWho) override {
    if (pWho->GetTypeId() == TYPEID_PLAYER && !((Player *)pWho)->IsGameMaster() &&
        m_creature->IsWithinDistInMap(pWho, 20.0f) && m_creature->IsWithinLOSInMap(pWho))
...
UnitAI *GetAI_mob_fel_orc_convert(Creature *pCreature) { return new mob_fel_orc_convertAI(pCreature); }
