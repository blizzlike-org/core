...
struct boss_kelidan_the_breakerAI : public CombatAI {
  boss_kelidan_the_breakerAI(Creature *creature)
      : CombatAI(creature, KELIDAN_ACTION_MAX),
        m_instance(static_cast<instance_blood_furnace *>(creature->GetInstanceData())),
        m_isRegularMode(creature->GetMap()->IsRegularDifficulty()), m_bDidMagtheridonYell(false) {
    m_creature->GetCombatManager().SetLeashingCheck([](Unit *, float, float y, float) { return y < -158.23f; });
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(SAY_WAKE, m_creature); }
  ...
  void DoSetupAdds() {
    ...
    if (!lAddGuids.empty()) {
      ...
      // Sort them by angle
      lAdds.sort([=](Creature *left, Creature *right) -> bool {
        return m_creature->GetAngle(left) < m_creature->GetAngle(right);
      });
  ...
  void AddJustReachedHome() { ResetTimer(KELIDAN_SETUP_ADDS, 2000); }
...
struct mob_shadowmoon_channelerAI : public CombatAI {
  mob_shadowmoon_channelerAI(Creature *creature)
