...
struct boss_the_makerAI : public CombatAI {
  boss_the_makerAI(Creature *creature)
      : CombatAI(creature, MAKER_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case MAKER_EXPLODING_BEAKER:
      if (DoCastSpellIfCan(m_creature->GetVictim(),
                           m_isRegularMode ? SPELL_EXPLODING_BREAKER : SPELL_EXPLODING_BREAKER_H) == CAST_OK)
