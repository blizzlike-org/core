...
SD%Complete: 80
SDComment: Phase 3 transition requires additional research. The Manticron cubes require additional core support. Timers
need to be revised. SDCategory: Hellfire Citadel, Magtheridon's lair EndScriptData */
...
enum {
  ...
  SPELL_QUAKE_REMOVAL =
      30572, // removes quake from all triggers if blastnova starts during
...
struct boss_magtheridonAI : public CombatAI {
  boss_magtheridonAI(Creature *creature)
      : CombatAI(creature, MAGTHERIDON_ACTION_MAX),
...
struct mob_hellfire_channelerAI : public CombatAI {
  mob_hellfire_channelerAI(Creature *creature)
      : CombatAI(creature, CHANNELER_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case CHANNELER_INFERNAL: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BURNING_ABYSSAL, SELECT_FLAG_PLAYER))
    ...
    case CHANNELER_FEAR: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_FEAR,
    ...
    if (apply) {
      if (target->GetAuraCount(30166) == 5) {
        ...
        target->InterruptSpell(
            CURRENT_CHANNELED_SPELL); // if he is casting blast nova interrupt channel, only magth channel spell
      }
    } else {
