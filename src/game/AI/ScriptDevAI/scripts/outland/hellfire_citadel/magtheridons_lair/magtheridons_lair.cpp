...
void instance_magtheridons_lair::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
struct MentalInterferenceSpellScript : public SpellScript {
  SpellCastResult OnCheckCast(Spell *spell, bool /*strict*/) const override {
    if (ObjectGuid target = spell->m_targets.getUnitTargetGuid())
      if (target.GetEntry() != 16943 &&
          target.GetEntry() != 20928) // Mental Interference can be cast only on these two targets
...
InstanceData *GetInstanceData_instance_magtheridons_lair(Map *pMap) { return new instance_magtheridons_lair(pMap); }
