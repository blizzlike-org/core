...
struct boss_yorAI : public ScriptedAI, public CombatActions {
  ...
  void ExecuteActions() override {
    ...
    for (uint32 i = 0; i < YOR_COMBAT_ACTION_MAX; ++i) {
      ...
      switch (i) {
      case YOR_ACTION_STOMP:
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_STOMP,
                                                             SELECT_FLAG_USE_EFFECT_RADIUS)) {
...
struct npc_ethereum_prisoner_dungeonAI : public ScriptedAI, public CombatActions {
  ...
  void UpdateAI(const uint32 diff) override { UpdateTimers(diff, m_creature->IsInCombat()); }
};
