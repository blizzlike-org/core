...
struct boss_nexusprince_shaffarAI : public RangedCombatAI {
  boss_nexusprince_shaffarAI(Creature *creature)
      : RangedCombatAI(creature, SHAFFAR_ACTION_MAX),
  ...
  void MoveInLineOfSight(Unit *who) override {
    if (!m_hasTaunted && who->GetTypeId() == TYPEID_PLAYER && m_creature->IsWithinDistInMap(who, 100.0f) &&
  ...
  void JustDied(Unit * /*killer*/) override { DoScriptText(SAY_DEAD, m_creature); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case SHAFFAR_ACTION_BEACON:
      ...
      break;
    case SHAFFAR_ACTION_FROSTNOVA:
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_FROSTNOVA,
    ...
    case SHAFFAR_ACTION_BLINK:
      if (m_creature->GetVictim() && m_creature->CanReachWithMeleeAttack(m_creature->GetVictim()) &&
          DoCastSpellIfCan(nullptr, SPELL_BLINK) == CAST_OK) {
        ResetCombatAction(action, GetSubsequentActionTimer(ShaffarActions(action)));
        return;
      }
      break;
