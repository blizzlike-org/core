...
SD%Complete: 95
SDComment: Most of event implemented, possibly make some better code for switching his dark side in to better "images"
of player. SDCategory: Auchindoun, Auchenai Crypts EndScriptData */
...
struct mob_stolen_soulAI : public ScriptedAI {
  ...
  void Reset() override { m_uiSpellTimer = 1000; }
...
UnitAI *GetAI_mob_stolen_soul(Creature *pCreature) { return new mob_stolen_soulAI(pCreature); }
...
struct boss_exarch_maladaarAI : public ScriptedAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    if (!m_bHasTaunted && pWho->GetTypeId() == TYPEID_PLAYER && m_creature->IsWithinDistInMap(pWho, 150.0f) &&
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiStolenSoulTimer < uiDiff) {
      if (Unit *pTarget =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_STOLEN_SOUL, SELECT_FLAG_PLAYER)) {
...
UnitAI *GetAI_boss_exarch_maladaar(Creature *pCreature) { return new boss_exarch_maladaarAI(pCreature); }
