...
enum {
  ...
  SPELL_VOID_PORTAL_A = 33566, // spell only summon one unit, but we use it for the visual effect and summon the 4 other
  ...
  SPELL_SUMMON_VOID_SUMMONER =
      33927, // serverside unused currently, summoned trigger 19427 probably handles add spawning
...
UnitAI *GetAI_boss_grandmaster_vorpil(Creature *pCreature) { return new boss_grandmaster_vorpilAI(pCreature); }
...
UnitAI *GetAI_npc_void_traveler(Creature *pCreature) { return new npc_void_travelerAI(pCreature); }
