...
struct boss_blackheart_the_inciterAI : public CombatAI {
  boss_blackheart_the_inciterAI(Creature *creature)
      : CombatAI(creature, BLACKHEART_ACTION_MAX),
  ...
  uint32 GetSubsequentActionTimer(BlackheartActions id) {
    ...
    case BLACKHEART_INCITE_CHAOS:
      return urand(52000, 60000);
    case BLACKHEART_CHARGE:
      return urand(26000, 44000);
...
struct dummy_blackheart_the_inciterAI : public ScriptedAI {
  dummy_blackheart_the_inciterAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())), m_threat(false),
        m_attack(false) {
    SetCombatMovement(false);
  }
  ...
  void UpdateAI(const uint32 /*diff*/) override {
    ...
    if (Unit *target =
            m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DUMMY_NUKE, SELECT_FLAG_PLAYER))
...
struct InciteChaos : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override {
    ...
    case 0:
      spellId = SPELL_INCITE_CHAOS_SPAWN_1;
      break;
    case 1:
      spellId = SPELL_INCITE_CHAOS_SPAWN_2;
      break;
    case 2:
      spellId = SPELL_INCITE_CHAOS_SPAWN_3;
      break;
    case 3:
      spellId = SPELL_INCITE_CHAOS_SPAWN_4;
      break;
    case 4:
      spellId = SPELL_INCITE_CHAOS_SPAWN_5;
      break;
    }
