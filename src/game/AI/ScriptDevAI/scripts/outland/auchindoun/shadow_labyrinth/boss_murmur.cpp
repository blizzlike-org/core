...
enum {
  ...
  SPELL_MURMURS_TOUCH =
      33711, // on expire silences the party members using shockwave - 33686 - also related to spell 33760
...
struct boss_murmurAI : public Scripted_NoMovementAI {
  ...
  void Reset() override {
    ...
    m_uiMurmursTouchTimer = m_bIsRegularMode ? SPELL_MURMURS_TOUCH_TIMER_N
                                             : urand(SPELL_MURMURS_TOUCH_MIN_TIMER_H, SPELL_MURMURS_TOUCH_MAX_TIMER_H);
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiMurmursTouchTimer < uiDiff) {
      if (DoCastSpellIfCan(m_creature, m_bIsRegularMode ? SPELL_MURMURS_TOUCH : SPELL_MURMURS_TOUCH_H) == CAST_OK)
        m_uiMurmursTouchTimer = m_bIsRegularMode
                                    ? SPELL_MURMURS_TOUCH_TIMER_N
                                    : urand(SPELL_MURMURS_TOUCH_MIN_TIMER_H, SPELL_MURMURS_TOUCH_MAX_TIMER_H);
    } else
    ...
    if (m_bIsRegularMode) {
      ...
      if (m_uiMagneticPullTimer < uiDiff) {
        if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MAGNETIC_PULL,
                                                              SELECT_FLAG_PLAYER)) {
      ...
      if (m_uiSonicShockTimer < uiDiff) {
        if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SONIC_SHOCK,
                                                              SELECT_FLAG_PLAYER | SELECT_FLAG_IN_MELEE_RANGE)) {
      ...
      if (m_uiThunderingStormTimer < uiDiff) {
        ...
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_FARTHEST_AWAY, 0, nullptr,
...
UnitAI *GetAI_boss_murmur(Creature *pCreature) { return new boss_murmurAI(pCreature); }

struct SuppressionBlast : public SpellScript {
  void OnInit(Spell *spell) const override { spell->SetMaxAffectedTargets(5); }
