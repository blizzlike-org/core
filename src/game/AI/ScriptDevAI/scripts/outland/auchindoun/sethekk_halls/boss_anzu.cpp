...
float spiritSpawns[][4] = {
    {-118.1717f, 284.5299f, 121.2287f, 2.775074f}, {-87.59662f, 263.5181f, 92.70478f, 1.658063f},
    {-60.94724f, 272.2065f, 111.3746f, 5.759586f}, {-98.15528f, 293.4469f, 109.2385f, 0.1745329f},
    {-99.7016f, 270.1699f, 98.27389f, 6.178465f},  {-69.25543f, 303.0768f, 97.84479f, 5.532694f},
};
...
struct boss_anzuAI : public ScriptedAI, public CombatActions {
  ...
  uint32 GetSubsequentActionTimer(AnzuActions id) {
    ...
    case ANZU_ACTION_SCREECH:
      return urand(31000, 35000);
    case ANZU_ACTION_SPELL_BOMB:
      return urand(24000, 40000);
    case ANZU_ACTION_CYCLONE:
      return 21000;
    case ANZU_ACTION_DIVE:
      return 20000;
  ...
  void DoSummonBroodsOfAnzu() {
    ...
    for (uint8 i = 0; i < MAX_BROODS; ++i)
      m_creature->SummonCreature(NPC_BROOD_OF_ANZU, spiritSpawns[i][0], spiritSpawns[i][1], spiritSpawns[i][2],
                                 spiritSpawns[i][3], TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 30000, true, true,
                                 urand(1, 2));
  }
      ...
      case ANZU_ACTION_SPELL_BOMB:
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
          ...
          if (DoCastSpellIfCan(target, SPELL_SPELL_BOMB) == CAST_OK) {
            switch (urand(0, 2)) {
            case 0:
              DoScriptText(SAY_WHISPER_MAGIC_1, m_creature, target);
              break;
            case 1:
              DoScriptText(SAY_WHISPER_MAGIC_2, m_creature, target);
              break;
            case 2:
              DoScriptText(SAY_WHISPER_MAGIC_3, m_creature, target);
              break;
            }
            SetActionReadyStatus(i, false);
            ResetTimer(i, GetSubsequentActionTimer(AnzuActions(i)));
...
struct npc_anzu_bird_spiritAI : public ScriptedAI {
  ...
  uint32 GetSpellId() {
    ...
    case NPC_HAWK_SPIRIT:
      return SPELL_PROTECTION_OF_THE_HAWK;
    case NPC_FALCON_SPIRIT:
      return SPELL_SPEED_OF_THE_FALCON;
    case NPC_EAGLE_SPIRIT:
      return SPELL_SPITE_OF_THE_EAGLE;
    default:
      return 0;
    }
  }
...
UnitAI *GetAI_boss_anzu(Creature *pCreature) { return new boss_anzuAI(pCreature); }

UnitAI *GetAI_npc_anzu_bird_spirit(Creature *pCreature) { return new npc_anzu_bird_spiritAI(pCreature); }
