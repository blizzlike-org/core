...
void instance_sethekk_halls::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
float triggerPositions[TRIGGER_COUNT][4] = {
    {-55.26986f, 313.4585f, 50.90593f, 0.3316126f}, {-59.17055f, 314.39f, 51.65655f, 0.541052f},
    {-79.32272f, 314.4245f, 50.42631f, 2.181662f},  {-86.93535f, 314.4072f, 50.36619f, 5.916666f},
    {-82.65174f, 314.4917f, 49.48673f, 0.4712389f}, {-110.0913f, 314.4524f, 50.17954f, 1.43117f},
    {-113.7065f, 314.51f, 51.44148f, 0.2617994f},   {-112.4148f, 258.7079f, 50.66346f, 0.4363323f},
    {-108.5766f, 258.7865f, 51.30656f, 3.124139f},  {-88.22328f, 258.7913f, 50.12397f, 2.600541f},
    {-84.26708f, 258.7147f, 49.33371f, 4.328416f},  {-80.07829f, 258.7955f, 50.39458f, 0.2268928f},
    {-59.91186f, 259.3824f, 50.82623f, 1.37881f},   {-55.34531f, 258.1663f, 51.48593f, 1.623156f},
};

void instance_sethekk_halls::StartAnzuIntro(Player *player) {
  Creature *target = player->SummonCreature(NPC_INVIS_RAVEN_GOD_TARGET, -87.08602f, 287.8433f, 83.77484f, 3.455752f,
                                            TEMPSPAWN_MANUAL_DESPAWN, 0, true);
  target->CastSpell(nullptr, SPELL_PURPLE_BANISH_STATE, TRIGGERED_NONE);
  Creature *portal = player->SummonCreature(NPC_INVIS_RAVEN_GOD_PORTAL, -87.28478f, 287.6349f, 27.17761f, 0.5759587f,
                                            TEMPSPAWN_MANUAL_DESPAWN, 0, true);
  portal->CastSpell(nullptr, SPELL_OTHERWORLDLY_PORTAL, TRIGGERED_NONE);
  for (uint32 i = 0; i < TRIGGER_COUNT; ++i) {
    Creature *caster =
        player->SummonCreature(NPC_INVIS_RAVEN_GOD_CASTER, triggerPositions[i][0], triggerPositions[i][1],
                               triggerPositions[i][2], triggerPositions[i][3], TEMPSPAWN_MANUAL_DESPAWN, 0, true);
...
bool ProcessEventId_event_spell_summon_raven_god(uint32 /*uiEventId*/, Object *source, Object * /*pTarget*/,
                                                 bool bIsStart) {
...
InstanceData *GetInstanceData_instance_sethekk_halls(Map *pMap) { return new instance_sethekk_halls(pMap); }
