...
struct npc_skyguard_rangerAI : public ScriptedAI, public CombatActions {
  npc_skyguard_rangerAI(Creature *creature)
      : ScriptedAI(creature), CombatActions(RANGER_COMBAT_ACTION_MAX),
        m_spawnId(PATH_ID_DISMOUNT) // implicit default for lieutenant
  {
      ...
      if (GetActionReadyStatus(i)) {
        ...
        case FIEND_ACTION_EAT_FRIEND: {
          ...
          if (slaveringSlave) // found corpse of ally
          {
            ...
            float x, y, z;
            m_creature->GetVictim()->GetNearPoint(m_creature, x, y, z, m_creature->GetObjectBoundingRadius(),
                                                  m_creature->GetCombinedCombatReach(slaveringSlave),
                                                  slaveringSlave->GetAngle(m_creature));
...
struct npc_disruptor_towerAI : public ScriptedAI, public TimerManager {
  ...
  void JustSummoned(Creature *summoned) override { summoned->CastSpell(m_creature, SPELL_BOLT_BURST, TRIGGERED_NONE); }
...
struct npc_grand_collectorAI : public ScriptedAI, public CombatActions {
  npc_grand_collectorAI(Creature *creature)
      : ScriptedAI(creature), CombatActions(COLLECTOR_COMBAT_ACTION_MAX), m_introEventId(0),
  ...
  void JustRespawned() override { DoScriptText(SAY_COLLECTOR_SPAWN, m_creature); }
