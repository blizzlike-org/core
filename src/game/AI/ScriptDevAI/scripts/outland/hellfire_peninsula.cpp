...
UnitAI *GetAI_npc_aeranas(Creature *pCreature) { return new npc_aeranasAI(pCreature); }
...
enum {
  ...
  SPELL_SELF_SNARE =
      30011, // Quest - Ancestral Spirit Wolf Self Snare, casted on self shortly after spawn (purpose unknown)
...
struct npc_ancestral_wolfAI : public npc_escortAI {
  ...
  void Reset() override { DoCastSpellIfCan(m_creature, SPELL_ANCESTRAL_WOLF_BUFF); }
...
UnitAI *GetAI_npc_ancestral_wolf(Creature *pCreature) { return new npc_ancestral_wolfAI(pCreature); }
...
UnitAI *GetAI_npc_demoniac_scryer(Creature *pCreature) { return new npc_demoniac_scryerAI(pCreature); }
...
struct npc_wounded_blood_elfAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 14:
      ...
      m_creature->SummonCreature(NPC_TALONGUARD, -983.206f, 4163.884f, 38.01043f, 0.72f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 10000);
      m_creature->SummonCreature(NPC_TALONGUARD, -985.732f, 4157.546f, 43.50933f, 0.80f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 10000);
      break;
    case 20:
    ...
    case 22:
      ...
      m_creature->SummonCreature(NPC_WINDWALKER, -864.001f, 4253.191f, 43.89390f, 5.27f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 10000);
      break;
    case 39:
...
UnitAI *GetAI_npc_wounded_blood_elf(Creature *pCreature) { return new npc_wounded_blood_elfAI(pCreature); }
...
UnitAI *GetAI_npc_fel_guard_hound(Creature *pCreature) { return new npc_fel_guard_houndAI(pCreature); }

bool EffectDummyCreature_npc_fel_guard_hound(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
struct npc_colonel_julesAI : public ScriptedAI {
    ...
    if (pSummoned->GetEntry() == NPC_DARKNESS_RELEASED) {
      if (darknessReleasedCount < 3 && m_creature->HasAura(SPELL_JULES_THREATENS_AURA)) {
        ...
        if (Creature *pBarada = GetClosestCreatureWithEntry(m_creature, NPC_ANCHORITE_BARADA, 15.0f)) {
          ...
          pSummoned->GetMotionMaster()->MoveRandomAroundPoint(pBarada->GetPositionX(), pBarada->GetPositionY(),
                                                              pBarada->GetPositionZ(), 5.0f);
        }
      } else {
  ...
  void SummonedCreatureJustDied(Creature *pSummoned) override {
    ...
    m_vSummonVector.erase(std::remove(m_vSummonVector.begin(), m_vSummonVector.end(), pSummoned->GetObjectGuid()),
                          m_vSummonVector.end());
  }
...
UnitAI *GetAI_npc_colonel_jules(Creature *pCreature) { return new npc_colonel_julesAI(pCreature); }

static const DialogueEntry aExorcismDialogue[] = {{SAY_EXORCISM_1, NPC_ANCHORITE_BARADA, 2500},
...
static const int32 aAnchoriteTexts[7] = {BARADA_SAY_EXORCISM_RANDOM_1, BARADA_SAY_EXORCISM_RANDOM_2,
                                         BARADA_SAY_EXORCISM_RANDOM_3, BARADA_SAY_EXORCISM_RANDOM_4,
                                         BARADA_SAY_EXORCISM_RANDOM_5, BARADA_SAY_EXORCISM_RANDOM_6,
                                         BARADA_SAY_EXORCISM_RANDOM_7};
static const int32 aColonelTexts[5] = {JULES_SAY_EXORCISM_RANDOM_1, JULES_SAY_EXORCISM_RANDOM_2,
                                       JULES_SAY_EXORCISM_RANDOM_3, JULES_SAY_EXORCISM_RANDOM_4,
                                       JULES_SAY_EXORCISM_RANDOM_5};
...
struct npc_anchorite_baradaAI : public ScriptedAI, private DialogueHelper {
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case NPC_EXORCISM_LIGHTNING_CLOUD:
      if (Creature *pColonel = m_creature->GetMap()->GetCreature(m_colonelGuid))
        pColonel->SummonCreature(NPC_EXORCISM_LIGHTNING_CLOUD, -709.8715f, 2756.557f, 113.6571f, 4.747295f,
                                 TEMPSPAWN_TIMED_DESPAWN, 200000);
      break;
    case SPELL_BARADA_FALTERS:
      ...
      if (Creature *pColonel = m_creature->GetMap()->GetCreature(m_colonelGuid)) {
        ...
        pColonel->GetMotionMaster()->MovePoint(10, pColonel->GetPositionX(), pColonel->GetPositionY(),
                                               pColonel->GetPositionZ() + 2.0f);
      }
      break;
  ...
  void JustDied(Unit * /*pKiller*/) override {
    ...
    } else if (Creature *pColonel = GetClosestCreatureWithEntry(m_creature, NPC_COLONEL_JULES, 30.0f))
  ...
  Creature *GetSpeakerByEntry(uint32 uiEntry) override {
    ...
    case NPC_ANCHORITE_BARADA:
      return m_creature;
    case NPC_COLONEL_JULES:
      return m_creature->GetMap()->GetCreature(m_colonelGuid);
...
UnitAI *GetAI_npc_anchorite_barada(Creature *pCreature) { return new npc_anchorite_baradaAI(pCreature); }
  ...
  if (pPlayer->IsCurrentQuest(QUEST_ID_EXORCISM, 1))
    if (npc_anchorite_baradaAI *baradaAI = dynamic_cast<npc_anchorite_baradaAI *>(pCreature->AI()))
      if (!baradaAI->m_bEventInProgress)
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_EXORCISM, GOSSIP_SENDER_MAIN,
...
struct npc_magister_aledisAI : public RangedCombatAI {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ALEDIS_ACTION_FROSTNOVA: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_FROST_NOVA,
                                                           SELECT_FLAG_PLAYER | SELECT_FLAG_USE_EFFECT_RADIUS))
...
struct npc_living_flareAI : public ScriptedPetAI {
  npc_living_flareAI(Creature *pCreature) : ScriptedPetAI(pCreature), m_uiStacks(0) { Reset(); }
  ...
  void CheckPortalDist() {
    if (Creature *questCredit = GetClosestCreatureWithEntry(
            m_creature, NPC_GENERIC_QUEST_TRIGGER_LAB, 50.f)) // if within distance of portal, initiate final sequence
    {
      m_uiCheckTimer = 0;
      m_uiFollow = false;
...
UnitAI *GetAI_danath_trollbane(Creature *pCreature) { return new npc_danath_trollbaneAI(pCreature); }
...
UnitAI *GetAI_nazgrel(Creature *pCreature) { return new npc_nazgrelAI(pCreature); }
...
struct npc_vindicator_sedaiAI : public ScriptedAI, public CombatActions {
  npc_vindicator_sedaiAI(Creature *creature) : ScriptedAI(creature), CombatActions(SEDAI_COMBAT_ACTION_MAX) {
    ...
    AddCustomAction(SEDAI_ACTION_FELORC_SPAWN_ATTACK, true, [&]() {
      if (Creature *orc = m_creature->SummonCreature(NPC_FEL_ORC, 258.168854f, 4109.307617f, 91.639290f, 2.644194f,
                                                     TEMPSPAWN_TIMED_DESPAWN, 60000, true)) {
      ...
      if (Creature *orc = m_creature->SummonCreature(NPC_FEL_ORC, 256.429932f, 4105.590820f, 90.982086f, 2.734515f,
                                                     TEMPSPAWN_TIMED_DESPAWN, 60000, true)) {
  ...
  void JustRespawned() override {
    if (Creature *maghar = m_creature->SummonCreature(NPC_MAGHAR_ESCORT, 216.0346f, 4125.61f, 80.22345f, 3.41032f,
                                                      TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 300000, true)) {
    ...
    if (Creature *maghar = m_creature->SummonCreature(NPC_MAGHAR_ESCORT, 214.1331f, 4123.221f, 79.6478f, 2.233855f,
                                                      TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 300000, true)) {
      maghar->SetCorpseDelay(20);
  ...
  void MovementInform(uint32 /*movementType*/, uint32 data) override {
    ...
    case 3: {
      if (Creature *krun = m_creature->SummonCreature(NPC_KRUN, 225.445526f, 4120.721191f, 82.345039f, 2.443936f,
                                                      TEMPSPAWN_TIMED_DESPAWN, 60000, true)) {
...
UnitAI *GetAI_npc_vindicator_sedai(Creature *creature) { return new npc_vindicator_sedaiAI(creature); }
...
enum KrunActions : uint32 { KRUN_ACTION_EXECUTE_SEDAI, KRUN_ACTION_LAUGH, KRUN_ACTION_DESPAWN };
...
UnitAI *GetAI_npc_krun(Creature *creature) { return new npc_krunAI(creature); }
...
struct npc_laughing_skullAI : public ScriptedAI {
  ...
  void JustReachedHome() override { m_creature->GetMotionMaster()->MoveIdle(); }
};

UnitAI *GetAI_npc_laughing_skull(Creature *creature) { return new npc_laughing_skullAI(creature); }
...
UnitAI *GetAI_npc_maghar_escort(Creature *creature) { return new npc_maghar_escortAI(creature); }
...
bool ProcessEventId_sedai_vision(uint32 /*eventId*/, Object *source, Object * /*target*/, bool /*isStart*/) {
  if (Creature *sedai = GetClosestCreatureWithEntry((WorldObject *)source, NPC_SEDAI, 100.0f))
    ...
    sedai = player->SummonCreature(17404, 211.1362f, 4126.989f, 78.81913f, 2.281034f, TEMPSPAWN_TIMED_OR_DEAD_DESPAWN,
                                   300000, true);
...
struct go_smoke_beacon : public GameObjectAI {
  go_smoke_beacon(GameObject *go) : GameObjectAI(go) {
    m_uiCustomAnimTimer = 250;
    m_uiAnimCount = 0;
  }
...
GameObjectAI *GetAI_go_smoke_beacon(GameObject *go) { return new go_smoke_beacon(go); }
  ...
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    ...
    if (Unit *unitTarget = spell->GetUnitTarget()) {
      if (Creature *summoner = GetClosestCreatureWithEntry(unitTarget, NPC_CREDIT_MARKER_THEY_MUST_BURN, 250.f)) {
        if (npc_credit_marker_they_must_burnAI *summonerAI =
                dynamic_cast<npc_credit_marker_they_must_burnAI *>(summoner->AI())) {
