...
SD%Complete: 90
SDComment: Quest support: 10503, 10504, 10506, 10512, 10545, 10556, 10609, 10674, 10859, 10998, 11058, 11080.
(npc_daranelle needs bit more work before consider complete) SDCategory: Blade's Edge Mountains EndScriptData */
...
struct mobs_nether_drakeAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_bIsNihil) {
      if (m_uiNihilSpeechTimer < uiDiff) {
        ...
        case 4:
          ...
          m_creature->GetMotionMaster()->MovePoint(1, m_creature->GetPositionX() + 50.0f, m_creature->GetPositionY(),
                                                   m_creature->GetPositionZ() + 50.0f);
          break;
        }
    ...
    if (m_uiManaBurnTimer < uiDiff) {
      if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MANA_BURN,
...
UnitAI *GetAI_mobs_nether_drake(Creature *pCreature) { return new mobs_nether_drakeAI(pCreature); }

/*######
## npc_daranelle
######*/

enum { SAY_SPELL_INFLUENCE = -1000174, SPELL_LASHHAN_CHANNEL = 36904, SPELL_DISPELLING_ANALYSIS = 37028 };
...
UnitAI *GetAI_npc_daranelle(Creature *pCreature) { return new npc_daranelleAI(pCreature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiStartTimer) {
      if (m_uiStartTimer <= uiDiff) {
        ...
        switch (urand(0, 2)) {
        case 0:
          DoScriptText(SAY_BREW_1, pOgre);
          break;
        case 1:
          DoScriptText(SAY_BREW_2, pOgre);
          break;
        case 2:
          DoScriptText(SAY_BREW_3, pOgre);
          break;
        }
...
UnitAI *GetAI_npc_bloodmaul_stout_trigger(Creature *pCreature) { return new npc_bloodmaul_stout_triggerAI(pCreature); }
...
struct npc_simon_game_bunnyAI : public ScriptedAI {
  ...
  void GetAIInformation(ChatHandler &reader) override {
    reader.PSendSysMessage("Simon Game Bunny, current game phase = %u, current level = %u", uint32(m_uiGamePhase),
                           m_uiLevelCount);
  }
  ...
  void DoPrepareLevel() {
    ...
    DoCastSpellIfCan(nullptr, m_bIsLargeEvent ? SPELL_PRE_GAME_YELLOW_LARGE : SPELL_PRE_GAME_YELLOW_AURA,
                     CAST_TRIGGERED);
      ...
      if (eventType == AI_EVENT_CUSTOM_C) {
        ...
        if (uiMiscValue == aApexisGameData[m_vColors[m_uiLevelStage]].m_uiIntrospection) {
          ...
          // cast tick sound
            ...
            DoCastSpellIfCan(pInvoker, m_bIsLargeEvent ? SPELL_VISUAL_GAME_TICK_LARGE : SPELL_VISUAL_GAME_TICK,
                             CAST_TRIGGERED);
        }
        // bad button pressed
        else {
...
UnitAI *GetAI_npc_simon_game_bunny(Creature *pCreature) { return new npc_simon_game_bunnyAI(pCreature); }

bool EffectDummyCreature_npc_simon_game_bunny(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
bool EffectScriptEffectCreature_npc_simon_game_bunny(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                                     Creature *pCreatureTarget, ObjectGuid originalCasterGuid) {
  if ((uiSpellId == SPELL_INTROSPECTION_BLUE || uiSpellId == SPELL_INTROSPECTION_GREEN ||
       uiSpellId == SPELL_INTROSPECTION_RED || uiSpellId == SPELL_INTROSPECTION_YELLOW) &&
      uiEffIndex == EFFECT_INDEX_1) {
    if ((pCreatureTarget->GetEntry() == NPC_SIMON_GAME_BUNNY ||
         pCreatureTarget->GetEntry() == NPC_SIMON_GAME_BUNNY_LARGE) &&
...
struct npc_light_orb_collectorAI : public ScriptedAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    ...
    } else if (m_bOrbPulled && pWho->GetObjectGuid() == m_selectedOrbGuid &&
               m_creature->IsWithinDistInMap(pWho, 3.5f)) {
...
UnitAI *GetAI_npc_light_orb_collector(Creature *pCreature) { return new npc_light_orb_collectorAI(pCreature); }
    ...
    if (m_uiActiveObelisk == 5) {
      if (m_uiActivateTimer) {
        if (m_uiActivateTimer <= uiDiff && !m_uiSpawnBoss) {
          Creature *m_uiDoomcryer =
              m_creature->SummonCreature(NPC_DOOMCRYER, m_creature->GetPositionX(), m_creature->GetPositionY(), 283.65f,
                                         0.32f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
...
UnitAI *GetAI_obelisk_triggerAI(Creature *pCreature) { return new npc_obelisk_triggerAI(pCreature); }
...
struct npc_vimgol_AI : public ScriptedAI {
  ...
  npc_vimgol_AI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
...
UnitAI *GetAI_npc_vimgol(Creature *pCreature) { return new npc_vimgol_AI(pCreature); }
...
struct npc_vimgol_visual_bunnyAI : public ScriptedAI {
  ...
  void Reset() override { m_uiCastTimer = 0; }

  uint32 GetScriptData() override { return m_uiBunnyId; }
...
UnitAI *GetAI_npc_vimgol_visual_bunny(Creature *pCreature) { return new npc_vimgol_visual_bunnyAI(pCreature); }
...
struct npc_vimgol_middle_bunnyAI : public ScriptedAI {
  ...
  uint8 playersInsideCircles() {
    uint32 tmpAuras[5] = {SPELL_VIMGOL_POP_TEST_A, SPELL_VIMGOL_POP_TEST_B, SPELL_VIMGOL_POP_TEST_C,
    ...
    for (auto &itr : playerList) {
      if (!itr->HasAura(SPELL_VIMGOL_POP_TEST_A) && !itr->HasAura(SPELL_VIMGOL_POP_TEST_B) &&
          !itr->HasAura(SPELL_VIMGOL_POP_TEST_C) && !itr->HasAura(SPELL_VIMGOL_POP_TEST_D) &&
          !itr->HasAura(SPELL_VIMGOL_POP_TEST_E))
        continue;
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiScanTimer <= uiDiff) {
      ...
      if (m_uiSpawned) {
        ...
        if (playersInsideCircles() == 5)
          m_creature->SummonCreature(NPC_VIMGOL_THE_VILE, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                     m_creature->GetPositionZ(), m_creature->GetOrientation(),
                                     TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 90000);
        if (m_uiBeamTimer == 0) {
          CastBunnySpell(nullptr, SPELL_PENTAGRAM_BEAM);
          m_uiBeamTimer = 5;
        }
...
UnitAI *GetAI_npc_vimgol_middle_bunny(Creature *pCreature) { return new npc_vimgol_middle_bunnyAI(pCreature); }
      ...
      if (m_uiTimer <= uiDiff) {
        ...
        case 0: {
          if (Creature *taskM = m_creature->GetMap()->GetCreature(m_taskmasterGuid))
            m_creature->GetMotionMaster()->MovePoint(1, taskM->GetPositionX(), taskM->GetPositionY(),
                                                     taskM->GetPositionZ());
          m_uiTimer = 0;
          m_uiPhase++;
          break;
        ...
        case 1: {
          ...
          if (Unit *summoner = m_creature->GetMap()->GetUnit(m_creature->GetSpawnerGuid()))
            m_creature->GetMotionMaster()->MovePoint(2, summoner->GetPositionX(), summoner->GetPositionY(),
...
UnitAI *GetAI_npc_bird_spirit(Creature *pCreature) { return new npc_bird_spiritAI(pCreature); }
...
enum {
  SPELL_REND = 13443,
  SPELL_RINAS_DIMINUTION_POWDER =
      36310, // Note: spell also throws event id 13584; requires more research on the purpose of this event
...
UnitAI *GetAI_npc_bloodmaul_dire_wolf(Creature *pCreature) { return new npc_bloodmaul_dire_wolfAI(pCreature); }

bool EffectScriptEffectCreature_spell_diminution_powder(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                                        Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/) {
  if (uiSpellId == SPELL_RINAS_DIMINUTION_POWDER && uiEffIndex == EFFECT_INDEX_1 &&
      pCreatureTarget->GetEntry() == NPC_BLOODMAUL_DIRE_WOLF) {
    ...
    // Note: we can't remove all auras because it will also remove the current aura; so currently we only remove
    // periodic damage auras This might be wrong and we might need to change this to something like
    // "RemoveAllAurasExceptId(...)"
...
bool AreaTrigger_at_raven_prophecy(Player *pPlayer, AreaTriggerEntry const *pAt) {
  if (/*pPlayer->IsGameMaster() ||*/ pPlayer->IsAlive() && pPlayer->HasAura(UNDERSTAND_RAVENSPEECH_AURA) &&
    ...
    if (prophecyIterator != prophecies.end()) {
      ...
      if (pPlayer->GetReqKillOrCastCurrentCount(QUEST_WHISPERS_OF_THE_RAVEN_GOD, prophecy.creature) == 0) {
        if (Creature *whisper = ((ScriptedMap *)pPlayer->GetInstanceData())
                                    ->GetSingleCreatureFromStorage(NPC_WHISPER_RAVEN_GOD_TEMPLATE)) {
          if (pPlayer->SummonCreature(NPC_VISION_RAVEN_GOD_TEMPLATE, prophecy.x, prophecy.y, prophecy.z, prophecy.o,
...
struct go_aura_generator_000AI : public GameObjectAI {
  go_aura_generator_000AI(GameObject *go)
      : GameObjectAI(go), m_auraSearchTimer(1000),
        m_spellInfo(sSpellTemplate.LookupEntry<SpellEntry>(SPELL_OSCILLATING_FREQUENCY_SCANNER)) {}
  ...
  void UpdateAI(const uint32 diff) override {
    if (m_auraSearchTimer <= diff) {
      ...
      if (Player *player = m_go->GetMap()->GetPlayer(m_player)) {
        ...
        bool isCloseEnough =
            player->GetDistance(x, y, z, DIST_CALC_COMBAT_REACH) <
            GetSpellRadius(sSpellRadiusStore.LookupEntry(m_spellInfo->EffectRadiusIndex[EFFECT_INDEX_0]));
        if (!myHolder) {
          if (isCloseEnough) {
            myHolder = CreateSpellAuraHolder(m_spellInfo, player, m_go);
            GameObjectAura *Aur =
                new GameObjectAura(m_spellInfo, EFFECT_INDEX_0, nullptr, nullptr, myHolder, player, m_go);
...
UnitAI *GetAI_npc_frequency_scanner(Creature *creature) { return new npc_frequency_scanner(creature); }

GameObjectAI *GetGOAI_go_aura_generator_000(GameObject *go) { return new go_aura_generator_000AI(go); }
...
UnitAI *GetAI_npc_fel_cannon(Creature *pCreature) { return new npc_fel_cannon(pCreature); }
...
static float fireSpawns[14][3] = {
    {2199.255f, 5474.860f, 153.578f}, {2185.102f, 5470.938f, 164.374f}, {2185.104f, 5484.197f, 154.817f},
    {2188.340f, 5476.629f, 155.069f}, {2194.513f, 5481.483f, 164.991f}, {2178.581f, 5477.184f, 157.264f},
    {2188.340f, 5476.629f, 155.069f}, {1982.273f, 5331.832f, 153.952f}, {1987.307f, 5307.395f, 169.677f},
    {1980.868f, 8325.251f, 169.290f}, {1981.730f, 5315.390f, 156.600f}, {1989.396f, 5322.008f, 155.384f},
    {1973.351f, 5299.629f, 155.395f}, {1981.730f, 5315.390f, 156.600f}};
  ...
  void SpellHit(Unit *pCaster, const SpellEntry *pSpell) override {
    if (pSpell->Id == SPELL_ARTILLERY_ON_THE_WARP_GATE) {
      if (!m_uiHitCounter) {
        if (m_resetTimer) {
          m_resetTimer = 0;
          m_creature->GetInstanceData()->SetData(m_creature->GetEntry() == NPC_DEATHS_DOOR_NORTH_WARP_GATE
                                                     ? TYPE_DEATHS_DOOR_NORTH
                                                     : TYPE_DEATHS_DOOR_SOUTH,
                                                 0);
        }
      ...
      case 6: {
        ...
        if (Creature *cannon = m_creature->GetMap()->GetCreature(m_guidFelCannon)) {
          if (Player *player = dynamic_cast<Player *>(cannon->GetCharmer())) {
            player->RewardPlayerAndGroupAtEventCredit(m_creature->GetEntry() == NPC_DEATHS_DOOR_NORTH_WARP_GATE
                                                          ? NPC_NORTH_WARP_GATE_CREDIT
                                                          : NPC_SOUTH_WARP_GATE_CREDIT,
                                                      m_creature);
          }
        }
      ...
      m_creature->CastSpell(fireSpawns[i * 7 + m_uiHitCounter][0], fireSpawns[i * 7 + m_uiHitCounter][1],
                            fireSpawns[i * 7 + m_uiHitCounter][2], spellId, TRIGGERED_OLD_TRIGGERED);

      m_uiHitCounter++;
      m_creature->GetInstanceData()->SetData(
          m_creature->GetEntry() == NPC_DEATHS_DOOR_NORTH_WARP_GATE ? TYPE_DEATHS_DOOR_NORTH : TYPE_DEATHS_DOOR_SOUTH,
          m_uiHitCounter);
      if (m_uiHitCounter == 7) {
        m_resetTimer = 30000;
        Reset();
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiHitCounter) {
      ...
      if (m_uiSpawnImpTimer <= uiDiff) {
        uint32 i = m_creature->GetEntry() - NPC_DEATHS_DOOR_NORTH_WARP_GATE;
        if (m_creature->SummonCreature(NPC_UNSTABLE_FEL_IMP, impSpawns[i][0], impSpawns[i][1], impSpawns[i][2],
                                       impSpawns[i][3], TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 20000, true, true))
...
UnitAI *GetAI_npc_warp_gate(Creature *pCreature) { return new npc_warp_gate(pCreature); }
...
bool ProcessEventId_Soulgrinder(uint32 /*uiEventId*/, Object *pSource, Object * /*pTarget*/, bool /*bIsStart*/) {
  ...
  player->SummonCreature(NPC_SOULGRINDER, 3535.111f, 5590.628f, 0.3859383f, 0.7853982f,
                         TEMPSPAWN_TIMED_OR_CORPSE_DESPAWN, 265000);
  return true;
}
...
struct npc_soulgrinderAI : public ScriptedAI {
  ...
  void JustRespawned() override {
    ...
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.525f, 5624.78f, -9.746364f, 5.707227f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3558.613f, 5543.979f, -2.24089f, 0.9948376f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.293f, 5546.184f, -2.307737f, 5.096361f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3507.783f, 5607.564f, -2.115761f, 2.146755f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3525.318f, 5624.862f, -5.340961f, 0.2792527f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3573.157f, 5592.371f, -4.614707f, 2.984513f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.814f, 5622.923f, -3.886532f, 0.1919862f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.056f, 5574.628f, -5.577652f, 0.2792527f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3559.653f, 5559.587f, -2.850421f, 3.01942f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.718f, 5609.495f, -3.285291f, 1.32645f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3541.38f, 5559.73f, -2.665792f, 2.827433f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.222f, 5573.395f, -2.64414f, 4.276057f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3557.254f, 5591.203f, -2.993953f, 3.01942f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3558.495f, 5577.132f, -3.076451f, 3.892084f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3557.081f, 5612.431f, -4.562363f, 1.518436f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.203f, 5608.064f, -3.378048f, 0.1570796f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3558.009f, 5626.f, -5.146327f, 3.857178f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3524.67f, 5608.019f, -3.321486f, 2.042035f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3510.031f, 5575.792f, -1.80589f, 4.904375f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3508.348f, 5589.707f, -0.8961017f, 1.623156f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3507.988f, 5561.777f, 1.993673f, 2.80998f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3509.509f, 5625.191f, -4.461998f, 0.06981317f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3525.131f, 5573.122f, -1.622976f, 6.126106f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3493.974f, 5594.064f, -0.03712566f, 2.670354f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3526.678f, 5560.159f, -0.8016807f, 0.9424778f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
    m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.228f, 5559.057f, -6.009155f, 6.021386f, TEMPSPAWN_CORPSE_DESPAWN,
                               1000);
  }
      ...
      if (m_uiTimer <= uiDiff) {
        ...
        case 0: {
          ...
          if (m_uiOgreCounter == 2) {
            Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3491.224f, 5529.023f, 17.14335f,
                                                         6.195919f, TEMPSPAWN_MANUAL_DESPAWN, 0);
            Creature *gronn = m_creature->SummonCreature(NPC_SKULLOC_SOULGRINDER, 3478.563f, 5550.74f, 7.838801f,
                                                         0.3665192f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
            ...
            Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3464.954f, 5564.684f, 17.99977f,
                                                         5.67232f, TEMPSPAWN_MANUAL_DESPAWN, 0);
            bunny->CastSpell(m_creature, SPELL_BEAM, TRIGGERED_NONE);
            if (Creature *gronn = m_creature->GetMap()->GetCreature(m_skullocSoulgrinder)) {
            ...
            Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3515.02f, 5524.39f, 16.8927f, 3.560477f,
            ...
            Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3468.82f, 5581.41f, 17.5205f,
                                                         0.0174533f, TEMPSPAWN_MANUAL_DESPAWN, 0);
            bunny->CastSpell(m_creature, SPELL_BEAM, TRIGGERED_NONE);
            if (Creature *gronn = m_creature->GetMap()->GetCreature(m_skullocSoulgrinder)) {
...
UnitAI *GetAI_npc_soulgrinder(Creature *pCreature) { return new npc_soulgrinderAI(pCreature); }
...
struct npc_supplicantAI : public ScriptedAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    ...
    if (m_phase == PHASE_MASS_KNEEL_TO_PLAYER || m_phase == PHASE_KNEEL_TO_PLAYER ||
        m_phase == PHASE_UNKNEEL_TO_PLAYER || m_phase == PHASE_DRUMMER)
      return;
...
UnitAI *GetAI_npc_supplicant(Creature *pCreature) { return new npc_supplicantAI(pCreature); }
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit * /*pSender*/, Unit * /*pInvoker*/, uint32 /*uiMiscValue*/) override {
    if (eventType == AI_EVENT_CUSTOM_A) {
      if (Unit *spawner = m_creature->GetSpawner())
        m_creature->GetMotionMaster()->MovePoint(POINT_PLAYER_POSITION, spawner->GetPositionX(),
                                                 spawner->GetPositionY(), spawner->GetPositionZ());
    }
  }
...
struct npc_deadsoul_orb : public ScriptedAI {
  npc_deadsoul_orb(Creature *pCreature) : ScriptedAI(pCreature) {
    Reset();
    m_creature->SetActiveObjectState(
        true); // Need to be active since the area is so large they might get unloaded on the way to the destination
  ...
  void MoveToNextTrigger() {
    if (pointCount == 6)
      ...
      if (Creature *waypointTrigger = GetClosestCreatureWithEntry(m_creature, nextTrigger, 95)) {
        m_creature->GetMotionMaster()->MovePoint(pointCount, waypointTrigger->GetPositionX(),
                                                 waypointTrigger->GetPositionY(), waypointTrigger->GetPositionZ());
      } else {
...
UnitAI *GetAI_npc_deadsoul_orb(Creature *pCreature) { return new npc_deadsoul_orb(pCreature); }
...
struct npc_evergrove_druidAI : public ScriptedAI {
  npc_evergrove_druidAI(Creature *pCreature)
      : ScriptedAI(pCreature), returnTimer(0), landingDone(false), alreadySummoned(false) {}
  ...
  void JustReachedHome() override { m_creature->ForcedDespawn(); }
...
UnitAI *GetAI_npc_evergrove_druidAI(Creature *creature) { return new npc_evergrove_druidAI(creature); }
...
struct npc_apexis_flayerAI : public ScriptedAI, public CombatActions {
  ...
  uint32 GetInitialActionTimer(uint32 id) // Timers are copied from the EventAI script
  {
    ...
    case FLAYER_ACTION_REND:
      return urand(8300, 13300);
    case FLAYER_ACTION_SHRED_ARMOR:
      return urand(4200, 6200);
    default:
  ...
  uint32 GetSubsequentActionTimer(uint32 id) // Timers are copied from the EventAI script
  {
    switch (id) {
    case FLAYER_ACTION_REND:
      return urand(22000, 26000);
    case FLAYER_ACTION_SHRED_ARMOR:
      return urand(28100, 31600);
    default:
      return 0; // never occurs but for compiler
    }
  }
...
UnitAI *GetAI_npc_apexis_flayerAI(Creature *creature) { return new npc_apexis_flayerAI(creature); }
