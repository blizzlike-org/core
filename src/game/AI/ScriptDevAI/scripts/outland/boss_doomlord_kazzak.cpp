...
struct boss_doomlordkazzakAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiTwistedReflectionTimer < uiDiff) {
      if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_TWISTED_REFLECTION,
                                                            SELECT_FLAG_PLAYER)) {
...
UnitAI *GetAI_boss_doomlordkazzak(Creature *pCreature) { return new boss_doomlordkazzakAI(pCreature); }
