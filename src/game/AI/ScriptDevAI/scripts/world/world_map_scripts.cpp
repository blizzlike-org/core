...
bool ProcessEventTransports(uint32 uiEventId, Object *pSource, Object * /*pTarget*/, bool /*bIsStart*/) {
  ...
  if (entry)
    if (Creature *zeppelinMaster =
            ((ScriptedInstance *)transport->GetMap()->GetInstanceData())->GetSingleCreatureFromStorage(entry))
...
uint32 ShadeOfTheHorsemanData::GetDataAttackPhase(uint32 id) const { return m_shadeOfTheHorsemanAttackPhases[id]; }
