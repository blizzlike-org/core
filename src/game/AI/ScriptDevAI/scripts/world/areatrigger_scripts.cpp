...
bool AreaTrigger_at_childrens_week_spot(Player *pPlayer, AreaTriggerEntry const *pAt) {
  for (auto &i : TriggerOrphanSpell) {
    if (pAt->id == i[0] && pPlayer->GetMiniPet() && pPlayer->GetMiniPet()->GetEntry() == i[1]) {
...
enum { GO_COILFANG_WATERFALL = 184212 };
...
enum { QUEST_MANOR_RAVENHOLDT = 6681, NPC_RAVENHOLDT = 13936 };
...
enum { QUEST_SCENT_OF_LARKORWI = 4291, NPC_LARKORWI_MATE = 9683 };

bool AreaTrigger_at_scent_larkorwi(Player *pPlayer, AreaTriggerEntry const *pAt) {
  if (pPlayer->IsAlive() && !pPlayer->IsGameMaster() &&
      pPlayer->GetQuestStatus(QUEST_SCENT_OF_LARKORWI) == QUEST_STATUS_INCOMPLETE) {
    if (!GetClosestCreatureWithEntry(pPlayer, NPC_LARKORWI_MATE, 25.0f, false, false))
      pPlayer->SummonCreature(NPC_LARKORWI_MATE, pAt->x, pAt->y, pAt->z, 3.3f, TEMPSPAWN_TIMED_OOC_DESPAWN,
...
bool AreaTrigger_at_murkdeep(Player *pPlayer, AreaTriggerEntry const * /*pAt*/) {
  ...
  if (pPlayer->IsAlive() && !pPlayer->IsGameMaster() &&
      pPlayer->GetQuestStatus(QUEST_WANTED_MURKDEEP) == QUEST_STATUS_INCOMPLETE) {
    ...
    for (uint8 i = 0; i < 3; ++i) {
      ...
      pPlayer->GetRandomPoint(aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][0], aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][1],
                              aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][2], 5.0f, fX, fY, fZ);

      if (Creature *pTemp =
              pPlayer->SummonCreature(NPC_GREYMIST_COASTRUNNNER, fX, fY, fZ, aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][3],
                                      TEMPSPAWN_DEAD_DESPAWN, 0)) {
        pTemp->SetWalk(false);
        pTemp->GetRandomPoint(aSpawnLocations[POS_IDX_MURKDEEP_MOVE][0], aSpawnLocations[POS_IDX_MURKDEEP_MOVE][1],
...
bool AreaTrigger_at_ancient_leaf(Player *pPlayer, AreaTriggerEntry const * /*pAt*/) {
  ...
  if (pPlayer->GetQuestStatus(QUEST_ANCIENT_LEAF) == QUEST_STATUS_COMPLETE) {
    ...
    if (GetClosestCreatureWithEntry(pPlayer, NPC_VARTRUS, 50.0f) ||
        GetClosestCreatureWithEntry(pPlayer, NPC_STOMA, 50.0f) ||
        GetClosestCreatureWithEntry(pPlayer, NPC_HASTAT, 50.0f))
      return true;

    for (const auto &afSpawnLocation : afSpawnLocations)
      pPlayer->SummonCreature(afSpawnLocation.uiEntry, afSpawnLocation.fX, afSpawnLocation.fY, afSpawnLocation.fZ,
                              afSpawnLocation.fO, TEMPSPAWN_TIMED_DESPAWN, 5 * MINUTE * IN_MILLISECONDS);
  }
...
bool AreaTrigger_at_huldar_miran(Player *pPlayer, AreaTriggerEntry const * /*pAt*/) {
  ...
  if (m_saean)
    ...
    m_saean = m_huldar->SummonCreature(NPC_SAEAN, m_miranAmbushSpawns[0].m_fX, m_miranAmbushSpawns[0].m_fY,
                                       m_miranAmbushSpawns[0].m_fZ, m_miranAmbushSpawns[0].m_fO,
                                       TEMPSPAWN_CORPSE_TIMED_DESPAWN, 25000);
    if (m_saean)
      m_saean->SetFactionTemporary(FACTION_HOSTILE, TEMPFACTION_RESTORE_RESPAWN);
  }
  ...
  if (!GetClosestCreatureWithEntry(pPlayer, NPC_DARK_IRON_AMBUSHER, 60.0f, false, false)) {
    m_saean->SummonCreature(NPC_DARK_IRON_AMBUSHER, m_miranAmbushSpawns[1].m_fX, m_miranAmbushSpawns[1].m_fY,
                            m_miranAmbushSpawns[1].m_fZ, m_miranAmbushSpawns[1].m_fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                            25000);
    m_saean->SummonCreature(NPC_DARK_IRON_AMBUSHER, m_miranAmbushSpawns[2].m_fX, m_miranAmbushSpawns[2].m_fY,
                            m_miranAmbushSpawns[2].m_fZ, m_miranAmbushSpawns[2].m_fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                            25000);
  }
...
enum { SPELL_A52_NEURALYZER = 34400 };
...
bool AreaTrigger_at_twilight_grove(Player *player, AreaTriggerEntry const * /*pAt*/) {
  ...
  if (Creature *twilightCorrupter =
          player->SummonCreature(NPC_TWILIGHT_CORRUPTER, m_twilightCorrupterSpawn.m_fX, m_twilightCorrupterSpawn.m_fY,
                                 m_twilightCorrupterSpawn.m_fZ, m_twilightCorrupterSpawn.m_fO,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, 30 * MINUTE * IN_MILLISECONDS)) {
...
bool AreaTrigger_at_hive_tower(Player *player, AreaTriggerEntry const * /*pAt*/) {
  ...
  if (scriptedMap->GetData(TYPE_HIVE) !=
      NOT_STARTED) // Only summon more Hive'Ashi Drones if the 5 minutes timer is elapsed
    return false;
  ...
  if (player->IsAlive() && !player->IsGameMaster()) {
    ...
    for (uint8 i = POS_IDX_HIVE_DRONES_START; i <= POS_IDX_HIVE_DRONES_STOP; ++i)
      player->SummonCreature(NPC_HIVE_ASHI_DRONES, aSpawnLocations[i][0], aSpawnLocations[i][1], aSpawnLocations[i][2],
                             aSpawnLocations[i][3], TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 5 * MINUTE * IN_MILLISECONDS);
