...
struct boss_emerald_dragonAI : public ScriptedAI {
  ...
  void KilledUnit(Unit *pVictim) override {
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER)
      pVictim->CastSpell(pVictim, SPELL_MARK_OF_NATURE_PLAYER, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiSeepingFogTimer < uiDiff) {
      ...
      m_uiSeepingFogTimer =
          urand(120000, 150000); // Rather Guesswork, but one Fog has 2min duration, hence a bit longer
    } else
...
struct boss_emerissAI : public boss_emerald_dragonAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_EMERISS_AGGRO, m_creature); }

  void KilledUnit(Unit *pVictim) override {
    // summon a mushroom on the spot the player dies
    if (pVictim->GetTypeId() == TYPEID_PLAYER)
      pVictim->CastSpell(pVictim, SPELL_PUTRID_MUSHROOM, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                         m_creature->GetObjectGuid());
...
UnitAI *GetAI_boss_emeriss(Creature *pCreature) { return new boss_emerissAI(pCreature); }
...
enum {
  ...
  // SPELL_SUMMON_SPIRIT_SHADE   = 24810,                    // Summon spell was removed in TBC, was using
  // SPELL_EFFECT_SUMMON_DEMON Classic uses same code than TBC and WotLK for consistency
...
struct boss_lethonAI : public boss_emerald_dragonAI {
  ...
  // Summon a spirit which moves toward the boss and heals him for each player hit by the spell; used at 75%, 50% and
  // 25%
  bool DoSpecialDragonAbility() {
  ...
  void SpellHitTarget(Unit *pTarget, const SpellEntry *pSpell) override {
    ...
    if (pTarget->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_DRAW_SPIRIT) {
      ...
      if (Creature *pSummoned = pTarget->SummonCreature(NPC_SPIRIT_SHADE, 0.0f, 0.0f, 0.0f, pTarget->GetOrientation(),
                                                        TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 60 * IN_MILLISECONDS, false, 0,
                                                        0, false, false, true))
        pSummoned->CastSpell(pSummoned, SPELL_SPIRIT_SHAPE_VISUAL, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
...
struct npc_spirit_shadeAI : public ScriptedAI {
  ...
  void JustDied(Unit * /*pKiller*/) override { m_creature->ForcedDespawn(); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    // NPC does not start to move when summoned, it waits 4 seconds then removes invisibility and moves towards boss
    // Lethon
...
UnitAI *GetAI_boss_lethon(Creature *pCreature) { return new boss_lethonAI(pCreature); }

UnitAI *GetAI_npc_spirit_shade(Creature *pCreature) { return new npc_spirit_shadeAI(pCreature); }
...
struct boss_taerarAI : public boss_emerald_dragonAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_TAERAR_AGGRO, m_creature); }
...
UnitAI *GetAI_boss_taerar(Creature *pCreature) { return new boss_taerarAI(pCreature); }
...
struct boss_ysondreAI : public boss_emerald_dragonAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_YSONDRE_AGGRO, m_creature); }
...
UnitAI *GetAI_boss_ysondre(Creature *pCreature) { return new boss_ysondreAI(pCreature); }
