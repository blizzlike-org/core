...
enum ShadeOfTheHorseman {
  SPELL_JACK_O_LANTERNED =
      44185,                    // should this be here at all? shade is shown as the caster of this spell once in sniff
  SPELL_SHADE_DURATION = 44738, // casted on self at spawn, duration 15 minutes
  SPELL_MANIACAL_LAUGH_OTHER_DELAY_17 =
      44000,                             // purpose unk (possibly random between this and 43884 to play alternate sound)
  ...
  SPELL_START_FIRE_TARGET_TEST =
      42143,                // periodically triggered by 42140 every 1.25 sec, and triggers 42132/42142 on hit targets
  SPELL_START_FIRE = 42132, // heal to 100%, SPELL_AURA_OBS_MOD_HEALTH 4 periodic 2000, used currently to trigger spells
                            // 42176, 42074, and 42112
  ...
  SPELL_CONFLAGRATION_SOUND_THROTTLE =
      48148, // happens at same time as conflag, 7 second duration - don't repeat conflag sound during this time

  SPELL_MANIACAL_LAUGHTER_DELAYED_9 =
      43886,                   // triggers spell 43885 and plays sound 11975 after 10 secs, casted on death
  SPELL_VICTORY_EVENT = 42682, // casted on death, but why?
  SPELL_SUMMON_LARGE_JACKOLANTERN =
      44255, // casted 2-3 seconds after death, true script target unknown (we substitue npc 23686 Headless Horseman
             // Flame Bunny), triggers spell 44231 which summons reward gameobject 186887
  SPELL_WISP_ESCAPE_MISSILE = 43034, // 2-3 seconds after death

  SPELL_SHADE_VISUAL = 43904,        // ghostly appearance
  SPELL_CLIMAX_GHOST_VISUAL = 42575, // same appearance as 43904 except not visible as aura, we'll use this as it's
                                     // unknown what truly should be used here
  ...
  SPELL_START_FIRE_GUARD =
      43715, // used by guard npc to start fires on Fire Effigy objects (Fire Brigade Practice/Fire Training quests)
  SPELL_START_FIRE_TARGET_TEST_GUARD = 43711, // used by guard npc to look for targets to set fires on (works similar to
                                              // Shade's Start Fires Target Test spell 42143)
  ...
  SPELL_LOOT_JACK_O_LANTERN =
      44242, // creates Crudely Wrapped Gift 34077 (actually in TBC this is "Smashed Pumpkin Loot")
  SPELL_HAS_JACK_O_LANTERN_LOOT = 44245, // purpose unk
  SPELL_HAS_LOOTED_JACK_O_LANTERN =
      44246,           // casted on player after using reward jack-o-lantern object, prevents usage again for an hour
  ...
  // SPELL_ORC_CHILD_CRYING            = 64894, // since this doesn't seem to exist in TBC, we'll reuse 43323 for the
  // orcs instead (sounds 11816/11817)
  ...
  SPELL_INVISIBLE_CAMPFIRE_CREATE =
      42176, // summons 186249 "Invisible Cooking Object" for 60 seconds, when hit by 42132?
  ...
  SPELL_VISUAL_LARGE_FIRE = 42075, // used to make practice fires on Fire Effigy objects bigger from the start, there
                                   // are probably many other ways to do this
...
#define GOSSIP_ITEM_WHO_IS_HORSEMAN                                                                                    \
  "Who is the Headless Horseman?" // should be linked to broadcast text id 22620 instead of hardcoded here - how can we
                                  // do this in SD2?
#define GOSSIP_ITEM_SMASH_PUMPKIN                                                                                      \
  "Smash the pumpkin!" // should be linked to broadcast text id 23651 instead of hardcoded here - how can we do this in
                       // SD2?
...
struct npc_orphan_matronAI : public ScriptedAI {
  npc_orphan_matronAI(Creature *creature) : ScriptedAI(creature), m_instance(creature->GetInstanceData()) { Reset(); }
    ...
    case AREAID_RAZOR_HILL:
      if (Creature *horseman =
              m_creature->SummonCreature(NPC_SHADE_OF_THE_HORSEMAN, 143.9746f, -4824.7197f, 19.112612f, 0.5197f,
                                         TEMPSPAWN_DEAD_DESPAWN, 30 * MINUTE * IN_MILLISECONDS, true))
        horseman->GetMotionMaster()->MovePath(PATH_ID_RAZOR_HILL_FLY_IN, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_NONE,
                                              true);
      break;
    ...
    case AREAID_AZURE_WATCH:
      if (Creature *horseman =
              m_creature->SummonCreature(NPC_SHADE_OF_THE_HORSEMAN, -4086.784f, -12718.129f, 52.912674f, 2.01762f,
                                         TEMPSPAWN_DEAD_DESPAWN, 30 * MINUTE * IN_MILLISECONDS, true))
        horseman->GetMotionMaster()->MovePath(PATH_ID_AZURE_WATCH_FLY_IN, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_NONE,
                                              true);
      break;
...
struct npc_shade_of_the_horsemanAI : public ScriptedAI {
  npc_shade_of_the_horsemanAI(Creature *creature)
      : ScriptedAI(creature), m_performedDoubleLaugh(false), m_stopPeriodicFireTimer(30000), m_bombingRunTimer(148000),
        m_finalWarningTimer(268000), m_failTimer(388000), m_landing(false), m_startedFires(false),
        m_allFiresOutTestTimer(0), m_instance(creature->GetInstanceData()) {
  ...
  void StartLanding() {
    ...
    // complete Stop the Fires! quest for all players within 100 yds (both horde/alliance version, and wotlk version of
    // the quests)
    ...
    case SHADE_VILLAGE_KHARANOS:
      m_creature->GetMotionMaster()->MovePath(PATH_ID_KHARANOS_LANDING, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_NONE, true);
      break;
    case SHADE_VILLAGE_AZURE_WATCH:
      m_creature->GetMotionMaster()->MovePath(PATH_ID_AZURE_WATCH_LANDING, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_NONE,
                                              true);
      break;
    case SHADE_VILLAGE_RAZOR_HILL:
      m_creature->GetMotionMaster()->MovePath(PATH_ID_RAZOR_HILL_LANDING, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_NONE,
                                              true);
      break;
    case SHADE_VILLAGE_BRILL:
      m_creature->GetMotionMaster()->MovePath(PATH_ID_BRILL_LANDING, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_NONE, true);
      break;
    ...
    if (!m_landing) {
      ...
      if (m_bombingRunTimer) {
        if (m_bombingRunTimer <= diff) {
          ...
          m_creature->GetMotionMaster()->MovePath(m_creature->GetMotionMaster()->GetPathId() + 1, PATH_FROM_EXTERNAL,
                                                  FORCED_MOVEMENT_NONE, true);
          m_bombingRunTimer = 0;
        } else
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_spreadTimer && reachedFullSize && !startedShrinking) {
      if (m_spreadTimer <= diff) {
        uint32 phase =
            m_creature->GetInstanceData()->GetData(TYPE_SHADE_OF_THE_HORSEMAN_ATTACK_PHASE +
                                                   ShadeOfTheHorsemanData::GetTypeFromZoneId(m_creature->GetZoneId()));
        // only spread if we're in the putting out fires phase still, otherwise let the fires die out naturally (we
        // can't just let the towns burn forever) the 5 practice fires for each location should be allowed to spread all
        // the time
...
bool GossipSelect_go_large_jack_o_lantern(Player *player, GameObject *go, uint32 /*sender*/, uint32 action) {
  if (action == GOSSIP_ACTION_INFO_DEF + 1) {
    ...
    if (player->HasAura(SPELL_VICTORY)) // avoid applying double stacked buff (should maybe prevent from happening in
                                        // the first place)
...
bool GOUse_go_large_jack_o_lantern(Player * /*player*/, GameObject *go) { return false; };
    ...
    if (Unit *unitTarget = spell->GetUnitTarget()) {
      if (Unit *caster = spell->GetCaster()) {
        if (spell->m_spellInfo->Id == SPELL_START_FIRE_TARGET_TEST) {
          ...
          unitTarget->AI()->SendAIEvent(
              AI_EVENT_CUSTOM_C, unitTarget,
              unitTarget); // inform fire that it is involved with Fire Brigade Practice/Fire Training
        }
      }
    }
  }
