...
UnitAI *GetAInpc_ethereum_prisoner(Creature *creature) { return new npc_ethereum_prisonerAI(creature); }
...
enum { SPELL_JUMP_A_TRON = 33382, NPC_JUMP_A_TRON = 19041 };
...
enum BellHourlyObjects { GO_HORDE_BELL = 175885, GO_ALLIANCE_BELL = 176573, GO_KARAZHAN_BELL = 182064 };
...
GameObjectAI *GetAI_go_bells(GameObject *go) { return new go_ai_bell(go); }

/*####
## go_darkmoon_faire_music
####*/

enum { MUSIC_DARKMOON_FAIRE_MUSIC = 8440 };

struct go_ai_dmf_music : public GameObjectAI {
  go_ai_dmf_music(GameObject *go) : GameObjectAI(go) { m_uiMusicTimer = 5000; }
...
GameObjectAI *GetAI_go_darkmoon_faire_music(GameObject *go) { return new go_ai_dmf_music(go); }
...
struct go_brewfest_music : public GameObjectAI {
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_musicStartTimer <= 0) {
      switch (m_zoneTeam) {
      case TEAM_NONE:
        m_go->GetMap()->ExecuteDistWorker(m_go, m_go->GetVisibilityData().GetVisibilityDistance(), [&](Player *player) {
...
GameObjectAI *GetAIgo_brewfest_music(GameObject *go) { return new go_brewfest_music(go); }
...
struct go_midsummer_music : public GameObjectAI {
  go_midsummer_music(GameObject *go) : GameObjectAI(go) { m_musicTimer = 1000; }
...
GameObjectAI *GetAIgo_midsummer_music(GameObject *go) { return new go_midsummer_music(go); }
...
struct go_pirate_day_music : public GameObjectAI {
  go_pirate_day_music(GameObject *go) : GameObjectAI(go) { m_musicTimer = 1000; }
...
GameObjectAI *GetAIgo_pirate_day_music(GameObject *go) { return new go_pirate_day_music(go); }
...
GameObjectAI *GetAI_go_elemental_rift(GameObject *go) { return new go_elemental_rift(go); }

std::function<bool(Unit *)> function = &TrapTargetSearch;

enum {
  SPELL_BOMBING_RUN_DUMMY_SUMMON =
      40181, // Bombing Run: Summon Bombing Run Target Dummy - missing serverside cast by GO
...
struct go_fel_cannonball_stack_trap : public GameObjectAI {
  ...
  void JustSpawned() override {
    Creature *bunny = m_go->SummonCreature(NPC_BOMBING_RUN_TARGET_BUNNY, m_go->GetPositionX(), m_go->GetPositionY(),
...
GameObjectAI *GetAI_go_fel_cannonball_stack_trap(GameObject *go) { return new go_fel_cannonball_stack_trap(go); }
...
GameObjectAI *GetAI_go_dragon_head(GameObject *go) { return new go_dragon_head(go); }
...
GameObjectAI *GetAI_go_unadorned_spike(GameObject *go) { return new go_unadorned_spike(go); }

/*######################
## go_containment_coffer
######################*/

enum { NPC_RIFT_SPAWN = 6492 };
...
GameObjectAI *GetAI_go_containment(GameObject *go) { return new go_containment(go); }
