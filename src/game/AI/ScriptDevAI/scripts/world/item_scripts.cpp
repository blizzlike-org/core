...
enum { SPELL_DOMINION_SOUL = 16053, NPC_EMBERSTRIFE = 10321 };
...
enum { SPELL_ARCANE_CHARGES = 45072 };
...
enum { NPC_TH_DIRE_WOLF = 20748, SPELL_GORDREKS_OINTMENT = 32578 };
...
struct GDRPeriodicDamage : public AuraScript {
  ...
  void OnApply(Aura *aura, bool apply) const override {
    if (!apply && aura->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE) {
      ...
      aura->GetTarget()->CastCustomSpell(aura->GetTarget()->GetTarget(), SPELL_GDR_DAMAGE_HIT, &dmg, nullptr, nullptr,
                                         TRIGGERED_OLD_TRIGGERED);
    }
  }
};
