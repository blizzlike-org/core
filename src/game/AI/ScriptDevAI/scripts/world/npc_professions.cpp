...
SD%Complete: 90
SDComment: Provides learn/unlearn/relearn options for profession specializations. Not supported: Unlearn engineering,
re-learn engineering SDCategory: NPCs EndScriptData */
...
#define BOX_UNLEARN_DRAGON                                                                                             \
  "Do you really want to unlearn your Dragonscale Leatherworking specialization and lose all associated recipes?\n"
...
#define BOX_UNLEARN_ELEMENTAL                                                                                          \
  "Do you really want to unlearn your Elemental Leatherworking specialization and lose all associated recipes?\n"
...
#define BOX_UNLEARN_TRIBAL                                                                                             \
  "Do you really want to unlearn your Tribal Leatherworking specialization and lose all associated recipes?\n"
    ...
    for (int j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j) {
      ...
      if (pItem)
        if (pItem->GetProto()->RequiredSpell == reqSpell) {
          ...
          debug_log("SD2: player attempt to unlearn spell %u, but item %u is equipped.", reqSpell,
...
bool GossipHello_npc_prof_blacksmith(Player *pPlayer, Creature *pCreature) {
  ...
  if (pPlayer->HasSpell(S_WEAPON) && pPlayer->getLevel() > 49 &&
      pPlayer->GetSkillValueBase(SKILL_BLACKSMITHING) >= 250) {
    switch (eCreature) {
  ...
  if (uiAction) {
    ...
    case N_TRAINER_ARMOR2:
      pPlayer->ADD_GOSSIP_ITEM_EXTENDED(0, GOSSIP_UNLEARN_SMITH_SPEC, GOSSIP_SENDER_CHECK, uiAction,
                                        BOX_UNLEARN_ARMORORWEAPON, GetUnlearnCostLow(pPlayer), false);
    ...
    case N_TRAINER_HAMMER:
      pPlayer->ADD_GOSSIP_ITEM_EXTENDED(0, GOSSIP_UNLEARN_HAMMER, GOSSIP_SENDER_CHECK, uiAction,
                                        BOX_UNLEARN_WEAPON_SPEC, GetUnlearnCostMedium(pPlayer), false);
...
bool IsEligibleSpecializeLW(Player *pPlayer) {
  // Allow re-learn specialization as long as player is not specialized currently and has completed any specialization
  // quest
  if ((!pPlayer->HasSpell(S_DRAGON) && !pPlayer->HasSpell(S_ELEMENTAL) && !pPlayer->HasSpell(S_TRIBAL)) &&
      (pPlayer->GetQuestRewardStatus(Q_DRAGON_ALLY) || pPlayer->GetQuestRewardStatus(Q_DRAGON_HORDE) ||
       pPlayer->GetQuestRewardStatus(Q_ELEMENTAL_ALLY) || pPlayer->GetQuestRewardStatus(Q_ELEMENTAL_HORDE) ||
       pPlayer->GetQuestRewardStatus(Q_TRIBAL_ALLY) || pPlayer->GetQuestRewardStatus(Q_TRIBAL_HORDE)))
    return true;
  return false;
}
...
bool GossipHello_npc_prof_leather(Player *pPlayer, Creature *pCreature) {
  ...
  if (pPlayer->HasSkill(SKILL_LEATHERWORKING) && pPlayer->GetSkillValueBase(SKILL_LEATHERWORKING) >= 225 &&
      pPlayer->getLevel() > 39) {
    switch (eCreature) {
    case N_TRAINER_DRAGON1:
    case N_TRAINER_DRAGON2:
      if (pPlayer->HasSpell(S_DRAGON)) {
        pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_DRAGON, GOSSIP_SENDER_CHECK,
                                          GOSSIP_ACTION_INFO_DEF + 1, BOX_UNLEARN_DRAGON, GetUnlearnCostMedium(pPlayer),
                                          false);
        if (pCreature->isTrainer())
          pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, GOSSIP_TEXT_TRAIN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRAIN);
      } else if (IsEligibleSpecializeLW(pPlayer))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_DRAGON, GOSSIP_SENDER_CHECK,
                                 GOSSIP_ACTION_INFO_DEF + 2);
      break;
    case N_TRAINER_ELEMENTAL1:
    case N_TRAINER_ELEMENTAL2:
      if (pPlayer->HasSpell(S_ELEMENTAL)) {
        pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_ELEMENTAL, GOSSIP_SENDER_CHECK,
                                          GOSSIP_ACTION_INFO_DEF + 3, BOX_UNLEARN_ELEMENTAL,
                                          GetUnlearnCostMedium(pPlayer), false);
        if (pCreature->isTrainer())
          pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, GOSSIP_TEXT_TRAIN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRAIN);
      } else if (IsEligibleSpecializeLW(pPlayer))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_ELEMENTAL, GOSSIP_SENDER_CHECK,
                                 GOSSIP_ACTION_INFO_DEF + 4);
      break;
    case N_TRAINER_TRIBAL1:
    case N_TRAINER_TRIBAL2:
      if (pPlayer->HasSpell(S_TRIBAL)) {
        pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_TRIBAL, GOSSIP_SENDER_CHECK,
                                          GOSSIP_ACTION_INFO_DEF + 5, BOX_UNLEARN_TRIBAL, GetUnlearnCostMedium(pPlayer),
                                          false);
        if (pCreature->isTrainer())
          pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, GOSSIP_TEXT_TRAIN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRAIN);
      } else if (IsEligibleSpecializeLW(pPlayer))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_TRIBAL, GOSSIP_SENDER_CHECK,
...
{
pPlayer->GetPlayerMenu()->GetGossipMenu()->AddMenuItem(0,GOSSIP_LEARN_DRAGON, GOSSIP_SENDER_INFO,
