...
float DeathswornSpawnCoords[8][3] = {{-3562.689f, 696.251f, -8.158f},  {-3559.341f, 699.388f, -8.877f},
                                     {-3567.493f, 699.383f, -8.488f},  {-3556.366f, 703.439f, -9.633f},
                                     {-3569.277f, 702.603f, -8.984f},  {-3554.079f, 707.530f, -10.408f},
                                     {-3571.522f, 706.726f, -9.5219f}, {-3562.928f, 704.436f, -9.5568f}};

float ShadowlordSpawnCoords[6][3] = {{-3612.274f, 400.077f, 32.444f}, {-3591.716f, 392.962f, 32.895f},
                                     {-3601.192f, 403.606f, 32.621f}, {-3513.504f, 410.263f, 30.440f},
                                     {-3515.933f, 376.395f, 33.786f}, {-3508.507f, 392.346f, 32.418f}};
...
struct npc_xiri : public ScriptedAI {
  npc_xiri(Creature *creature) : ScriptedAI(creature) { Reset(); }
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit *sender, Unit * /*invoker*/, uint32 /*miscValue*/) override {
    ...
    if (eventType == AI_EVENT_CUSTOM_EVENTAI_A) {
      if (sender->GetEntry() == m_creature->GetEntry()) {
        ...
        if (Creature *leader = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornPackLeader[0],
                                                          DeathswornPackLeader[1], DeathswornPackLeader[2], 0.0f,
                                                          TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
          m_leaderGUID = leader->GetObjectGuid();

          if (Creature *l1 = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornLeft1[0],
                                                        DeathswornLeft1[1], DeathswornLeft1[2], 0.0f,
                                                        TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
            l1->GetMotionMaster()->MoveFollow(leader, l1->GetDistance(leader), M_PI_F / 2 + l1->GetAngle(leader), true);

            if (Creature *l2 = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornLeft2[0],
                                                          DeathswornLeft2[1], DeathswornLeft2[2], 0.0f,
                                                          TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
              l2->GetMotionMaster()->MoveFollow(l1, l2->GetDistance(l1), M_PI_F / 2 + l2->GetAngle(l1), true);

              if (Creature *l3 = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornLeft3[0],
                                                            DeathswornLeft3[1], DeathswornLeft3[2], 0.0f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true))
                l3->GetMotionMaster()->MoveFollow(l2, l3->GetDistance(l2), M_PI_F / 2 + l3->GetAngle(l2), true);
            }
          }
          if (Creature *r1 = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornRight1[0],
                                                        DeathswornRight1[1], DeathswornRight1[2], 0.0f,
                                                        TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
            r1->GetMotionMaster()->MoveFollow(leader, r1->GetDistance(leader), M_PI_F / 2 + r1->GetAngle(leader), true);

            if (Creature *r2 = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornRight2[0],
                                                          DeathswornRight2[1], DeathswornRight2[2], 0.0f,
                                                          TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
              r2->GetMotionMaster()->MoveFollow(r1, r2->GetDistance(r1), M_PI_F / 2 + r2->GetAngle(r1), true);

              if (Creature *r3 = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornRight3[0],
                                                            DeathswornRight3[1], DeathswornRight3[2], 0.0f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true))
                r3->GetMotionMaster()->MoveFollow(r2, r3->GetDistance(r2), M_PI_F / 2 + r3->GetAngle(r2), true);
            }
          }

          if (Creature *c = m_creature->SummonCreature(NPC_ASHTONGUE_DEATHSWORN, DeathswornCenter[0],
                                                       DeathswornCenter[1], DeathswornCenter[2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true))
      ...
      if (sender->GetEntry() == m_creature->GetEntry()) {
        if (Creature *leader = m_creature->SummonCreature(NPC_SHADOWLORD, ShadowlordLeader1[0], ShadowlordLeader1[1],
                                                          ShadowlordLeader1[2], 0.0f,
                                                          TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
          leader->SetWalk(true);
          leader->GetMotionMaster()->MoveWaypoint(0, 2);

          if (Creature *left =
                  m_creature->SummonCreature(NPC_SHADOWLORD, ShadowlordLeft1[0], ShadowlordLeft1[1], ShadowlordLeft1[2],
                                             0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
            left->GetMotionMaster()->MoveFollow(leader, left->GetDistance(leader), M_PI_F / 2 + left->GetAngle(leader),
                                                true);

            if (Creature *right = m_creature->SummonCreature(NPC_SHADOWLORD, ShadowlordRight1[0], ShadowlordRight1[1],
                                                             ShadowlordRight1[2], 0.0f,
                                                             TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true))
              right->GetMotionMaster()->MoveFollow(leader, right->GetDistance(leader),
                                                   M_PI_F / 2 + right->GetAngle(leader), true);
          }
        }

        if (Creature *leader = m_creature->SummonCreature(NPC_SHADOWLORD, ShadowlordLeader2[0], ShadowlordLeader2[1],
                                                          ShadowlordLeader2[2], 0.0f,
                                                          TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
          leader->SetWalk(true);
          leader->GetMotionMaster()->MoveWaypoint(1, 2);

          if (Creature *left =
                  m_creature->SummonCreature(NPC_SHADOWLORD, ShadowlordLeft2[0], ShadowlordLeft2[1], ShadowlordLeft2[2],
                                             0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true)) {
            left->GetMotionMaster()->MoveFollow(leader, left->GetDistance(leader), M_PI_F / 2 + left->GetAngle(leader),
                                                true);

            if (Creature *right = m_creature->SummonCreature(NPC_SHADOWLORD, ShadowlordRight2[0], ShadowlordRight2[1],
                                                             ShadowlordRight2[2], 0.0f,
                                                             TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000, true, true))
              right->GetMotionMaster()->MoveFollow(leader, right->GetDistance(leader),
                                                   M_PI_F / 2 + right->GetAngle(leader), true);
          }
  ...
  void SummonedMovementInform(Creature *pSummoned, uint32 /*uiMotionType*/, uint32 uiPointId) override {
    if (/*pSummoned->GetObjectGuid() == m_leaderGUID*/ pSummoned->GetEntry() == NPC_ASHTONGUE_DEATHSWORN &&
        uiPointId == 5) {
  ...
  void UpdateAI(const uint32 diff) override { ScriptedAI::UpdateAI(diff); }
};

UnitAI *GetAI_npc_xiri(Creature *pCreature) { return new npc_xiri(pCreature); }
