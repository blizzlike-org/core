...
enum {
  ...
  SPELL_CREATE_INFERNALING_SUMMONER = 33536, // summons gameobjects 183283 Infernaling Summoner Visual and 183282
                                             // Infernaling Summoner Damage Trap - triggers spell 29115 Flames
  SPELL_CREATE_INFERNALING_SUMMONER_C =
      33541, // triggered by 33536, summons trap gameobject 183278 Infernaling Summoner (go_infernaling_summoner)
...
struct boss_highlord_kruulAI : public ScriptedAI, public CombatActions {
  ...
  void SummonFormation() {
    if (Creature *hand1 =
            m_creature->SummonCreature(NPC_HAND_OF_THE_HIGHLORD, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                       m_creature->GetPositionZ(), 0.0f, TEMPSPAWN_DEAD_DESPAWN, 1800000, true)) {
  ...
  void JustSummoned(Creature *pSummoned) override {
    if (pSummoned->GetEntry() == NPC_INFERNAL_HOUND) {
      pSummoned->GetMotionMaster()->MoveRandomAroundPoint(pSummoned->GetPositionX(), pSummoned->GetPositionY(),
                                                          pSummoned->GetPositionZ(), 25.f);
    }
  }
  ...
  void JustDied(Unit * /*killer*/) override { sWorldState.HandleExternalEvent(CUSTOM_EVENT_HIGHLORD_KRUUL_DIED, 0); }

  void JustRespawned() override { DoScriptText(urand(0, 1) ? SAY_INTRO1 : SAY_INTRO2, m_creature); }
  ...
  void ExecuteActions() {
    ...
    for (uint32 i = 0; i < KRUUL_ACTION_MAX; ++i) {
      if (GetActionReadyStatus(i)) {
        ...
        case KRUUL_ACTION_TWISTED_REFLECTION: {
          if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_TWISTED_REFLECTION,
                                                                SELECT_FLAG_PLAYER)) {
        ...
        case KRUUL_ACTION_MARK_OF_KAZZAK: {
          if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MARK_OF_KAZZAK,
...
UnitAI *GetAI_boss_highlord_kruul(Creature *pCreature) { return new boss_highlord_kruulAI(pCreature); }
...
struct npc_infernal_houndAI : public ScriptedAI {
  ...
  void Reset() override { DoCastSpellIfCan(m_creature, SPELL_INFERNALING_IMMOLATION); }
};

UnitAI *GetAI_npc_infernal_hound(Creature *pCreature) { return new npc_infernal_houndAI(pCreature); }

struct go_infernaling_summoner : public GameObjectAI {
  go_infernaling_summoner(GameObject *go) : GameObjectAI(go) { m_uiSummonTimer = 15000; }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiSummonTimer <= uiDiff) {
      if (Creature *pKruul = ((ScriptedInstance *)m_go->GetMap()->GetInstanceData())
                                 ->GetSingleCreatureFromStorage(NPC_HIGHLORD_KRUUL)) {
        if (m_go->GetMap()->SpawnedCountForEntry(NPC_INFERNAL_HOUND) < MAX_INFERNAL_HOUNDS) {
          pKruul->CastSpell(pKruul, SPELL_SUMMON_INFERNALING, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                            m_go->GetObjectGuid());
...
GameObjectAI *GetAI_go_infernaling_summoner(GameObject *go) { return new go_infernaling_summoner(go); }
