...
struct npc_muglashAI : public npc_escortAI {
  ...
  void DoWaveSummon() {
    switch (m_uiWaveId) {
    case 1:
      m_creature->SummonCreature(NPC_WRATH_RIDER, m_afFirstNagaCoord[0][0], m_afFirstNagaCoord[0][1],
                                 m_afFirstNagaCoord[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_DESPAWN, 60000);
      m_creature->SummonCreature(NPC_WRATH_SORCERESS, m_afFirstNagaCoord[1][0], m_afFirstNagaCoord[1][1],
                                 m_afFirstNagaCoord[1][2], 0.0f, TEMPSPAWN_TIMED_OOC_DESPAWN, 60000);
      m_creature->SummonCreature(NPC_WRATH_RAZORTAIL, m_afFirstNagaCoord[2][0], m_afFirstNagaCoord[2][1],
                                 m_afFirstNagaCoord[2][2], 0.0f, TEMPSPAWN_TIMED_OOC_DESPAWN, 60000);
      break;
    case 2:
      m_creature->SummonCreature(NPC_WRATH_PRIESTESS, m_afSecondNagaCoord[0][0], m_afSecondNagaCoord[0][1],
                                 m_afSecondNagaCoord[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_DESPAWN, 60000);
      m_creature->SummonCreature(NPC_WRATH_MYRMIDON, m_afSecondNagaCoord[1][0], m_afSecondNagaCoord[1][1],
                                 m_afSecondNagaCoord[1][2], 0.0f, TEMPSPAWN_TIMED_OOC_DESPAWN, 60000);
      m_creature->SummonCreature(NPC_WRATH_SEAWITCH, m_afSecondNagaCoord[2][0], m_afSecondNagaCoord[2][1],
                                 m_afSecondNagaCoord[2][2], 0.0f, TEMPSPAWN_TIMED_OOC_DESPAWN, 60000);
      break;
    case 3:
      m_creature->SummonCreature(NPC_VORSHA, m_fVorshaCoord[0], m_fVorshaCoord[1], m_fVorshaCoord[2], 0.0f,
...
UnitAI *GetAI_npc_muglash(Creature *pCreature) { return new npc_muglashAI(pCreature); }
...
struct npc_ruul_snowhoofAI : public npc_escortAI {
  ...
  void Reset() override { DoCastSpellIfCan(m_creature, SPELL_RUUL_SHAPECHANGE, TRIGGERED_OLD_TRIGGERED); }

  void DoSpawnAmbush(uint8 index) {
    for (auto ambusherEntry : m_ruulAmbushers) {
      float fx, fy, fz;
      m_creature->GetRandomPoint(m_ruulAmbushCoords[index][0], m_ruulAmbushCoords[index][1],
                                 m_ruulAmbushCoords[index][2], 7.0f, fx, fy, fz);
      if (Creature *ambusher =
              m_creature->SummonCreature(ambusherEntry, fx, fy, fz, 0, TEMPSPAWN_DEAD_DESPAWN, 60 * IN_MILLISECONDS)) {
        ambusher->SetWalk(false);
        ambusher->GetMotionMaster()->MovePoint(0, m_creature->GetPositionX(), m_creature->GetPositionY(),
...
UnitAI *GetAI_npc_ruul_snowhoofAI(Creature *pCreature) { return new npc_ruul_snowhoofAI(pCreature); }
...
struct npc_torekAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 20:
      ...
      m_creature->SummonCreature(NPC_SILVERWING_SENTINEL, 1774.64f, -2049.41f, 109.83f, 1.40f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, 25000);
      m_creature->SummonCreature(NPC_SILVERWING_WARRIOR, 1778.73f, -2049.50f, 109.83f, 1.67f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, 25000);
      break;
    case 21:
...
UnitAI *GetAI_npc_torek(Creature *pCreature) { return new npc_torekAI(pCreature); }
...
static const float aEliteSummonPositions[3][4] = {
    {4243.12f, 108.22f, 38.12f, 3.62f}, {4240.95f, 114.04f, 38.35f, 3.56f}, {4235.78f, 118.09f, 38.08f, 4.12f}};
...
struct npc_feero_ironhandAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 30:
      ...
      m_creature->SummonCreature(NPC_BALIZAR_THE_UMBRAGE, aEliteSummonPositions[0][0], aEliteSummonPositions[0][1],
                                 aEliteSummonPositions[0][2], aEliteSummonPositions[0][3], TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      m_creature->SummonCreature(NPC_ALIGAR_THE_TORMENTOR, aEliteSummonPositions[1][0], aEliteSummonPositions[1][1],
                                 aEliteSummonPositions[1][2], aEliteSummonPositions[1][3], TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      m_creature->SummonCreature(NPC_CAEDAKAR_THE_VICIOUS, aEliteSummonPositions[2][0], aEliteSummonPositions[2][1],
                                 aEliteSummonPositions[2][2], aEliteSummonPositions[2][3], TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      break;
    case 31:
      // Complete the quest
...
UnitAI *GetAI_npc_feero_ironhand(Creature *pCreature) { return new npc_feero_ironhandAI(pCreature); }
...
bool QuestAccept_npc_feero_ironhand(Player *pPlayer, Creature *pCreature, const Quest *pQuest) {
  if (pQuest->GetQuestId() == QUEST_SUPPLIES_TO_AUBERDINE) {
    ...
    pCreature->SetFactionTemporary(FACTION_ESCORT_A_NEUTRAL_PASSIVE,
                                   TEMPFACTION_RESTORE_RESPAWN | TEMPFACTION_TOGGLE_IMMUNE_TO_NPC);
