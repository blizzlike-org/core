...
UnitAI *GetAI_npc_kerlonian(Creature *pCreature) { return new npc_kerlonianAI(pCreature); }
...
bool EffectDummyCreature_awaken_kerlonian(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
UnitAI *GetAI_npc_prospector_remtravel(Creature *pCreature) { return new npc_prospector_remtravelAI(pCreature); }
...
UnitAI *GetAI_npc_threshwackonator(Creature *pCreature) { return new npc_threshwackonatorAI(pCreature); }
...
struct npc_volcorAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 2:
      ...
      break;
    case 5:
      m_creature->SummonCreature(NPC_BLACKWOOD_SHAMAN, aVolcorSpawnLocs[0].m_fX, aVolcorSpawnLocs[0].m_fY,
                                 aVolcorSpawnLocs[0].m_fZ, aVolcorSpawnLocs[0].m_fO, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      m_creature->SummonCreature(NPC_BLACKWOOD_URSA, aVolcorSpawnLocs[1].m_fX, aVolcorSpawnLocs[1].m_fY,
                                 aVolcorSpawnLocs[1].m_fZ, aVolcorSpawnLocs[1].m_fO, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      break;
    case 6:
      ...
      break;
    case 11:
      m_creature->SummonCreature(NPC_BLACKWOOD_SHAMAN, aVolcorSpawnLocs[2].m_fX, aVolcorSpawnLocs[2].m_fY,
                                 aVolcorSpawnLocs[2].m_fZ, aVolcorSpawnLocs[2].m_fO, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      m_creature->SummonCreature(NPC_BLACKWOOD_URSA, aVolcorSpawnLocs[3].m_fX, aVolcorSpawnLocs[3].m_fY,
                                 aVolcorSpawnLocs[3].m_fZ, aVolcorSpawnLocs[3].m_fO, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
    case 13:
      m_creature->SummonCreature(NPC_BLACKWOOD_URSA, aVolcorSpawnLocs[4].m_fX, aVolcorSpawnLocs[4].m_fY,
                                 aVolcorSpawnLocs[4].m_fZ, aVolcorSpawnLocs[4].m_fO, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      m_creature->SummonCreature(NPC_BLACKWOOD_URSA, aVolcorSpawnLocs[5].m_fX, aVolcorSpawnLocs[5].m_fY,
                                 aVolcorSpawnLocs[5].m_fZ, aVolcorSpawnLocs[5].m_fO, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 20000);
      break;
    case 15:
...
UnitAI *GetAI_npc_volcor(Creature *pCreature) { return new npc_volcorAI(pCreature); }
...
UnitAI *GetAI_npc_therylune(Creature *pCreature) { return new npc_theryluneAI(pCreature); }
...
bool QuestAccept_npc_therylune(Player *pPlayer, Creature *pCreature, const Quest *pQuest) {
  if (pQuest->GetQuestId() == QUEST_ID_THERYLUNE_ESCAPE) {
    if (npc_theryluneAI *pEscortAI = dynamic_cast<npc_theryluneAI *>(pCreature->AI())) {
      ...
      pCreature->SetFactionTemporary(FACTION_ESCORT_A_NEUTRAL_ACTIVE,
                                     TEMPFACTION_RESTORE_RESPAWN | TEMPFACTION_TOGGLE_IMMUNE_TO_NPC);
...
struct npc_rabid_bearAI : public ScriptedAI {
  ...
  void Reset() override { m_uiRabiesTimer = urand(12000, 18000); }
...
UnitAI *GetAI_npc_rabid_bear(Creature *pCreature) { return new npc_rabid_bearAI(pCreature); }
...
struct npc_blackwood_warriorAI : public npc_corrupted_furbolgAI {
  ...
  bool UpdateFurbolgAI(const uint32 diff) {
    if (m_thunderclapTimer < diff) {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_IN_MELEE_RANGE)) {
...
UnitAI *GetAI_npc_blackwood_warrior(Creature *creature) { return new npc_blackwood_warriorAI(creature); }
...
UnitAI *GetAI_npc_blackwood_totemic(Creature *creature) { return new npc_blackwood_totemicAI(creature); }
...
bool ProcessEventId_event_purify_food(uint32 /*eventId*/, Object *source, Object *target, bool /*isStart*/) {
  if (source->GetTypeId() == TYPEID_PLAYER && target->GetTypeId() == TYPEID_GAMEOBJECT) {
    ...
    for (int8 i = 0; i < 2; ++i) {
      ...
      if (Creature *warrior =
              bowl->SummonCreature(NPC_BLACKWOOD_WARRIOR, x, y, z, 0.0f, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 120000))
...
bool GOUse_go_furbolg_food(Player *player, GameObject *go) {
  ...
  // Else spawn a new one (and make it attack)
    ...
    if (Creature *warrior =
            go->SummonCreature(NPC_BLACKWOOD_WARRIOR, x, y, z, 0.0f, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 120000))
      warrior->AI()->AttackStart(player);
  }
