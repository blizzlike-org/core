...
instance_zulfarrak::instance_zulfarrak(Map *pMap) : ScriptedInstance(pMap), m_uiPyramidEventTimer(0) { Initialize(); }

void instance_zulfarrak::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_zulfarrak::Update(uint32 uiDiff) {
  if (m_uiPyramidEventTimer) {
    if (m_uiPyramidEventTimer <= uiDiff) {
      ...
      // Move the selected troll to the top of the pyramid. Note: the algorythm may be more complicated than this, but
...
InstanceData *GetInstanceData_instance_zulfarrak(Map *pMap) { return new instance_zulfarrak(pMap); }
