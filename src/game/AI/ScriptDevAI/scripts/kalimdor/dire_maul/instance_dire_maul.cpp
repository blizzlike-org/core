...
SD%Complete: 70
SDComment: Ogre costume suit missing for Tribute Run, Cho'Rush spells randomisation is not handled properly, Warpwood
pods are not implemented, the Maul event is not handled SDCategory: Dire Maul EndScriptData */

#include "AI/ScriptDevAI/include/sc_common.h"
#include "dire_maul.h"

instance_dire_maul::instance_dire_maul(Map *pMap)
    : ScriptedInstance(pMap), m_bWallDestroyed(false), m_bDoNorthBeforeWest(false) {
  Initialize();
}

void instance_dire_maul::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_dire_maul::Update(uint32 uiDiff) {
  if (GetData(TYPE_DREADSTEED) == IN_PROGRESS) {
    ...
    if (Creature *imp = instance->GetCreature(m_npcEntryGuidStore[NPC_JEEVEE])) {
      ...
      if (bell && bell->GetGoState() != GO_STATE_ACTIVE && wheel && wheel->GetGoState() != GO_STATE_ACTIVE && candle &&
          candle->GetGoState() != GO_STATE_ACTIVE)
        SetData(TYPE_DREADSTEED, FAIL);
    }
  }
}
  ...
  case NPC_PRINCE_TORTHELDRIN:
    if (m_auiEncounter[TYPE_IMMOLTHAR] == DONE)
      pCreature->SetFactionTemporary(FACTION_HOSTILE,
                                     TEMPFACTION_RESTORE_RESPAWN | TEMPFACTION_TOGGLE_IMMUNE_TO_PLAYER);
    break;
...
void instance_dire_maul::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_IMMOLTHAR:
    if (uiData == DONE) {
      if (Creature *pPrince = GetSingleCreatureFromStorage(NPC_PRINCE_TORTHELDRIN)) {
        ...
        pPrince->SetFactionTemporary(FACTION_HOSTILE,
                                     TEMPFACTION_RESTORE_RESPAWN | TEMPFACTION_TOGGLE_IMMUNE_TO_PLAYER);
        // Despawn Chest-Aura
  ...
  case TYPE_KING_GORDOK:
    ...
    if (uiData == DONE) {
      ...
      if (Creature *pOgre = GetSingleCreatureFromStorage(NPC_CHORUSH)) {
        ...
        if (Creature *pMizzle =
                pOgre->SummonCreature(NPC_MIZZLE_THE_CRAFTY, afMizzleSpawnLoc[0], afMizzleSpawnLoc[1],
                                      afMizzleSpawnLoc[2], afMizzleSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0, true)) {
  ...
  if (uiData == DONE) {
    ...
    saveStream << m_auiEncounter[0] << " " << m_auiEncounter[1] << " " << m_auiEncounter[2] << " " << m_auiEncounter[3]
               ...
               << m_auiEncounter[17] << " " << m_auiEncounter[18];
...
void instance_dire_maul::Load(const char *chrIn) {
  ...
  loadStream >> m_auiEncounter[0] >> m_auiEncounter[1] >> m_auiEncounter[2] >> m_auiEncounter[3] >> m_auiEncounter[4] >>
      ...
      m_auiEncounter[10] >> m_auiEncounter[11] >> m_auiEncounter[12] >> m_auiEncounter[13] >> m_auiEncounter[14] >>
      m_auiEncounter[15] >> m_auiEncounter[16] >> m_auiEncounter[17] >> m_auiEncounter[18];
...
bool instance_dire_maul::CheckConditionCriteriaMeet(Player const *pPlayer, uint32 uiInstanceConditionId,
  ...
  {
    uint8 uiTributeRunAliveBosses =
        (GetData(TYPE_MOLDAR) != DONE ? 1 : 0) + (GetData(TYPE_FENGUS) != DONE ? 1 : 0) +
        (GetData(TYPE_SLIPKIK) != DONE ? 1 : 0) + (GetData(TYPE_KROMCRUSH) != DONE ? 1 : 0) +
        (GetData(TYPE_CHORUSH) != DONE ? 1 : 0) + (GetData(TYPE_STOMPER_KREEG) != DONE ? 1 : 0);

    return uiInstanceConditionId == uiTributeRunAliveBosses;
  }
  }

  script_error_log("instance_dire_maul::CheckConditionCriteriaMeet called with unsupported Id %u. Called with param "
...
GuidVector instance_dire_maul::GetRitualSymbolGuids() { return m_lRitualSymbolGUIDs; }

bool instance_dire_maul::CheckAllGeneratorsDestroyed() {
  return !(m_auiEncounter[TYPE_PYLON_1] != DONE || m_auiEncounter[TYPE_PYLON_2] != DONE ||
           m_auiEncounter[TYPE_PYLON_3] != DONE || m_auiEncounter[TYPE_PYLON_4] != DONE ||
...
void instance_dire_maul::ProcessForceFieldOpening() {
  ...
  for (GuidList::const_iterator itr = m_luiHighborneSummonerGUIDs.begin(); itr != m_luiHighborneSummonerGUIDs.end();
       ++itr) {
...
GameObjectAI *GetAI_go_fixed_trap(GameObject *go) { return new go_ai_fixed_trap(go); }

/*####################################
## Guard Slip'kik Trigger dummy effect
####################################*/

bool EffectDummyCreature_spell_guard_slip_kik(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex /* uiEffIndex */,
