...
UnitAI *GetAI_npc_gilthares(Creature *pCreature) { return new npc_giltharesAI(pCreature); }
...
bool QuestAccept_npc_gilthares(Player *pPlayer, Creature *pCreature, const Quest *pQuest) {
  if (pQuest->GetQuestId() == QUEST_FREE_FROM_HOLD) {
    pCreature->SetFactionTemporary(FACTION_ESCORT_H_NEUTRAL_ACTIVE,
...
UnitAI *GetAI_npc_taskmaster_fizzule(Creature *creature) { return new npc_taskmaster_fizzuleAI(creature); }
...
static const float aAffrayChallengerLoc[8][4] = {
    {-1683.0f, -4326.0f, 2.79f, 0.00f},   {-1682.0f, -4329.0f, 2.79f, 0.00f}, {-1683.0f, -4330.0f, 2.79f, 0.00f},
    {-1680.0f, -4334.0f, 2.79f, 1.49f},   {-1674.0f, -4326.0f, 2.79f, 3.49f}, {-1677.0f, -4334.0f, 2.79f, 1.66f},
...
struct npc_twiggy_flatheadAI : public ScriptedAI {
  ...
  void SetChallengers() {
    ...
    for (uint8 i = 0; i < MAX_CHALLENGERS; ++i)
      m_creature->SummonCreature(NPC_AFFRAY_CHALLENGER, aAffrayChallengerLoc[i][0], aAffrayChallengerLoc[i][1],
                                 aAffrayChallengerLoc[i][2], aAffrayChallengerLoc[i][3],
  ...
  void JustSummoned(Creature *pSummoned) override {
    if (pSummoned->GetEntry() == NPC_BIG_WILL) {
      ...
      pSummoned->GetMotionMaster()->MovePoint(1, aAffrayChallengerLoc[7][0], aAffrayChallengerLoc[7][1],
                                              aAffrayChallengerLoc[7][2]);
    } else {
      pSummoned->setFaction(FACTION_FRIENDLY);
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiEventTimer < uiDiff) {
      ...
      case 1:
        ...
        if (Creature *pChallenger =
                m_creature->GetMap()->GetCreature(m_vAffrayChallengerGuidsVector[m_uiChallengerCount]))
        ...
        break;
      case 2:
        m_creature->SummonCreature(NPC_BIG_WILL, aAffrayChallengerLoc[6][0], aAffrayChallengerLoc[6][1],
                                   aAffrayChallengerLoc[6][2], aAffrayChallengerLoc[6][3],
                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
        m_uiEventTimer = 15000;
        ++m_uiStep;
        break;
      default:
...
UnitAI *GetAI_npc_twiggy_flathead(Creature *pCreature) { return new npc_twiggy_flatheadAI(pCreature); }
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 18:
      if (Creature *pTemp = m_creature->SummonCreature(NPC_MERCENARY, 1128.489f, -3037.611f, 92.701f, 1.472f,
                                                       TEMPSPAWN_TIMED_OOC_DESPAWN, 120000)) {
        DoScriptText(SAY_MERCENARY, pTemp);
        m_creature->SummonCreature(NPC_MERCENARY, 1160.172f, -2980.168f, 97.313f, 3.690f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                   120000);
      }
      break;
    case 25:
...
UnitAI *GetAI_npc_wizzlecranks_shredder(Creature *pCreature) { return new npc_wizzlecranks_shredderAI(pCreature); }
...
struct npc_gallywixAI : public ScriptedAI {
  npc_gallywixAI(Creature *creature) : ScriptedAI(creature) { Reset(); }

  void Reset() override {}

  void OnStealthAlert(Unit *who) override { DoScriptText(SAY_STEALTH_ALERT_GALLYWIX, m_creature, who); }
  ...
  void DamageTaken(Unit * /*dealer*/, uint32 & /*damage*/, DamageEffectType /*damagetype*/,
    ...
    if (spellInfo && spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE &&
        spellInfo->IsFitToFamilyMask(0x0000000000800200)) // on Ambush
      m_creature->CastSpell(nullptr, SPELL_DECIMATE, TRIGGERED_OLD_TRIGGERED);
  }
};

struct npc_venture_co_lookoutAI : public ScriptedAI {
  npc_venture_co_lookoutAI(Creature *creature) : ScriptedAI(creature) { Reset(); }
  ...
  void DamageTaken(Unit * /*dealer*/, uint32 & /*damage*/, DamageEffectType /*damagetype*/,
                   SpellEntry const *spellInfo) override {
    if (spellInfo && spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE &&
        spellInfo->IsFitToFamilyMask(0x0000000000820000)) // on Eviscerate
      m_creature->CastSpell(nullptr, SPELL_SLUSH, TRIGGERED_OLD_TRIGGERED);
  }
};

struct npc_foreman_silixizAI : public ScriptedAI {
  npc_foreman_silixizAI(Creature *creature) : ScriptedAI(creature) { Reset(); }

  void Reset() override {}

  void OnStealthAlert(Unit *who) override { DoScriptText(SAY_STEALTH_ALERT_SILIXIZ, m_creature, who); }
};

UnitAI *GetAI_npc_gallywix(Creature *creature) { return new npc_gallywixAI(creature); }

UnitAI *GetAI_npc_venture_co_lookout(Creature *creature) { return new npc_venture_co_lookoutAI(creature); }

UnitAI *GetAI_npc_foreman_silixiz(Creature *creature) { return new npc_foreman_silixizAI(creature); }
...
static const SpawnPointVector spawnPointsHorde{
    {-307.23f, -1919.84f, 91.66f, 0.64f}, {-281.15f, -1906.39f, 91.66f, 1.88f}, {-270.09f, -1901.58f, 91.66f, 1.88f},
    {-275.24f, -1901.96f, 91.66f, 1.90f}, {-295.84f, -1913.58f, 91.66f, 1.86f}, {-221.61f, -1936.54f, 94.00f, 0.41f},
    {-226.65f, -1927.87f, 93.24f, 0.41f},
};

static const SpawnPointVector spawnPointsKromzar{
    {-281.19f, -1855.54f, 92.58f, 4.85f}, {-283.66f, -1858.45f, 92.47f, 4.85f}, {-286.50f, -1856.18f, 92.44f, 4.85f}};

static const SpawnPointVector spawnPointsKolkar{
    {-290.26f, -1860.85f, 92.48f, 3.68f}, {-311.46f, -1871.16f, 92.64f, 6.06f}, {-321.75f, -1868.48f, 93.73f, 0.30f},
    {-338.12f, -1852.10f, 94.09f, 0.01f}, {-311.46f, -1847.78f, 94.93f, 6.15f}, {-292.02f, -1840.00f, 93.06f, 3.51f},
    {-267.14f, -1853.97f, 93.24f, 3.52f}, {-267.68f, -1832.77f, 92.69f, 3.52f}, {-279.70f, -1845.67f, 92.81f, 3.83f},
    {-338.99f, -1868.68f, 93.50f, 0.17f}, {-210.28f, -1916.22f, 92.87f, 4.89f}, {-209.09f, -1922.44f, 93.26f, 3.87f},
    {-206.64f, -1924.83f, 93.78f, 2.41f}, {-283.57f, -1883.91f, 92.60f, 3.53f}, {-197.50f, -1929.31f, 94.03f, 3.51f}};
...
private:
  ...
  // keep track of killed creature by saving their respawntime those lists are sorted, the first one should be respawned
  // first
  ...
  // Spawn one creature if there is at least one killed in killedGuidList (DB waypoint for all spawnpoint index less or
  // equal given moveToWP argument)
  bool SpawnCreature(SpawnPointVector const &spawnPoints, SpawnPointIndexMap &creatureSlotMap,
                     FullGuidList &killedGuidList, uint32 entry1, uint32 entry2, int32 moveToWP = -1) {
    ...
    if (spawnSlot.respawnTime < m_creature->GetMap()->GetCurrentClockTime()) {
      ...
      if (Creature *creature =
              m_creature->SummonCreature(summonEntry, spawnPoint.fX, spawnPoint.fY, spawnPoint.fZ, spawnPoint.fO,
                                         TEMPSPAWN_CORPSE_TIMED_DESPAWN, 40 * IN_MILLISECONDS)) {
        // Force Horde NPCs spawned in main point to move towards kolkar invaders. We give them WP movement to ensure
        // they always move there event after evade/reset. WP are handled in DB
  ...
  // spawn all slot with given entry1/entry2 randomly and store them to SpawnPointIndexMap storage (moveTOWP is max slot
  // index that should use DB waypoint)
  void SpawnAllSlot(SpawnPointVector const &spawnPoints, SpawnPointIndexMap &creatureSlotMap, uint32 entry1,
                    uint32 entry2, int32 moveToWP = -1) {
    int32 counter = 0;
...
public:
  ...
  void UpdateAI(const uint32 uiDiff) {
    ...
    if (!m_isKromzarSpawned) {
      ...
      {
        m_creature->SummonCreature(NPC_KOLKAR_INVADER, spawnPointsKromzar[0].fX, spawnPointsKromzar[0].fY,
                                   spawnPointsKromzar[0].fZ, spawnPointsKromzar[0].fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                   40 * IN_MILLISECONDS);
        m_creature->SummonCreature(NPC_WARLORD_KROMZAR, spawnPointsKromzar[1].fX, spawnPointsKromzar[1].fY,
                                   spawnPointsKromzar[1].fZ, spawnPointsKromzar[1].fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                   40 * IN_MILLISECONDS);
        m_creature->SummonCreature(NPC_KOLKAR_STORMSEER, spawnPointsKromzar[2].fX, spawnPointsKromzar[2].fY,
                                   spawnPointsKromzar[2].fZ, spawnPointsKromzar[2].fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                   40 * IN_MILLISECONDS);
      ...
      SpawnCreature(spawnPointsHorde, m_hordeCreatureMap, m_killedHordeGuid, NPC_HORDE_DEFENDER, NPC_HORDE_AXE_THROWER,
                    5);

      // Summon kolkar if need
      SpawnCreature(spawnPointsKolkar, m_kolkarCreatureMap, m_killedKolkarGuid, NPC_KOLKAR_STORMSEER,
                    NPC_KOLKAR_INVADER);
...
UnitAI *GetAI_npc_regthar_deathgate(Creature *creature) { return new npc_regthar_deathgateAI(creature); }
...
bool GossipHello_npc_regthar_deathgate(Player *player, Creature *creature) {
  ...
  if (player->GetQuestStatus(QUEST_COUNTERATTACK) == QUEST_STATUS_INCOMPLETE) {
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Where is Warlord Krom'zar?", GOSSIP_SENDER_MAIN,
