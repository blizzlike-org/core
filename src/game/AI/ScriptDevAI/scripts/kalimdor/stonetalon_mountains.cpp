...
struct npc_kayaAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 17:
      ...
      m_creature->SummonCreature(NPC_GRIMTOTEM_RUFFIAN, -50.75f, -500.77f, -46.13f, 0.4f,
                                 TEMPSPAWN_CORPSE_TIMED_DESPAWN, 30000);
      m_creature->SummonCreature(NPC_GRIMTOTEM_BRUTE, -40.05f, -510.89f, -46.05f, 1.7f, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 30000);
      m_creature->SummonCreature(NPC_GRIMTOTEM_SORCERER, -32.21f, -499.20f, -45.35f, 2.8f,
                                 TEMPSPAWN_CORPSE_TIMED_DESPAWN, 30000);
      break;
    // Award quest credit
    case 19:
...
UnitAI *GetAI_npc_kaya(Creature *pCreature) { return new npc_kayaAI(pCreature); }
...
static const uint32 ventureCoList[5] = {NPC_VENTURE_DEFORESTER, NPC_VENTURE_LOGGER, NPC_VENTURE_DIGGER,
                                        NPC_VENTURE_OVERLORD, NPC_VENTURE_OPERATOR};

struct go_covertopsAI : public GameObjectAI {
  go_covertopsAI(GameObject *go) : GameObjectAI(go) { Reset(); }
  uint32 m_phase;
  ...
  void JustSpawned() override { Reset(); }
...
GameObjectAI *GetAIgo_covertops(GameObject *pGo) { return new go_covertopsAI(pGo); }
