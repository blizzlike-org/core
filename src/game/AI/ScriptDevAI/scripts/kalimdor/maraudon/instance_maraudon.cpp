...
void instance_maraudon::Initialize() { memset(&m_encounter, 0, sizeof(m_encounter)); }
    ...
    if (m_spewLarvaTimer <= uiDiff) {
      if (GameObject *pGo = GetSingleGameObjectFromStorage(GO_LARVA_SPEWER))
        pGo->SummonCreature(NPC_SPEWED_LARVA, spawnLocation.m_fX, spawnLocation.m_fY, spawnLocation.m_fZ,
                            ...
                            10 * MINUTE * IN_MILLISECONDS); // Despawn is handled by DBScript we just want to avoid an
                                                            // infinite number of larvas near Noxxion
...
InstanceData *GetInstanceData_instance_maraudon(Map *map) { return new instance_maraudon(map); }
...
GameObjectAI *GetAI_go_larva_spewer(GameObject *go) { return new go_ai_larva_spewer(go); }
