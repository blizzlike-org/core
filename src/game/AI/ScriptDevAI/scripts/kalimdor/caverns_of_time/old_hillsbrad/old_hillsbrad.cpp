...
SD%Complete: 90
SDComment: Quest support: 10283, 10284. All friendly NPC's. Thrall escort event is complete, possible a few details are
still missing. SDCategory: Caverns of Time, Old Hillsbrad Foothills EndScriptData */
...
bool GossipHello_npc_erozion(Player *pPlayer, Creature *pCreature) {
  ...
  if (pInstance && pInstance->GetData(TYPE_BARREL_DIVERSION) != DONE && !pPlayer->HasItemCount(ITEM_ENTRY_BOMBS, 1))
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_NEED_BOMBS, GOSSIP_SENDER_MAIN,
                                GOSSIP_ACTION_INFO_DEF + 1);

  // Need info, should have option to teleport or not
  /*if (!pPlayer->GetQuestRewardStatus(QUEST_ENTRY_RETURN) && pPlayer->GetQuestStatus(QUEST_ENTRY_RETURN) ==
     QUEST_STATUS_COMPLETE) pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "[PH] Teleport please, i'm tired.",
...
struct npc_thrall_old_hillsbradAI : public npc_escortAI, private DialogueHelper {
  npc_thrall_old_hillsbradAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aThrallDialogue) {
  ...
  void JustDied(Unit * /*pKiller*/) override {
    ...
    for (GuidList::const_iterator itr = m_lTarrenMillSoldiersGuids.begin(); itr != m_lTarrenMillSoldiersGuids.end();
         ++itr) {
      if (Creature *pTemp = m_creature->GetMap()->GetCreature(*itr))
        pTemp->ForcedDespawn();
    }
  ...
  void CorpseRemoved(uint32 &uiRespawnDelay) override {
    ...
    if (m_pInstance &&
        (m_pInstance->GetData(TYPE_THRALL_EVENT) == DONE || m_pInstance->GetData(TYPE_THRALL_EVENT) == FAIL))
  ...
  void JustRespawned() override {
    ...
    if (m_pInstance->GetData(TYPE_THRALL_EVENT) == IN_PROGRESS) {
      ...
      if (m_pInstance->GetData(TYPE_SKARLOC) != DONE) {
        ...
        // resummon the mount
        m_creature->SummonCreature(NPC_SKARLOC_MOUNT, 2047.775f, 253.4088f, 62.91183f, 5.37f, TEMPSPAWN_DEAD_DESPAWN,
                                   0);
  ...
  void JustSummoned(Creature *pSummoned) override {
    ...
    case NPC_INFINITE_SLAYER:
      ...
      if (!m_bHasEpochYelled) {
        switch (urand(0, 3)) {
        case 0:
          DoScriptText(SAY_INFINITE_DRAGON_AGGRO_1, pSummoned);
          break;
        case 1:
          DoScriptText(SAY_INFINITE_DRAGON_AGGRO_2, pSummoned);
          break;
        case 2:
          DoScriptText(SAY_INFINITE_DRAGON_AGGRO_3, pSummoned);
          break;
        case 3:
          DoScriptText(SAY_INFINITE_DRAGON_AGGRO_4, pSummoned);
          break;
        }
    ...
    case NPC_IMAGE_OF_ERONZION:
      if (m_pInstance)
        DoScriptText(m_pInstance->GetThrallEventCount() < MAX_WIPE_COUNTER ? SAY_ERONZION_RESET_THRALL
                                                                           : SAY_ERONZION_RESET_LAST,
                     pSummoned);
  ...
  void SummonedCreatureJustDied(Creature *pSummoned) override {
    ...
    case NPC_INFINITE_SLAYER:
      ...
      if (m_lTarrenMillSoldiersGuids.empty()) {
        ...
        case 1:
          m_creature->SummonCreature(NPC_INFINITE_DEFILER, 2595.477f, 684.3738f, 55.95534f, 6.05f,
                                     TEMPSPAWN_DEAD_DESPAWN, 0);
          m_creature->SummonCreature(NPC_INFINITE_SABOTEOR, 2602.208f, 678.2955f, 56.34682f, 6.07f,
                                     TEMPSPAWN_DEAD_DESPAWN, 0);
          m_creature->SummonCreature(NPC_INFINITE_SLAYER, 2602.8f, 686.2845f, 55.79315f, 5.95f, TEMPSPAWN_DEAD_DESPAWN,
                                     0);
          ++m_uiEpochWaveId;
          break;
        case 2:
          m_creature->SummonCreature(NPC_INFINITE_DEFILER, 2646.289f, 718.5257f, 57.90024f, 4.32f,
                                     TEMPSPAWN_DEAD_DESPAWN, 0);
          m_creature->SummonCreature(NPC_INFINITE_SABOTEOR, 2641.788f, 719.7106f, 57.4023f, 4.46f,
                                     TEMPSPAWN_DEAD_DESPAWN, 0);
          m_creature->SummonCreature(NPC_INFINITE_SLAYER, 2645.725f, 709.7153f, 56.69411f, 4.38f,
                                     TEMPSPAWN_DEAD_DESPAWN, 0);
          m_creature->SummonCreature(NPC_INFINITE_SLAYER, 2639.641f, 710.5246f, 56.23582f, 4.60f,
                                     TEMPSPAWN_DEAD_DESPAWN, 0);
          ++m_uiEpochWaveId;
          break;
        case 3:
  ...
  void WaypointReached(uint32 uiPoint) override {
    ...
    case 21:
      ...
      m_creature->SummonCreature(NPC_VETERAN, 2190.508f, 140.4597f, 88.21628f, 6.04f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      m_creature->SummonCreature(NPC_VETERAN, 2189.543f, 139.0996f, 88.23965f, 0.21f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      break;
    case 24:
      ...
      m_creature->SummonCreature(NPC_VETERAN, 2149.212f, 107.2005f, 74.15676f, 1.71f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
    ...
    case 27:
      ...
      m_creature->SummonCreature(NPC_VETERAN, 2140.146f, 169.2364f, 66.30494f, 2.49f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      break;
    case 29:
      ...
      m_creature->SummonCreature(NPC_VETERAN, 2108.486f, 189.9346f, 66.30494f, 2.68f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      m_creature->SummonCreature(NPC_VETERAN, 2112.387f, 195.4947f, 66.30494f, 2.39f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      break;
    ...
    case 68:
      m_creature->SummonCreature(NPC_SKARLOC_MOUNT, 2488.779f, 623.9724f, 58.07383f, 1.37f, TEMPSPAWN_TIMED_DESPAWN,
                                 30000);
      m_creature->Unmount();
      m_bHadMount = false;
      break;
    ...
    case 98:
      ...
      break;
    case 99:
      m_creature->SummonCreature(NPC_CHURCH_PROTECTOR, 2627.88f, 657.63f, 55.98f, 4.28f,
                                 TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 5000);
      m_creature->SummonCreature(NPC_CHURCH_LOOKOUT, 2627.27f, 655.17f, 56.03f, 4.50f,
                                 TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 5000);
      m_creature->SummonCreature(NPC_CHURCH_LOOKOUT, 2629.21f, 654.81f, 56.04f, 4.38f,
                                 TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 5000);
      m_creature->SummonCreature(NPC_CHURCH_GUARDSMAN, 2629.98f, 656.96f, 55.96f, 4.34f,
                                 TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 5000);
      break;
    case 100:
    ...
    case 111:
      m_creature->SummonCreature(NPC_TARREN_MILL_PROTECTOR, 2652.71f, 660.31f, 61.93f, 1.67f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, 5000);
      m_creature->SummonCreature(NPC_TARREN_MILL_LOOKOUT, 2648.96f, 662.59f, 61.93f, 0.79f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      m_creature->SummonCreature(NPC_TARREN_MILL_LOOKOUT, 2657.36f, 662.34f, 61.93f, 2.68f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                 5000);
      m_creature->SummonCreature(NPC_TARREN_MILL_GUARDSMAN, 2656.39f, 659.77f, 61.93f, 2.61f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, 5000);
      SetRun(false);
      break;
    ...
    case 123:
      // begin fight
      m_lTarrenMillSoldiersGuids.clear();
      if (Creature *pGuardsman = m_creature->SummonCreature(NPC_TARREN_MILL_GUARDSMAN, 2629.452f, 716.2737f, 56.55614f,
                                                            4.73f, TEMPSPAWN_DEAD_DESPAWN, 0))
        pGuardsman->GetMotionMaster()->MoveWaypoint(1);

      if (Creature *pLookout = m_creature->SummonCreature(NPC_TARREN_MILL_LOOKOUT, 2639.85f, 717.0549f, 56.36302f,
                                                          4.49f, TEMPSPAWN_DEAD_DESPAWN, 0))
        pLookout->GetMotionMaster()->MoveWaypoint(1);

      if (Creature *pProtector = m_creature->SummonCreature(NPC_TARREN_MILL_PROTECTOR, 2655.716f, 698.5595f, 57.72154f,
                                                            3.17f, TEMPSPAWN_DEAD_DESPAWN, 0))
  ...
  void UpdateAI(const uint32 uiDiff) override { UpdateEscortAI(uiDiff); }
...
UnitAI *GetAI_npc_thrall_old_hillsbrad(Creature *pCreature) { return new npc_thrall_old_hillsbradAI(pCreature); }
...
bool GossipHello_npc_thrall_old_hillsbrad(Player *pPlayer, Creature *pCreature) {
  ...
  if (instance_old_hillsbrad *pInstance = (instance_old_hillsbrad *)pCreature->GetInstanceData()) {
    ...
    // Escort - barn to inn
      ...
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_TARREN_1, GOSSIP_SENDER_MAIN,
    ...
    // Escort - after Skarloc is defeated
      ...
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_SKARLOC_1, GOSSIP_SENDER_MAIN,
...
bool GossipSelect_npc_thrall_old_hillsbrad(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/, uint32 uiAction) {
  ...
  case GOSSIP_ACTION_INFO_DEF + 2: {
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_SKARLOC_2, GOSSIP_SENDER_MAIN,
                                GOSSIP_ACTION_INFO_DEF + 20);
    pPlayer->SEND_GOSSIP_MENU(TEXT_ID_SKARLOC_2, pCreature->GetObjectGuid());
    break;
  }
  case GOSSIP_ACTION_INFO_DEF + 20: {
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_SKARLOC_3, GOSSIP_SENDER_MAIN,
...
enum {
  ...
  GOSSIP_ITEM_EPOCH_2 =
      -3560006,           // "We'll get you out. Taretha. Don't worry. I doubt the wizard would wander too far away."
...
struct npc_tarethaAI : public npc_escortAI, private DialogueHelper {
  npc_tarethaAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aTarethaDialogue) {
    m_pInstance = (instance_old_hillsbrad *)pCreature->GetInstanceData();
    InitializeDialogueHelper(m_pInstance);
    Reset();
  }
  ...
  void Reset() override { m_bHasStartedEpilogue = false; }
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case SAY_TR_THEN_WHO:
      m_creature->SummonCreature(NPC_EROZION, 2646.47f, 680.416f, 55.38f, 4.16f, TEMPSPAWN_TIMED_DESPAWN,
...
UnitAI *GetAI_npc_taretha(Creature *pCreature) { return new npc_tarethaAI(pCreature); }
