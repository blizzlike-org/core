...
instance_old_hillsbrad::instance_old_hillsbrad(Map *pMap)
    : ScriptedInstance(pMap), m_uiBarrelCount(0), m_uiThrallEventCount(0), m_uiThrallResetTimer(0) {
  Initialize();
}

void instance_old_hillsbrad::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_old_hillsbrad::OnPlayerEnter(Player *pPlayer) {
  ...
  pPlayer->SummonCreature(NPC_DRAKE, aDrakeSummonLoc[0], aDrakeSummonLoc[1], aDrakeSummonLoc[2], aDrakeSummonLoc[3],
...
void instance_old_hillsbrad::OnCreatureDeath(Creature *pCreature) {
  switch (pCreature->GetEntry()) {
  case NPC_DRAKE:
    SetData(TYPE_DRAKE, DONE);
    break;
  case NPC_SKARLOC:
    SetData(TYPE_SKARLOC, DONE);
    break;
  case NPC_EPOCH:
    SetData(TYPE_EPOCH, DONE);
    break;
  }
}
...
void instance_old_hillsbrad::OnCreatureEnterCombat(Creature *pCreature) {
  ...
  case NPC_DRAKE:
    ...
    break;
  case NPC_SKARLOC:
    SetData(TYPE_SKARLOC, IN_PROGRESS);
    break;
  case NPC_EPOCH:
    SetData(TYPE_EPOCH, IN_PROGRESS);
    break;
  }
}
...
void instance_old_hillsbrad::OnCreatureEvade(Creature *pCreature) {
  switch (pCreature->GetEntry()) {
  case NPC_DRAKE:
    SetData(TYPE_DRAKE, FAIL);
    break;
  case NPC_SKARLOC:
    SetData(TYPE_SKARLOC, FAIL);
    break;
  case NPC_EPOCH:
    SetData(TYPE_EPOCH, FAIL);
    break;
  }
}
    ...
    if (uiData == IN_PROGRESS) {
      ...
      if (m_uiBarrelCount == MAX_BARRELS) {
        ...
        if (Player *pPlayer = GetPlayerInMap()) {
          pPlayer->SummonCreature(NPC_DRAKE, aDrakeSummonLoc[0], aDrakeSummonLoc[1], aDrakeSummonLoc[2],
                                  aDrakeSummonLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0);
          ...
          float fX, fY, fZ;
          for (GuidList::const_iterator itr = m_lRightPrisonersList.begin(); itr != m_lRightPrisonersList.end();
               ++itr) {
  ...
  case TYPE_THRALL_EVENT:
    ...
    if (uiData == FAIL) {
      ...
      // If we already respawned Thrall too many times, the event is failed for good
        ...
        debug_log("SD2: Instance Old Hillsbrad: Thrall event failed %u times. Reset instance required.",
                  m_uiThrallEventCount);
    }
    break;
  case TYPE_DRAKE:
...
InstanceData *GetInstanceData_instance_old_hillsbrad(Map *pMap) { return new instance_old_hillsbrad(pMap); }

bool ProcessEventId_event_go_barrel_old_hillsbrad(uint32 /*uiEventId*/, Object *pSource, Object *pTarget,
                                                  bool bIsStart) {
  if (bIsStart && pSource->GetTypeId() == TYPEID_PLAYER) {
    if (instance_old_hillsbrad *pInstance = (instance_old_hillsbrad *)((Player *)pSource)->GetInstanceData()) {
      if (pInstance->GetData(TYPE_BARREL_DIVERSION) == DONE ||
          (GameObject *)pTarget->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_NO_INTERACT))
        return true;
