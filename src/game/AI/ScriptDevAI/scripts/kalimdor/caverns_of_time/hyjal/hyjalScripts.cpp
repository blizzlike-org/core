  ...
  if (instance_mount_hyjal *instance = (instance_mount_hyjal *)creature->GetInstanceData()) {
    if (hyjalAI *jainaAI = dynamic_cast<hyjalAI *>(creature->AI())) {
      ...
      case 101:
        ...
        break;
      case 102:
        if (instance->GetData(TYPE_AZGALOR) != DONE) // Jaina has the same gossip menu when spawned in orc base, but
                                                     // nothing should happen when selecting her gossip menu options
        {
          creature->SetActiveObjectState(true); // Set active object to prevent issues if players go out of range after
                                                // talking to her (could lead to ores not spawning)
          instance->StartEvent(JAINA_WIN);
          creature->RemoveFlag(
              UNIT_NPC_FLAGS,
              UNIT_NPC_FLAG_GOSSIP); // ToDo: Jaina should not lose gossip flag, but also should not stop movement when
                                     // a player opens her gossip window. Currently we have no way to achieve this.
        }
...
bool GossipSelect_npc_thrall(Player *player, Creature *creature, uint32 /*uiSender*/, uint32 action) {
  if (instance_mount_hyjal *instance = (instance_mount_hyjal *)creature->GetInstanceData()) {
    if (hyjalAI *thrallAI = dynamic_cast<hyjalAI *>(creature->AI())) {
      ...
      case 102:
        creature->SetActiveObjectState(true); // Set active object to prevent issues if players go out of range after
                                              // talking to him (could lead to ores not spawning)
        instance->StartEvent(THRALL_WIN);
        creature->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP); // ToDo: Thrall should not lose gossip flag, but
                                                                    // nothing should happen when choosing his options
        player->CLOSE_GOSSIP_MENU();
        break;
      }
    }
...
struct npc_building_triggerAI : public ScriptedAI {
  ...
  void Reset() override { m_creature->AI()->SetReactState(REACT_PASSIVE); }
  ...
  void MoveInLineOfSight(Unit *who) override {
    ...
    if (m_creature->IsWithinDistInMap(who, 35.0f)) {
      ...
      if (who->GetEntry() == NPC_GHOUL) {
        ...
        who->GetMotionMaster()->MovePoint(1, m_creature->GetPositionX(), m_creature->GetPositionY(),
...
struct RaiseDeadHyjal : public SpellScript {
  SpellCastResult OnCheckCast(Spell *spell, bool strict) const override {
    if (strict) {
      ...
      return spell->CheckScriptTargeting(EFFECT_INDEX_0, 1, radius, TARGET_LOCATION_SCRIPT_NEAR_CASTER, tempUnitList,
                                         tempGOList);
    }
