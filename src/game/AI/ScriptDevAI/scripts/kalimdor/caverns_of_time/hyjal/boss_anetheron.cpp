...
struct boss_anetheronAI : public CombatAI {
  boss_anetheronAI(Creature *creature)
      : CombatAI(creature, ANETHERON_ACTION_MAX),
  ...
  uint32 GetSubsequentActionTimer(const uint32 action) const {
    ...
    case ANETHERON_ACTION_SLEEP:
      return urand(35000, 48000);
    case ANETHERON_ACTION_INFERNO:
      return urand(50000, 55000);
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ANETHERON_ACTION_CARRION_SWARM: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CARRION_SWARM, SELECT_FLAG_PLAYER)) {
    ...
    case ANETHERON_ACTION_INFERNO: {
      ...
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_INFERNO, SELECT_FLAG_PLAYER))
