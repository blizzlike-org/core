...
instance_mount_hyjal::instance_mount_hyjal(Map *pMap)
    : ScriptedInstance(pMap), m_hyjalOverheadEnable(0), m_hyjalEnemyCount(0), m_hyjalWaves(0), m_invasionWaves(0),
      m_hyjalWavesWorldstate(0), m_nextWaveTimer(0), m_nextInvasionWaveTimer(0), m_retreatTimer(0), m_infernalsTimer(0),
      m_failTimer(0), m_startCheck(false) {
  Initialize();
}
...
std::array<InvasionWave, MAX_INVASION_WAVES> InitInvasionWaves() {
  ...
  // Alliance Invasion
  // Wave 0: Building Triggers (These creatures do not have any creation packets in sniffs (they only appear as targets
  // of Ghouls), so coordinates are guesswork based on the Ghouls position)
...
void instance_mount_hyjal::Initialize() { memset(&m_encounter, 0, sizeof(m_encounter)); }
...
void instance_mount_hyjal::OnCreatureCreate(Creature *creature) {
  ...
  case NPC_STALK: {
    ...
    if (spawner && spawner.GetEntry() != NPC_HYJAL_DESPAWN_TRIGGER &&
        m_hyjalOverheadEnable) // Overrun mobs are not counted
  ...
  case NPC_INFERNAL_RELAY:
    ...
    std::sort(m_infernalRelays.begin(), m_infernalRelays.end(),
              [](ObjectGuid const &a, ObjectGuid const &b) -> bool { return a.GetCounter() < b.GetCounter(); });
    break;
  case NPC_INFERNAL_TARGET:
    m_infernalTargets.push_back(creature->GetObjectGuid());
    std::sort(m_infernalTargets.begin(), m_infernalTargets.end(),
              [](ObjectGuid const &a, ObjectGuid const &b) -> bool { return a.GetCounter() < b.GetCounter(); });
    break;
  }
}
...
void instance_mount_hyjal::OnCreatureRespawn(Creature *creature) {
  ...
  case NPC_INFERNAL_TARGET:
    ...
    creature->SetCanEnterCombat(false); // on retail they enter combat, likely for some guardian purposes for the
                                        // infernals - on our end they bug out friendlies
    break;
...
void instance_mount_hyjal::OnCreatureDeath(Creature *creature) {
  ...
  case NPC_WINTERCHILL:
    ...
    m_waveSpawns.erase(std::remove(m_waveSpawns.begin(), m_waveSpawns.end(), creature->GetObjectGuid()),
                       m_waveSpawns.end());
    break;
  case NPC_ANETHERON: {
...
void instance_mount_hyjal::FillInitialWorldStates(ByteBuffer &data, uint32 &count, uint32 /*zoneId*/,
                                                  uint32 /*areaId*/) {
...
void instance_mount_hyjal::DoUpdateWorldState(uint32 stateId, uint32 stateData) {
  switch (stateId) {
  case WORLD_STATE_MOUNT_HYJAL_ENABLE:
    if (m_hyjalOverheadEnable == stateData)
      return;
    m_hyjalOverheadEnable = stateData;
    break;
  case WORLD_STATE_MOUNT_HYJAL_ENEMYCOUNT:
    if (m_hyjalEnemyCount == stateData)
      return;
    m_hyjalEnemyCount = stateData;
    break;
  case WORLD_STATE_MOUNT_HYJAL_WAVES:
    if (m_hyjalWavesWorldstate == stateData)
      return;
    m_hyjalWavesWorldstate = stateData;
    break;
  }
...
void instance_mount_hyjal::SpawnArchimonde() {
  ...
  if (Player *pPlayer = GetPlayerInMap())
    pPlayer->SummonCreature(NPC_ARCHIMONDE, aArchimondeSpawnLoc[0], aArchimondeSpawnLoc[1], aArchimondeSpawnLoc[2],
                            aArchimondeSpawnLoc[3], TEMPSPAWN_MANUAL_DESPAWN, 0);
}
...
void instance_mount_hyjal::SpawnWave(uint32 index, bool setTimer) {
  ...
  for (HyjalWaveMob &mob : m_hyjalWavesData[index].waveMobs) {
    Creature *waveSpawn =
        spawner->SummonCreature(mob.mobEntry, mob.x, mob.y, mob.z, mob.ori, TEMPSPAWN_DEAD_DESPAWN, 0, true,
                                GetRunningPerWaveMobEntry(mob.mobEntry, index), GetRandomWavePath(mob.path));
    if (index == 0) // The very first wave should never grant XP, loot or reputation. ToDo: According to a wowhead
                    // comment there should be a cooldown period after you've killed one boss after which the first wave
                    // of the next boss also won't grant any XP, loot or reputation
    {
...
void instance_mount_hyjal::SpawnInvasionWave(uint32 index, bool setTimer) {
  ...
  if (Creature *spawner = GetSingleCreatureFromStorage(NPC_HYJAL_DESPAWN_TRIGGER)) {
    for (HyjalWaveMob &mob : m_invasionWavesData[index].waveMobs) {
      TempSpawnSettings settings(spawner, mob.mobEntry, mob.x, mob.y, mob.z, mob.ori, TEMPSPAWN_DEAD_DESPAWN, 0, true,
                                 true, GetRandomWavePath(mob.path));
...
void instance_mount_hyjal::RetreatBase(BaseArea index) {
  ...
  case BASE_HORDE: {
    if (Creature *thrall = GetSingleCreatureFromStorage(NPC_THRALL)) {
      ...
      thrall->SummonCreature(NPC_JAINA, 5486.305f, -2777.72f, 1496.687f, 1.082104f, TEMPSPAWN_TIMED_OR_DEAD_DESPAWN,
                             120000, true, false, 2);
      thrall->SummonCreature(NPC_ALLIANCE_KNIGHT, 5489.709f, -2781.63f, 1497.728f, 1.32645f,
                             TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 120000);
      thrall->SummonCreature(NPC_ALLIANCE_KNIGHT, 5480.393f, -2778.908f, 1497.069f, 1.047198f,
                             TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 120000);
    }
    break;
  }
...
void instance_mount_hyjal::ShowChatCommands(ChatHandler *handler) {
  handler->SendSysMessage("This instance supports the following commands:\n despawnbase (0,1,2), spawnbase (0,1,2), "
                          "spawnwave (0-35), spawnarchimonde, retreatbase (0,1), spawnoverrun (0,1,2), despawnoverrun "
                          "(0,1,2), despawnwavespawns, spawninfernals (count)");
}
...
void instance_mount_hyjal::ExecuteChatCommand(ChatHandler *handler, char *args) {
  ...
  if (val == "despawnbase") {
    ...
    if (baseId > MAX_BASE - 1) {
      handler->PSendSysMessage("Could not despawn base %u because it doesn't exist. Please specify a value between 0-2",
                               baseId);
      return;
    }
    ...
    if (baseId > MAX_BASE - 1) {
      handler->PSendSysMessage("Could not spawn base %u because it doesn't exist. Please specify a value between 0-2",
                               baseId);
      return;
    }
    ...
    if (waveId > MAX_WAVES - 1) {
      handler->PSendSysMessage("Could not spawn wave %u because it doesn't exist. Please specify a value between 0-35",
                               waveId);
      return;
    }

    if (!GetSingleCreatureFromStorage(waveId > 17 ? NPC_THRALL : NPC_JAINA)) {
      handler->PSendSysMessage("Could not spawn wave %u because %s is despawned.", waveId,
                               waveId > 17 ? "Thrall" : "Jaina");
      return;
    }
    ...
    if (baseId > MAX_BASE - 2) {
      handler->PSendSysMessage("Could not retreat base %u because it doesn't exist. Please specify a value between 0-1",
                               baseId);
      return;
    }
    ...
    if (baseId > MAX_BASE - 1) {
      handler->PSendSysMessage(
          "Could not spawn overrun waves in base %u because it doesn't exist. Please specify a value between 0-2",
          baseId);
      return;
    }
    ...
    if (baseId > MAX_BASE - 1) {
      handler->PSendSysMessage(
          "Could not despawn overrun waves in base %u because it doesn't exist. Please specify a value between 0-2",
          baseId);
      return;
    }
...
bool instance_mount_hyjal::CheckConditionCriteriaMeet(Player const *player, uint32 instanceConditionId,
                                                      WorldObject const *conditionSource,
  ...
  case INSTANCE_CONDITION_ID_ANETHERON_STARTABLE:
    return GetData(TYPE_WINTERCHILL) == DONE &&
           (GetData(TYPE_ANETHERON) == NOT_STARTED || GetData(TYPE_ANETHERON) == FAIL) && m_waveSpawns.size() <= 0;
  case INSTANCE_CONDITION_ID_ANETHERON_DONE:
    return GetData(TYPE_ANETHERON) == DONE;
  case INSTANCE_CONDITION_ID_KAZROGAL_STARTABLE:
    return GetData(TYPE_ANETHERON) == DONE &&
           (GetData(TYPE_KAZROGAL) == NOT_STARTED || GetData(TYPE_KAZROGAL) == FAIL) && m_waveSpawns.size() <= 0;
  ...
  script_error_log("instance_mount_hyjal::CheckConditionCriteriaMeet called with unsupported Id %u. Called with param "
...
InstanceData *GetInstanceData_instance_mount_hyjal(Map *map) { return new instance_mount_hyjal(map); }
