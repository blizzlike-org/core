...
hyjalAI::hyjalAI(Creature *pCreature, uint32 maxActions)
    : CombatAI(pCreature, maxActions), m_instance(static_cast<instance_mount_hyjal *>(pCreature->GetInstanceData())) {
...
void hyjalAI::DoTalk(YellType pYellType) {
  ...
  if (pYell) {
    if (pYell->uiCreatureEntry == NPC_THRALL &&
        pYell->m_pYellType == DEATH) // Thrall doesn't have a text on Death, only sound
...
void hyjalAI::Retreat() { DoTalk(RETREAT); }

void hyjalAI::JustDied(Unit * /*killer*/) { DoTalk(DEATH); }
