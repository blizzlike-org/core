...
struct boss_rage_winterchillAI : public CombatAI {
  boss_rage_winterchillAI(Creature *creature)
      : CombatAI(creature, WINTERCHILL_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case WINTERCHILL_ACTION_ICEBOLT: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_ICEBOLT, SELECT_FLAG_PLAYER))
    ...
    case WINTERCHILL_ACTION_FROST_NOVA: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_FROST_NOVA, SELECT_FLAG_PLAYER)) {
    ...
    case WINTERCHILL_ACTION_DEATH_AND_DECAY: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DEATH_AND_DECAY,
                                                           SELECT_FLAG_PLAYER)) {
