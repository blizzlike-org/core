...
struct boss_archimondeAI : public CombatAI {
  boss_archimondeAI(Creature *creature)
      : CombatAI(creature, ARCHIMONDE_ACTION_MAX),
  ...
  void JustRespawned() override { m_drainNordrassilTimer = 5000; }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ARCHIMONDE_ACTION_FINGER_OF_DEATH_COOLUP:
      if (Unit *closest =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, nullptr, SELECT_FLAG_PLAYER)) {
    ...
    case ARCHIMONDE_ACTION_FINGER_OF_DEATH:
      if (Unit *closest =
        ...
        if (!m_creature->CanReachWithMeleeAttack(closest)) {
          ...
          SetActionReadyStatus(action, false); // if someone is tanking again, disable
      }
      return;
    case ARCHIMONDE_ACTION_SOUL_CHARGE:
      uint32 spellId = 0;
  ...
  void HandleOutro(const uint32 diff) {
    ...
    if (m_stageCounter) {
      ...
      {
      case 0:
        spellId = SPELL_FINGER_DEATH_SCRIPT;
        break;
      case 1:
        spellId = SPELL_FINGER_DEATH_DUMMY;
        break;
      default:
        spellId = urand(0, 2) ? SPELL_FINGER_DEATH_SCRIPT : SPELL_FINGER_DEATH_DUMMY;
        break;
      }
      if (DoCastSpellIfCan(nullptr, spellId) == CAST_OK)
        ++m_stageCounter;
  ...
  void CheckLeash(const uint32 diff) {
    if (m_leashCheckTimer) {
      if (m_leashCheckTimer <= diff) {
        ...
        // Range stuff here
        if (m_creature->GetDistance2d(pos.GetPositionX(), pos.GetPositionY()) >= 155.f ||
            (pos.GetPositionX() < 5534.752f && pos.GetPositionX() > 5381.371f && pos.GetPositionY() < -3507.099f &&
             pos.GetPositionY() > -3587.244f)) {
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_drainNordrassilTimer) {
      if (m_drainNordrassilTimer <= diff) {
        m_creature->RemoveAurasDueToSpell(
            SPELL_SOUL_CHARGE_YELLOW_CHARGE); // workaround for archi not despawning after wipe
...
/* This is the script for the Doomfire Spirit Mob. This mob controls the doomfire npc and allows it to move randomly
 * around the map. */
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiChangeTargetTimer < uiDiff) {
      float nextOrientation =
          MapManager::NormalizeOrientation(m_creature->GetOrientation() + irand(-1, 1) * m_turnConstant);
      Position pos;
      m_creature->GetFirstCollisionPosition(pos, 8.f,
                                            nextOrientation); // both orientation and distance verified with sniffs
