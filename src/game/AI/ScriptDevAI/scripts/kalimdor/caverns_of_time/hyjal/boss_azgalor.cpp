...
struct boss_azgalorAI : public CombatAI {
  boss_azgalorAI(Creature *creature)
      : CombatAI(creature, AZGALOR_ACTION_MAX),
  ...
  uint32 GetSubsequentActionTimer(const uint32 action) const {
    switch (action) {
    case AZGALOR_ACTION_CLEAVE:
      return urand(8000, 16000);
    case AZGALOR_RAIN_OF_FIRE:
      return 15000;
    case AZGALOR_HOWL_OF_AZGALOR:
      return urand(18000, 20000);
    case AZGALOR_DOOM:
      return urand(45000, 55000);
    case AZGALOR_ENRAGE:
      return 300000;
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case AZGALOR_RAIN_OF_FIRE: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_RAIN_OF_FIRE,
                                                SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_AOE_RANGE, m_rainOfFireParams))
