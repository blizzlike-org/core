...
struct boss_chrono_lord_dejaAI : public ScriptedAI {
  ...
  void JustDied(Unit * /*pVictim*/) override { DoScriptText(SAY_DEATH, m_creature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiArcaneBlastTimer < uiDiff) {
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_bIsRegularMode ? SPELL_ARCANE_BLAST : SPELL_ARCANE_BLAST_H) ==
          CAST_OK)
...
UnitAI *GetAI_boss_chrono_lord_deja(Creature *pCreature) { return new boss_chrono_lord_dejaAI(pCreature); }
