...
UnitAI *GetAI_npc_medivh_black_morass(Creature *pCreature) { return new npc_medivh_black_morassAI(pCreature); }

bool EffectDummyCreature_npc_medivh_black_morass(Unit * /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                                 Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/) {
  // always check spellid and effectindex
  if ((uiSpellId == SPELL_CORRUPT && uiEffIndex == EFFECT_INDEX_0) ||
      (uiSpellId == SPELL_CORRUPT_AEONUS && uiEffIndex == EFFECT_INDEX_0)) {
...
enum {
  ...
  SPELL_QUIET_SUICIDE_UNUSED = 3617, // Currently unused. Should be used by Medivh on self when shield runs out, 3 sec
                                     // after he says his death text
};
...
static const RiftWaveData aPortalWaves[] = {{{NPC_ASSASSIN, NPC_WHELP, NPC_CHRONOMANCER, 0}},
...
struct npc_time_riftAI : public ScriptedAI {
  ...
  void DoSummon() {
    ...
    if (m_bIsFirstSummon) {
      ...
      case 6:
        uiSpellId = (m_pInstance->GetData(TYPE_CHRONO_LORD) == DONE && !m_bIsRegularMode) ? SPELL_INFINITE_CHRONO_LORD
                                                                                          : SPELL_CHRONO_LORD_DEJA;
        break;
      case 12:
        uiSpellId = (m_pInstance->GetData(TYPE_TEMPORUS) == DONE && !m_bIsRegularMode) ? SPELL_INFINITE_TIMEREAVER
                                                                                       : SPELL_TEMPORUS;
        break;
      case 18:
      ...
      default:
        switch (urand(0, 3)) {
        case 0:
          uiSpellId = SPELL_RIFT_LORD;
          break;
        case 1:
          uiSpellId = SPELL_RIFT_LORD_2;
          break;
        case 2:
          uiSpellId = SPELL_RIFT_KEEPER;
          break;
        case 3:
          uiSpellId = SPELL_RIFT_KEEPER_2;
          break;
        }
        break;
      }
...
UnitAI *GetAI_npc_time_rift(Creature *pCreature) { return new npc_time_riftAI(pCreature); }

bool EffectDummyCreature_npc_time_rift_channel(Unit * /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                               Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/) {
  // always check spellid and effectindex
