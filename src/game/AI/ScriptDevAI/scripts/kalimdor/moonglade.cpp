...
struct npc_clintar_dw_spiritAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 i) override {
    ...
    case 28:
      ...
      break;
    case 32:
      m_creature->SummonCreature(NPC_ASPECT_OF_RAVEN, 7465.321f, -3088.515f, 429.006f, 5.550f,
                                 TEMPSPAWN_TIMED_OOC_DESPAWN, 10000);
      break;
    case 36:
      m_creature->HandleEmote(EMOTE_STATE_USESTANDING_NOSHEATHE);
      break;
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(urand(0, 1) ? SAY_AGGRO_1 : SAY_AGGRO_2, m_creature); }
...
UnitAI *GetAI_npc_clintar_dw_spirit(Creature *pCreature) { return new npc_clintar_dw_spiritAI(pCreature); }

// we expect this spell to be triggered from spell casted at questAccept
bool EffectDummyCreature_npc_clintar_dw_spirit(Unit *pCaster, uint32 spellId, SpellEffectIndex effIndex,
...
enum {
  ...
  NPC_TYRANDE_WHISPERWIND =
      15633, // appears with the priestess during the event to help the players - should cast healing spells
...
static EventLocations aTyrandeLocations[] = {
    // Tyrande should appear along the pathway, but because of the missing pathfinding we'll summon here closer to
    // Eranikus
...
static EventLocations aShadowsLocations[] = {
    ...
    // Outside shade points - basically only the first set of coords is used for the summoning; there is no solid proof
    // of using the other coords
...
struct npc_keeper_remulosAI : public npc_escortAI, private DialogueHelper {
  npc_keeper_remulosAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aIntroDialogue) { Reset(); }
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case NPC_ERANIKUS_TYRANT:
      ...
      if (Creature *pEranikus = m_creature->GetMap()->GetCreature(m_eranikusGuid))
        pEranikus->GetMotionMaster()->MovePoint(POINT_ID_ERANIKUS_FLIGHT, aEranikusLocations[1].m_fX,
    ...
    if (m_uiOutroTimer) {
      if (m_uiOutroTimer <= uiDiff) {
        ...
        case 0:
          ...
          break;
        case 1:
          // Despawn Remulos after the outro is finished - he will respawn automatically at his home position after a
          // few min
      ...
      if (m_uiShadesummonTimer <= uiDiff) {
        ...
        if (m_bIsFirstWave) {
          ...
          for (uint8 i = 0; i < MAX_SHADOWS; ++i)
            m_creature->SummonCreature(NPC_NIGHTMARE_PHANTASM, aShadowsLocations[i].m_fX, aShadowsLocations[i].m_fY,
                                       aShadowsLocations[i].m_fZ, 0, TEMPSPAWN_DEAD_DESPAWN, 0);
        ...
        if (m_uiSummonCount < MAX_SUMMON_TURNS) {
          for (uint8 i = 0; i < MAX_SHADOWS; ++i) {
            m_creature->GetRandomPoint(aShadowsLocations[uiSummonPoint].m_fX, aShadowsLocations[uiSummonPoint].m_fY,
                                       aShadowsLocations[uiSummonPoint].m_fZ, 10.0f, fX, fY, fZ);
        ...
        if (m_uiSummonCount == MAX_SUMMON_TURNS) {
          ...
          if (Creature *pEranikus = m_creature->GetMap()->GetCreature(m_eranikusGuid)) {
            ...
            pEranikus->GetMotionMaster()->MovePoint(POINT_ID_ERANIKUS_COMBAT, aEranikusLocations[2].m_fX,
                                                    aEranikusLocations[2].m_fY, aEranikusLocations[2].m_fZ);
          }
        } else
...
UnitAI *GetAI_npc_keeper_remulos(Creature *pCreature) { return new npc_keeper_remulosAI(pCreature); }
...
bool EffectDummyCreature_conjure_rift(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                      Creature * /*pCreatureTarget*/, ObjectGuid /*originalCasterGuid*/) {
  // always check spellid and effectindex
  if (uiSpellId == SPELL_CONJURE_RIFT && uiEffIndex == EFFECT_INDEX_0) {
    pCaster->SummonCreature(NPC_ERANIKUS_TYRANT, aEranikusLocations[0].m_fX, aEranikusLocations[0].m_fY,
                            aEranikusLocations[0].m_fZ, aEranikusLocations[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
...
struct boss_eranikusAI : public ScriptedAI {
  ...
  void EnterEvadeMode() override {
    if (m_creature->GetHealthPercent() < 20.0f) {
      ...
      // There may be a core issue related to the reached home function for summoned creatures so we are cleaning things
      // up here if the creature evades while the event is in progress then we despawn all the summoned, including
      // himself
  ...
  void DoSummonHealers() {
    ...
    for (uint8 j = 0; j < MAX_PRIESTESS; ++j) {
      m_creature->GetRandomPoint(aTyrandeLocations[0].m_fX, aTyrandeLocations[0].m_fY, aTyrandeLocations[0].m_fZ, 10.0f,
                                 fX, fY, fZ);
  ...
  void JustSummoned(Creature *pSummoned) override {
    ...
    case NPC_TYRANDE_WHISPERWIND:
      ...
      pSummoned->GetMotionMaster()->MovePoint(POINT_ID_TYRANDE_HEAL, aTyrandeLocations[1].m_fX,
    ...
    case NPC_ELUNE_PRIESTESS:
      ...
      float fX, fY, fZ;
      pSummoned->SetWalk(false);
      m_creature->GetRandomPoint(aTyrandeLocations[1].m_fX, aTyrandeLocations[1].m_fY, aTyrandeLocations[1].m_fZ, 10.0f,
                                 fX, fY, fZ);
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiEventTimer) {
      if (m_uiEventTimer <= uiDiff) {
        ...
        case 3:
          ...
          m_creature->GetMotionMaster()->MovePoint(POINT_ID_ERANIKUS_REDEEMED, aEranikusLocations[3].m_fX,
                                                   aEranikusLocations[3].m_fY, aEranikusLocations[3].m_fZ);
          m_uiEventTimer = 0;
          break;
        case 4:
    ...
    if (m_uiTyrandeMoveTimer) {
      if (m_uiTyrandeMoveTimer <= uiDiff) {
        if (Creature *pTyrande = m_creature->GetMap()->GetCreature(m_tyrandeGuid))
          pTyrande->GetMotionMaster()->MovePoint(POINT_ID_TYRANDE_ABSOLUTION, aTyrandeLocations[2].m_fX,
                                                 aTyrandeLocations[2].m_fY, aTyrandeLocations[2].m_fZ);
        m_uiTyrandeMoveTimer = 0;
      } else
    ...
    if (m_creature->GetHealthPercent() < m_uiHealthCheck) {
      ...
      case 85:
        ...
        if (Creature *pTyrande = m_creature->SummonCreature(NPC_TYRANDE_WHISPERWIND, aTyrandeLocations[0].m_fX,
                                                            aTyrandeLocations[0].m_fY, aTyrandeLocations[0].m_fZ, 0,
                                                            TEMPSPAWN_TIMED_DESPAWN, 1000))
      ...
      case 50:
        ...
        m_creature->SummonCreature(NPC_TYRANDE_WHISPERWIND, aTyrandeLocations[0].m_fX, aTyrandeLocations[0].m_fY,
                                   aTyrandeLocations[0].m_fZ, 0, TEMPSPAWN_CORPSE_DESPAWN, 0);
        m_uiHealthCheck = 35;
        break;
      case 35:
...
UnitAI *GetAI_boss_eranikus(Creature *pCreature) { return new boss_eranikusAI(pCreature); }
