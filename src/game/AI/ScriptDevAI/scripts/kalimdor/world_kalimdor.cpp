...
struct world_map_kalimdor : public ScriptedMap {
  ...
  bool CheckConditionCriteriaMeet(Player const *player, uint32 instanceConditionId, WorldObject const *conditionSource,
    ...
    script_error_log("world_map_kalimdor::CheckConditionCriteriaMeet called with unsupported Id %u. Called with param "
      ...
      if (pCreature->IsTemporarySummon()) // Only count the ones summoned for Murkdeep quest
      {
        ...
        if (m_uiMurkdeepAdds_KilledAddCount == 3) {
          ...
          for (uint8 i = 0; i < 2; ++i) {
            ...
            if (Creature *pTemp =
                    pCreature->SummonCreature(NPC_GREYMIST_WARRIOR, fX, fY, fZ,
                                              aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
              pTemp->SetWalk(false);
      ...
      if (pCreature->IsTemporarySummon()) // Only count the ones summoned for Murkdeep quest
      {
        ...
        if (m_uiMurkdeepAdds_KilledAddCount == 2) {
          ...
          for (uint8 i = 0; i < 2; ++i) {
            ...
            if (Creature *pTemp =
                    pCreature->SummonCreature(!i ? NPC_MURKDEEP : NPC_GREYMIST_HUNTER, fX, fY, fZ,
                                              aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
              pTemp->SetWalk(false);
              pTemp->GetRandomPoint(aSpawnLocations[POS_IDX_MURKDEEP_MOVE][0],
                                    aSpawnLocations[POS_IDX_MURKDEEP_MOVE][1],
  ...
  bool GhostOPlasmEventStep(GhostOPlasmEvent &eventData) {
    if (eventData.despawnTimer > 180000) {
      ...
      if (GameObject *go = instance->GetGameObject(eventData.guid))
        go->AddObjectToRemoveList(); // TODO: Establish rules for despawning temporary GOs that were used in their
                                     // lifetime (buttons for example)
    ...
    if (GameObject *go = instance->GetGameObject(eventData.guid)) {
      if (eventData.despawnTimer / 15000 >= eventData.phaseCounter) {
        float x, y, z;
        go->GetPosition(x, y, z); // do some urand radius shenanigans to spawn it further and make it walk to go using
                                  // doing X and Y yourself and using function in MAP to get proper Z
        uint32 random = urand(0, 35);
        float xR = x + random, yR = y + (40 - random), zR = z;
        instance->GetHeightInRange(xR, yR, zR);
        Creature *creature = go->SummonCreature(NPC_MAGRAMI_SPECTRE, xR, yR, zR, 0, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
    ...
    case TYPE_OMEN: {
      ...
      case NOT_STARTED:
        ...
        if (m_uiRocketsCounter < MAX_ROCKETS) {
          // 25% chance of spawning Minions of Omen (guessed), only if not already spawned (encounter is set to FAIL in
          // that case)
        ...
        // Set the event in motion and notify the GO to summon Omen
                 ...
                 GetData(TYPE_OMEN) != DONE) // Check that Omen is not already spawned and event is reset
      ...
      case SPECIAL:
        if (GameObject *pRocketCluster = GetSingleGameObjectFromStorage(GO_ROCKET_CLUSTER)) {
          for (uint8 i = POS_IDX_MINION_OMEN_START; i <= POS_IDX_MINION_OMEN_STOP; i++)
            pRocketCluster->SummonCreature(NPC_MINION_OMEN, aSpawnLocations[i][0], aSpawnLocations[i][1],
                                           aSpawnLocations[i][2], aSpawnLocations[i][3],
      ...
      case DONE:
        ...
        m_uiOmenResetTimer =
            5 * MINUTE * IN_MILLISECONDS; // Prevent another summoning of Omen for 5 minutes (based on spell duration)
        break;
      }
    }
    case TYPE_TETHYR: {
...
InstanceData *GetInstanceData_world_map_kalimdor(Map *pMap) { return new world_map_kalimdor(pMap); }
