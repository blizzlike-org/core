...
instance_onyxias_lair::instance_onyxias_lair(Map *pMap) : ScriptedInstance(pMap) { Initialize(); }
...
void instance_onyxias_lair::OnObjectCreate(GameObject *pGo) {
  ...
  case GO_WHELP_SPAWNER:
    if (Creature *pTrigger = GetSingleCreatureFromStorage(NPC_ONYXIA_TRIGGER)) {
      pTrigger->SummonCreature(NPC_ONYXIA_WHELP, pGo->GetPositionX(), pGo->GetPositionY(), pGo->GetPositionZ(), 0.0f,
...
InstanceData *GetInstanceData_instance_onyxias_lair(Map *pMap) { return new instance_onyxias_lair(pMap); }
