...
instance_razorfen_downs::instance_razorfen_downs(Map *pMap) : ScriptedInstance(pMap), m_uiWaveCounter(0) {
  Initialize();
}

void instance_razorfen_downs::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_razorfen_downs::DoSpawnWaveIfCan(GameObject *pGo) {
  ...
  for (uint8 i = 0; i < aWaveSummonInformation[m_uiWaveCounter].m_uiNPCperWave; ++i) {
    ...
    pGo->GetRandomPoint(aSpawnLocations[uiPos].m_fX, aSpawnLocations[uiPos].m_fY, aSpawnLocations[uiPos].m_fZ, 5.0f,
                        fPosX, fPosY, fPosZ);

    // move the summoned NPC toward the gong
    if (Creature *pSummoned = pGo->SummonCreature(aWaveSummonInformation[m_uiWaveCounter].m_uiNpcEntry, fPosX, fPosY,
                                                  fPosZ, aSpawnLocations[uiPos].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
