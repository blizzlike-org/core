...
enum {
  ...
  SPELL_SURRENDER =
      39889, // should be triggered by periodic check, if player comes in certain distance with quest incomplete
...
struct npc_rizzle_sprysprocketAI : public npc_escortAI {
  ...
  void MoveInLineOfSight(Unit *pUnit) override {
    if (HasEscortState(STATE_ESCORT_ESCORTING) && pUnit->GetTypeId() == TYPEID_PLAYER) {
      if (!HasEscortState(STATE_ESCORT_PAUSED) && m_creature->IsWithinDistInMap(pUnit, INTERACTION_DISTANCE) &&
...
UnitAI *GetAI_npc_rizzle_sprysprocket(Creature *pCreature) { return new npc_rizzle_sprysprocketAI(pCreature); }
...
bool GossipSelect_npc_rizzle_sprysprocket(Player *pPlayer, Creature * /*pCreature*/, uint32 /*uiSender*/,
                                          uint32 uiAction) {
...
UnitAI *GetAI_npc_depth_charge(Creature *pCreature) { return new npc_depth_chargeAI(pCreature); }
...
struct mobs_spitelashesAI : public ScriptedAI {
  ...
  void SpellHit(Unit *pCaster, const SpellEntry *pSpell) override {
    ...
    if (pCaster->GetTypeId() == TYPEID_PLAYER &&
        ((Player *)pCaster)->GetQuestStatus(QUEST_FRAGMENTED_MAGIC) == QUEST_STATUS_INCOMPLETE &&
  ...
  bool CanUseSpecialAbility(uint32 uiIndex) {
    ...
    case TARGET_TYPE_RANDOM:
      pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, m_aSpitelashAbility[uiIndex].m_uiSpellId,
                                                  SELECT_FLAG_IN_LOS);
      break;
...
UnitAI *GetAI_mobs_spitelashes(Creature *pCreature) { return new mobs_spitelashesAI(pCreature); }
...
bool GossipSelect_npc_loramus_thalipedes(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/, uint32 uiAction) {
  ...
  case GOSSIP_ACTION_INFO_DEF + 23:
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "I will do this with or your help, Loramus", GOSSIP_SENDER_MAIN,
...
UnitAI *GetAI_npc_felhound_tracker(Creature *pCreature) { return new npc_felhound_trackerAI(pCreature); }
...
bool ProcessEventId_arcanite_buoy(uint32 uiEventId, Object *pSource, Object * /*pTarget*/, bool /*bIsStart*/) {
  if (uiEventId == EVENT_ARCANITE_BUOY) {
    if (pSource->GetTypeId() == TYPEID_PLAYER) {
      ...
      // Summon Maws
      if (Creature *maws = ((Player *)pSource)
                               ->SummonCreature(NPC_MAWS, aMawsSpawn.m_fX, aMawsSpawn.m_fY, aMawsSpawn.m_fZ, 1.6f,
...
GameObjectAI *GetAI_go_lightning(GameObject *go) { return new go_ai_lightning(go); }
...
struct boss_mawsAI : public ScriptedAI {
  boss_mawsAI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    // Despawn timer handled in UpdateAI because creature must perform actions on despawn that cannot be done in
    // OnDespawn() (creature is already despawned)
...
UnitAI *GetAI_boss_maws(Creature *pCreature) { return new boss_mawsAI(pCreature); }
