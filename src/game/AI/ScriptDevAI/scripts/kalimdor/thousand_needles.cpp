...
struct npc_kanatiAI : public npc_escortAI {
  ...
  void DoSpawnGalak() {
    for (int i = 0; i < 3; ++i)
      m_creature->SummonCreature(NPC_GALAK_ASS, m_afGalakLoc[0], m_afGalakLoc[1], m_afGalakLoc[2], 0.0f,
...
UnitAI *GetAI_npc_kanati(Creature *pCreature) { return new npc_kanatiAI(pCreature); }
...
struct npc_lakota_windsongAI : public npc_escortAI {
  ...
  void DoSpawnBandits(int uiAmbushId) {
    for (int i = 0; i < 2; ++i)
      m_creature->SummonCreature(NPC_GRIM_BANDIT, m_afBanditLoc[i + uiAmbushId][0], m_afBanditLoc[i + uiAmbushId][1],
                                 m_afBanditLoc[i + uiAmbushId][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
  }

  void JustSummoned(Creature *pSummoned) override { pSummoned->AI()->AttackStart(m_creature); }
};

UnitAI *GetAI_npc_lakota_windsong(Creature *pCreature) { return new npc_lakota_windsongAI(pCreature); }
...
struct npc_paoka_swiftmountainAI : public npc_escortAI {
  ...
  void DoSpawnWyvern() {
    for (auto &i : m_afWyvernLoc)
      m_creature->SummonCreature(NPC_WYVERN, i[0], i[1], i[2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
  }
};

UnitAI *GetAI_npc_paoka_swiftmountain(Creature *pCreature) { return new npc_paoka_swiftmountainAI(pCreature); }
...
enum { FACTION_FRIENDLY = 35, QUEST_SCOOP = 1950, SPELL_PLUCKY_HUMAN = 9192, SPELL_PLUCKY_CHICKEN = 9220 };

#define GOSSIP_ITEM_QUEST "Please tell me the Phrase.."

struct npc_plucky_johnsonAI : public ScriptedAI {
  npc_plucky_johnsonAI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
  ...
  void ReceiveEmote(Player *pPlayer, uint32 uiTextEmote) override {
    if (pPlayer->GetQuestStatus(QUEST_SCOOP) == QUEST_STATUS_INCOMPLETE) {
      if (uiTextEmote == TEXTEMOTE_BECKON) {
        m_creature->SetFactionTemporary(FACTION_FRIENDLY,
...
UnitAI *GetAI_npc_plucky_johnson(Creature *pCreature) { return new npc_plucky_johnsonAI(pCreature); }
