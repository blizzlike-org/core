...
struct mob_aquementasAI : public ScriptedAI {
  ...
  void SendItem(Player *pReceiver) {
    if (pReceiver->HasItemCount(ITEM_BOOK_AQUOR, 1) && pReceiver->HasItemCount(ITEM_SILVERY_CLAWS, 11) &&
        pReceiver->HasItemCount(ITEM_IRONTREE_HEART, 1) && !pReceiver->HasItemCount(ITEM_SILVER_TOTEM, 1)) {
...
UnitAI *GetAI_mob_aquementas(Creature *pCreature) { return new mob_aquementasAI(pCreature); }
...
struct npc_custodian_of_timeAI : public npc_escortAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    ...
    if (pWho->GetTypeId() == TYPEID_PLAYER) {
      if (pWho->HasAura(SPELL_CUSTODIAN_OF_TIME) &&
          ((Player *)pWho)->GetQuestStatus(QUEST_ID_CAVERNS_OF_TIME) == QUEST_STATUS_INCOMPLETE) {
        float fRadius = 10.0f;
...
UnitAI *GetAI_npc_custodian_of_time(Creature *pCreature) { return new npc_custodian_of_timeAI(pCreature); }
...
struct npc_oox17tnAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 i) override {
    ...
    case 23:
      ...
      m_creature->SummonCreature(NPC_SCORPION, -8340.70f, -4448.17f, 9.17f, 3.10f, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 30000);
      m_creature->SummonCreature(NPC_SCORPION, -8343.18f, -4444.35f, 9.44f, 2.35f, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 30000);
      m_creature->SummonCreature(NPC_SCORPION, -8348.70f, -4457.80f, 9.58f, 2.02f, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 30000);
      break;
    ...
    case 29:
      ...
      m_creature->SummonCreature(NPC_SCOFFLAW, -7488.02f, -4786.56f, 10.67f, 3.74f, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 10000);
      m_creature->SummonCreature(NPC_SHADOW_MAGE, -7486.41f, -4791.55f, 10.54f, 3.26f, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 30000);

      if (Creature *pCreature = m_creature->SummonCreature(NPC_SCOFFLAW, -7488.47f, -4800.77f, 9.77f, 2.50f,
                                                           TEMPSPAWN_CORPSE_TIMED_DESPAWN, 30000))
...
UnitAI *GetAI_npc_oox17tn(Creature *pCreature) { return new npc_oox17tnAI(pCreature); }
...
bool GossipSelect_npc_stone_watcher_of_norgannon(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/,
                                                 uint32 uiAction) {
  switch (uiAction) {
...
struct npc_toogaAI : public FollowerAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    ...
    if (!m_creature->GetVictim() && !HasFollowState(STATE_FOLLOW_COMPLETE | STATE_FOLLOW_POSTEVENT) &&
      ...
      if (HasFollowState(STATE_FOLLOW_POSTEVENT)) {
        if (m_uiPostEventTimer < uiDiff) {
          ...
          case 6:
            ...
            m_creature->GetMotionMaster()->MovePoint(POINT_ID_TO_WATER, m_afToWaterLoc[0], m_afToWaterLoc[1],
                                                     m_afToWaterLoc[2]);
            break;
          }
...
UnitAI *GetAI_npc_tooga(Creature *pCreature) { return new npc_toogaAI(pCreature); }
