...
struct npc_ranshallaAI : public npc_escortAI, private DialogueHelper {
  npc_ranshallaAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aIntroDialogue) { Reset(); }
  ...
  void SpellHit(Unit * /*pCaster*/, const SpellEntry *pSpell) override {
    // If Ranshalla cast the torch lightening spell for too long she will trigger SPELL_RANSHALLA_DESPAWN (quest failed
    // and despawn)
  ...
  void DoSummonPriestess() {
    ...
    if (Creature *pPriestess = m_creature->SummonCreature(NPC_PRIESTESS_ELUNE, aWingThicketLocations[0].m_fX,
                                                          aWingThicketLocations[0].m_fY, aWingThicketLocations[0].m_fZ,
                                                          aWingThicketLocations[0].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0)) {
      pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[3].m_fX, aWingThicketLocations[3].m_fY,
                                               aWingThicketLocations[3].m_fZ);
      m_firstPriestessGuid = pPriestess->GetObjectGuid();
    }
    if (Creature *pPriestess = m_creature->SummonCreature(NPC_PRIESTESS_ELUNE, aWingThicketLocations[1].m_fX,
                                                          aWingThicketLocations[1].m_fY, aWingThicketLocations[1].m_fZ,
                                                          aWingThicketLocations[1].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0)) {
      // Left priestess should have a distinct move point because she is the one who starts the dialogue at point reach
      pPriestess->GetMotionMaster()->MovePoint(1, aWingThicketLocations[4].m_fX, aWingThicketLocations[4].m_fY,
                                               aWingThicketLocations[4].m_fZ);
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case SAY_PRIESTESS_ALTAR_9:
      ...
      if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_firstPriestessGuid))
        pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[6].m_fX, aWingThicketLocations[6].m_fY,
                                                 aWingThicketLocations[6].m_fZ);
      break;
    case SAY_PRIESTESS_ALTAR_13:
      // summon the Guardian of Elune
      if (Creature *pGuard = m_creature->SummonCreature(NPC_GUARDIAN_ELUNE, aWingThicketLocations[2].m_fX,
                                                        aWingThicketLocations[2].m_fY, aWingThicketLocations[2].m_fZ,
                                                        aWingThicketLocations[2].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0)) {
        pGuard->GetMotionMaster()->MovePoint(0, aWingThicketLocations[5].m_fX, aWingThicketLocations[5].m_fY,
                                             aWingThicketLocations[5].m_fZ);
        m_guardEluneGuid = pGuard->GetObjectGuid();
      }
      // summon the Voice of Elune
      if (GameObject *pAltar = m_creature->GetMap()->GetGameObject(m_altarGuid)) {
        if (Creature *pVoice =
                m_creature->SummonCreature(NPC_VOICE_ELUNE, pAltar->GetPositionX(), pAltar->GetPositionY(),
                                           pAltar->GetPositionZ(), 0, TEMPSPAWN_TIMED_DESPAWN, 30000))
    ...
    case SAY_VOICE_ALTAR_15:
      ...
      if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_secondPriestessGuid)) {
        ...
        pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[7].m_fX, aWingThicketLocations[7].m_fY,
                                                 aWingThicketLocations[7].m_fZ);
      }
    ...
    case SAY_PRIESTESS_ALTAR_19:
      ...
      if (Creature *pGuard = m_creature->GetMap()->GetCreature(m_guardEluneGuid)) {
        pGuard->GetMotionMaster()->MovePoint(0, aWingThicketLocations[2].m_fX, aWingThicketLocations[2].m_fY,
    ...
    case SAY_PRIESTESS_ALTAR_20:
      ...
      if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_firstPriestessGuid)) {
        pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[0].m_fX, aWingThicketLocations[0].m_fY,
                                                 aWingThicketLocations[0].m_fZ);
    ...
    case SAY_PRIESTESS_ALTAR_21:
      ...
      if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_secondPriestessGuid)) {
        pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[1].m_fX, aWingThicketLocations[1].m_fY,
                                                 aWingThicketLocations[1].m_fZ);
        pPriestess->ForcedDespawn(4000);
      }
      break;
    case DATA_EVENT_END:
  ...
  void UpdateEscortAI(const uint32 uiDiff) override {
    ...
    if (m_uiDelayTimer) {
      if (m_uiDelayTimer <= uiDiff) {
        ...
        // We need to manually set the movement manager to the next waypoint in order to reset the timer unless the NPC
        // will wait 3 min before moving again
...
UnitAI *GetAI_npc_ranshalla(Creature *pCreature) { return new npc_ranshallaAI(pCreature); }
...
UnitAI *GetAI_npc_artorius(Creature *pCreature) { return new npc_artoriusAI(pCreature); }
