...
const uint32 possibleSpawns[31] = {17322, 17661, 17496, 17522, 17340, 17352, 17333, 17524, 17654, 17348,
                                   17339, 17345, 17353, 17336, 17550, 17330, 17701, 17321, 17325, 17320,
...
struct mob_webbed_creatureAI : public Scripted_NoMovementAI {
  ...
  void JustDied(Unit *pKiller) override {
    ...
    if (uiSpawnCreatureEntry)
      if (Creature *spawnedCreature = m_creature->SummonCreature(
              uiSpawnCreatureEntry, 0.0f, 0.0f, 0.0f, m_creature->GetOrientation(), TEMPSPAWN_TIMED_OOC_DESPAWN, 25000))
        spawnedCreature->GetMotionMaster()->MoveRandomAroundPoint(
            spawnedCreature->GetPositionX(), spawnedCreature->GetPositionY(), spawnedCreature->GetPositionZ(),
            5.f); // Needed because one or more of the spawned creatures has MovementType 2 by default, causing them to
                  // go flying to the first default waypoint
  }
};

UnitAI *GetAI_mob_webbed_creature(Creature *pCreature) { return new mob_webbed_creatureAI(pCreature); }
...
struct npc_demolitionist_legosoAI : public npc_escortAI, private DialogueHelper {
  npc_demolitionist_legosoAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aLegosoDialogue) {
  ...
  void DoRespawnFires(bool bFirstEvent) {
    ...
    if (lFiresInRange.empty()) {
      script_error_log("Bloodmyst Isle: ERROR Failed to find any gameobjects of entry %u and %u in range.",
                       GO_COIL_FIRE_L, GO_COIL_FIRE_S);
      return;
    }
...
UnitAI *GetAI_npc_demolitionist_legoso(Creature *pCreature) { return new npc_demolitionist_legosoAI(pCreature); }
