...
instance_ruins_of_ahnqiraj::instance_ruins_of_ahnqiraj(Map *pMap)
    : ScriptedInstance(pMap), m_uiArmyDelayTimer(0), m_uiCurrentArmyWave(0) {
  Initialize();
}

void instance_ruins_of_ahnqiraj::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_ruins_of_ahnqiraj::OnCreatureDeath(Creature *pCreature) {
  ...
  case NPC_SWARMGUARD_NEEDLER: {
    ...
    if (m_sArmyWavesGuids[m_uiCurrentArmyWave - 1].find(pCreature->GetObjectGuid()) !=
        m_sArmyWavesGuids[m_uiCurrentArmyWave - 1].end()) {
...
void instance_ruins_of_ahnqiraj::DoSpawnAndorovIfCan() {
  ...
  for (const auto &aAndorovSpawnLoc : aAndorovSpawnLocs)
    pPlayer->SummonCreature(aAndorovSpawnLoc.m_uiEntry, aAndorovSpawnLoc.m_fX, aAndorovSpawnLoc.m_fY,
                            aAndorovSpawnLoc.m_fZ, aAndorovSpawnLoc.m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
}
...
void instance_ruins_of_ahnqiraj::DoSortArmyWaves() {
  ...
  for (uint8 i = 0; i < MAX_ARMY_WAVES; ++i) {
    ...
    if (Creature *pTemp = GetSingleCreatureFromStorage(aArmySortingParameters[i].m_uiEntry)) {
      ...
      GetCreatureListWithEntryInGrid(lCreatureList, pTemp, NPC_SWARMGUARD_NEEDLER,
                                     aArmySortingParameters[i].m_fSearchDist);
  ...
  if (m_uiCurrentArmyWave == MAX_ARMY_WAVES) {
    ...
    for (auto itr : m_sArmyWavesGuids[m_uiCurrentArmyWave]) {
      if (Creature *pTemp = instance->GetCreature(itr)) {
        ...
        pTemp->SetWalk(false);
        pTemp->GetRandomPoint(aAndorovMoveLocs[4].m_fX, aAndorovMoveLocs[4].m_fY, aAndorovMoveLocs[4].m_fZ, 10.0f, fX,
                              fY, fZ);
