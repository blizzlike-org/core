...
struct boss_moamAI : public CombatAI {
  boss_moamAI(Creature *creature) : CombatAI(creature, MOAM_ACTION_MAX), m_uiPhase(0) {
    ...
    AddCustomAction(MOAM_CANCEL_ENERGIZE, true, [&]() { m_creature->RemoveAurasDueToSpell(SPELL_ENERGIZE); });
  ...
  void Aggro(Unit * /*who*/) override { DoScriptText(EMOTE_AGGRO, m_creature); }
