...
static const DialogueEntry aIntroDialogue[] = {
    {SAY_ANDOROV_INTRO_1, NPC_GENERAL_ANDOROV, 7000},   {SAY_ANDOROV_INTRO_2, NPC_GENERAL_ANDOROV, 0},
    {SAY_ANDOROV_INTRO_3, NPC_GENERAL_ANDOROV, 4000},   {SAY_ANDOROV_INTRO_4, NPC_GENERAL_ANDOROV, 6000},
    {SAY_ANDOROV_ATTACK_START, NPC_GENERAL_ANDOROV, 0}, {0, 0, 0},
};
...
struct npc_general_andorovAI : public CombatAI, private DialogueHelper {
  npc_general_andorovAI(Creature *creature)
      : CombatAI(creature, ANDOROV_ACTION_MAX), DialogueHelper(aIntroDialogue),
  ...
  void JustRespawned() override { ResetTimer(ANDOROV_MOVE, 5000); }
  ...
  void MovementInform(uint32 uiType, uint32 uiPointId) override {
    ...
    case 3:
      ++m_pointId;
      m_creature->GetMotionMaster()->MovePoint(m_pointId, aAndorovMoveLocs[m_pointId].m_fX,
                                               aAndorovMoveLocs[m_pointId].m_fY, aAndorovMoveLocs[m_pointId].m_fZ);
      break;
  ...
  void EnterEvadeMode() override {
    ...
    if (m_creature->IsAlive()) {
      ...
      if (m_pointId >= 4)
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_MOVE_ATTACK, aAndorovMoveLocs[4].m_fX,
                                                 aAndorovMoveLocs[4].m_fY, aAndorovMoveLocs[4].m_fZ);
      // reset to intro position
      else
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_MOVE_INTRO, aAndorovMoveLocs[2].m_fX,
                                                 aAndorovMoveLocs[2].m_fY, aAndorovMoveLocs[2].m_fZ);
    }
  ...
  void DoInitializeFollowers() {
    ...
    for (GuidList::const_iterator itr = m_lKaldoreiGuids.begin(); itr != m_lKaldoreiGuids.end(); ++itr)
      if (Creature *kaldorei = m_creature->GetMap()->GetCreature(*itr))
        kaldorei->GetMotionMaster()->MoveFollow(m_creature, kaldorei->GetDistance(m_creature),
                                                kaldorei->GetAngle(m_creature));
  }
...
UnitAI *GetAI_npc_general_andorov(Creature *creature) { return new npc_general_andorovAI(creature); }
...
struct npc_kaldorei_eliteAI : public CombatAI {
  npc_kaldorei_eliteAI(Creature *creature)
      : CombatAI(creature, KALDOREI_ACTION_MAX),
  ...
  void EnterEvadeMode() override {
    ...
    if (m_creature->IsAlive()) {
      if (Creature *andorov = m_instance->GetSingleCreatureFromStorage(NPC_GENERAL_ANDOROV)) {
        if (andorov->IsAlive())
          m_creature->GetMotionMaster()->MoveFollow(andorov, m_creature->GetDistance(andorov),
                                                    m_creature->GetAngle(andorov));
      }
    }
...
UnitAI *GetAI_npc_kaldorei_elite(Creature *creature) { return new npc_kaldorei_eliteAI(creature); }
