...
static const std::vector<uint32> aWeaknessSpell{SPELL_WEAKNESS_FIRE, SPELL_WEAKNESS_FROST, SPELL_WEAKNESS_NATURE,
                                                SPELL_WEAKNESS_ARCANE, SPELL_WEAKNESS_SHADOW};
...
struct boss_ossirianAI : public CombatAI {
  ...
  boss_ossirianAI(Creature *creature)
      : CombatAI(creature, OSSIRIAN_ACTION_MAX),
        m_instance(static_cast<instance_ruins_of_ahnqiraj *>(m_creature->GetInstanceData())), m_uiCrystalPosition(0),
        m_saidIntro(false) {
  ...
  void Aggro(Unit * /*who*/) override {
    ...
    for (auto aSandVortexSpawnPo : aSandVortexSpawnPos)
      m_creature->SummonCreature(NPC_SAND_VORTEX, aSandVortexSpawnPo[0], aSandVortexSpawnPo[1], aSandVortexSpawnPo[2],
                                 aSandVortexSpawnPo[3], TEMPSPAWN_CORPSE_DESPAWN, 0);

    if (m_instance)
      m_instance->instance->SetWeather(ZONE_ID_RUINS_AQ, WEATHER_TYPE_STORM, 1.0f, true);
  }

  void JustDied(Unit * /*killer*/) override { DoScriptText(SAY_DEATH, m_creature); }
  ...
  void JustSummoned(Creature *summoned) override {
    if (summoned->GetEntry() == NPC_SAND_VORTEX) {
      ...
      summoned->GetMotionMaster()->MoveRandomAroundPoint(aCrystalSpawnPos[0], aCrystalSpawnPos[1], aCrystalSpawnPos[2],
                                                         100.0f);
    }
  }
