...
static const SummonLocation aAyamissSpawnLocs[] = {
    ...
    {-9717.18f, 1517.72f,
     27.4677f}, // teleport location - need to be hardcoded because the player is teleported after the larva is summoned
};
...
struct boss_ayamissAI : public CombatAI {
  boss_ayamissAI(Creature *creature) : CombatAI(creature, AYAMISS_ACTION_MAX), m_phase(0) {
  ...
  void JustSummoned(Creature *summoned) override {
    ...
    if (summoned->GetEntry() == NPC_SWARMER) {
      ...
      switch (urand(0, 4)) {
      case 0:
        spellId = SPELL_SWARMER_TELEPORT_1;
        break;
      case 1:
        spellId = SPELL_SWARMER_TELEPORT_2;
        break;
      case 2:
        spellId = SPELL_SWARMER_TELEPORT_3;
        break;
      case 3:
        spellId = SPELL_SWARMER_TELEPORT_4;
        break;
      case 4:
        spellId = SPELL_SWARMER_TELEPORT_5;
        break;
      default:
        break;
      }
  ...
  void StartLanding() { m_creature->GetMotionMaster()->MovePath(1, PATH_FROM_ENTRY, FORCED_MOVEMENT_NONE, true); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case AYAMISS_POISON_STINGER: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, SPELL_POISON_STINGER, SELECT_FLAG_PLAYER))
...
struct npc_hive_zara_larvaAI : public ScriptedAI {
  npc_hive_zara_larvaAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_ruins_of_ahnqiraj *>(m_creature->GetInstanceData())) {
    if (m_instance)
    ...
    if (m_instance) {
      if (m_instance->GetData(TYPE_AYAMISS) == IN_PROGRESS) {
        if (m_creature->CanReachWithMeleeAttack(who) && who == m_creature->GetVictim()) {
          m_creature->CastSpell(who, SPELL_FEED, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
