...
UnitAI *GetAI_mobs_risen_husk_spirit(Creature *pCreature) { return new mobs_risen_husk_spiritAI(pCreature); }
...
UnitAI *GetAI_npc_restless_apparition(Creature *pCreature) { return new npc_restless_apparitionAI(pCreature); }
...
UnitAI *GetAI_npc_morokk(Creature *pCreature) { return new npc_morokkAI(pCreature); }
        ...
        if (m_uiGlobalTimer < uiDiff) {
          ...
          case PHASE_INTRO: {
            ...
            case 5:
              ...
              m_creature->SummonCreature(NPC_CALDWELL, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                         TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
              m_creature->SummonCreature(NPC_HALLAN, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                         TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
              m_creature->SummonCreature(NPC_SKIRMISHER, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                         TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
              m_creature->SummonCreature(NPC_SKIRMISHER, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                         TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
...
bool QuestAccept_npc_ogron(Player *pPlayer, Creature *pCreature, const Quest *pQuest) {
  if (pQuest->GetQuestId() == QUEST_QUESTIONING) {
    if (npc_ogronAI *pEscortAI = dynamic_cast<npc_ogronAI *>(pCreature->AI())) {
      ...
      pCreature->SetFactionTemporary(FACTION_ESCORT_N_FRIEND_PASSIVE,
...
UnitAI *GetAI_npc_ogron(Creature *pCreature) { return new npc_ogronAI(pCreature); }
...
const Location lOutroSpawns[] = {
    {-2857.604492f, -3354.784912f, 35.369640f, 3.16604f, -2881.546631f, -3346.477539f, 34.143719f, NPC_TERVOSH},
    {-2858.120117f, -3358.469971f, 36.086300f, 3.16604f, -2879.697998f, -3347.789063f, 34.772892f, NPC_JAINA},
    {-2857.379883f, -3351.370117f, 34.178001f, 3.16604f, -2879.959961f, -3344.469971f, 34.670502f, NPC_PAINED}};
  ...
  void DamageTaken(Unit *dealer, uint32 &damage, DamageEffectType /*damagetype*/,
    ...
    if (damage > m_creature->GetHealth() || m_creature->GetHealthPercent() < 20.0f) {
      ...
      for (const auto &lOutroSpawn : lOutroSpawns) {
        Creature *pCreature = m_creature->SummonCreature(lOutroSpawn.uiEntry, lOutroSpawn.fX, lOutroSpawn.fY,
                                                         lOutroSpawn.fZ, lOutroSpawn.fO, TEMPSPAWN_TIMED_DESPAWN,
                                                         3 * MINUTE * IN_MILLISECONDS, false, true);
        if (pCreature) {
...
UnitAI *GetAI_npc_private_hendel(Creature *pCreature) { return new npc_private_hendelAI(pCreature); }
...
struct npc_stinky_ignatzAI : public npc_escortAI {
  ...
  void Aggro(Unit *pWho) override {
    switch (urand(0, 3)) {
    case 0:
      DoScriptText(SAY_STINKY_AGGRO_1, m_creature);
      break;
    case 1:
      DoScriptText(SAY_STINKY_AGGRO_2, m_creature);
      break;
    case 2:
      DoScriptText(SAY_STINKY_AGGRO_3, m_creature);
      break;
    case 3:
      DoScriptText(SAY_STINKY_AGGRO_4, m_creature, pWho);
      break;
    }
  }
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 26:
      ...
      if (GameObject *pBogbeanPlant =
              GetClosestGameObjectWithEntry(m_creature, GO_BOGBEAN_PLANT, DEFAULT_VISIBILITY_DISTANCE)) {
    ...
    case 40:
      if (Player *pPlayer = GetPlayerForEscort()) {
        pPlayer->RewardPlayerAndGroupAtEventExplored(pPlayer->GetTeam() == ALLIANCE ? QUEST_ID_STINKYS_ESCAPE_ALLIANCE
                                                                                    : QUEST_ID_STINKYS_ESCAPE_HORDE,
                                                     m_creature);
...
UnitAI *GetAI_npc_stinky_ignatz(Creature *pCreature) { return new npc_stinky_ignatzAI(pCreature); }
...
bool AreaTrigger_at_nats_landing(Player *pPlayer, const AreaTriggerEntry * /*pAt*/) {
  if (pPlayer->GetQuestStatus(QUEST_NATS_BARGAIN) == QUEST_STATUS_INCOMPLETE && pPlayer->HasAura(SPELL_FISH_PASTE)) {
    ...
    if (!pShark)
      pShark = pPlayer->SummonCreature(NPC_LURKING_SHARK, -4254.339f, -3914.995f, -15.61402f, 0.0f,
                                       TEMPSPAWN_TIMED_OOC_DESPAWN, 100000);
...
struct mob_invis_firework_helper : public Scripted_NoMovementAI {
  mob_invis_firework_helper(Creature *pCreature) : Scripted_NoMovementAI(pCreature) { Reset(); }
  ...
  bool GroupDeterminerFunc() { return m_creature->GetPositionY() > fireworkMidPoint; }
...
UnitAI *GetAI_mob_invis_firework_helper(Creature *pCreature) { return new mob_invis_firework_helper(pCreature); }
  ...
  void MovementInform(uint32 uiMotionType, uint32 uiPointId) override {
    if (uiMotionType == WAYPOINT_MOTION_TYPE) {
      ...
      case 15: {
        ...
        for (std::list<GameObject *>::const_iterator itr = lCannonsInRange.begin(); itr != lCannonsInRange.end();
             ++itr) {
      ...
      if (uiPointId) {
        if (DoCastSpellIfCan(m_creature, m_bSpoutDirection ? SPELL_SPOUT_RIGHT : SPELL_SPOUT_LEFT,
...
UnitAI *GetAI_boss_tethyr(Creature *pCreature) { return new boss_tethyrAI(pCreature); }

/*######
## npc_major_mills
######*/

enum { DBSCRIPT_RELAY_TAKE_DOWN_TETHYR = 10161 };

struct npc_major_mills : public ScriptedAI {
  npc_major_mills(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
...
UnitAI *GetAI_npc_major_mills(Creature *pCreature) { return new npc_major_mills(pCreature); }
...
bool AreaTrigger_at_sentry_point(Player *pPlayer, const AreaTriggerEntry * /*pAt*/) {
  ...
  if (!GetClosestCreatureWithEntry(pPlayer, NPC_TERVOSH, 100.0f)) {
    if (Creature *pTervosh = pPlayer->SummonCreature(NPC_TERVOSH, -3476.51f, -4105.94f, 17.1f, 5.3816f,
...
static const DialogueEntry SmolderDialogue[] = {
    {DO_NOTHING, 0, 2000},          {SAY_1, NPC_SMOLDERWING, 6500},
    {SAY_2, NPC_SMOLDERWING, 5000}, {SPELL_SMOLDERWING_FIRE_BREATH, 0, 4000},
    {GO_STONEMAUL_BANNER, 0, 0},    {0, 0, 0},
};

struct npc_smolderwing : public ScriptedAI, private DialogueHelper {
  npc_smolderwing(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(SmolderDialogue) { Reset(); }
...
UnitAI *GetAI_npc_smolderwing(Creature *pCreature) { return new npc_smolderwing(pCreature); }
