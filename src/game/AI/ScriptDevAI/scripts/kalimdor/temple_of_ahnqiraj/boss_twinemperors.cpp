...
struct boss_twin_emperorsAI : public CombatAI {
  boss_twin_emperorsAI(Creature *creature, uint32 actionCount)
      : CombatAI(creature, actionCount),
        m_instance(static_cast<instance_temple_of_ahnqiraj *>(creature->GetInstanceData())) {
  ...
  void DamageTaken(Unit * /*dealer*/, uint32 &damage, DamageEffectType /*damagetype*/,
    ...
    if (Creature *twin = m_instance->GetSingleCreatureFromStorage(m_creature->GetEntry() == NPC_VEKLOR ? NPC_VEKNILASH
                                                                                                       : NPC_VEKLOR)) {
...
struct boss_veknilashAI : public boss_twin_emperorsAI {
  ...
  void MoveInLineOfSight(Unit *who) override {
    if (m_instance && m_instance->GetData(TYPE_TWINS) == IN_PROGRESS && who->GetEntry() == NPC_VEKLOR &&
        who->IsWithinDistInMap(m_creature, 60.0f))
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(SAY_VEKNILASH_SLAY, m_creature); }
  ...
  bool DoHandleBugAbility() { return DoCastSpellIfCan(nullptr, SPELL_MUTATE_BUG) == CAST_OK; }

  bool DoHandleBerserk() { return DoCastSpellIfCan(nullptr, SPELL_BERSERK) == CAST_OK; }

  void ExecuteAction(uint32 action) override {
    switch (action) {
    case VEKNILASH_UPPERCUT: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_UPPERCUT,
...
struct boss_veklorAI : public boss_twin_emperorsAI {
  ...
  void MoveInLineOfSight(Unit *who) override {
    if (m_instance && m_instance->GetData(TYPE_TWINS) == IN_PROGRESS && who->GetEntry() == NPC_VEKNILASH &&
        who->IsWithinDistInMap(m_creature, 60.0f))
      DoCastSpellIfCan(who, SPELL_HEAL_BROTHER);
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(SAY_VEKLOR_SLAY, m_creature); }
  ...
  void SpellHit(Unit * /*caster*/, const SpellEntry *spellInfo) override {
    if (spellInfo->Id == SPELL_TWIN_TELEPORT) {
      ...
      if (Creature *veknilash = m_instance->GetSingleCreatureFromStorage(NPC_VEKNILASH)) {
        ...
        TeleportTwin(m_creature, veknilash->GetPositionX(), veknilash->GetPositionY(), veknilash->GetPositionZ(),
                     veknilash->GetOrientation());
  ...
  bool DoHandleBugAbility() override { return DoCastSpellIfCan(nullptr, SPELL_EXPLODE_BUG) == CAST_OK; }

  bool DoHandleBerserk() { return DoCastSpellIfCan(nullptr, SPELL_FRENZY) == CAST_OK; }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case VEKLOR_BLIZZARD: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BLIZZARD, SELECT_FLAG_PLAYER))
        if (DoCastSpellIfCan(target, SPELL_BLIZZARD) == CAST_OK)
          ResetCombatAction(action, urand(15000, 30000));
      break;
    }
    case VEKLOR_ARCANE_BURST: {
      if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, SPELL_ARCANE_BURST,
