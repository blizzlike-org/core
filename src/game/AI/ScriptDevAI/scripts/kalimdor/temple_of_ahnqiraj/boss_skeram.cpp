...
struct boss_skeramAI : public CombatAI {
  boss_skeramAI(Creature *creature)
      : CombatAI(creature, SKERAM_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
        m_isImage(!m_creature->GetSpawnerGuid().IsEmpty()) {
    if (!m_isImage) {
  ...
  void HandleInitImages() {
    std::shuffle(m_teleports.begin(), m_teleports.end(),
                 *GetRandomGenerator()); // Shuffle the teleport spells to ensure that boss and images have a different
                                         // location assigned randomly
    m_teleportCounter = 1;
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case SKERAM_SPLIT: {
      ...
      if (m_creature->GetHealthPercent() < m_hpCheck) {
        ...
        if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_IMAGES) == CAST_OK) {
          m_hpCheck -= 25.0f;
          StopAttacking();
          // Teleport shortly after the images are summoned and set invisible to clear the selection (Workaround
          // alert!!!)
    ...
    case SKERAM_TRUE_FULFILMENT: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 1, SPELL_TRUE_FULFILLMENT,
                                                           SELECT_FLAG_PLAYER))
    ...
    case SKERAM_BLINK: {
      std::shuffle(m_teleports.begin(), m_teleports.end(),
                   *GetRandomGenerator()); // Shuffle the teleport spells to ensure that boss and images have a
                                           // different location assigned randomly
      m_teleportCounter = 1;
      DoTeleport(true);
...
struct InitializeImages : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override {
    if (Unit *unitTarget = spell->GetUnitTarget()) {
      ...
      unitTarget->CastCustomSpell(nullptr, SPELL_INITIALIZE_IMAGE, &healthPercent, nullptr, nullptr,
