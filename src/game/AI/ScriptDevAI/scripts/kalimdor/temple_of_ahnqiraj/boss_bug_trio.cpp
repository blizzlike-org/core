...
struct boss_silithidRoyaltyAI : public CombatAI {
  boss_silithidRoyaltyAI(Creature *creature, uint32 actionCount)
      : CombatAI(creature, actionCount), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
        m_deathAbility(0) {
    AddCustomAction(ROYALTY_DEVOUR_DELAY, true, [&]() { HandleDevourDelay(); });
    m_creature->GetCombatManager().SetLeashingCheck([&](Unit * /*unit*/, float /*x*/, float /*y*/,
                                                        float /*z*/) -> bool {
  ...
  void DoSpecialAbility() { DoCastSpellIfCan(nullptr, m_deathAbility); }
...
struct boss_kriAI : public boss_silithidRoyaltyAI {
  boss_kriAI(Creature *creature)
      : boss_silithidRoyaltyAI(creature, KRI_ACTION_MAX),
...
struct boss_vemAI : public boss_silithidRoyaltyAI {
  boss_vemAI(Creature *creature)
      : boss_silithidRoyaltyAI(creature, VEM_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
  ...
  void ExecuteAction(uint32 action) override {
    switch (action) {
    case VEM_CHARGE: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CHARGE,
...
struct boss_yaujAI : public boss_silithidRoyaltyAI {
  boss_yaujAI(Creature *creature)
      : boss_silithidRoyaltyAI(creature, YAUJ_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
