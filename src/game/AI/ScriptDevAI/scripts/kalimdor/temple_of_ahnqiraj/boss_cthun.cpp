...
SDName: Boss_Cthun
SD%Complete: 95
SDComment: Transform spell has some minor core issues. Digestive Acid should be handled in core. Spell to kill players
in stomach on reset needs implementation. SDCategory: Temple of Ahn'Qiraj EndScriptData */
...
enum {
  ...
  SPELL_EYE_BEAM_INIT =
      32950, // Cast three times on victim at encounter start, then replaced by spell 26134 on random targets
  SPELL_DARK_GLARE = 26029,
  SPELL_ROTATE_TRIGGER =
      26137, // phase switch spell - triggers 26009 or 26136. These trigger the Dark Glare spell - 26029
  ...
  SPELL_GIANT_EYE_TENTACLES_1 =
      26766, // Periodically triggers 26767 that cast 26768 on random target to summon NPC 15334
  SPELL_GIANT_EYE_TENTACLES_2 = 26767,
  SPELL_SUMMON_GIANT_EYE_TENTACLE = 26768, // Summon NPC 15334
  SPELL_SUMMON_GIANT_HOOKS_1 =
      26213, // Periodically triggers 26217 that cast 26216 on random target to summon NPC 15728
  SPELL_SUMMON_GIANT_HOOKS_2 = 26217,
  SPELL_SUMMON_GIANT_HOOK_TENTACLE = 26216, // Summon NPC 15728
  SPELL_SUMMON_MOUTH_TENTACLES_1 =
      26236, // Periodically triggers 26237 that cast 26332 on random target to summon NPC 15910
  ...
  SPELL_EXIT_STOMACH = 26221, // Summon NPC 15800 that will handle the initial knockback outside the stomach through
                              // spell 26224 (EventAI)

  NPC_WORLD_TRIGGER = 15384,

  DISPLAY_ID_CTHUN_BODY = 15786, // Helper display id; This is needed in order to have the proper transform animation.
...
struct boss_eye_of_cthunAI : public CombatAI {
  boss_eye_of_cthunAI(Creature *creature)
      : CombatAI(creature, EYECTHUN_ACTION_MAX),
...
struct boss_cthunAI : public CombatAI {
  boss_cthunAI(Creature *creature)
      : CombatAI(creature, CTHUN_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    // There are 30 seconds offset between Giant Eye and Giant Claw Tentacles which are both on a 60 seconds period
    // handled by periodic spells
  ...
  void SummonedCreatureDespawn(Creature * /*summoned*/) override { m_creature->AI()->EnterEvadeMode(); }
  ...
  void DoSpawnTentacles() {
    ...
    for (uint8 i = 0; i < MAX_FLESH_TENTACLES; ++i)
      m_creature->SummonCreature(NPC_FLESH_TENTACLE, cthunLocations[i][0], cthunLocations[i][1], cthunLocations[i][2],
                                 cthunLocations[i][3], TEMPSPAWN_DEAD_DESPAWN, 0);
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case CTHUN_CLAWTENTACLEDELAY:
      ...
      if (Unit *player = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0),
    ...
    case CTHUN_EYETENTACLEDELAY:
      // The first Giant Eye Tentacle (at phase 2 start of after weakened state, 30 after first Giant Claw Tentacle) is
      // a single isolated cast, all after are on periodic timer through dedicated spell Check for valid player
      if (Unit *player = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0),
                                                           SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER)) {
        ...
        if (DoCastSpellIfCan(m_creature, SPELL_GIANT_EYE_TENTACLES_1, CAST_TRIGGERED | CAST_AURA_NOT_PRESENT) ==
            CAST_OK)
          DisableCombatAction(action);
        // In case of failure, try another single cast
        else
...
struct npc_giant_claw_tentacleAI : public CombatAI {
  npc_giant_claw_tentacleAI(Creature *creature)
      : CombatAI(creature, GIANTCLAW_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
...
bool AreaTrigger_at_stomach_cthun(Player *player, AreaTriggerEntry const *at) {
  ...
  if (at->id == AREATRIGGER_STOMACH_3) {
    // We need to check for the presence of Digestive Acid aura on player so the area trigger will only activate and
    // knockback players coming from C'Thun's stomach The knockback spell will remove the Digestive Acid aura
...
struct PeriodicSummonEyeTrigger : public AuraScript {
  void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData & /*data*/) const override {
    if (Unit *caster = aura->GetCaster()) {
      uint32 eyeTentaclesSpells[] = {SPELL_SUMMON_EYE_TENTACLE_1, SPELL_SUMMON_EYE_TENTACLE_2,
                                     SPELL_SUMMON_EYE_TENTACLE_3, SPELL_SUMMON_EYE_TENTACLE_4,
                                     SPELL_SUMMON_EYE_TENTACLE_5, SPELL_SUMMON_EYE_TENTACLE_6,
                                     SPELL_SUMMON_EYE_TENTACLE_7, SPELL_SUMMON_EYE_TENTACLE_8};
  ...
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    if (effIdx == EFFECT_INDEX_0) {
      if (Unit *target = spell->GetUnitTarget()) {
        ...
        case SPELL_SUMMON_MOUTH_TENTACLES_2:
          spellId = SPELL_SUMMON_MOUTH_TENTACLE;
          break;
        case SPELL_GIANT_EYE_TENTACLES_2:
          spellId = SPELL_SUMMON_GIANT_EYE_TENTACLE;
          break;
        case SPELL_SUMMON_HOOK_TENTACLES_2:
          spellId = SPELL_SUMMON_HOOK_TENTACLE;
          break;
        case SPELL_SUMMON_GIANT_HOOKS_2:
          spellId = SPELL_SUMMON_GIANT_HOOK_TENTACLE;
          break;
        default:
          return;
        }
...
struct RotateTrigger : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    if (effIdx == EFFECT_INDEX_0) {
      if (Unit *target = spell->GetUnitTarget())
        target->CastSpell(nullptr, urand(0, 1) ? SPELL_ROTATE_360_LEFT : SPELL_ROTATE_360_RIGHT,
