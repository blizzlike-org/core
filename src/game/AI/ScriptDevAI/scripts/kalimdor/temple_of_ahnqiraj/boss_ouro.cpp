...
struct boss_ouroAI : public CombatAI {
  boss_ouroAI(Creature *creature)
      : CombatAI(creature, OURO_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
        m_firstMound(true) {
  ...
  void JustSummoned(GameObject *go) override { m_base = go->GetObjectGuid(); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case OURO_SPAWN: {
      ...
      if (m_creature->GetUInt32Value(UNIT_CREATED_BY_SPELL)) {
        if (Creature *trigger = GetClosestCreatureWithEntry(
                m_creature, NPC_OURO_TRIGGER, 100.f)) // if not sufficient in the future, do using SPELL_SET_OURO_HEALTH
    ...
    case OURO_BOULDER: {
      ...
      if (m_creature->GetVictim() && m_creature->CanReachWithMeleeAttack(m_creature->GetVictim()))
        ...
        if (m_rangeCheckState > 1)
          if (Unit *target =
                  m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BOULDER, SELECT_FLAG_PLAYER))
...
struct npc_ouro_spawnerAI : public Scripted_NoMovementAI {
  ...
  void Aggro(Unit * /*who*/) override {
    if (!m_bHasSummoned) {
      m_creature->SummonCreature(NPC_OURO, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                 m_creature->GetPositionZ(), m_creature->GetOrientation(), TEMPSPAWN_DEAD_DESPAWN, 0,
                                 true);
      m_bHasSummoned = true;
    }
  }
...
struct npc_ouro_moundAI : public ScriptedAI, public TimerManager {
  ...
  void Reset() override { DoCastSpellIfCan(nullptr, SPELL_DIRTMOUND_PASSIVE, CAST_AURA_NOT_PRESENT | CAST_TRIGGERED); }
  ...
  void JustSummoned(Creature * /*summoned*/) override { m_creature->ForcedDespawn(1); }
