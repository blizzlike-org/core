...
enum {
  ...
  SPELL_WYVERN_STING = 26180, // triggers 26233 on aura removal, doing 500 nature damage if 26180 expires (confirmed)
                              // and about 3K if dispelled early (amount unconfirmed)
...
struct boss_huhuranAI : public CombatAI {
  boss_huhuranAI(Creature *creature)
      : CombatAI(creature, HUHURAN_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case HUHURAN_NOXIOUS_POISON: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_NOXIOUS_POISON, SELECT_FLAG_PLAYER))
  ...
  void OnApply(Aura *aura, bool apply) const override {
    if (!apply) {
      if (Unit *caster = aura->GetCaster()) {
        int32 natureDamage = (aura->GetRemoveMode() == AURA_REMOVE_BY_DISPEL)
                                 ? 3000
                                 : 500; // Deal 500 nature damage on spell expire but 3000 if dispelled
