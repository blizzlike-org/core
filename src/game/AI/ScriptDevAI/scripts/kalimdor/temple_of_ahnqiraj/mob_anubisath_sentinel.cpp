...
struct npc_anubisath_sentinelAI : public CombatAI {
  npc_anubisath_sentinelAI(Creature *creature)
      : CombatAI(creature, ANUBSENTINEL_ACTION_MAX),
  ...
  void Reset() override {
    CombatAI::Reset();
    m_myAbility = 0;
    m_abilities = {SPELL_PERIODIC_MANA_BURN,
                   SPELL_MENDING,
  ...
  void GetAIInformation(ChatHandler &reader) override {
    ...
    if (m_assistList.size() == MAX_BUDDY)
      ...
      reader.PSendSysMessage(
          "Anubisath Sentinel - not correct number of mobs for group found. Number found %u, should be %u",
          uint32(m_assistList.size()), MAX_BUDDY);
  }
  ...
  void Aggro(Unit *who) override {
    ...
    if (m_assistList.empty()) {
      ...
      if (m_assistList.size() != MAX_BUDDY)
        script_error_log("npc_anubisath_sentinel for %s found too few/too many buddies, expected %u.",
                         m_creature->GetGuidStr().c_str(), MAX_BUDDY);
    }

    if (!m_myAbility) {
      std::shuffle(
          m_abilities.begin(), m_abilities.end(),
          *GetRandomGenerator()); // shuffle the abilities, they will be set to the current creature and its siblings
      SetAbility();
      InitSentinelsNear(who);
    }
  }

  void JustDied(Unit * /*killer*/) override { DoTransferAbility(); }
  ...
  void SetAbility(bool forced = false, uint32 abilityId = 0) {
    ...
    // Check that the requested ability is valid
      ...
      script_error_log("npc_anubisath_sentinel for %s: request illegal spell ID as ability %u.",
                       m_creature->GetGuidStr().c_str(), abilityId);
      return;
    }
  ...
  void DoTransferAbility() {
    ...
    for (auto guid : m_assistList) {
      if (Creature *buddy = m_creature->GetMap()->GetCreature(guid)) {
        ...
        m_creature->CastCustomSpell(buddy, SPELL_TRANSFER_POWER, &m_myAbility, nullptr, nullptr,
...
struct npc_anubisath_defenderAI : public CombatAI {
  npc_anubisath_defenderAI(Creature *creature)
      : CombatAI(creature, ANUBDEFENDER_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ANUBDEFENDER_PLAGUE: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_SKIP_TANK)) {
