...
bool GossipHello_npc_mountaineer_pebblebitty(Player *pPlayer, Creature *pCreature) {
  ...
  if (!pPlayer->GetQuestRewardStatus(3181) == 1)
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Open the gate please, i need to get to Searing Gorge",
...
bool GossipSelect_npc_mountaineer_pebblebitty(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/,
                                              uint32 uiAction) {
  switch (uiAction) {
  case GOSSIP_ACTION_INFO_DEF + 1:
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "What's the worst that could happen?", GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF + 2);
    pPlayer->SEND_GOSSIP_MENU(1833, pCreature->GetObjectGuid());
    break;
  case GOSSIP_ACTION_INFO_DEF + 2:
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Another way? Do tell...", GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF + 3);
    pPlayer->SEND_GOSSIP_MENU(1834, pCreature->GetObjectGuid());
    break;
  case GOSSIP_ACTION_INFO_DEF + 3:
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Orcs? Badlands? I'm invulnerable!", GOSSIP_SENDER_MAIN,
  ...
  case GOSSIP_ACTION_INFO_DEF + 5:
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "My apologies, Pebblebitty.", GOSSIP_SENDER_MAIN,
...
static const Location m_afAmbushSpawn[] = {{-5691.93f, -3745.91f, 319.159f, 2.21f},
...
struct npc_miranAI : public npc_escortAI {
  ...
  void WaypointReached(uint32 uiPointId) override {
    ...
    case 19:
      ...
      m_creature->SummonCreature(NPC_DARK_IRON_DWARF, m_afAmbushSpawn[0].m_fX, m_afAmbushSpawn[0].m_fY,
                                 m_afAmbushSpawn[0].m_fZ, m_afAmbushSpawn[0].m_fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 25000);
      m_creature->SummonCreature(NPC_DARK_IRON_DWARF, m_afAmbushSpawn[1].m_fX, m_afAmbushSpawn[1].m_fY,
                                 m_afAmbushSpawn[1].m_fZ, m_afAmbushSpawn[1].m_fO, TEMPSPAWN_CORPSE_TIMED_DESPAWN,
                                 25000);
      break;
    case 23:
...
UnitAI *GetAI_npc_miran(Creature *pCreature) { return new npc_miranAI(pCreature); }
