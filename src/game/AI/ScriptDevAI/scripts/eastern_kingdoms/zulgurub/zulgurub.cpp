...
struct npc_soulflayerAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiSoulTapTimer < uiDiff) {
      if (Unit *pTarget =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SOUL_TAP, SELECT_TARGET_ANY_ENEMY))
...
UnitAI *GetAI_npc_soulflayer(Creature *pCreature) { return new npc_soulflayerAI(pCreature); }
