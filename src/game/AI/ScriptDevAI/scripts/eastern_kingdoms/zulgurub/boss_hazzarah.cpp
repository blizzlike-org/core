...
struct boss_hazzarahAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiManaBurnTimer < uiDiff) {
      if (Unit *pTarget =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CHAIN_BURN, SELECT_FLAG_POWER_MANA)) {
...
UnitAI *GetAI_boss_hazzarah(Creature *pCreature) { return new boss_hazzarahAI(pCreature); }
