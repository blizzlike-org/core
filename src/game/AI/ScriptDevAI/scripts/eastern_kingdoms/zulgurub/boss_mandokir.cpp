...
struct boss_mandokirAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override {
    ...
    for (auto &aSpirit : aSpirits)
      m_creature->SummonCreature(NPC_CHAINED_SPIRIT, aSpirit.fX, aSpirit.fY, aSpirit.fZ, aSpirit.fAng,
  ...
  void EnterEvadeMode() override {
    ...
    if (m_creature->IsAlive())
      m_creature->GetMotionMaster()->MovePoint(0, aMandokirDownstairsPos[0], aMandokirDownstairsPos[1],
    ...
    if (m_uiWatchTimer < uiDiff) {
      ...
      if (m_watchTargetGuid) {
        ...
        if (pWatchTarget && pWatchTarget->IsAlive() &&
            m_creature->getThreatManager().getThreat(pWatchTarget) > m_fTargetThreat) {
...
struct mob_ohganAI : public ScriptedAI {
  ...
  void Reset() override { m_uiSunderArmorTimer = 5000; }
...
UnitAI *GetAI_boss_mandokir(Creature *pCreature) { return new boss_mandokirAI(pCreature); }

UnitAI *GetAI_mob_ohgan(Creature *pCreature) { return new mob_ohganAI(pCreature); }
