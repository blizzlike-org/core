  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (!m_bIsPhaseTwo) {
      ...
      if (m_uiMarkTimer < uiDiff) {
        if (Unit *pTarget =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MARK_ARLOKK, SELECT_FLAG_PLAYER)) {
...
struct npc_zulian_prowlerAI : public ScriptedAI {
  ...
  void Reset() override {
    ...
    if (m_bMoveToAid) {
      ...
      m_creature->GetMotionMaster()->MovePoint(1, aArlokkWallShieldPos[0], aArlokkWallShieldPos[1],
                                               aArlokkWallShieldPos[2]);
...
UnitAI *GetAI_boss_arlokk(Creature *pCreature) { return new boss_arlokkAI(pCreature); }

UnitAI *GetAI_npc_zulian_prowler(Creature *pCreature) { return new npc_zulian_prowlerAI(pCreature); }
