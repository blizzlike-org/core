...
SD%Complete: 85
SDComment: Mind Control not working because of core bug. Shades invisible is removed as of Attacking (core bug) - MANY
HACKZ!! SDCategory: Zul'Gurub EndScriptData */
...
static const float aPitTeleportLocs[4] = {-11583.7783f, -1249.4278f, 77.5471f, 4.745f};
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiDelusionsTimer < uiDiff) {
      ...
      if (DoCastSpellIfCan(pTarget, SPELL_DELUSIONS_OF_JINDO) == CAST_OK) {
        float fX, fY, fZ;
        m_creature->GetRandomPoint(pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 5.0f, fX,
                                   fY, fZ);
        if (Creature *pSummoned =
                m_creature->SummonCreature(NPC_SHADE_OF_JINDO, fX, fY, fZ, 0, TEMPSPAWN_TIMED_OOC_DESPAWN, 15000))
    ...
    if (m_uiTeleportTimer < uiDiff) {
      if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0)) {
        ...
        for (uint8 i = 0; i < MAX_SKELETONS; ++i) {
          ...
          if (Creature *pSummoned = m_creature->SummonCreature(NPC_SACRIFICED_TROLL, fX, fY, fZ, 0.0f,
                                                               TEMPSPAWN_TIMED_OOC_DESPAWN, 15000))
...
UnitAI *GetAI_boss_jindo(Creature *pCreature) { return new boss_jindoAI(pCreature); }

UnitAI *GetAI_mob_healing_ward(Creature *pCreature) { return new mob_healing_wardAI(pCreature); }
