...
struct boss_thekalBaseAI : public ScriptedAI {
  boss_thekalBaseAI(Creature *pCreature) : ScriptedAI(pCreature) { m_uiPhase = PHASE_NORMAL; }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    case PHASE_NORMAL:
      if (m_uiDispelTimer < uiDiff) {
        ...
        if (!dispelTarget &&
            (m_creature->isInRoots() || m_creature->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_DECREASE_SPEED)))
...
bool EffectDummyCreature_thekal_resurrection(Unit * /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
UnitAI *GetAI_boss_thekal(Creature *pCreature) { return new boss_thekalAI(pCreature); }

UnitAI *GetAI_mob_zealot_lorkhan(Creature *pCreature) { return new mob_zealot_lorkhanAI(pCreature); }

UnitAI *GetAI_mob_zealot_zath(Creature *pCreature) { return new mob_zealot_zathAI(pCreature); }
