    ...
    // Troll phase
      ...
      if (m_uiFlyingBatsTimer) {
        if (m_uiFlyingBatsTimer <= uiDiff) {
          ...
          for (uint8 i = 0; i < 3; ++i) {
            if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
              m_creature->SummonCreature(NPC_BAT_RIDER, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                         pTarget->GetPositionZ() + 15.0f, 0, TEMPSPAWN_DEAD_DESPAWN, 0);
          }
...
UnitAI *GetAI_boss_jeklik(Creature *pCreature) { return new boss_jeklikAI(pCreature); }

UnitAI *GetAI_npc_gurubashi_bat_rider(Creature *pCreature) { return new npc_gurubashi_bat_riderAI(pCreature); }
