...
struct npc_a_special_surpriseAI : public ScriptedAI, private DialogueHelper {
  npc_a_special_surpriseAI(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(aSurpriseDialogue) {
  ...
  bool MeetQuestCondition(Player *pPlayer) const {
    ...
    case NPC_PRISONER_HUMAN:
      return pPlayer->GetQuestStatus(12742) == QUEST_STATUS_INCOMPLETE; // Ellen Stanbridge
    case NPC_PRISONER_ORC:
      return pPlayer->GetQuestStatus(12748) == QUEST_STATUS_INCOMPLETE; // Kug Ironjaw
    case NPC_PRISONER_DWARF:
      return pPlayer->GetQuestStatus(12744) == QUEST_STATUS_INCOMPLETE; // Donovan Pulfrost
    case NPC_PRISONER_NIGHT_ELF:
      return pPlayer->GetQuestStatus(12743) == QUEST_STATUS_INCOMPLETE; // Yazmina Oakenthorn
    case NPC_PRISONER_UNDEAD:
      return pPlayer->GetQuestStatus(12750) == QUEST_STATUS_INCOMPLETE; // Antoine Brack
    case NPC_PRISONER_TAUREN:
      return pPlayer->GetQuestStatus(12739) == QUEST_STATUS_INCOMPLETE; // Malar Bravehorn
    case NPC_PRISONER_GNOME:
      return pPlayer->GetQuestStatus(12745) == QUEST_STATUS_INCOMPLETE; // Goby Blastenheimer
    case NPC_PRISONER_TROLL:
      return pPlayer->GetQuestStatus(12749) == QUEST_STATUS_INCOMPLETE; // Iggy Darktusk
    case NPC_PRISONER_BLOOD_ELF:
      return pPlayer->GetQuestStatus(12747) == QUEST_STATUS_INCOMPLETE; // Lady Eonys
    case NPC_PRISONER_DRAENEI:
      return pPlayer->GetQuestStatus(12746) == QUEST_STATUS_INCOMPLETE; // Valok the Righteous
    }
  ...
  Unit *GetDialogueTarget() override { return m_creature->GetMap()->GetPlayer(m_playerGuid); }
...
int32 m_auiRandomSay[] = {SAY_DUEL_A, SAY_DUEL_B, SAY_DUEL_C, SAY_DUEL_D, SAY_DUEL_E,
                          SAY_DUEL_F, SAY_DUEL_G, SAY_DUEL_H, SAY_DUEL_I};
...
struct npc_death_knight_initiateAI : public CombatAI, private DialogueHelper {
  npc_death_knight_initiateAI(Creature *creature)
      : CombatAI(creature, INITIATE_ACTION_MAX), DialogueHelper(aInitiateDialogue) {
  ...
  void DamageTaken(Unit * /*pDoneBy*/, uint32 &uiDamage, DamageEffectType /*damagetype*/,
  ...
  Unit *GetDialogueTarget() override { return m_creature->GetMap()->GetPlayer(m_duelerGuid); }
...
bool GossipHello_npc_death_knight_initiate(Player *pPlayer, Creature *pCreature) {
  if (pPlayer->GetQuestStatus(QUEST_DEATH_CHALLENGE) == QUEST_STATUS_INCOMPLETE) {
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ACCEPT_DUEL, GOSSIP_SENDER_MAIN,
...
bool GossipSelect_npc_death_knight_initiate(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/,
                                            uint32 uiAction) {
...
bool EffectDummyCreature_npc_death_knight_initiate(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
enum {
  // SPELL_EYE_CONTROL    = 51852,                        // apply phase aura: 2; summon creature 28511 and apply player
  // control aura
    ...
    case 0: // initialization and move to start position
    {
      if (m_creature->GetBeneficiaryPlayer()) {
        if (DoCastSpellIfCan(m_creature, SPELL_EYE_VISUAL) == CAST_OK)
          m_creature->GetMotionMaster()->MovePoint(POINT_EYE_DESTINATION, aEyeStartPos[0], aEyeStartPos[1],
                                                   aEyeStartPos[2]);
      } else {
    ...
    case 1: // wait for start position reached, then wait 5 sec before the journey to the end point
    {
      ...
      if (m_flightTimer < uiDiff) {
        ...
        if (DoCastSpellIfCan(m_creature, SPELL_EYE_FLIGHT_BOOST) == CAST_OK) {
          ...
          m_creature->GetMotionMaster()->MovePoint(POINT_EYE_DESTINATION, tgtPos, FORCED_MOVEMENT_NONE,
                                                   m_creature->GetSpawner()->GetSpeed(MOVE_RUN));
...
enum {
  ...
  SPELL_GHOULPLOSION = 52519, // cast by Gothik on the Scarlet Ghoul before destroying the creature; triggers 52555 from
                              // ghoul target to ghoul master
...
bool EffectDummyCreature_npc_scarlet_ghoul(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
                                           Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/) {
...
enum LightOfDawn {
  ...
  SPELL_THE_LIGHT_OF_DAWN_DAMAGE_LOSS =
      53645, // cast by the scourge units
...
struct npc_highlord_darion_mograineAI : public CombatAI, private DialogueHelper {
  npc_highlord_darion_mograineAI(Creature *creature)
      : CombatAI(creature, DARION_ACTION_MAX),
        m_instance(static_cast<world_map_ebon_hold *>(creature->GetInstanceData())), DialogueHelper(aDarionDialogue) {
  ...
  void DoSendQuestCredit() {
    ...
    for (const auto &itr : PlayerList) {
      ...
      if (pPlayer && pPlayer->GetQuestStatus(QUEST_ID_LIGHT_OF_DAWN) == QUEST_STATUS_INCOMPLETE && pPlayer->IsAlive() &&
          pPlayer->IsWithinDistInMap(m_creature, 100.0f))
  ...
  void DoStartConclusion() {
    ...
    for (auto &i : aGuardsOutroSpawnLoc) {
      if (Creature *pGuard = m_creature->SummonCreature(NPC_DEFENDER_OF_THE_LIGHT, i.m_fX, i.m_fY, i.m_fZ, i.m_fO,
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit *pSender, Unit *pInvoker, uint32 /*uiMiscValue*/) override {
    ...
    case AI_EVENT_CUSTOM_C:
      m_creature->SummonCreature(NPC_HIGHLORD_TIRION_FORDRING, aEventLocations[0].m_fX, aEventLocations[0].m_fY,
                                 aEventLocations[0].m_fZ, aEventLocations[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0, true);
      break;
  ...
  void JustDidDialogueStep(int32 entry) override {
    ...
    case NPC_HIGHLORD_ALEXANDROS_MOGRAINE:
      m_creature->SummonCreature(NPC_HIGHLORD_ALEXANDROS_MOGRAINE, aEventLocations[1].m_fX, aEventLocations[1].m_fY,
                                 aEventLocations[1].m_fZ, aEventLocations[1].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0);
      break;
    ...
    case SAY_LIGHT_OF_DAWN_VISION_3:
      m_creature->SetStandState(UNIT_STAND_STATE_KNEEL);
      m_creature->SummonCreature(NPC_DARION_MOGRAINE, aEventLocations[2].m_fX, aEventLocations[2].m_fY,
                                 aEventLocations[2].m_fZ, aEventLocations[2].m_fO, TEMPSPAWN_TIMED_DESPAWN,
    ...
    case SAY_LIGHT_OF_DAWN_VISION_8:
      ...
      break;
    case NPC_THE_LICH_KING:
      m_creature->SummonCreature(NPC_THE_LICH_KING, aEventLocations[3].m_fX, aEventLocations[3].m_fY,
                                 aEventLocations[3].m_fZ, aEventLocations[3].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0);
      break;
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case DARION_COMBAT_YELL:
      switch (urand(0, 8)) {
      case 0:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_1, m_creature);
        break;
      case 1:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_2, m_creature);
        break;
      case 2:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_3, m_creature);
        break;
      case 3:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_4, m_creature);
        break;
      case 4:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_5, m_creature);
        break;
      case 5:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_6, m_creature);
        break;
      case 6:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_7, m_creature);
        break;
      case 7:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_8, m_creature);
        break;
      case 8:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_9, m_creature);
        break;
      }
      ...
      if (!m_creature->IsWithinDist2d(aEventLocations[1].m_fX, aEventLocations[1].m_fY, 75.0f))
        m_creature->GetMotionMaster()->MovePoint(0, aEventLocations[1].m_fX, aEventLocations[1].m_fY,
                                                 aEventLocations[1].m_fZ);
    ...
    case DARION_DEFEAT_YELL:
      switch (urand(0, 6)) {
      case 0:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_10, m_creature);
        break;
      case 1:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_11, m_creature);
        break;
      case 2:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_12, m_creature);
        break;
      case 3:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_13, m_creature);
        break;
      case 4:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_14, m_creature);
        break;
      case 5:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_15, m_creature);
        break;
      case 6:
        DoScriptText(SAY_LIGHT_OF_DAWN_BATTLE_16, m_creature);
        break;
      }
...
bool GossipHello_npc_highlord_darion_mograine(Player *pPlayer, Creature *pCreature) {
  ...
  if (world_map_ebon_hold *pInstance = static_cast<world_map_ebon_hold *>(pCreature->GetInstanceData())) {
    if (pPlayer->GetQuestStatus(QUEST_ID_LIGHT_OF_DAWN) == QUEST_STATUS_INCOMPLETE &&
        pInstance->GetData(TYPE_BATTLE) == BATTLE_STATE_NOT_STARTED)
...
bool GossipSelect_npc_highlord_darion_mograine(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/,
                                               uint32 uiAction) {
  if (uiAction == GOSSIP_ACTION_INFO_DEF + 1) {
    if (world_map_ebon_hold *pInstance = static_cast<world_map_ebon_hold *>(pCreature->GetInstanceData())) {
...
struct npc_scarlet_courierAI : public ScriptedAI {
  ...
  void AttackedBy(Unit * /*pAttacker*/) override { m_creature->Unmount(); }
  ...
  void JustDied(Unit * /*pKiller*/) override { DoDespawnTree(); }
...
struct spell_death_knight_initiate_visual : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    ...
    case RACE_HUMAN:
      spellId = isMale ? 51520 : 51534;
      break;
    case RACE_DWARF:
      spellId = isMale ? 51538 : 51537;
      break;
    case RACE_NIGHTELF:
      spellId = isMale ? 51535 : 51536;
      break;
    case RACE_GNOME:
      spellId = isMale ? 51539 : 51540;
      break;
    case RACE_DRAENEI:
      spellId = isMale ? 51541 : 51542;
      break;
    case RACE_ORC:
      spellId = isMale ? 51543 : 51544;
      break;
    case RACE_UNDEAD:
      spellId = isMale ? 51549 : 51550;
      break;
    case RACE_TAUREN:
      spellId = isMale ? 51547 : 51548;
      break;
    case RACE_TROLL:
      spellId = isMale ? 51546 : 51545;
      break;
    case RACE_BLOODELF:
      spellId = isMale ? 51551 : 51552;
      break;
    default:
      return;
    }
...
struct spell_siphon_of_acherus : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    ...
    case 28525:
      spellId = 51973;
      break; // New Avalon Forge
    case 28542:
      spellId = 51979;
      break; // Scarlet Hold
    case 28543:
      spellId = 51976;
      break; // New Avalon Town Hall
    case 28544:
      spellId = 51981;
      break; // Chapel of the Crimson Flame
    default:
      return;
    }
...
struct spell_siphon_of_acherus_credit : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    ...
    case 51973:
      triggerSpellId = 51974;
      break; // New Avalon Forge
    case 51979:
      triggerSpellId = 51980;
      break; // Scarlet Hold
    case 51976:
      triggerSpellId = 51977;
      break; // New Avalon Town Hall
    case 51981:
      triggerSpellId = 51982;
      break; // Chapel of the Crimson Flame
    default:
      return;
    }
...
struct spell_gift_of_the_harvester : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override {
    ...
    caster->CastSpell(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(),
                      urand(0, 2) || summonedGhouls >= 5 ? 52505 : spellId, TRIGGERED_OLD_TRIGGERED);
  }
};
