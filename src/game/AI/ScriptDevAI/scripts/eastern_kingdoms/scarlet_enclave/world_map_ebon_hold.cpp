...
world_map_ebon_hold::world_map_ebon_hold(Map *pMap)
    : ScriptedInstance(pMap), m_uiGothikYellTimer(0), m_uiBattleEncounter(0), m_uiBattleProgressTimer(0),
      m_uiTrapUsageTimer(0), m_uiDefendersDead(0), m_uiAttackersDead(0), m_uiAbominationsDead(0) {
  Initialize();
}
...
void world_map_ebon_hold::OnCreatureDeath(Creature *pCreature) {
  ...
  case NPC_FLESH_BEHEMOTH: {
    ...
    if (GetData(TYPE_BATTLE) == BATTLE_STATE_IN_PROGRESS) {
      if (Creature *pBehemoth = pCreature->SummonCreature(NPC_FLESH_BEHEMOTH, aAbominationLocations[0].m_fX,
                                                          aAbominationLocations[0].m_fY, aAbominationLocations[0].m_fZ,
                                                          aAbominationLocations[0].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0)) {
  ...
  case NPC_RAMPAGING_ABOMINATION: {
    ...
    if (m_uiAbominationsDead % MAX_ABOMINATIONS == 0 && GetData(TYPE_BATTLE) == BATTLE_STATE_IN_PROGRESS) {
      for (uint8 i = 0; i < MAX_ABOMINATIONS; ++i) {
        if (Creature *pAbomination = pCreature->SummonCreature(
                aAbominationLocations[i].m_uiEntry, aAbominationLocations[i].m_fX, aAbominationLocations[i].m_fY,
                aAbominationLocations[i].m_fZ, aAbominationLocations[i].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0)) {
  ...
  case NPC_RIMBLAT_EARTHSHATTER:
  case NPC_RAYNE:
    pCreature->SummonCreature(pCreature->GetEntry(), pCreature->GetPositionX(), pCreature->GetPositionY(),
                              pCreature->GetPositionZ(), pCreature->GetOrientation(), TEMPSPAWN_CORPSE_DESPAWN, 0);
    break;
  }
}
...
void world_map_ebon_hold::DoSpawnUndeadWave() {
  ...
  for (uint8 i = 0; i < MAX_UNDEAD_WARRIORS; ++i) {
    ...
    pSource->GetRandomPoint(pSource->GetPositionX(), pSource->GetPositionY(), pSource->GetPositionZ(), 50.0f, fX, fY,
                            fZ);
  ...
  if (m_uiAttackersDead < MAX_UNDEAD_WARRIORS * 2) {
    for (uint8 i = 0; i < MAX_ABOMINATIONS; ++i) {
      if (Creature *pAbomination = pSource->SummonCreature(
              aAbominationLocations[i].m_uiEntry, aAbominationLocations[i].m_fX, aAbominationLocations[i].m_fY,
              aAbominationLocations[i].m_fZ, aAbominationLocations[i].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0)) {
        pAbomination->SetWalk(false);
        pAbomination->GetMotionMaster()->MovePoint(0, pSource->GetPositionX(), pSource->GetPositionY(),
                                                   pSource->GetPositionZ());
      }
    }
  }
}
...
InstanceData *GetInstance_world_map_ebon_hold(Map *pMap) { return new world_map_ebon_hold(pMap); }
