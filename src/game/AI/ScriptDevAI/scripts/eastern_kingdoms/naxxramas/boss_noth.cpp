...
enum {
  ...
  SPELL_DESPAWN_SUMMONS =
      30228, // Spell ID unsure: there are several spells doing the same thing in DBCs within Naxxramas spell IDs range
...
static uint32 const spellSummonPlaguedChampionWestList[4] = {SPELL_SUMMON_CHAMP07, SPELL_SUMMON_CHAMP08,
...
struct boss_nothAI : public ScriptedAI {
  ...
  uint8 m_phase;
  uint8 m_phaseSub;
  uint8 m_subWavesCount;  // On balcony phase, counts if this is the first or the second wave. Used to make Noth returns
                          // early
  ...
  void JustSummoned(Creature *summoned) override { summoned->SetInCombatWithZone(); }
  ...
  void SummonedJustReachedHome(Creature *summoned) override { summoned->ForcedDespawn(); }

  void KilledUnit(Unit * /*victim*/) override { DoScriptText(urand(0, 1) ? SAY_SLAY1 : SAY_SLAY2, m_creature); }
  ...
  void DoSummonPlaguedChampions(bool isThirdWave) {
    // Four Plagued Champions are to be summoned, one in each of the three corners and the fourth randomly along the
    // western wall On the third wave, we use specific spell to also summon a Plagued Construct in West part and
    // North-East corner
    if (DoCastSpellIfCan(m_creature, spellSummonPlaguedChampionSWList[urand(0, 1)], CAST_TRIGGERED) == CAST_OK)
      ++m_wavesNpcsCount; // We count the addition of a new NPC on each spell cast because JustSummoned() can possibly
                          // be called more than once per summoned NPC, making the count there incorrect
    ...
    if (DoCastSpellIfCan(m_creature,
                         (isThirdWave ? SPELL_SUMMON_CONSTR02 : spellSummonPlaguedChampionNEList[urand(0, 1)]),
                         CAST_TRIGGERED) == CAST_OK)
      (isThirdWave ? m_wavesNpcsCount += 2 : ++m_wavesNpcsCount);
    if (DoCastSpellIfCan(m_creature,
                         (isThirdWave ? SPELL_SUMMON_CONSTR01 : spellSummonPlaguedChampionWestList[urand(0, 3)]),
                         CAST_TRIGGERED) == CAST_OK)
      (isThirdWave ? m_wavesNpcsCount += 2 : ++m_wavesNpcsCount);
  }
      ...
      if (m_phaseTimer) // After PHASE_SKELETON_3 we won't have a balcony phase
      {
        if (m_phaseTimer <= diff) {
          ...
          if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT) == CAST_OK) {
            DoCastSpellIfCan(m_creature, SPELL_IMMUNE_ALL,
                             CAST_TRIGGERED); // Prevent players from damaging Noth when he is on the balcony
            ...
            case PHASE_SKELETON_1:
              m_phaseTimer = 70 * IN_MILLISECONDS;
              break;
            case PHASE_SKELETON_2:
              m_phaseTimer = 97 * IN_MILLISECONDS;
              break;
            case PHASE_SKELETON_3:
              m_phaseTimer = 120 * IN_MILLISECONDS;
              break;
            }
            return;
          } else
    ...
    } else // PHASE_BALCONY
    {
      if (m_phaseTimer < diff) {
        if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT_RETURN, CAST_TRIGGERED) == CAST_OK) {
          ...
          switch (m_phaseSub) {
          case PHASE_SKELETON_1:
            m_phaseTimer = 110 * IN_MILLISECONDS;
            break;
          case PHASE_SKELETON_2:
            m_phaseTimer = 180 * IN_MILLISECONDS;
            break;
          case PHASE_SKELETON_3:
            m_phaseTimer = 0;
            break;
      ...
      if (m_summonTimer < diff) {
        ...
        case PHASE_SKELETON_3: {
          DoSummonPlaguedChampions(true); // Set parameter to true because we need to indicate this is the third wave
                                          // and Plagued Construct need to be also summoned
          DoSummonPlaguedGuardians();
...
UnitAI *GetAI_boss_noth(Creature *creature) { return new boss_nothAI(creature); }
