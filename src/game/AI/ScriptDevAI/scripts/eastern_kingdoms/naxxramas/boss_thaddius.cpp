...
UnitAI *GetAI_boss_thaddius(Creature *creature) { return new boss_thaddiusAI(creature); }

bool EffectDummyNPC_spell_thaddius_encounter(Unit * /*caster*/, uint32 spellId, SpellEffectIndex effIndex,
...
// The superclass and add specific structure will use Feugen NPC as the master NPC that will initialise and synchronise
// both adds. This design is hinted in Classic spells DBCs
...
struct boss_thaddiusAddsAI : public ScriptedAI {
  ...
  // For Stalagg returns feugen, for Feugen returns stalagg
  {
    ...
    case NPC_FEUGEN:
      otherAdd = m_instance->GetSingleCreatureFromStorage(NPC_STALAGG);
      break;
    case NPC_STALAGG:
      otherAdd = m_instance->GetSingleCreatureFromStorage(NPC_FEUGEN);
      break;
    default:
      return nullptr;
    }
  ...
  bool DoMagneticPullIfCan() {
    if (!m_isFakingDeath && GetOtherAdd()) {
      if (DoCastSpellIfCan(GetOtherAdd(),
                           (m_creature->GetEntry() == NPC_FEUGEN ? SPELL_MAGNETIC_PULL_A : SPELL_MAGNETIC_PULL_B),
  ...
  bool IsCountingDead() const { return m_isFakingDeath || m_creature->IsDead(); }
...
struct boss_stalaggAI : public boss_thaddiusAddsAI {
  boss_stalaggAI(Creature *creature) : boss_thaddiusAddsAI(creature) { Reset(); }
  ...
  void JustDied(Unit * /*killer*/) override { DoScriptText(SAY_STAL_DEATH, m_creature); }
...
UnitAI *GetAI_boss_stalagg(Creature *creature) { return new boss_stalaggAI(creature); }
...
struct boss_feugenAI : public boss_thaddiusAddsAI {
  boss_feugenAI(Creature *creature) : boss_thaddiusAddsAI(creature) { Reset(); }
  ...
  void JustDied(Unit * /*killer*/) override { DoScriptText(SAY_FEUG_DEATH, m_creature); }
  ...
  void UpdateAddAI(const uint32 diff) {
    if (m_magneticPullTimer < diff) {
      if (!m_isFakingDeath &&
          GetOtherAdd(
              true)) // No need to check if we have a victim that Stalagg can pull: this is already done in super class
      {
...
UnitAI *GetAI_boss_feugen(Creature *creature) { return new boss_feugenAI(creature); }
