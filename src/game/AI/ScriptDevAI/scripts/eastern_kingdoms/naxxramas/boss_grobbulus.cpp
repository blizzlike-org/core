...
enum {
  ...
  SPELL_DESPAWN_SUMMONS =
      30134, // Spell ID unsure: there are several spells doing the same thing in DBCs within Naxxramas spell IDs range
...
struct boss_grobbulusAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_injectionTimer < diff) {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MUTATING_INJECTION,
...
UnitAI *GetAI_boss_grobbulus(Creature *creature) { return new boss_grobbulusAI(creature); }
