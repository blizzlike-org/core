...
static uint32 const spiritSpellList[4] = {SPELL_SPIRIT_BLAUMEUX, SPELL_SPIRIT_MOGRAINE, SPELL_SPIRIT_KORTHAZZ,
                                          SPELL_SPIRIT_ZELIEK};
static uint32 const markSpellList[4] = {SPELL_MARK_OF_BLAUMEUX, SPELL_MARK_OF_MOGRAINE, SPELL_MARK_OF_KORTHAZZ,
...
static const Location2DPoint resetCoords[3] = {{2577.3f, -3024.0f}, {2585.9f, -3015.1f}, {2594.6f, -3006.7f}};
struct boss_horsmenAI : public ScriptedAI {
  boss_horsmenAI(Creature *creature) : ScriptedAI(creature) {
    ...
    m_creature->GetCombatManager().SetLeashingCheck([&](Unit *, float x, float y, float /*z*/) {
      return (x > resetCoords[0].x && y < resetCoords[0].y) || (x > resetCoords[1].x && y < resetCoords[1].y) ||
  ...
  void Reset() override {
    ...
    m_markCounter = 0;
    m_horsmenIndex =
        INDEX_BLAUMEUX; // Default: Lady Blaumeux (1: Highlord Morgraine, 2: Thane Korth'azz, 3: Sir Zeliek)
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(killSayList[m_horsmenIndex], m_creature); }
...
UnitAI *GetAI_boss_horsmen(Creature *creature) { return new boss_horsmenAI(creature); }
...
struct boss_lady_blaumeuxAI : public boss_horsmenAI {
  ...
  void UpdateHorsmenAI(const uint32 diff) override {
    if (m_voidZoneTimer < diff) {
      Unit *pTarget =
          m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_VOID_ZONE, SELECT_FLAG_PLAYER);
      if (pTarget) {
...
UnitAI *GetAI_boss_lady_blaumeux(Creature *creature) { return new boss_lady_blaumeuxAI(creature); }
...
UnitAI *GetAI_boss_alexandros_mograine(Creature *creature) { return new boss_alexandros_mograineAI(creature); }
...
UnitAI *GetAI_boss_thane_korthazz(Creature *creature) { return new boss_thane_korthazzAI(creature); }
...
UnitAI *GetAI_boss_sir_zeliek(Creature *creature) { return new boss_sir_zeliekAI(creature); }
