...
static const DialogueEntry naxxDialogue[] = {{NPC_KELTHUZAD, 0, 10000},
...
instance_naxxramas::instance_naxxramas(Map *pMap)
    : ScriptedInstance(pMap), DialogueHelper(naxxDialogue), m_sapphironSpawnTimer(0), m_tauntTimer(0),
      m_horsemenKilled(0), m_livingPoisonTimer(0), m_screamsTimer(2 * MINUTE * IN_MILLISECONDS),
      m_horsemenTauntTimer(30 * MINUTE * IN_MILLISECONDS), m_despawnKTTriggerTimer(0), m_checkGuardiansTimer(0),
      m_shackledGuardians(0), isFaerlinaIntroDone(false) {
  Initialize();
}
...
void instance_naxxramas::OnPlayerEnter(Player *player) {
  ...
  player->SummonCreature(NPC_SAPPHIRON, sapphironPositions[0], sapphironPositions[1], sapphironPositions[2],
                         sapphironPositions[3], TEMPSPAWN_DEAD_DESPAWN, 0);
}
...
void instance_naxxramas::OnCreatureCreate(Creature *creature) {
  ...
  case NPC_SUB_BOSS_TRIGGER:
    m_gothikTriggerList.push_back(creature->GetObjectGuid());
    break;
  case NPC_TESLA_COIL:
    m_thaddiusTeslaCoilList.push_back(creature->GetObjectGuid());
    break;
...
void instance_naxxramas::OnObjectCreate(GameObject *gameObject) {
  ...
  case GO_KELTHUZAD_TRIGGER:
    if (m_auiEncounter[TYPE_KELTHUZAD] !=
        NOT_STARTED) // Only spawn the visual trigger for Kel'Thuzad when encounter is not started
...
void instance_naxxramas::OnCreatureDeath(Creature *creature) {
  ...
  case NPC_UNREL_TRAINEE:
    if (Creature *anchor = GetClosestAnchorForGothik(creature, true))
      creature->CastSpell(anchor, SPELL_A_TO_ANCHOR_1, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
  ...
  case NPC_UNREL_DEATH_KNIGHT:
    if (Creature *anchor = GetClosestAnchorForGothik(creature, true))
      creature->CastSpell(anchor, SPELL_B_TO_ANCHOR_1, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                          creature->GetObjectGuid());
    m_unrelentingSideList.remove(creature->GetObjectGuid());
    creature->ForcedDespawn(4000);
    break;
  case NPC_UNREL_RIDER:
    if (Creature *anchor = GetClosestAnchorForGothik(creature, true))
      creature->CastSpell(anchor, SPELL_C_TO_ANCHOR_1, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                          creature->GetObjectGuid());
    m_unrelentingSideList.remove(creature->GetObjectGuid());
    creature->ForcedDespawn(4000);
    break;
...
void instance_naxxramas::SetData(uint32 type, uint32 data) {
  ...
  case TYPE_HEIGAN:
    m_auiEncounter[type] = data;
    // Open the entrance door on encounter win or failure (we specifically set the GOState to avoid issue in case
    // encounter is reset before gate is closed in Heigan script)
  ...
  case TYPE_FOUR_HORSEMEN: {
    ...
    std::vector<uint32> entries = {NPC_SPIRIT_OF_BLAUMEUX, NPC_SPIRIT_OF_MOGRAINE, NPC_SPIRIT_OF_KORTHAZZ,
                                   NPC_SPIRIT_OF_ZELIREK};
    for (uint32 entry : entries) {
  ...
  if (m_livingPoisonTimer) {
    if (m_livingPoisonTimer <= diff) {
      if (Creature *trigger = GetSingleCreatureFromStorage(NPC_NAXXRAMAS_TRIGGER)) {
        ...
        for (uint8 i = 0; i < 3; i++)
          if (Creature *poison = trigger->SummonCreature(NPC_LIVING_POISON, livingPoisonPositions[i].m_fX,
                                                         livingPoisonPositions[i].m_fY, livingPoisonPositions[i].m_fZ,
                                                         livingPoisonPositions[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
            poison->GetMotionMaster()->MovePoint(0, livingPoisonPositions[i + 3].m_fX,
                                                 livingPoisonPositions[i + 3].m_fY, livingPoisonPositions[i + 3].m_fZ);
...
// Initialize all triggers used in Gothik the Harvester encounter by flagging them with their position in the room and
// what kind of NPC they will summon
...
struct npc_stoneskin_gargoyleAI : public ScriptedAI {
  ...
  void JustReachedHome() override { TryStoneForm(); }

  void MoveInLineOfSight(Unit *pWho) override {
    if (m_creature->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE)) {
      if (pWho->GetTypeId() == TYPEID_PLAYER && !m_creature->IsInCombat() &&
          m_creature->IsWithinDistInMap(pWho, 17.0f) && !pWho->HasAuraType(SPELL_AURA_FEIGN_DEATH) &&
