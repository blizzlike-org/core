...
enum {
  ...
  SPELL_ANUB_AURA = 29103,   // Periodically apply aura 29104 onto players to handle the corpse scarabs summon when they
                             // die (spell 29105)
  ...
  //  SPELL_DELAY_SUMMON_SCARABS1 = 28992,                    // Probably used to prevent two dead Crypt Guards to
  //  explode in a short time SPELL_DELAY_SUMMON_SCARABS2 = 28994,                    // Probably used to prevent two
  //  dead Crypt Guards to explode in a short time by triggering same spell cooldown than 28961
};

static const DialogueEntry introDialogue[] = {
    {SAY_GREET1, NPC_ANUB_REKHAN, 7000},  {SAY_GREET2, NPC_ANUB_REKHAN, 13000}, {SAY_GREET3, NPC_ANUB_REKHAN, 11000},
    {SAY_GREET4, NPC_ANUB_REKHAN, 10000}, {SAY_GREET5, NPC_ANUB_REKHAN, 0},
};

struct boss_anubrekhanAI : public ScriptedAI {
  boss_anubrekhanAI(Creature *creature) : ScriptedAI(creature), m_introDialogue(introDialogue) {
  ...
  void EnterEvadeMode() override {
    // We despawn the guardians before entering evade mode to prevent despawning also the static adds that are linked to
    // respawn on evade
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_locustSwarmTimer < diff) {
      if (DoCastSpellIfCan(m_creature, SPELL_LOCUSTSWARM) == CAST_OK) {
        ...
        m_impaleTimer +=
            23 * IN_MILLISECONDS; // Delay next Impale by Locust Swarm duration (20 sec) + casting time (3 sec), this
                                  // prevent Impale to be always cast right after Locust Swarm ends
      }
    } else
...
UnitAI *GetAI_boss_anubrekhan(Creature *creature) { return new boss_anubrekhanAI(creature); }

bool GOUse_go_anub_door(Player * /*pPlayer*/, GameObject *go) {
  if (ScriptedInstance *instance = (ScriptedInstance *)go->GetInstanceData()) {
    if (instance->GetData(TYPE_ANUB_REKHAN) == IN_PROGRESS ||
        instance->GetData(TYPE_ANUB_REKHAN) ==
            DONE) // GOs always open once used (or handled by script), so this check is only there for safety
      return false;
    else {
