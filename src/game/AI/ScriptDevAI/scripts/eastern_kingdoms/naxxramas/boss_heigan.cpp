...
SDComment: Missing poison inside the eye stalk tunnel in phase 2
           Candidate spell is 30122 (correct damage range and already used in encounter) but there is no evidence of
this and of how the spell is cast and by who (no data found in sniffs as this part was removed in WotLK) SDCategory:
Naxxramas EndScriptData */
...
enum {
  ...
  SPELL_PLAGUE_WAVE_SLOW =
      29351, // Activates the traps during phase 1; triggers spell 30116, 30117, 30118, 30119 each 10 secs
  SPELL_PLAGUE_WAVE_FAST =
      30114, // Activates the traps during phase 2; triggers spell 30116, 30117, 30118, 30119 each 3 secs
...
struct boss_heiganAI : public ScriptedAI {
  ...
  void Reset() override {
    ...
    ResetPhase();
    m_eruptionStartDelay =
        5 *
        IN_MILLISECONDS; // Override value from ResetPhase() on combat start only: 5 more seconds are given at that time
      ...
      if (m_teleportTimer) {
        if (m_teleportTimer < diff) {
          ...
          m_creature->SelectAttackingTargets(targets, ATTACKING_TARGET_ALL_SUITABLE, 0, nullptr,
                                             SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_TANK, m_teleportParams);
...
UnitAI *GetAI_boss_heigan(Creature *creature) { return new boss_heiganAI(creature); }
...
struct npc_diseased_maggotAI : public ScriptedAI {
  ...
  void Reset() override { m_resetCheckTimer = 3 * IN_MILLISECONDS; }
...
UnitAI *GetAI_npc_diseased_maggot(Creature *creature) { return new npc_diseased_maggotAI(creature); }
