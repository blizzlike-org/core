...
struct boss_faerlinaAI : public CombatAI {
  boss_faerlinaAI(Creature *creature)
      : CombatAI(creature, FAERLINA_ACTION_MAX),
  ...
  void SpellHit(Unit * /*caster*/, const SpellEntry *spellEntry) override {
    if (spellEntry->Id == SPELL_WIDOWS_EMBRACE_1) {
      DoCastSpellIfCan(
          m_creature,
          SPELL_WIDOWS_EMBRACE_2); // Start spell category cooldown, delaying next Poison Bolt and Frenzy by 30 secs

      if (m_creature->HasAura(SPELL_ENRAGE)) {
        m_creature->RemoveAurasDueToSpell(SPELL_ENRAGE);
        ResetCombatAction(FAERLINA_ENRAGE,
                          60 * IN_MILLISECONDS); // Delay next enrage by 60 sec if Faerlina was already enraged
      }
    }
  }
    ...
    case FAERLINA_ENRAGE: {
      if (!m_creature->HasAura(SPELL_ENRAGE)) {
        if (DoCastSpellIfCan(m_creature, SPELL_ENRAGE) == CAST_OK) {
          switch (urand(0, 2)) {
          case 0:
            DoScriptText(SAY_ENRAGE_1, m_creature);
            break;
          case 1:
            DoScriptText(SAY_ENRAGE_2, m_creature);
            break;
          case 2:
            DoScriptText(SAY_ENRAGE_3, m_creature);
            break;
          }
