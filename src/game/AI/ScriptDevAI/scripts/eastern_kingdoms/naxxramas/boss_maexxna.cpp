...
struct boss_maexxnaAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    // Web Wrap
    // ToDo: the targets and triggers selection is probably done through spells 29280, 29281, 29282, 29283, 29285 &
    // 29287 But because we are walking in Guessland until more reliable data are available, let's do the selection
    // manually here
    if (m_webWrapTimer < diff) {
      ...
      m_creature->SelectAttackingTargets(targets, ATTACKING_TARGET_ALL_SUITABLE, 0, nullptr,
                                         SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_TANK, m_webWrapParams);
      ...
      if (!targets.empty()) {
        ...
        if (m_summoningTriggers.size() < targets.size()) {
          script_error_log("Error in script Naxxramas::boss_maexxna: less summoning NPCs (entry %u) than expected "
                           "targets (%u) for Web Wrap ability. Check your DB",
                           NPC_INVISIBLE_MAN, MAX_PLAYERS_WEB_WRAP);
          return;
        }
        ...
        for (uint8 i = 0; i < targets.size(); i++)
          targets[i]->CastSpell(invisibleMen[i], SPELL_WEB_WRAP_INIT, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
    ...
    if (m_doWebWrapVisualTimer) {
      if (m_doWebWrapVisualTimer < diff) {
        if (Player *victim = m_creature->GetMap()->GetPlayer(m_victimGuid)) {
          if (victim->IsAlive()) {
            // Make the player cast the visual effects spells with a delay to ensure he/she has reach his/her
            // destination
...
UnitAI *GetAI_npc_web_wrap(Creature *creature) { return new npc_web_wrapAI(creature); }

UnitAI *GetAI_boss_maexxna(Creature *creature) { return new boss_maexxnaAI(creature); }

UnitAI *GetAI_npc_invible_man(Creature *creature) { return new npc_invisible_manAI(creature); }
