...
SDName: Boss_Gluth
SD%Complete: 99
SDComment: Though gameplay is Blizzlike, logic in Zombie Chow Search and Call All Zombie Chow is guess work and can
probably be improved. Also, based on similar encounters, Trigger NPCs for the summoning should probably be spawned by a
(currently unknown) spell and the despawn of all adds should also be handled by a spell. SDCategory: Naxxramas
EndScriptData */
...
struct boss_gluthAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*who*/) override {
    ...
    DoCastSpellIfCan(m_creature, SPELL_ZOMBIE_CHOW_SEARCH,
                     CAST_TRIGGERED | CAST_AURA_NOT_PRESENT); // Aura periodically looking for NPC 16360
    DoCastSpellIfCan(m_creature, SPELL_CALL_ALL_ZOMBIE_CHOW,
                     CAST_TRIGGERED | CAST_AURA_NOT_PRESENT); // Aura periodically calling all NPCs 16360
...
UnitAI *GetAI_boss_gluth(Creature *creature) { return new boss_gluthAI(creature); }
