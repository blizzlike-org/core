...
struct boss_gothikAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*who*/) override {
    ...
    m_summonTimer = 4 * IN_MILLISECONDS; // First spawn of trainees 24 secs after engage, but the periodic summoning
                                         // aura already has an internal 20 secs timer
  }
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    case PHASE_SPEECH:
      if (m_speechTimer < diff) {
        ...
        case 1:
          ...
          m_speechTimer = 4 * IN_MILLISECONDS;
          break;
        case 2:
          DoScriptText(SAY_SPEECH_2, m_creature);
          m_speechTimer = 6 * IN_MILLISECONDS;
          break;
        case 3:
          DoScriptText(SAY_SPEECH_3, m_creature);
          m_speechTimer = 5 * IN_MILLISECONDS;
          break;
        case 4:
    ...
    case PHASE_TELEPORTING: // Phase is only reached if m_instance is valid
      if (m_teleportTimer < diff) {
        uint32 teleportSpellId =
            m_instance->IsInRightSideGothikArea(m_creature) ? SPELL_TELEPORT_LEFT : SPELL_TELEPORT_RIGHT;
    ...
    case PHASE_STOP_TELEPORTING:
      ...
      // Shadowbolt cooldown finished, cast when ready
        ...
        // Select valid target
        if (Unit *target =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOWBOLT, SELECT_FLAG_IN_LOS))
...
UnitAI *GetAI_boss_gothik(Creature *creature) { return new boss_gothikAI(creature); }

bool EffectDummyCreature_spell_anchor(Unit * /*caster*/, uint32 spellId, SpellEffectIndex effIndex,
          ...
          for (const auto &playerInList : playersList) {
            if (Player *player = playerInList.getSource()) {
              ...
              if (spell->m_spellInfo->Id == SPELL_CHECK_UNRELENTING_SIDE && instance->IsInRightSideGothikArea(player))
                caster->AI()->AttackClosestEnemy();
              else if (spell->m_spellInfo->Id == SPELL_CHECK_SPECTRAL_SIDE &&
                       !instance->IsInRightSideGothikArea(player))
                caster->AI()->AttackClosestEnemy();
              else
                return;
            }
