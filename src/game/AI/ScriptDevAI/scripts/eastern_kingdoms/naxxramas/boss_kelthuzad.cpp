...
enum {
  ...
  SPELL_GUARDIAN_INIT = 29898,     // Cast at start of phase 3 by Kel'Thuzad (start of phase 2 in sniffs but this way we
                                   // avoid running a timer needlessly)

  MAX_CONTROLLED_TARGETS =
      4, // 5 targets are mind controlled by Chains of Kel'Thuzad but the main tank is always the fifth one
};
...
static const std::map<uint32, std::vector<uint32>> summoningSpells{
    {NPC_SOUL_WEAVER,
     {SPELL_SUMMON_PERIODIC_C_1, SPELL_SUMMON_PERIODIC_C_2, SPELL_SUMMON_PERIODIC_C_3, SPELL_SUMMON_PERIODIC_C_4,
      SPELL_SUMMON_PERIODIC_C_5}},
    {NPC_UNSTOPPABLE_ABOM,
     {SPELL_SUMMON_PERIODIC_B_1, SPELL_SUMMON_PERIODIC_B_2, SPELL_SUMMON_PERIODIC_B_3, SPELL_SUMMON_PERIODIC_B_4,
      SPELL_SUMMON_PERIODIC_B_5}},
    {NPC_SOLDIER_FROZEN,
     {SPELL_SUMMON_PERIODIC_A_1, SPELL_SUMMON_PERIODIC_A_2, SPELL_SUMMON_PERIODIC_A_3, SPELL_SUMMON_PERIODIC_A_4,
      SPELL_SUMMON_PERIODIC_A_5}}};
...
struct boss_kelthuzadAI : public ScriptedAI {
  ...
  void Reset() override {
    ...
    m_guardiansSummonTimer =
        61 * IN_MILLISECONDS; // One Guardian of Icecrown is summoned every 10 sec ; five of them in total
  ...
  void SpellHit(Unit * /*caster*/, const SpellEntry *spell) override {
    ...
    // Phase 1 periodic update every 1 second (everything in phase 1 is handled here rather than in UpdateAI())
      ...
      default:
        UpdateSummoning();
        // Kel'Thuzad does not enter combat until phase 2, so we check every second if there are still players alive and
        // force him to evade otherwise
  ...
  void UpdateSummoning() {
    for (auto add : phaseOneAdds) {
      ...
      // Clear all summoning auras for current add before applying the new one (especially needed in case new one is
      // NULL)
  ...
  bool SummonIntroCreatures(uint32 entry, uint8 count) {
    ...
    for (auto &trigger : m_summoningTriggers) {
      ...
      for (uint8 i = 0; i < count; ++i) {
        m_creature->GetRandomPoint(trigger->GetPositionX(), trigger->GetPositionY(), trigger->GetPositionZ(), 12.0f,
                                   newX, newY, newZ);
        if (Creature *summoned = m_creature->SummonCreature(entry, newX, newY, newZ, 0.0f, TEMPSPAWN_CORPSE_DESPAWN,
                                                            5 * MINUTE * IN_MILLISECONDS)) {
          if (summoned->AI())
            summoned->AI()->SetReactState(
                REACT_PASSIVE); // Intro mobs only attack if engaged or hostile target in range
        }
      }
    }
    return true;
  ...
  void UpdateAI(const uint32 diff) override {
    ...
    if (m_manaDetonationTimer < diff) {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MANA_DETONATION,
...
UnitAI *GetAI_boss_kelthuzad(Creature *creature) { return new boss_kelthuzadAI(creature); }
...
struct npc_icecrown_guardianAI : public ScriptedAI {
  ...
  void KilledUnit(Unit * /*victim*/) override { DoCastSpellIfCan(m_creature->GetVictim(), SPELL_BLOOD_TAP); }
...
UnitAI *GetAI_npc_icecrown_guardian(Creature *creature) { return new npc_icecrown_guardianAI(creature); }
...
struct TriggerKTAdd : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex /* effIdx */) const override {
    if (Unit *unitTarget = spell->GetUnitTarget()) {
      ...
      case 28415:
        unitTarget->CastSpell(unitTarget, SPELL_SUMMON_TYPE_A, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                              spell->GetCaster()->GetObjectGuid(), nullptr);
        break;
      case 28416:
        unitTarget->CastSpell(unitTarget, SPELL_SUMMON_TYPE_B, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                              spell->GetCaster()->GetObjectGuid(), nullptr);
        break;
      case 28417:
        unitTarget->CastSpell(unitTarget, SPELL_SUMMON_TYPE_C, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                              spell->GetCaster()->GetObjectGuid(), nullptr);
        break;
      case 28455:
        unitTarget->CastSpell(unitTarget, SPELL_SUMMON_TYPE_D, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                              spell->GetCaster()->GetObjectGuid(), nullptr);
        break;
      default:
        break;
...
struct ChainsKelThuzad : public SpellScript {
  void OnEffectExecute(Spell *spell, SpellEffectIndex /* effIdx */) const override {
    ...
    caster->SelectAttackingTargets(targets, ATTACKING_TARGET_ALL_SUITABLE, 0, nullptr,
                                   SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_TANK, chainsParams);
...
struct FrostBlast : public AuraScript {
  void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData & /* data */) const override {
    ...
    if (Unit *target = aura->GetTarget()) {
      ...
      caster->CastCustomSpell(target, SPELL_FROST_BLAST_DAMAGE, &basePointsDamage, nullptr, nullptr,
...
struct GuardianPeriodic : public AuraScript {
  void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData &data) const override {
    if (Unit *target = aura->GetTarget()) {
      if (target->HasAuraType(SPELL_AURA_MOD_STUN)) // Shackle Undead applies SPELL_AURA_MOD_STUN, so we cover all ranks
                                                    // of the spell this way
