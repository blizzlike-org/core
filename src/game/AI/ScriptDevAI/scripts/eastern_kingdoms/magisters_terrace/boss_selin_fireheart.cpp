...
struct boss_selin_fireheartAI : public CombatAI {
  boss_selin_fireheartAI(Creature *creature)
      : CombatAI(creature, SELIN_ACTION_MAX),
        m_instance(static_cast<instance_magisters_terrace *>(creature->GetInstanceData())),
  ...
  uint32 GetSubsequentActionTimer(uint32 id) {
    ...
    case SELIN_ACTION_DRAIN_LIFE:
      return urand(12000, 22000);
    case SELIN_ACTION_DRAIN_MANA:
      return urand(17500, 25000);
    case SELIN_ACTION_DRAIN_CRYSTAL:
      return m_isRegularMode ? urand(40000, 55000) : urand(30000, 40000);
    default:
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case SELIN_ACTION_DRAIN_LIFE: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DRAIN_LIFE,
    ...
    case SELIN_ACTION_DRAIN_MANA: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DRAIN_MANA,
                                                SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER | SELECT_FLAG_POWER_MANA))
...
UnitAI *GetAI_boss_selin_fireheart(Creature *creature) { return new boss_selin_fireheartAI(creature); };
...
struct mob_fel_crystalAI : public ScriptedAI {
  ...
  void JustRespawned() override { m_creature->CastSpell(nullptr, SPELL_FEL_CRYSTAL_VISUAL, TRIGGERED_NONE); }
...
UnitAI *GetAI_mob_fel_crystal(Creature *creature) { return new mob_fel_crystalAI(creature); };
