...
static const uint32 aDelrissaLackeys[MAX_COMPANIONS] = {NPC_KAGANI,  NPC_ELLRYS,   NPC_ERAMAS, NPC_YAZZAI,
                                                        NPC_SALARIS, NPC_GARAXXAS, NPC_APOKO,  NPC_ZELFAN};
...
struct boss_priestess_delrissaAI : public CombatAI {
  boss_priestess_delrissaAI(Creature *creature)
      : CombatAI(creature, DELRISSA_ACTION_MAX),
  ...
  void DoInitializeCompanions() {
    ...
    if (m_vuiLackeyEnties.empty()) {
      ...
      for (uint8 i = 0; i < MAX_DELRISSA_ADDS; ++i)
        m_creature->SummonCreature(m_vuiLackeyEnties[i], aLackeyLocations[i][0], aLackeyLocations[i][1],
                                   aLackeyLocations[i][2], aLackeyLocations[i][3], TEMPSPAWN_CORPSE_DESPAWN, 0);
    }
  ...
  virtual void ExecuteActions() override {
    if (!m_creature->IsAlive() || m_creature->GetCombatManager().IsEvadingHome() ||
...
UnitAI *GetAI_boss_priestess_delrissa(Creature *creature) { return new boss_priestess_delrissaAI(creature); }
...
struct priestess_companion_commonAI : public CombatAI {
  priestess_companion_commonAI(Creature *creature, uint32 actions)
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case KAGANI_VANISH:
      if (DoCastSpellIfCan(nullptr, SPELL_VANISH) == CAST_OK) {
        ...
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0),
                                                             SELECT_FLAG_POWER_MANA | SELECT_FLAG_PLAYER)) {
          DoResetThreat();
          AttackStart(target);
        }
...
UnitAI *GetAI_npc_kagani_nightstrike(Creature *creature) { return new npc_kagani_nightstrikeAI(creature); }
...
UnitAI *GetAI_npc_ellris_duskhallow(Creature *creature) { return new npc_ellris_duskhallowAI(creature); }
...
UnitAI *GetAI_npc_eramas_brightblaze(Creature *creature) { return new npc_eramas_brightblazeAI(creature); }
...
UnitAI *GetAI_npc_yazzai(Creature *creature) { return new npc_yazzaiAI(creature); }
...
struct npc_warlord_salarisAI : public priestess_companion_commonAI {
  ...
  void Aggro(Unit * /*who*/) override { DoCastSpellIfCan(m_creature, SPELL_BATTLE_SHOUT); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case SALARIS_INTERCEPT_STUN:
      ...
      if (!m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_IN_MELEE_RANGE)) {
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_INTERCEPT_STUN,
...
UnitAI *GetAI_npc_warlord_salaris(Creature *creature) { return new npc_warlord_salarisAI(creature); }
...
UnitAI *GetAI_npc_garaxxas(Creature *creature) { return new npc_garaxxasAI(creature); }
...
struct npc_apokoAI : public priestess_companion_commonAI {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case APOKO_TOTEM:
      ...
      switch (urand(0, 2)) {
      case 0:
        DoCastSpellIfCan(nullptr, SPELL_WINDFURY_TOTEM);
        break;
      case 1:
        DoCastSpellIfCan(nullptr, SPELL_FIRE_NOVA_TOTEM);
        break;
      case 2:
        DoCastSpellIfCan(nullptr, SPELL_EARTHBIND_TOTEM);
        break;
      }
    ...
    case APOKO_HEALING_WAVE:
      if (Unit *target = DoSelectLowestHpFriendly(50.0f))
        if (DoCastSpellIfCan(target, m_isRegularMode ? SPELL_LESSER_HEALING_WAVE : SPELL_LESSER_HEALING_WAVE_H) ==
            CAST_OK)
          ResetCombatAction(action, 5000);
      break;
    case APOKO_FROST_SHOCK:
      if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_FROST_SHOCK : SPELL_FROST_SHOCK_H) ==
          CAST_OK)
        ResetCombatAction(action, 7000);
      break;
    }
...
UnitAI *GetAI_npc_apoko(Creature *creature) { return new npc_apokoAI(creature); }
...
UnitAI *GetAI_npc_zelfan(Creature *creature) { return new npc_zelfanAI(creature); }
