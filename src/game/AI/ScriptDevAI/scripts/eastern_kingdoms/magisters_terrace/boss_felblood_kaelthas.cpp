...
enum {
  ...
  SPELL_GRAVITY_LAPSE_VISUAL = 44251, // Channeled; blue beam animation to every enemy in range - when removed the
                                      // Gravity Lapse auras are removed from players
  ...
  SPELL_GRAVITY_LAPSE_FLY =
      44227, // Hastens flyspeed and allows flying for 1 minute. Requires aura stacking exception for 44226.
...
struct boss_felblood_kaelthasAI : public CombatAI {
  boss_felblood_kaelthasAI(Creature *creature)
      : CombatAI(creature, KAEL_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
        m_isRegularMode(creature->GetMap()->IsRegularDifficulty()), m_introStarted(false) {
    ...
    AddCustomAction(KAEL_INTRO, true, [&]() { HandleIntro(); });
    AddCustomAction(KAEL_GRAVITY_LAPSE_SCRIPT, true, [&]() { HandleGravityLapse(); });
    AddCustomAction(KAEL_OUTRO, true, [&]() { HandleOutro(); });
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case KAEL_ACTION_FLAMESTRIKE: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_FLAME_STRIKE, SELECT_FLAG_PLAYER)) {
...
struct mob_arcane_sphereAI : public ScriptedAI {
  mob_arcane_sphereAI(Creature *creature)
...
UnitAI *GetAI_boss_felblood_kaelthas(Creature *creature) { return new boss_felblood_kaelthasAI(creature); }

UnitAI *GetAI_mob_arcane_sphere(Creature *creature) { return new mob_arcane_sphereAI(creature); }
