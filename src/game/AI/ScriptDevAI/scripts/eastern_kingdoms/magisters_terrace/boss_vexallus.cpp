...
struct boss_vexallusAI : public CombatAI {
  boss_vexallusAI(Creature *creature)
      : CombatAI(creature, VEXALLUS_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    switch (action) {
    case VEXALLUS_ACTION_CHAIN_LIGHTNING: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CHAIN_LIGHTNING,
    ...
    case VEXALLUS_ACTION_SHOCK: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_ARCANE_SHOCK,
                                                           SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER))
...
UnitAI *GetAI_boss_vexallus(Creature *creature) { return new boss_vexallusAI(creature); };
...
UnitAI *GetAI_mob_pure_energy(Creature *creature) { return new mob_pure_energyAI(creature); };
