...
instance_blackwing_lair::instance_blackwing_lair(Map *pMap)
    : ScriptedInstance(pMap), m_uiResetTimer(0), m_uiDefenseTimer(0), m_uiScepterEpicTimer(0),
      m_uiNefarianSpawnTimer(0), m_uiScepterQuestStep(0), m_uiDragonspawnCount(0), m_uiBlackwingDefCount(0),
      m_uiDeadDrakonidsCount(0), m_bIsMainGateOpen(true) {
  Initialize();
}

void instance_blackwing_lair::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_blackwing_lair::SetData(uint32 uiType, uint32 uiData) {
  ...
  // If an encounter is failed or won, open the main gate only if it is currently closed and no other event is in
  // progress
  ...
  case TYPE_NEFA_RTUNNEL:
    m_auiEncounter[uiType] =
        uiData; // Store the spell IDs/NPC entries of the two breaths/drakonids used by Chromaggus/Nefarian for all the
                // instance lifetime. Breaths/Drakonids are picked randomly in Chromaggus/Nefarian script
    break;
  }
...
bool instance_blackwing_lair::CheckConditionCriteriaMeet(Player const *pPlayer, uint32 uiInstanceConditionId,
  ...
  script_error_log("instance_blackwing_lair::CheckConditionCriteriaMeet called with unsupported Id %u. Called with "
                   "param plr %s, src %s, condition source type %u",
                   uiInstanceConditionId, pPlayer ? pPlayer->GetGuidStr().c_str() : "NULL",
                   pConditionSource ? pConditionSource->GetGuidStr().c_str() : "NULL", conditionSourceType);
  return false;
}
...
void instance_blackwing_lair::Update(uint32 uiDiff) {
  ...
  if (m_uiNefarianSpawnTimer) {
    if (m_uiNefarianSpawnTimer <= uiDiff) {
      if (Creature *nefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS)) {
        nefarius->SummonCreature(NPC_NEFARIAN, aNefarianLocs[2].m_fX, aNefarianLocs[2].m_fY, aNefarianLocs[2].m_fZ, 0,
                                 TEMPSPAWN_DEAD_DESPAWN, 0, true);
        // Despawn Nefarius
  ...
  if (m_uiDefenseTimer < uiDiff) {
    ...
    for (uint8 i = 0; i < MAX_EGGS_DEFENDERS; ++i) {
      ...
      uint8 uiMaxRange = (m_uiDragonspawnCount < MAX_DRAGONSPAWN
                              ? 3
                              : 1); // If all dragonspawns are already spawned, don't roll for them below
...
void instance_blackwing_lair::InitiateDrakonid(uint32 uiEventId) {
  ...
  // Left tunnel
  case 8520:
    leftTunnel = NPC_BLACK_SPAWNER;
    break;
  case 8521:
    leftTunnel = NPC_RED_SPAWNER;
    break;
  case 8522:
    leftTunnel = NPC_GREEN_SPAWNER;
    break;
  case 8523:
    leftTunnel = NPC_BRONZE_SPAWNER;
    break;
  case 8524:
    leftTunnel = NPC_BLUE_SPAWNER;
    break;
  // Right tunnel
  case 8525:
    rightTunnel = NPC_BLACK_SPAWNER;
    break;
  case 8526:
    rightTunnel = NPC_RED_SPAWNER;
    break;
  case 8527:
    rightTunnel = NPC_GREEN_SPAWNER;
    break;
  case 8528:
    rightTunnel = NPC_BRONZE_SPAWNER;
    break;
  case 8529:
    rightTunnel = NPC_BLUE_SPAWNER;
    break;
  }
  if (leftTunnel) {
    ...
    debug_log("SD2 Instance Blackwing Lair: Nefarian's lair left tunnel set with drakonid spawner %u",
              GetData(TYPE_NEFA_LTUNNEL));
  }
  if (rightTunnel) {
...
struct go_ai_suppression : public GameObjectAI {
  ...
  // Visual effects for each GO is played on a 5 seconds timer. Sniff show that the GO should also be used (trap spell
  // is cast) but we need core support for GO casting for that
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiFumeTimer) {
      if (m_uiFumeTimer <= uiDiff) {
        // TODO replace by go->Use(go) or go->Use(nullptr) once GO casting is added in core
        // The loot state check may be removed in that case because it should probably be handled in the
        // Gameobject::Use() code
...
GameObjectAI *GetAI_go_suppression(GameObject *go) { return new go_ai_suppression(go); }

/*###################################################
## Chromaggus' breaths and Nefarian tunnels selection
####################################################*/

bool ProcessEventId_event_weekly_chromatic_selection(uint32 uiEventId, Object *pSource, Object * /*pTarget*/,
                                                     bool /*bIsStart*/) {
