...
struct boss_vaelastraszAI : public CombatAI {
  boss_vaelastraszAI(Creature *creature)
      : CombatAI(creature, VAEL_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
  ...
  void HandleIntro() {
    ...
    case 0: {
      ...
      m_creature->SummonCreature(NPC_LORD_VICTOR_NEFARIUS, aNefariusSpawnLoc[0], aNefariusSpawnLoc[1],
                                 aNefariusSpawnLoc[2], aNefariusSpawnLoc[3], TEMPSPAWN_TIMED_DESPAWN, 25000);
  ...
  void HandleSpeech() {
    ...
    case 0:
      ...
      break;
    case 1:
      // This one is actually 16 seconds but we only go to 10 seconds because he starts attacking after he says "I must
      // fight this!"
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case VAEL_BURNING_ADRENALINE_TARGET: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BURNING_ADRENALINE,
...
UnitAI *GetAI_boss_vaelastrasz(Creature *creature) { return new boss_vaelastraszAI(creature); }
