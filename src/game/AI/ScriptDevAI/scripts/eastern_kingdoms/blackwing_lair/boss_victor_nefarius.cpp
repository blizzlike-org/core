...
struct boss_victor_nefariusAI : public CombatAI, private DialogueHelper {
  boss_victor_nefariusAI(Creature *creature)
      : CombatAI(creature, NEFARIUS_ACTION_MAX), DialogueHelper(aIntroDialogue),
  ...
  void JustDidDialogueStep(int32 entry) override {
    ...
    if (entry == SPELL_SHADOWBLINK) {
      ...
      m_creature->SetFactionTemporary(FACTION_BLACK_DRAGON,
                                      TEMPFACTION_RESTORE_REACH_HOME | TEMPFACTION_RESTORE_RESPAWN);
      ...
      if (m_instance) {
        // Left
        m_creature->SummonCreature(m_instance->GetData(TYPE_NEFA_LTUNNEL), aNefarianLocs[0].m_fX, aNefarianLocs[0].m_fY,
                                   aNefarianLocs[0].m_fZ, 5.000f, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN,
                                   HOUR * IN_MILLISECONDS);
        // Right
        m_creature->SummonCreature(m_instance->GetData(TYPE_NEFA_RTUNNEL), aNefarianLocs[1].m_fX, aNefarianLocs[1].m_fY,
                                   aNefarianLocs[1].m_fZ, 5.000f, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN,
                                   HOUR * IN_MILLISECONDS);
      }
    }
  }

  void DoStartIntro() { StartNextDialogueText(SAY_GAMESBEGIN_1); }

  void ExecuteAction(uint32 action) override {
    switch (action) {
    case NEFARIUS_SHADOW_BOLT: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOWBOLT, SELECT_FLAG_PLAYER))
    ...
    case NEFARIUS_SILENCE: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SILENCE, SELECT_FLAG_PLAYER))
    ...
    case NEFARIUS_SHADOW_COMMAND: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_SHADOW_COMMAND, SELECT_FLAG_PLAYER))
...
bool GossipSelect_boss_victor_nefarius(Player *player, Creature *creature, uint32 /*uiSender*/, uint32 action) {
  ...
  case GOSSIP_ACTION_INFO_DEF + 1:
    ...
    player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_NEFARIUS_2, GOSSIP_SENDER_MAIN,
                               GOSSIP_ACTION_INFO_DEF + 2);
    player->SEND_GOSSIP_MENU(GOSSIP_TEXT_NEFARIUS_2, creature->GetObjectGuid());
    break;
  case GOSSIP_ACTION_INFO_DEF + 2:
    creature->HandleEmote(EMOTE_ONESHOT_TALK);
    player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_NEFARIUS_3, GOSSIP_SENDER_MAIN,
