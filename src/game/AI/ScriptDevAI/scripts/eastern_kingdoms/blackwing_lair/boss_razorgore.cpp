...
SD%Complete: 95
SDComment: Threat management after Mind Control is released needs core support (boss should have aggro on its previous
controller and its previous victim should have threat transfered from boss to controlling player) SDCategory: Blackwing
Lair EndScriptData */
...
struct boss_razorgoreAI : public CombatAI {
  boss_razorgoreAI(Creature *creature)
      : CombatAI(creature, RAZORGORE_ACTION_MAX),
        m_instance(static_cast<instance_blackwing_lair *>(creature->GetInstanceData())) {
  ...
  void Aggro(Unit * /*attacker*/) override { m_creature->RemoveAurasDueToSpell(SPELL_POSSESS_VISUAL); }
  ...
  void JustPreventedDeath(Unit * /*attacker*/) override { m_instance->SetData(TYPE_RAZORGORE, FAIL); }
...
struct npc_blackwing_orbAI : public ScriptedAI {
  npc_blackwing_orbAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_blackwing_lair *>(creature->GetInstanceData())) {
    Reset();
  }
  ...
  void Reset() override { m_uiIntroVisualTimer = 4000; }
  ...
  void SpellHit(Unit * /*caster*/, const SpellEntry *spellInfo) override {
    ...
    if (spellInfo->Id == SPELL_FIREBALL) {
      if (Creature *pTemp =
              m_creature->SummonCreature(NPC_ORB_DOMINATION, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                         m_creature->GetPositionZ(), 0, TEMPSPAWN_TIMED_DESPAWN, 5 * IN_MILLISECONDS))
...
bool ProcessEventIdRazorgorePossess(uint32 /*eventId*/, Object *source, Object * /*target*/, bool /*isStart*/) {
  ...
  if (Creature *trigger = static_cast<ScriptedInstance *>(player->GetMap()->GetInstanceData())
                              ->GetSingleCreatureFromStorage(NPC_BLACKWING_ORB_TRIGGER))
  ...
  void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override {
    ...
    if (ScriptedInstance *pInstance = static_cast<ScriptedInstance *>(target->GetInstanceData())) {
      if (urand(0, 1)) {
        switch (urand(0, 2)) {
        case 0:
          DoScriptText(SAY_EGGS_BROKEN_1, razorgore);
          break;
        case 1:
          DoScriptText(SAY_EGGS_BROKEN_2, razorgore);
          break;
        case 2:
          DoScriptText(SAY_EGGS_BROKEN_3, razorgore);
          break;
        }
      }
...
struct ExplosionRazorgore : public SpellScript {
  bool OnCheckTarget(const Spell * /*spell*/, Unit *target, SpellEffectIndex /*effIdx*/) const override {
    ...
    static std::set<uint32> validEntries = {NPC_GRETHOK_CONTROLLER, NPC_BLACKWING_GUARDSMAN, NPC_BLACKWING_LEGIONNAIRE,
                                            NPC_BLACKWING_MAGE,     NPC_DRAGONSPAWN,         NPC_RAZORGORE};
