...
enum ArugalVoidWalker { VOIDWALKER_DARKOFFERING, VOIDWALKER_CHECK_GROUP, VOIDWALKER_ACTIONS_MAX };
...
struct mob_arugal_voidwalkerAI : public CombatAI {
  ...
  bool IsLeader() { return m_position == 0; }
  ...
  void InitVoidWalkersGroup() {
    ...
    if (position + 1 > MAX_VOID_WALKERS)
      script_error_log("mob_arugal_voidwalker for %s: has more buddies than expected (%u expected, found: %u).",
  ...
  void SetFollower(ObjectGuid leaderGuid, uint8 position) {
    ...
    if (Creature *leader = m_creature->GetMap()->GetCreature(m_leaderGuid))
      ...
      script_error_log("mob_arugal_voidwalker for %s: tried to follow master but master not found in map.",
...
enum ArugalPosition { POSITION_SPAWN_LEDGE = 0, POSITION_UPPER_LEDGE = 1, POSITION_STAIRS = 2 };
...
struct boss_arugalAI : public RangedCombatAI {
  boss_arugalAI(Creature *creature)
      : RangedCombatAI(creature, ARUGAL_ACTIONS_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ARUGAL_THUNDERSHOCK: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, uint32(0), SELECT_FLAG_IN_MELEE_RANGE)) {
    ...
    case ARUGAL_TELEPORT: {
      ...
      // Do nothing if we would teleport to our actual place, unless we are on the upper ledge (to prevent tank from
      // breaking aggro if away for too long)
