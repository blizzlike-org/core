  ...
  void DoHandleEmberseerGrowing() {
    ...
    if (m_uiGrowingStacks == MAX_GROWING_STACKS * 0.5f)
      ...
      if (m_pInstance) {
        ...
        // Redundant check: if for some reason the event isn't set in progress until this point - avoid using the altar
        // again when the boss is fully grown
...
UnitAI *GetAI_boss_pyroguard_emberseer(Creature *pCreature) { return new boss_pyroguard_emberseerAI(pCreature); }
