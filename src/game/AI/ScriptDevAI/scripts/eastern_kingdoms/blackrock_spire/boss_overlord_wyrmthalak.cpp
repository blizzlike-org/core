...
struct boss_overlordwyrmthalakAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (!m_bSummoned && m_creature->GetHealthPercent() < 51.0f) {
      Creature *pGuard1 =
          m_creature->SummonCreature(uSummons[urand(0, 2)], afLocations[0][0], afLocations[0][1], afLocations[0][2],
                                     afLocations[0][3], TEMPSPAWN_TIMED_DESPAWN, 300000);
      Creature *pGuard2 =
          m_creature->SummonCreature(uSummons[urand(0, 2)], afLocations[1][0], afLocations[1][1], afLocations[1][2],
                                     afLocations[1][3], TEMPSPAWN_TIMED_DESPAWN, 300000);
      if (pGuard1) {
...
UnitAI *GetAI_boss_overlordwyrmthalak(Creature *pCreature) { return new boss_overlordwyrmthalakAI(pCreature); }
