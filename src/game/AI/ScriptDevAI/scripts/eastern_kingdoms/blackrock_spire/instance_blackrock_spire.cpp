...
instance_blackrock_spire::instance_blackrock_spire(Map *pMap)
    : ScriptedInstance(pMap), DialogueHelper(aStadiumDialogue), m_bBeastIntroDone(false), m_bBeastOutOfLair(false),
      m_bUpperDoorOpened(false), m_uiDragonspineDoorTimer(0), m_uiDragonspineGoCount(0), m_uiFlamewreathEventTimer(0),
      m_uiFlamewreathWaveCount(0), m_uiStadiumEventTimer(0), m_uiStadiumWaves(0), m_uiStadiumMobsAlive(0) {
  Initialize();
}
...
void instance_blackrock_spire::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_EMBERSEER:
    ...
    if (uiData == FAIL) {
      for (GuidList::const_iterator itr = m_lIncarceratorGUIDList.begin(); itr != m_lIncarceratorGUIDList.end();
           ++itr) {
  ...
  case TYPE_STADIUM:
    ...
    if (uiData == IN_PROGRESS)
      ...
      for (GuidList::const_iterator itr = m_lStadiumSpectatorsGUIDList.begin();
           itr != m_lStadiumSpectatorsGUIDList.end(); ++itr) {
...
void instance_blackrock_spire::DoSortRoomEventMobs() {
  ...
  for (uint8 i = 0; i < MAX_ROOMS; ++i) {
    if (GameObject *pRune = instance->GetGameObject(m_aRoomRuneGuid[i])) {
      for (GuidList::const_iterator itr = m_lRoomEventMobGUIDList.begin(); itr != m_lRoomEventMobGUIDList.end();
           ++itr) {
  ...
  case NPC_BLACKHAND_HANDLER:
    ...
    if (urand(0, 100) < 5) {
      if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS)) {
        switch (urand(0, 7)) {
        case 0:
          DoScriptText(SAY_NEFARIUS_TAUNT1, pNefarius);
          break;
        case 1:
          DoScriptText(SAY_NEFARIUS_TAUNT2, pNefarius);
          break;
        case 2:
          DoScriptText(SAY_NEFARIUS_TAUNT3, pNefarius);
          break;
        case 3:
          DoScriptText(SAY_NEFARIUS_TAUNT4, pNefarius);
          break;
        case 4:
          DoScriptText(SAY_NEFARIUS_TAUNT5, pNefarius);
          break;
        case 5:
          DoScriptText(SAY_NEFARIUS_TAUNT6, pNefarius);
          break;
        case 6:
          DoScriptText(SAY_NEFARIUS_TAUNT7, pNefarius);
          break;
        case 7:
          DoScriptText(SAY_NEFARIUS_TAUNT8, pNefarius);
          break;
        }
      }
    }
    ...
    if (urand(0, 100) < 5) {
      if (Creature *pRend = GetSingleCreatureFromStorage(NPC_REND_BLACKHAND)) {
        switch (urand(0, 3)) {
        case 0:
          DoScriptText(SAY_REND_TAUNT1, pRend);
          break;
        case 1:
          DoScriptText(SAY_REND_TAUNT2, pRend);
          break;
        case 2:
          DoScriptText(SAY_REND_TAUNT3, pRend);
          break;
        case 3:
          DoScriptText(SAY_REND_TAUNT4, pRend);
          break;
        }
      }
    }
...
void instance_blackrock_spire::DoUseEmberseerRunes(bool bReset) {
  ...
  for (GuidList::const_iterator itr = m_lEmberseerRunesGUIDList.begin(); itr != m_lEmberseerRunesGUIDList.end();
       ++itr) {
    if (bReset) {
  ...
  case NPC_BLACKHAND_HANDLER:
    ...
    if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS)) {
      ...
      for (uint8 i = 0; i < 12; i++) {
        Creature *pSpectator = pNefarius->SummonCreature(aStadiumSpectators[i], aSpectatorsSpawnLocs[i].m_fX,
                                                         aSpectatorsSpawnLocs[i].m_fY, aSpectatorsSpawnLocs[i].m_fZ,
                                                         aSpectatorsSpawnLocs[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
        if (pSpectator) {
          pSpectator->SetFacingTo(aSpectatorsTargetLocs[i].m_fO);
          pSpectator->SetWalk(false);
          pSpectator->GetMotionMaster()->MovePoint(0, aSpectatorsTargetLocs[i].m_fX, aSpectatorsTargetLocs[i].m_fY,
                                                   aSpectatorsTargetLocs[i].m_fZ);
...
void instance_blackrock_spire::DoSendNextStadiumWave() {
  if (m_uiStadiumWaves < MAX_STADIUM_WAVES) {
    ...
    if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS)) {
      ...
      for (uint8 i = 0; i < MAX_STADIUM_MOBS_PER_WAVE; ++i) {
        ...
        if (Creature *pTemp = pNefarius->SummonCreature(aStadiumEventNpcs[m_uiStadiumWaves][i], fX, fY, fZ, 0.0f,
  ...
  // All waves are cleared - start Gyth intro
    ...
    if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS)) {
      ...
      if (Creature *pTemp = pNefarius->SummonCreature(NPC_GYTH, aStadiumLocs[1].m_fX, aStadiumLocs[1].m_fY,
                                                      aStadiumLocs[1].m_fZ, 0.0f, TEMPSPAWN_DEAD_DESPAWN, 0))
...
void instance_blackrock_spire::DoSendNextFlamewreathWave() {
  ...
  // TODO - The npcs would move nicer if they had DB waypoints, so i suggest to change their default movement to DB
  // waypoints, and random movement when they reached their goal
  ...
  if (m_uiFlamewreathWaveCount < 6) // Send two adds (6 waves, then boss)
  {
    ...
    for (uint8 i = 0; i < 2; ++i) {
      float fX, fY, fZ;
      pSummoner->GetRandomPoint(rookeryEventSpawnPos[0], rookeryEventSpawnPos[1], rookeryEventSpawnPos[2], 2.5f, fX, fY,
                                fZ);
      // Summon Rookery Hatchers in first wave, else random
      pSummoned = pSummoner->SummonCreature(urand(0, 1) && m_uiFlamewreathWaveCount ? NPC_ROOKERY_GUARDIAN
                                                                                    : NPC_ROOKERY_HATCHER,
                                            fX, fY, fZ, 0.0, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
      if (pSummoned) {
  ...
  {
    if (Creature *pSolakar = pSummoner->SummonCreature(NPC_SOLAKAR_FLAMEWREATH, rookeryEventSpawnPos[0],
                                                       rookeryEventSpawnPos[1], rookeryEventSpawnPos[2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, HOUR * IN_MILLISECONDS))
      // pSolakar->GetMotionMaster()->MovePoint(1, pSummoner->GetPositionX(), pSummoner->GetPositionY(),
      // pSummoner->GetPositionZ());
...
void instance_blackrock_spire::StartflamewreathEventIfCan() {
  ...
  if (m_auiEncounter[TYPE_FLAMEWREATH] == DONE || m_auiEncounter[TYPE_FLAMEWREATH] == IN_PROGRESS ||
      m_auiEncounter[TYPE_DRAKKISATH] == DONE)
    return;
...
bool AreaTrigger_at_blackrock_spire(Player *pPlayer, AreaTriggerEntry const *pAt) {
  ...
  case AREATRIGGER_STADIUM:
    if (instance_blackrock_spire *pInstance = (instance_blackrock_spire *)pPlayer->GetInstanceData()) {
      ...
      if (Creature *pNefarius =
              pPlayer->SummonCreature(NPC_LORD_VICTOR_NEFARIUS, aStadiumLocs[3].m_fX, aStadiumLocs[3].m_fY,
                                      aStadiumLocs[3].m_fZ, aStadiumLocs[3].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0))
        pNefarius->SetFactionTemporary(FACTION_BLACK_DRAGON, TEMPFACTION_NONE);
      if (Creature *pRend =
              pPlayer->SummonCreature(NPC_REND_BLACKHAND, aStadiumLocs[4].m_fX, aStadiumLocs[4].m_fY,
                                      aStadiumLocs[4].m_fZ, aStadiumLocs[4].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0))
  ...
  // Intro event when players enter The Furnace: three Blackhand Elite are spawned, flee from the Beast and are "killed"
  // by it The Blackhand Elite scripts are handled in database by waypoints movement
  case AREATRIGGER_BEAST_INTRO:
    if (instance_blackrock_spire *pInstance = (instance_blackrock_spire *)pPlayer->GetInstanceData()) {
      ...
      if (Creature *pBeast = pInstance->GetSingleCreatureFromStorage(NPC_THE_BEAST)) {
        for (uint8 i = 0; i < 3; i++) {
          if (Creature *pTemp =
                  pBeast->SummonCreature(NPC_BLACKHAND_ELITE, aBeastLocs[i].m_fX, aBeastLocs[i].m_fY,
                                         aBeastLocs[i].m_fZ, aBeastLocs[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
...
bool ProcessEventId_event_spell_altar_emberseer(uint32 /*uiEventId*/, Object *pSource, Object * /*pTarget*/,
                                                bool bIsStart) {
...
UnitAI *GetAI_npc_rookery_hatcher(Creature *pCreature) { return new npc_rookery_hatcherAI(pCreature); }
