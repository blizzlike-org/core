...
enum {
  ...
  // SPELL_HAS_BREW                = 47331,            // triggers 47344 - aura which asks for the second brew on item
  // expire SPELL_SEND_FIRST_MUG          = 47333,            // triggers 47345 SPELL_SEND_SECOND_MUG         = 47339,
  // // triggers 47340 - spell triggered by 47344 SPELL_BREWMAIDEN_DESPAWN_AURA = 48186,            // purpose unk
...
struct boss_coren_direbrewAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_creature->GetHealthPercent() < 66.0f && m_uiPhase == 0) {
      float fX, fY, fZ;
      m_creature->GetRandomPoint(m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), 10,
                                 fX, fY, fZ);
    ...
    if (m_uiChargeTimer < uiDiff) {
      if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DIREBREW_CHARGE,
                                                            SELECT_FLAG_NOT_IN_MELEE_RANGE | SELECT_FLAG_PLAYER)) {
...
UnitAI *GetAI_boss_coren_direbrew(Creature *pCreature) { return new boss_coren_direbrewAI(pCreature); }
