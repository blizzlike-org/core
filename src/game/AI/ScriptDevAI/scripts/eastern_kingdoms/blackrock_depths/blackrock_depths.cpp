...
bool GOUse_go_bar_beer_keg(Player * /*pPlayer*/, GameObject *pGo) {
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pGo->GetInstanceData()) {
    if (pInstance->GetData(TYPE_HURLEY) == IN_PROGRESS ||
        pInstance->GetData(TYPE_HURLEY) ==
            DONE) // GOs despawning on use, this check should never be true but this is proper to have it there
      return false;
...
bool AreaTrigger_at_shadowforge_bridge(Player *pPlayer, AreaTriggerEntry const * /*pAt*/) {
  if (instance_blackrock_depths *pInstance = (instance_blackrock_depths *)pPlayer->GetInstanceData()) {
    ...
    if (Creature *pMasterGuard = pPyromancer->SummonCreature(NPC_ANVILRAGE_GUARDMAN, aGuardSpawnPositions[0][0],
                                                             aGuardSpawnPositions[0][1], aGuardSpawnPositions[0][2],
                                                             aGuardSpawnPositions[0][3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
      ...
      if (Creature *pSlaveGuard = pPyromancer->SummonCreature(NPC_ANVILRAGE_GUARDMAN, aGuardSpawnPositions[1][0],
                                                              aGuardSpawnPositions[1][1], aGuardSpawnPositions[1][2],
                                                              aGuardSpawnPositions[1][3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
 ...
 * Quest-Event: This needs to be clearified - there is some suggestion, that Theldren&Adds also might come as first
 * wave.
 */
...
static const uint32 aGladiator[MAX_POSSIBLE_THELDREN_ADDS] = {NPC_LEFTY, NPC_ROTFANG, NPC_SNOKH,    NPC_MALGEN,
                                                              NPC_KORV,  NPC_REZZNIK, NPC_VAJASHNI, NPC_VOLIDA};
...
bool AreaTrigger_at_ring_of_law(Player *pPlayer, AreaTriggerEntry const *pAt) {
  if (instance_blackrock_depths *pInstance = (instance_blackrock_depths *)pPlayer->GetInstanceData()) {
    ...
    pInstance->SetData(TYPE_RING_OF_LAW,
                       pInstance->GetData(TYPE_RING_OF_LAW) == DATA_BANNER_BEFORE_EVENT ? SPECIAL : IN_PROGRESS);

    pPlayer->SummonCreature(NPC_GRIMSTONE, aSpawnPositions[POS_GRIMSTONE][0], aSpawnPositions[POS_GRIMSTONE][1],
                            aSpawnPositions[POS_GRIMSTONE][2], aSpawnPositions[POS_GRIMSTONE][3],
...
struct npc_grimstoneAI : public npc_escortAI {
  ...
  void SummonRingMob(uint32 uiEntry, uint8 uiNpcPerWave, SpawnPosition uiPosition) {
    ...
    for (uint8 i = 0; i < uiNpcPerWave; ++i) {
      m_creature->GetRandomPoint(aSpawnPositions[uiPosition][0], aSpawnPositions[uiPosition][1],
                                 aSpawnPositions[uiPosition][2], 2.0f, fX, fY, fZ);
...
UnitAI *GetAI_npc_grimstone(Creature *pCreature) { return new npc_grimstoneAI(pCreature); }

bool EffectDummyCreature_spell_banner_of_provocation(Unit * /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
  ...
  if (uiSpellId == SPELL_SUMMON_THELRIN_DND && uiEffIndex != EFFECT_INDEX_0) {
    ...
    if (pInstance && pInstance->GetData(TYPE_RING_OF_LAW) != DONE && pInstance->GetData(TYPE_RING_OF_LAW) != SPECIAL)
      pInstance->SetData(TYPE_RING_OF_LAW, pInstance->GetData(TYPE_RING_OF_LAW) == IN_PROGRESS
                                               ? uint32(SPECIAL)
...
UnitAI *GetAI_npc_phalanx(Creature *pCreature) { return new npc_phalanxAI(pCreature); }
...
UnitAI *GetAI_npc_mistress_nagmara(Creature *pCreature) { return new npc_mistress_nagmaraAI(pCreature); }
...
UnitAI *GetAI_npc_rocknot(Creature *pCreature) { return new npc_rocknotAI(pCreature); }
...
bool QuestRewarded_npc_rocknot(Player *pPlayer, Creature *pCreature, Quest const *pQuest) {
  ...
  if (pQuest->GetQuestId() == QUEST_ALE) {
    ...
    // the return state and the following of the script are handled in the Update->emote part of the Rocknot NPC escort
    // AI script
...
UnitAI *GetAI_npc_marshal_windsor(Creature *pCreature) { return new npc_marshal_windsorAI(pCreature); }
...
UnitAI *GetAI_npc_hurley_blackbreath(Creature *pCreature) { return new npc_hurley_blackbreathAI(pCreature); }
...
bool GossipHello_boss_doomrel(Player *pPlayer, Creature *pCreature) {
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if (pInstance->GetData(TYPE_TOMB_OF_SEVEN) == NOT_STARTED || pInstance->GetData(TYPE_TOMB_OF_SEVEN) == FAIL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_CHALLENGE, GOSSIP_SENDER_MAIN,
...
struct boss_plugger_spazzringAI : public ScriptedAI {
  ...
  void JustDied(Unit * /*pKiller*/) override {
    ...
    m_pInstance->SetData(TYPE_PLUGGER, IN_PROGRESS); // The event is set IN_PROGRESS even if Plugger is dead because his
                                                     // death triggers more actions that are part of the event
  }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_creature->SelectHostileTarget() && m_creature->GetVictim()) {
      ...
      if (m_uiCurseOfTonguesTimer < uiDiff) {
        if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CURSE_OF_TONGUES,
...
UnitAI *GetAI_boss_plugger_spazzring(Creature *pCreature) { return new boss_plugger_spazzringAI(pCreature); }
...
bool GOUse_go_bar_ale_mug(Player *pPlayer, GameObject *pGo) {
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pGo->GetInstanceData()) {
    if (pInstance->GetData(TYPE_PLUGGER) == IN_PROGRESS ||
        pInstance->GetData(TYPE_PLUGGER) ==
            DONE) // GOs despawning on use, this check should never be true but this is proper to have it there
      return false;
    if (Creature *pPlugger = pInstance->GetSingleCreatureFromStorage(NPC_PLUGGER_SPAZZRING)) {
      if (boss_plugger_spazzringAI *pPluggerAI = dynamic_cast<boss_plugger_spazzringAI *>(pPlugger->AI())) {
        // Every time we set the event to SPECIAL, the instance script increments the number of stolen mugs/boars,
        // capping at 3
...
UnitAI *GetAI_npc_ironhand_guardian(Creature *creature) { return new npc_ironhand_guardianAI(creature); }
