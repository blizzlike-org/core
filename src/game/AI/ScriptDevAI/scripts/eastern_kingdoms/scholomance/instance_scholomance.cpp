...
instance_scholomance::instance_scholomance(Map *pMap)
    : ScriptedInstance(pMap), m_uiGambitTransformTimer(0), m_uiGandlingEvent(0), m_bIsRoomReset(false) {
  Initialize();
}
...
void instance_scholomance::OnCreatureCreate(Creature *pCreature) {
  ...
  case NPC_RISEN_ABERRATION:
    if (GetData(TYPE_RATTLEGORE) != DONE && (pCreature->GetPositionZ() > aEntranceRoom.m_fCenterZ) &&
        (pCreature->GetPositionX() - aEntranceRoom.m_fCornerX < aEntranceRoom.m_uiLength) &&
        (pCreature->GetPositionY() - aEntranceRoom.m_fCornerY < aEntranceRoom.m_uiWidth))
...
void instance_scholomance::DoRespawnEntranceRoom(Player *pSummoner) {
  ...
  for (uint8 i = 0; i < MAX_GROUPS; ++i) {
    ...
    switch (urand(0, 1)) {
    case 0:
      uiMobEntry = NPC_REANIMATED_CORPSE;
      break;
    case 1:
      uiMobEntry = NPC_DISEASED_GHOUL;
      break;
    }
    ...
    for (uint8 j = 0; j < MAX_NPC_PER_GROUP; ++j)
      pSummoner->SummonCreature(uiMobList[j], aEntranceRoomSpawnLocs[4 * i + j].m_fX,
                                aEntranceRoomSpawnLocs[4 * i + j].m_fY, aEntranceRoomSpawnLocs[4 * i + j].m_fZ,
                                aEntranceRoomSpawnLocs[4 * i + j].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
  }
  ...
  if (Creature *pNecrofiend = pSummoner->SummonCreature(
          NPC_NECROFIEND, aEntranceRoomSpawnLocs[16].m_fX, aEntranceRoomSpawnLocs[16].m_fY,
          aEntranceRoomSpawnLocs[16].m_fZ, aEntranceRoomSpawnLocs[16].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
...
void instance_scholomance::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_GANDLING:
    m_auiEncounter[uiType] = uiData;
    // Close the door to main room, because the encounter will take place only in the main hall and random around all
    // the 6 rooms
...
void instance_scholomance::DoSpawnGandlingIfCan(bool bByPlayerEnter) {
  ...
  if (m_auiEncounter[TYPE_MALICIA] == DONE && m_auiEncounter[TYPE_THEOLEN] == DONE &&
      m_auiEncounter[TYPE_POLKELT] == DONE && m_auiEncounter[TYPE_RAVENIAN] == DONE &&
      m_auiEncounter[TYPE_ALEXEI_BAROV] == DONE && m_auiEncounter[TYPE_ILLUCIA_BAROV] == DONE) {
    if (Creature *pGandling = pPlayer->SummonCreature(NPC_DARKMASTER_GANDLING, aGandlingSpawnLocs[0].m_fX,
                                                      aGandlingSpawnLocs[0].m_fY, aGandlingSpawnLocs[0].m_fZ,
                                                      aGandlingSpawnLocs[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
...
void instance_scholomance::HandlePortalEvent(uint32 uiEventId, uint32 uiData) {
  ...
  // Toggle door and event state in case of state-switch
    ...
    if ((uiData == IN_PROGRESS && !find->second.m_bIsActive) || (uiData == FAIL && find->second.m_bIsActive) ||
...
void instance_scholomance::HandleDawnGambitEvent() {
  ...
  for (auto m_sViewingRoomGuid : m_sViewingRoomGuids) {
    if (Creature *pStudent = instance->GetCreature(m_sViewingRoomGuid)) {
      ...
      pStudent->GetMotionMaster()->MoveRandomAroundPoint(pStudent->GetPositionX(), pStudent->GetPositionY(),
                                                         pStudent->GetPositionZ(), 2.0f);
...
bool ProcessEventId_event_spell_gandling_shadow_portal(uint32 uiEventId, Object *pSource, Object * /*pTarget*/,
                                                       bool /*bIsStart*/) {
