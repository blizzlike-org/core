...
enum { FACTION_ENEMY = 168, QUEST_BEAT = 1640 };

struct npc_bartlebyAI : public ScriptedAI {
  npc_bartlebyAI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
...
UnitAI *GetAI_npc_bartleby(Creature *pCreature) { return new npc_bartlebyAI(pCreature); }
    ...
    if (m_uiStartEventTimer) {
      if (m_uiStartEventTimer <= uiDiff) {
        if (Player *pPlayer = m_creature->GetMap()->GetPlayer(m_playerGuid)) {
          ...
          if (Creature *pThug = m_creature->SummonCreature(NPC_OLD_TOWN_THUG, -8672.33f, 442.88f, 99.98f, 3.5f,
                                                           TEMPSPAWN_DEAD_DESPAWN, 300000))
            pThug->AI()->AttackStart(pPlayer);

          if (Creature *pThug = m_creature->SummonCreature(NPC_OLD_TOWN_THUG, -8691.59f, 441.66f, 99.41f, 6.1f,
                                                           TEMPSPAWN_DEAD_DESPAWN, 300000))
            pThug->AI()->AttackStart(pPlayer);
        }
...
UnitAI *GetAI_npc_dashel_stonefist(Creature *pCreature) { return new npc_dashel_stonefistAI(pCreature); }
...
struct npc_squire_roweAI : public npc_escortAI, private DialogueHelper {
  npc_squire_roweAI(Creature *m_creature) : npc_escortAI(m_creature), DialogueHelper(aIntroDialogue) {
  ...
  void WaypointReached(uint32 pointId) override {
    ...
    case 4:
      ...
      if (Creature *windsor = m_creature->SummonCreature(NPC_WINDSOR, aWindsorSpawnLoc[0], aWindsorSpawnLoc[1],
                                                         aWindsorSpawnLoc[2], 0, TEMPSPAWN_CORPSE_DESPAWN, 0))
...
UnitAI *GetAI_npc_squire_rowe(Creature *creature) { return new npc_squire_roweAI(creature); }
...
bool GossipHello_npc_squire_rowe(Player *player, Creature *creature) {
  ...
  if ((player->GetQuestStatus(QUEST_STORMWIND_RENDEZVOUS) == QUEST_STATUS_COMPLETE &&
       !player->GetQuestRewardStatus(QUEST_STORMWIND_RENDEZVOUS)) ||
      (player->GetQuestRewardStatus(QUEST_STORMWIND_RENDEZVOUS) &&
...
struct npc_reginald_windsorAI : public npc_escortAI, private DialogueHelper {
  ...
  void Aggro(Unit * /*who*/) override { DoCastSpellIfCan(m_creature, SPELL_SHIELD_WALL); }
    ...
    case 1:
      ...
      if (Creature *jonathan = m_scriptedMap->GetSingleCreatureFromStorage(NPC_JONATHAN)) {
        ...
        for (uint8 i = 0; i < MAX_ROYAL_GUARDS; ++i) {
          if (Creature *temp = m_creature->SummonCreature(NPC_GUARD_ROYAL, aGuardLocations[i][0], aGuardLocations[i][1],
                                                          aGuardLocations[i][2], aGuardLocations[i][3],
  ...
  void JustDidDialogueStep(int32 entry) override {
    ...
    case SAY_BOLVAR_KEEP_10:
      if (Creature *bolvar = m_scriptedMap->GetSingleCreatureFromStorage(NPC_BOLVAR)) {
        ...
        bolvar->SetFactionTemporary(11,
                                    TEMPFACTION_RESTORE_REACH_HOME); // Change Faction so he can fight Onyxia's Guards
      }
      break;
  ...
  void UpdateEscortAI(const uint32 diff) override {
    ...
    if (m_guardCheckTimer) {
      if (m_guardCheckTimer <= diff) {
        ...
        for (GuidList::const_iterator itr = m_lRoyalGuardsGuidList.begin(); itr != m_lRoyalGuardsGuidList.end();
             ++itr) {
          if (Creature *guard = m_creature->GetMap()->GetCreature(*itr)) {
...
UnitAI *GetAI_npc_reginald_windsor(Creature *creature) { return new npc_reginald_windsorAI(creature); }
...
bool GossipHello_npc_reginald_windsor(Player *player, Creature *creature) {
  ...
  if (isEventReady && player->GetQuestStatus(QUEST_THE_GREAT_MASQUERADE) != QUEST_STATUS_COMPLETE &&
      player->GetQuestRewardStatus(QUEST_STORMWIND_RENDEZVOUS)) {
