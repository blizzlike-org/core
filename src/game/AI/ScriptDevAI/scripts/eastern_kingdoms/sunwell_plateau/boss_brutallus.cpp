...
struct boss_brutallusAI : public ScriptedAI, private DialogueHelper {
  boss_brutallusAI(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(aIntroDialogue) {
  ...
  void GetAIInformation(ChatHandler &reader) override {
    if (m_pInstance) {
      if (m_pInstance->GetData(TYPE_BRUTALLUS) == SPECIAL)
        ...
        reader.PSendSysMessage("Brutallus intro event is currently %s",
                               m_pInstance->GetData(TYPE_BRUTALLUS) == NOT_STARTED ? "not started" : "completed");
      ...
      if (m_pInstance->GetData(TYPE_BRUTALLUS) != NOT_STARTED) {
        if (Creature *pMadrigosa = m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA, true))
          reader.PSendSysMessage("Madrigosa guid is %s and has %u health.", pMadrigosa->GetGuidStr().c_str(),
  ...
  void SummonedCreatureJustDied(Creature *pSummoned) override {
    ...
    if (pSummoned->GetEntry() == NPC_MADRIGOSA)
      script_error_log("Npc %u, %s, died unexpectedly. Felmyst won't be summoned anymore.", pSummoned->GetEntry(),
                       pSummoned->GetName());
  }
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case NPC_MADRIGOSA:
      if (Creature *pTrigger = m_pInstance->GetSingleCreatureFromStorage(NPC_FLIGHT_TRIGGER_LEFT))
        m_creature->SummonCreature(NPC_MADRIGOSA, pTrigger->GetPositionX(), pTrigger->GetPositionY(),
                                   pTrigger->GetPositionZ(), 0, TEMPSPAWN_DEAD_DESPAWN, 0);
    ...
    case YELL_MADR_INTRO:
      if (Creature *pMadrigosa = m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
        pMadrigosa->GetMotionMaster()->MovePoint(POINT_MOVE_GROUND, aMadrigosaLoc[0].m_fX, aMadrigosaLoc[0].m_fY,
                                                 aMadrigosaLoc[0].m_fZ);
      break;
    case YELL_INTRO:
      if (Creature *pMadrigosa = m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
    ...
    case POINT_MOVE_ICE_BLOCK:
      ...
      if (Creature *pMadrigosa = m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA)) {
        pMadrigosa->GetMotionMaster()->MovePoint(POINT_MOVE_ICE_BLOCK, aMadrigosaLoc[1].m_fX, aMadrigosaLoc[1].m_fY,
                                                 aMadrigosaLoc[1].m_fZ);
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiBurnTimer < uiDiff) {
      if (Unit *pTarget =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BURN, SELECT_FLAG_PLAYER)) {
...
UnitAI *GetAI_boss_brutallus(Creature *pCreature) { return new boss_brutallusAI(pCreature); }
    ...
    if (Creature *pTarget = (Creature *)pAura->GetTarget()) {
      if (ScriptedInstance *pInstance = (ScriptedInstance *)pTarget->GetInstanceData()) {
        if (Creature *pMadrigosa = pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA)) {
          ...
          pMadrigosa->SetRespawnCoord(pMadrigosa->GetPositionX(), pMadrigosa->GetPositionY(),
                                      pMadrigosa->GetPositionZ(), pMadrigosa->GetOrientation());
