...
struct boss_felmystAI : public ScriptedAI {
  ...
  void MoveInLineOfSight(Unit *pWho) override {
    if (!m_bHasTransformed) {
      if (pWho->GetTypeId() == TYPEID_PLAYER && pWho->IsWithinLOSInMap(m_creature) &&
  ...
  void KilledUnit(Unit * /*pVictim*/) override { DoScriptText(urand(0, 1) ? SAY_KILL_1 : SAY_KILL_2, m_creature); }
  ...
  void MovementInform(uint32 uiMoveType, uint32 uiPointId) override {
    ...
    case SUBPHASE_BREATH_PREPARE:
      ...
      if (Creature *pTrigger = m_creature->GetMap()->GetCreature(
              m_pInstance->SelectFelmystFlightTrigger(!m_bIsLeftSide, m_uiCorruptionIndex))) {
        ...
        m_creature->GetMotionMaster()->MovePoint(SUBPHASE_BREATH_MOVE, pTrigger->GetPositionX(),
    ...
    case SUBPHASE_BREATH_MOVE:
      ...
      if (Creature *pTrigger = m_pInstance->GetSingleCreatureFromStorage(!m_bIsLeftSide ? NPC_FLIGHT_TRIGGER_LEFT
                                                                                        : NPC_FLIGHT_TRIGGER_RIGHT))
        m_creature->GetMotionMaster()->MovePoint(SUBPHASE_VAPOR, pTrigger->GetPositionX(), pTrigger->GetPositionY(),
                                                 pTrigger->GetPositionZ());
  ...
  void SpellHitTarget(Unit *pTarget, const SpellEntry *pSpell) override {
    if (pTarget->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_ENCAPSULATE_CHANNEL)
      pTarget->CastSpell(pTarget, SPELL_ENCAPSULATE, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiMovementTimer) {
      if (m_uiMovementTimer <= uiDiff) {
        m_creature->SetLevitate(true);
        m_creature->GetMotionMaster()->MovePoint(PHASE_GROUND, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                                 50.083f);
        m_uiMovementTimer = 0;
      } else
    ...
    case PHASE_GROUND:
      ...
      if (m_uiFlyPhaseTimer < uiDiff) {
        ...
        m_creature->GetMotionMaster()->MovePoint(PHASE_AIR, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                                 50.083f);
        ...
        if (m_uiDemonicVaporTimer < uiDiff) {
          ...
          if (m_uiDemonicVaporCount == 2) {
            ...
            if (Creature *pTrigger = m_pInstance->GetSingleCreatureFromStorage(
                    m_bIsLeftSide ? NPC_FLIGHT_TRIGGER_LEFT : NPC_FLIGHT_TRIGGER_RIGHT))
              m_creature->GetMotionMaster()->MovePoint(SUBPHASE_VAPOR, pTrigger->GetPositionX(),
                                                       pTrigger->GetPositionY(), pTrigger->GetPositionZ());
          } else {
      ...
      case SUBPHASE_BREATH_PREPARE:
        ...
        if (m_uiCorruptionTimer) {
          if (m_uiCorruptionTimer <= uiDiff) {
            ...
            if (Creature *pTrigger = m_creature->GetMap()->GetCreature(
                    m_pInstance->SelectFelmystFlightTrigger(m_bIsLeftSide, m_uiCorruptionIndex)))
              m_creature->GetMotionMaster()->MovePoint(SUBPHASE_BREATH_PREPARE, pTrigger->GetPositionX(),
                                                       pTrigger->GetPositionY(), pTrigger->GetPositionZ());
...
UnitAI *GetAI_boss_felmyst(Creature *pCreature) { return new boss_felmystAI(pCreature); }
...
UnitAI *GetAI_npc_demonic_vapor(Creature *pCreature) { return new npc_demonic_vaporAI(pCreature); }
