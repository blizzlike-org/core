...
enum {
  ...
  SPELL_OPEN_ALL_PORTALS =
      46177, // dummy spell which opens all the portals to begin the transition phase - has muru portal as target
  SPELL_SUMMON_ENTROPIUS = 46217,
  SPELL_ENTROPIUS_SPAWN = 46223, // visual effect after spawn

  // entropius spells
  SPELL_NEGATIVE_ENERGY_ENT = 46284, // periodic aura spell; triggers 46289 which has script effect. Damage spell is
                                     // 46285 but it needs core support
...
struct boss_entropiusAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoCastSpellIfCan(m_creature, SPELL_NEGATIVE_ENERGY_ENT); }
  ...
  void JustReachedHome() override {
    if (m_pInstance) {
      ...
      // respawn muru
      m_creature->SummonCreature(NPC_MURU, afMuruSpawnLoc[0], afMuruSpawnLoc[1], afMuruSpawnLoc[2], afMuruSpawnLoc[3],
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiTransformTimer) {
      if (m_uiTransformTimer <= uiDiff) {
        ...
        ++m_uiTransformCount;
        DoCastSpellIfCan(m_creature, (m_uiTransformCount % 2) ? SPELL_TRANSFORM_VISUAL_1 : SPELL_TRANSFORM_VISUAL_2,
                         CAST_TRIGGERED);
...
UnitAI *GetAI_boss_muru(Creature *pCreature) { return new boss_muruAI(pCreature); }

UnitAI *GetAI_boss_entropius(Creature *pCreature) { return new boss_entropiusAI(pCreature); }

UnitAI *GetAI_npc_portal_target(Creature *pCreature) { return new npc_portal_targetAI(pCreature); }

UnitAI *GetAI_npc_darkness(Creature *pCreature) { return new npc_darknessAI(pCreature); }

UnitAI *GetAI_npc_singularity(Creature *pCreature) { return new npc_singularityAI(pCreature); }
