...
enum {
  ...
  SPELL_PORTAL_FROM_SHATTRATH =
      46801, // Riftwalker after reached point 1, summon NPC_RIFTWALKER and cast on him this spell.
...
struct npc_kiljaeden_controllerAI : public Scripted_NoMovementAI, private DialogueHelper {
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case NPC_KALECGOS:
      ...
      m_creature->SummonCreature(NPC_CORE_ENTROPIUS, aOutroLocations[5].m_fX, aOutroLocations[5].m_fY,
                                 aOutroLocations[5].m_fZ, aOutroLocations[5].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0);
      break;
    case NPC_RIFTWALKER:
      if (Creature *pRiftwalker =
              m_creature->SummonCreature(NPC_RIFTWALKER, 1688.42f, 641.82f, 27.60f, 0.67f, TEMPSPAWN_DEAD_DESPAWN, 0)) {
      ...
      if (Creature *pRiftwalker =
              m_creature->SummonCreature(NPC_RIFTWALKER, 1712.58f, 616.29f, 27.78f, 0.76f, TEMPSPAWN_DEAD_DESPAWN, 0)) {
        pRiftwalker->SetWalk(false);
        pRiftwalker->CastSpell(pRiftwalker, SPELL_TELEPORT_VISUAL, TRIGGERED_OLD_TRIGGERED);
    ...
    case NPC_INERT_PORTAL:
      ...
      m_creature->SummonCreature(NPC_INERT_PORTAL, aOutroLocations[0].m_fX, aOutroLocations[0].m_fY,
                                 aOutroLocations[0].m_fZ, aOutroLocations[0].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0);
      break;
    case POINT_SUMMON_SOLDIERS:
      // ToDo: summon soldiers to the left
      break;
    case NPC_VELEN:
      m_creature->SummonCreature(NPC_VELEN, aOutroLocations[1].m_fX, aOutroLocations[1].m_fY, aOutroLocations[1].m_fZ,
                                 aOutroLocations[1].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0);
      break;
    case NPC_LIADRIN:
      m_creature->SummonCreature(NPC_LIADRIN, aOutroLocations[2].m_fX, aOutroLocations[2].m_fY, aOutroLocations[2].m_fZ,
                                 aOutroLocations[2].m_fO, TEMPSPAWN_TIMED_DESPAWN, 4 * MINUTE * IN_MILLISECONDS);
      break;
    ...
    case POINT_MOVE_LIADRIN:
      if (Creature *pLiadrin = m_pInstance->GetSingleCreatureFromStorage(NPC_LIADRIN))
        pLiadrin->GetMotionMaster()->MovePoint(0, aOutroLocations[4].m_fX, aOutroLocations[4].m_fY,
    ...
    case SPELL_BLAZE_TO_LIGHT:
      if (Creature *pEntropius = m_creature->GetMap()->GetCreature(m_EntropiusGuid)) {
        ...
        pEntropius->GetMotionMaster()->MovePoint(2, m_creature->GetPositionX(), m_creature->GetPositionY(),
    ...
    case POINT_EVENT_EXIT:
      ...
      if (Creature *pVelen = m_pInstance->GetSingleCreatureFromStorage(NPC_VELEN))
        pVelen->GetMotionMaster()->MovePoint(1, aOutroLocations[1].m_fX, aOutroLocations[1].m_fY,
                                             aOutroLocations[1].m_fZ);
      break;
    }
  }
  ...
  void JustSummoned(Creature *pSummoned) override {
    ...
    case NPC_VELEN:
      ...
      pSummoned->GetMotionMaster()->MovePoint(0, aOutroLocations[3].m_fX, aOutroLocations[3].m_fY,
...
struct boss_kiljaedenAI : public Scripted_NoMovementAI, private DialogueHelper {
  boss_kiljaedenAI(Creature *pCreature) : Scripted_NoMovementAI(pCreature), DialogueHelper(aPhaseDialogue) {
    m_pInstance = ((instance_sunwell_plateau *)pCreature->GetInstanceData());
    InitializeDialogueHelper(m_pInstance);
    Reset();
  }
  ...
  void JustSummoned(Creature *pSummoned) override {
    if (pSummoned->GetEntry() == NPC_KALECGOS) {
      ...
      pSummoned->GetMotionMaster()->MoveRandomAroundPoint(m_creature->GetPositionX(), m_creature->GetPositionY(),
                                                          pSummoned->GetPositionZ(), 30.0f);
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    case PHASE_TRANSITION:
      // Transition phase is handled in the dialogue helper; however we don't want the spell timers to be decreased so
      // we use a specific phase
      break;
    case PHASE_SACRIFICE:
    ...
    case PHASE_INFERNO:
      ...
      if (m_uiKalecSummonTimer) {
        if (m_uiKalecSummonTimer <= uiDiff) {
          m_creature->SummonCreature(NPC_KALECGOS, aKalegSpawnLoc[0], aKalegSpawnLoc[1], aKalegSpawnLoc[2],
                                     aKalegSpawnLoc[3], TEMPSPAWN_CORPSE_DESPAWN, 0);
...
bool EffectAuraDummy_spell_aura_dummy_darkness_of_souls(const Aura *pAura, bool bApply) {
  ...
  if (pAura->GetId() == SPELL_DARKNESS_OF_SOULS && pAura->GetEffIndex() == EFFECT_INDEX_0 && !bApply) {
    if (Creature *pTarget = (Creature *)pAura->GetTarget()) {
      pTarget->CastSpell(pTarget, pAura->GetSpellProto()->CalculateSimpleValue(EFFECT_INDEX_2),
...
UnitAI *GetAI_boss_kiljaeden(Creature *pCreature) { return new boss_kiljaedenAI(pCreature); }

UnitAI *GetAI_npc_kiljaeden_controller(Creature *pCreature) { return new npc_kiljaeden_controllerAI(pCreature); }

UnitAI *GetAI_npc_shield_orb(Creature *pCreature) { return new npc_shield_orbAI(pCreature); }

UnitAI *GetAI_npc_power_blue_flight(Creature *pCreature) { return new npc_power_blue_flightAI(pCreature); }
