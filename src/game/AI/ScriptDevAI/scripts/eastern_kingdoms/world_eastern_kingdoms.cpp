...
struct world_map_eastern_kingdoms : public ScriptedMap, public TimerManager {
  world_map_eastern_kingdoms(Map *pMap)
      : ScriptedMap(pMap), m_shadeData({AREAID_GOLDSHIRE, AREAID_KHARANOS, AREAID_BRILL}) {
    AddCustomAction(EVENT_SPAWN, true, [&] {
      if (_spawn.size() < 16) {
        switch (urand(0, 4)) {
        case 0:
          DoSpawn(NPC_INVADING_FELGUARD);
          break;
        case 1:
          DoSpawn(NPC_INVADING_VOIDWALKER);
          break;
        case 2:
    ...
    AddCustomAction(EVENT_SPAWN_BOSS, true, [&] {
      if (_spawnBoss.size() < 1) {
        urand(0, 1) ? ResetTimer(EVENT_SPAWN_DREADKNIGHT, urand(5000, 10000))
                    : ResetTimer(EVENT_SPAWN_FELGUARD_LIEUTENANT, urand(5000, 10000));
      }
    ...
    AddCustomAction(EVENT_SPAWN_REINFORCEMENTS, true, [&] {
      switch (urand(0, 2)) {
      case 0:
        DoSpawn(NPC_LORD_MARSHAL_RAYNOR);
        break;
      case 1:
        DoSpawn(NPC_JUSTINIUS_THE_HARBINGER);
        break;
      case 2:
        DoSpawn(NPC_MELGROMM_HIGHMOUNTAIN);
        break;
      }
    });
        ...
        for (std::set<ObjectGuid>::iterator it = _spawnSupport.begin(); it != _spawnSupport.end(); ++it) {
          if (Creature *add = instance->GetCreature(*it)) {
            ...
            case NPC_LORD_MARSHAL_RAYNOR:
              switch (urand(0, 4)) {
              case 0:
                randomScriptTextId = LMR_RANDOM_TEXT_1;
                break;
              case 1:
                randomScriptTextId = LMR_RANDOM_TEXT_2;
                break;
              case 2:
                randomScriptTextId = LMR_RANDOM_TEXT_3;
                break;
              case 3:
                randomScriptTextId = LMR_RANDOM_TEXT_4;
                break;
              case 4:
                randomScriptTextId = LMR_RANDOM_TEXT_5;
                break;
              }
              break;
            case NPC_JUSTINIUS_THE_HARBINGER:
              switch (urand(0, 3)) {
              case 0:
                randomScriptTextId = JTH_RANDOM_TEXT_1;
                break;
              case 1:
                randomScriptTextId = JTH_RANDOM_TEXT_2;
                break;
              case 2:
                randomScriptTextId = JTH_RANDOM_TEXT_3;
                break;
              }
              break;
            case NPC_MELGROMM_HIGHMOUNTAIN:
              switch (urand(0, 5)) {
              case 0:
                randomScriptTextId = MH_RANDOM_TEXT_1;
                break;
              case 1:
                randomScriptTextId = MH_RANDOM_TEXT_2;
                break;
              case 2:
                randomScriptTextId = MH_RANDOM_TEXT_3;
                break;
              case 3:
                randomScriptTextId = MH_RANDOM_TEXT_4;
                break;
              case 4:
                randomScriptTextId = MH_RANDOM_TEXT_5;
                break;
              case 5:
                randomScriptTextId = MH_RANDOM_TEXT_6;
                break;
              }
              break;
            }
  ...
  void Initialize() override { m_shadeData.Reset(); }
  ...
  bool CheckConditionCriteriaMeet(Player const *player, uint32 instanceConditionId, WorldObject const *conditionSource,
    ...
    script_error_log("world_map_eastern_kingdoms::CheckConditionCriteriaMeet called with unsupported Id %u. Called "
                     "with param plr %s, src %s, condition source type %u",
  ...
  void DoSpawn(uint32 spawnId) {
    ...
    if (Creature *pProudwell = GetSingleCreatureFromStorage(NPC_AGENT_PROUDWELL, true)) {
      if (Creature *summon = pProudwell->SummonCreature(spawnId, spawnPos.x, spawnPos.y, spawnPos.z, spawnPos.o,
                                                        spawnType, despawnTimer)) {
        if (spawnScriptTextId)
          DoScriptText(spawnScriptTextId, summon);

        if (formationCreatureEntry) {
          if (Creature *add =
                  summon->SummonCreature(formationCreatureEntry, summon->GetPositionX(), summon->GetPositionY(),
                                         summon->GetPositionZ(), 0.0f, spawnType, despawnTimer))
...
struct go_infernaling_summoner_portal_hound : public GameObjectAI {
  go_infernaling_summoner_portal_hound(GameObject *go) : GameObjectAI(go) { m_uiSummonTimer = 15000; }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiSummonTimer <= uiDiff) {
      if (Creature *pProudwell = ((ScriptedInstance *)m_go->GetMap()->GetInstanceData())
                                     ->GetSingleCreatureFromStorage(NPC_AGENT_PROUDWELL)) {
        pProudwell->CastSpell(pProudwell, SPELL_SUMMON_INFERNALING_PORTAL_HOUND, TRIGGERED_OLD_TRIGGERED, nullptr,
                              nullptr, m_go->GetObjectGuid());
      }
...
bool GOUse_go_infernaling_summoner_portal_hound(Player * /*pPlayer*/, GameObject *pGo) {
  ...
  if (Creature *pProudwell = pInstance->GetSingleCreatureFromStorage(NPC_AGENT_PROUDWELL)) {
    pProudwell->CastSpell(pProudwell, SPELL_SUMMON_INFERNALING_PORTAL_HOUND, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                          pGo->GetObjectGuid());
  }
...
UnitAI *GetAI_npc_agent_proudwell(Creature *pCreature) { return new npc_agent_proudwell(pCreature); }

InstanceData *GetInstanceData_world_map_eastern_kingdoms(Map *pMap) { return new world_map_eastern_kingdoms(pMap); }
