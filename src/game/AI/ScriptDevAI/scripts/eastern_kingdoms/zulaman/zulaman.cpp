...
instance_zulaman::instance_zulaman(Map *map)
    : ScriptedInstance(map), m_startCheck(false), m_uiEventTimer(MINUTE * IN_MILLISECONDS), m_uiBearEventPhase(0),
      m_isBearPhaseInProgress(false), m_bIsAkilzonGauntletInProgress(false), m_spiritFadeTimer(0) {
  Initialize();
}
...
void instance_zulaman::OnCreatureCreate(Creature *creature) {
  ...
  case NPC_TANZAR:
    m_aEventNpcInfo[INDEX_NALORAKK].npGuid = creature->GetObjectGuid();
    break;
  case NPC_KRAZ:
    m_aEventNpcInfo[INDEX_JANALAI].npGuid = creature->GetObjectGuid();
    break;
  case NPC_ASHLI:
    m_aEventNpcInfo[INDEX_HALAZZI].npGuid = creature->GetObjectGuid();
    break;
  case NPC_HARKOR:
    m_aEventNpcInfo[INDEX_AKILZON].npGuid = creature->GetObjectGuid();
    break;
...
void instance_zulaman::OnCreatureDeath(Creature *creature) {
  ...
  case NPC_AXETHROWER:
    if (m_uiBearEventPhase > 3) // prevent crashes during testing
      return;
    if (m_nalorakkEvent[m_uiBearEventPhase].nalorakkTrashSet.find(creature->GetObjectGuid()) !=
        m_nalorakkEvent[m_uiBearEventPhase].nalorakkTrashSet.end()) {
      ++m_nalorakkEvent[m_uiBearEventPhase].trashKilledCount;
      if (m_nalorakkEvent[m_uiBearEventPhase].trashKilledCount ==
          m_nalorakkEvent[m_uiBearEventPhase].nalorakkTrashSet.size()) {
        if (Creature *nalorakk = GetSingleCreatureFromStorage(NPC_NALORAKK)) {
          ...
          if (m_uiBearEventPhase == MAX_BEAR_WAVES)
            ...
            nalorakk->GetMotionMaster()->MovePoint(1, aBearEventInfo[m_uiBearEventPhase].x,
                                                   aBearEventInfo[m_uiBearEventPhase].y,
                                                   aBearEventInfo[m_uiBearEventPhase].z);
          }
        }
      }
    }
...
void instance_zulaman::OnObjectCreate(GameObject *pGo) {
  ...
  case GO_MASSIVE_GATE:
    ...
    if (m_auiEncounter[TYPE_EVENT_RUN] == DONE || m_auiEncounter[TYPE_EVENT_RUN] == FAIL ||
        m_auiEncounter[TYPE_EVENT_RUN] == IN_PROGRESS)
...
void instance_zulaman::FillInitialWorldStates(ByteBuffer &data, uint32 &count, uint32 /*zoneId*/, uint32 /*areaId*/) {
  FillInitialWorldStateData(data, count, WORLD_STATE_ZUL_AMAN_EVENT_RUN_IS_ACTIVE,
                            GetData(TYPE_EVENT_RUN) == IN_PROGRESS);
#ifdef BUILD_TBC
...
void instance_zulaman::SetData(uint32 type, uint32 data) {
  ...
  if (data == DONE && GetKilledPreBosses() == 4 &&
      (type == TYPE_AKILZON || type == TYPE_NALORAKK || type == TYPE_JANALAI || type == TYPE_HALAZZI)) {
    DoUseDoorOrButton(GO_HEXLORD_ENTRANCE);
    if (m_auiEncounter[TYPE_EVENT_RUN] == IN_PROGRESS)
...
void instance_zulaman::Load(const char *chrIn) {
  ...
  if (m_auiEncounter[TYPE_RUN_EVENT_TIME] != 0 && m_auiEncounter[TYPE_EVENT_RUN] != DONE &&
...
void instance_zulaman::SendNextBearWave(Unit * /*target*/) {
  ...
  if (mainGuy)
    for (Creature *follower : followers)
      follower->GetMotionMaster()->MoveFollow(mainGuy, follower->GetDistance(mainGuy, true, DIST_CALC_COMBAT_REACH),
                                              follower->GetAngle(mainGuy), true);
  m_nalorakkEvent[m_uiBearEventPhase].trashKilledCount = 0;
  m_isBearPhaseInProgress = true;
}

bool instance_zulaman::CheckConditionCriteriaMeet(Player const *player, uint32 instanceConditionId,
  ...
  script_error_log("instance_zulaman::CheckConditionCriteriaMeet called with unsupported Id %u. Called with param plr "
                   "%s, src %s, condition source type %u",
...
uint8 instance_zulaman::GetKilledPreBosses() {
  return (m_auiEncounter[TYPE_AKILZON] == DONE ? 1 : 0) + (m_auiEncounter[TYPE_NALORAKK] == DONE ? 1 : 0) +
         (m_auiEncounter[TYPE_JANALAI] == DONE ? 1 : 0) + (m_auiEncounter[TYPE_HALAZZI] == DONE ? 1 : 0);
}
...
void instance_zulaman::DoTimeRunSay(RunEventSteps data) {
  ...
  case RUN_START:
    ...
    break;
  case RUN_FAIL:
    DoOrSimulateScriptTextForThisInstance(urand(0, 1) ? SAY_INST_SACRIF1 : SAY_INST_SACRIF2, NPC_MALACRASS);
    break;
  case RUN_DONE:
  ...
  case RUN_PROGRESS:
    ...
    case 1:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_PROGRESS_1, NPC_MALACRASS);
      break;
    case 2:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_PROGRESS_2, NPC_MALACRASS);
      break;
    case 3:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_PROGRESS_3, NPC_MALACRASS);
      break;
    }
    break;
  case RUN_FAIL_SOON:
    ...
    case 0:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_WARN_1, NPC_MALACRASS);
      break;
    case 1:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_WARN_2, NPC_MALACRASS);
      break;
    case 2:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_WARN_3, NPC_MALACRASS);
      break;
    case 3:
      DoOrSimulateScriptTextForThisInstance(SAY_INST_WARN_4, NPC_MALACRASS);
      break;
    }
    break;
  }
...
void instance_zulaman::SpawnMalacrass() {
  ...
  // Summon Malacrass (Archimonde)
  if (Player *pPlayer = GetPlayerInMap())
    pPlayer->SummonCreature(NPC_MALACRASS, aMalacrassSpawnLoc[0], aMalacrassSpawnLoc[1], aMalacrassSpawnLoc[2],
                            aMalacrassSpawnLoc[3], TEMPSPAWN_MANUAL_DESPAWN, 0);
}

void instance_zulaman::StartSpiritTimer() { m_spiritFadeTimer = 30000; }
...
InstanceData *GetInstanceData_instance_zulaman(Map *map) { return new instance_zulaman(map); }
