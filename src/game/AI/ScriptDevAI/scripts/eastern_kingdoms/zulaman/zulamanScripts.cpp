    ...
    if (m_uiEventTimer) {
      if (m_uiEventTimer <= diff) {
        ...
        case 1:
          ...
          case NPC_MANNUTH:
            DoScriptText(SAY_MANNUTH_1, m_creature, player);
            break;
          case NPC_DEEZ:
            DoScriptText(SAY_DEEZ_1, m_creature, player);
            break;
          case NPC_GALATHRYN:
            DoScriptText(SAY_GALATHRYN_1, m_creature, player);
            break;
          case NPC_ADARRAH:
            DoScriptText(SAY_ADARRAH_1, m_creature, player);
            break;
          case NPC_DARWEN:
            DoScriptText(SAY_DARWEN_1, m_creature, player);
            break;
          case NPC_FUDGERICK:
            DoScriptText(SAY_FUDGERICK_1, m_creature, player);
            break;
          case NPC_GUNTER:
            DoScriptText(SAY_GUNTER_1, m_creature, player);
            break;
          case NPC_KYREN:
            DoScriptText(SAY_KYREN_1, m_creature, player);
            break;
          case NPC_MITZI:
            DoScriptText(SAY_MITZI_1, m_creature, player);
            break;
          case NPC_CHRISTIAN:
            DoScriptText(SAY_CHRISTIAN_1, m_creature, player);
            break;
          case NPC_BRENNAN:
            DoScriptText(SAY_BRENNAN_1, m_creature, player);
            break;
          case NPC_HOLLEE:
            DoScriptText(SAY_HOLLEE_1, m_creature, player);
            break;
          }
          m_uiEvent = 2;
        ...
        case 3:
          ...
          switch (m_creature->GetEntry()) {
          case NPC_MANNUTH:
            DoScriptText(SAY_MANNUTH_3, m_creature, player);
            break;
          case NPC_DEEZ:
            DoScriptText(SAY_DEEZ_3, m_creature, player);
            break;
          case NPC_GALATHRYN:
            DoScriptText(SAY_GALATHRYN_3, m_creature, player);
            break;
          case NPC_ADARRAH:
            DoScriptText(SAY_ADARRAH_3, m_creature, player);
            break;
          case NPC_DARWEN:
            DoScriptText(SAY_DARWEN_3, m_creature, player);
            break;
          case NPC_FUDGERICK:
            DoScriptText(SAY_FUDGERICK_3, m_creature, player);
            break;
          case NPC_GUNTER:
            DoScriptText(SAY_GUNTER_3, m_creature, player);
            break;
          case NPC_KYREN:
            DoScriptText(SAY_KYREN_3, m_creature, player);
            break;
          case NPC_MITZI:
            DoScriptText(SAY_MITZI_3, m_creature, player);
            break;
          case NPC_CHRISTIAN:
            DoScriptText(SAY_CHRISTIAN_3, m_creature, player);
            break;
          case NPC_BRENNAN:
            DoScriptText(SAY_BRENNAN_3, m_creature, player);
            break;
          case NPC_HOLLEE:
            DoScriptText(SAY_HOLLEE_3, m_creature, player);
            break;
          }
          m_uiEvent = 4;
          if (m_creature->GetEntry() == NPC_GUNTER || m_creature->GetEntry() == NPC_KYREN)
            m_uiEventTimer =
                5 * MINUTE * IN_MILLISECONDS; // vendors wait for 5 minutes before running away and despawning
          else
            m_uiEventTimer = 6000;
          break;
        case 4:
          ...
          switch (m_creature->GetEntry()) {
          case NPC_MANNUTH:
            DoScriptText(SAY_MANNUTH_4, m_creature, player);
            break;
          case NPC_DEEZ:
            DoScriptText(SAY_DEEZ_4, m_creature, player);
            break;
          case NPC_GALATHRYN:
            DoScriptText(SAY_GALATHRYN_4, m_creature, player);
            break;
          case NPC_ADARRAH:
            DoScriptText(SAY_ADARRAH_4, m_creature, player);
            break;
          case NPC_DARWEN:
            DoScriptText(SAY_DARWEN_4, m_creature, player);
            break;
          case NPC_FUDGERICK:
            DoScriptText(SAY_FUDGERICK_4, m_creature, player);
            break;
          case NPC_GUNTER:
            DoScriptText(SAY_GUNTER_4, m_creature, player);
            break;
          case NPC_KYREN:
            DoScriptText(SAY_KYREN_4, m_creature, player);
            break;
          case NPC_MITZI:
            DoScriptText(SAY_MITZI_4, m_creature, player);
            break;
          case NPC_CHRISTIAN:
            DoScriptText(SAY_CHRISTIAN_4, m_creature, player);
            break;
          case NPC_BRENNAN:
            DoScriptText(SAY_BRENNAN_4, m_creature, player);
            break;
          case NPC_HOLLEE:
            DoScriptText(SAY_HOLLEE_4, m_creature, player);
            break;
          }
          m_uiEvent = 5;
          m_uiEventTimer = 2000;
  ...
  void SpellHit(Unit *caster, const SpellEntry *spell) override {
    if (spell->Id == SPELL_REMOVE_AMANI_CURSE && caster->GetTypeId() == TYPEID_PLAYER &&
      ...
      if (!urand(0, 49)) {
        ...
        m_creature->GetMotionMaster()->MovePoint(POINT_DESPAWN, caster->GetPositionX(), caster->GetPositionY(),
                                                 caster->GetPositionZ());
      } else
        DoSpawnRandom();
    }
...
UnitAI *GetAI_npc_forest_frog(Creature *creature) { return new npc_forest_frogAI(creature); }
...
struct npc_harrison_jones_zaAI : public npc_escortAI {
  npc_harrison_jones_zaAI(Creature *creature)
      : npc_escortAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
    Reset();
  }
...
UnitAI *GetAI_npc_harrison_jones_za(Creature *creature) { return new npc_harrison_jones_zaAI(creature); }
...
enum { SAY_GAUNTLET_START = -1568088 };

struct npc_amanishi_lookoutAI : public ScriptedAI {
  npc_amanishi_lookoutAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData())) {
    Reset();
  }
  ...
  void MoveInLineOfSight(Unit *who) override {
    ...
    if (who->GetTypeId() == TYPEID_PLAYER && !static_cast<Player *>(who)->IsGameMaster() &&
        m_creature->IsWithinDistInMap(who, 25.0f)) {
...
UnitAI *GetAI_npc_amanishi_lookout(Creature *creature) { return new npc_amanishi_lookoutAI(creature); }
...
struct npc_amanishi_tempestAI : public ScriptedAI {
  npc_amanishi_tempestAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData())) {
    Reset();
  }
  ...
  void JustSummoned(Creature *summoned) override { summoned->SetWalk(false); }
...
UnitAI *GetAI_npc_amanishi_tempest(Creature *creature) { return new npc_amanishi_tempestAI(creature); }
...
struct npc_harkorAI : public ScriptedAI {
  npc_harkorAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData())) {
    Reset();
  }
  ...
  void MoveInLineOfSight(Unit *who) override {
    ...
    if (m_bCanCelebrate && who->GetTypeId() == TYPEID_PLAYER && !((Player *)who)->IsGameMaster() &&
        m_creature->IsWithinDistInMap(who, 30.0f)) {
...
UnitAI *GetAI_npc_harkor(Creature *creature) { return new npc_harkorAI(creature); }
...
bool GossipHello_npc_harkor(Player *player, Creature *creature) {
  ...
  if (npc_harkorAI *harkorAI = dynamic_cast<npc_harkorAI *>(creature->AI())) {
    if (instance && instance->GetData(TYPE_AKILZON) == DONE && !harkorAI->m_bCompletedChestEvent)
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_HARKOR_FREE, GOSSIP_SENDER_MAIN,
      ...
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_HARKOR_DONE, GOSSIP_SENDER_MAIN,
...
struct npc_tanzarAI : public ScriptedAI {
  npc_tanzarAI(Creature *creature)
      : ScriptedAI(creature), m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData())) {
    Reset();
  }
...
UnitAI *GetAI_npc_tanzar(Creature *creature) { return new npc_tanzarAI(creature); }
...
bool GossipHello_npc_tanzar(Player *player, Creature *creature) {
  ...
  if (npc_tanzarAI *tanzarAI = dynamic_cast<npc_tanzarAI *>(creature->AI())) {
    if (instance && instance->GetData(TYPE_NALORAKK) == DONE && !tanzarAI->m_bCompletedChestEvent)
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_TANZAR_FREE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 1);
    else if (instance && instance->GetData(TYPE_NALORAKK) == DONE) {
      player->PrepareGossipMenu(creature, GOSSIP_MENU_ID_TANZAR_DONE);
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_TANZAR_DONE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 2);
    }
  }
...
bool GossipSelect_npc_tanzar(Player *player, Creature *creature, uint32 /*uiSender*/, uint32 action) {
  if (npc_tanzarAI *tanzarAI = dynamic_cast<npc_tanzarAI *>(creature->AI())) {
    if (action == GOSSIP_ACTION_INFO_DEF + 1 && !tanzarAI->m_bCompletedChestEvent) {
      ...
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_TANZAR_DONE2, GOSSIP_SENDER_MAIN,
...
UnitAI *GetAI_npc_kraz(Creature *creature) { return new npc_krazAI(creature); }
...
bool GossipHello_npc_kraz(Player *player, Creature *creature) {
  ...
  if (npc_krazAI *krazAI = dynamic_cast<npc_krazAI *>(creature->AI())) {
    if (instance && instance->GetData(TYPE_JANALAI) == DONE && !krazAI->m_bCompletedChestEvent) {
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_KRAZ_FREE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 1);
      player->SendPreparedGossip(creature);
    } else if (instance && instance->GetData(TYPE_JANALAI) == DONE) {
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_KRAZ_DONE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 2);
...
UnitAI *GetAI_npc_ashli(Creature *creature) { return new npc_ashliAI(creature); }
...
bool GossipHello_npc_ashli(Player *player, Creature *creature) {
  ...
  if (npc_ashliAI *ashliAI = dynamic_cast<npc_ashliAI *>(creature->AI())) {
    if (instance && instance->GetData(TYPE_HALAZZI) == DONE && !ashliAI->m_bCompletedChestEvent)
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_ASHLI_FREE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 1);
    else if (instance && instance->GetData(TYPE_HALAZZI) == DONE) {
      player->PrepareGossipMenu(creature, GOSSIP_MENU_ID_ASHLI_DONE);
      player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_ASHLI_DONE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 2);
    }
  }
  ...
  void Aggro(Unit * /*who*/) override {
    ...
    if (m_instance) {
      ...
      if (Creature *pCloseDrum = m_instance->instance->GetCreature(m_targetDrumGuid)) {
        m_creature->GetMotionMaster()->MoveIdle();
        m_creature->GetMotionMaster()->MovePoint(1, pCloseDrum->GetPositionX(), pCloseDrum->GetPositionY(),
                                                 pCloseDrum->GetPositionZ());
      }
    }
  }
...
UnitAI *GetAI_npc_amanishi_scout(Creature *creature) { return new npc_amanishi_scoutAI(creature); }
