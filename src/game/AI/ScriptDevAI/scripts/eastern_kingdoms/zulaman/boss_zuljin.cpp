...
static const BossPhase aZuljinPhases[] = {
    ...
    {SPELL_SHAPE_OF_THE_DRAGONHAWK, SAY_DRAGONHAWK_TRANSFORM, EMOTE_DRAGONHAWK_SPIRIT, NPC_DRAGONHAWK_SPIRIT,
     PHASE_DRAGONHAWK}};
...
struct boss_zuljinAI : public CombatAI {
  boss_zuljinAI(Creature *creature)
      : CombatAI(creature, ZULJIN_ACTION_MAX),
        m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData())) {
    ...
    AddCustomAction(ZULJIN_SPIRIT_ACQUIRED, true, [&]() { SpiritAcquired(); });
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(urand(0, 1) ? SAY_KILL1 : SAY_KILL2, m_creature); }
  ...
  void DoDespawnVortexes() { DespawnGuids(m_summons); }
  ...
  void SpellHit(Unit * /*caster*/, const SpellEntry *spell) override {
    ...
    } else if (spell->Id == SPELL_SHAPE_OF_THE_BEAR || spell->Id == SPELL_SHAPE_OF_THE_EAGLE ||
               spell->Id == SPELL_SHAPE_OF_THE_LYNX || spell->Id == SPELL_SHAPE_OF_THE_DRAGONHAWK) {
  ...
  void SpiritAcquired() { DoCastSpellIfCan(m_creature, aZuljinPhases[m_phase].spiritSpellId); }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ZULJIN_ACTION_CLAW_RAGE:
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_CLAW_RAGE, SELECT_FLAG_PLAYER))
    ...
    case ZULJIN_ACTION_SUMMON_PILLAR:
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SUMMON_PILLAR, SELECT_FLAG_PLAYER))
...
struct npc_feather_vortexAI : public ScriptedAI, public TimerManager {
  ...
  void JustRespawned() override { m_creature->SetInCombatWithZone(); }
  ...
  void PickNewTarget() {
    if (Creature *zuljin = m_instance->GetSingleCreatureFromStorage(NPC_ZULJIN)) {
      ...
      if (Unit *target = zuljin->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
                                                       SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_CUSTOM)) {
        DoResetThreat();
...
UnitAI *GetAI_boss_zuljin(Creature *creature) { return new boss_zuljinAI(creature); }

UnitAI *GetAI_npc_feather_vortex(Creature *creature) { return new npc_feather_vortexAI(creature); }
