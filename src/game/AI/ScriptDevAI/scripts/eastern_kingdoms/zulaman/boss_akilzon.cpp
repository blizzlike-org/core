  ...
  void DoSummonEagles() {
    for (uint32 i = 0; i < MAX_EAGLE_COUNT; ++i) {
      if (!m_eagles[i].IsEmpty())
        continue;
      Creature *eagle = m_creature->SummonCreature(NPC_SOARING_EAGLE, eagleSummonPositions[i].x,
                                                   eagleSummonPositions[i].y, eagleSummonPositions[i].z,
...
UnitAI *GetAI_boss_akilzon(Creature *creature) { return new boss_akilzonAI(creature); }
...
struct mob_soaring_eagleAI : public ScriptedAI {
  ...
  void Reset() override { m_uiEagleSwoopTimer = urand(4000, 12000); }
...
UnitAI *GetAI_mob_soaring_eagle(Creature *creature) { return new mob_soaring_eagleAI(creature); }

struct TeleportSelf : public SpellScript {
  void OnDestTarget(Spell *spell) const override { spell->m_targets.m_destPos.z += 10.f; }
};
