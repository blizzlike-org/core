...
enum HalazziPhase { PHASE_SINGLE = 0, PHASE_TOTEM = 1, PHASE_FINAL = 2 };
...
struct boss_halazziAI : public CombatAI {
  boss_halazziAI(Creature *creature)
      : CombatAI(creature, HALAZZI_ACTION_MAX),
  ...
  void DoReuniteSpirits() {
    ...
    case 3:
      spellId = SPELL_TRANSFORM_TO_LYNX_75;
      break;
    case 2:
      spellId = SPELL_TRANSFORM_TO_LYNX_50;
      break;
    case 1:
      spellId = SPELL_TRANSFORM_TO_LYNX_25;
      break;
    }
...
enum { SPELL_LYNX_FRENZY = 43290, SPELL_SHRED_ARMOR = 43243 };
...
struct boss_spirit_lynxAI : public CombatAI {
  boss_spirit_lynxAI(Creature *creature)
      : CombatAI(creature, LYNX_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
...
struct npc_corrupted_lightning_totemAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 /*diff*/) override {
    if (m_creature->IsSpellReady(SPELL_LIGHTNING))
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_LIGHTNING, SELECT_FLAG_PLAYER))
        DoCastSpellIfCan(target, SPELL_LIGHTNING);
  }
};

UnitAI *GetAI_boss_halazzi(Creature *creature) { return new boss_halazziAI(creature); }

UnitAI *GetAI_boss_spirit_lynx(Creature *creature) { return new boss_spirit_lynxAI(creature); }
