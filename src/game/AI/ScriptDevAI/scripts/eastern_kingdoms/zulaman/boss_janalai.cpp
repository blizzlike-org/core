...
struct boss_janalaiAI : public CombatAI {
  boss_janalaiAI(Creature *creature)
      : CombatAI(creature, JANALAI_ACTION_MAX),
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(urand(0, 1) ? SAY_SLAY_1 : SAY_SLAY_2, m_creature); }
...
struct npc_amanishi_hatcherAI : public ScriptedAI {
  npc_amanishi_hatcherAI(Creature *creature)
  ...
  void MovementInform(uint32 type, uint32 pointId) override {
    ...
    case PATH_ID_LEFT:
      if (pointId != LEFT_LAST_POINT)
        return;
      break;
    case PATH_ID_RIGHT:
      if (pointId != RIGHT_LAST_POINT)
        return;
      break;
    case PATH_ID_LEFT_SHORT:
    case PATH_ID_RIGHT_SHORT:
      break;
    }
  ...
  void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit *invoker, uint32 miscValue) override {
    if (eventType == AI_EVENT_CUSTOM_A) {
      ...
      if (Unit *target = static_cast<Creature *>(invoker)->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
                                                                                 SELECT_FLAG_PLAYER))
        AttackStart(target);
  ...
  uint32 GetScriptData() override { return m_hatchlingCount; }
...
UnitAI *GetAI_boss_janalaiAI(Creature *creature) { return new boss_janalaiAI(creature); }

UnitAI *GetAI_npc_amanishi_hatcherAI(Creature *creature) { return new npc_amanishi_hatcherAI(creature); }
