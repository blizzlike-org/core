...
struct boss_malacrassAI : public CombatAI {
  boss_malacrassAI(Creature *creature)
      : CombatAI(creature, MALACRASS_ACTION_MAX),
  ...
  void DoInitializeAdds() {
    ...
    if (m_addsEntryList.empty()) {
      ...
      for (uint8 i = 0; i < MAX_ACTIVE_ADDS; ++i) {
        ...
        Creature *creature =
            m_creature->SummonCreature(aSpawnEntries[i][addVersion], m_aAddPositions[i][0], m_aAddPositions[i][1],
                                       m_aAddPositions[i][2], m_aAddPositions[i][3], TEMPSPAWN_DEAD_DESPAWN, 0);
    ...
    // Resummon the killed adds
      ...
      for (uint8 i = 0; i < MAX_ACTIVE_ADDS; ++i) {
        ...
        m_creature->SummonCreature(m_addsEntryList[i], m_aAddPositions[i][0], m_aAddPositions[i][1],
                                   m_aAddPositions[i][2], m_aAddPositions[i][3], TEMPSPAWN_DEAD_DESPAWN, 0);
      }
    }
  }
  ...
  void SummonedCreatureJustDied(Creature * /*summoned*/) override {
    switch (urand(0, 2)) {
    case 0:
      DoScriptText(SAY_ADD_DIED1, m_creature);
      break;
    case 1:
      DoScriptText(SAY_ADD_DIED2, m_creature);
      break;
    case 2:
      DoScriptText(SAY_ADD_DIED3, m_creature);
      break;
    }
  }
  ...
  bool CanUseSpecialAbility(uint32 spellIndex) {
    ...
    case TARGET_TYPE_RANDOM_NOT_MAIN:
      target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case MALACRASS_PLAYER_ABILITY_4:
      if (CanUseSpecialAbility(action - MALACRASS_PLAYER_ABILITY_1))
        ResetCombatAction(action,
                          aMalacrassStolenAbility[m_playerClass][action - MALACRASS_PLAYER_ABILITY_1].m_uiCooldown);
      break;
    }
  }
...
UnitAI *GetAI_boss_malacrass(Creature *creature) { return new boss_malacrassAI(creature); }
