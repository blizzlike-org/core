...
SD%Complete: 70
SDComment: Undead side 90% implemented, event needs better implementation, Barthildas relocation for reload case is
missing, Baron Combat handling is buggy. SDCategory: Stratholme EndScriptData */

#include "AI/ScriptDevAI/include/sc_common.h"
#include "stratholme.h"

instance_stratholme::instance_stratholme(Map *pMap)
    : ScriptedInstance(pMap), m_uiBaronRunTimer(0), m_uiBarthilasRunTimer(0), m_uiMindlessSummonTimer(0),
      m_uiSlaugtherSquareTimer(0), m_uiSlaughterDoorTimer(0), m_uiBlackGuardsTimer(0), m_uiAuriusSummonTimer(0),
      m_bIsSlaughterDoorOpen(false), m_uiYellCounter(0), m_uiMindlessCount(0), m_uiPostboxesUsed(0),
      m_uiSilverHandKilled(0), m_jarienKilled(false), m_sothosKilled(false) {
  Initialize();
}
...
void instance_stratholme::OnPlayerEnter(Player *pPlayer) {
  ...
  if (GetData(TYPE_BARON_RUN) == DONE)
    pPlayer->SummonCreature(NPC_YSIDA, aStratholmeLocation[8].m_fX, aStratholmeLocation[8].m_fY,
                            aStratholmeLocation[8].m_fZ, aStratholmeLocation[8].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
  // Baron ultimatum failed: summon Ysida outside the cage dead
  else if (GetData(TYPE_BARON_RUN) == FAIL) {
    if (Creature *pYsida = pPlayer->SummonCreature(NPC_YSIDA, aStratholmeLocation[8].m_fX, aStratholmeLocation[8].m_fY,
                                                   aStratholmeLocation[8].m_fZ, aStratholmeLocation[8].m_fO,
...
bool instance_stratholme::StartSlaugtherSquare() {
  if (m_auiEncounter[TYPE_BARONESS] == SPECIAL && m_auiEncounter[TYPE_NERUB] == SPECIAL &&
...
void instance_stratholme::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_BARON_RUN:
    ...
    case IN_PROGRESS:
      ...
      if (Creature *pBaron = GetSingleCreatureFromStorage(NPC_BARON)) {
        ...
        if (Creature *pYsida = pBaron->SummonCreature(NPC_YSIDA, aStratholmeLocation[7].m_fX,
                                                      aStratholmeLocation[7].m_fY, aStratholmeLocation[7].m_fZ,
                                                      aStratholmeLocation[7].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
  ...
  case TYPE_RAMSTEIN:
    if (uiData == SPECIAL) {
      ...
      if (!uiCount) {
        // Open the Slaughterhouse door and set a timer to close it after 10 sec to let some time to Ramstein to move
        // out
        ...
        if (Creature *pBaron = GetSingleCreatureFromStorage(NPC_BARON)) {
          ...
          if (Creature *pRamstein = pBaron->SummonCreature(NPC_RAMSTEIN, aStratholmeLocation[2].m_fX,
                                                           aStratholmeLocation[2].m_fY, aStratholmeLocation[2].m_fZ,
                                                           aStratholmeLocation[2].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
            pRamstein->GetMotionMaster()->MovePoint(0, aStratholmeLocation[5].m_fX, aStratholmeLocation[5].m_fY,
    ...
    if (uiData == DONE) {
      ...
      if (Creature *pBaron = GetSingleCreatureFromStorage(NPC_BARON)) {
        for (uint8 i = 0; i < 5; ++i) {
          float fX, fY, fZ;
          pBaron->GetRandomPoint(aStratholmeLocation[6].m_fX, aStratholmeLocation[6].m_fY, aStratholmeLocation[6].m_fZ,
                                 5.0f, fX, fY, fZ);
          if (Creature *pTemp = pBaron->SummonCreature(NPC_BLACK_GUARD, aStratholmeLocation[6].m_fX,
                                                       aStratholmeLocation[6].m_fY, aStratholmeLocation[6].m_fZ,
                                                       aStratholmeLocation[6].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
    ...
    if (uiData == IN_PROGRESS) {
      ...
      if (m_auiEncounter[TYPE_BARON_RUN] == IN_PROGRESS) {
        ...
        if (Creature *pYsida = GetSingleCreatureFromStorage(NPC_YSIDA)) {
          ...
          pYsida->GetMotionMaster()->MovePoint(0, aStratholmeLocation[8].m_fX, aStratholmeLocation[8].m_fY,
                                               aStratholmeLocation[8].m_fZ);
      ...
      if (GetData(TYPE_AURIUS) == DONE) {
        // Baron killed and Aurius is alive: give him his NPC Flags back, so players can complete the quest and fake his
        // death
  ...
  case TYPE_BARTHILAS_RUN:
    if (uiData == IN_PROGRESS) {
      ...
      if (pBarthilas && pBarthilas->IsAlive() && !pBarthilas->IsInCombat()) {
        ...
        pBarthilas->GetMotionMaster()->MovePoint(0, aStratholmeLocation[0].m_fX, aStratholmeLocation[0].m_fY,
                                                 aStratholmeLocation[0].m_fZ);
  ...
  case TYPE_TRUE_MASTERS:
    ...
    return;
  case TYPE_AURIUS:
    // Prevent further players to complete the quest in that instance or autocomplete the follow-up quest. the flag will
    // be set back if event is succeed
    if (uiData == DONE) {
      if (Creature *pAurius = GetSingleCreatureFromStorage(NPC_AURIUS))
...
void instance_stratholme::OnCreatureDeath(Creature *pCreature) {
  ...
  case NPC_MINDLESS_UNDEAD:
    ...
    if (m_luiUndeadGUIDs.empty())
      m_uiBlackGuardsTimer = 60000; // Wait 1 min before sending the Black Guards outside. In Classic, this allows
                                    // players to resplenish health and mana
    break;
  case NPC_BLACK_GUARD:
...
void instance_stratholme::OnCreatureRespawn(Creature *creature) {
  ...
  case NPC_BARTHILAS:
    if (GetData(TYPE_BARTHILAS_RUN) != NOT_STARTED)
      creature->NearTeleportTo(aStratholmeLocation[1].m_fX, aStratholmeLocation[1].m_fY, aStratholmeLocation[1].m_fZ,
...
void instance_stratholme::DoSpawnScarletGuards(uint8 uiStep, Player *pSummoner) {
  ...
  for (uint8 i = 0; i < 2; i++) {
    if (Creature *pTemp = pSummoner->SummonCreature(uiNPCEntry[i], aScarletGuards[uiIndex].m_fX,
                                                    aScarletGuards[uiIndex].m_fY, aScarletGuards[uiIndex].m_fZ,
                                                    aScarletGuards[uiIndex].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
      pTemp->SetWalk(false);
      pTemp->GetMotionMaster()->MovePoint(0, aScarletGuards[uiIndex + i + 1].m_fX, aScarletGuards[uiIndex + i + 1].m_fY,
                                          aScarletGuards[uiIndex + i + 1].m_fZ);
    }
  }
}
...
void instance_stratholme::DoSpawnScourgeInvaders(uint8 uiStep, Player *pSummoner) {
  ...
  switch (urand(0, 1)) {
  case 0:
    uiMobEntry = NPC_SKELETAL_GUARDIAN;
    break;
  case 1:
    uiMobEntry = NPC_SKELETAL_BERSERKER;
    break;
  }
  ...
  for (uint8 i = 0; i < 5; i++) {
    ...
    if (Creature *pTemp = pSummoner->SummonCreature(uiMobList[i], aScourgeInvaders[uiIndex].m_fX,
                                                    aScourgeInvaders[uiIndex].m_fY, aScourgeInvaders[uiIndex].m_fZ,
                                                    aScourgeInvaders[uiIndex].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
      pTemp->SetWalk(false);
      pTemp->GetRandomPoint(aScourgeInvaders[uiIndex + 1].m_fX, aScourgeInvaders[uiIndex + 1].m_fY,
                            aScourgeInvaders[uiIndex + 1].m_fZ, 6.0f, fTargetPosX, fTargetPosY, fTargetPosZ);
...
void instance_stratholme::DoMoveBackDefenders(uint8 uiStep, Creature *pCreature) {
  ...
  for (GuidList::const_iterator itr = m_suiCrimsonDefendersLowGuids[uiIndex].begin();
       itr != m_suiCrimsonDefendersLowGuids[uiIndex].end(); ++itr) {
    ...
    if (pGuard && pGuard->IsAlive() && !pGuard->IsInCombat()) {
      ...
      pGuard->GetMotionMaster()->MovePoint(0, aScarletLastStand[uiTreshold + uiFoundGuards].m_fX,
                                           aScarletLastStand[uiTreshold + uiFoundGuards].m_fY,
                                           aScarletLastStand[uiTreshold + uiFoundGuards].m_fZ);
      uiFoundGuards++;
    }
...
void instance_stratholme::DoScarletBastionDefense(uint8 uiStep, Creature *pCreature) {
  ...
  case STAIRS:
    ...
    return;
  case TIMMY:
    pCreature->SummonCreature(NPC_TIMMY_THE_CRUEL, aScourgeInvaders[0].m_fX, aScourgeInvaders[0].m_fY,
                              aScourgeInvaders[0].m_fZ, aScourgeInvaders[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
    return;
...
void instance_stratholme::Update(uint32 uiDiff) {
  // Loop over the two Gate traps, each one has up to three timers (trap reset, gate opening delay, critters spawning
  // delay)
  for (uint8 i = 0; i < 2; i++) {
  ...
  if (m_uiBlackGuardsTimer) {
    if (m_uiBlackGuardsTimer <= uiDiff) {
      ...
      for (GuidList::const_iterator itr = m_luiGuardGUIDs.begin(); itr != m_luiGuardGUIDs.end(); ++itr) {
        ...
        if (pGuard && pGuard->IsAlive() && !pGuard->IsInCombat()) {
          float fX, fY, fZ;
          pGuard->GetRandomPoint(aStratholmeLocation[5].m_fX, aStratholmeLocation[5].m_fY, aStratholmeLocation[5].m_fZ,
                                 10.0f, fX, fY, fZ);
  ...
  if (m_uiBarthilasRunTimer) {
    if (m_uiBarthilasRunTimer <= uiDiff) {
      ...
      if (pBarthilas && pBarthilas->IsAlive() && !pBarthilas->IsInCombat()) {
        ...
        pBarthilas->NearTeleportTo(aStratholmeLocation[1].m_fX, aStratholmeLocation[1].m_fY,
                                   aStratholmeLocation[1].m_fZ, aStratholmeLocation[1].m_fO);
      }
  ...
  if (m_uiAuriusSummonTimer) {
    if (m_uiAuriusSummonTimer <= uiDiff) {
      ...
      if (pAurius && pAurius->IsAlive() && !pAurius->IsInCombat()) {
        if (Creature *pBaron = GetSingleCreatureFromStorage(NPC_BARON)) {
          float fX, fY, fZ;
          pBaron->GetRandomPoint(pBaron->GetPositionX(), pBaron->GetPositionY(), pBaron->GetPositionZ(), 4.0f, fX, fY,
                                 fZ);
  ...
  if (m_uiBaronRunTimer && GetData(TYPE_BARON) != IN_PROGRESS) {
    ...
    if (m_uiBaronRunTimer <= uiDiff) {
      if (GetData(TYPE_BARON_RUN) != FAIL) {
        ...
        m_uiBaronRunTimer =
            8000; // We reset the timer so the speech of Ysida is not said at the same time than the Baron's one
      } else {
        ...
        debug_log("SD2: Instance Stratholme: Baron run event reached end. Event has state %u.",
    ...
    if (m_uiMindlessCount < 30) {
      if (m_uiMindlessSummonTimer <= uiDiff) {
        if (Creature *pBaron = GetSingleCreatureFromStorage(NPC_BARON)) {
          ...
          if (Creature *pTemp = pBaron->SummonCreature(NPC_MINDLESS_UNDEAD, aStratholmeLocation[4].m_fX,
                                                       aStratholmeLocation[4].m_fY, aStratholmeLocation[4].m_fZ,
                                                       aStratholmeLocation[4].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0)) {
            float fX, fY, fZ;
            pBaron->GetRandomPoint(aStratholmeLocation[5].m_fX, aStratholmeLocation[5].m_fY,
                                   aStratholmeLocation[5].m_fZ, 20.0f, fX, fY, fZ);
            pTemp->SetWalk(false);
    ...
    if (m_uiSlaugtherSquareTimer <= uiDiff) {
      ...
      for (auto itr : m_sAbomnationGUID) {
        ...
        if (!pAbom || !pAbom->IsAlive() ||
            pAbom->GetMotionMaster()->GetCurrentMovementGeneratorType() == POINT_MOTION_TYPE)
          continue;
        ...
        if (!pAbom->IsInCombat()) {
          if (GameObject *pDoor = GetSingleGameObjectFromStorage(GO_PORT_SLAUGTHER)) {
            float fX, fY, fZ;
            pAbom->GetRandomPoint(pDoor->GetPositionX(), pDoor->GetPositionY(), pDoor->GetPositionZ(), 10.0f, fX, fY,
                                  fZ);
