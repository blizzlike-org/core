...
struct boss_silver_hand_bossesAI : public ScriptedAI {
  ...
  bool CanUseSpecialAbility(uint32 uiIndex) {
    ...
    case TARGET_TYPE_RANDOM:
      pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, m_aSilverHandAbility[uiIndex].m_uiSpellId,
                                                  SELECT_FLAG_IN_LOS);
      break;
...
UnitAI *GetAI_boss_silver_hand_bossesAI(Creature *pCreature) { return new boss_silver_hand_bossesAI(pCreature); }
