...
enum { SPELL_FROSTBOLT = 17503, SPELL_DRAIN_LIFE = 17238, SPELL_DRAIN_MANA = 17243, SPELL_ICE_TOMB = 16869 };
...
struct boss_maleki_the_pallidAI : public ScriptedAI {
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiDrainManaTimer < uiDiff) {
      if (Unit *pTarget =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DRAIN_MANA, SELECT_FLAG_POWER_MANA)) {
...
UnitAI *GetAI_boss_maleki_the_pallid(Creature *pCreature) { return new boss_maleki_the_pallidAI(pCreature); }
