...
struct boss_baron_geddonAI : public CombatAI {
  boss_baron_geddonAI(Creature *creature)
      : CombatAI(creature, GEDDON_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) {
    ...
    case GEDDON_LIVING_BOMB: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_LIVING_BOMB, SELECT_FLAG_PLAYER))
...
UnitAI *GetAI_boss_baron_geddon(Creature *creature) { return new boss_baron_geddonAI(creature); }
