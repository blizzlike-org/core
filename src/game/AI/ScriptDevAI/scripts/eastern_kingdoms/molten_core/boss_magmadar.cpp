...
struct boss_magmadarAI : public CombatAI {
  boss_magmadarAI(Creature *creature)
      : CombatAI(creature, MAGMADAR_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case MAGMADAR_LAVABOMB: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_LAVABOMB, SELECT_FLAG_POWER_NOT_MANA))
        if (DoCastSpellIfCan(target, SPELL_LAVABOMB) == CAST_OK)
          ResetCombatAction(action, urand(12000, 15000));
      break;
    }
    case MAGMADAR_LAVABOMB_MANA: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_LAVABOMB_MANA,
...
UnitAI *GetAI_boss_magmadar(Creature *creature) { return new boss_magmadarAI(creature); }
