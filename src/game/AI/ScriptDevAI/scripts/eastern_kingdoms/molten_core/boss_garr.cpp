...
enum {
  ...
  SPELL_SEPARATION_ANXIETY =
      23487, // Aura cast on himself by Garr, if adds move out of range, they will cast spell 23492 on themselves
...
struct boss_garrAI : public CombatAI {
  boss_garrAI(Creature *creature)
      : CombatAI(creature, GARR_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
...
UnitAI *GetAI_boss_garr(Creature *creature) { return new boss_garrAI(creature); }

UnitAI *GetAI_mob_firesworn(Creature *creature) { return new mob_fireswornAI(creature); }
