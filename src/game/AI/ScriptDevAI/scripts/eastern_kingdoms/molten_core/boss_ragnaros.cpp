...
struct boss_ragnarosAI : public CombatAI {
  boss_ragnarosAI(Creature *creature)
      : CombatAI(creature, RAGNAROS_ACTION_MAX),
  ...
  void JustSummoned(Creature *summoned) override {
    if (summoned->GetEntry() == NPC_SON_OF_FLAME) {
      ...
      summoned->CastSpell(nullptr, SPELL_INTENSE_HEAT, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
  ...
  void SpellHitTarget(Unit *target, const SpellEntry *spellInfo, SpellMissInfo /*missInfo*/) override {
    ...
    else if (spellInfo->Id == SPELL_ELEMENTAL_FIRE_KILL && target->GetTypeId() == TYPEID_UNIT &&
             target->GetEntry() == NPC_MAJORDOMO)
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case RAGNAROS_MIGHT_OF_RAGNAROS: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MIGHT_OF_RAGNAROS,
    ...
    case RAGNAROS_MAGMA_BLAST: {
      ...
      // Spam Waterbolt spell when not tanked
        ...
        if (m_rangeCheckState > 1) {
          if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MAGMA_BLAST,
                                                               SELECT_FLAG_PLAYER)) {
    ...
    case RAGNAROS_LAVA_BURST: {
      if (DoCastSpellIfCan(nullptr, SPELL_LAVA_BURST) == CAST_OK)
        ResetCombatAction(action,
                          urand(5000, 25000)); // Upper value is guess work based on videos. It could be up to 45 secs
      break;
    }
    }
  }
...
UnitAI *GetAI_boss_ragnaros(Creature *creature) { return new boss_ragnarosAI(creature); }
