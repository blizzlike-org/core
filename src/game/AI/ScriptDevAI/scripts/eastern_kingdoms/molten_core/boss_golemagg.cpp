...
struct boss_golemaggAI : public CombatAI {
  boss_golemaggAI(Creature *creature)
      : CombatAI(creature, GOLEMAGG_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case GOLEMAGG_PYROBLAST: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_PYROBLAST, SELECT_FLAG_PLAYER))
...
struct mob_core_ragerAI : public CombatAI {
  mob_core_ragerAI(Creature *creature)
      : CombatAI(creature, CORE_RAGER_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
...
UnitAI *GetAI_boss_golemagg(Creature *creature) { return new boss_golemaggAI(creature); }

UnitAI *GetAI_mob_core_rager(Creature *creature) { return new mob_core_ragerAI(creature); }
