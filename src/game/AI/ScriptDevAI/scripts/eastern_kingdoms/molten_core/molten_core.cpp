...
instance_molten_core::instance_molten_core(Map *pMap) : ScriptedInstance(pMap) { Initialize(); }

void instance_molten_core::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_molten_core::OnObjectCreate(GameObject *pGo) {
  ...
  case GO_CIRCLE_GOLEMAGG:
    ...
    m_goEntryGuidStore[pGo->GetEntry()] = pGo->GetObjectGuid();
    for (auto m_aMoltenCoreRune : m_aMoltenCoreRunes) {
      if (m_aMoltenCoreRune.m_uiFlamesCircleEntry == pGo->GetEntry() &&
          (GetData(m_aMoltenCoreRune.m_uiType) == SPECIAL || GetData(m_aMoltenCoreRune.m_uiType) == DONE)) {
...
void instance_molten_core::SpawnMajordomo(Unit *summoner, bool initialSummon, uint8 summonPos) {
  ...
  if (Creature *pMajordomo = summoner->SummonCreature(
          m_aMajordomoLocations[summonPos].m_uiEntry, m_aMajordomoLocations[summonPos].m_fX,
          m_aMajordomoLocations[summonPos].m_fY, m_aMajordomoLocations[summonPos].m_fZ,
          m_aMajordomoLocations[summonPos].m_fO, TEMPSPAWN_MANUAL_DESPAWN, 2 * HOUR * IN_MILLISECONDS)) {
    ...
    } else // Else yell and summon adds
    {
      ...
      for (auto &m_aBosspawnLoc : m_aBosspawnLocs)
        pMajordomo->SummonCreature(m_aBosspawnLoc.m_uiEntry, m_aBosspawnLoc.m_fX, m_aBosspawnLoc.m_fY,
                                   m_aBosspawnLoc.m_fZ, m_aBosspawnLoc.m_fO, TEMPSPAWN_MANUAL_DESPAWN,
                                   DAY * IN_MILLISECONDS);
    }
  }
}
  ...
  if (val == "spawnmajordomo") {
    ...
    if (summonPos > 1) {
      handler->PSendSysMessage(
          "Could not spawn majordomo because spawn %u was invalid. Please specify a value between 0-1", summonPos);
      return;
    }
