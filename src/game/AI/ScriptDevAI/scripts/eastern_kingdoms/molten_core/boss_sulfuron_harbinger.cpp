...
struct boss_sulfuronAI : public CombatAI {
  boss_sulfuronAI(Creature *creature)
      : CombatAI(creature, SULFURON_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case SULFURON_FLAME_SPEAR: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOWWORD_PAIN, SELECT_FLAG_PLAYER))
...
struct mob_flamewaker_priestAI : public CombatAI {
  mob_flamewaker_priestAI(Creature *creature)
      : CombatAI(creature, FLAMEWAKER_PRIEST_ACTION_MAX),
        m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())) {
...
UnitAI *GetAI_boss_sulfuron(Creature *creature) { return new boss_sulfuronAI(creature); }

UnitAI *GetAI_mob_flamewaker_priest(Creature *creature) { return new mob_flamewaker_priestAI(creature); }
