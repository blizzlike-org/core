  ...
  void DoSpawnChallengers() {
    for (uint8 i = 0; i < MAX_CHALLENGER; ++i) {
      if (Creature *pCreature = m_creature->SummonCreature(
              uiChallengerId[i], fChallengerLoc[i][0], fChallengerLoc[i][1], fChallengerLoc[i][2], fChallengerLoc[i][3],
...
UnitAI *GetAI_npc_kelerun_bloodmourn(Creature *pCreature) { return new npc_kelerun_bloodmournAI(pCreature); }
...
UnitAI *GetAI_npc_prospector_anvilward(Creature *pCreature) { return new npc_prospector_anvilwardAI(pCreature); }
...
struct npc_apprentice_mirvedaAI : public ScriptedAI {
  ...
  void JustDied(Unit * /*pKiller*/) override { FailEvent(); }
  ...
  void JustSummoned(Creature *pSummoned) override {
    ...
    pSummoned->GetMotionMaster()->MovePoint(0, m_creature->GetPositionX(), m_creature->GetPositionY(),
  ...
  void StartEvent(Player *pPlayer) {
    ...
    m_creature->SummonCreature(NPC_ANGERSHADE, 8756.952f, -7124.688f, 35.227f, 3.925692f,
                               TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 60000, true);
    m_creature->SummonCreature(NPC_GHARSUL, 8756.182f, -7130.453f, 35.227f, 3.816502f,
                               TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 60000, true);
    m_creature->SummonCreature(NPC_ANGERSHADE, 8761.380f, -7132.638f, 35.696f, 3.664015f,
                               TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 60000, true);
...
UnitAI *GetAI_npc_apprentice_mirvedaAI(Creature *pCreature) { return new npc_apprentice_mirvedaAI(pCreature); }
...
static const float aSummonPos[6][4] = {
    {8250.539f, -7239.028f, 139.7099f, 0.8975816f},  {8263.437f, -7181.188f, 139.4102f, 5.237229f},
    {8317.124f, -7210.098f, 140.1064f, 3.022202f},   {8293.848f, -7179.062f, 138.6693f, 4.153376f},
    {8239.229f, -7207.673f, 139.1196f, 0.06059111f}, {8301.548f, -7247.548f, 139.974f, 1.828518f}};
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiWaveTimer) {
      if (m_uiWaveTimer <= uiDiff) {
        if (m_bFirstWave) {
          for (uint8 i = 0; i < 3; ++i)
            m_creature->SummonCreature(NPC_ENRAGED_WRAITH, aSummonPos[i][0], aSummonPos[i][1], aSummonPos[i][2],
                                       aSummonPos[i][3], TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
...
UnitAI *GetAI_npc_infused_crystalAI(Creature *pCreature) { return new npc_infused_crystalAI(pCreature); }
