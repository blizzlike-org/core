...
instance_gnomeregan::instance_gnomeregan(Map *pMap) : ScriptedInstance(pMap) { Initialize(); }
...
void instance_gnomeregan::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_GRUBBIS:
    ...
    if (uiData == IN_PROGRESS) {
      ...
      if (!m_luiExplosiveChargeGUIDs.empty()) {
        ...
        for (GuidList::const_iterator itr = m_luiExplosiveChargeGUIDs.begin(); itr != m_luiExplosiveChargeGUIDs.end();
             ++itr) {
        ...
        if (pCaveInSouth && pCaveInNorth) {
          for (auto &lExplosiveCharge : lExplosiveCharges) {
            if (lExplosiveCharge->GetDistanceOrder(pCaveInSouth, pCaveInNorth) &&
  ...
  case TYPE_EXPLOSIVE_CHARGE:
    ...
    case DATA_EXPLOSIVE_CHARGE_USE:
      ...
      if (!pBlastmaster)
        break;
      for (GuidList::const_iterator itr = m_luiSpawnedExplosiveChargeGUIDs.begin();
