...
struct boss_netherspiteAI : public CombatAI {
  boss_netherspiteAI(Creature *creature)
      : CombatAI(creature, NETHERSPITE_ACTION_MAX),
  ...
  void DoSummonPortals() {
    for (uint8 i = 0; i < MAX_PORTALS; ++i)
      m_creature->SummonCreature(m_vPortalEntryList[i], aPortalCoordinates[i].fX, aPortalCoordinates[i].fY,
                                 aPortalCoordinates[i].fZ, aPortalCoordinates[i].fO, TEMPSPAWN_DEAD_DESPAWN, 0);
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case NETHERSPITE_NETHERBREATH: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_NETHERBREATH, SELECT_FLAG_PLAYER))
...
struct npc_netherspite_portalAI : public Scripted_NoMovementAI {
  ...
  void Reset() override { m_uiCurrentBeamTarget = ObjectGuid(); }
...
struct NetherBeam : public SpellScript {
  void OnInit(Spell *spell) const override { spell->SetFilteringScheme(EFFECT_INDEX_0, false, SCHEME_CLOSEST); }
