...
struct boss_maiden_of_virtueAI : public CombatAI {
  boss_maiden_of_virtueAI(Creature *creature)
      : CombatAI(creature, MAIDEN_ACTION_MAX),
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float x, float y, float /*z*/) { return y > -2071.f && x > -10924.f; });
  }
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case MAIDEN_ACTION_HOLY_FIRE: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_HOLYFIRE,
    ...
    case MAIDEN_ACTION_HOLY_WRATH: {
      if (Unit *target =
              m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_HOLYWRATH, SELECT_FLAG_PLAYER))
