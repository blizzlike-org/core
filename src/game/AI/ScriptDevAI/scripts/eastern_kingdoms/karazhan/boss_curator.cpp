...
static uint32 astralFlareSpells[]{SPELL_ASTRAL_FLARE_NE, SPELL_ASTRAL_FLARE_NW, SPELL_ASTRAL_FLARE_SE,
                                  SPELL_ASTRAL_FLARE_SW};
...
struct boss_curatorAI : public CombatAI {
  boss_curatorAI(Creature *creature)
      : CombatAI(creature, CURATOR_ACTION_MAX),
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float x, float y, float z) { return y < -1926.f && z < 164.f && x > -11045.f; });
    Reset();
  }
  ...
  uint32 GetSubsequentActionTimer(uint32 id) {
    ...
    case CURATOR_ACTION_HATEFUL_BOLT:
      return GetActionReadyStatus(CURATOR_ACTION_ARCANE_INFUSION) ? 15000 : 7000;
    default:
