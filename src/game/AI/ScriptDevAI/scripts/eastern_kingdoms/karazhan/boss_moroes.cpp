...
static const float guestLocations[MAX_ACTIVE_GUESTS][4] = {{-10987.086f, -1884.002f, 81.73f, 1.352331f},
                                                           {-10989.889f, -1882.592f, 81.73f, 0.932928f},
                                                           {-10979.159f, -1885.326f, 81.73f, 2.017563f},
                                                           {-10976.789f, -1883.946f, 81.73f, 2.328581f}};

static const uint32 guests[MAX_GUESTS] = {NPC_LADY_KEIRA_BERRYBUCK,        NPC_LADY_CATRIONA_VON_INDI,
                                          NPC_LORD_CRISPIN_FERENCE,        NPC_BARON_RAFE_DREUGER,
                                          NPC_BARONESS_DOROTHEA_MILLSTIPE, NPC_LORD_ROBIN_DARIS};
...
struct boss_moroesAI : public CombatAI {
  boss_moroesAI(Creature *creature)
      : CombatAI(creature, MOROES_ACTION_MAX),
  ...
  void DoSpawnGuests() {
    ...
    if (m_vGuestsEntryList.empty()) {
      ...
      for (uint8 i = 0; i < MAX_ACTIVE_GUESTS; ++i)
        m_creature->SummonCreature(m_vGuestsEntryList[i], guestLocations[i][0], guestLocations[i][1],
                                   guestLocations[i][2], guestLocations[i][3], TEMPSPAWN_CORPSE_DESPAWN, 0);
    }
  ...
  void ExecuteAction(uint32 action) override {
    switch (action) {
    case MOROES_ACTION_VANISH: {
      Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_GARROTE,
    ...
    case MOROES_ACTION_BLIND: {
      Unit *target =
          m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_GARROTE,
                                            SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_AURA | SELECT_FLAG_SKIP_TANK);
      if (!target) // if no target without garrote found - select any random
        target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER);
      if (!target)
        break;
