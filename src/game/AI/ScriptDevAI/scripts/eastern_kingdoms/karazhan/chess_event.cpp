...
enum {
  ...
  SPELL_MOVE_2 = 37388,       // spell which selects AI move square (for long range pieces)
                        // SPELL_MOVE_PAWN              = 37146,                    // individual move spells (used only
                        // by controlled npcs) SPELL_MOVE_KNIGHT            = 37144, SPELL_MOVE_QUEEN             =
                        // 37148, SPELL_MOVE_ROCK              = 37151, SPELL_MOVE_BISHOP            = 37152,
  ...
  SPELL_IS_SQUARE_USED = 39400, // cast when a chess piece moves to another square
                                // SPELL_SQUARED_OCCUPIED       = 39399,                    // triggered by 39400; used
                                // to check if the square is occupied (if hits a target); Missing in 2.4.3
  ...
  SPELL_MELEE_DAMAGE = 32247,          // melee damage spell - used by all chess pieces
                              // SPELL_AI_SNAPSHOT_TIMER      = 37440,                    // used to trigger spell
                              // 32260; purpose and usage unk SPELL_DISABLE_SQUARE_SELF    = 32260, // used when a piece
                              // moves to another square SPELL_AI_ACTION_TIMER        = 37504,                    //
                              // handle some kind of event check. Cast by npc 17459. Currently the way it works is unk
                              // SPELL_DISABLE_SQUARE         = 30271, // not used SPELL_FIND_ENEMY             = 32303,
                              // // not used SPELL_MOVE_NEAR_UNIT         = 30417,                    // not used
...
struct npc_echo_of_medivhAI : public ScriptedAI {
  ...
  void Reset() override { m_uiCheatTimer = 90000; }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiCheatTimer < uiDiff) {
      DoCastSpellIfCan(m_creature, urand(0, 1)
                                       ? (m_pInstance->GetPlayerTeam() == ALLIANCE ? SPELL_HAND_OF_MEDIVH_HORDE
                                                                                   : SPELL_HAND_OF_MEDIVH_ALLIANCE)
                                       : (m_pInstance->GetPlayerTeam() == ALLIANCE ? SPELL_FURY_OF_MEDIVH_ALLIANCE
                                                                                   : SPELL_FURY_OF_MEDIVH_HORDE));

      switch (urand(0, 2)) {
      case 0:
        DoPlaySoundToSet(m_creature, SOUND_ID_CHEAT_1);
        break;
      case 1:
        DoPlaySoundToSet(m_creature, SOUND_ID_CHEAT_2);
        break;
      case 2:
        DoPlaySoundToSet(m_creature, SOUND_ID_CHEAT_3);
        break;
      }
...
UnitAI *GetAI_npc_echo_of_medivh(Creature *pCreature) { return new npc_echo_of_medivhAI(pCreature); }
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if (pInstance->GetData(TYPE_CHESS) != DONE && pInstance->GetData(TYPE_CHESS) != SPECIAL)
      ...
      if (pInstance->GetData(TYPE_CHESS) == SPECIAL)
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_RESET_BOARD, GOSSIP_SENDER_MAIN,
...
struct npc_chess_piece_genericAI : public Scripted_NoMovementAI {
  ...
  Unit *GetTargetByType(uint8 uiType, float fRange, float fArc = M_PI_F) const {
    ...
    uint32 uiTeam =
        m_creature->getFaction() == FACTION_ID_CHESS_ALLIANCE ? FACTION_ID_CHESS_HORDE : FACTION_ID_CHESS_ALLIANCE;
  ...
  Unit *GetMovementSquare() const {
    ...
    m_pInstance->GetChessPiecesByFaction(lTempList, m_creature->getFaction() == FACTION_ID_CHESS_ALLIANCE
                                                        ? FACTION_ID_CHESS_HORDE
                                                        : FACTION_ID_CHESS_ALLIANCE);
    for (GuidList::const_iterator itr = lTempList.begin(); itr != lTempList.end(); ++itr) {
      Creature *pTemp = m_creature->GetMap()->GetCreature(*itr);
      if (pTemp && pTemp->IsAlive())
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (!m_pInstance ||
        (m_pInstance->GetData(TYPE_CHESS) != IN_PROGRESS && m_pInstance->GetData(TYPE_CHESS) != SPECIAL))
      return;
    ...
    if (m_uiMoveTimer) {
      if (m_uiMoveTimer <= uiDiff) {
        if (Creature *pSquare = m_creature->GetMap()->GetCreature(m_currentSquareGuid)) {
          ...
          m_creature->GetMotionMaster()->MovePoint(1, pSquare->GetPositionX(), pSquare->GetPositionY(),
                                                   pSquare->GetPositionZ());
        }
        m_uiMoveTimer = 0;
      } else
...
bool EffectDummyCreature_npc_chess_generic(Unit *pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
UnitAI *GetAI_npc_king_llane(Creature *pCreature) { return new npc_king_llaneAI(pCreature); }
...
bool GossipHello_npc_king_llane(Player *pPlayer, Creature *pCreature) {
  ...
  if (instance_karazhan *pInstance = (instance_karazhan *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) != DONE && pPlayer->GetTeam() == ALLIANCE) || pInstance->IsFriendlyGameReady())
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_KING_LLANE, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_warchief_blackhand(Creature *pCreature) { return new npc_warchief_blackhandAI(pCreature); }
...
bool GossipHello_npc_warchief_blackhand(Player *pPlayer, Creature *pCreature) {
  ...
  if (instance_karazhan *pInstance = (instance_karazhan *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) != DONE && pPlayer->GetTeam() == HORDE) || pInstance->IsFriendlyGameReady())
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_WARCHIEF_BLACKHAND, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_human_conjurer(Creature *pCreature) { return new npc_human_conjurerAI(pCreature); }
...
bool GossipHello_npc_human_conjurer(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == ALLIANCE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_HUMAN_CONJURER, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_orc_warlock(Creature *pCreature) { return new npc_orc_warlockAI(pCreature); }
...
bool GossipHello_npc_orc_warlock(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == HORDE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ORC_WARLOCK, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
struct npc_human_footmanAI : public npc_chess_piece_genericAI {
  ...
  void JustDied(Unit *pKiller) override {
    ...
    if (m_pInstance->GetPlayerTeam() == ALLIANCE) {
      switch (urand(0, 2)) {
      case 0:
        DoPlaySoundToSet(pMedivh, SOUND_ID_LOSE_PAWN_PLAYER_1);
        break;
      case 1:
        DoPlaySoundToSet(pMedivh, SOUND_ID_LOSE_PAWN_PLAYER_2);
        break;
      case 2:
        DoPlaySoundToSet(pMedivh, SOUND_ID_LOSE_PAWN_PLAYER_3);
        break;
      }
    } else {
      switch (urand(0, 2)) {
      case 0:
        DoPlaySoundToSet(pMedivh, SOUND_ID_LOSE_PAWN_MEDIVH_1);
        break;
      case 1:
        DoPlaySoundToSet(pMedivh, SOUND_ID_LOSE_PAWN_MEDIVH_2);
        break;
      case 2:
        DoPlaySoundToSet(pMedivh, SOUND_ID_LOSE_PAWN_MEDIVH_3);
        break;
      }
    }
...
UnitAI *GetAI_npc_human_footman(Creature *pCreature) { return new npc_human_footmanAI(pCreature); }
...
bool GossipHello_npc_human_footman(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == ALLIANCE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_HUMAN_FOOTMAN, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_orc_grunt(Creature *pCreature) { return new npc_orc_gruntAI(pCreature); }
...
bool GossipHello_npc_orc_grunt(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == HORDE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ORC_GRUNT, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_water_elemental(Creature *pCreature) { return new npc_water_elementalAI(pCreature); }
...
bool GossipHello_npc_water_elemental(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == ALLIANCE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_WATER_ELEMENTAL, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_summoned_daemon(Creature *pCreature) { return new npc_summoned_daemonAI(pCreature); }
...
bool GossipHello_npc_summoned_daemon(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == HORDE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_SUMMONED_DEAMON, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_human_charger(Creature *pCreature) { return new npc_human_chargerAI(pCreature); }
...
bool GossipHello_npc_human_charger(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == ALLIANCE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_HUMAN_CHARGER, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_orc_wolf(Creature *pCreature) { return new npc_orc_wolfAI(pCreature); }
...
bool GossipHello_npc_orc_wolf(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == HORDE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ORC_WOLF, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_human_cleric(Creature *pCreature) { return new npc_human_clericAI(pCreature); }
...
bool GossipHello_npc_human_cleric(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == ALLIANCE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_HUMAN_CLERIC, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
UnitAI *GetAI_npc_orc_necrolyte(Creature *pCreature) { return new npc_orc_necrolyteAI(pCreature); }
...
bool GossipHello_npc_orc_necrolyte(Player *pPlayer, Creature *pCreature) {
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    if ((pInstance->GetData(TYPE_CHESS) == IN_PROGRESS && pPlayer->GetTeam() == HORDE) ||
        pInstance->GetData(TYPE_CHESS) == SPECIAL)
      pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ORC_NECROLYTE, GOSSIP_SENDER_MAIN,
                                  GOSSIP_ACTION_INFO_DEF + 1);
  }
...
struct go_chessboard : public GameObjectAI {
  go_chessboard(GameObject *go)
      : GameObjectAI(go), m_auraSearchTimer(1000),
        m_spellInfo(sSpellTemplate.LookupEntry<SpellEntry>(SPELL_GAME_IN_SESSION)), m_started(false) {}
...
GameObjectAI *GetGOAI_go_chessboard(GameObject *go) { return new go_chessboard(go); }
