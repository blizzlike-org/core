...
struct boss_aranAI : public RangedCombatAI {
  boss_aranAI(Creature *creature)
      : RangedCombatAI(creature, ARAN_ACTION_MAX),
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ARAN_ACTION_DRINK:
      if (m_creature->GetPowerType() == POWER_MANA &&
          (m_creature->GetPower(POWER_MANA) * 100 / m_creature->GetMaxPower(POWER_MANA)) < 20) {
...
void AddSC_boss_shade_of_aran() {
  ...
  sObjectMgr.AddCreatureCooldown(NPC_SHADOW_OF_ARAN, SPELL_FROSTBOLT,
                                 boss_aranAI::GetNormalSpellCooldown(SPELL_FROSTBOLT),
                                 boss_aranAI::GetNormalSpellCooldown(SPELL_FROSTBOLT));
  sObjectMgr.AddCreatureCooldown(NPC_SHADOW_OF_ARAN, SPELL_FIREBALL,
                                 boss_aranAI::GetNormalSpellCooldown(SPELL_FIREBALL),
                                 boss_aranAI::GetNormalSpellCooldown(SPELL_FIREBALL));
  sObjectMgr.AddCreatureCooldown(NPC_SHADOW_OF_ARAN, SPELL_ARCANE_MISSILES,
                                 boss_aranAI::GetNormalSpellCooldown(SPELL_ARCANE_MISSILES),
                                 boss_aranAI::GetNormalSpellCooldown(SPELL_ARCANE_MISSILES));
}
