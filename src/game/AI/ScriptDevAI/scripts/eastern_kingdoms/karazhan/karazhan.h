...
enum {
  ...
  MIN_BASEMENT_MOBS =
      45, // note: some DB provides have more spawns, up to 55. However we need to keep this compatible with all
  ...
  // NPC_WAITING_ROOM_STALKER     = 17459,                    // trigger which marks the teleport location of the
  // players; also used to cast some control spells during the game
...
enum OperaEvents { OPERA_EVENT_WIZARD_OZ = 1, OPERA_EVENT_RED_RIDING_HOOD = 2, OPERA_EVENT_ROMULO_AND_JUL = 3 };
...
public:
  ...
  void GetChessPiecesByFaction(GuidList &lList, uint32 uiFaction) {
    lList = uiFaction == FACTION_ID_CHESS_ALLIANCE ? m_lChessPiecesAlliance : m_lChessPiecesHorde;
  }

  GuidVector &GetAranTeleportNPCs() { return m_aranTeleportNPCs; }

  void GetNightbaneTriggers(GuidList &lList, bool bGround) {
    lList = bGround ? m_lNightbaneGroundTriggers : m_lNightbaneAirTriggers;
  }
