...
instance_karazhan::instance_karazhan(Map *pMap)
    : ScriptedInstance(pMap), m_uiOperaEvent(0), m_uiOzDeathCount(0), m_uiTeam(0), m_uiChessResetTimer(0),
      m_uiChessEndingTimer(0), m_uiAllianceStalkerCount(0), m_uiHordeStalkerCount(0), m_bFriendlyGame(false),
      m_bBasementBossReady(false) {
  Initialize();
}

void instance_karazhan::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_karazhan::Load(const char *chrIn) {
  ...
  loadStream >> m_auiEncounter[0] >> m_auiEncounter[1] >> m_auiEncounter[2] >> m_auiEncounter[3] >> m_auiEncounter[4] >>
      ...
      m_auiEncounter[10] >> m_uiOperaEvent;
...
void instance_karazhan::OnCreatureDeath(Creature *pCreature) {
  ...
  case NPC_STRAWMAN:
    ...
    if (m_uiOzDeathCount == MAX_OZ_OPERA_MOBS) {
      if (Creature *pCrone =
              pCreature->SummonCreature(NPC_CRONE, afChroneSpawnLoc[0], afChroneSpawnLoc[1], afChroneSpawnLoc[2],
                                        afChroneSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
  ...
  case NPC_COLDMIST_WIDOW:
    ...
    if (m_sBasementMobsSet.find(pCreature->GetObjectGuid()) != m_sBasementMobsSet.end()) {
      ...
      if (m_sBasementMobsSet.empty()) {
        ...
        if (Creature *pBoss = pCreature->SummonCreature(aBasementEnum[uiIndex].uiEntry, aBasementEnum[uiIndex].fX,
                                                        aBasementEnum[uiIndex].fY, aBasementEnum[uiIndex].fZ,
                                                        aBasementEnum[uiIndex].fO, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                                        2 * HOUR * IN_MILLISECONDS, true)) {
...
void instance_karazhan::DoPrepareChessEvent() {
  ...
  for (GuidList::const_iterator itr = m_lChessPiecesAlliance.begin(); itr != m_lChessPiecesAlliance.end(); ++itr) {
    if (Creature *pChessPiece = instance->GetCreature(*itr)) {
      ...
      if (!pSquare) {
        script_error_log("Instance Karazhan: ERROR Failed to properly load the Chess square for %s.",
                         pChessPiece->GetGuidStr().c_str());
        return;
      }
  ...
  lStalkers.clear();
  for (GuidList::const_iterator itr = m_lChessAllianceStalkerList.begin(); itr != m_lChessAllianceStalkerList.end();
       ++itr) {
    if (Creature *pTemp = instance->GetCreature(*itr))
      lStalkers.push_back(pTemp);
  }
  ...
  if (lStalkers.empty()) {
    script_error_log(
        "Instance Karazhan: ERROR Failed to properly load the alliance side stalkers for the Chess Event.");
    return;
  }
...
void instance_karazhan::DoMoveChessPieceToSides(uint32 uiSpellId, uint32 uiFaction, bool bGameEnd /*= false*/) {
  ...
  // get the proper statusBar npc
  Creature *pStatusBar =
      instance->GetCreature(uiFaction == FACTION_ID_CHESS_ALLIANCE ? m_AllianceStatusGuid : m_HordeStatusGuid);
  if (!pStatusBar)
    return;
...
void instance_karazhan::DoPrepareOperaStage(Creature *pOrganizer) {
  ...
  debug_log("SD2: Barnes Opera Event - Introduction complete - preparing encounter %d",
            GetData(TYPE_OPERA_PERFORMANCE));
  ...
  case OPERA_EVENT_RED_RIDING_HOOD:
    pOrganizer->SummonCreature(aOperaLocWolf.uiEntry, aOperaLocWolf.fX, aOperaLocWolf.fY, aOperaLocWolf.fZ,
                               aOperaLocWolf.fO, TEMPSPAWN_DEAD_DESPAWN, 0);
  ...
  case OPERA_EVENT_ROMULO_AND_JUL:
    pOrganizer->SummonCreature(aOperaLocJul.uiEntry, aOperaLocJul.fX, aOperaLocJul.fY, aOperaLocJul.fZ, aOperaLocJul.fO,
...
void instance_karazhan::Update(uint32 uiDiff) {
  if (m_uiChessResetTimer) {
    ...
    if (m_uiChessResetTimer <= uiDiff) {
      ...
      for (GuidList::const_iterator itr = m_lChessHordeStalkerList.begin(); itr != m_lChessHordeStalkerList.end();
           ++itr) {
        if (Creature *pTemp = instance->GetCreature(*itr)) {
          pTemp->Respawn();
          pTemp->HandleEmote(EMOTE_STATE_NONE);
            ...
            if (!m_uiVictoryTimersPhase[i]) {
              if (Creature *tool = instance->GetCreature(m_vVictoryDummyTools[i])) {
                if (Creature *square = instance->GetCreature(m_vChessSquares[urand(0, m_vChessSquares.size() - 1)]))
                  tool->NearTeleportTo(square->GetPositionX(), square->GetPositionY(), square->GetPositionZ(),
                                       square->GetOrientation());
              }
...
InstanceData *GetInstanceData_instance_karazhan(Map *pMap) { return new instance_karazhan(pMap); }
