...
struct boss_dorotheeAI : public RangedCombatAI {
  boss_dorotheeAI(Creature *creature)
      : RangedCombatAI(creature, DOROTHEE_ACTION_MAX),
  ...
  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_DOROTHEE_DEATH, m_creature); }
  ...
  void ExecuteAction(uint32 action) override {
    switch (action) {
    case DOROTHEE_ACTION_WATERBOLT: {
      if (DoCastSpellIfCan(m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER),
                           SPELL_WATERBOLT) == CAST_OK)
        ResetCombatAction(action, GetSubsequentActionTimer(DorotheeActions(action)));
      break;
    }
    case DOROTHEE_ACTION_FRIGHTENEDSCREAM: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_FRIGHTENEDSCREAM,
  ...
  void HandleIntro() { DoScriptText(SAY_DOROTHEE_AGGRO, m_creature); }
...
struct boss_strawmanAI : public ScriptedAI {
  ...
  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_STRAWMAN_DEATH, m_creature); }

  void KilledUnit(Unit * /*pVictim*/) override { DoScriptText(SAY_STRAWMAN_SLAY, m_creature); }
...
struct boss_tinheadAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_TINHEAD_AGGRO, m_creature); }
  ...
  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_TINHEAD_DEATH, m_creature); }

  void KilledUnit(Unit * /*pVictim*/) override { DoScriptText(SAY_TINHEAD_SLAY, m_creature); }
...
struct boss_roarAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_ROAR_AGGRO, m_creature); }
  ...
  void JustDied(Unit * /*pKiller*/) override { DoScriptText(SAY_ROAR_DEATH, m_creature); }

  void KilledUnit(Unit * /*pVictim*/) override { DoScriptText(SAY_ROAR_SLAY, m_creature); }
...
struct boss_croneAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override {
    ...
    if (Creature *cMainCyclone =
            m_creature->SummonCreature(NPC_CYCLONE, afCycloneSpawnLoc1[0], afCycloneSpawnLoc1[1], afCycloneSpawnLoc1[2],
                                       afCycloneSpawnLoc1[3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
      if (Creature *cFollowCyclone =
              m_creature->SummonCreature(NPC_CYCLONE, afCycloneSpawnLoc2[0], afCycloneSpawnLoc2[1],
                                         afCycloneSpawnLoc2[2], afCycloneSpawnLoc2[3], TEMPSPAWN_DEAD_DESPAWN, 0))
        cFollowCyclone->GetMotionMaster()->MoveFollow(cMainCyclone, cFollowCyclone->GetDistance(cMainCyclone),
                                                      cFollowCyclone->GetAngle(cMainCyclone));
      if (Creature *cFollowCyclone =
              m_creature->SummonCreature(NPC_CYCLONE, afCycloneSpawnLoc3[0], afCycloneSpawnLoc3[1],
                                         afCycloneSpawnLoc3[2], afCycloneSpawnLoc3[3], TEMPSPAWN_DEAD_DESPAWN, 0))
        cFollowCyclone->GetMotionMaster()->MoveFollow(cMainCyclone, cFollowCyclone->GetDistance(cMainCyclone),
                                                      cFollowCyclone->GetAngle(cMainCyclone));
    }
  }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    if (m_uiIntroTimer) {
      if (m_uiIntroTimer <= uiDiff) {
        DoScriptText(urand(0, 1) ? SAY_CRONE_INTRO : SAY_CRONE_INTRO2,
                     m_creature); // TODO: should be said at player who started event
        m_uiIntroTimer = 0;
      } else
...
bool GossipSelect_npc_grandmother(Player *pPlayer, Creature *creature, uint32 /*uiSender*/, uint32 uiAction) {
  ...
  case GOSSIP_ACTION_INFO_DEF:
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_GRANDMA_SECOND, GOSSIP_SENDER_MAIN,
  ...
  case GOSSIP_ACTION_INFO_DEF + 1:
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_GRANDMA_THIRD, GOSSIP_SENDER_MAIN,
...
struct boss_bigbadwolfAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_WOLF_AGGRO, m_creature); }
...
struct boss_julianneAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_JULIANNE_AGGRO, m_creature); }
  ...
  void UpdateAI(const uint32 uiDiff) override {
    ...
    if (m_uiSummonRomuloTimer) {
      if (m_uiSummonRomuloTimer <= uiDiff) {
        m_creature->SummonCreature(NPC_ROMULO, afRomuloSpawnLoc[0], afRomuloSpawnLoc[1], afRomuloSpawnLoc[2],
                                   afRomuloSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0);
        m_uiSummonRomuloTimer = 0;
      } else
...
bool EffectDummyCreature_spell_drink_poison(Unit * /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex,
...
struct boss_romuloAI : public ScriptedAI {
  ...
  void Aggro(Unit * /*pWho*/) override { DoScriptText(SAY_ROMULO_AGGRO, m_creature); }
