...
static const DialogueEntry aIntroDialogue[] = {
    {SAY_BARNES_OZ_1, NPC_BARNES, 6000},    {SAY_BARNES_OZ_2, NPC_BARNES, 18000},
    {SAY_BARNES_OZ_3, NPC_BARNES, 9000},    {SAY_BARNES_OZ_4, NPC_BARNES, 15000},
    {OPERA_EVENT_WIZARD_OZ, 0, 0},          {SAY_BARNES_HOOD_1, NPC_BARNES, 6000},
    {SAY_BARNES_HOOD_2, NPC_BARNES, 10000}, {SAY_BARNES_HOOD_3, NPC_BARNES, 14000},
    {SAY_BARNES_HOOD_4, NPC_BARNES, 15000}, {OPERA_EVENT_RED_RIDING_HOOD, 0, 0},
    {SAY_BARNES_RAJ_1, NPC_BARNES, 5000},   {SAY_BARNES_RAJ_2, NPC_BARNES, 7000},
    {SAY_BARNES_RAJ_3, NPC_BARNES, 14000},  {SAY_BARNES_RAJ_4, NPC_BARNES, 14000},
    {OPERA_EVENT_ROMULO_AND_JUL, 0, 0},     {0, 0, 0},
};

struct npc_barnesAI : public npc_escortAI, private DialogueHelper {
  npc_barnesAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aIntroDialogue) {
  ...
  void Reset() override { m_spotlightGuid.Clear(); }
...
UnitAI *GetAI_npc_barnesAI(Creature *pCreature) { return new npc_barnesAI(pCreature); }
...
bool GossipHello_npc_barnes(Player *pPlayer, Creature *pCreature) {
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
    ...
    if (pInstance->GetData(TYPE_MOROES) != DONE)
      ...
      case NOT_STARTED:
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_OPERA_1, GOSSIP_SENDER_MAIN,
                                    GOSSIP_ACTION_INFO_DEF + 1);
        // for GMs we add the possibility to change the event
        if (pPlayer->IsGameMaster()) {
          pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "[GM] Change event to EVENT_OZ", GOSSIP_SENDER_MAIN,
                                   GOSSIP_ACTION_INFO_DEF + 3);
          pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "[GM] Change event to EVENT_HOOD", GOSSIP_SENDER_MAIN,
                                   GOSSIP_ACTION_INFO_DEF + 4);
          pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "[GM] Change event to EVENT_RAJ", GOSSIP_SENDER_MAIN,
      ...
      case FAIL:
        ...
        case OPERA_EVENT_WIZARD_OZ:
          gossipItemText = GOSSIP_ITEM_OPERA_OZ_WIPE;
          gossipMenuText = TEXT_ID_OPERA_OZ_WIPE;
          break;
        case OPERA_EVENT_RED_RIDING_HOOD:
          gossipItemText = GOSSIP_ITEM_WOLF_WIPE;
          gossipMenuText = TEXT_ID_OPERA_WOLF_WIPE;
          break;
        default:
        case OPERA_EVENT_ROMULO_AND_JUL:
          gossipItemText = GOSSIP_ITEM_JUL_WIPE;
          gossipMenuText = TEXT_ID_OPERA_JUL_WIPE;
          break;
        }
...
struct npc_image_of_medivhAI : public ScriptedAI, private DialogueHelper {
  ...
  void JustSummoned(Creature *pSummoned) override {
    if (pSummoned->GetEntry() == NPC_IMAGE_OF_ARCANAGOS) {
      ...
      pSummoned->GetMotionMaster()->MovePoint(POINT_ID_INTRO, afArcanagosMoveLoc[0], afArcanagosMoveLoc[1],
                                              afArcanagosMoveLoc[2]);
  ...
  void JustDidDialogueStep(int32 iEntry) override {
    ...
    case SPELL_CONFLAG_BLAST:
      ...
      if (Creature *pDragon = m_pInstance->GetSingleCreatureFromStorage(NPC_IMAGE_OF_ARCANAGOS)) {
        ...
        pDragon->GetMotionMaster()->MovePoint(POINT_ID_DESPAWN, afArcanagosFleeLoc[0], afArcanagosFleeLoc[1],
                                              afArcanagosFleeLoc[2]);
      }
      break;
    }
  }
...
UnitAI *GetAI_npc_image_of_medivhAI(Creature *pCreature) { return new npc_image_of_medivhAI(pCreature); }
...
UnitAI *GetAI_npc_image_arcanagosAI(Creature *pCreature) { return new npc_image_arcanagosAI(pCreature); }

/*######
# event_spell_medivh_journal
######*/

bool ProcessEventId_event_spell_medivh_journal(uint32 /*uiEventId*/, Object *pSource, Object * /*pTarget*/,
                                               bool bIsStart) {
  if (bIsStart && pSource->GetTypeId() == TYPEID_PLAYER) {
    // Summon Medivh and Arcanagos
    if (Creature *pMedivh = ((Player *)pSource)
                                ->SummonCreature(NPC_IMAGE_OF_MEDIVH, afMedivhSpawnLoc[0], afMedivhSpawnLoc[1],
                                                 afMedivhSpawnLoc[2], afMedivhSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0)) {
      pMedivh->SummonCreature(NPC_IMAGE_OF_ARCANAGOS, afArcanagosSpawnLoc[0], afArcanagosSpawnLoc[1],
                              afArcanagosSpawnLoc[2], afArcanagosSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0);
...
struct BlinkArcaneAnomaly : public SpellScript {
  void OnInit(Spell *spell) const override { spell->SetMaxAffectedTargets(1); }
