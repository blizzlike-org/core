...
enum {
  ...
  // These points are a placeholder for the air phase movement. The dragon should do some circles around the area before
  // landing again
  POINT_ID_AIR = 1,
  POINT_ID_GROUND = 2,
...
struct boss_nightbaneAI : public CombatAI {
  boss_nightbaneAI(Creature *creature)
      : CombatAI(creature, NIGHTBANE_ACTION_MAX),
        m_instance(static_cast<instance_karazhan *>(creature->GetInstanceData())) {
  ...
  void StartIntro() { m_creature->GetMotionMaster()->MovePath(0, PATH_FROM_EXTERNAL, FORCED_MOVEMENT_RUN); }
  ...
  void SummonedCreatureJustDied(Creature *summoned) override {
    m_skeletons.erase(std::remove(m_skeletons.begin(), m_skeletons.end(), summoned->GetObjectGuid()),
                      m_skeletons.end());
    ...
    if (motionType == PATH_MOTION_TYPE) {
      if (m_bCombatStarted) // combat movement
      {
        if (pointId == POINT_LANDING_END) {
          ...
          m_creature->SetCanFly(false);
          m_creature->SetHover(false);
          m_creature->GetMotionMaster()->MovePoint(POINT_ID_GROUND, -11162.23f, -1900.329f,
                                                   91.47265f); // noted as falling in sniff
        }
      } else // intro movement
      {
  ...
  void DoMoveToClosestTrigger(bool ground) {
    ...
    for (GuidList::const_iterator itr = lTriggersList.begin(); itr != lTriggersList.end(); ++itr) {
      if (Creature *pTrigger = m_creature->GetMap()->GetCreature(*itr)) {
        if (!chosenTrigger || (ground ? !m_creature->GetDistanceOrder(pTrigger, chosenTrigger, false, DIST_CALC_NONE)
                                      : m_creature->GetDistanceOrder(pTrigger, chosenTrigger, false, DIST_CALC_NONE)))
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case NIGHTBANE_DISTRACTING_ASH: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER))
        if (DoCastSpellIfCan(target, m_phase == PHASE_GROUND ? SPELL_DISTRACTING_ASH_GROUND : SPELL_DISTRACTING_ASH) ==
            CAST_OK) // should be used in both phases
    ...
    case NIGHTBANE_FIREBALL_BARRAGE: {
      if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_FARTHEST_AWAY, 0, SPELL_FIREBALL_BARRAGE,
                                                           SELECT_FLAG_PLAYER))
        if (m_creature->GetDistance(target, false, DIST_CALC_COMBAT_REACH) < 60.f ||
            (m_creature->CastSpell(m_creature->GetVictim(), SPELL_FIREBALL_BARRAGE, TRIGGERED_NONE) == SPELL_CAST_OK))
...
bool ProcessEventId_event_spell_summon_nightbane(uint32 /*eventId*/, Object *source, Object * /*target*/,
                                                 bool bIsStart) {
