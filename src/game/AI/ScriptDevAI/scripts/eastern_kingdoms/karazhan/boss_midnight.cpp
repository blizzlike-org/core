...
struct boss_midnightAI : public CombatAI {
  boss_midnightAI(Creature *creature)
      : CombatAI(creature, MIDNIGHT_ACTION_MAX),
    ...
    m_creature->GetCombatManager().SetLeashingCheck(
        [&](Unit *, float x, float y, float z) { return (y < -1945.f && x > -11096.f) || z > 73.5f; });
  }
  ...
  void JustSummoned(Creature *summoned) override {
    ...
    if (summoned->GetEntry() == NPC_ATTUMEN) {
      ...
      switch (urand(0, 2)) {
      case 0:
        DoScriptText(SAY_APPEAR_1, summoned);
        break;
      case 1:
        DoScriptText(SAY_APPEAR_2, summoned);
        break;
      case 2:
        DoScriptText(SAY_APPEAR_3, summoned);
        break;
      }
    }
  }
  ...
  void DoPrepareMount(Creature *target) {
    if (target) {
      ...
      m_creature->GetMotionMaster()->MovePoint(1, target->GetPositionX(), target->GetPositionY(),
                                               target->GetPositionZ());
...
struct boss_attumenAI : public CombatAI {
  boss_attumenAI(Creature *creature)
      : CombatAI(creature, ATTUMEN_ACTION_MAX),
        m_instance(static_cast<instance_karazhan *>(creature->GetInstanceData())) {
  ...
  void ExecuteAction(uint32 action) override {
    ...
    case ATTUMEN_ACTION_KNOCKDOWN: {
      ...
      DoCastSpellIfCan(m_creature->GetVictim(),
                       m_creature->GetEntry() == NPC_ATTUMEN_MOUNTED ? SPELL_KNOCKDOWN : SPELL_UPPERCUT);
      ResetCombatAction(action, GetSubsequentActionTimer(action));
      return;
    }
    case ATTUMEN_ACTION_CHARGE: {
      ...
      if (m_creature->GetEntry() == NPC_ATTUMEN_MOUNTED)
        if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_CHARGE,
  ...
  void KilledUnit(Unit * /*victim*/) override { DoScriptText(urand(0, 1) ? SAY_KILL_1 : SAY_KILL_2, m_creature); }
  ...
  void JustSummoned(Creature *summoned) override {
    if (summoned->GetEntry() == NPC_ATTUMEN_MOUNTED) {
      ...
      if (Creature *midnight = m_instance->GetSingleCreatureFromStorage(NPC_MIDNIGHT))
        summoned->SetHealth(midnight->GetHealth() > m_creature->GetHealth() ? midnight->GetHealth()
