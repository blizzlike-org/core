...
enum { QUEST_THE_CHARGE_OF_DRAGONFLIGHTS = 8555, QUEST_ERANIKUS_TYRANT_OF_DREAMS = 8733 };
  ...
  if (ScriptedInstance *pInstance = (ScriptedInstance *)pPlayer->GetInstanceData()) {
    ...
    if (pPlayer->GetQuestRewardStatus(QUEST_THE_CHARGE_OF_DRAGONFLIGHTS) &&
      ...
      if (pInstance->GetData(TYPE_MALFURION) != IN_PROGRESS) {
        pPlayer->SummonCreature(NPC_MALFURION, aSunkenTempleLocation[2].m_fX, aSunkenTempleLocation[2].m_fY,
                                aSunkenTempleLocation[2].m_fZ, aSunkenTempleLocation[2].m_fO, TEMPSPAWN_DEAD_DESPAWN,
                                0);
...
struct npc_malfurionAI : public ScriptedAI {
  npc_malfurionAI(Creature *pCreature) : ScriptedAI(pCreature), m_uiSayTimer(3000), m_uiSpeech(0) {
...
UnitAI *GetAI_npc_malfurion(Creature *pCreature) { return new npc_malfurionAI(pCreature); }

/*######
## event_antalarion_statues
######*/

bool ProcessEventId_event_antalarion_statue_activation(uint32 uiEventId, Object *pSource, Object *pTarget,
                                                       bool /*bIsStart*/) {
    ...
    if (instance_sunken_temple *pInstance = (instance_sunken_temple *)((Player *)pSource)->GetInstanceData()) {
      ...
      if (pInstance->ProcessStatueEvent(uiEventId)) {
        ...
        if (GameObject *pLight =
                GetClosestGameObjectWithEntry((GameObject *)pTarget, GO_ATALAI_LIGHT, INTERACTION_DISTANCE))
        ...
        if (GameObject *pTrap =
                GetClosestGameObjectWithEntry((GameObject *)pTarget, GO_ATALAI_TRAP_1, INTERACTION_DISTANCE))
          pTrap->Use((Unit *)pSource);
        else if (GameObject *pTrap2 =
                     GetClosestGameObjectWithEntry((GameObject *)pTarget, GO_ATALAI_TRAP_2, INTERACTION_DISTANCE))
          pTrap2->Use((Unit *)pSource);
        else if (GameObject *pTrap3 =
                     GetClosestGameObjectWithEntry((GameObject *)pTarget, GO_ATALAI_TRAP_3, INTERACTION_DISTANCE))
