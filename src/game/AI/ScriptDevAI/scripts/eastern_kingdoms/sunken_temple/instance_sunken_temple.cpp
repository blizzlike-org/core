...
instance_sunken_temple::instance_sunken_temple(Map *pMap)
    : ScriptedInstance(pMap), m_uiProtectorsRemaining(0), m_uiStatueCounter(0), m_uiFlameCounter(0),
      m_uiAvatarSummonTimer(0), m_uiSupressorTimer(0), m_suppressionTimer(0), m_bIsFirstHakkarWave(false),
      m_bCanSummonBloodkeeper(false) {
  Initialize();
}

void instance_sunken_temple::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_sunken_temple::SetData(uint32 uiType, uint32 uiData) {
  ...
  case TYPE_AVATAR:
    if (uiData == SPECIAL) {
      ...
      case 1:
        DoScriptText(SAY_AVATAR_BRAZIER_1, shadeofHakkar);
        break;
      case 2:
        DoScriptText(SAY_AVATAR_BRAZIER_2, shadeofHakkar);
        break;
      case 3:
        DoScriptText(SAY_AVATAR_BRAZIER_3, shadeofHakkar);
        break;
    ...
    if (uiData == IN_PROGRESS) {
      ...
      if (Creature *pShade = pPlayer->SummonCreature(NPC_SHADE_OF_HAKKAR, aSunkenTempleLocation[1].m_fX,
                                                     aSunkenTempleLocation[1].m_fY, aSunkenTempleLocation[1].m_fZ,
                                                     aSunkenTempleLocation[1].m_fO, TEMPSPAWN_MANUAL_DESPAWN, 0)) {
...
void instance_sunken_temple::DoSpawnAtalarionIfCan() {
  ...
  pPlayer->SummonCreature(NPC_ATALARION, aSunkenTempleLocation[0].m_fX, aSunkenTempleLocation[0].m_fY,
                          aSunkenTempleLocation[0].m_fZ, aSunkenTempleLocation[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
...
void instance_sunken_temple::Update(uint32 uiDiff) {
      ...
      if (m_bIsFirstHakkarWave) // First wave summoned
      {
        ...
        for (GuidVector::const_iterator itr = m_evilCircleGuidList.begin(); itr != m_evilCircleGuidList.end(); ++itr) {
          if (GameObject *pCircle = instance->GetGameObject(*itr))
            pShade->SummonCreature(NPC_HAKKARI_MINION, pCircle->GetPositionX(), pCircle->GetPositionY(),
                                   pCircle->GetPositionZ(), 0, TEMPSPAWN_DEAD_DESPAWN, 0);
        }

        // Summon Bloodkeeper at random circle
        if (GameObject *pCircle =
                instance->GetGameObject(m_evilCircleGuidList[urand(0, m_evilCircleGuidList.size() - 1)]))
          pShade->SummonCreature(NPC_BLOODKEEPER, pCircle->GetPositionX(), pCircle->GetPositionY(),
                                 pCircle->GetPositionZ(), 0, TEMPSPAWN_DEAD_DESPAWN, 0);
  ...
  if (m_uiSupressorTimer) {
    if (m_uiSupressorTimer <= uiDiff) {
      ...
      if (Creature *suppressor = pShade->SummonCreature(
              NPC_SUPPRESSOR, aHakkariDoorLocations[uiSummonLoc].m_fX, aHakkariDoorLocations[uiSummonLoc].m_fY,
              aHakkariDoorLocations[uiSummonLoc].m_fZ, 0, TEMPSPAWN_DEAD_DESPAWN, 0)) {
...
InstanceData *GetInstanceData_instance_sunken_temple(Map *pMap) { return new instance_sunken_temple(pMap); }
