...
instance_uldaman::instance_uldaman(Map *pMap)
    : ScriptedInstance(pMap), m_uiKeeperCooldown(0), m_uiStoneKeepersFallen(0) {
  Initialize();
}

void instance_uldaman::Initialize() { memset(&m_auiEncounter, 0, sizeof(m_auiEncounter)); }
...
void instance_uldaman::DoResetKeeperEvent() {
  if (m_lKeepers.empty()) {
    script_error_log("Instance Uldaman: ERROR creature %u couldn't be found or something really bad happened.",
                     NPC_STONE_KEEPER);
    return;
  }
...
bool ProcessEventId_event_spell_altar_boss_aggro(uint32 uiEventId, Object *pSource, Object * /*pTarget*/,
                                                 bool bIsStart) {
