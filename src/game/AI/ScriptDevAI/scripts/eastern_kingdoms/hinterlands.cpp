...
UnitAI *GetAI_npc_00x09hl(Creature *pCreature) { return new npc_00x09hlAI(pCreature); }
...
struct npc_rinjiAI : public npc_escortAI {
  ...
  void DoSpawnAmbush(bool bFirst) {
    ...
    m_creature->SummonCreature(NPC_RANGER, m_afAmbushSpawn[m_iSpawnId].m_fX, m_afAmbushSpawn[m_iSpawnId].m_fY,
                               m_afAmbushSpawn[m_iSpawnId].m_fZ, 0.0f, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 60000);

    for (int i = 0; i < 2; ++i) {
      m_creature->SummonCreature(NPC_OUTRUNNER, m_afAmbushSpawn[m_iSpawnId].m_fX, m_afAmbushSpawn[m_iSpawnId].m_fY,
                                 m_afAmbushSpawn[m_iSpawnId].m_fZ, 0.0f, TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 60000);
    }
  }
...
UnitAI *GetAI_npc_rinji(Creature *pCreature) { return new npc_rinjiAI(pCreature); }
...
bool ProcessEventId_WildhammerMessage(uint32 /*eventId*/, Object *source, Object * /*target*/, bool /*isStart*/) {
  ...
  if (Creature *falstad = static_cast<ScriptedInstance *>(player->GetInstanceData())
                              ->GetSingleCreatureFromStorage(NPC_FALSTAD_WILDHAMMER))
