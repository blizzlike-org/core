...
void SystemMgr::LoadScriptWaypoints() {
  ...
  result = WorldDatabase.PQuery("SELECT entry, pathId, pointid, position_x, position_y, position_z, orientation, "
                                "waittime, script_id FROM script_waypoint ORDER BY entry, pathId, pointid");
  ...
  if (result) {
    ...
    do {
      ...
      if (pointId == 0) {
        ...
        error_db_log("SD2: DB table `script_waypoint` has invalid point 0 for entry %u in path %u. Skipping.", entry,
                     pathId);
      }
      ...
      if (blacklistWaypoints.find((entry << 8) + pathId) == blacklistWaypoints.end())
        sWaypointMgr.AddExternalNode(entry, pathId, pointId, position_x, position_y, position_z, orientation, waitTime,
                                     scriptId);
