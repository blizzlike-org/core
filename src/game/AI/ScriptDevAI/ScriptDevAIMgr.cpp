...
void LoadDatabase() {
  // Load content
  // pSystemMgr.LoadVersion(); // currently we are not checking for version; function to be completely removed in the
  // future
...
void DoScriptText(int32 iTextEntry, WorldObject *pSource, Unit *pTarget) {
  ...
  if (iTextEntry >= 0) {
    script_error_log("DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but "
                     "text entry must be negative.",
...
void DoOrSimulateScriptTextForMap(int32 iTextEntry, uint32 uiCreatureEntry, Map *pMap,
  ...
  if (iTextEntry >= 0) {
    script_error_log("DoOrSimulateScriptTextForMap with source entry %u for map %u attempts to process text entry %i, "
                     "but text entry must be negative.",
  ...
  if (!pInfo) {
    script_error_log("DoOrSimulateScriptTextForMap has invalid source entry %u for map %u.", uiCreatureEntry,
                     pMap->GetId());
    return;
  }
  ...
  if (!pData) {
    script_error_log("DoOrSimulateScriptTextForMap with source entry %u for map %u could not find text entry %i.",
                     uiCreatureEntry, pMap->GetId(), iTextEntry);
    return;
  }

  debug_log("SD2: DoOrSimulateScriptTextForMap: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u", iTextEntry,
            pData->SoundId, pData->Type, pData->LanguageId, pData->Emote);
...
void Script::RegisterSelf(bool bReportError) {
  if (uint32 id = sScriptDevAIMgr.GetScriptId(Name.c_str()))
    ...
    if (bReportError)
      script_error_log("Script registering but ScriptName %s is not assigned in database. Script will not be used.",
                       Name.c_str());
...
void ScriptDevAIMgr::LoadScriptNames() {
  ...
  QueryResult *result =
      WorldDatabase.Query("SELECT DISTINCT(ScriptName) FROM creature_template WHERE ScriptName <> '' "
                          "UNION "
...
void ScriptDevAIMgr::LoadAreaTriggerScripts() {
  ...
  do {
    ...
    if (!sAreaTriggerStore.LookupEntry(triggerId)) {
      sLog.outErrorDb("Table `scripted_areatrigger` has area trigger (ID: %u) not listed in `AreaTrigger.dbc`.",
                      triggerId);
      continue;
    }
...
void ScriptDevAIMgr::LoadEventIdScripts() {
  ...
  do {
    ...
    if (itr == eventIds.end())
      sLog.outErrorDb(
          "Table `scripted_event_id` has id %u not referring to any gameobject_template type 10 data2 field, type 3 "
          "data6 field, type 13 data 2 field, type 29 or any spell effect %u or path taxi node data",
