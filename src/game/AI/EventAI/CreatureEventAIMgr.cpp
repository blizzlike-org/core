...
void CreatureEventAIMgr::LoadCreatureEventAI_Texts(bool check_entry_use) {
  // Load EventAI Text
  sObjectMgr.LoadMangosStrings(WorldDatabase, "creature_ai_texts", MIN_CREATURE_AI_TEXT_STRING_ID,
  ...
  if (result) {
    ...
    do {
      ...
      if (!MaNGOS::IsValidMapCoord(temp.position_x, temp.position_y, temp.position_z, temp.orientation)) {
        sLog.outErrorEventAI("Summon id %u have wrong coordinates (%f, %f, %f, %f), skipping.", temp.id,
...
bool IsValidTargetType(EventAI_Type eventType, EventAI_ActionType actionType, uint32 targetType, uint32 eventId,
                       uint8 action) {
  switch (targetType) {
  case TARGET_T_SELF:
    if (actionType == ACTION_T_QUEST_EVENT || actionType == ACTION_T_CAST_EVENT ||
        actionType == ACTION_T_QUEST_EVENT_ALL || actionType == ACTION_T_KILLED_MONSTER) {
      sLog.outErrorEventAI("Event %u Action%u uses incorrect Target type %u for event-type %u (must target player)",
                           eventId, action, targetType, eventType);
      return false;
    }
  ...
  case TARGET_T_HOSTILE_RANDOM_NOT_TOP:
    if (actionType == ACTION_T_QUEST_EVENT || actionType == ACTION_T_CAST_EVENT ||
        actionType == ACTION_T_QUEST_EVENT_ALL || actionType == ACTION_T_KILLED_MONSTER)
      sLog.outErrorEventAI("Event %u Action%u uses LIKELY bad Target type %u for event-type %u (must target player)",
                           eventId, action, targetType, eventType);
  ...
  case TARGET_T_HOSTILE_RANDOM_NOT_TOP_PLAYER:
    ...
    case EVENT_T_REACHED_HOME:
      sLog.outErrorEventAI("Event %u Action%u uses incorrect Target type %u for event-type %u (cannot be used OOC)",
                           eventId, action, targetType, eventType);
      return false;
    case EVENT_T_TIMER_GENERIC:
      sLog.outErrorEventAI(
          "Event %u Action%u uses LIKELY incorrect Target type %u for event-type %u (cannot be used OOC)", eventId,
          action, targetType, eventType);
      return true; // Does not need to be an error
    default:
      return true;
    }
  case TARGET_T_ACTION_INVOKER: // Unit who caused this Event to occur (only works for EVENT_T_AGGRO, EVENT_T_KILL,
                                // EVENT_T_DEATH, EVENT_T_SPELLHIT, EVENT_T_OOC_LOS, EVENT_T_FRIENDLY_HP,
                                // EVENT_T_FRIENDLY_IS_CC, EVENT_T_FRIENDLY_MISSING_BUFF, EVENT_T_RECEIVE_EMOTE,
                                // EVENT_T_RECEIVE_AI_EVENT)
  case TARGET_T_ACTION_INVOKER_OWNER: // Unit who is responsible for Event to occur (only works for EVENT_T_AGGRO,
                                      // EVENT_T_KILL, EVENT_T_DEATH, EVENT_T_SPELLHIT, EVENT_T_OOC_LOS,
                                      // EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC, EVENT_T_FRIENDLY_MISSING_BUFF,
                                      // EVENT_T_RECEIVE_EMOTE, EVENT_T_RECEIVE_AI_EVENT)
    switch (eventType) {
    case EVENT_T_AGGRO:
    case EVENT_T_KILL:
    case EVENT_T_DEATH:
    ...
    case EVENT_T_SPELLHIT_TARGET:
      return true;
    default:
      sLog.outErrorEventAI("Event %u Action%u uses incorrect Target type %u for event-type %u", eventId, action,
                           targetType, eventType);
      return false;
    }
  case TARGET_T_EVENT_SENDER: // Unit who sent an AIEvent that was received with EVENT_T_RECEIVE_AI_EVENT
    if (eventType != EVENT_T_RECEIVE_AI_EVENT && eventType != EVENT_T_SPELLHIT &&
        eventType != EVENT_T_SPELLHIT_TARGET) {
      sLog.outErrorEventAI("Event %u Action%u uses incorrect Target type %u for event-type %u", eventId, action,
                           targetType, eventType);
      return false;
    }
...
void CreatureEventAIMgr::LoadCreatureEventAI_Scripts() {
  ...
  // Gather event data
  QueryResult *result =
      WorldDatabase.Query("SELECT id, creature_id, event_type, event_inverse_phase_mask, event_chance, event_flags, "
  ...
  if (result) {
    ...
    do {
      ...
      if (!sCreatureStorage.LookupEntry<CreatureInfo>(temp.creature_id)) {
        sLog.outErrorEventAI("Event %u has script for non-existing creature entry (%u), skipping.", i,
                             temp.creature_id);
        continue;
      }
      ...
      // Chance above 100, force it to be 100
        ...
        sLog.outErrorEventAI(
            "Creature %u are using event %u with more than 100 percent chance. Adjusting to 100 percent.",
            temp.creature_id, i);
      ...
      case EVENT_T_TIMER_GENERIC:
        if (temp.timer.initialMax < temp.timer.initialMin)
          sLog.outErrorEventAI("Creature %u are using timed event(%u) with param2 < param1 (InitialMax < InitialMin). "
                               "Event will never repeat.",
                               temp.creature_id, i);
        if (temp.timer.repeatMax < temp.timer.repeatMin)
          sLog.outErrorEventAI("Creature %u are using repeatable event(%u) with param4 < param3 (RepeatMax < "
      ...
      case EVENT_T_ENERGY:
        if (temp.percent_range.percentMax > 100)
          sLog.outErrorEventAI(
              "Creature %u are using percentage event(%u) with param2 (MinPercent) > 100. Event will never trigger! ",
              temp.creature_id, i);

        if (temp.percent_range.percentMax <= temp.percent_range.percentMin)
          sLog.outErrorEventAI("Creature %u are using percentage event(%u) with param1 <= param2 (MaxPercent <= "
        ...
        if (temp.event_flags & EFLAG_REPEATABLE && !temp.percent_range.repeatMin && !temp.percent_range.repeatMax) {
          sLog.outErrorEventAI("Creature %u has param3 and param4=0 (RepeatMin/RepeatMax) but cannot be repeatable "
                               "without timers. Removing EFLAG_REPEATABLE for event %u.",
                               temp.creature_id, i);
      ...
      case EVENT_T_SPELLHIT:
        if (temp.spell_hit.spellId) {
          ...
          if (!pSpell) {
            sLog.outErrorEventAI("Creature %u has nonexistent SpellID(%u) defined in event %u.", temp.creature_id,
                                 temp.spell_hit.spellId, i);
            continue;
          }

          if ((temp.spell_hit.schoolMask & pSpell->SchoolMask) != pSpell->SchoolMask)
            sLog.outErrorEventAI("Creature %u has param1(spellId %u) but param2 is not -1 and not equal to spell's "
                                 "school mask. Event %u can never trigger.",
                                 temp.creature_id, temp.spell_hit.schoolMask, i);
        }

        if (!temp.spell_hit.schoolMask)
          sLog.outErrorEventAI("Creature %u is using invalid SpellSchoolMask(%u) defined in event %u.",
                               temp.creature_id, temp.spell_hit.schoolMask, i);
      ...
      case EVENT_T_RANGE:
        if (temp.range.maxDist < temp.range.minDist)
          sLog.outErrorEventAI(
              "Creature %u are using event(%u) with param2 < param1 (MaxDist < MinDist). Event will never repeat.",
              temp.creature_id, i);
      ...
      case EVENT_T_OOC_LOS:
        if (temp.ooc_los.conditionId && !sConditionStorage.LookupEntry<ConditionEntry>(temp.ooc_los.conditionId)) {
          sLog.outErrorDb(
              "Creature %u has `ConditionId` = %u but does not exist. Setting ConditionId to 0 for event %u.",
              temp.creature_id, temp.ooc_los.conditionId, i);
      ...
      case EVENT_T_SPAWNED:
        ...
        case SPAWNED_EVENT_MAP:
          if (!sMapStore.LookupEntry(temp.spawned.conditionValue1))
            sLog.outErrorEventAI("Creature %u are using spawned event(%u) with param1 = %u 'map specific' but map "
                                 "(param2: %u) does not exist. Event will never repeat.",
        ...
        case SPAWNED_EVENT_ZONE:
          if (!GetAreaEntryByAreaID(temp.spawned.conditionValue1))
            sLog.outErrorEventAI("Creature %u are using spawned event(%u) with param1 = %u 'area specific' but area "
                                 "(param2: %u) does not exist. Event will never repeat.",
                                 temp.creature_id, i, temp.spawned.condition, temp.spawned.conditionValue1);
          break;
        default:
          sLog.outErrorEventAI("Creature %u are using invalid spawned event %u mode (%u) in param1", temp.creature_id,
                               i, temp.spawned.condition);
          break;
        }
        break;
      case EVENT_T_FRIENDLY_HP:
      ...
      case EVENT_T_FRIENDLY_MISSING_BUFF: {
        ...
        if (!pSpell) {
          sLog.outErrorEventAI("Creature %u has nonexistent SpellID(%u) defined in event %u.", temp.creature_id,
                               temp.friendly_buff.spellId, i);
          continue;
        }
        if (temp.friendly_buff.radius <= 0) {
          sLog.outErrorEventAI(
              "Creature %u has wrong radius (%u) for EVENT_T_FRIENDLY_MISSING_BUFF defined in event %u.",
      ...
      case EVENT_T_KILL:
        if (temp.kill.repeatMax < temp.kill.repeatMin)
          sLog.outErrorEventAI(
              "Creature %u are using event(%u) with param2 < param1 (RepeatMax < RepeatMin). Event will never repeat.",
              temp.creature_id, i);
        break;
      ...
      case EVENT_T_SUMMONED_JUST_DESPAWN:
        if (!sCreatureStorage.LookupEntry<CreatureInfo>(temp.summoned.creatureId))
          sLog.outErrorEventAI(
              "Creature %u are using event(%u) with nonexistent creature template id (%u) in param1, skipped.",
      ...
      case EVENT_T_QUEST_COMPLETE:
        if (!sObjectMgr.GetQuestTemplate(temp.quest.questId))
          sLog.outErrorEventAI("Creature %u are using event(%u) with nonexistent quest id (%u) in param1, skipped.",
                               temp.creature_id, i, temp.quest.questId);
        sLog.outErrorEventAI("Creature %u using not implemented event (%u) in event %u.", temp.creature_id,
                             temp.event_id, i);
        continue;
      case EVENT_T_DEATH: {
        if (temp.death.conditionId && !sConditionStorage.LookupEntry<ConditionEntry>(temp.death.conditionId)) {
          ...
          sLog.outErrorDb(
              "Creature %u has `ConditionId` = %u but does not exist. Setting ConditionId to 0 for event %u.",
              temp.creature_id, temp.death.conditionId, i);
      ...
      case EVENT_T_REACHED_HOME: {
        if (temp.event_flags & EFLAG_REPEATABLE) {
          sLog.outErrorEventAI(
              "Creature %u has EFLAG_REPEATABLE set. Event can never be repeatable. Removing flag for event %u.",
              temp.creature_id, i);
          temp.event_flags &= ~EFLAG_REPEATABLE;
        }

        if (temp.event_flags & EFLAG_COMBAT_ACTION) {
          sLog.outErrorEventAI("Creature %u has EFLAG_COMBAT_ACTION set. Event can never be done during combat. "
                               "Removing flag for event %u.",
                               temp.creature_id, i);
      ...
      case EVENT_T_RECEIVE_EMOTE: {
        if (!sEmotesTextStore.LookupEntry(temp.receive_emote.emoteId)) {
          sLog.outErrorEventAI("Creature %u using event %u: param1 (EmoteTextId: %u) are not valid.", temp.creature_id,
                               i, temp.receive_emote.emoteId);
          continue;
        }

        if (temp.receive_emote.conditionId &&
            !sConditionStorage.LookupEntry<ConditionEntry>(temp.receive_emote.conditionId)) {
          sLog.outErrorDb(
              "Creature %u has `ConditionId` = %u but does not exist. Setting ConditionId to 0 for event %u.",
              temp.creature_id, temp.receive_emote.conditionId, i);
        ...
        if (!(temp.event_flags & EFLAG_REPEATABLE)) {
          sLog.outErrorEventAI(
              "Creature %u using event %u: EFLAG_REPEATABLE not set. Event must always be repeatable. Flag applied.",
              temp.creature_id, i);
          temp.event_flags |= EFLAG_REPEATABLE;
        }
      ...
      case EVENT_T_TARGET_MISSING_AURA: {
        ...
        if (!pSpell) {
          sLog.outErrorEventAI("Creature %u has nonexistent SpellID(%u) defined in event %u.", temp.creature_id,
                               temp.buffed.spellId, i);
          continue;
        }
        if (temp.buffed.amount < 1) {
          sLog.outErrorEventAI("Creature %u has wrong spell stack size (%u) defined in event %u.", temp.creature_id,
                               temp.buffed.amount, i);
          continue;
        }
      ...
      case EVENT_T_RECEIVE_AI_EVENT: {
        ...
        if (temp.receiveAIEvent.senderEntry &&
            !sCreatureStorage.LookupEntry<CreatureInfo>(temp.receiveAIEvent.senderEntry)) {
          sLog.outErrorEventAI("Event %u has nonexisting creature (%u) defined for event RECEIVE_AI_EVENT, skipping.",
                               i, temp.receiveAIEvent.senderEntry);
          continue;
        }
        // Event-Type is not defined
        if (temp.receiveAIEvent.eventType >= MAXIMAL_AI_EVENT_EVENTAI) {
          sLog.outErrorEventAI("Event %u has unfitting event-type (%u) defined for event RECEIVE_AI_EVENT (must be "
                               "less than %u), skipping.",
                               i, temp.receiveAIEvent.eventType, MAXIMAL_AI_EVENT_EVENTAI);
          continue;
        }
        break;
      }
      ...
      case EVENT_T_FACING_TARGET: {
        ...
        if (temp.facingTarget.backOrFront && temp.facingTarget.backOrFront != 1) {
          sLog.outErrorEventAI(
              "Event %u has unfitting value (%u) for param1 in event EVENT_T_FACING_TARGET (must be 0 or 1), skipping.",
              i, temp.facingTarget.backOrFront);
          continue;
        }
        ...
        if (temp.event_flags & EFLAG_REPEATABLE && temp.facingTarget.repeatMax < temp.facingTarget.repeatMin)
          sLog.outErrorEventAI("Creature %u is using repeatable event(%u) with param4 < param3 (RepeatMax < "
                               "RepeatMin). Event will never repeat.",
                               temp.creature_id, i);
        break;
      }
      case EVENT_T_SPELLHIT_TARGET:
        if (temp.spell_hit_target.spellId) {
          ...
          if (!spellInfo) {
            sLog.outErrorEventAI("Creature %u has nonexistent SpellID(%u) defined in event %u.", temp.creature_id,
                                 temp.spell_hit_target.spellId, i);
            continue;
          }
        ...
        if (!temp.spell_hit_target.schoolMask)
          sLog.outErrorEventAI("Creature %u is using invalid SpellSchoolMask(%u) defined in event %u.",
                               temp.creature_id, temp.spell_hit_target.schoolMask, i);
      ...
      case EVENT_T_DEATH_PREVENTED:
        break;
      default:
        sLog.outErrorEventAI("Creature %u using not checked at load event (%u) in event %u. Need check code update?",
      ...
      for (uint32 j = 0; j < MAX_ACTIONS; ++j) {
        ...
        if (action_type >= ACTION_T_END) {
          sLog.outErrorEventAI("Event %u Action %u has incorrect action type (%u), replace by ACTION_T_NONE.", i, j + 1,
                               action_type);
        ...
        case ACTION_T_CHANCED_TEXT:
          ...
          if (!action.chanced_text.chance)
            ...
            sLog.outErrorEventAI("Event %u Action %u has set chance param1 >= 100. Text will always be displayed", i,
                                 j + 1);
          ...
          for (int k = firstTextParam; k < 3; ++k) {
            if (action.text.TextId[k]) {
              if (k > firstTextParam && not_set)
                sLog.outErrorEventAI(
                    "Event %u Action %u has param%d, but it follow after not set param. Required for randomized text.",
                    i, j + 1, k + 1);
              ...
              // range negative
                ...
                sLog.outErrorEventAI("Event %u Action %u param%d references out-of-range entry (%i) in texts table.", i,
                                     j + 1, k + 1, action.text.TextId[k]);
                action.text.TextId[k] = 0;
              } else if (!sObjectMgr.GetMangosStringLocale(action.text.TextId[k])) {
                sLog.outErrorEventAI("Event %u Action %u param%d references non-existing entry (%i) in texts table.", i,
                                     j + 1, k + 1, action.text.TextId[k]);
                action.text.TextId[k] = 0;
              } else
        ...
        case ACTION_T_SET_FACTION:
          if (action.set_faction.factionId != 0 && !sFactionTemplateStore.LookupEntry(action.set_faction.factionId)) {
            sLog.outErrorEventAI("Event %u Action %u uses nonexistent FactionId %u.", i, j + 1,
        ...
        case ACTION_T_MORPH_TO_ENTRY_OR_MODEL:
          if (action.morph.creatureId != 0 || action.morph.modelId != 0) {
            if (action.morph.creatureId && !sCreatureStorage.LookupEntry<CreatureInfo>(action.morph.creatureId)) {
              sLog.outErrorEventAI("Event %u Action %u uses nonexistent Creature entry %u.", i, j + 1,
            ...
            if (action.morph.modelId) {
              if (action.morph.creatureId) {
                sLog.outErrorEventAI("Event %u Action %u have unused ModelId %u with also set creature id %u.", i,
                                     j + 1, action.morph.modelId, action.morph.creatureId);
        ...
        case ACTION_T_EMOTE:
          if (!sEmotesStore.LookupEntry(action.emote.emoteId))
            sLog.outErrorEventAI("Event %u Action %u param1 (EmoteId: %u) are not valid.", i, j + 1,
                                 action.emote.emoteId);
          break;
        case ACTION_T_RANDOM_SOUND:
          if (!sSoundEntriesStore.LookupEntry(action.random_sound.soundId1))
            sLog.outErrorEventAI("Event %u Action %u param1 uses nonexistent SoundID %u.", i, j + 1,
          ...
          if (action.random_sound.soundId2 >= 0 && !sSoundEntriesStore.LookupEntry(action.random_sound.soundId2))
            sLog.outErrorEventAI("Event %u Action %u param2 uses nonexistent SoundID %u.", i, j + 1,
          ...
          if (action.random_sound.soundId3 >= 0 && !sSoundEntriesStore.LookupEntry(action.random_sound.soundId3))
            sLog.outErrorEventAI("Event %u Action %u param3 uses nonexistent SoundID %u.", i, j + 1,
        ...
        case ACTION_T_RANDOM_EMOTE:
          ...
          if (action.random_emote.emoteId2 >= 0 && !sEmotesStore.LookupEntry(action.random_emote.emoteId2))
            sLog.outErrorEventAI("Event %u Action %u param2 (EmoteId: %u) are not valid.", i, j + 1,
                                 action.random_emote.emoteId2);
          if (action.random_emote.emoteId3 >= 0 && !sEmotesStore.LookupEntry(action.random_emote.emoteId3))
            sLog.outErrorEventAI("Event %u Action %u param3 (EmoteId: %u) are not valid.", i, j + 1,
          ...
          else
          {
              if (spell->RecoveryTime > 0 && temp.event_flags & EFLAG_REPEATABLE)
              {
                  ...
                  if (temp.event_param3 < spell->RecoveryTime)
                      DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS, "CreatureEventAI:  Event %u Action %u uses SpellID
          %u but cooldown is longer(%u) than minumum defined in event param3(%u).", i, j+1,action.cast.spellId,
          ...
          */
          ...
          if (spell && spell->Targets) // causes crash if not handled
          {
            if (action.cast.target == TARGET_T_NONE || action.cast.target == TARGET_T_NEAREST_AOE_TARGET) {
              sLog.outErrorEventAI("Event %u Action %u uses SpellID %u that must have a target supplied (target is "
                                   "%u). Resetting to TARGET_T_HOSTILE.",
                                   i, j + 1, action.cast.spellId, action.cast.target);
          ...
          if (!sLog.HasLogFilter(LOG_FILTER_EVENT_AI_DEV) && spell) {
            ...
            if ((IsOnlySelfTargeting(spell) || spell->rangeIndex == SPELL_RANGE_IDX_SELF_ONLY) &&
                action.cast.target != TARGET_T_SELF && !(action.cast.castFlags & CAST_FORCE_TARGET_SELF))
              sLog.outErrorEventAI("Event %u Action %u uses SpellID %u that must be self cast (target is %u)", i, j + 1,
                                   action.cast.spellId, action.cast.target);
            ...
            if (action.cast.target == TARGET_T_ACTION_INVOKER &&
              ...
              sLog.outErrorEventAI("Event %u Action %u has TARGET_T_ACTION_INVOKER(%u) target type, but should have "
                                   ...
                                   i, j + 1, TARGET_T_ACTION_INVOKER, TARGET_T_ACTION_INVOKER_OWNER);

            // Spell that should only target players, but could get any
            if (spell->HasAttribute(SPELL_ATTR_EX3_TARGET_ONLY_PLAYER) &&
                (action.cast.target == TARGET_T_ACTION_INVOKER || action.cast.target == TARGET_T_HOSTILE_RANDOM ||
                 action.cast.target == TARGET_T_HOSTILE_RANDOM_NOT_TOP))
              sLog.outErrorEventAI("Event %u Action %u uses Target type %u for a spell (%u) that should only target "
                                   "players. This could be wrong.",
                                   i, j + 1, action.cast.target, action.cast.spellId);

            if (spell->Targets != 0 &&
                (action.cast.target == TARGET_T_NONE || action.cast.target == TARGET_T_NEAREST_AOE_TARGET)) {
              sLog.outErrorEventAI("Event %u Action %u uses Target type %u for a spell (%u) that needs target for "
                                   "casting. Resetting it to TARGET_T_HOSTILE.",
                                   i, j + 1, action.cast.target, action.cast.spellId);
              action.cast.target = TARGET_T_HOSTILE;
            }
          }
        ...
        case ACTION_T_QUEST_EVENT:
          if (Quest const *qid = sObjectMgr.GetQuestTemplate(action.quest_event.questId)) {
            if (!qid->HasSpecialFlag(QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT))
              sLog.outErrorEventAI("Event %u Action %u. SpecialFlags for quest entry %u does not include |2, Action "
                                   "will not have any effect.",
                                   i, j + 1, action.quest_event.questId);
          } else
            sLog.outErrorEventAI("Event %u Action %u uses nonexistent Quest entry %u.", i, j + 1,
        ...
        case ACTION_T_CAST_EVENT:
          ...
          if (!sSpellTemplate.LookupEntry<SpellEntry>(action.cast_event.spellId))
            sLog.outErrorEventAI("Event %u Action %u uses nonexistent SpellID %u.", i, j + 1,
        ...
        case ACTION_T_SET_UNIT_FIELD:
          if (action.set_unit_field.field < OBJECT_END || action.set_unit_field.field >= UNIT_END)
            sLog.outErrorEventAI("Event %u Action %u param1 (UNIT_FIELD*). Index out of range for intended use.", i,
                                 j + 1);
        ...
        case ACTION_T_SET_PHASE:
          if (action.set_phase.phase >= MAX_PHASE)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set phase >= %u. Phase mask cannot be used past phase %u.", i, j + 1,
        ...
        case ACTION_T_INC_PHASE:
          if (action.set_inc_phase.step == 0)
            ...
            sLog.outErrorEventAI("Event %u Action %u is change phase by too large for any use %i.", i, j + 1,
        ...
        case ACTION_T_RANDOM_PHASE: // PhaseId1, PhaseId2, PhaseId3
          if (action.random_phase.phase1 >= MAX_PHASE)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set phase1 >= %u. Phase mask cannot be used past phase %u.", i, j + 1,
                MAX_PHASE, MAX_PHASE - 1);
          if (action.random_phase.phase2 >= MAX_PHASE)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set phase2 >= %u. Phase mask cannot be used past phase %u.", i, j + 1,
                MAX_PHASE, MAX_PHASE - 1);
          if (action.random_phase.phase3 >= MAX_PHASE)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set phase3 >= %u. Phase mask cannot be used past phase %u.", i, j + 1,
                MAX_PHASE, MAX_PHASE - 1);
          break;
        case ACTION_T_RANDOM_PHASE_RANGE: // PhaseMin, PhaseMax
          if (action.random_phase_range.phaseMin >= MAX_PHASE)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set phaseMin >= %u. Phase mask cannot be used past phase %u.", i, j + 1,
                MAX_PHASE, MAX_PHASE - 1);
          if (action.random_phase_range.phaseMin >= MAX_PHASE)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set phaseMax >= %u. Phase mask cannot be used past phase %u.", i, j + 1,
                MAX_PHASE, MAX_PHASE - 1);
        ...
        case ACTION_T_SUMMON_ID:
          ...
          if (m_CreatureEventAI_Summon_Map.find(action.summon_id.spawnId) == m_CreatureEventAI_Summon_Map.end())
            sLog.outErrorEventAI("Event %u Action %u summons missing CreatureEventAI_Summon %u", i, j + 1,
        ...
        case ACTION_T_SET_INST_DATA:
          if (!(temp.event_flags & EFLAG_DIFFICULTY_ALL))
            sLog.outErrorEventAI("Event %u Action %u. Cannot set instance data without difficulty event flags.", i,
                                 j + 1);
          if (action.set_inst_data.value > 4 /*SPECIAL*/)
            sLog.outErrorEventAI(
                "Event %u Action %u attempts to set instance data above encounter state 4. Custom case?", i, j + 1);
          break;
        ...
        case ACTION_T_SET_DEATH_PREVENTION:
          if (action.deathPrevention.state > 1) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid death prevention state %u. Setting to 1.", i, j + 1,
                                 action.deathPrevention.state);
          ...
          if (action.mount.creatureId != 0 || action.mount.modelId != 0) {
            ...
            if (action.mount.modelId) {
              if (action.mount.creatureId) {
                sLog.outErrorEventAI("Event %u Action %u have unused ModelId %u with also set creature id %u.", i,
                                     j + 1, action.mount.modelId, action.mount.creatureId);
        ...
        case ACTION_T_COMBAT_MOVEMENT:   // AllowCombatMovement (0 = stop combat based movement, anything else continue
                                         // attacking)
        ...
        case ACTION_T_RANDOM_TEXTEMOTE:
          sLog.outErrorEventAI("Event %u Action %u currently unused ACTION type. Did you forget to update database?", i,
                               j + 1);
          break;

        case ACTION_T_THROW_AI_EVENT:
          if (action.throwEvent.eventType >= MAXIMAL_AI_EVENT_EVENTAI) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid event type %u (must be less than %u), skipping", i,
                                 j + 1, action.throwEvent.eventType, MAXIMAL_AI_EVENT_EVENTAI);
            continue;
          }
          if (action.throwEvent.radius > (uint32)SIZE_OF_GRIDS)
            sLog.outErrorEventAI("Event %u Action %u uses unexpectedly huge radius %u (expected to be less than %f)", i,
                                 j + 1, action.throwEvent.radius, SIZE_OF_GRIDS);

          if (action.throwEvent.radius == 0) {
            sLog.outErrorEventAI("Event %u Action %u uses unexpected radius 0 (set to %f of "
                                 "CONFIG_FLOAT_CREATURE_FAMILY_ASSISTANCE_RADIUS)",
                                 i, j + 1, sWorld.getConfig(CONFIG_FLOAT_CREATURE_FAMILY_ASSISTANCE_RADIUS));
        ...
        case ACTION_T_SET_THROW_MASK:
          if (action.setThrowMask.eventTypeMask & ~((1 << MAXIMAL_AI_EVENT_EVENTAI) - 1)) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid AIEvent-typemask %u (must be smaller than %u)", i,
                                 j + 1, action.setThrowMask.eventTypeMask, MAXIMAL_AI_EVENT_EVENTAI << 1);
          }
          break;
        case ACTION_T_SET_STAND_STATE:
          if (action.setStandState.standState >= MAX_UNIT_STAND_STATE) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid unit stand state %u (must be smaller than %u)", i,
                                 j + 1, action.setStandState.standState, MAX_UNIT_STAND_STATE);
          }
          break;
        case ACTION_T_CHANGE_MOVEMENT:
          if (action.changeMovement.movementType >= MAX_DB_MOTION_TYPE) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid movement type %u (must be smaller than %u)", i, j + 1,
                                 action.changeMovement.movementType, MAX_DB_MOTION_TYPE);
          }
        ...
        case ACTION_T_SET_REACT_STATE:
          if (action.setReactState.reactState > REACT_AGGRESSIVE) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid react state %u (must be smaller than %u)", i, j + 1,
                                 action.setReactState.reactState, REACT_AGGRESSIVE);
          }
          break;
        ...
        case ACTION_T_INTERRUPT_SPELL:
          if (action.interruptSpell.currentSpellType >= CURRENT_MAX_SPELL) {
            sLog.outErrorEventAI(
                "Event %u Action %u uses invalid current spell type %u (must be smaller or equal to %u)", i, j + 1,
                action.interruptSpell.currentSpellType, CURRENT_MAX_SPELL - 1);
          }
          break;
        case ACTION_T_START_RELAY_SCRIPT:
          if (action.relayScript.relayId > 0) {
            if (sRelayScripts.second.find(action.relayScript.relayId) == sRelayScripts.second.end()) {
              sLog.outErrorEventAI("Event %u Action %u references invalid dbscript_on_relay id %u", i, j + 1,
                                   action.relayScript.relayId);
            }
          } else {
            if (!sScriptMgr.CheckScriptRelayTemplateId(uint32(-action.relayScript.relayId))) {
              sLog.outErrorEventAI("Event %u Action %u references non-existing entry for relay Id template (%i) in "
                                   "dbscript_random_templates table.",
                                   i, j + 1, action.relayScript.relayId);
              break;
            }
          }
          break;
        case ACTION_T_TEXT_NEW:
          if (action.textNew.textId) {
            if (!sObjectMgr.GetMangosStringLocale(action.textNew.textId)) {
              sLog.outErrorEventAI("Event %u Action %u references non-existing entry (%i) in texts table.", i, j + 1,
                                   action.textNew.textId);
            ...
            if (!sScriptMgr.CheckScriptStringTemplateId(action.textNew.templateId)) {
              sLog.outErrorEventAI("Event %u Action %u references non-existing entry for text template (%i) in "
                                   "dbscript_random_templates table.",
                                   i, j + 1, action.textNew.textId);
              break;
            }
        ...
        case ACTION_T_DESPAWN_GUARDIANS:
          if (action.despawnGuardians.entryId &&
              !sCreatureStorage.LookupEntry<CreatureInfo>(action.despawnGuardians.entryId)) {
            sLog.outErrorEventAI("Event %u Action %u uses nonexistent Creature entry %u.", i, j + 1,
                                 action.despawnGuardians.entryId);
            action.despawnGuardians.entryId = 0;
          }
          break;
        case ACTION_T_SET_RANGED_MODE:
          if (action.rangedMode.type >= TYPE_MAX) {
            sLog.outErrorEventAI("Event %u Action %u uses nonexistent ranged mode type %u. Setting to 0.", i, j + 1,
                                 action.rangedMode.type);
            action.rangedMode.type = 0;
          }
          if (action.rangedMode.chaseDistance > 200) {
            sLog.outErrorEventAI("Event %u Action %u uses too large chase distance %u. Setting to 30.", i, j + 1,
                                 action.rangedMode.chaseDistance);
        ...
        case ACTION_T_SET_WALK:
          ...
          default:
            sLog.outErrorEventAI("Event %u Action %u uses invalid walking mode %u. Setting to RUN_DEFAULT.", i, j + 1,
                                 action.walkSetting.type);
        ...
        case ACTION_T_SET_FACING:
          if (action.setFacing.reset > 1) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid facing setting %u. Setting to 0.", i, j + 1,
                                 action.setFacing.reset);
        ...
        case ACTION_T_SET_SPELL_SET:
          if (!sObjectMgr.GetCreatureTemplateSpellSet(creature_id, action.spellSet.setId)) {
            sLog.outErrorEventAI("Event %u Action %u uses invalid spell set %u. Setting to 0.", i, j + 1,
                                 action.spellSet.setId);
        ...
        default:
          sLog.outErrorEventAI(
              "Event %u Action %u have currently not checked at load action type (%u). Need check code update?", i,
              j + 1, temp.action[j].type);
          break;
        }
      }
    ...
    for (uint32 i = 1; i < sCreatureStorage.GetMaxEntry(); ++i) {
      if (CreatureInfo const *cInfo = sCreatureStorage.LookupEntry<CreatureInfo>(i)) {
        ...
        if (ainame && !hasevent)
          ...
          sLog.outErrorEventAI("EventAI has script for creature entry (%u), but AIName = '%s' instead 'EventAI'.", i,
                               cInfo->AIName);
      }
    }
