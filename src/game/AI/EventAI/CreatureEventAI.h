...
#define LOG_PROCESS_EVENT                                                                                              \
  DEBUG_FILTER_LOG(LOG_FILTER_EVENT_AI_DEV,                                                                            \
                   "CreatureEventAI: Event type %u (script %u) triggered for %s (invoked by %s)",                      \
                   holder.event.event_type, holder.event.event_id, m_creature->GetGuidStr().c_str(),                   \
...
enum EventAI_Type {
  ...
  EVENT_T_TARGET_MISSING_AURA =
      28,                        // Param1 = SpellID, Param2 = Number of time stacked expected, Param3/4 Repeat Min/Max
...
enum EventAI_ActionType {
  ...
  ACTION_T_TEXT = 1, // TextId1, optionally -TextId2, optionally -TextId3(if -TextId2 exist). If more than just -TextId1
                     // is defined, randomize. Negative values.
  ACTION_T_SET_FACTION = 2, // FactionId (or 0 for default)
  ACTION_T_MORPH_TO_ENTRY_OR_MODEL =
      3,                         // Creature_template entry(param1) OR ModelId (param2) (or 0 for both to demorph)
  ...
  ACTION_T_COMBAT_MOVEMENT =
      21,                  // AllowCombatMovement (0 = stop combat based movement, anything else continue attacking)
  ...
  ACTION_T_MOUNT_TO_ENTRY_OR_MODEL =
      43,                     // Creature_template entry(param1) OR ModelId (param2) (or 0 for both to unmount)
  ACTION_T_CHANCED_TEXT = 44, // Chance to display the text, TextId1, optionally TextId2. If more than just -TextId1 is
  ...
  ACTION_T_CHANGE_MOVEMENT =
      48,              // MovementType, WanderDistance if Movement Type 1 and PathId if Movement Type 2, asDefault
...
enum Target {
  ...
  TARGET_T_ACTION_INVOKER =
      6, // Unit who caused this Event to occur (only works for EVENT_T_AGGRO, EVENT_T_KILL, EVENT_T_DEATH,
         // EVENT_T_SPELLHIT, EVENT_T_OOC_LOS, EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC,
         // EVENT_T_FRIENDLY_MISSING_BUFF, EVENT_T_RECEIVE_EMOTE, EVENT_T_RECEIVE_AI_EVENT)
  TARGET_T_ACTION_INVOKER_OWNER =
      7, // Unit who is responsible for Event to occur (only works for EVENT_T_AGGRO, EVENT_T_KILL, EVENT_T_DEATH,
         // EVENT_T_SPELLHIT, EVENT_T_OOC_LOS, EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC,
         // EVENT_T_FRIENDLY_MISSING_BUFF, EVENT_T_RECEIVE_EMOTE, EVENT_T_RECEIVE_AI_EVENT)
...
struct CreatureEventAI_Action {
  ...
  union {
    ...
    struct {
      ...
      uint32 melee; // if set: at stop send melee combat stop if in combat, use for terminate melee fighting state for
                    // switch to ranged
    } combat_movement;
...
struct CreatureEventAI_Event {
  ...
  union {
    ...
    struct {
      uint32 spellId;
      uint32 schoolMask; // -1 (==0xffffffff) is ok value for full mask, or must be more limited mask like (0 < 1) = 1
...
struct CreatureEventAIHolder {
  CreatureEventAIHolder(CreatureEventAI_Event p)
      : event(p), timer(0), enabled(true), inProgress(false), eventTarget(nullptr) {}
...
public:
  ...
  void CheckAndReadyEventForExecution(CreatureEventAIHolder &holder, Unit *actionInvoker = nullptr,
                                      Unit *AIEventSender = nullptr);
  void IncreaseDepthIfNecessary() {
    if (m_depth >= m_creatureEventAITempList.size())
      m_creatureEventAITempList.resize(m_depth + 1);
  }
  virtual bool ProcessEvent(CreatureEventAIHolder &holder, Unit *actionInvoker = nullptr,
                            Unit *AIEventSender = nullptr);
  virtual bool ProcessAction(CreatureEventAI_Action const &action, uint32 rnd, uint32 eventId, Unit *actionInvoker,
                             Unit *AIEventSender, Unit *eventTarget);
  ...
  inline Unit *GetTargetByType(uint32 target, Unit *actionInvoker, Unit *AIEventSender, Unit *eventTarget,
                               bool &isError, uint32 forSpellId = 0, uint32 selectFlags = 0) const;
  ...
  SpellSchoolMask GetMainAttackSchoolMask() const override {
    return m_currentRangedMode ? m_mainAttackMask : CreatureAI::GetMainAttackSchoolMask();
  }
...
protected:
  ...
  std::vector<std::vector<std::reference_wrapper<CreatureEventAIHolder>>>
      m_creatureEventAITempList; // Holder for events that are ready to go off
  uint32 m_depth;
