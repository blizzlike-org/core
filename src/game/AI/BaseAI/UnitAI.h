...
enum CastFlags {
  ...
  CAST_DISTANCE_YOURSELF =
      0x400, // If spell with this cast flag hits main aggro target, caster distances himself - EAI only
...
public:
   ...
   * @param uiRespawnDelay Delay (in seconds). If != 0, then this is the time after which the creature will respawn, if
   * = 0 the default respawn-delay will be used
   */
  ...
  virtual void SpellHitTarget(Unit *target, const SpellEntry *spellInfo, SpellMissInfo /*missInfo*/) {
    SpellHitTarget(target, spellInfo);
  }
  ...
  void SetMoveChaseParams(float dist, float angle, bool moveFurther) {
  ...
  virtual void JustRootedTarget(SpellEntry const *spellInfo, Unit *victim) {
    JustStoppedMovementOfTarget(spellInfo, victim);
  }
  virtual void JustStunnedTarget(SpellEntry const *spellInfo, Unit *victim) {
    JustStoppedMovementOfTarget(spellInfo, victim);
  }
...
template <class REAL_AI> struct CreatureAIFactory : public SelectableAI {
