...
enum AIEventType {
  ...
  AI_EVENT_CUSTOM_EVENTAI_E =
      10, // Sender = Npc that throws custom event, Invoker = TARGET_T_ACTION_INVOKER (if exists)
  AI_EVENT_CUSTOM_EVENTAI_F =
      11, // Sender = Npc that throws custom event, Invoker = TARGET_T_ACTION_INVOKER (if exists)
