...
float computeFallElevation(float t_passed, bool isSafeFall, float start_velocity) {
  ...
  if (t_passed > terminal_time) {
    result = terminalVelocity * (t_passed - terminal_time) + start_velocity * terminal_time +
...
template <class Flags, int N> void print_flags(Flags t, const char *(&names)[N], std::string &str) {
  for (int i = 0; i < N; ++i) {
