...
bool GameEventMgr::CheckOneGameEvent(uint16 entry, time_t currenttime) const {
  ...
  return m_gameEvents[entry].start < currenttime && currenttime < m_gameEvents[entry].end &&
         ((currenttime - m_gameEvents[entry].start) % (m_gameEvents[entry].occurence * MINUTE)) <
             (m_gameEvents[entry].length * MINUTE);
}
...
uint32 GameEventMgr::NextCheck(uint16 entry) const {
  ...
  if ((((currenttime - m_gameEvents[entry].start) % (m_gameEvents[entry].occurence * 60)) <
       (m_gameEvents[entry].length * 60)))
    // we return the delay before it ends
    delay = (m_gameEvents[entry].length * MINUTE) -
            ((currenttime - m_gameEvents[entry].start) % (m_gameEvents[entry].occurence * MINUTE));
  else // not in window, we return the delay before next start
    delay = (m_gameEvents[entry].occurence * MINUTE) -
            ((currenttime - m_gameEvents[entry].start) % (m_gameEvents[entry].occurence * MINUTE));
...
void GameEventMgr::LoadFromDB() {
  ...
  std::unique_ptr<QueryResult> result(WorldDatabase.Query(
      "SELECT entry,schedule_type,occurence,length,holiday,linkedTo,EventGroup,description FROM game_event"));
  if (!result) {
    m_gameEvents.clear();
    sLog.outString(">> Table game_event is empty!");
    sLog.outString();
...

  {
    ...
    do {
      ...
      if (gameEvent.length == 0 &&
          gameEvent.scheduleType != GAME_EVENT_SCHEDULE_SERVERSIDE) // length>0 is validity check
      {
        sLog.outErrorDb("`game_event` game event id (%i) have length 0 and can't be used.", event_id);
        gameEvent.start = time_t(FAR_FUTURE);
      }

      if (gameEvent.occurence <
          gameEvent.length) // occurence < length is useless. This also asserts that occurence > 0!
      {
        sLog.outErrorDb("`game_event` game event id (%i) has occurence %u  < length %u and can't be used.", event_id,
                        gameEvent.occurence, gameEvent.length);
        gameEvent.start = time_t(FAR_FUTURE);
      }
    ...
    for (uint16 itr = 1; itr < m_gameEvents.size(); ++itr) {
      if (m_gameEvents[itr].isValid() && m_gameEvents[itr].linkedTo != 0 &&
          (m_gameEvents[itr].linkedTo >= m_gameEvents.size() || !m_gameEvents[m_gameEvents[itr].linkedTo].isValid())) {
        sLog.outErrorDb("`game_event` game event id (%i) is Linked to invalid event %u", itr,
                        m_gameEvents[itr].linkedTo);
  ...
  result.reset(
      WorldDatabase.Query("SELECT entry,UNIX_TIMESTAMP(start_time),UNIX_TIMESTAMP(end_time) FROM game_event_time"));
  if (!result) {
    ...
    do {
      ...
      if (gameEvent.scheduleType != GAME_EVENT_SCHEDULE_DATE) {
        sLog.outErrorDb("`game_event` game event id (%i) has game_event_time but is not date scheduled - ignoring",
                        event_id);
        continue;
      }
  ...
  //                                   1              2
  result.reset(
      WorldDatabase.Query("SELECT creature.guid, game_event_creature.event "
        ...
        if (uint16 topPoolId = sPoolMgr.IsPartOfTopPool<Creature>(guid)) {
          ...
          if (eventRef != 0) {
            if (eventRef != event_id)
              sLog.outErrorDb(
                  "`game_event_creature` have creature (GUID: %u) for event %i from pool or subpool of pool (ID: %u) "
                  "but pool have already content from event %i. Pool don't must have content for different events!",
  ...
  //                                   1                2
  result.reset(
      WorldDatabase.Query("SELECT gameobject.guid, game_event_gameobject.event "
      ...
      if (event_id > 0) {
        ...
        if (uint16 topPoolId = sPoolMgr.IsPartOfTopPool<GameObject>(guid)) {
          ...
          if (eventRef != 0) {
            if (eventRef != event_id)
              sLog.outErrorDb(
                  "`game_event_gameobject` have gameobject (GUID: %u) for event %i from pool or subpool of pool (ID: "
                  "%u) but pool have already content from event %i. Pool don't must have content for different events!",
                  guid, event_id, topPoolId, eventRef);
          } else {
            eventRef = event_id;
            m_gameEventSpawnPoolIds[event_id].push_back(topPoolId);
  ...
  if (!result) {
    ...
    do {
      ...
      if (newData.equipment_id && !sObjectMgr.GetEquipmentInfo(newData.equipment_id) &&
        ...
        sLog.outErrorDb("Table `game_event_creature_data` have creature (Guid: %u) with equipment_id %u not found in "
                        "table `creature_equip_template` or `creature_equip_template_raw`, set to no equipment.",
                        guid, newData.equipment_id);
        newData.equipment_id = 0;
      }

      if (newData.vendor_id) {
        if (QueryResult *testResult =
                WorldDatabase.PQuery("SELECT 1 FROM npc_vendor_template where entry = '%u'", newData.vendor_id))
          delete testResult;
        else {
          sLog.outErrorDb("Table `game_event_creature_data` has a creature with (Guid: %u) and vendor_id %u which was "
                          "not found in table `npc_vendor_template`, set to no vendor.",
                          guid, newData.vendor_id);
          newData.vendor_id = 0;
        }
      }

      if (newData.entry_id && !ObjectMgr::GetCreatureTemplate(newData.entry_id)) {
        sLog.outErrorDb("Table `game_event_creature_data` have creature (Guid: %u) with event time entry %u not found "
                        "in table `creature_template`, set to no 0.",
                        guid, newData.entry_id);
        newData.entry_id = 0;
      }

      if (newData.spell_id_start && !sSpellTemplate.LookupEntry<SpellEntry>(newData.spell_id_start)) {
        sLog.outErrorDb("Table `game_event_creature_data` have creature (Guid: %u) with nonexistent spell_start %u, "
                        "set to no start spell.",
      ...
      if (newData.spell_id_end && !sSpellTemplate.LookupEntry<SpellEntry>(newData.spell_id_end)) {
        sLog.outErrorDb("Table `game_event_creature_data` have creature (Guid: %u) with nonexistent spell_end %u, set "
                        "to no end spell.",
  ...
  if (!result) {
    ...
    do {
      ...
      if (!pQuest) {
        sLog.outErrorDb(
            "Table `game_event_quest` contain entry for quest %u (event %u) but this quest does not exist. Skipping.",
            quest, event_id);
        continue;
      }
  ...
  if (!result) {
    ...
    do {
      ...
      if (!(mail.raceMask & RACEMASK_ALL_PLAYABLE)) {
        sLog.outErrorDb("Table `game_event_mail` have raceMask (%u) requirement for game event %i that not include any "
      ...
      if (mail.questId && !sObjectMgr.GetQuestTemplate(mail.questId)) {
        sLog.outErrorDb("Table `game_event_mail` have nonexistent quest (%u) requirement for game event %i, ignoring.",
                        mail.questId, event_id);
        continue;
      }

      if (!sMailTemplateStore.LookupEntry(mail.mailTemplateId)) {
        sLog.outErrorDb("Table `game_event_mail` have invalid mailTemplateId (%u) for game event %i that invalid not "
      ...
      if (!ObjectMgr::GetCreatureTemplate(mail.senderEntry)) {
        sLog.outErrorDb("Table `game_event_mail` have nonexistent sender creature entry (%u) for game event %i that "
                        "invalid not include any player races, ignoring.",
...
void GameEventMgr::Initialize(MapPersistentState *state) {
  ...
  for (uint16 m_ActiveEvent : m_activeEvents)
    for (IdList::iterator pool_itr = m_gameEventSpawnPoolIds[m_ActiveEvent].begin();
         pool_itr != m_gameEventSpawnPoolIds[m_ActiveEvent].end(); ++pool_itr)
...
uint32 GameEventMgr::Update(ActiveEvents const *activeAtShutdown /*= nullptr*/) {
  ...
  for (uint16 itr = 1; itr < m_gameEvents.size(); ++itr) {
    if ((m_gameEvents[itr].occurence == 0 && m_gameEvents[itr].scheduleType != GAME_EVENT_SCHEDULE_SERVERSIDE) ||
        (m_gameEvents[itr].scheduleType == GAME_EVENT_SCHEDULE_SERVERSIDE && m_isGameEventsInit))
      continue;
...
void GameEventMgr::ApplyNewEvent(uint16 event_id, bool resume) {
  ...
  // Not send mails at game event startup, if game event just resume after server shutdown (has been active at server
  // before shutdown)
  if (!resume)
...
void GameEventMgr::GameEventSpawn(int16 event_id) {
  ...
  if (internal_event_id < 0 || (size_t)internal_event_id >= m_gameEventCreatureGuids.size()) {
    sLog.outError("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventCreatureGuids element %i "
                  "(size: " SIZEFMTD ")",
                  internal_event_id, m_gameEventCreatureGuids.size());
    return;
  }
  ...
  if (internal_event_id < 0 || (size_t)internal_event_id >= m_gameEventGameobjectGuids.size()) {
    sLog.outError("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventGameobjectGuids element %i "
                  "(size: " SIZEFMTD ")",
                  internal_event_id, m_gameEventGameobjectGuids.size());
    return;
  }
  ...
  if (event_id > 0) {
    if ((size_t)event_id >= m_gameEventSpawnPoolIds.size()) {
      sLog.outError("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventSpawnPoolIds element %i "
                    "(size: " SIZEFMTD ")",
                    event_id, m_gameEventSpawnPoolIds.size());
      return;
    }
...
void GameEventMgr::GameEventUnspawn(int16 event_id) {
  ...
  if (internal_event_id < 0 || (size_t)internal_event_id >= m_gameEventCreatureGuids.size()) {
    sLog.outError("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventCreatureGuids element %i "
                  "(size: " SIZEFMTD ")",
                  internal_event_id, m_gameEventCreatureGuids.size());
    return;
  }
  ...
  if (internal_event_id < 0 || (size_t)internal_event_id >= m_gameEventGameobjectGuids.size()) {
    sLog.outError("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventGameobjectGuids element %i "
                  "(size: " SIZEFMTD ")",
                  internal_event_id, m_gameEventGameobjectGuids.size());
    return;
  }
  ...
  if (event_id > 0) {
    if ((size_t)event_id >= m_gameEventSpawnPoolIds.size()) {
      sLog.outError("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventSpawnPoolIds element %i "
                    "(size: " SIZEFMTD ")",
                    event_id, m_gameEventSpawnPoolIds.size());
      return;
    }
...
struct GameEventUpdateCreatureDataInMapsWorker {
  GameEventUpdateCreatureDataInMapsWorker(ObjectGuid guid, CreatureData const *data, GameEventCreatureData *event_data,
                                          bool activate)
  ...
  for (auto mail : mails) {
    if (mail.questId) {
      ...
      ss << "SELECT characters.guid FROM characters, character_queststatus "
         ...
         << mail.raceMask
         << " AND characters.deleteDate IS NULL AND character_queststatus.guid = characters.guid AND "
            "character_queststatus.quest = "
         << mail.questId << " AND character_queststatus.rewarded <> 0";
      sMassMailMgr.AddMassMailTask(new MailDraft(mail.mailTemplateId), MailSender(MAIL_CREATURE, mail.senderEntry),
                                   ss.str().c_str());
    } else
      sMassMailMgr.AddMassMailTask(new MailDraft(mail.mailTemplateId), MailSender(MAIL_CREATURE, mail.senderEntry),
                                   mail.raceMask);
...
template <> int16 GameEventMgr::GetGameEventId<Creature>(uint32 guid_or_poolid) {
  for (uint16 i = 0; i < m_gameEventCreatureGuids.size(); ++i) // 0 <= i <= 2*(S := m_gameEvents.size()) - 2
    for (GuidList::const_iterator itr = m_gameEventCreatureGuids[i].begin(); itr != m_gameEventCreatureGuids[i].end();
         ++itr)
...
template <> int16 GameEventMgr::GetGameEventId<GameObject>(uint32 guid_or_poolid) {
  for (uint16 i = 0; i < m_gameEventGameobjectGuids.size(); ++i)
    for (GuidList::const_iterator itr = m_gameEventGameobjectGuids[i].begin();
         itr != m_gameEventGameobjectGuids[i].end(); ++itr)
      if (*itr == guid_or_poolid)
        return i + 1 - m_gameEvents.size(); // -S *1 + 1 <= . <= 1*S - 1
  return 0;
}
...
template <> int16 GameEventMgr::GetGameEventId<Pool>(uint32 guid_or_poolid) {
  for (uint16 i = 0; i < m_gameEventSpawnPoolIds.size(); ++i)
    for (IdList::const_iterator itr = m_gameEventSpawnPoolIds[i].begin(); itr != m_gameEventSpawnPoolIds[i].end();
         ++itr)
      if (*itr == guid_or_poolid)
        return i;
  return 0;
...
GameEventMgr::GameEventMgr() { m_isGameEventsInit = false; }
...
bool IsHolidayActive(HolidayIds id) { return sGameEventMgr.IsActiveHoliday(id); }
...
void GameEventMgr::ComputeEventStartAndEndTime(GameEventData &data) {
  ...
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_1:
  case GAME_EVENT_SCHEDULE_DMF_1:
    firstMonday.tm_mon += (3 - monthRemainder) % 3;
    break; // 0 2 1 - x + 3 - 3
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_1_2:
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_2:
  case GAME_EVENT_SCHEDULE_DMF_2:
    firstMonday.tm_mon += (3 - monthRemainder + 1) % 3;
    break; // 1 0 2 - x + 3 - 2
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_1_3:
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_3:
  case GAME_EVENT_SCHEDULE_DMF_3:
    firstMonday.tm_mon += (3 - monthRemainder + 2) % 3;
    break; // 2 1 0 - x + 3 - 1
  default:
    break;
  }
  ...
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_1_2:
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_1_3:
    firstMonday.tm_mday -= 2;
    break;
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_1:
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_2:
  case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_3:
    firstMonday.tm_mday -= 1;
    break;
  default:
    break;
  }
