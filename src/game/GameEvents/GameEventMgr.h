...
public:
  ...
  bool IsValidEvent(uint16 event_id) const {
    return event_id < m_gameEvents.size() && m_gameEvents[event_id].isValid();
  }
  ...
  template <typename T> int16 GetGameEventId(uint32 guid_or_poolid);
...
protected:
  ...
  typedef std::pair<GameEventCreatureDataPerGuidMap::const_iterator, GameEventCreatureDataPerGuidMap::const_iterator>
