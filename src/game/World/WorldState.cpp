...
WorldState::WorldState()
    : m_emeraldDragonsState(0xF), m_emeraldDragonsTimer(0), m_emeraldDragonsChosenPositions(4, 0),
      m_isMagtheridonHeadSpawnedHorde(false), m_isMagtheridonHeadSpawnedAlliance(false), m_adalSongOfBattleTimer(0),
      m_expansion(EXPANSION_TBC), m_highlordKruulSpawned(false), m_highlordKruulTimer(0),
      m_highlordKruulChosenPosition(0) {
    ...
    do {
      ...
      case SAVE_ID_EMERALD_DRAGONS: {
        ...
        if (data.size()) {
          ...
          if (respawnTime) {
            TimePoint respawnTimePoint =
                std::chrono::time_point_cast<std::chrono::milliseconds>(Clock::from_time_t(respawnTime));
            m_emeraldDragonsTimer =
                std::chrono::duration_cast<std::chrono::milliseconds>(respawnTimePoint - curTime).count();
          } else
...
void WorldState::Save(SaveIds saveId) {
  ...
  case SAVE_ID_EMERALD_DRAGONS: {
    ...
    CharacterDatabase.PExecute("INSERT INTO world_state(Id,Data) VALUES('%u','%s')", SAVE_ID_EMERALD_DRAGONS,
                               dragonsData.data());
    break;
  }
  case SAVE_ID_AHN_QIRAJ: {
...
void WorldState::HandleGameObjectUse(GameObject *go, Unit *user) {
  ...
  case OBJECT_EVENT_TRAP_THRALL: {
    ...
    SendWorldstateUpdate(m_loveIsInTheAirMutex, GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL),
                         WORLD_STATE_LOVE_IS_IN_THE_AIR_THRALL);
    uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) + GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL) +
                      GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS);
  ...
  case OBJECT_EVENT_TRAP_CAIRNE: {
    ...
    SendWorldstateUpdate(m_loveIsInTheAirMutex, GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE),
                         WORLD_STATE_LOVE_IS_IN_THE_AIR_CAIRNE);
    uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) + GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL) +
                      GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS);
    SendWorldstateUpdate(m_loveIsInTheAirMutex, hordeSum, WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_HORDE);
    break;
  ...
  case OBJECT_EVENT_TRAP_SYLVANAS: {
    ...
    SendWorldstateUpdate(m_loveIsInTheAirMutex, GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS),
                         WORLD_STATE_LOVE_IS_IN_THE_AIR_SYLVANAS);
    uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) + GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL) +
                      GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS);
    SendWorldstateUpdate(m_loveIsInTheAirMutex, hordeSum, WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_HORDE);
    break;
  ...
  case OBJECT_EVENT_TRAP_BOLVAR: {
    ...
    SendWorldstateUpdate(m_loveIsInTheAirMutex, GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR),
                         WORLD_STATE_LOVE_IS_IN_THE_AIR_BOLVAR);
    uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) + GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) +
                         GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
  ...
  case OBJECT_EVENT_TRAP_MAGNI: {
    ...
    SendWorldstateUpdate(m_loveIsInTheAirMutex, GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI),
                         WORLD_STATE_LOVE_IS_IN_THE_AIR_MAGNI);
    uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) + GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) +
                         GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
    SendWorldstateUpdate(m_loveIsInTheAirMutex, allianceSum, WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_ALLIANCE);
    break;
  ...
  case OBJECT_EVENT_TRAP_TYRANDE: {
    ...
    SendWorldstateUpdate(m_loveIsInTheAirMutex, GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE),
                         WORLD_STATE_LOVE_IS_IN_THE_AIR_TYRANDE);
    uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) + GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) +
                         GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
    SendWorldstateUpdate(m_loveIsInTheAirMutex, allianceSum, WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_ALLIANCE);
    break;
...
void WorldState::HandlePlayerLeaveZone(Player *player, uint32 zoneId) {
  ...
  case ZONEID_UNDERCITY: {
    {
      std::lock_guard<std::mutex> guard(m_loveIsInTheAirMutex);
      auto position = std::find(m_loveIsInTheAirCapitalsPlayers.begin(), m_loveIsInTheAirCapitalsPlayers.end(),
    ...
    {
      std::lock_guard<std::mutex> guard(m_aqData.m_warEffortMutex);
      auto position = std::find(m_aqData.m_warEffortWorldstatesPlayers.begin(),
                                m_aqData.m_warEffortWorldstatesPlayers.end(), player->GetObjectGuid());
  ...
  case ZONEID_MAGTHERIDON_LAIR: {
    ...
    auto position =
        std::find(m_magtheridonHeadPlayers.begin(), m_magtheridonHeadPlayers.end(), player->GetObjectGuid());
  ...
  case ZONEID_ARCATRAZ: {
    ...
    auto position =
        std::find(m_adalSongOfBattlePlayers.begin(), m_adalSongOfBattlePlayers.end(), player->GetObjectGuid());
  ...
  case ZONEID_SUNWELL_PLATEAU: {
    ...
    auto position = std::find(m_sunsReachData.m_sunsReachReclamationPlayers.begin(),
                              m_sunsReachData.m_sunsReachReclamationPlayers.end(), player->GetObjectGuid());
...
static float emeraldDragonSpawns[4][4] = {{-10428.8f, -392.176f, 43.7411f, 0.932375f},
                                          {753.619f, -4012.f, 94.043f, 3.193f},
                                          {-2872.66f, 1884.25f, 52.7336f, 2.6529f},
                                          {3301.05f, -3732.57f, 173.544f, 2.9147f}};

static uint32 pathIds[4] = {1, 0, 0, 0};

bool WorldState::IsDragonSpawned(uint32 entry) { return ((1 << (entry - NPC_YSONDRE)) & m_emeraldDragonsState) == 0; }
...
void WorldState::RespawnEmeraldDragons() {
  ...
  sMapMgr.DoForAllMapsWithMapId(0, [&](Map *map) {
    if (IsDragonSpawned(m_emeraldDragonsChosenPositions[0])) {
      if (Creature *duskwoodDragon = WorldObject::SummonCreature(
              TempSpawnSettings(nullptr, m_emeraldDragonsChosenPositions[0], emeraldDragonSpawns[0][0],
                                emeraldDragonSpawns[0][1], emeraldDragonSpawns[0][2], emeraldDragonSpawns[0][3],
                                TEMPSPAWN_DEAD_DESPAWN, 0, false, false, pathIds[0]),
              map))
        duskwoodDragon->GetMotionMaster()->MoveWaypoint(pathIds[0]);
    }
    if (IsDragonSpawned(m_emeraldDragonsChosenPositions[1]))
      WorldObject::SummonCreature(TempSpawnSettings(nullptr, m_emeraldDragonsChosenPositions[1],
                                                    emeraldDragonSpawns[1][0], emeraldDragonSpawns[1][1],
                                                    emeraldDragonSpawns[1][2], emeraldDragonSpawns[1][3],
                                                    TEMPSPAWN_DEAD_DESPAWN, 0, false, false, pathIds[1]),
                                  map);
  });
  sMapMgr.DoForAllMapsWithMapId(1, [&](Map *map) {
    if (IsDragonSpawned(m_emeraldDragonsChosenPositions[2]))
      WorldObject::SummonCreature(TempSpawnSettings(nullptr, m_emeraldDragonsChosenPositions[2],
                                                    emeraldDragonSpawns[2][0], emeraldDragonSpawns[2][1],
                                                    emeraldDragonSpawns[2][2], emeraldDragonSpawns[2][3],
                                                    TEMPSPAWN_DEAD_DESPAWN, 0, false, false, pathIds[2]),
                                  map);
    if (IsDragonSpawned(m_emeraldDragonsChosenPositions[3]))
      WorldObject::SummonCreature(TempSpawnSettings(nullptr, m_emeraldDragonsChosenPositions[3],
                                                    emeraldDragonSpawns[3][0], emeraldDragonSpawns[3][1],
                                                    emeraldDragonSpawns[3][2], emeraldDragonSpawns[3][3],
                                                    TEMPSPAWN_DEAD_DESPAWN, 0, false, false, pathIds[3]),
                                  map);
  });
}
...
void WorldState::AddWarEffortProgress(AQResources resource, uint32 count) {
  ...
  if (id >= aqWorldStateTotalsMap.size())
    id -= 5;
  if (m_aqData.m_WarEffortCounters[resource] >=
      aqWorldStateTotalsMap[id].second) // fulfilled this condition - check all
  {
    bool success = true;
    for (uint32 i = 0; i < RESOURCE_MAX; ++i) {
...
void WorldState::StopWarEffortEvent() {
  switch (m_aqData.m_phase) {
  case PHASE_1_GATHERING_RESOURCES:
    sGameEventMgr.StopEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_1);
    break;
  case PHASE_2_TRANSPORTING_RESOURCES:
    sGameEventMgr.StopEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_2);
    break;
  case PHASE_3_GONG_TIME:
    sGameEventMgr.StopEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_3);
    break;
  case PHASE_4_10_HOUR_WAR:
    sGameEventMgr.StopEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_4);
    break;
  case PHASE_5_DONE:
    sGameEventMgr.StopEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_5);
    break;
  default:
    break;
  }
...
void WorldState::StartWarEffortEvent() {
  switch (m_aqData.m_phase) {
  case PHASE_1_GATHERING_RESOURCES:
    sGameEventMgr.StartEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_1);
    break;
  case PHASE_2_TRANSPORTING_RESOURCES:
    sGameEventMgr.StartEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_2);
    break;
  case PHASE_3_GONG_TIME:
    sGameEventMgr.StartEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_3);
    break;
  case PHASE_4_10_HOUR_WAR:
    sGameEventMgr.StartEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_4);
    break;
  case PHASE_5_DONE:
    sGameEventMgr.StartEvent(GAME_EVENT_AHN_QIRAJ_EFFORT_PHASE_5);
    break;
  default:
    break;
  }
...
std::string WorldState::GetAQPrintout() {
  std::string output =
      "Phase: " + std::to_string(m_aqData.m_phase) + " Timer: " + std::to_string(m_aqData.m_timer) + "\nValues:";
...
// Highlord Kruul
enum HighlordKruul : uint32 { NPC_HIGHLORD_KRUUL = 18338 };
...
void WorldState::RespawnHighlordKruul() {
  ...
  sMapMgr.DoForAllMapsWithMapId(mapId, [&](Map *map) {
    WorldObject::SummonCreature(
        TempSpawnSettings(nullptr, NPC_HIGHLORD_KRUUL, highlordKruulSpawns[m_highlordKruulChosenPosition][0],
                          highlordKruulSpawns[m_highlordKruulChosenPosition][1],
                          highlordKruulSpawns[m_highlordKruulChosenPosition][2],
                          highlordKruulSpawns[m_highlordKruulChosenPosition][3], TEMPSPAWN_DEAD_DESPAWN, 0, true, false,
                          m_highlordKruulChosenPosition),
        map);
  });
}
...
void WorldState::HandleSunsReachPhaseTransition(uint32 newPhase) {
  ...
  SendWorldstateUpdate(m_sunsReachData.m_sunsReachReclamationMutex, m_sunsReachData.m_phase,
...
void WorldState::StopSunsReachPhase(bool forward) {
  ...
  case SUNS_REACH_PHASE_1_STAGING_AREA:
    sGameEventMgr.StopEvent(GAME_EVENT_QUEL_DANAS_PHASE_1);
    break;
  case SUNS_REACH_PHASE_2_SANCTUM:
    sGameEventMgr.StopEvent(GAME_EVENT_QUEL_DANAS_PHASE_2_ONLY);
    if (!forward)
      sGameEventMgr.StopEvent(GAME_EVENT_QUEL_DANAS_PHASE_2_PERMANENT);
    break;
  case SUNS_REACH_PHASE_3_ARMORY:
    sGameEventMgr.StopEvent(GAME_EVENT_QUEL_DANAS_PHASE_3_ONLY);
    if (!forward)
      sGameEventMgr.StopEvent(GAME_EVENT_QUEL_DANAS_PHASE_3_PERMANENT);
    break;
  case SUNS_REACH_PHASE_4_HARBOR:
    sGameEventMgr.StopEvent(GAME_EVENT_QUEL_DANAS_PHASE_4);
    break;
  default:
    break;
  }
...
void WorldState::StartSunsReachPhase(bool initial) {
  ...
  case SUNS_REACH_PHASE_3_ARMORY:
    if (initial)
      sGameEventMgr.StartEvent(GAME_EVENT_QUEL_DANAS_PHASE_2_PERMANENT);
    sGameEventMgr.StartEvent(GAME_EVENT_QUEL_DANAS_PHASE_3_ONLY);
...
std::string WorldState::GetSunsReachPrintout() {
  std::string output = "Phase: " + std::to_string(m_sunsReachData.m_phase) +
                       " Subphase mask: " + std::to_string(m_sunsReachData.m_subphaseMask) + "\nValues:";
...
uint32 SunsReachReclamationData::GetPhasePercentage(uint32 phase) {
  switch (phase) {
  case SUNS_REACH_PHASE_1_STAGING_AREA:
    return uint32((m_sunsReachReclamationCounters[COUNTER_ERRATIC_BEHAVIOR] +
                   m_sunsReachReclamationCounters[COUNTER_SANCTUM_WARDS]) *
                  100 / (2 * COUNTER_MAX_VAL_REQ));
  case SUNS_REACH_PHASE_2_SANCTUM:
    return uint32((m_sunsReachReclamationCounters[COUNTER_BATTLE_FOR_THE_SUNS_REACH_ARMORY] +
                   m_sunsReachReclamationCounters[COUNTER_DISTRACTION_AT_THE_DEAD_SCAR]) *
                  100 / (2 * COUNTER_MAX_VAL_REQ));
  case SUNS_REACH_PHASE_3_ARMORY:
    return uint32((m_sunsReachReclamationCounters[COUNTER_INTERCEPT_THE_REINFORCEMENTS] +
                   m_sunsReachReclamationCounters[COUNTER_TAKING_THE_HARBOR]) *
                  100 / (2 * COUNTER_MAX_VAL_REQ));
  default:
    return 0;
  }
...
uint32 SunsReachReclamationData::GetSubPhasePercentage(uint32 subPhase) {
  switch (subPhase) {
  case SUBPHASE_PORTAL:
    return uint32(m_sunsReachReclamationCounters[COUNTER_INTERCEPTING_THE_MANA_CELLS] * 100 / COUNTER_MAX_VAL_REQ);
  case SUBPHASE_ANVIL:
    return uint32(m_sunsReachReclamationCounters[COUNTER_MAKING_READY] * 100 / COUNTER_MAX_VAL_REQ);
  case SUBPHASE_ALCHEMY_LAB:
    return uint32(m_sunsReachReclamationCounters[COUNTER_DISCOVERING_YOUR_ROOTS] * 100 / COUNTER_MAX_VAL_REQ);
  case SUBPHASE_MONUMENT:
    return uint32(m_sunsReachReclamationCounters[COUNTER_A_CHARITABLE_DONATION] * 100 / COUNTER_MAX_VAL_REQ);
  default:
    return 0;
  }
}
...
void WorldState::FillInitialWorldStates(ByteBuffer &data, uint32 &count, uint32 zoneId, uint32 /*areaId*/) {
  ...
  case ZONEID_UNDERCITY: {
    if (sGameEventMgr.IsActiveHoliday(HOLIDAY_LOVE_IS_IN_THE_AIR)) {
      uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) +
                           GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) + GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
      uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) + GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL) +
                        GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS);
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_BOLVAR,
                                GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR));
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_TYRANDE,
                                GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE));
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_MAGNI,
                                GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI));
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_ALLIANCE, allianceSum);
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_CAIRNE,
                                GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE));
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_THRALL,
                                GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL));
      FillInitialWorldStateData(data, count, WORLD_STATE_LOVE_IS_IN_THE_AIR_SYLVANAS,
                                GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS));
    ...
    if (m_aqData.m_phase == PHASE_1_GATHERING_RESOURCES) {
      ...
      FillInitialWorldStateData(data, count, WORLD_STATE_AQ_DAYS_LEFT,
                                uint32(m_aqData.m_timer / DAY * IN_MILLISECONDS));
    break;
  }
  case ZONEID_ISLE_OF_QUEL_DANAS:
  case ZONEID_MAGISTERS_TERRACE:
  ...
  case ZONEID_SHATTRATH: {
    switch (m_sunsReachData.m_phase) {
    case SUNS_REACH_PHASE_1_STAGING_AREA:
      FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_SANCTUM,
                                m_sunsReachData.GetPhasePercentage(m_sunsReachData.m_phase));
      break;
    case SUNS_REACH_PHASE_2_SANCTUM:
      FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_ARMORY,
                                m_sunsReachData.GetPhasePercentage(m_sunsReachData.m_phase));
      break;
    case SUNS_REACH_PHASE_3_ARMORY:
      FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_HARBOR,
                                m_sunsReachData.GetPhasePercentage(m_sunsReachData.m_phase));
      break;
    case SUNS_REACH_PHASE_4_HARBOR:
      if ((m_sunsReachData.m_subphaseMask & SUBPHASE_ALCHEMY_LAB) == 0)
        FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_ALCHEMY_LAB,
                                  m_sunsReachData.GetSubPhasePercentage(SUBPHASE_ALCHEMY_LAB));
      if ((m_sunsReachData.m_subphaseMask & SUBPHASE_MONUMENT) == 0)
        FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_MONUMENT,
                                  m_sunsReachData.GetSubPhasePercentage(SUBPHASE_MONUMENT));
      break;
    }
    if (m_sunsReachData.m_phase >= SUNS_REACH_PHASE_3_ARMORY && (m_sunsReachData.m_subphaseMask & SUBPHASE_ANVIL) == 0)
      FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_ANVIL,
                                m_sunsReachData.GetSubPhasePercentage(SUBPHASE_ANVIL));

    FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_MUSIC, m_sunsReachData.m_phase);

    if (m_sunsReachData.m_phase >= SUNS_REACH_PHASE_2_SANCTUM &&
        (m_sunsReachData.m_subphaseMask & SUBPHASE_PORTAL) == 0)
      FillInitialWorldStateData(data, count, WORLD_STATE_QUEL_DANAS_PORTAL,
                                m_sunsReachData.GetSubPhasePercentage(SUBPHASE_PORTAL));
    break;
  }
  }
}
