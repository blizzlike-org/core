...
Spell::Spell(Unit *caster, SpellEntry const *info, uint32 triggeredFlags, ObjectGuid originalCasterGUID,
  ...
  MANGOS_ASSERT(info == sSpellTemplate.LookupEntry<SpellEntry>(info->Id) &&
  ...
  m_ignoreConcurrentCasts = m_IsTriggeredSpell || ((triggeredFlags & TRIGGERED_IGNORE_CURRENT_CASTED_SPELL) != 0) ||
                            m_spellInfo->HasAttribute(SPELL_ATTR_EX4_CAN_CAST_WHILE_CASTING);
  m_hideInCombatLog = (m_IsTriggeredSpell && !IsAutoRepeatRangedSpell(m_spellInfo)) ||
                      ((triggeredFlags & TRIGGERED_HIDE_CAST_IN_COMBAT_LOG) != 0);
  ...
  memcpy(m_filteringScheme, SpellTargetMgr::GetSpellTargetingData(m_spellInfo->Id).filteringScheme,
         sizeof(m_filteringScheme));
...
template <typename T> WorldObject *Spell::FindCorpseUsing() {
...
void Spell::FillTargetMap() {
  ...
  uint8 effToIndex[MAX_EFFECT_INDEX] = {
      0, 1, 2}; // Helper array, to link to another tmpUnitList, if the targets for both effects match
  for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i) {
    ...
    if (effectTargetType == TARGET_TYPE_SPECIAL_UNIT) // area auras need custom handling
    {
      ...
      if (targetA && !ignoredTargets.first) {
        if (SpellTargetInfoTable[targetA].type == TARGET_TYPE_UNIT &&
            SpellTargetInfoTable[targetA].enumerator == TARGET_ENUMERATOR_SINGLE)
          hadTarget = true;
        SetTargetMap(SpellEffectIndex(i), targetA, false, targetingData);
      }
      if (targetB && !ignoredTargets.second) {
        if (SpellTargetInfoTable[targetB].type == TARGET_TYPE_UNIT &&
            SpellTargetInfoTable[targetB].enumerator == TARGET_ENUMERATOR_SINGLE)
          hadTarget = true;
      ...
      if (targetA == TARGET_NONE && targetB == TARGET_NONE) {
        ...
        else if (uint32 defaultTarget = SpellEffectInfoTable[m_spellInfo->Effect[i]]
                                            .defaultTarget) // else resort to default effect type if it exists
      ...
      } else // normal case, use existing spell data
      {
        ...
        if (targetB && !ignoredTargets.second)
          SetTargetMap(SpellEffectIndex(i), targetB, true, targetingData);
        if (effectTargetType ==
            TARGET_TYPE_UNIT_DEST) // special case - no unit target, but need to check for valid units
          if (SpellTargetInfoTable[targetA].type != TARGET_TYPE_UNIT &&
              SpellTargetInfoTable[targetB].type != TARGET_TYPE_UNIT) // no fill for unit out of targets
            FillFromTargetFlags(targetingData, SpellEffectIndex(i)); // inefficient call, very rare, less code duplicity
      }
    }

    bool processedUnits =
        false;                 // TODO: implement adding of executionless targets which are not immune to SMSG_SPELL_GO
    ...
    case TARGET_TYPE_SPECIAL_UNIT:
      processedUnits = true;
      for (uint8 rightTarget = 0; rightTarget < 2;
           ++rightTarget) // need to process target A and B separately due to effect masks
      {
...
void Spell::prepareDataForTriggerSystem() {
  ...
  if ((m_CastItem && m_spellInfo->SpellFamilyFlags == nullptr) ||
      m_spellInfo->HasAttribute(SPELL_ATTR_EX3_CANT_TRIGGER_PROC) || m_doNotProc)
    m_canTrigger = false; // Do not trigger from item cast spell
  else if (!m_IsTriggeredSpell)
    m_canTrigger = true; // Normal cast - can trigger
  else if (!m_triggeredByAuraSpell)
    m_canTrigger = true; // Triggered from SPELL_EFFECT_TRIGGER_SPELL - can trigger
  else if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_TRIGGERED_CAN_TRIGGER_PROC) ||
           m_spellInfo->HasAttribute(SPELL_ATTR_EX3_TRIGGERED_CAN_TRIGGER_SPECIAL))
...
void Spell::PrepareMasksForProcSystem(uint8 effectMask, uint32 &procAttacker, uint32 &procVictim, WorldObject *caster,
                                      WorldObject *target) {
...
void Spell::AddUnitTarget(Unit *target, uint8 effectMask, CheckException exception) {
  ...
  targetInfo.missCondition =
      (m_ignoreHitResult ? SPELL_MISS_NONE
                         : m_caster->SpellHitResult(target, m_spellInfo, targetInfo.effectMask, m_reflectable, false,
                                                    &targetInfo.heartbeatResistChance));
  ...
  if (speed > 0.0f && affectiveObject && target != affectiveObject) {
    ...
    float dist = affectiveObject->GetDistance(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(),
                                              DIST_CALC_NONE);
  ...
  if (targetInfo.missCondition == SPELL_MISS_REFLECT) {
    ...
    if (!m_originalCasterGUID.IsUnit()) {
      Unit::ProcDamageAndSpell(ProcSystemArguments(nullptr, target, PROC_FLAG_NONE,
                                                   PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1,
                                                   BASE_ATTACK, m_spellInfo));
      ...
      targetInfo.reflectResult = m_caster->SpellHitResult(m_caster, m_spellInfo, targetInfo.effectMask, m_reflectable,
                                                          true, &targetInfo.heartbeatResistChance);
...
void Spell::AddGOTarget(GameObject *target, uint8 effectMask) {
  m_targets.m_targetMask |=
      TARGET_FLAG_GAMEOBJECT; // all spells with GO target must have this flag TODO: do this based on target type
...
void Spell::DoAllEffectOnTarget(TargetInfo *target) {
  ...
  Unit *unit = m_caster->GetObjectGuid() == target->targetGUID ? m_caster
  ...
  if (((procAttacker | procVictim) & NEGATIVE_TRIGGER_MASK) && !(target->effectHitMask & m_negativeEffectMask) &&
  ...
  if (m_healing) {
    ...
    if (real_caster)
      unitTarget->getHostileRefManager().threatAssist(
          real_caster, float(gain) * 0.5f * sSpellMgr.GetSpellThreatMultiplier(m_spellInfo), m_spellInfo, false,
          m_IsTriggeredSpell);

    // Do triggers for unit (reflect triggers passed on hit phase for correct drop charge)
    if (m_canTrigger && missInfo != SPELL_MISS_REFLECT)
      Unit::ProcDamageAndSpell(ProcSystemArguments(caster, unitTarget,
                                                   real_caster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim,
                                                   procEx, addhealth, m_attackType, m_spellInfo, this, gain, true));
  }
  // Do damage and triggers
    ...
    Unit::DealDamageMods(caster, spellDamageInfo.target, spellDamageInfo.damage, &spellDamageInfo.absorb,
                         SPELL_DIRECT_DAMAGE, m_spellInfo);
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN &&
        m_spellInfo->SpellFamilyFlags & uint64(0x0000000800000000) && m_spellInfo->SpellIconID == 153) {
    ...
    // Bloodthirst
             ...
             m_spellInfo->SpellFamilyFlags & uint64(0x40000000000)) {
      uint32 BTAura = 0;
    ...
    if (m_canTrigger && missInfo != SPELL_MISS_REFLECT)
      Unit::ProcDamageAndSpell(ProcSystemArguments(caster, unitTarget,
                                                   real_caster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim,
                                                   procEx, spellDamageInfo.damage, m_attackType, m_spellInfo, this));
  }
  // Passive spell hits/misses or active spells only misses (only triggers if proc flags set)
    ...
    if (m_canTrigger && missInfo != SPELL_MISS_REFLECT)
      ...
      Unit::ProcDamageAndSpell(ProcSystemArguments(
          caster, procAttacker & PROC_FLAG_ON_TRAP_ACTIVATION ? m_targets.getUnitTarget() : unit,
          real_caster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim, procEx, 0, m_attackType, m_spellInfo, this));
  }
  ...
  if (unit->GetTypeId() == TYPEID_UNIT) {
    ...
    if (real_caster && !((Creature *)unit)->IsPet() && !IsAutoRepeat() && !IsNextMeleeSwingSpell(m_spellInfo) &&
        !IsChannelActive())
  ...
  if (real_caster && real_caster != m_caster && real_caster->GetTypeId() == TYPEID_UNIT &&
...
void Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask, TargetInfo *target, bool isReflected) {
  ...
  if (traveling && realCaster && realCaster != unit) {
    ...
    if (unit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE) && !IsSpellCanTargetUnattackable(m_spellInfo) &&
  ...
  if (m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) || IsCreatureDRSpell(m_spellInfo)) {
    ...
    if ((GetDiminishingReturnsGroupType(m_diminishGroup) == DRTYPE_PLAYER && unit->GetTypeId() == TYPEID_PLAYER) ||
      ...
      unit->IncrDiminishing(m_diminishGroup,
                            (unit->GetTypeId() == TYPEID_PLAYER && m_caster->GetTypeId() == TYPEID_PLAYER));
  }
  ...
  if (m_spellAuraHolder) {
    ...
    if (!m_spellAuraHolder->IsEmptyHolder()) {
      ...
      if (originalDuration > 0 && target && target->heartbeatResistChance && !m_spellAuraHolder->IsPositive()) {
        ...
        if (bool heartbeat = !(m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
                               unit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED)))
          m_spellAuraHolder->SetHeartbeatResist(target->heartbeatResistChance, originalDuration,
                                                uint32(m_diminishLevel));
      }
...
void Spell::DoAllTargetlessEffects(bool dest) {
  ...
  if (effectMask) {
    ...
    if (unitCaster) {
      ...
      Unit::ProcDamageAndSpell(ProcSystemArguments(
          unitCaster, procAttacker & PROC_FLAG_ON_TRAP_ACTIVATION ? m_targets.getUnitTarget() : nullptr,
          unitCaster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim, procEx, 0, m_attackType, m_spellInfo, this));
    }
  }
}
...
void Spell::InitializeDamageMultipliers() {
  for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i) {
    ...
    if ((m_spellInfo->EffectImplicitTargetA[i] == TARGET_UNIT_ENEMY ||
         m_spellInfo->EffectImplicitTargetA[i] == TARGET_UNIT_FRIEND_CHAIN_HEAL) &&
        (EffectChainTarget > 1))
...
bool Spell::IsAliveUnitPresentInTargetList() {
  ...
  for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit) {
    if (ihit->missCondition == SPELL_MISS_NONE && (channelAuraMask & ihit->effectHitMask)) {
      Unit *unit = m_caster->GetObjectGuid() == ihit->targetGUID ? m_caster
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, bool targetB, TempTargetingData &targetingData) {
  ...
  case TARGET_LOCATION_CASTER_RANDOM_SIDE:
    ...
    // Get a random point in circle. Use sqrt(rand) to correct distribution when converting polar to Cartesian
    // coordinates.
  ...
  case TARGET_LOCATION_CASTER_RANDOM_CIRCUMFERENCE: {
    ...
    case TARGET_LOCATION_UNIT_RANDOM_SIDE:
    case TARGET_LOCATION_UNIT_RANDOM_CIRCUMFERENCE:
      target = m_targets.getUnitTarget() ? m_targets.getUnitTarget() : m_caster;
      break;
    default:
      target = m_caster;
      break;
  ...
  case TARGET_LOCATION_DYNOBJ_POSITION:
    ...
    if (DynamicObject *dynObj =
            m_caster->GetDynObject(m_triggeredByAuraSpell ? m_triggeredByAuraSpell->Id : m_spellInfo->Id))
  ...
  case TARGET_LOCATION_DATABASE: {
    if (SpellTargetPosition const *st = sSpellMgr.GetSpellTargetPosition(m_spellInfo->Id)) {
      ...
      // far-teleport spells are handled in SpellEffect, elsewise report an error about an unexpected map (spells are
      // always locally)
      if (st->target_mapId != m_caster->GetMapId() && m_spellInfo->Effect[effIndex] != SPELL_EFFECT_TELEPORT_UNITS &&
          m_spellInfo->Effect[effIndex] != SPELL_EFFECT_BIND)
        sLog.outError("SPELL: wrong map (%u instead %u) target coordinates for spell ID %u", st->target_mapId,
                      m_caster->GetMapId(), m_spellInfo->Id);
    } else
  ...
  case TARGET_LOCATION_CASTER_FRONT_LEAP: {
    ...
    const float IN_OR_UNDER_LIQUID_RANGE =
        0.8f; // range to make player under liquid or on liquid surface from liquid level
    ...
    // falling case
    if (!m_caster->GetMap()->GetHeightInRange(prevPos.x, prevPos.y, groundZ, 3.0f) &&
      ...
      nextPos.z = prevPos.z - 2.0f; // little hack to avoid the impression to go up when teleporting instead of continue
                                    // to fall. This value may need some tweak
      ...
      if (abs(prevPos.z - nextPos.z) > 40.f) // dont move too high - magical constant - might need verification
      {
        ...
        m_caster->GetMap()->GetHitPosition(prevPos.x, prevPos.y, prevPos.z + 0.5f, nextPos.x, nextPos.y, nextPos.z,
                                           -0.5f);
    } else {
      ...
      for (uint32 i = 1; i < numChecks + 1; ++i) {
        ...
        if (isInLiquid || (!isInLiquidTested &&
                           m_caster->GetMap()->GetTerrain()->IsInWater(nextPos.x, nextPos.y, nextPos.z, &liquidData))) {
          ...
          if ((liquidData.level - IN_OR_UNDER_LIQUID_RANGE) > nextPos.z)
            ...
            nextPos.z =
                liquidData.level - IN_OR_UNDER_LIQUID_RANGE; // we are on water surface, so next z equal liquid level
        ...
        // unitTarget->SummonCreature(VISUAL_WAYPOINT, nextPos.x, nextPos.y, nextPos.z, 0, TEMPSUMMON_TIMED_DESPAWN,
        // 15000);
        float hitZ = nextPos.z + 1.5f;
        if (m_caster->GetMap()->GetHitPosition(prevPos.x, prevPos.y, prevPos.z + 1.5f, nextPos.x, nextPos.y, hitZ,
                                               -1.0f)) {
  ...
  case TARGET_LOCATION_UNIT_POSITION: {
    if (Unit *currentTarget = m_targets.getUnitTarget())
      m_targets.setDestination(currentTarget->GetPositionX(), currentTarget->GetPositionY(),
  ...
  case TARGET_LOCATION_CASTER_FISHING_SPOT: {
    ...
    m_caster->GetNearPoint2d(x, y, dis + m_caster->GetObjectBoundingRadius(),
                             m_caster->GetOrientation() + angle_offset);

    GridMapLiquidData liqData;
    if (!m_caster->GetTerrain()->IsInWater(
            x, y, m_caster->GetTerrain()->GetWaterLevel(x, y, m_caster->GetPositionZ()) - 1.0f, &liqData)) {
  ...
  case TARGET_UNIT_NEAR_CASTER: {
    ...
    case TARGET_UNIT_NEAR_CASTER: // TODO: Rename TARGET_UNIT_NEAR_CASTER to something better and find real difference
                                  // with TARGET_UNIT_FRIEND_NEAR_CASTER.
    {
    ...
    SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> bounds =
        sSpellScriptTargetStorage.getBounds<SpellTargetEntry>(m_spellInfo->Id);
  ...
  case TARGET_ENUM_UNITS_SCRIPT_AOE_AT_SRC_LOC: {
    ...
    FillAreaTargets(bounds.first != bounds.second ? tempTargetUnitMap : tempUnitList, radius, cone, PUSH_SRC_CENTER,
                    SPELL_TARGETS_ALL);
  ...
  case TARGET_ENUM_UNITS_SCRIPT_AOE_AT_DEST_LOC: {
    ...
    SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> bounds =
        sSpellScriptTargetStorage.getBounds<SpellTargetEntry>(m_spellInfo->Id);
    // fill real target list if no spell script target defined
    FillAreaTargets(bounds.first != bounds.second ? tempTargetUnitMap : tempUnitList, radius, cone, PUSH_DEST_CENTER,
                    SPELL_TARGETS_ALL);
  ...
  case TARGET_ENUM_GAMEOBJECTS_SCRIPT_AOE_AT_DEST_LOC: {
    ...
    SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> bounds =
        sSpellScriptTargetStorage.getBounds<SpellTargetEntry>(m_spellInfo->Id);
    for (SQLMultiStorage::SQLMultiSIterator<SpellTargetEntry> i_spellST = bounds.first; i_spellST != bounds.second;
         ++i_spellST) {
  ...
  case TARGET_UNIT_RAID: {
    if (Unit *target = m_targets.getUnitTarget()) {
      if (m_caster->CanAssistSpell(target, m_spellInfo) &&
          (targetMode != TARGET_UNIT_RAID || target->IsInGroup(m_caster)))
        ...
        if (m_spellInfo->HasAttribute(SPELL_ATTR_EX5_ALLOW_TARGET_OF_TARGET_AS_TARGET)) {
          if (Unit *targetOfUnitTarget = target->GetTarget(m_caster)) {
            if (m_caster->CanAssistSpell(targetOfUnitTarget, m_spellInfo) &&
                (targetMode != TARGET_UNIT_RAID || targetOfUnitTarget->IsInGroup(m_caster)))
  ...
  case TARGET_ENUM_UNITS_FRIEND_AOE_AT_DEST_LOC:
    ...
    break;
  // TARGET_UNIT_PARTY means that the spells can only be casted on a party member and not on the caster (some seals,
  // fire shield from imp, etc..)
  case TARGET_UNIT_PARTY: {
    ...
    if (target && target != m_caster) {
      ...
      if (pGroup) {
        ...
        if (targetOwner) {
          if (targetOwner->GetTypeId() == TYPEID_PLAYER && target->GetTypeId() == TYPEID_UNIT &&
              (((Creature *)target)->IsPet()) && target->GetOwnerGuid() == targetOwner->GetObjectGuid() &&
  ...
  case TARGET_ENUM_UNITS_SCRIPT_IN_CONE_60: {
    ...
    case TARGET_ENUM_UNITS_ENEMY_IN_CONE_24:
    case TARGET_ENUM_UNITS_ENEMY_IN_CONE_54:
      targetType = SPELL_TARGETS_AOE_ATTACKABLE;
      break;
    case TARGET_ENUM_UNITS_FRIEND_IN_CONE:
      targetType = SPELL_TARGETS_ASSISTABLE;
      break;
    }
    ...
    case TARGET_ENUM_UNITS_SCRIPT_IN_CONE_60: {
      ...
      SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> bounds =
          sSpellScriptTargetStorage.getBounds<SpellTargetEntry>(m_spellInfo->Id);
      // fill real target list if no spell script target defined
      FillAreaTargets(bounds.first != bounds.second ? tempTargetUnitMap : tempUnitList, radius, cone, PUSH_CONE,
                      targetType);
      if (!tempTargetUnitMap.empty())
        CheckSpellScriptTargets(bounds, tempTargetUnitMap, tempUnitList, effIndex);
      break;
  ...
  case TARGET_UNIT_FRIEND_AND_PARTY: {
    ...
    if (pGroup) {
      ...
      for (GroupReference *itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next()) {
        ...
        if (groupTarget && groupTarget->GetSubGroup() == subgroup &&
            m_caster->CanAssistSpell(groupTarget, m_spellInfo)) {
      ...
      for (GroupReference *itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next()) {
        ...
        if (target) {
          if (target->GetDistance(x, y, z, DIST_CALC_COMBAT_REACH) <= radius &&
              targetPlayer->getClass() == target->getClass() && m_caster->CanAssistSpell(target, m_spellInfo)) {
            tempUnitList.push_back(target);
          }
          if (Pet *pet = target->GetPet()) {
            if (pet->GetDistance(x, y, z, DIST_CALC_COMBAT_REACH) <= radius &&
                targetPlayer->getClass() == pet->getClass() && m_caster->CanAssistSpell(pet, m_spellInfo))
              tempUnitList.push_back(pet);
          }
        }
      }
  ...
  case TARGET_LOCATION_SCRIPT_NEAR_CASTER: {
    SpellCastResult result = CheckScriptTargeting(effIndex, targetingData.chainTargetCount[effIndex], radius,
                                                  targetMode, tempUnitList, tempGOList);
...
SpellCastResult Spell::CheckScriptTargeting(SpellEffectIndex effIndex, uint32 chainTargets, float radius,
                                            uint32 targetMode, UnitList &tempUnitList, GameObjectList &tempGOList) {
  SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> bounds =
      sSpellScriptTargetStorage.getBounds<SpellTargetEntry>(m_spellInfo->Id);

  if (bounds.first == bounds.second) {
    if (m_spellInfo->EffectImplicitTargetA[effIndex] == TARGET_UNIT_SCRIPT_NEAR_CASTER ||
        m_spellInfo->EffectImplicitTargetB[effIndex] == TARGET_UNIT_SCRIPT_NEAR_CASTER)
      sLog.outErrorDb("Spell entry %u, effect %i has EffectImplicitTargetA/EffectImplicitTargetB = "
                      "TARGET_UNIT_SCRIPT_NEAR_CASTER, but creature are not defined in `spell_script_target`",
                      m_spellInfo->Id, effIndex);

    if (m_spellInfo->EffectImplicitTargetA[effIndex] == TARGET_LOCATION_SCRIPT_NEAR_CASTER ||
        m_spellInfo->EffectImplicitTargetB[effIndex] == TARGET_LOCATION_SCRIPT_NEAR_CASTER)
      sLog.outErrorDb(
          "Spell entry %u, effect %i has EffectImplicitTargetA/EffectImplicitTargetB = "
          "TARGET_LOCATION_SCRIPT_NEAR_CASTER, but gameobject or creature are not defined in `spell_script_target`",
          m_spellInfo->Id, effIndex);

    if (m_spellInfo->EffectImplicitTargetA[effIndex] == TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER ||
        m_spellInfo->EffectImplicitTargetB[effIndex] == TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER)
      sLog.outErrorDb("Spell entry %u, effect %i has EffectImplicitTargetA/EffectImplicitTargetB = "
                      "TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER, but gameobject are not defined in `spell_script_target`",
                      m_spellInfo->Id, effIndex);
  ...
  case SPELL_TARGET_TYPE_GAMEOBJECT_GUID: {
    if (type == SPELL_TARGET_TYPE_GAMEOBJECT_GUID) {
      ...
      MaNGOS::GameObjectListSearcher<MaNGOS::AllGameObjectEntriesListInObjectRangeCheck> checker(foundScriptGOTargets,
                                                                                                 go_check);
      Cell::VisitGridObjects(m_caster, checker, radius);
    }
  ...
  case SPELL_TARGET_TYPE_CREATURE_GUID: {
    if (Unit *target = m_targets.getUnitTarget()) {
      if (target->GetTypeId() == TYPEID_UNIT &&
          entriesToUse.find(type == SPELL_TARGET_TYPE_CREATURE_GUID ? target->GetGUIDLow() : target->GetEntry()) !=
              entriesToUse.end()) {
    ...
    if (foundScriptCreatureTargets.size() < targetCount) {
      if (radius == 50000.f) // only guid scripting
      {
        ...
        MaNGOS::AllCreatureEntriesWithLiveStateInObjectRangeCheck u_check(
            *caster, entriesToUse, type != SPELL_TARGET_TYPE_DEAD, radius, type == SPELL_TARGET_TYPE_CREATURE_GUID,
            false, true);
        MaNGOS::CreatureListSearcher<MaNGOS::AllCreatureEntriesWithLiveStateInObjectRangeCheck> searcher(
            foundScriptCreatureTargets, u_check);
  ...
  if (!foundScriptCreatureTargets.empty()) {
    ...
    if (targetMode == TARGET_LOCATION_SCRIPT_NEAR_CASTER) {
      ...
      m_targets.setDestination(creatureScriptTarget->GetPositionX(), creatureScriptTarget->GetPositionY(),
                               creatureScriptTarget->GetPositionZ());
    }
    ...
    if (targetMode == TARGET_LOCATION_SCRIPT_NEAR_CASTER) {
      ...
      m_targets.setDestination(goScriptTarget->GetPositionX(), goScriptTarget->GetPositionY(),
                               goScriptTarget->GetPositionZ());
    }
...
void Spell::CheckSpellScriptTargets(SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> &bounds,
                                    UnitList &tempTargetUnitMap, UnitList &targetUnitMap, SpellEffectIndex effIndex) {
  for (UnitList::const_iterator iter = tempTargetUnitMap.begin(); iter != tempTargetUnitMap.end(); ++iter) {
    ...
    for (SQLMultiStorage::SQLMultiSIterator<SpellTargetEntry> i_spellST = bounds.first; i_spellST != bounds.second;
      ...
      if (spellST->type == SPELL_TARGET_TYPE_CREATURE_GUID ? target->GetGUIDLow() == spellST->targetEntry
                                                           : target->GetEntry() == spellST->targetEntry) {
        switch (spellST->type) {
        case SPELL_TARGET_TYPE_DEAD:
...
SpellCastResult Spell::PreCastCheck(Aura *triggeredByAura /*= nullptr*/) {
  ...
  if (result != SPELL_CAST_OK &&
      (!IsAutoRepeat() || m_triggerAutorepeat)) // always cast autorepeat dummy for triggering
  {
    if (triggeredByAura) {
      SendChannelUpdate(0);
...
void Spell::cancel() {
  ...
  // channeled spells don't display interrupted message even if they are interrupted, possible other cases with no
  // "Interrupted" message
...
void Spell::cast(bool skipCheck) {
  ...
  if (!m_caster->CheckAndIncreaseCastCounter()) {
    if (m_triggeredByAuraSpell)
      sLog.outError("Spell %u triggered by aura spell %u too deep in cast chain for cast. Cast not allowed for prevent "
                    "overflow stack crash.",
  ...
  if (m_caster->GetTypeId() != TYPEID_PLAYER && m_targets.getUnitTarget() && m_targets.getUnitTarget() != m_caster &&
      !m_spellInfo->HasAttribute(SPELL_ATTR_EX5_DONT_TURN_DURING_CAST)) {
    Unit *charmer = m_caster->GetCharmer();
    if (charmer && !(charmer->GetTypeId() == TYPEID_PLAYER &&
                     ((Player *)charmer)->GetCamera().GetBody() ==
                         m_caster)) // need to check if target doesnt have a player controlling it
...
uint64 Spell::handle_delayed(uint64 t_offset) {
  ...
  // now recheck units targeting correctness (need before any effects apply to prevent adding immunity at first effect
  // not allow apply second spell effect and similar cases)
  for (auto &ihit : m_UniqueTargetInfo) {
    if (!ihit.processed) {
...
void Spell::_handle_immediate_phase() {
  ...
  // start channeling if applicable (after _handle_immediate_phase for get persistent effect dynamic object for channel
  // target
...
void Spell::update(uint32 difftime) {
  ...
  if (((m_caster->GetTypeId() == TYPEID_PLAYER || m_caster->GetTypeId() == TYPEID_UNIT) && m_timer != 0) &&
      (m_castPositionX != m_caster->GetPositionX() || m_castPositionY != m_caster->GetPositionY() ||
       m_castPositionZ != m_caster->GetPositionZ()) &&
      (m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK ||
       !m_caster->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLINGFAR))) {
    // always cancel for channeled spells
    if (m_spellState == SPELL_STATE_CHANNELING) {
      if (m_spellInfo->ChannelInterruptFlags & CHANNEL_FLAG_MOVEMENT &&
          !m_spellInfo->HasAttribute(SPELL_ATTR_EX5_CAN_CHANNEL_WHEN_MOVING))
        cancel();
    }
    // don't cancel for melee, autorepeat, triggered and instant spells
    else if (!IsNextMeleeSwingSpell(m_spellInfo) && !IsAutoRepeat() && !m_IsTriggeredSpell &&
    ...
    if (m_timer > 0) {
      if (m_caster->GetTypeId() == TYPEID_PLAYER || m_caster->GetTypeId() == TYPEID_UNIT) {
        ...
        if (m_spellInfo->HasAttribute(SPELL_ATTR_EX_CHANNEL_TRACK_TARGET) &&
            m_UniqueTargetInfo.begin() != m_UniqueTargetInfo.end()) {
          if (Unit *target = m_caster->GetChannelObject()) {
            if (target != m_caster) {
        ...
        if (m_spellInfo->ChannelInterruptFlags & CHANNEL_FLAG_TURNING) {
          ...
          case TYPEID_PLAYER: {
            ...
            if (m_spellInfo->HasAttribute(
                    SPELL_ATTR_EX_CHANNEL_TRACK_TARGET)) // On start, client turns player to face object, need leeway so
                                                         // that we dont break unnecessarily
              targetOrientationDiff += M_PI_F * (std::max(1200 - (m_duration - int32(m_timer)), 0) / 1200.f);
            if ((M_PI_F - std::abs(std::abs(m_castOrientation - m_caster->GetOrientation()) - M_PI_F)) >
                targetOrientationDiff)
              cancel();
          }
          }
        }
    ...
    if (m_timer == 0) {
      ...
      if (!IsAutoRepeat() && !IsNextMeleeSwingSpell(m_spellInfo)) {
        if (Player *p = m_caster->GetBeneficiaryPlayer()) {
          ...
          for (GOTargetList::const_iterator ihit = m_UniqueGOTargetInfo.begin(); ihit != m_UniqueGOTargetInfo.end();
               ++ihit) {
...
void Spell::finish(bool ok) {
  ...
  if (Player *modOwner = m_caster->GetSpellModOwner()) {
    if (ok || (m_spellState >
               uint32(SPELL_STATE_CASTING))) // fail after start channeling or throw to target not affect spell mods
  ...
  // Normal spells proc on finish, channeled spells proc on start when they have duration, thats when channeledChannel
  // is true
...
void Spell::SendCastResult(Player const *caster, SpellEntry const *spellInfo, uint8 cast_count, SpellCastResult result,
                           bool isPetCastResult /*=false*/) {
  if (result == SPELL_CAST_OK)
    return;

  WorldPacket data(isPetCastResult ? SMSG_PET_CAST_FAILED : SMSG_CAST_RESULT,
                   (4 + 1 + 2)); // single cast or multi 2.3 (0/1)
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    for (uint8 i = 0; i < MAX_VIRTUAL_ITEM_SLOT; ++i) {
      ...
      if (uint32 item_class =
              m_caster->GetByteValue(UNIT_VIRTUAL_ITEM_INFO + (i * 2) + 0, VIRTUAL_ITEM_INFO_0_OFFSET_CLASS)) {
        if (item_class == ITEM_CLASS_WEAPON) {
          ...
          case ITEM_SUBCLASS_WEAPON_THROWN:
            ...
            ammoInventoryType =
                m_caster->GetByteValue(UNIT_VIRTUAL_ITEM_INFO + (i * 2) + 1, VIRTUAL_ITEM_INFO_1_OFFSET_INVENTORYTYPE);
            break;
...
void Spell::WriteSpellGoTargets(WorldPacket &data) {
  ...
  if (m_UniqueTargetInfo.size() > 255) {
    sLog.outError("Spell ID %u cast by %s hit/missed too many unit targets %u. Ignored after 255.", m_spellInfo->Id,
                  m_caster->GetObjectGuid().GetString().c_str(), (uint32)m_UniqueTargetInfo.size());
    m_UniqueTargetInfo.resize(255);
  }

  if (m_UniqueGOTargetInfo.size() > 255) {
    sLog.outError("Spell ID %u cast by %s hit/missed too many GO targets %u. Ignored after 255.", m_spellInfo->Id,
                  m_caster->GetObjectGuid().GetString().c_str(), (uint32)m_UniqueGOTargetInfo.size());
    m_UniqueGOTargetInfo.resize(255);
  }

  if (m_UniqueGOTargetInfo.size() + m_UniqueTargetInfo.size() > 255) {
    sLog.outError("Spell ID %u cast by %s hit/missed too many targets %u. Ignored after 255.", m_spellInfo->Id,
                  m_caster->GetObjectGuid().GetString().c_str(),
                  (uint32)(m_UniqueGOTargetInfo.size() + m_UniqueTargetInfo.size()));
  ...
  for (auto &ihit : m_UniqueTargetInfo) {
    ...
    } else if (ihit.missCondition == SPELL_MISS_NONE) // Add only hits
    {
      ...
      if (IsChanneledSpell(m_spellInfo) &&
          (ihit.missCondition == SPELL_MISS_RESIST || ihit.missCondition == SPELL_MISS_REFLECT))
        m_duration = 0; // cancel aura to avoid visual effect continue
      ++miss;
    }
  }
  ...
  if (time == 0) {
    ...
    if (stackable)
      ...
      m_caster->RemoveAurasByCasterSpell(m_spellInfo->Id, m_caster->GetObjectGuid(),
                                         m_timer == 0 ? AURA_REMOVE_BY_EXPIRE : AURA_REMOVE_BY_CANCEL);
...
void Spell::SendChannelStart(uint32 duration) {
  ...
  // select first not resisted target from target list for _0_ effect
    ...
    for (TargetList::const_iterator itr = m_UniqueTargetInfo.begin(); itr != m_UniqueTargetInfo.end(); ++itr) {
      if (((itr->effectHitMask & (1 << EFFECT_INDEX_0)) && itr->reflectResult == SPELL_MISS_NONE && m_CastItem) ||
...
void Spell::HandleThreatSpells() {
  ...
  if (m_negativeEffectMask & effectMask) {
    // can only handle spells with clearly defined positive/negative effect, check at spell_threat loading probably not
    // perfect so abort when only some effects are negative.
    if ((m_negativeEffectMask & effectMask) != effectMask) {
      DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST,
                       "Spell %u, rank %u, is not clearly positive or negative, ignoring bonus threat", m_spellInfo->Id,
                       sSpellMgr.GetSpellRank(m_spellInfo->Id));
      return;
    }
    positive = false;
  }

  // since 2.0.1 threat from positive effects also is distributed among all targets, so the overall caused threat is at
  // most the defined bonus
  ...
  for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit) {
    ...
    Unit *target =
        m_caster->GetObjectGuid() == ihit->targetGUID ? m_caster : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID);
    if (!target)
      continue;
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "Spell %u added an additional %f threat for %s " SIZEFMTD " target(s)",
                   m_spellInfo->Id, threat, positive ? "assisting" : "harming", m_UniqueTargetInfo.size());
}
...
void Spell::HandleEffect(Unit *pUnitTarget, Item *pItemTarget, GameObject *pGOTarget, SpellEffectIndex i,
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "Spell %u Effect%d : %u Targets: %s, %s, %s", m_spellInfo->Id, i, eff,
...
void Spell::AddTriggeredSpell(uint32 spellId) {
  ...
  if (!spellInfo) {
    sLog.outError("Spell::AddTriggeredSpell: unknown spell id %u used as triggred spell for spell %u)", spellId,
                  m_spellInfo->Id);
    return;
  }
...
void Spell::AddPrecastSpell(uint32 spellId) {
  ...
  if (!spellInfo) {
    sLog.outError("Spell::AddPrecastSpell: unknown spell id %u used as pre-cast spell for spell %u)", spellId,
                  m_spellInfo->Id);
    return;
  }
...
SpellCastResult Spell::CheckCast(bool strict) {
  ...
  if (!m_caster->IsAlive() && m_caster->GetTypeId() == TYPEID_PLAYER &&
      !m_spellInfo->HasAttribute(SPELL_ATTR_CASTABLE_WHILE_DEAD) && !m_spellInfo->HasAttribute(SPELL_ATTR_PASSIVE))
    return SPELL_FAILED_CASTER_DEAD;

  if (!m_IsTriggeredSpell && !m_caster->IsStandState() &&
      m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER && !((Player *)m_caster)->IsGameMaster() &&
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR_OUTDOORS_ONLY) &&
        !m_caster->GetTerrain()->IsOutdoors(m_caster->GetPositionX(), m_caster->GetPositionY(),
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    if (((Player *)m_caster)->IsMoving()) {
      ...
      if ((!((Player *)m_caster)->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLINGFAR) ||
           m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK) &&
  ...
  if (!m_caster->IsInCombat() && m_spellInfo->HasAttribute(SPELL_ATTR_STOP_ATTACK_TARGET) &&
      m_spellInfo->HasAttribute(SPELL_ATTR_EX2_UNK26))
    return SPELL_FAILED_CASTER_AURASTATE;
  ...
  if (target) {
    // TARGET_UNIT_SCRIPT_NEAR_CASTER fills unit target per client requirement but should not be checked against common
    // things
    ...
    if (!IsSpellWithScriptUnitTarget(m_spellInfo)) {
      ...
      if (m_spellInfo->HasAttribute(SPELL_ATTR_EX_CHANNEL_TRACK_TARGET) // TODO: Investigate this condition
          && m_spellInfo->HasAttribute(SPELL_ATTR_EX5_HASTE_AFFECT_DURATION) && target->GetTypeId() == TYPEID_UNIT &&
      ...
      if (non_caster_target) {
        ...
        if (m_spellInfo->TargetAuraState &&
            !target->HasAuraStateForCaster(AuraState(m_spellInfo->TargetAuraState), m_caster->GetObjectGuid()))
          return SPELL_FAILED_TARGET_AURASTATE;
        ...
        if (!IsIgnoreLosSpellCast(m_spellInfo) && !m_IsTriggeredSpell &&
            VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_caster->IsWithinLOSInMap(target, true))
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER) {
          ...
          if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_CANT_TARGET_TAPPED) && !IsAreaOfEffectSpell(m_spellInfo))
            if (Creature const *targetCreature = dynamic_cast<Creature *>(target))
              if ((!targetCreature->GetLootRecipientGuid().IsEmpty()) &&
                  !targetCreature->IsTappedBy((Player *)m_caster))
                return SPELL_FAILED_CANT_CAST_ON_TAPPED;
        }

        if (strict && m_spellInfo->HasAttribute(SPELL_ATTR_EX3_TARGET_ONLY_PLAYER) &&
            target->GetTypeId() != TYPEID_PLAYER && !IsAreaOfEffectSpell(m_spellInfo))
          return SPELL_FAILED_BAD_TARGETS;
      }

      if ((m_targets.m_targetMask == TARGET_FLAG_SELF || m_caster == target) &&
          m_spellInfo->HasAttribute(SPELL_ATTR_EX_CANT_TARGET_SELF))
        return SPELL_FAILED_BAD_TARGETS;
      ...
      if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_FACING_TARGETS_BACK) &&
          m_spellInfo->HasAttribute(SPELL_ATTR_EX_FACING_TARGET) && !m_caster->IsFacingTargetsBack(target)) {
      ...
      if (((m_spellInfo->Attributes == (SPELL_ATTR_ABILITY | SPELL_ATTR_NOT_SHAPESHIFT |
                                        SPELL_ATTR_DONT_AFFECT_SHEATH_STATE | SPELL_ATTR_STOP_ATTACK_TARGET)) &&
           !m_caster->IsFacingTargetsFront(target))
      ...
      if (target->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION) &&
          !m_spellInfo->HasAttribute(SPELL_ATTR_EX3_DEATH_PERSISTENT))
        return SPELL_FAILED_BAD_TARGETS;

      // Check if more powerful spell applied on target (if spell only contains non-aoe auras)
      if (IsAuraApplyEffects(m_spellInfo, EFFECT_MASK_ALL) && !IsAreaOfEffectSpell(m_spellInfo) &&
          !HasAreaAuraEffect(m_spellInfo)) {
  ...
  for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i) {
    ...
    default: // needs target
    {
      ...
      if (data.type == TARGET_TYPE_UNIT && data.filter != TARGET_SCRIPT &&
          (data.enumerator == TARGET_ENUMERATOR_SINGLE || data.enumerator == TARGET_ENUMERATOR_CHAIN)) {
        if (!target)
          return SPELL_FAILED_BAD_TARGETS;
        switch (data.filter) {
        case TARGET_HARMFUL:
          if (!originalCaster->CanAttackSpell(target, m_spellInfo))
            return SPELL_FAILED_BAD_TARGETS;
          break;
        case TARGET_HELPFUL:
          if (!originalCaster->CanAssistSpell(target, m_spellInfo))
            return SPELL_FAILED_BAD_TARGETS;
          break;
        case TARGET_PARTY:
        case TARGET_GROUP:
          if (!m_caster->CanAssistSpell(target, m_spellInfo) ||
              !m_caster->IsInGroup(target, targetType == TARGET_UNIT_PARTY))
            return SPELL_FAILED_BAD_TARGETS;
          break;
        default:
          break;
  ...
  for (int i = 0; i < MAX_EFFECT_INDEX; ++i) {
    ...
    case SPELL_EFFECT_DUMMY: {
      ...
      } else if (m_spellInfo->Id == 51582) // Rocket Boots Engaged
      {
        if (m_caster->IsInWater() &&
            (m_caster->GetTypeId() != TYPEID_PLAYER || static_cast<Player *>(m_caster)->IsInHighLiquid()))
    ...
    case SPELL_EFFECT_CREATE_ITEM: {
      if (Unit *target = m_targets.getUnitTarget()) {
        ...
        uint32 no_space = 0;
        InventoryResult msg = static_cast<Player *>(target)->CanStoreNewItem(
            NULL_BAG, NULL_SLOT, dest, m_spellInfo->EffectItemType[i], count, &no_space);
        if (msg != EQUIP_ERR_OK)
    ...
    case SPELL_EFFECT_TAMECREATURE: {
      ...
      if (!caster || caster->GetTypeId() != TYPEID_PLAYER || !tameTarget || tameTarget->GetTypeId() == TYPEID_PLAYER)
        return SPELL_FAILED_BAD_TARGETS;
    ...
    case SPELL_EFFECT_POWER_DRAIN: {
      // Can be area effect, Check only for players and not check if target - caster (spell can have multiply drain/burn
      // effects)
      if (m_caster->GetTypeId() == TYPEID_PLAYER)
        if (Unit *target = m_targets.getUnitTarget())
    ...
    case SPELL_EFFECT_CHARGE: {
      ...
      if (Unit *target = m_targets.getUnitTarget()) {
        ...
        // (pos.coord_z < m_caster->GetPositionZ()) && (fabs(pos.coord_z - m_caster->GetPositionZ()) < 3.0f))
        //{
    ...
    case SPELL_EFFECT_SKINNING: {
      ...
      if (creature->GetLootStatus() !=
          CREATURE_LOOT_STATUS_LOOTED) // || creature->GetCreatureType() != CREATURE_TYPE_CRITTER)
      ...
      if (!strict && skillValue < sWorld.GetConfigMaxSkillValue() &&
    ...
    case SPELL_EFFECT_OPEN_LOCK: {
      ...
      if (GameObject *go = m_targets.getGOTarget()) {
        ...
        if (((Player *)m_caster)->InBattleGround() && !((Player *)m_caster)->CanUseBattleGroundObject())
          return SPELL_FAILED_TRY_AGAIN;
      ...
      SpellCastResult res = CanOpenLock(effIdx, lockId, m_effectSkillInfo[effIdx].skillId,
                                        m_effectSkillInfo[effIdx].reqSkillValue, m_effectSkillInfo[effIdx].skillValue);
      ...
      if (!strict && m_effectSkillInfo[effIdx].skillId != SKILL_NONE) {
        bool canFailAtMax =
            m_effectSkillInfo[effIdx].skillId != SKILL_HERBALISM && m_effectSkillInfo[effIdx].skillId != SKILL_MINING;

        // chance for failure in orange gather / lockpick (gathering skill can't fail at maxskill)
        if ((canFailAtMax || m_effectSkillInfo[effIdx].skillValue < sWorld.GetConfigMaxSkillValue()) &&
            m_effectSkillInfo[effIdx].reqSkillValue >
                irand(m_effectSkillInfo[effIdx].skillValue - 25, m_effectSkillInfo[effIdx].skillValue + 37))
          return SPELL_FAILED_TRY_AGAIN;
      }
      if (m_CastItem)
    ...
    case SPELL_EFFECT_SUMMON: {
      if (SummonPropertiesEntry const *summon_prop =
              sSummonPropertiesStore.LookupEntry(m_spellInfo->EffectMiscValueB[i])) {
  ...
  for (int i = 0; i < MAX_EFFECT_INDEX; ++i) {
    ...
    case SPELL_AURA_MOD_POSSESS: {
      ...
      if (expectedTarget) // target may not be known at CheckCast time - TODO: add support for these checks on
                          // CheckTarget
      {
    ...
    case SPELL_AURA_MOUNTED: {
      ...
      if (m_caster->GetTypeId() == TYPEID_PLAYER && !m_IsTriggeredSpell && !m_spellInfo->AreaId &&
...
SpellCastResult Spell::CheckPetCast(Unit *target) {
  ...
  if (m_caster->IsNonMeleeSpellCasted(false) &&
      !m_ignoreConcurrentCasts) // prevent spellcast interruption by another spellcast
    return SPELL_FAILED_SPELL_IN_PROGRESS;
  ...
  if (m_caster->GetTypeId() == TYPEID_UNIT && (((Creature *)m_caster)->IsPet() || m_caster->HasCharmer())) {
    ...
    for (unsigned int i : m_spellInfo->EffectImplicitTargetA) {
      if (i == TARGET_UNIT_ENEMY || i == TARGET_UNIT_FRIEND || i == TARGET_UNIT_RAID || i == TARGET_UNIT ||
          i == TARGET_UNIT_PARTY || i == TARGET_LOCATION_CASTER_TARGET_POSITION) {
        need = true;
        if (!target)
          return SPELL_FAILED_BAD_IMPLICIT_TARGETS;
        break;
...
SpellCastResult Spell::CheckCasterAuras() const {
  ...
  if (unitflag & UNIT_FLAG_STUNNED) {
    // spell is usable while stunned, check if caster has only mechanic stun auras, another stun types must prevent cast
    // spell
  ...
  if (prevented_reason != SPELL_CAST_OK) {
    if (school_immune || mechanic_immune || dispel_immune) {
      ...
      for (const auto &itr : auras) {
        ...
        if ((GetSpellSchoolMask(pEntry) & school_immune) &&
            !pEntry->HasAttribute(SPELL_ATTR_EX_UNAFFECTED_BY_SCHOOL_IMMUNE))
          continue;
...
std::pair<float, float> Spell::GetMinMaxRange(bool strict) {
  ...
  if (spellRange) {
    ...
    if (target && caster && caster->IsMovingForward() && target->IsMovingForward() && !caster->IsWalking() &&
        !target->IsWalking() && ((spellRange->Flags & SPELL_RANGE_FLAG_MELEE) || target->GetTypeId() == TYPEID_PLAYER))
...
SpellCastResult Spell::CheckRange(bool strict) {
  ...
  if (m_targets.m_targetMask == TARGET_FLAG_DEST_LOCATION) {
    float dist =
        m_caster->GetDistance(m_targets.m_destPos.x, m_targets.m_destPos.y, m_targets.m_destPos.z, DIST_CALC_NONE);
    if (dist > maxRange * maxRange)
      return SPELL_FAILED_OUT_OF_RANGE;
    if (minRange && dist < minRange * minRange)
      return SPELL_FAILED_TOO_CLOSE;
  ...
  if (m_targets.m_targetMask == TARGET_FLAG_SOURCE_LOCATION) {
    float dist =
        m_caster->GetDistance(m_targets.m_srcPos.x, m_targets.m_srcPos.y, m_targets.m_srcPos.z, DIST_CALC_NONE);
    if (dist > maxRange * maxRange)
      return SPELL_FAILED_OUT_OF_RANGE;
    if (minRange && dist < minRange * minRange)
      return SPELL_FAILED_TOO_CLOSE;
...
int32 Spell::CalculateSpellEffectDamage(Unit *unitTarget, int32 damage) {
  ...
  case SPELL_DAMAGE_CLASS_MELEE: {
    ...
    damage = m_caster->MeleeDamageBonusDone(unitTarget, damage, m_attackType, m_spellSchoolMask, m_spellInfo,
                                            SPELL_DIRECT_DAMAGE);
    damage = unitTarget->MeleeDamageBonusTaken(m_caster, damage, m_attackType, m_spellSchoolMask, m_spellInfo,
                                               SPELL_DIRECT_DAMAGE);
  } break;
  // Magical Attacks
...
uint32 Spell::CalculatePowerCost(SpellEntry const *spellInfo, Unit *caster, Spell *spell, Item *castItem,
  ...
  int32 powerCost = spellInfo->manaCost +
                    spellInfo->manaCostPerlevel * (int32(caster->GetSpellRank(spellInfo)) / 5 - spellInfo->baseLevel);
...
SpellCastResult Spell::CheckItems() {
  ...
  if (m_CastItem) {
    ...
    if (proto->Class == ITEM_CLASS_CONSUMABLE) {
      ...
      for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i) {
        // skip check, pet not required like checks, and for TARGET_UNIT_CASTER_PET m_targets.getUnitTarget() is not the
        // real target but the caster
        if (m_spellInfo->EffectImplicitTargetA[i] == TARGET_UNIT_CASTER_PET)
          continue;

        Unit *target =
            m_spellInfo->EffectImplicitTargetA[i] == TARGET_UNIT_CASTER ? m_caster : m_targets.getUnitTarget();
        if (!target)
          continue;
  ...
  if (m_targets.getItemTargetGuid()) {
    if (m_caster->GetTypeId() != TYPEID_PLAYER)
      return m_IsTriggeredSpell && !(m_targets.m_targetMask & TARGET_FLAG_TRADE_ITEM) ? SPELL_FAILED_DONT_REPORT
  ...
  for (int i = 0; i < MAX_EFFECT_INDEX; ++i) {
    ...
    case SPELL_EFFECT_CREATE_ITEM: {
      ...
      if (!m_IsTriggeredSpell && m_spellInfo->EffectItemType[i]) {
        ...
        InventoryResult msg =
            playerTarget->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, m_spellInfo->EffectItemType[i], count);
        if (msg != EQUIP_ERR_OK) {
...
void Spell::Delayed() {
  ...
  DETAIL_FILTER_LOG(LOG_FILTER_SPELL_CAST, "Spell %u partially interrupted for (%d) ms at damage", m_spellInfo->Id,
                    delaytime);
...
void Spell::DelayedChannel() {
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "Spell %u partially interrupted for %i ms, new duration: %u ms",
                   m_spellInfo->Id, delaytime, m_timer);

  for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit) {
    if ((*ihit).missCondition == SPELL_MISS_NONE) {
      if (Unit *unit = m_caster->GetObjectGuid() == ihit->targetGUID
                           ? m_caster
                           : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID))
...
bool Spell::CheckTarget(Unit *target, SpellEffectIndex eff, bool targetB, CheckException exception) const {
  // Check targets for creature type mask and remove not appropriate (skip explicit self target case, maybe need other
  // explicit targets)
  ...
  if (!targetB)
    targetType = m_spellInfo->EffectImplicitTargetA[eff],
    info = SpellTargetInfoTable[m_spellInfo->EffectImplicitTargetA[eff]];
  else
    targetType = m_spellInfo->EffectImplicitTargetB[eff],
    info = SpellTargetInfoTable[m_spellInfo->EffectImplicitTargetB[eff]];
  ...
  if (target != realCaster) {
    ...
    if (target->GetMasterGuid() != realCaster->GetObjectGuid() && !scriptTarget) {
      ...
      if (!IsSpellCanTargetUnattackable(m_spellInfo) && !m_ignoreUnattackableTarget &&
  ...
  if (target->IsCreature() && target != m_targets.getUnitTarget() && info.enumerator == TARGET_ENUMERATOR_CHAIN &&
      info.filter == TARGET_HARMFUL) // TODO: Mother Shahraz beams can target totems
    if (static_cast<Creature *>(target)->IsCritter())
      return false;

  // If spell have ingore CC attr & unit is CC
  if (m_spellInfo->AttributesEx6 & SPELL_ATTR_EX6_IGNORE_CC_TARGETS && target != m_targets.getUnitTarget() &&
...
bool Spell::IsNeedSendToClient() const {
  return m_spellInfo->SpellVisual != 0 || IsChanneledSpell(m_spellInfo) || m_spellInfo->speed > 0.0f ||
...
SpellCastResult Spell::CanOpenLock(SpellEffectIndex effIndex, uint32 lockId, SkillType &skillId, int32 &reqSkillValue,
                                   int32 &skillValue) {
  ...
  for (int j = 0; j < 8; ++j) {
    ...
    case LOCK_KEY_SKILL: {
      ...
      bool oldCalc = true;
      if (tempSkillId ==
          SKILL_NONE) // these must not be carried over to the reference variable - unless more research comes up
      {
      ...
      if (tempSkillId != SKILL_NONE) {
        ...
        if (oldCalc) // still correct but not usable for spell skill ids
        {
          ...
          // add the damage modifier from the spell casted (cheat lock / skeleton key etc.) (use m_currentBasePoints,
          // CalculateDamage returns wrong value)
          ...
          skillValue = m_CastItem || m_caster->GetTypeId() != TYPEID_PLAYER
                           ? 0
                           : ((Player *)m_caster)->GetSkillValue(tempSkillId);
 ...
 * @param spellTargets         Additional rules for target selection base at hostile/friendly state to original spell
 caster
 * @param originalCaster       If provided set alternative original caster, if =nullptr then used
 Spell::GetAffectiveObject() return
 */
void Spell::FillAreaTargets(UnitList &targetUnitMap, float radius, float cone, SpellNotifyPushType pushType,
                            SpellTargets spellTargets, WorldObject *originalCaster /*=nullptr*/) {
  MaNGOS::SpellNotifierCreatureAndPlayer notifier(*this, targetUnitMap, radius, cone, pushType, spellTargets,
                                                  originalCaster);
  Cell::VisitAllObjects(notifier.GetCenterX(), notifier.GetCenterY(), m_caster->GetMap(), notifier, radius);
}

void Spell::FillRaidOrPartyTargets(UnitList &targetUnitMap, Unit *member, float radius, bool raid, bool withPets,
                                   bool withcaster) const {
  ...
  if (pGroup) {
    ...
    for (GroupReference *itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next()) {
      ...
      // CanAssist check duel and controlled by enemy
      if (Target && (raid || subgroup == Target->GetSubGroup()) && m_caster->CanAssistSpell(Target, m_spellInfo)) {
...
float Spell::GetSpellSpeed() const {
  if (m_trueCaster->IsGameObject()) // 4 spells in all of wotlk and doesnt seem like GO casting supports travelling
                                    // delay from sniffs
    return 0.f;
  if (IsChanneledSpell(m_spellInfo))
    return 0.f;
...
bool Spell::CanExecuteTriggersOnHit(uint8 effMask, SpellEntry const *triggeredByAura) const {
  ...
  // If triggeredByAura has SPELL_ATTR4_PROC_ONLY_ON_CASTER then it can only proc on a cast spell with
  // TARGET_UNIT_CASTER
  for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
...
void Spell::SelectMountByAreaAndSkill(Unit *target, SpellEntry const *parentSpell, uint32 spellId75, uint32 spellId150,
  ...
  if (skillval >= 225 && (spellId300 > 0 || spellId225 > 0)) {
    ...
    if (!pSpell) {
      sLog.outError("SelectMountByAreaAndSkill: unknown spell id %i by caster: %s", spellid,
    ...
    SpellCastResult locRes = sSpellMgr.GetSpellAllowedInLocationError(pSpell, target->GetMapId(), zone, area,
                                                                      target->GetBeneficiaryPlayer());
    if (locRes != SPELL_CAST_OK)
      ...
      for (PlayerSpellMap::const_iterator iter = ((Player *)target)->GetSpellMap().begin();
...
void Spell::RegisterAuraProc(Aura *aura) { m_procOnceHolder.insert(aura); }

bool Spell::IsAuraProcced(Aura *aura) { return m_procOnceHolder.find(aura) != m_procOnceHolder.end(); }
...
void Spell::GetSpellRangeAndRadius(SpellEffectIndex effIndex, float &radius, bool targetB, uint32 &EffectChainTarget) {
  ...
  if (radius == 50000.f) // safety against bad data
  {
    if (data.filter != TARGET_SCRIPT) {
      ...
      case TARGET_TYPE_UNIT:
      case TARGET_TYPE_GAMEOBJECT:
        if (data.enumerator == TARGET_ENUMERATOR_CHAIN || data.enumerator == TARGET_ENUMERATOR_AOE ||
            data.enumerator == TARGET_ENUMERATOR_CONE)
          radius = 200.f;
        break;
      default:
        break;
    ...
    case TARGET_LOCATION_CASTER_LEFT:
    case TARGET_LOCATION_CASTER_RIGHT:
      if (radius == 0.f && m_spellInfo->EffectRadiusIndex[effIndex] !=
                               36) // All shaman totems have 0 radius - need to override with proper value
        radius = 2.f;
      break;
    }
  }
...
void Spell::ProcReflectProcs(TargetInfo &targetInfo) {
  Unit *target = m_caster->GetObjectGuid() == targetInfo.targetGUID
                     ? m_caster
  ...
  Unit::ProcDamageAndSpell(ProcSystemArguments(m_caster, target, PROC_FLAG_NONE,
                                               PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1,
                                               BASE_ATTACK, m_spellInfo));
  if (targetInfo.reflectResult == SPELL_MISS_REFLECT)
    // Apply reflect procs on self
    Unit::ProcDamageAndSpell(ProcSystemArguments(m_caster, m_caster, PROC_FLAG_NONE,
                                                 PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1,
                                                 BASE_ATTACK, m_spellInfo));
}
...
void Spell::FillFromTargetFlags(TempTargetingData &targetingData, SpellEffectIndex effIdx) {
  if (m_spellInfo->Targets & (TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_ENEMY)) {
    ...
    if ((m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION) == 0) {
      sLog.outError("No destination for spell with flag TARGET_FLAG_DEST_LOCATION, spell ID: %u",
                    m_spellInfo->Id); // should never occur
      if (WorldObject *caster = GetCastingObject())
        m_targets.setDestination(caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ());
    }
  }
...
SpellCastResult Spell::OnCheckCast(bool strict) {
  ...
  case 7914: // Capture Spirit
  {
    if (ObjectGuid target = m_targets.getUnitTargetGuid()) // can be cast only on these targets
      if (target.GetEntry() != 4663 && target.GetEntry() != 4664 && target.GetEntry() != 4665 &&
          target.GetEntry() != 4666 && target.GetEntry() != 4667 && target.GetEntry() != 4668 &&
          target.GetEntry() != 4705 && target.GetEntry() != 13019)
        return SPELL_FAILED_BAD_TARGETS;
    break;
  }
...
bool Spell::OnCheckTarget(Unit *target, SpellEffectIndex eff) const {
  ...
  case 9082: // Create Containment Coffer
    if (!target->HasAura(9032))
      return false;
    break;
  case 14297: // Shadow Storm (exclude targets below 25 yards and above 40 yards. Max value is handled by spell effect
              // radius)
  case 26546:
  case 26555: {
    float x, y, z;
  ...
  case 16807:                                                           // Mass Teleport
    ...
    // as it is not possible to filter out targets based on their class from data in spells template (like spell family
    // masks for example)
  case 23397: // Berserk
  ...
  case 30835: // Infernal Relay - Malchezaar - must only hit unused infernals
  {
    ...
    MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(creature,
                                                                                                       creature_check);
  ...
  case 37433: // Spout (The Lurker Below), only players affected if its not in water
    if (target->IsInWater() &&
        (target->GetTypeId() != TYPEID_PLAYER || static_cast<Player *>(target)->IsInHighLiquid()))
      return false;
    break;
  ...
  case 37895:
    if (target->HasAura(
            m_spellInfo->Id)) // scorp already possessed cannot be repossessed by a diff player TODO: Check on retail
      return false;
    break;
  ...
  case 38782: // Druid Signal
    if (target->HasAura(38776) ||
        target->HasAura(39158)) // Don't target druids that have already been summoned by another player
      return false;
    break;
  ...
  {
    float dist = sqrt(target->GetDistance(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(),
                                          DIST_CALC_NONE));
  ...
  {
    if (target->GetTypeId() != TYPEID_UNIT || target->GetEntry() != 23040 || m_caster->GetTypeId() != TYPEID_UNIT ||
        m_caster->GetEntry() != 23040)
      return false;
    ...
    for (auto &creature : creatureList) {
      ...
      if (creature->AI() && target->AI() && creature->AI()->GetScriptData() == target->AI()->GetScriptData())
