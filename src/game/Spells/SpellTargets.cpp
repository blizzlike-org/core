...
SpellTargetInfo::SpellTargetInfo(char const *name, SpellTargetImplicitType type, SpellTargetFilter filter,
...
SpellTargetInfo SpellTargetInfoTable[MAX_SPELL_TARGETS] = {
    ...
    /*[51]*/
    {"TARGET_ENUM_GAMEOBJECTS_SCRIPT_AOE_AT_SRC_LOC", TARGET_TYPE_GAMEOBJECT, TARGET_SCRIPT, TARGET_ENUMERATOR_AOE},
    /*[52]*/
    {"TARGET_ENUM_GAMEOBJECTS_SCRIPT_AOE_AT_DEST_LOC", TARGET_TYPE_GAMEOBJECT, TARGET_SCRIPT, TARGET_ENUMERATOR_AOE},
...
SpellEffectInfo SpellEffectInfoTable[MAX_SPELL_EFFECTS] = {
    ...
    /*[7]*/ {"SPELL_EFFECT_ENVIRONMENTAL_DAMAGE", TARGET_TYPE_NONE, TARGET_NONE}, // none is a hack - should be unit -
                                                                                  // GO casting
    ...
    /*[152]*/ {"SPELL_EFFECT_SUMMON_RAF_FRIEND", TARGET_TYPE_NONE, TARGET_NONE}, // TODO: Investigate when implementing
                                                                                 // RAF
...
bool CheckBoundsEqualForEffects(SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> &bounds,
                                SpellEffectIndex effIdxSource, SpellEffectIndex effIdxTarget) {
  ...
  for (uint32 i = 0; i <= sSpellTemplate.GetMaxEntry(); ++i) {
    ...
    for (uint32 effIdx = 0; effIdx < MAX_EFFECT_INDEX; ++effIdx) {
      ...
      if (SpellEffectInfoTable[effect].requiredTarget != TARGET_TYPE_DYNAMIC &&
          SpellEffectInfoTable[effect].requiredTarget != TARGET_TYPE_DYNAMIC_ANY)
        data.implicitType[effIdx] = SpellEffectInfoTable[effect].requiredTarget;
      else {
        if (SpellTargetInfoTable[targetA].type == TARGET_TYPE_LOCK ||
            SpellTargetInfoTable[targetB].type == TARGET_TYPE_LOCK) {
          data.implicitType[effIdx] = TARGET_TYPE_LOCK;
          continue;
        }
        if (SpellTargetInfoTable[targetA].type == TARGET_TYPE_CORPSE ||
            SpellTargetInfoTable[targetB].type == TARGET_TYPE_CORPSE) {
          data.implicitType[effIdx] = TARGET_TYPE_CORPSE;
          continue;
        }
        if (SpellTargetInfoTable[targetA].type == TARGET_TYPE_GAMEOBJECT ||
            SpellTargetInfoTable[targetB].type == TARGET_TYPE_GAMEOBJECT) {
          data.implicitType[effIdx] = TARGET_TYPE_GAMEOBJECT;
          continue;
        }
        if ((SpellTargetInfoTable[targetA].type == TARGET_TYPE_LOCATION_DEST &&
             SpellTargetInfoTable[targetB].type == TARGET_TYPE_UNIT) ||
            (SpellTargetInfoTable[targetA].type == TARGET_TYPE_UNIT &&
             SpellTargetInfoTable[targetB].type == TARGET_TYPE_LOCATION_DEST)) {
        ...
        if ((SpellTargetInfoTable[targetA].type == TARGET_TYPE_LOCATION_SRC || targetA == TARGET_NONE) &&
            targetB == TARGET_NONE) {
        ...
        for (uint32 effIdxTarget = effIdxSource; effIdxTarget < MAX_EFFECT_INDEX; ++effIdxTarget) {
          ...
          for (uint8 rightTarget = effIdxSource == effIdxTarget ? rightSource + 1 : 0; rightTarget < 2; ++rightTarget) {
            ...
            if (targetSource == targetTarget) // if both target types are the same check if subsequent can be skipped
            {
              ...
              if (implicitEffectType == TARGET_TYPE_SPECIAL_UNIT && info.type == TARGET_TYPE_UNIT &&
                  info.enumerator != TARGET_ENUMERATOR_SINGLE)
                ignore = true;
                ...
                case TARGET_TYPE_CORPSE:
                  ignore = true;
                  break; // always ignore subsequent
                ...
                case TARGET_TYPE_PLAYER: {
                  ...
                  if (info.filter == TARGET_SCRIPT && effIdxSource != effIdxTarget) {
                    ...
                    if (!CheckBoundsEqualForEffects(bounds, SpellEffectIndex(effIdxSource),
                                                    SpellEffectIndex(effIdxTarget)))
                      continue;
                  }
                  ignore = true;
                  break;
...
bool SpellTargetMgr::CanEffectBeFilledWithMask(uint32 spellId, uint32 effIdx, uint32 mask) {
  ...
  case TARGET_TYPE_LOCATION_DEST:
    return bool(mask & TARGET_FLAG_DEST_LOCATION);
  case TARGET_TYPE_GAMEOBJECT:
    return bool(mask & (TARGET_FLAG_GAMEOBJECT | TARGET_FLAG_LOCKED));
  case TARGET_TYPE_UNIT_DEST:
    return bool(mask & (TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT_DEAD |
                        TARGET_FLAG_DEST_LOCATION));
  case TARGET_TYPE_PLAYER:
  case TARGET_TYPE_UNIT:
    return bool(mask & (TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT_DEAD));
  case TARGET_TYPE_CORPSE:
    return bool(mask & (TARGET_FLAG_CORPSE_ENEMY | TARGET_FLAG_CORPSE_ALLY));
  case TARGET_TYPE_LOCK:
    return bool(mask & (TARGET_FLAG_GAMEOBJECT | TARGET_FLAG_LOCKED | TARGET_FLAG_ITEM));
  case TARGET_TYPE_ITEM:
    return bool(mask & (TARGET_FLAG_LOCKED | TARGET_FLAG_ITEM));
  default:
    return false;
  }
}
...
SpellTargetFilterScheme SpellTargetMgr::GetSpellTargetingFilterScheme(SpellTargetFilterScheme oldScheme,
                                                                      uint32 spellId) {
  switch (spellId) {
