...
pAuraProcHandler AuraProcHandler[TOTAL_AURAS] = {
    ...
    &Unit::HandleNULLProc, // 70 SPELL_AURA_EXTRA_ATTACKS      Useless, used by only one spell that has only visual
                           // effect
...
void Unit::ProcSkillsAndReactives(bool isVictim, Unit *target, uint32 procFlags, uint32 procEx,
  ...
  if (procFlags & MELEE_BASED_TRIGGER_MASK) {
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
      ...
      if (procEx &
          (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT | PROC_EX_MISS | PROC_EX_DODGE | PROC_EX_PARRY | PROC_EX_BLOCK)) {
...
void Unit::ProcDamageAndSpell(ProcSystemArguments &&data) {
  ...
  if (data.attacker) {
    ...
    if (data.attacker->GetTypeId() == TYPEID_PLAYER &&
        (data.procExtra & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT)) != 0)
      if ((data.procFlagsAttacker & PROC_FLAG_ON_DO_PERIODIC) == 0) // do not proc this on DOTs
        if (!data.procSpell || (data.procSpell->EquippedItemClass == ITEM_CLASS_WEAPON &&
                                !data.procSpell->HasAttribute(SPELL_ATTR_STOP_ATTACK_TARGET)))
          static_cast<Player *>(data.attacker)
              ->CastItemCombatSpell(data.victim, data.attType,
                                    data.procSpell ? !IsNextMeleeSwingSpell(data.procSpell) : false);
...
ProcExecutionData::ProcExecutionData(ProcSystemArguments &data, bool isVictim)
    : isVictim(isVictim), procExtra(data.procExtra), attType(data.attType), damage(data.damage),
      procSpell(data.procSpell), spell(data.spell), healthGain(data.healthGain), triggeredByAura(nullptr), cooldown(0),
...
void Unit::ProcDamageAndSpellFor(ProcSystemArguments &argData, bool isVictim) {
  ...
  for (SpellAuraHolderMap::const_iterator itr = GetSpellAuraHolderMap().begin(); itr != GetSpellAuraHolderMap().end();
       ++itr) {
    ...
    for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i) {
      ...
      if (execData.procSpell) {
        if (spellProcEvent) {
          ...
          else if (!execData.triggeredByAura->CanProcFrom(execData.procSpell, spellProcEvent->procEx,
                                                          execData.procExtra, execData.damage != 0,
                                                          !spellProcEvent->schoolMask))
            continue;
        } else if (!execData.triggeredByAura->CanProcFrom(execData.procSpell, PROC_EX_NONE, execData.procExtra,
                                                          execData.damage != 0, true))
          continue;
      }
...
bool Unit::IsTriggeredAtSpellProcEvent(ProcExecutionData &data, SpellAuraHolder *holder,
                                       SpellProcEventEntry const *&spellProcEvent) {
  ...
  if (!SpellMgr::IsSpellProcEventCanTriggeredBy(spellProcEvent, EventProcFlag, data.procSpell, data.procFlags,
                                                data.procExtra))
    return false;
  ...
  if (!data.isVictim && GetTypeId() == TYPEID_PLAYER) {
    if (spellProto->EquippedItemClass == ITEM_CLASS_WEAPON) {
      ...
      if (!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_WEAPON ||
          !((1 << item->GetProto()->SubClass) & spellProto->EquippedItemSubClassMask))
        return false;
    } else if (spellProto->EquippedItemClass == ITEM_CLASS_ARMOR) {
      // Check if player is wearing shield
      Item *item = ((Player *)this)->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
      if (!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_ARMOR ||
          !((1 << item->GetProto()->SubClass) & spellProto->EquippedItemSubClassMask))
        return false;
    }
  }
...
SpellAuraProcResult Unit::TriggerProccedSpell(Unit *target, std::array<int32, MAX_EFFECT_INDEX> &basepoints,
...
SpellAuraProcResult Unit::TriggerProccedSpell(Unit *target, std::array<int32, MAX_EFFECT_INDEX> &basepoints,
  ...
  if (basepoints[EFFECT_INDEX_0] || basepoints[EFFECT_INDEX_1] || basepoints[EFFECT_INDEX_2])
    CastCustomSpell(target, spellInfo, basepoints[EFFECT_INDEX_0] ? &basepoints[EFFECT_INDEX_0] : nullptr,
                    basepoints[EFFECT_INDEX_1] ? &basepoints[EFFECT_INDEX_1] : nullptr,
                    basepoints[EFFECT_INDEX_2] ? &basepoints[EFFECT_INDEX_2] : nullptr, TRIGGERED_OLD_TRIGGERED,
...
SpellAuraProcResult Unit::HandleHasteAuraProc(ProcExecutionData &data) {
  Unit *pVictim = data.victim;
  uint32 damage = data.damage;
  Aura *triggeredByAura = data.triggeredByAura;
  uint32 cooldown = data.cooldown;
...
SpellAuraProcResult Unit::HandleDummyAuraProc(ProcExecutionData &data) {
  ...
  uint32 procFlags = data.procFlags;
  uint32 procEx = data.procExtra;
  uint32 cooldown = data.cooldown;
  ...
  case SPELLFAMILY_GENERIC: {
    ...
    case 28764: {
      ...
      for (auto iter : mRegenInterrupt) {
        if (SpellEntry const *iterSpellProto = iter->GetSpellProto()) {
          if (iterSpellProto->SpellFamilyName == SPELLFAMILY_MAGE &&
              (iterSpellProto->SpellFamilyFlags & uint64(0x10000000))) {
            found = true;
            break;
          }
        }
    ...
    case 32052: // Red
    {
      switch (dummySpell->Id) {
      case 32045:
        triggered_spell_id = 32054;
        break;
      case 32051:
        triggered_spell_id = 32057;
        break;
      case 32052:
        triggered_spell_id = 32053;
        break;
      }
    ...
    case 36035: {
      ...
      pCaster->SummonCreature(uiSoulEntry, GetPositionX(), GetPositionY(), GetPositionZ(), 0,
                              TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 10000);
      break;
    }
    ...
    // 41406 Dementia: Every 5 seconds either gives you +5% damage/healing. (Druid, Shaman, Priest, Warlock, Mage,
    // Paladin) 41409 Dementia: Every 5 seconds either gives you -5% damage/healing. (Druid, Shaman, Priest, Warlock,
    // Mage, Paladin)
    case 39446: {
      if (GetTypeId() != TYPEID_PLAYER)
        return SPELL_AURA_PROC_FAILED;
  ...
  case SPELLFAMILY_MAGE: {
    ...
    case 11129: {
      ...
      return (procEx & PROC_EX_CRITICAL_HIT)
                 ? SPELL_AURA_PROC_OK
                 : SPELL_AURA_PROC_FAILED; // charge update only at crit hits, no hidden cooldowns
    }
    case 30554: // Wrath of the Titans - Karazhan
    {
      ...
      case SPELL_SCHOOL_FIRE:
        ...
        break; // Flame of Khaz'goroth
      case SPELL_SCHOOL_NATURE:
        triggered_spell_id = 30606;
        break; // Bolt of Eonar
      case SPELL_SCHOOL_FROST:
        triggered_spell_id = 30609;
        break; // Chill of Norgannon
      case SPELL_SCHOOL_SHADOW:
        triggered_spell_id = 30608;
        break; // Spite of Sargeras
      case SPELL_SCHOOL_ARCANE:
        triggered_spell_id = 30605;
        break; // Blast of Amanthul
      default:
        return SPELL_AURA_PROC_FAILED;
      }
      break;
  ...
  case SPELLFAMILY_WARRIOR: {
    ...
    if (dummySpell->SpellIconID == 1697) {
      // only for spells and hit/crit (trigger start always) and not start from self casted spells (5530 Mace Stun
      // Effect for example)
  ...
  case SPELLFAMILY_PRIEST: {
    ...
    if (dummySpell->SpellFamilyFlags & uint64(0x0000040000000000)) {
      ...
      pVictim->CastCustomSpell(pVictim, 34919, &basepoints[0], nullptr, nullptr, TRIGGERED_OLD_TRIGGERED, castItem,
                               triggeredByAura);
      return SPELL_AURA_PROC_OK; // no hidden cooldown
    }
    switch (dummySpell->Id) {
    ...
    case 15286: {
      ...
      // heal amount
      basepoints[0] = triggerAmount * damage / 100;
      pVictim->CastCustomSpell(pVictim, 15290, &basepoints[0], nullptr, nullptr, TRIGGERED_OLD_TRIGGERED, castItem,
                               triggeredByAura);
      return SPELL_AURA_PROC_OK; // no hidden cooldown
    }
  ...
  case SPELLFAMILY_PALADIN: {
    ...
    if ((dummySpell->SpellFamilyFlags & uint64(0x000000008000000)) &&
      ...
      int32 damagePoint =
          int32(damageBasePoints +
                0.03f * (GetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE) + GetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE)) /
                    2.0f) +
          1;

      // apply damage bonuses manually
      if (damagePoint >= 0) {
        // currently uses same spell damage fetch as flametongue - need to verify whether SP is supposed to be applied
        // pre-triggered spell bonuses or post
        int32 bonusDamage = SpellBaseDamageBonusDone(GetSpellSchoolMask(dummySpell)) +
                            pVictim->SpellBaseDamageBonusTaken(GetSpellSchoolMask(dummySpell));
      ...
      CastCustomSpell(pVictim, spellId, &damagePoint, nullptr, nullptr, TRIGGERED_OLD_TRIGGERED, nullptr,
                      triggeredByAura);
      return SPELL_AURA_PROC_OK; // no hidden cooldown
    }
  ...
  case SPELLFAMILY_SHAMAN: {
    ...
    case 33757: {
      ...
      for (uint32 i = 0; i < 2; ++i)
        CastCustomSpell(pVictim, triggered_spell_id, &basepoints[0], nullptr, nullptr, TRIGGERED_OLD_TRIGGERED,
                        castItem, triggeredByAura);
    ...
    if (dummySpell->SpellFamilyFlags & uint64(0x0000000000200000)) {
      ...
      Item *usedWeapon =
          ((Player *)this)
              ->GetWeaponForAttack(procFlags & PROC_FLAG_SUCCESSFUL_OFFHAND_HIT ? OFF_ATTACK : BASE_ATTACK, true, true);
      if (usedWeapon != castItem)
        return SPELL_AURA_PROC_FAILED;

      switch (dummySpell->Id) {
      case 10400:
        triggered_spell_id = 8026;
        break; // Rank 1
      case 15567:
        triggered_spell_id = 8028;
        break; // Rank 2
      case 15568:
        triggered_spell_id = 8029;
        break; // Rank 3
      case 15569:
        triggered_spell_id = 10445;
        break; // Rank 4
      case 16311:
        triggered_spell_id = 16343;
        break; // Rank 5
      case 16312:
        triggered_spell_id = 16344;
        break; // Rank 6
      case 16313:
        triggered_spell_id = 25488;
        break; // Rank 7
      default:
        return SPELL_AURA_PROC_FAILED;
      }
    ...
    if (dummySpell->SpellIconID == 2018) // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
    {
      ...
      if (!procSpellEntry) {
        sLog.outError("Unit::HandleDummyAuraProc: nonexistent spell id: %u (Lightning Bolt and Chain Lightning)",
                      spellId);
        return SPELL_AURA_PROC_FAILED;
      }
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(ProcExecutionData &data) {
  ...
  case SPELLFAMILY_GENERIC:
    ...
    case 36096:                  // Spell Reflection
      return SPELL_AURA_PROC_OK; // Missing Trigger spell with no evidence to tell what to trigger, need to return to
                                 // trigger consumption
    ...
    case 38164: // Unyielding Knights - this can only proc in hellfire peninsula with a maximum of 2 guardians against
                // fel orc faction only
    ...
    case 45057: // Evasive Maneuvers (Commendation of Kael`thas trinket)
      // reduce you below $s1% health (in fact in this specific case can proc from any attack while health in result
      // less $s1%)
      ...
      for (auto i : mAddFlatModifier) {
        if (i->GetModifier()->m_miscvalue == SPELLMOD_CHANCE_OF_SUCCESS && i->GetSpellProto()->SpellIconID == 113) {
          ...
          basepoints[EFFECT_INDEX_0] =
              GetMaxPower(POWER_MANA) * i->GetSpellProto()->CalculateSimpleValue(EFFECT_INDEX_2) / 100;
    ...
    if (auraSpellInfo->SpellIconID == 2137)
    {
        switch (auraSpellInfo->Id)
        {
            ...
            case 31830:                         // Rank 3
                break;
            default:
                sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u miss posibly Blessed Life",
    auraSpellInfo->Id); return SPELL_AURA_PROC_FAILED;
        }
    }
    */
    ...
    if (auraSpellInfo->SpellFamilyFlags & uint64(0x0000000000080000)) {
      ...
      case 27164:
        ...
        break; // Rank 4
      default:
        sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u miss posibly Judgement of Light/Wisdom",
                      auraSpellInfo->Id);
        return SPELL_AURA_PROC_FAILED;
      }
      pVictim->CastSpell(nullptr, trigger_spell_id,
    ...
    // Illumination
      ...
      if (!originalSpell) {
        sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u unknown but selected as original in Illu",
                      originalSpellId);
        return SPELL_AURA_PROC_FAILED;
      }
  ...
  case SPELLFAMILY_SHAMAN: {
    ...
    // Nature's Guardian
      ...
      // Check health condition - should drop to less 30% (trigger at any attack with result health less 30%,
      // independent original health state)
  ...
  if (!triggerEntry) {
    ...
    // sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u have 0 in EffectTriggered[%d], not handled custom
  ...
  if (target == nullptr)
    target = !(procFlags & (PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS | PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS)) &&
                     IsPositiveSpellTargetMode(triggerEntry, this, pVictim)
                 ? this
                 : pVictim;
...
SpellAuraProcResult Unit::HandleProcTriggerDamageAuraProc(ProcExecutionData &data) {
  Unit *victim = data.victim;
  Aura *triggeredByAura = data.triggeredByAura;
  SpellEntry const *spellInfo = triggeredByAura->GetSpellProto();
  DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST,
                   "ProcDamageAndSpell: doing %u damage from spell id %u (triggered by auratype %u of spell %u)",
                   triggeredByAura->GetModifier()->m_amount, spellInfo->Id, triggeredByAura->GetModifier()->m_auraname,
                   triggeredByAura->GetId());
  // Trigger damage can be resisted...
  if (SpellMissInfo missInfo =
          this->SpellHitResult(victim, spellInfo, uint8(1 << triggeredByAura->GetEffIndex()), false)) {
  ...
  if (jumps > 0 && GetTypeId() == TYPEID_PLAYER && caster_guid.IsPlayer()) {
    ...
    if (Unit *caster = triggeredByAura->GetCaster()) {
      if (Player *target = GetNextRandomRaidMember(radius, SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE)) {
        ...
        // lock aura holder (currently SPELL_AURA_RAID_PROC_FROM_CHARGE is single target spell, so will attempt removing
        // from old target when applied to new one)
        ...
        CastSpell(target, animationSpellId,
                  TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL | TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
      }
    }
  }

  CastSpell(nullptr, triggeredSpellId,
            TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL | TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
  return SPELL_AURA_PROC_OK;
}
...
SpellAuraProcResult Unit::HandleRaidProcFromChargeWithValueAuraProc(ProcExecutionData &data) {
  ...
  if (jumps > 0 && GetTypeId() == TYPEID_PLAYER && caster_guid.IsPlayer()) {
    ...
    if (Player *caster = static_cast<Player *>(triggeredByAura->GetCaster())) {
      ...
      if (Player *target = static_cast<Player *>(this)->GetNextRaidMemberWithLowestLifePercentage(
              radius, SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE)) {
        // This custom nonsense was later removed in 3.0.2 for this proc effect only and replaced by casting the
        // original spell
        SpellAuraHolder *holder = GetSpellAuraHolder(spellProto->Id, caster->GetObjectGuid());
        SpellAuraHolder *new_holder = CreateSpellAuraHolder(spellProto, target, caster);
        ...
        // lock aura holder (currently SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE is single target spell, so will
        // attempt removing from old target when applied to new one)
        if (!target->AddSpellAuraHolder(new_holder))
          delete new_holder;
        else
          new_holder->SetState(SPELLAURAHOLDER_STATE_READY);
  ...
  // heal
  CastCustomSpell(nullptr, 33110, &heal, nullptr, nullptr,
                  TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL | TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
  return SPELL_AURA_PROC_OK;
}
...
SpellAuraProcResult Unit::HandleModCastingSpeedNotStackAuraProc(ProcExecutionData &data) {
  ...
  return !(procSpell == nullptr || GetSpellCastTime(procSpell, this) == 0) ? SPELL_AURA_PROC_OK
...
SpellAuraProcResult Unit::HandleReflectSpellsSchoolAuraProc(ProcExecutionData &data) {
  ...
  return !(procSpell == nullptr || (triggeredByAura->GetModifier()->m_miscvalue & procSpell->SchoolMask) == 0)
             ? SPELL_AURA_PROC_OK
             : SPELL_AURA_PROC_FAILED;
}
...
SpellAuraProcResult Unit::HandleModPowerCostSchoolAuraProc(ProcExecutionData &data) {
  ...
  return !(procSpell == nullptr || (procSpell->manaCost == 0 && procSpell->ManaCostPercentage == 0) || // Cost check
           (triggeredByAura->GetModifier()->m_miscvalue & procSpell->SchoolMask) == 0)
             ? SPELL_AURA_PROC_OK
...
SpellAuraProcResult Unit::HandleInvisibilityAuraProc(ProcExecutionData &data) {
  Aura *triggeredByAura = data.triggeredByAura;
  if (triggeredByAura->GetSpellProto()->HasAttribute(SPELL_ATTR_PASSIVE) ||
      triggeredByAura->GetSpellProto()->HasAttribute(SPELL_ATTR_AURA_IS_DEBUFF))
    return SPELL_AURA_PROC_FAILED;
