...
void WorldSession::HandleUseItemOpcode(WorldPacket &recvPacket) {
  ...
  DETAIL_LOG("WORLD: CMSG_USE_ITEM packet, bagIndex: %u, slot: %u, spell_index: %u , Item: %u, data length = %u",
             bagIndex, slot, spell_index, pItem->GetEntry(), (uint32)recvPacket.size());
  ...
  if (proto->Class == ITEM_CLASS_CONSUMABLE && !(proto->Flags & ITEM_FLAG_IGNORE_DEFAULT_ARENA_RESTRICTIONS) &&
      pUser->InArena()) {
    recvPacket.rpos(recvPacket.wpos()); // prevent spam at not read packet tail
...
void WorldSession::HandleOpenItemOpcode(WorldPacket &recvPacket) {
  ...
  {
    QueryResult *result = CharacterDatabase.PQuery("SELECT entry, flags FROM character_gifts WHERE item_guid = '%u'",
                                                   pItem->GetGUIDLow());
    if (result) {
...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket &recv_data) {
  ...
  if (!obj->IsSpawned()) {
    sLog.outError("HandleGameObjectUseOpcode: CMSG_GAMEOBJ_USE for despawned GameObject (Entry %u), didn't expect this "
                  "to happen.",
                  obj->GetEntry());
    return;
  }
  ...
  if (obj->GetGoType() == GAMEOBJECT_TYPE_GENERIC) {
    sLog.outError("HandleGameObjectUseOpcode: CMSG_GAMEOBJ_USE for not allowed GameObject type %u (Entry %u), didn't "
  ...
  if (obj->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_NO_INTERACT)) {
    sLog.outError("HandleGameObjectUseOpcode: CMSG_GAMEOBJ_USE for GameObject (Entry %u) with non intractable flag "
                  "(Flags %u), didn't expect this to happen.",
...
void WorldSession::HandleCastSpellOpcode(WorldPacket &recvPacket) {
  ...
  DEBUG_LOG("WORLD: CMSG_CAST_SPELL, spellId - %u, cast_count: %u data length = " SIZEFMTD, spellId, cast_count,
            recvPacket.size());
  ...
  if (Unit *target = targets.getUnitTarget()) {
    // if rank not found then function return nullptr but in explicit cast case original spell can be casted and later
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket &recvPacket) {
  ...
  if (guid != GetPlayer()->GetPetGuid() && !GetPlayer()->HasCharm(guid)) {
    sLog.outError("HandlePetCancelAura. %s isn't pet of %s", guid.GetString().c_str(),
...
void WorldSession::HandleSelfResOpcode(WorldPacket & /*recv_data*/) {
  ...
  if (_player->GetUInt32Value(PLAYER_SELF_RES_SPELL)) {
    SpellEntry const *spellInfo =
        sSpellTemplate.LookupEntry<SpellEntry>(_player->GetUInt32Value(PLAYER_SELF_RES_SPELL));
    if (spellInfo)
