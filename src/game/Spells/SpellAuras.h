...
public:
  ...
  void SetDeleted() {
    m_deleted = true;
    m_spellAuraHolderState = SPELLAURAHOLDER_STATE_REMOVING;
  }
...
private:
  ...
  SpellAuraHolderState m_spellAuraHolderState; // State used to be sure init part is finished (ex there is still some
                                               // aura to add or effect to process)
...
// Second rule: Code must be guarded by if(Real) check if it modifies object state (start/stop attack, send packets to
// client, etc)
//
// Other case choice: each code line moved under if(Real) check is mangos speedup,
//      each setting object update field code line moved under if(Real) check is significant mangos speedup, and less
//      server->client data sends each packet sending code moved under if(Real) check is _large_ mangos speedup, and lot
//      less server->client data sends

class Aura {
  friend struct ReapplyAffectedPassiveAurasHelper;
  friend Aura *CreateAura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage,
                          int32 const *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster,
                          Item *castItem);
...
protected:
  Aura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage, int32 const *currentBasePoints,
       SpellAuraHolder *holder, Unit *target, Unit *caster = nullptr, Item *castItem = nullptr);
  ...
  void PickTargetsForSpellTrigger(Unit *&triggerCaster, Unit *&triggerTarget, WorldObject *&triggerTargetObject,
  ...
  uint32 m_periodicTick; // Tick count pass (including current if use in tick code) from aura apply, used for some tick
                         // count dependent aura effects
...
public:
  AreaAura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage,
           int32 const *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster = nullptr,
           Item *castItem = nullptr, uint32 originalRankSpellId = 0);
  virtual ~AreaAura();
...
public:
  PersistentAreaAura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage,
                     int32 const *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster = nullptr,
...
protected:
  SingleEnemyTargetAura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage,
                        int32 const *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster = nullptr,
                        Item *castItem = nullptr);
...
public:
  GameObjectAura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage,
                 int32 const *currentBasePoints, SpellAuraHolder *holder, Unit *target, GameObject *caster);
