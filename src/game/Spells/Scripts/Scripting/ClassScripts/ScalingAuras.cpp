...
struct DruidPetScaling1 : public AuraScript {
  int32 OnAuraValueCalculate(Aura *aura, Unit *caster, int32 value) const override {
    ...
    case EFFECT_INDEX_1: // attack power
      if (Unit *owner = caster->GetOwner()) {
        if (owner->IsPlayer()) {
          int32 nature = int32(owner->GetUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + SPELL_SCHOOL_NATURE)) -
                         owner->GetUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + SPELL_SCHOOL_NATURE);
          if (nature < 0)
            nature = 0;
          value = nature * 1.f;
        }
