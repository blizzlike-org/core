    ...
    if (aura->GetEffIndex() == EFFECT_INDEX_2) {
      ...
      if (Unit *caster = aura->GetCaster()) {
        ...
        for (Unit::AuraList::const_iterator j = mclassScritAuras.begin(); j != mclassScritAuras.end(); ++j) {
          ...
          case 5240:
            damage =
                std::min((*j)->GetModifier()->m_amount, auraValue); // use lower, so that talent doesnt make low lvls OP
            break;
          }
        }
      }
...
struct Misdirection : public SpellScript {
  SpellCastResult OnCheckCast(Spell *spell, bool /* strict*/) const override {
    ...
    // Misdirection: If a Hunter attempts to use this ability on a target which already has an active Misdirection, the
    // spell will fail to apply due to a more powerful spell already being in effect.
