...
struct UnstableAffliction : public AuraScript {
  void OnDispel(SpellAuraHolder *holder, Unit *dispeller, uint32 /*dispellingSpellId*/,
    ...
    dispeller->CastCustomSpell(dispeller, SPELL_UNSTABLE_AFFLICTION_SILENCE, &damage, nullptr, nullptr,
                               TRIGGERED_OLD_TRIGGERED, nullptr, nullptr, holder->GetCasterGuid());
  }
};
...
struct LifeTap : public SpellScript {
  void OnInit(Spell *spell) const override {
    ...
    int32 dmg =
        caster->SpellDamageBonusDone(caster, spell->m_spellInfo, uint32(cost > 0 ? cost : 0), SPELL_DIRECT_DAMAGE);
  ...
  void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override {
    ...
    caster->CastCustomSpell(nullptr, 31818, &mana, nullptr, nullptr,
  ...
  int32 OnAuraValueCalculate(Aura *aura, Unit *caster, int32 value) const override {
    ...
    if (caster) {
      ...
      if (!aura->GetScriptValue())
        aura->SetScriptValue((target->HasAura(35693)   ? 12
                              : target->HasAura(35692) ? 8
                              : target->HasAura(35691) ? 4
                                                       : 0));
