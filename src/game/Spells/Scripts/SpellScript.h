...
struct PeriodicTriggerData {
  ...
  int32 *basePoints;
  PeriodicTriggerData(Unit *caster, Unit *target, WorldObject *targetObject, SpellEntry const *spellInfo,
                      int32 *basePoints)
...
struct SpellScript {
  ...
  virtual void OnRadiusCalculate(Spell * /*spell*/, SpellEffectIndex /*effIdx*/, bool /*targetB*/,
                                 float & /*radius*/) const {}
  // called on adding dest target
  virtual void OnDestTarget(Spell * /*spell*/) const {}
  // called on Unit Spell::CheckTarget
  virtual bool OnCheckTarget(const Spell * /*spell*/, GameObject * /*target*/, SpellEffectIndex /*eff*/) const {
    return true;
  }
  // called on GameObject target checking
  virtual bool OnCheckTarget(const Spell * /*spell*/, Unit * /*target*/, SpellEffectIndex /*eff*/) const {
    return true;
  }
...
struct AuraScript {
  ...
  // if an aura is applied before, its removed after, and if some aura needs to do something after aura effect is
  // applied, need to revert that change before its removed called before aura apply and after aura unapply
  virtual void OnApply(Aura * /*aura*/, bool /*apply*/) const {}
  // the following two hooks are done in an alternative fashion due to how they are usually used
  // if an aura is applied before, its removed after, and if some aura needs to do something after aura effect is
  // applied, need to revert that change before its removed called after aura apply and before aura unapply
  ...
  virtual SpellAuraProcResult OnProc(Aura * /*aura*/, ProcExecutionData & /*procData*/) const {
    return SPELL_AURA_PROC_OK;
  }
  // called on absorb of this aura
  virtual void OnAbsorb(Aura * /*aura*/, int32 & /*currentAbsorb*/, uint32 & /*reflectedSpellId*/,
                        int32 & /*reflectDamage*/, bool & /*preventedDeath*/) const {}
  ...
  virtual void OnDispel(SpellAuraHolder * /*holder*/, Unit * /*dispeller*/, uint32 /*dispellingSpellId*/,
                        uint32 /*originalStacks*/) const {}
...
template <class T> void RegisterScript(std::string stringName) {
...
template <class T> void RegisterSpellScript(std::string stringName) {
  static_assert(std::is_base_of<SpellScript, T>::value, "T not derived from SpellScript");
  SpellScriptMgr::SetSpellScript(stringName, new T());
}

template <class U> void RegisterAuraScript(std::string stringName) {
