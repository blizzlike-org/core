  ...
  if (result) {
    do {
      ...
      if (!sSpellTemplate.LookupEntry<SpellEntry>(spellId)) {
        sLog.outErrorDb("Spell %u has script %s in spell_scripts but spell does not exist. Skipping.", spellId,
                        scriptName.data());
        continue;
      }
      ...
      if (!found) {
        sLog.outErrorDb("Spell %u has script %s in spell_scripts but script does not exist. Skipping.", spellId,
                        scriptName.data());
        continue;
      }
...
void SpellScriptMgr::SetSpellScript(uint32 spellId, SpellScript *script) { m_spellScriptMap.emplace(spellId, script); }

void SpellScriptMgr::SetAuraScript(uint32 spellId, AuraScript *script) { m_auraScriptMap.emplace(spellId, script); }
