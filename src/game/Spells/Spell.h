...
enum SpellNotifyPushType { PUSH_CONE, PUSH_SELF_CENTER, PUSH_SRC_CENTER, PUSH_DEST_CENTER, PUSH_TARGET_CENTER };
...
public:
  ...
  void getDestination(WorldLocation &loc) {
    loc.coord_x = m_destPos.x;
    loc.coord_y = m_destPos.y;
    loc.coord_z = m_destPos.z;
  }
...
enum SpellTargets { SPELL_TARGETS_ASSISTABLE, SPELL_TARGETS_AOE_ATTACKABLE, SPELL_TARGETS_ALL };
...
public:
  ...
  template <typename... Args> void AddLog(uint32 spellEffect, Args... args) {
...
private:
  ...
  template <typename T> void AddLogData(T const &data) { m_spellLogData << data; }

  template <typename T, typename... Args> void AddLogData(T const &data, Args const &...args) {
    AddLogData(data);
    AddLogData(args...);
  }
...
public:
  ...
  Spell(Unit *caster, SpellEntry const *info, uint32 triggeredFlags, ObjectGuid originalCasterGUID = ObjectGuid(),
  ...
  int32 CalculateSpellEffectValue(SpellEffectIndex i, Unit *target, bool maximum = false) {
    return m_caster->CalculateSpellEffectValue(target, m_spellInfo, i, &m_currentBasePoints[i], maximum);
  }
  int32 CalculateSpellEffectDamage(Unit *unitTarget, int32 damage);
  static uint32 CalculatePowerCost(SpellEntry const *spellInfo, Unit *caster, Spell *spell = nullptr,
                                   Item *castItem = nullptr, bool finalUse = false);
  ...
  struct CreaturePosition {
    CreaturePosition() : x(0.0f), y(0.0f), z(0.0f), creature(nullptr) {}
  ...
  bool DoSummonWild(CreatureSummonPositions &list, SummonPropertiesEntry const *prop, SpellEffectIndex effIdx,
                    uint32 level);
  bool DoSummonGuardian(CreatureSummonPositions &list, SummonPropertiesEntry const *prop, SpellEffectIndex effIdx,
                        uint32 level);
  bool DoSummonCritter(CreatureSummonPositions &list, SummonPropertiesEntry const *prop, SpellEffectIndex effIdx,
                       uint32 level);
  bool DoSummonPossessed(CreatureSummonPositions &list, SummonPropertiesEntry const *prop, SpellEffectIndex effIdx,
                         uint32 level);

  void ProcessDispelList(std::list<std::pair<SpellAuraHolder *, uint32>> &dispelList,
                         std::list<std::pair<SpellAuraHolder *, uint32>> &successList, std::list<uint32> &failList);
  ...
  static void SendCastResult(Player const *caster, SpellEntry const *spellInfo, uint8 cast_count,
                             SpellCastResult result, bool isPetCastResult = false);
  ...
  void HandleEffect(Unit *unitTarget, Item *itemTarget, GameObject *GOTarget, SpellEffectIndex effIdx,
  ...
  int32 m_currentBasePoints[MAX_EFFECT_INDEX]; // cache SpellEntry::CalculateSimpleValue and use for set custom base
                                               // points
  ...
  bool IsMeleeAttackResetSpell() const {
    return !m_IsTriggeredSpell && (m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT);
  }
  bool IsRangedAttackResetSpell() const {
    return !m_IsTriggeredSpell && IsRangedSpell() && (m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT);
  }
  ...
  bool IsNeedSendToClient() const; // use for hide spell cast for client in case when cast not have client side affect
                                   // (animation or log entries)
  bool IsTriggeredSpellWithRedundantCastTime()
      const; // use for ignore some spell data for triggered spells like cast time, some triggered spells have redundent
             // copy data from main spell for client use purpose
  ...
  // limited version returning nullptr in cases wild gameobject caster object, need for Aura (auras currently not
  // support non-Unit caster)
  ...
  void
  UpdatePointers(); // must be used at call Spell code after time delay (non triggered spell cast/update spell call/etc)
  ...
  static void SelectMountByAreaAndSkill(Unit *target, SpellEntry const *parentSpell, uint32 spellId75,
                                        uint32 spellId150, uint32 spellId225, uint32 spellId300, uint32 spellIdSpecial);

  bool CanBeInterrupted() const {
    return m_spellState <= SPELL_STATE_DELAYED || m_spellState == SPELL_STATE_CHANNELING;
  }
  ...
  SpellCastResult CheckScriptTargeting(SpellEffectIndex effIndex, uint32 chainTargets, float radius, uint32 targetMode,
                                       UnitList &tempUnitList, GameObjectList &tempGOList);
  ...
  void SetFilteringScheme(SpellEffectIndex effIdx, bool targetB, SpellTargetFilterScheme scheme) {
    m_filteringScheme[effIdx][uint32(targetB)] = scheme;
  }
...
protected:
  ...
  Corpse *corpseTarget;
  SpellAuraHolder
      *m_spellAuraHolder; // spell aura holder for current target, created only if spell has aura applying effect
  int32 damage;
  ...
  void PrepareMasksForProcSystem(uint8 effectMask, uint32 &procAttacker, uint32 &procVictim, WorldObject *caster,
                                 WorldObject *target);
  ...
  static void CheckSpellScriptTargets(SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> &bounds,
  ...
  void FillAreaTargets(UnitList &targetUnitMap, float radius, float cone, SpellNotifyPushType pushType,
                       SpellTargets spellTargets, WorldObject *originalCaster = nullptr);
  void FillRaidOrPartyTargets(UnitList &targetUnitMap, Unit *member, float radius, bool raid, bool withPets,
  ...
  SpellCastResult CanOpenLock(SpellEffectIndex effIndex, uint32 lockid, SkillType &skillId, int32 &reqSkillValue,
                              int32 &skillValue);
  ...
  SpellInfoList
      m_TriggerSpells; // casted by caster to same targets settings in m_targets at success finish of current spell
...
namespace MaNGOS {
struct SpellNotifierPlayer // Currently unused. When put to use this one requires handling for source-location (smilar
                           // to below)
{
  UnitList &i_data;
  Spell &i_spell;
  ...
  void Visit(PlayerMapType &m) {
    ...
    for (auto &itr : m) {
      ...
      if (pPlayer->IsWithinDist3d(i_spell.m_targets.m_destPos.x, i_spell.m_targets.m_destPos.y,
                                  i_spell.m_targets.m_destPos.z, i_radius))
...
struct SpellNotifierCreatureAndPlayer {
  ...
  SpellNotifierCreatureAndPlayer(Spell &spell, UnitList &data, float radius, float cone, SpellNotifyPushType type,
                                 SpellTargets TargetType = SPELL_TARGETS_AOE_ATTACKABLE,
