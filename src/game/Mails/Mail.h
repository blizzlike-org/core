...
public: // Constructors
  ...
  MailDraft(std::string subject, uint32 itemTextId = 0)
      : m_mailTemplateId(0), m_mailTemplateItemsNeed(false), m_subject(subject), m_bodyId(itemTextId), m_money(0),
        m_COD(0) {}
  /**
...
public: // modifiers
  // this two modifiers expected to be applied in normal case to blank draft and exclusively, It DON'T must overwrite
  // already set itemTextId, in other cases it will work and with mixed cases but this will be not normal way use.
  MailDraft &SetSubjectAndBodyId(const std::string &subject, uint32 itemTextId) {
    m_subject = subject;
...
public: // finishers
  ...
  void SendMailTo(MailReceiver const &receiver, MailSender const &sender, MailCheckMask checked = MAIL_CHECK_MASK_NONE,
...
private:
  ...
  MailItemMap
      m_items; ///< Keep the items in a map to avoid duplicate guids (which can happen), store only low part of guid
...
struct Mail {
   ...
   *
   */
  void prepareTemplateItems(Player *receiver); ///< called from _LoadMails for generate mailTemplateBase items not
                                               ///< generated for offline player
};
