...
void WorldSession::HandleSendMail(WorldPacket &recv_data) {
  ...
  if (!rc) {
    DETAIL_LOG("%s is sending mail to %s (GUID: nonexistent!) with subject %s and body %s includes %u items, %u copper "
               "and %u COD copper with unk1 = %u, unk2 = %u",
               pl->GetGuidStr().c_str(), receiver.c_str(), subject.c_str(), body.c_str(), items_count, money, COD, unk1,
               unk2);
    pl->SendMailResult(0, MAIL_SEND, MAIL_ERR_RECIPIENT_NOT_FOUND);
    return;
  }

  DETAIL_LOG("%s is sending mail to %s with subject %s and body %s includes %u items, %u copper and %u COD copper with "
             "unk1 = %u, unk2 = %u",
             pl->GetGuidStr().c_str(), rc.GetString().c_str(), subject.c_str(), body.c_str(), items_count, money, COD,
             unk1, unk2);
  ...
  if (receive) {
    ...
    if (QueryResult *result =
            CharacterDatabase.PQuery("SELECT COUNT(*) FROM mail WHERE receiver = '%u'", rc.GetCounter())) {
  ...
  if (!sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_MAIL) && pl->GetTeam() != rc_team &&
  ...
  if (items_count > 0 || money > 0) {
    ...
    if (items_count > 0) {
      for (uint8 i = 0; i < items_count; ++i) {
        ...
        item->SaveToDB(); // recursive and not have transaction guard into self, item not in inventory and can be save
                          // standalone
        // owner in data will set at mail receive and item extracting
        CharacterDatabase.PExecute("UPDATE item_instance SET owner_guid = '%u' WHERE guid='%u'", rc.GetCounter(),
                                   item->GetGUIDLow());
    ...
    if (money > 0 && GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE)) {
      sLog.outCommand(GetAccountId(), "GM %s (Account: %u) mail money: %u to player: %s (Account: %u)", GetPlayerName(),
                      GetAccountId(), money, receiver.c_str(), rc_account);
    }
  }
  ...
  draft.SetMoney(money).SetCOD(COD).SendMailTo(
      MailReceiver(receive, rc), pl, body.empty() ? MAIL_CHECK_MASK_COPIED : MAIL_CHECK_MASK_HAS_BODY, deliver_delay);
...
void WorldSession::HandleMailReturnToSender(WorldPacket &recv_data) {
  ...
  if (m->messageType == MAIL_NORMAL && m->sender) {
    ...
    draft.SetMoney(m->money).SendReturnToSender(GetAccountId(), m->receiverGuid,
  ...
  if (msg == EQUIP_ERR_OK) {
    ...
    if (m->COD > 0) // if there is COD, take COD money from player and send them to sender by mail
    {
      ...
      if (GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE)) {
        ...
        sLog.outCommand(GetAccountId(),
                        "GM %s (Account: %u) receive mail item: %s (Entry: %u Count: %u) and send COD money: %u to "
                        "player: %s (Account: %u)",
                        GetPlayerName(), GetAccountId(), it->GetProto()->Name1, it->GetEntry(), it->GetCount(), m->COD,
...
void WorldSession::HandleGetMailList(WorldPacket &recv_data) {
  ...
  for (PlayerMails::iterator itr = _player->GetMailBegin(); itr != _player->GetMailEnd(); ++itr) {
    ...
    size_t next_mail_size = 2 + 4 + 1 + 8 + 4 * 8 + ((*itr)->subject.size() + 1) + 1 +
