...
void Player::UpdateAttackPowerAndDamage(bool ranged) {
  ...
  if (ranged) {
    ...
    if (CanDualWield() && hasOffhandWeaponForAttack()) // allow update offhand damage only if player knows DualWield
                                                       // Spec and has equipped offhand weapon
...
uint32 Player::GetEnchantmentModifier(WeaponAttackType attType) { return m_enchantmentFlatMod[attType]; }
...
void Player::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, float &min_damage, float &max_damage,
  ...
  if (IsInFeralForm()) // check if player is druid and in cat or bear forms, non main hand attacks not allowed for this
                       // mode so not check attack type
  {
    uint32 lvl = getLevel();
    if (lvl > 60)
      lvl = 60;
...
void Creature::UpdateDamagePhysical(WeaponAttackType attType) {
  ...
  float att_pwr = (GetModifierValue(attPower, BASE_VALUE) * GetModifierValue(attPower, BASE_PCT) +
                   GetModifierValue(attPower, TOTAL_VALUE)) *
                  GetModifierValue(attPower, TOTAL_PCT);
