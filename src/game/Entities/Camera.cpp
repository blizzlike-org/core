...
void Camera::ReceivePacket(WorldPacket const &data) const { m_owner.SendDirectMessage(data); }
...
void Camera::UpdateVisibilityOf(WorldObject *target) const { m_owner.UpdateVisibilityOf(m_source, target); }

template <class T> void Camera::UpdateVisibilityOf(T *target, UpdateData &data, WorldObjectSet &vis) {
