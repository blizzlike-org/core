...
Object::~Object() {
  ...
  if (m_objectUpdated) {
    sLog.outError("Object::~Object (GUID: %u TypeId: %u) deleted but still have updated status!!", GetGUIDLow(),
                  GetTypeId());
    MANGOS_ASSERT(false);
  }
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData *data, Player *target) const {
  ...
  if (isType(TYPEMASK_UNIT)) {
    if (static_cast<Unit const *>(this)->GetVictim() &&
        static_cast<Unit const *>(this)->hasUnitState(UNIT_STAT_MELEE_ATTACKING))
      updateFlags |= UPDATEFLAG_HAS_ATTACKING_TARGET;
  }

  // DEBUG_LOG("BuildCreateUpdate: update-type: %u, object-type: %u got updateFlags: %X", updatetype, m_objectTypeId,
  // updateFlags);
  ...
  if (isType(TYPEMASK_UNIT)) // unit (creature/player) case
  {
    for (uint16 index = 0; index < m_valuesCount; ++index) {
      if (updateMask->GetBit(index)) {
          ...
          if (GetTypeId() == TYPEID_UNIT) {
            ...
            if (appendValue & UNIT_NPC_FLAG_FLIGHTMASTER) {
              QuestRelationsMapBounds bounds =
                  sObjectMgr.GetCreatureQuestRelationsMapBounds(((Creature *)this)->GetEntry());
          ...
          *data << uint32(m_floatValues[index]);
          ...
          if (!static_cast<const Unit *>(this)->IsFogOfWarVisibleHealth(target)) {
            ...
            case UNIT_FIELD_HEALTH:
              value = uint32(ceil((100.0 * value) / m_uint32Values[UNIT_FIELD_MAXHEALTH]));
              break;
        ...
        // Fog of War: hide stat values for non-allied units according to settings
        else if ((index == UNIT_FIELD_RANGEDATTACKTIME || index == UNIT_FIELD_MINDAMAGE ||
                  index == UNIT_FIELD_MAXDAMAGE || index == UNIT_FIELD_MINOFFHANDDAMAGE ||
                  index == UNIT_FIELD_MAXOFFHANDDAMAGE || (index >= UNIT_FIELD_STAT0 && index < UNIT_FIELD_BASE_MANA) ||
                  ...
                  index == UNIT_FIELD_MAXRANGEDDAMAGE ||
                  (index >= UNIT_FIELD_POWER_COST_MODIFIER && index <= UNIT_FIELD_MAXHEALTHMODIFIER)) &&
        ...
        // Hide special-info for non empathy-casters,
          ...
          if (dynflagsValue & UNIT_DYNFLAG_SPECIALINFO && static_cast<const Unit *>(this)->IsAlive()) {
            ...
            for (Unit::AuraList::const_iterator itr = mAuraEmpathy.begin(); !bIsCaster && itr != mAuraEmpathy.end();
                 ++itr) {
          ...
          if (GetTypeId() == TYPEID_UNIT) {
            ...
            if (creature->IsAlive()) {
              ...
              if (creature->IsInCombat()) {
                ...
                // sLog.outString(">> %s is not in combat so not tapped by %s", this->GetGuidStr().c_str(),
              ...
              if (creature->m_loot && creature->m_loot->CanLoot(target)) {
                ...
                dynflagsValue = dynflagsValue & ~UNIT_DYNFLAG_LOOTABLE;
                // sLog.outString(">> %s is not lootable for %s", this->GetGuidStr().c_str(),
                // target->GetGuidStr().c_str());
              }
            ...
            if (setTapFlags) {
              if (creature->IsTappedBy(target)) {
                ...
                dynflagsValue = dynflagsValue | UNIT_DYNFLAG_TAPPED;
                // sLog.outString(">> %s is not tapped by %s", this->GetGuidStr().c_str(),
                // target->GetGuidStr().c_str());
              }
            }
          }
          ...
          if (GetTypeId() == TYPEID_UNIT || GetTypeId() == TYPEID_PLAYER) {
            Unit const *unit = static_cast<const Unit *>(
                this); // hunters mark effects should only be visible to owners and not all players
...
bool Object::PrintIndexError(uint32 index, bool set) const {
  sLog.outError("Attempt %s nonexistent value field: %u (count: %u) for object typeid: %u type mask: %u",
...
bool Object::PrintEntryError(char const *descr) const {
  sLog.outError("Object Type %u, Entry %u (lowguid %u) with invalid call for %s", GetTypeId(), GetEntry(),
                GetObjectGuid().GetCounter(), descr);
...
void Object::BuildUpdateDataForPlayer(Player *pl, UpdateDataMapType &update_players) const {
  ...
  if (iter == update_players.end()) {
    std::pair<UpdateDataMapType::iterator, bool> p =
        update_players.insert(UpdateDataMapType::value_type(pl, UpdateData()));
...
void Object::AddToClientUpdateList() {
  sLog.outError("Unexpected call of Object::AddToClientUpdateList for object (TypeId: %u Update fields: %u)",
                GetTypeId(), m_valuesCount);
  MANGOS_ASSERT(false);
}

void Object::RemoveFromClientUpdateList() {
  sLog.outError("Unexpected call of Object::RemoveFromClientUpdateList for object (TypeId: %u Update fields: %u)",
                GetTypeId(), m_valuesCount);
  MANGOS_ASSERT(false);
}

void Object::BuildUpdateData(UpdateDataMapType & /*update_players */) {
  sLog.outError("Unexpected call of Object::BuildUpdateData for object (TypeId: %u Update fields: %u)", GetTypeId(),
                m_valuesCount);
  MANGOS_ASSERT(false);
}
...
WorldObject::WorldObject()
    : m_transportInfo(nullptr), m_isOnEventNotified(false), m_visibilityData(this), m_currMap(nullptr), m_mapId(0),
      m_InstanceId(0), m_isActiveObject(false), m_debugFlags(0), m_transport(nullptr) {}

void WorldObject::CleanupsBeforeDelete() { RemoveFromWorld(); }
...
uint32 WorldObject::GetZoneId() const { return GetTerrain()->GetZoneId(m_position.x, m_position.y, m_position.z); }

uint32 WorldObject::GetAreaId() const { return GetTerrain()->GetAreaId(m_position.x, m_position.y, m_position.z); }
...
bool WorldObject::GetDistanceOrder(WorldObject const *obj1, WorldObject const *obj2, bool is3D /* = true */,
                                   DistanceCalculation distcalc /* = NONE */) const {
  float distsq1 = GetDistance(obj1, is3D, distcalc);
  float distsq2 = GetDistance(obj2, is3D, distcalc);
  return distsq1 < distsq2;
}

bool WorldObject::IsInRange(WorldObject const *obj, float minRange, float maxRange, bool is3D /* = true */,
                            bool combat /*= false*/) const {
  float distsq = GetDistance(obj, is3D, DIST_CALC_NONE);
  float sizefactor;
  if (combat)
...
bool WorldObject::IsInRange3d(float x, float y, float z, float minRange, float maxRange,
                              bool combat /*= false*/) const {
  float distsq = GetDistance(x, y, z, DIST_CALC_NONE);
  float sizefactor;
  if (combat)
...
float WorldObject::GetAngle(float x, float y) const { return m_position.GetAngle(x, y); }
...
bool WorldObject::IsFacingTargetsBack(const WorldObject *target, float arc /*= M_PI_F*/) const {
  ...
  // if target is facing the current object then we know its not possible that the current object would be facing the
  // targets back
...
bool WorldObject::IsFacingTargetsFront(const WorldObject *target, float arc /*= M_PI_F*/) const {
  ...
  // if target is not facing the current object then we know its not possible that the current object would be facing
  // the targets front
  if (!target->HasInArc(this, arc))
    return false;
...
bool WorldObject::isInFront(WorldObject const *target, float distance, float arc /*= M_PI_F*/) const {
  return target->GetDistance(GetPositionX(), GetPositionY(), GetPositionZ(), DIST_CALC_COMBAT_REACH) <= distance &&
         HasInArc(target, arc);
}

bool WorldObject::isInBack(WorldObject const *target, float distance, float arc /*= M_PI_F*/) const {
  return target->GetDistance(GetPositionX(), GetPositionY(), GetPositionZ(), DIST_CALC_COMBAT_REACH) <= distance &&
         !HasInArc(target, 2 * M_PI_F - arc);
}

void WorldObject::GetRandomPoint(float x, float y, float z, float distance, float &rand_x, float &rand_y, float &rand_z,
                                 float minDist /*=0.0f*/, float const *ori /*=nullptr*/) const {
  if (distance == 0) {
    rand_x = x;
    rand_y = y;
    rand_z = z;
...
void WorldObject::MonsterSay(const char *text, uint32 /*language*/, Unit const *target) const {
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_SAY, text, LANG_UNIVERSAL, CHAT_TAG_NONE, GetObjectGuid(),
...
void WorldObject::MonsterYell(const char *text, uint32 /*language*/, Unit const *target) const {
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_YELL, text, LANG_UNIVERSAL, CHAT_TAG_NONE, GetObjectGuid(),
                               GetName(), target ? target->GetObjectGuid() : ObjectGuid(),
                               target ? target->GetName() : "");
...
void WorldObject::MonsterTextEmote(const char *text, Unit const *target, bool IsBossEmote) const {
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, IsBossEmote ? CHAT_MSG_RAID_BOSS_EMOTE : CHAT_MSG_MONSTER_EMOTE, text,
                               LANG_UNIVERSAL, CHAT_TAG_NONE, GetObjectGuid(), GetName(),
                               target ? target->GetObjectGuid() : ObjectGuid(), target ? target->GetName() : "");
  SendMessageToSetInRange(
      data, sWorld.getConfig(IsBossEmote ? CONFIG_FLOAT_LISTEN_RANGE_YELL : CONFIG_FLOAT_LISTEN_RANGE_TEXTEMOTE), true);
}
...
void WorldObject::MonsterWhisper(const char *text, Unit const *target, bool IsBossWhisper) const {
  ...
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, IsBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER : CHAT_MSG_MONSTER_WHISPER, text,
                               LANG_UNIVERSAL, CHAT_TAG_NONE, GetObjectGuid(), GetName(), target->GetObjectGuid(),
                               target->GetName());
...
public:
  MonsterChatBuilder(WorldObject const &obj, ChatMsg msgtype, MangosStringLocale const *textData, Language language,
                     Unit const *target)
      : i_object(obj), i_msgtype(msgtype), i_textData(textData), i_language(language), i_target(target),
        m_gender(obj.IsUnit() ? Gender(static_cast<Unit const &>(obj).getGender()) : GENDER_MALE) {}
  void operator()(WorldPacket &data, int32 loc_idx) {
    ...
    ChatHandler::BuildChatPacket(data, i_msgtype, text, i_language, CHAT_TAG_NONE, i_object.GetObjectGuid(),
                                 i_object.GetNameForLocaleIdx(loc_idx),
...
void _DoLocalizedTextAround(WorldObject const *source, MangosStringLocale const *textData, ChatMsg msgtype,
                            Language language, Unit const *target, float range) {
...
void WorldObject::MonsterText(MangosStringLocale const *textData, Unit const *target) const {
  ...
  case CHAT_TYPE_SAY:
    _DoLocalizedTextAround(this, textData, CHAT_MSG_MONSTER_SAY, languageId, target,
                           sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_SAY));
    break;
  case CHAT_TYPE_YELL:
    _DoLocalizedTextAround(this, textData, CHAT_MSG_MONSTER_YELL, languageId, target,
                           sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_YELL));
    break;
  case CHAT_TYPE_TEXT_EMOTE:
    _DoLocalizedTextAround(this, textData, CHAT_MSG_MONSTER_EMOTE, LANG_UNIVERSAL, target,
                           sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_TEXTEMOTE));
    break;
  case CHAT_TYPE_BOSS_EMOTE:
    _DoLocalizedTextAround(this, textData, CHAT_MSG_RAID_BOSS_EMOTE, LANG_UNIVERSAL, target,
                           sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_YELL));
    break;
  ...
  case CHAT_TYPE_ZONE_YELL: {
    ...
    GetMap()->ExecuteMapWorkerZone(zoneId, std::bind(&MaNGOS::LocalizedPacketDo<MaNGOS::MonsterChatBuilder>::operator(),
                                                     &say_do, std::placeholders::_1));
    break;
  }
...
void WorldObject::AddObjectToRemoveList() { GetMap()->AddObjectToRemoveList(this); }
...
Creature *WorldObject::SummonCreature(TempSpawnSettings settings, Map *map) {
  ...
  if (!cinfo) {
    sLog.outErrorDb("WorldObject::SummonCreature: Creature (Entry: %u) not existed for summoner: %s. ", settings.entry,
                    settings.spawner ? settings.spawner->GetGuidStr().c_str() : ObjectGuid().GetString().data());
    return nullptr;
  }
  ...
  if (!settings.tempSpawnMovegen)
    ...
    creature = new TemporarySpawnWaypoint(settings.spawner ? settings.spawner->GetObjectGuid() : ObjectGuid(),
                                          settings.waypointId, settings.spawnPathId, settings.pathOrigin);
...
Creature *WorldObject::SummonCreature(uint32 id, float x, float y, float z, float ang, TempSpawnType spwtype,
                                      uint32 despwtime, bool asActiveObject, bool setRun, uint32 pathId, uint32 faction,
                                      uint32 modelId, bool spawnCounting, bool forcedOnTop) {
  return WorldObject::SummonCreature(TempSpawnSettings(this, id, x, y, z, ang, spwtype, despwtime, asActiveObject,
                                                       setRun, pathId, faction, modelId, spawnCounting, forcedOnTop),
                                     GetMap());
}
...
public:
  NearUsedPosDo(WorldObject const &obj, WorldObject const *searcher, float absAngle, ObjectPosSelector &selector)
      : i_object(obj), i_searcher(searcher), i_absAngle(MapManager::NormalizeOrientation(absAngle)),
        i_selector(selector) {}
  ...
  template <class T> void operator()(T *u) const {
    // skip self or target
...
void WorldObject::GetNearPoint2dAt(const float posX, const float posY, float &x, float &y, float distance2d,
                                   float absAngle) {
...
void WorldObject::GetNearPointAt(const float posX, const float posY, const float posZ, WorldObject const *searcher,
                                 float &x, float &y, float &z, float searcher_bounding_radius, float distance2d,
                                 float absAngle, bool isInWater) const {
...
void WorldObject::PlayDistanceSound(uint32 sound_id,
                                    PlayPacketParameters parameters /*= PlayPacketParameters(PLAY_SET)*/) const {
...
void WorldObject::PlayMusic(uint32 sound_id,
                            PlayPacketParameters parameters /*= PlayPacketParameters(PLAY_SET)*/) const {
...
void WorldObject::PlaySpellVisual(uint32 artKitId,
                                  PlayPacketParameters parameters /*= PlayPacketParameters(PLAY_SET)*/) const {
...
struct WorldObjectChangeAccumulator {
  ...
  WorldObjectChangeAccumulator(WorldObject &obj, UpdateDataMapType &d) : i_updateDatas(d), i_object(obj) {
    ...
    // with new camera system when player's camera too far from player, camera wouldn't receive packets and changes from
    // player
...
bool WorldObject::PrintCoordinatesError(float x, float y, float z, char const *descr) const {
  sLog.outError("%s with invalid %s coordinates: mapid = %uu, x = %f, y = %f, z = %f", GetGuidStr().c_str(), descr,
                GetMapId(), x, y, z);
...
void WorldObject::AddGCD(SpellEntry const &spellEntry, uint32 forcedDuration /*= 0*/, bool /*updateClient = false*/) {
  ...
  m_GCDCatMap.emplace(spellEntry.StartRecoveryCategory,
                      std::chrono::milliseconds(gcdRecTime) + GetMap()->GetCurrentClockTime());
}
...
void WorldObject::AddCooldown(SpellEntry const &spellEntry, ItemPrototype const * /*itemProto = nullptr*/,
  ...
  if (recTimeDuration || spellEntry.CategoryRecoveryTime)
    m_cooldownMap.AddCooldown(GetMap()->GetCurrentClockTime(), spellEntry.Id, recTimeDuration, spellEntry.Category,
...
bool WorldObject::HasGCDOrCooldownWithinMargin(SpellEntry const &spellEntry,
...
void WorldObject::PrintCooldownList(ChatHandler &chat) const {
  ...
  for (auto &cdItr : m_GCDCatMap) {
    ...
    cdLine << "GCD category"
           << "(" << cdItr.first << ") have " << durationStr.str() << " cd";
  ...
  // print spell lockout
  static std::string schoolName[] = {"SPELL_SCHOOL_NORMAL", "SPELL_SCHOOL_HOLY",  "SPELL_SCHOOL_FIRE",
...
int32 WorldObject::CalculateSpellEffectValue(Unit const *target, SpellEntry const *spellProto,
                                             SpellEffectIndex effect_index, int32 const *effBasePoints,
                                             bool maximum) const {
