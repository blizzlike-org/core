...
uint32 VisibilityData::GetInvisibilityMask() const { return m_invisibilityMask; }
...
int32 VisibilityData::GetInvisibilityValue(uint32 index) const { return m_invisibilityValues[index]; }
...
int32 VisibilityData::GetInvisibilityDetectValue(uint32 index) const {
  if (m_owner->isType(TYPEMASK_UNIT)) {
    ...
    if (owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED)) {
      Player const *controller = owner->GetControllingPlayer();
      if (controller)
        return controller->GetVisibilityData()
            .m_invisibilityDetectValues[index]; // directly access value without bypass
    }
  }
