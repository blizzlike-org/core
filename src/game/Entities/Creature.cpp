...
VendorItem const *VendorItemData::FindItem(uint32 item_id) const {
  for (auto m_item : m_items) {
    // Skip checking for conditions, condition system is powerfull enough to not require additional entries only for the
    // conditions
...
bool CreatureCreatePos::Relocate(Creature *cr) const {
  ...
  if (!cr->IsPositionValid()) {
    sLog.outError("%s not created. Suggested coordinates isn't valid (X: %f Y: %f)", cr->GetGuidStr().c_str(),
                  cr->GetPositionX(), cr->GetPositionY());
    return false;
  }
...
Creature::Creature(CreatureSubtype subtype)
    : Unit(), m_lootMoney(0), m_lootGroupRecipientId(0), m_lootStatus(CREATURE_LOOT_STATUS_NONE), m_respawnTime(0),
      m_respawnDelay(25), m_respawnOverriden(false), m_respawnOverrideOnce(false), m_corpseDelay(60), m_canAggro(false),
      m_respawnradius(5.0f), m_subtype(subtype), m_defaultMovementType(IDLE_MOTION_TYPE), m_equipmentId(0),
      m_AlreadyCallAssistance(false), m_isDeadByDefault(false), m_temporaryFactionFlags(TEMPFACTION_NONE),
      m_originalEntry(0), m_dbGuid(0), m_gameEventVendorId(0), m_ai(nullptr), m_isInvisible(false), m_ignoreMMAP(false),
      m_forceAttackingCapability(false), m_noXP(false), m_noLoot(false), m_noReputation(false), m_countSpawns(false),
...
Creature::~Creature() { CleanupsBeforeDelete(); }
...
bool Creature::InitEntry(uint32 Entry, CreatureData const *data /*=nullptr*/,
  ...
  {
    sLog.outErrorDb("Creature (Entry: %u) has no model info defined in table `creature_model_info`, can't load.",
                    Entry);
    return false;
  }
  ...
  if (eventData && eventData->vendor_id) {
    m_gameEventVendorId =
        eventData->vendor_id; // use game event vendor id to override current vendor template id for any active event
  }
  ...
  if (!IsPet() || !static_cast<Pet *>(this)->isControlled())
    SetLevitate(
        (cinfo->InhabitType & INHABIT_AIR) !=
        0); // TODO: may not be correct to send opcode at this point (already handled by UPDATE_OBJECT createObject)

  // check if we need to add swimming movement. TODO: i thing movement flags should be computed automatically at each
  // movement of creature so we need a sort of UpdateMovementFlags() method
  if (cinfo->InhabitType & INHABIT_WATER && // check inhabit type water
      !(cinfo->ExtraFlags &
        CREATURE_EXTRA_FLAG_WALK_IN_WATER) && // check if creature is forced to walk (crabs, giant,...)
      GetMap()->GetTerrain()->IsSwimmable(
          m_respawnPos.x, m_respawnPos.y, m_respawnPos.z,
          GetCollisionHeight()))                       // check if creature is in water and have enough space to swim
...
bool Creature::UpdateEntry(uint32 Entry, const CreatureData *data /*=nullptr*/,
                           GameEventCreatureData const *eventData /*=nullptr*/, bool preserveHPAndPower /*=true*/) {
...
void Creature::ResetEntry(bool respawn) {
  ...
  GameEventCreatureData const *eventData =
      isPet ? nullptr : sGameEventMgr.GetCreatureUpdateDataForActiveEvent(m_dbGuid);
...
uint32 Creature::ChooseDisplayId(const CreatureInfo *cinfo, const CreatureData *data /*= nullptr*/,
                                 GameEventCreatureData const *eventData /*=nullptr*/) {
  ...
  if (cinfo->ModelId[3] && cinfo->ModelId[2] && cinfo->ModelId[1] && cinfo->ModelId[0]) {
    ...
    // Where it's expected to select one of two, model must have a alternative model defined (alternative model is
    // normally the same as defined in ModelId1). Same pattern is used in the above model selection, but the result may
    // be ModelId3 and not ModelId2 as here.
    uint32 modelid_tmp = sObjectMgr.GetCreatureModelAlternativeModel(cinfo->ModelId[1]);
  ...
  if (!display_id) {
    sLog.outErrorDb("Call customer support, ChooseDisplayId can not select native model for creature entry %u, model "
                    "from creature entry 1 will be used instead.",
                    cinfo->Entry);
...
void Creature::Update(const uint32 diff) {
  ...
  case DEAD: {
    if (m_respawnTime <= time(nullptr) &&
        (!m_isSpawningLinked || GetMap()->GetCreatureLinkingHolder()->CanSpawn(this))) {
...
bool Creature::Create(uint32 guidlow, CreatureCreatePos &cPos, CreatureInfo const *cinfo,
                      const CreatureData *data /*= nullptr*/, GameEventCreatureData const *eventData /*= nullptr*/) {
  SetMap(cPos.GetMap());
  ...
  case TRAINER_TYPE_CLASS:
    if (pPlayer->getClass() != GetCreatureInfo()->TrainerClass) {
      if (msg) {
        ...
        case CLASS_DRUID:
          pPlayer->GetPlayerMenu()->SendGossipMenu(4913, GetObjectGuid());
          break;
        case CLASS_HUNTER:
          pPlayer->GetPlayerMenu()->SendGossipMenu(10090, GetObjectGuid());
          break;
        case CLASS_MAGE:
          pPlayer->GetPlayerMenu()->SendGossipMenu(328, GetObjectGuid());
          break;
        case CLASS_PALADIN:
          pPlayer->GetPlayerMenu()->SendGossipMenu(1635, GetObjectGuid());
          break;
        case CLASS_PRIEST:
          pPlayer->GetPlayerMenu()->SendGossipMenu(4436, GetObjectGuid());
          break;
        case CLASS_ROGUE:
          pPlayer->GetPlayerMenu()->SendGossipMenu(4797, GetObjectGuid());
          break;
        case CLASS_SHAMAN:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5003, GetObjectGuid());
          break;
        case CLASS_WARLOCK:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5836, GetObjectGuid());
          break;
        case CLASS_WARRIOR:
          pPlayer->GetPlayerMenu()->SendGossipMenu(4985, GetObjectGuid());
          break;
        }
      }
      return false;
  ...
  case TRAINER_TYPE_MOUNTS:
    if (GetCreatureInfo()->TrainerRace && pPlayer->getRace() != GetCreatureInfo()->TrainerRace) {
      ...
      if (msg) {
        ...
        switch (GetCreatureInfo()->TrainerClass) {
        case RACE_DWARF:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5865, GetObjectGuid());
          break;
        case RACE_GNOME:
          pPlayer->GetPlayerMenu()->SendGossipMenu(4881, GetObjectGuid());
          break;
        case RACE_HUMAN:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5861, GetObjectGuid());
          break;
        case RACE_NIGHTELF:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5862, GetObjectGuid());
          break;
        case RACE_ORC:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5863, GetObjectGuid());
          break;
        case RACE_TAUREN:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5864, GetObjectGuid());
          break;
        case RACE_TROLL:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5816, GetObjectGuid());
          break;
        case RACE_UNDEAD:
          pPlayer->GetPlayerMenu()->SendGossipMenu(624, GetObjectGuid());
          break;
        case RACE_BLOODELF:
          pPlayer->GetPlayerMenu()->SendGossipMenu(5862, GetObjectGuid());
          break;
...
bool Creature::CanInteractWithBattleMaster(Player *pPlayer, bool msg) const {
  ...
  if (!pPlayer->GetBGAccessByLevel(bgTypeId)) {
    ...
    case BATTLEGROUND_AV:
      pPlayer->GetPlayerMenu()->SendGossipMenu(7616, GetObjectGuid());
      break;
    case BATTLEGROUND_WS:
      pPlayer->GetPlayerMenu()->SendGossipMenu(7599, GetObjectGuid());
      break;
    case BATTLEGROUND_AB:
      pPlayer->GetPlayerMenu()->SendGossipMenu(7642, GetObjectGuid());
      break;
    ...
    case BATTLEGROUND_RL:
      pPlayer->GetPlayerMenu()->SendGossipMenu(10024, GetObjectGuid());
      break;
    default:
      break;
    }
...
bool Creature::CanTrainAndResetTalentsOf(Player *pPlayer) const {
  return pPlayer->getLevel() >= 10 && GetCreatureInfo()->TrainerType == TRAINER_TYPE_CLASS &&
...
/**
 * Return original player who tap creature, it can be different from player/group allowed to loot so not use it for loot
 * code
 */
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask) {
  ...
  if (cinfo) {
    ...
    if (displayId != cinfo->ModelId[0] && displayId != cinfo->ModelId[1] && displayId != cinfo->ModelId[2] &&
  ...
  // prevent add data integrity problems
  data.movementType =
      !m_respawnradius && GetDefaultMovementType() == RANDOM_MOTION_TYPE ? IDLE_MOTION_TYPE : GetDefaultMovementType();
  ...
  ss << "INSERT INTO creature VALUES (" << GetGUIDLow() << "," << data.id << "," << data.mapid << ","
     << uint32(data.spawnMask) << "," // cast to prevent save as symbol
     << data.modelid_override << "," << data.equipmentId << "," << data.posX << "," << data.posY << "," << data.posZ
     << "," << data.orientation << "," << data.spawntimesecsmin << "," // respawn time minimum
...
void Creature::SelectLevel(uint32 forcedLevel /*= USE_DEFAULT_DATABASE_LEVEL*/) {
  ...
  if (CreatureClassLvlStats const *cCLS =
    ...
    if (cinfo->DamageMultiplier >= 0) {
      ...
      mainMinDmg = ((cCLS->BaseDamage * cinfo->DamageVariance) + (cCLS->BaseMeleeAttackPower / 14.0f)) *
                   (cinfo->MeleeBaseAttackTime / 1000.0f) * damageMulti;
      mainMaxDmg = ((cCLS->BaseDamage * cinfo->DamageVariance * 1.5f) + (cCLS->BaseMeleeAttackPower / 14.0f)) *
                   (cinfo->MeleeBaseAttackTime / 1000.0f) * damageMulti;
      offMinDmg = mainMinDmg / 2.0f;
      offMaxDmg = mainMaxDmg / 2.0f;
      minRangedDmg = ((cCLS->BaseDamage * cinfo->DamageVariance) + (cCLS->BaseRangedAttackPower / 14.0f)) *
                     (cinfo->RangedBaseAttackTime / 1000.0f) * damageMulti;
      maxRangedDmg = ((cCLS->BaseDamage * cinfo->DamageVariance * 1.5f) + (cCLS->BaseRangedAttackPower / 14.0f)) *
                     (cinfo->RangedBaseAttackTime / 1000.0f) * damageMulti;
  ...
  if (!usedDamageMulti || health == -1 || mana == -1 || armor == -1.f) // some field needs to default to old db fields
  {
    if (forcedLevel == USE_DEFAULT_DATABASE_LEVEL || (forcedLevel >= minlevel && forcedLevel <= maxlevel)) {
      ...
      sLog.outError(
          "Creature::SelectLevel> Error trying to set level(%u) for creature %s without enough data to do it!", level,
          GetGuidStr().c_str());
      // probably wrong
...
bool Creature::CreateFromProto(uint32 guidlow, CreatureInfo const *cinfo, const CreatureData *data /*=nullptr*/,
                               GameEventCreatureData const *eventData /*=nullptr*/) {
...
struct CreatureRespawnDeleteWorker {
  ...
  void operator()(MapPersistentState *state) { state->SaveCreatureRespawnTime(i_guid, 0); }
...
void Creature::SetDeathState(DeathState s) {
  if ((s == JUST_DIED && !m_isDeadByDefault) || (s == JUST_ALIVED && m_isDeadByDefault)) {
    ...
    m_corpseExpirationTime =
        ...
        std::chrono::milliseconds(m_corpseDelay *
                                  IN_MILLISECONDS); // the max/default time for corpse decay (before creature is
                                                    // looted/AllLootRemovedFromCorpse() is called)
  ...
  if (s == JUST_DIED) {
    SetTargetGuid(
        ObjectGuid()); // remove target selection in any cases (can be set at aura remove in Unit::SetDeathState)
...
SpellEntry const *Creature::ReachWithSpellAttack(Unit *pVictim) {
  ...
  for (unsigned int m_spell : m_spells) {
    ...
    for (unsigned int j : spellInfo->Effect) {
      if ((j == SPELL_EFFECT_SCHOOL_DAMAGE) || (j == SPELL_EFFECT_INSTAKILL) ||
          (j == SPELL_EFFECT_ENVIRONMENTAL_DAMAGE) || (j == SPELL_EFFECT_HEALTH_LEECH)) {
        bcontinue = false;
        break;
      }
    }
    ...
    float dist = GetDistance(pVictim, true,
                             spellInfo->rangeIndex == SPELL_RANGE_IDX_COMBAT ? DIST_CALC_COMBAT_REACH_WITH_MELEE
...
void Creature::SaveRespawnTime() {
  ...
  if (m_respawnTime > time(nullptr)) // dead (no corpse)
    ...
    GetMap()->GetPersistentState()->SaveCreatureRespawnTime(
        m_dbGuid, std::chrono::system_clock::to_time_t(m_corpseExpirationTime));
}

CreatureDataAddon const *Creature::GetCreatureAddon() const {
  if (GetObjectGuid().GetHigh() !=
      HIGHGUID_PET) // pets have guidlow that is conflicting with normal guidlows hence GetGUIDLow() gives wrong info
  ...
  if (cainfo->auras) {
    for (uint32 const *cAura = cainfo->auras; *cAura; ++cAura) {
      if (HasAura(*cAura)) {
        if (!reload)
          sLog.outErrorDb(
              "Creature (GUIDLow: %u Entry: %u) has spell %u in `auras` field, but aura is already applied.",
...
void Creature::SetInCombatWithZone(bool checkAttackability) {
  ...
  if (!pMap->IsDungeon()) {
    sLog.outError("Creature entry %u call SetInCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(),
                  pMap->GetId());
    return;
  }
...
bool Creature::MeetsSelectAttackingRequirement(Unit *pTarget, SpellEntry const *pSpellInfo, uint32 selectFlags,
                                               SelectAttackingTargetParams params) const {
  if (selectFlags) {
...
Unit *Creature::SelectAttackingTarget(AttackingTarget target, uint32 position, uint32 spellId, uint32 selectFlags,
                                      SelectAttackingTargetParams params /*= SelectAttackingTargetParams()*/) const {
...
Unit *Creature::SelectAttackingTarget(AttackingTarget target, uint32 position,
                                      SpellEntry const *spellInfo /*= nullptr*/, uint32 selectFlags /*= 0*/,
                                      SelectAttackingTargetParams params /*= SelectAttackingTargetParams()*/) const {
  if (!CanHaveThreatList())
    return nullptr;
...
void Creature::SelectAttackingTargets(std::vector<Unit *> &selectedTargets, AttackingTarget target, uint32 position,
                                      uint32 spellId, uint32 selectFlags /*= 0*/,
                                      SelectAttackingTargetParams params /*= SelectAttackingTargetParams()*/) const {
  SelectAttackingTargets(selectedTargets, target, position, sSpellTemplate.LookupEntry<SpellEntry>(spellId),
                         selectFlags, params);
}
...
void Creature::UpdateSpellSet(uint32 spellSet) {
  ...
  CreatureTemplateSpells const *templateSpells =
      sObjectMgr.GetCreatureTemplateSpellSet(GetCreatureInfo()->Entry, spellSet);
  if (!templateSpells)
...
std::string Creature::GetAIName() const { return ObjectMgr::GetCreatureTemplate(GetEntry())->AIName; }

std::string Creature::GetScriptName() const { return sScriptDevAIMgr.GetScriptName(GetScriptId()); }

uint32 Creature::GetScriptId() const { return ObjectMgr::GetCreatureTemplate(GetEntry())->ScriptID; }
...
void Creature::ClearTemporaryFaction() {
  ...
  setFaction(GetCreatureInfo()->Faction);
  // Reset UNIT_FLAG_NON_ATTACKABLE, UNIT_FLAG_IMMUNE_TO_PLAYER, UNIT_FLAG_IMMUNE_TO_NPC, UNIT_FLAG_PACIFIED or
  // UNIT_FLAG_NOT_SELECTABLE flags
  if (m_temporaryFactionFlags & TEMPFACTION_TOGGLE_NON_ATTACKABLE &&
      GetCreatureInfo()->UnitFlags & UNIT_FLAG_NON_ATTACKABLE)
    SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
  if (m_temporaryFactionFlags & TEMPFACTION_TOGGLE_IMMUNE_TO_PLAYER &&
      GetCreatureInfo()->UnitFlags & UNIT_FLAG_IMMUNE_TO_PLAYER)
    SetImmuneToPlayer(false);
  if (m_temporaryFactionFlags & TEMPFACTION_TOGGLE_IMMUNE_TO_NPC &&
      GetCreatureInfo()->UnitFlags & UNIT_FLAG_IMMUNE_TO_NPC)
    SetImmuneToNPC(false);
  ...
  if (m_temporaryFactionFlags & TEMPFACTION_TOGGLE_NOT_SELECTABLE &&
      GetCreatureInfo()->UnitFlags & UNIT_FLAG_NOT_SELECTABLE)
...
void Creature::SetVirtualItem(VirtualItemSlot slot, uint32 item_id) {
  ...
  if (!proto) {
    sLog.outError("Not listed in 'item_template' item (ID:%u) used as virtual item for %s", item_id,
                  GetGuidStr().c_str());
    return;
  }
...
void Creature::ReduceCorpseDecayTimer() {
  ...
  if (!isDungeonEncounter && m_corpseExpirationTime > GetMap()->GetCurrentClockTime() +
                                                          std::chrono::milliseconds(2 * MINUTE * IN_MILLISECONDS))
    m_corpseExpirationTime = GetMap()->GetCurrentClockTime() +
                             std::chrono::milliseconds(2 * MINUTE * IN_MILLISECONDS); // 2 minutes for a creature
}
...
bool Creature::CanRestockPickpocketLoot() const { return GetMap()->GetCurrentClockTime() >= m_pickpocketRestockTime; }

void Creature::StartPickpocketRestockTimer() {
  m_pickpocketRestockTime =
      GetMap()->GetCurrentClockTime() +
      std::chrono::milliseconds(sWorld.getConfig(CONFIG_UINT32_CREATURE_PICKPOCKET_RESTOCK_DELAY) * IN_MILLISECONDS);
}

bool Creature::HasBeenHitBySpell(uint32 spellId) { return m_hitBySpells.find(spellId) != m_hitBySpells.end(); }

void Creature::RegisterHitBySpell(uint32 spellId) { m_hitBySpells.insert(spellId); }

void Creature::UnregisterHitBySpell(uint32 spellId) { m_hitBySpells.erase(spellId); }

void Creature::ResetSpellHitCounter() { m_hitBySpells.clear(); }

void Creature::AddCooldown(SpellEntry const &spellEntry, ItemPrototype const * /*itemProto*/, bool /*permanent*/,
                           uint32 forcedDuration) {
  ...
  if (recTime || spellEntry.CategoryRecoveryTime) {
    ...
    m_cooldownMap.AddCooldown(GetMap()->GetCurrentClockTime(), spellEntry.Id, recTime, spellEntry.Category,
                              categoryRecTime);
