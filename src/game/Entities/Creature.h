...
enum CreatureExtraFlags {
  ...
  CREATURE_EXTRA_FLAG_ACTIVE =
      0x00001000, // 4096 creature is active object. Grid of this creature will be loaded and creature set as active
  ...
  CREATURE_EXTRA_FLAG_FORCE_ATTACKING_CAPABILITY =
      0x00080000, // 524288 SetForceAttackingCapability(true); for nonattackable, nontargetable creatures that should be
                  // able to attack nontheless
  ...
  CREATURE_EXTRA_FLAG_HASTE_SPELL_IMMUNITY =
      0x00400000, // 4194304 immunity to COT or Mind Numbing Poison - very common in instances
...
// GCC have alternative #pragma pack(N) syntax and old gcc version not support pack(push,N), also any gcc version not
// support it at some platform
#if defined(__GNUC__)
#pragma pack(1)
#else
#pragma pack(push, 1)
...
#define USE_DEFAULT_DATABASE_LEVEL                                                                                     \
  0 // just used to show we don't want to force the new creature level and use the level stored in db
...
// GCC have alternative #pragma pack() syntax and old gcc version not support pack(pop), also any gcc version not
// support it at some platform
#if defined(__GNUC__)
#pragma pack()
#else
...
struct TrainerSpell {
  TrainerSpell()
      : spell(0), spellCost(0), reqSkill(0), reqSkillValue(0), reqLevel(0), learnedSpell(0), conditionId(0),
  ...
  TrainerSpell(uint32 _spell, uint32 _spellCost, uint32 _reqSkill, uint32 _reqSkillValue, uint32 _reqLevel,
               uint32 _learnedspell, bool _isProvidedReqLevel, uint32 _conditionId)
      : spell(_spell), spellCost(_spellCost), reqSkill(_reqSkill), reqSkillValue(_reqSkillValue), reqLevel(_reqLevel),
        learnedSpell(_learnedspell), conditionId(_conditionId), isProvidedReqLevel(_isProvidedReqLevel) {}
...
public:
  ...
  // if dist == 0.0f -> exactly object coordinates used, in other case close point to object (CONTACT_DIST can be used
  // as minimal distances)
  CreatureCreatePos(WorldObject *closeObject, float ori, float dist = 0.0f, float angle = 0.0f)
      : m_map(closeObject->GetMap()), m_closeObject(closeObject), m_angle(angle), m_dist(dist) {
    m_pos.o = ori;
  }
...
{
  TEMPFACTION_NONE = 0x00, // When no flag is used in temporary faction change, faction will be persistent. It will then
                           // require manual change back to default/another faction when changed once
  TEMPFACTION_RESTORE_RESPAWN = 0x01, // Default faction will be restored at respawn
  TEMPFACTION_RESTORE_COMBAT_STOP =
      0x02, // ... at CombatStop() (happens at creature death, at evade or custom scripte among others)
  ...
  TEMPFACTION_TOGGLE_NON_ATTACKABLE =
      0x08, // Remove UNIT_FLAG_NON_ATTACKABLE(0x02) when faction is changed (reapply when temp-faction is removed)
  TEMPFACTION_TOGGLE_IMMUNE_TO_PLAYER =
      0x10, // Remove UNIT_FLAG_IMMUNE_TO_PLAYER(0x100) when faction is changed (reapply when temp-faction is removed)
  TEMPFACTION_TOGGLE_IMMUNE_TO_NPC =
      0x20, // Remove UNIT_FLAG_IMMUNE_TO_NPC(0x200) when faction is changed (reapply when temp-faction is removed)
  TEMPFACTION_TOGGLE_PACIFIED =
      0x40, // Remove UNIT_FLAG_PACIFIED(0x20000) when faction is changed (reapply when temp-faction is removed)
  TEMPFACTION_TOGGLE_NOT_SELECTABLE =
      0x80, // Remove UNIT_FLAG_NOT_SELECTABLE(0x2000000) when faction is changed (reapply when temp-faction is removed)
  TEMPFACTION_ALL,
};
...
public:
  ...
  bool Create(uint32 guidlow, CreatureCreatePos &cPos, CreatureInfo const *cinfo, const CreatureData *data = nullptr,
  ...
  bool CanFly() const override {
    return (GetCreatureInfo()->InhabitType & INHABIT_AIR) ||
           m_movementInfo.HasMovementFlag((MovementFlags)(MOVEFLAG_LEVITATING | MOVEFLAG_HOVER | MOVEFLAG_CAN_FLY));
  }
  bool IsFlying() const override {
    return m_movementInfo.HasMovementFlag((MovementFlags)(MOVEFLAG_FLYING | MOVEFLAG_HOVER | MOVEFLAG_LEVITATING));
  }
  ...
  uint32
  GetLevelForTarget(Unit const *target) const override; // overwrite Unit::GetLevelForTarget for boss level support

  bool AIM_Initialize();

  virtual UnitAI *AI() override {
    if (m_charmInfo && m_charmInfo->GetAI())
  ...
  bool UpdateEntry(uint32 Entry, const CreatureData *data = nullptr, GameEventCreatureData const *eventData = nullptr,
  ...
  static float
  _GetHealthMod(int32 Rank); ///< Get custom factor to scale health (default 1, CONFIG_FLOAT_RATE_CREATURE_*_HP)
  static float _GetDamageMod(int32 Rank); ///< Get custom factor to scale damage (default 1, CONFIG_FLOAT_RATE_*_DAMAGE)
  static float _GetSpellDamageMod(
      int32 Rank); ///< Get custom factor to scale spell damage (default 1, CONFIG_FLOAT_RATE_*_SPELLDAMAGE)
  ...
  static uint32 ChooseDisplayId(const CreatureInfo *cinfo, const CreatureData *data = nullptr,
                                GameEventCreatureData const *eventData = nullptr);
  ...
  } // do not use this from scripts
  void Respawn();
  void SaveRespawnTime() override;

  uint32 GetRespawnDelay() const { return m_respawnDelay; }
  void SetRespawnDelay(uint32 delay, bool once = false) {
  ...
  Unit *SelectAttackingTarget(AttackingTarget target, uint32 position, uint32 spellId, uint32 selectFlags = 0,
                              SelectAttackingTargetParams params = SelectAttackingTargetParams()) const;
  Unit *SelectAttackingTarget(AttackingTarget target, uint32 position, SpellEntry const *spellInfo = nullptr,
                              uint32 selectFlags = 0,
                              SelectAttackingTargetParams params = SelectAttackingTargetParams()) const;
  void SelectAttackingTargets(std::vector<Unit *> &selectedTargets, AttackingTarget target, uint32 position,
  ...
  bool hasWeaponForAttack(WeaponAttackType type) const override {
    return (Unit::hasWeaponForAttack(type) && hasWeapon(type));
  }
  ...
  void OnEventHappened(uint16 eventId, bool activate, bool resume) override {
    return AI()->OnEventHappened(eventId, activate, resume);
  }
...
protected:
  bool MeetsSelectAttackingRequirement(Unit *pTarget, SpellEntry const *pSpellInfo, uint32 selectFlags,
                                       SelectAttackingTargetParams params) const;

  bool CreateFromProto(uint32 guidlow, CreatureInfo const *cinfo, const CreatureData *data = nullptr,
                       GameEventCreatureData const *eventData = nullptr);
  ...
  uint32 m_lootMoney;
  ObjectGuid
      m_lootRecipientGuid; // player who will have rights for looting if m_lootGroupRecipient==0 or group disbanded
...
public:
  ForcedDespawnDelayEvent(Creature &owner, bool onlyAlive = false)
      : BasicEvent(), m_owner(owner), m_onlyAlive(onlyAlive) {}
