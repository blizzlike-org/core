...
void MapManager::LoadTransports() {
  ...
  do {
    ...
    if (!goinfo) {
      sLog.outErrorDb("Transport ID:%u, Name: %s, will not be loaded, gameobject_template missing", entry,
                      name.c_str());
      continue;
    }

    if (goinfo->type != GAMEOBJECT_TYPE_MO_TRANSPORT) {
      sLog.outErrorDb("Transport ID:%u, Name: %s, will not be loaded, gameobject_template type wrong", entry,
                      name.c_str());
      continue;
    }
    ...
    if (!transportTemplate) {
      sLog.outErrorDb("Transport ID:%u, Name: %s, will not be loaded, transport template missing (core generated)",
                      entry, name.c_str());
      continue;
    }
  ...
  result = WorldDatabase.Query("SELECT gameobject.guid,gameobject.id,transports.name FROM gameobject,transports WHERE "
                               "gameobject.id = transports.entry");
  if (result) // wrong data found
  {
    do {
      ...
      sLog.outErrorDb("Transport %u '%s' have record (GUID: %u) in `gameobject`. Transports MUST NOT have any records "
                      "in `gameobject` or its behavior will be unpredictable/bugged.",
...
Transport::Transport(TransportTemplate const &transportTemplate)
    : GenericTransport(), m_transportTemplate(transportTemplate), m_isMoving(true), m_pendingStop(false) {
  // 2.3.2 - 0x5A
...
bool Transport::Create(uint32 guidlow, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress) {
  ...
  if (!goinfo) {
    sLog.outErrorDb("Transport not created: entry in `gameobject_template` not found, guidlow: %u map: %u  (X: %f Y: "
                    "%f Z: %f) ang: %f",
...
void Transport::TeleportTransport(uint32 newMapid, float x, float y, float z, float o) {
  ...
  // we need to create and save new Map object with 'newMapid' because if not done -> lead to invalid Map object
  // reference... player far teleport would try to create same instance, but we need it NOW for transport... correct me
  // if I'm wrong O.o
  if (mapChange) {
    RemoveModelFromMap();
...
bool GenericTransport::AddPassenger(Unit *passenger, bool adjustCoords) {
  if (m_passengers.find(passenger) == m_passengers.end()) {
    ...
    if (changedTransports && adjustCoords) {
      ...
      CalculatePassengerOffset(passenger->m_movementInfo.t_pos.x, passenger->m_movementInfo.t_pos.y,
                               passenger->m_movementInfo.t_pos.z, &passenger->m_movementInfo.t_pos.o);
    }
...
bool GenericTransport::RemovePassenger(Unit *passenger) {
  ...
  if (erased) {
    ...
    if (Pet *pet = passenger->GetPet()) {
      RemovePassenger(pet);
      pet->NearTeleportTo(passenger->m_movementInfo.pos.x, passenger->m_movementInfo.pos.y,
                          passenger->m_movementInfo.pos.z, passenger->m_movementInfo.pos.o);
    }
...
bool GenericTransport::AddPetToTransport(Unit *passenger, Pet *pet) {
  if (AddPassenger(pet)) {
    pet->m_movementInfo.SetTransportData(GetObjectGuid(), passenger->m_movementInfo.t_pos.x,
                                         passenger->m_movementInfo.t_pos.y, passenger->m_movementInfo.t_pos.z,
                                         passenger->m_movementInfo.t_pos.o, GetPathProgress());
    pet->NearTeleportTo(passenger->m_movementInfo.pos.x, passenger->m_movementInfo.pos.y,
                        passenger->m_movementInfo.pos.z, passenger->m_movementInfo.pos.o);
    return true;
  }
...
void Transport::Update(const uint32 /*diff*/) {
  ...
  while (true) {
    ...
    if (m_currentFrame->Node->mapid != GetMapId() || m_currentFrame->IsTeleportFrame()) {
      TeleportTransport(m_nextFrame->Node->mapid, m_nextFrame->Node->x, m_nextFrame->Node->y, m_nextFrame->Node->z,
                        m_nextFrame->InitialOrientation);
      return;
    }
    ...
    DETAIL_FILTER_LOG(LOG_FILTER_TRANSPORT_MOVES, "%s moved to %f %f %f %d", GetName(), m_currentFrame->Node->x,
                      m_currentFrame->Node->y, m_currentFrame->Node->z, m_currentFrame->Node->mapid);
  }
...
bool ElevatorTransport::Create(uint32 guidlow, uint32 name_id, Map *map, float x, float y, float z, float ang,
                               float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress,
                               GOState go_state) {
  if (GenericTransport::Create(guidlow, name_id, map, x, y, z, ang, rotation0, rotation1, rotation2, rotation3,
                               animprogress, go_state)) {
    m_pathProgress = 0;
...
void ElevatorTransport::Update(const uint32 /*diff*/) {
  ...
  if (nodeNext && nodePrev) {
    ...
    if (posPrev == posNext)
      ...
      float velocityX = float(segmentDiff.x) / timeDiff, velocityY = float(segmentDiff.y) / timeDiff,
            velocityZ = float(segmentDiff.z) / timeDiff;
...
void GenericTransport::UpdatePassengerPosition(WorldObject *passenger) {
  ...
  if (!MaNGOS::IsValidMapCoord(x, y, z)) {
    sLog.outError("[TRANSPORTS] Object %s [guid %u] has invalid position on transport.", passenger->GetName(),
                  passenger->GetGUIDLow());
    return;
  }
...
void GenericTransport::CalculatePassengerPosition(float &x, float &y, float &z, float *o, float transX, float transY,
                                                  float transZ, float transO) {
...
void GenericTransport::CalculatePassengerOffset(float &x, float &y, float &z, float *o, float transX, float transY,
                                                float transZ, float transO) {
  if (o)
...
void Transport::DoEventIfAny(TaxiPathNodeEntry const &node, bool departure) {
  if (uint32 eventid = departure ? node.departureEventID : node.arrivalEventID) {
    DEBUG_FILTER_LOG(LOG_FILTER_TRANSPORT_MOVES, "Taxi %s event %u of node %u of %s \"%s\") path",
                     departure ? "departure" : "arrival", eventid, node.index, GetGuidStr().c_str(), GetName());
