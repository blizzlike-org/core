...
bool CheckPathToTarget(Unit *pet, Unit *target) {
  ...
  float x, y, z;
  target->GetNearPoint(pet, x, y, z, pet->GetObjectBoundingRadius(), pet->AI()->GetAttackDistance(), angle,
                       target->IsInWater());
  ...
  for (size_t i = 1; i < path.size(); ++i) {
    ...
    if (!pet->GetMap()->IsInLineOfSight(dataPrevious.x, dataPrevious.y, dataPrevious.z + collisionHeight, data.x,
                                        data.y, data.z + collisionHeight, true))
      return false;
  }
...
void WorldSession::HandlePetAction(WorldPacket &recv_data) {
  ...
  DETAIL_LOG("HandlePetAction: %s flag is %u, spellid is %u, target %s.", petGuid.GetString().c_str(), uint32(flag),
             spellid, targetGuid.GetString().c_str());
  ...
  if (_player->GetObjectGuid() != petUnit->GetMasterGuid()) {
    sLog.outError("HandlePetAction: %s isn't controlled by %s.", petGuid.GetString().c_str(),
  ...
  if (!charmInfo) {
    sLog.outError("WorldSession::HandlePetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have "
                  "a charminfo!",
                  petUnit->GetGUIDLow(), petUnit->GetTypeId());
    return;
  }
  ...
  if (!pet) {
    if (petUnit->hasUnitState(UNIT_STAT_POSSESSED)) {
      ...
      if (flag != uint8(ACT_COMMAND)) {
        sLog.outError("PetHAndler: unknown PET flag Action %i and spellid %i. For possessed %s", uint32(flag), spellid,
                      petUnit->GetGuidStr().c_str());
        return;
      }
      ...
      case COMMAND_DISMISS:
        ...
        break;
      default:
        sLog.outError("PetHandler: Not allowed action %i and spellid %i. Pet %s owner is %s", uint32(flag), spellid,
                      petUnit->GetGuidStr().c_str(), _player->GetGuidStr().c_str());
        break;
      }
    }
  ...
  case ACT_ENABLED:  // 0xC1    spell
  {
    ...
    for (unsigned int i : spellInfo->EffectImplicitTargetA) {
      if (i == TARGET_ENUM_UNITS_ENEMY_AOE_AT_SRC_LOC || i == TARGET_ENUM_UNITS_ENEMY_AOE_AT_DEST_LOC ||
    ...
    if (unit_target &&
        !(petUnit->IsWithinDistInMap(unit_target, sRange->maxRange) && petUnit->IsWithinLOSInMap(unit_target, true)) &&
    ...
    if (result == SPELL_CAST_OK) {
      ...
      // actually this only seems to happen on special spells, fire shield for imp, torment for voidwalker, but it's
      // stupid to check every spell
      if (petUnit->GetTypeId() == TYPEID_UNIT)
        if (static_cast<Creature *>(petUnit)->IsPet() && (static_cast<Pet *>(petUnit)->getPetType() == SUMMON_PET) &&
            (petUnit != unit_target) && (urand(0, 100) < 10))
...
void WorldSession::HandlePetStopAttack(WorldPacket &recv_data) {
  ...
  if (_player->GetObjectGuid() != pet->GetMasterGuid()) {
    sLog.outError("HandlePetStopAttack: %s isn't charm/pet of %s.", petGuid.GetString().c_str(),
                  _player->GetGuidStr().c_str());
    return;
  }
...
void WorldSession::SendPetNameQuery(ObjectGuid petguid, uint32 petnumber) const {
  ...
  if (!pet || !pet->GetCharmInfo() ||
      (pet->GetUInt32Value(UNIT_FIELD_PETNUMBER) && pet->GetUInt32Value(UNIT_FIELD_PETNUMBER) != petnumber)) {
...
void WorldSession::HandlePetSetAction(WorldPacket &recv_data) {
  ...
  CharmInfo *charmInfo = petUnit->GetCharmInfo();
  if (!charmInfo) {
    sLog.outError("WorldSession::HandlePetSetAction: %s is considered pet-like but doesn't have a charminfo!",
                  petUnit->GetGuidStr().c_str());
    return;
  }
  ...
  // check swap (at command->spell swap client remove spell first in another packet, so check only command move
  // correctness)
  if (move_command) {
  ...
  for (uint8 i = 0; i < count; ++i) {
    ...
    DETAIL_LOG("Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X", _player->GetName(),
               position[i], spell_id, uint32(act_state));

    // if it's act for spell (en/disable/cast) and there is a spell given (0 = remove spell) which pet doesn't know,
    // don't add
    if (!((act_state == ACT_ENABLED || act_state == ACT_DISABLED || act_state == ACT_PASSIVE) && spell_id &&
          !petUnit->HasSpell(spell_id))) {
      // sign for autocast
...
void WorldSession::HandlePetRename(WorldPacket &recv_data) {
  ...
  // check it!
  if (!pet || pet->getPetType() != HUNTER_PET || !pet->HasByteFlag(UNIT_FIELD_BYTES_2, 2, UNIT_CAN_BE_RENAMED) ||
  ...
  if (isdeclined) {
    ...
    CharacterDatabase.PExecute("DELETE FROM character_pet_declinedname WHERE owner = '%u' AND id = '%u'",
                               _player->GetGUIDLow(), pet->GetCharmInfo()->GetPetNumber());
    CharacterDatabase.PExecute("INSERT INTO character_pet_declinedname (id, owner, genitive, dative, accusative, "
                               "instrumental, prepositional) VALUES ('%u','%u','%s','%s','%s','%s','%s')",
                               pet->GetCharmInfo()->GetPetNumber(), _player->GetGUIDLow(), declinedname.name[0].c_str(),
                               declinedname.name[1].c_str(), declinedname.name[2].c_str(), declinedname.name[3].c_str(),
                               declinedname.name[4].c_str());
  }

  CharacterDatabase.escape_string(name);
  CharacterDatabase.PExecute("UPDATE character_pet SET name = '%s', renamed = '1' WHERE owner = '%u' AND id = '%u'",
                             name.c_str(), _player->GetGUIDLow(), pet->GetCharmInfo()->GetPetNumber());
...
void WorldSession::HandlePetUnlearnOpcode(WorldPacket &recvPacket) {
  ...
  if (!pet || guid != pet->GetObjectGuid()) {
    sLog.outError("HandlePetUnlearnOpcode. %s isn't pet of %s .", guid.GetString().c_str(),
                  _player->GetGuidStr().c_str());
    return;
  }
  ...
  if (!charmInfo) {
    sLog.outError("WorldSession::HandlePetUnlearnOpcode: %s is considered pet-like but doesn't have a charminfo!",
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket &recvPacket) {
  ...
  if (!petUnit || (guid != _player->GetPetGuid() && !_player->HasCharm(guid))) {
    sLog.outError("HandlePetSpellAutocastOpcode. %s isn't pet of %s .", guid.GetString().c_str(),
                  _player->GetGuidStr().c_str());
    return;
  }
  ...
  CharmInfo *charmInfo = petUnit->GetCharmInfo();
  if (!charmInfo) {
    sLog.outError("WorldSession::HandlePetSpellAutocastOpcod: %s is considered pet-like but doesn't have a charminfo!",
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket &recvPacket) {
  ...
  if (spell->SpellStart(&targets) == SPELL_CAST_OK) {
    ...
    if (petUnit->GetTypeId() == TYPEID_UNIT)
      if (static_cast<Creature *>(petUnit)->IsPet() && (static_cast<Pet *>(petUnit)->getPetType() == SUMMON_PET) &&
          (petUnit != targets.getUnitTarget()) && (urand(0, 100) < 10))
        petUnit->SendPetTalk((uint32)PET_TALK_SPECIAL_SPELL);
  }
}
