...
#define MAX_VISIBLE_ITEM_OFFSET                                                                                        \
  16 // 16 fields per visible item (creator(2) + enchantments(12) + properties(1) + pad(1))

#define MAX_GEM_SOCKETS                                                                                                \
  MAX_ITEM_PROTO_SOCKETS // (BONUS_ENCHANTMENT_SLOT-SOCK_ENCHANTMENT_SLOT) and item proto size, equal value expected
...
enum ItemDynFlags {
  ...
  ITEM_DYNFLAG_UNLOCKED =
      0x00000004, // have meaning only for item with proto->LockId, if not set show as "Locked, req. lockpicking N"
  ...
  ITEM_DYNFLAG_READABLE =
      0x00000200, // can be open for read, it or item proto pagetText make show "Right click to read"
...
enum ItemRequiredTargetType { ITEM_TARGET_TYPE_CREATURE = 1, ITEM_TARGET_TYPE_DEAD = 2 };
