...
struct PlayPacketParameters {
  ...
  PlayPacketParameters(PlayPacketSettings setting, Player const *target) : setting(setting) {
...
public:
  CooldownData(TimePoint clockNow, uint32 spellId, uint32 duration, uint32 spellCategory, uint32 categoryDuration,
               uint32 itemId = 0, bool isPermanent = false)
      : m_spellId(spellId), m_category(spellCategory),
        m_expireTime(duration ? std::chrono::milliseconds(duration) + clockNow : TimePoint()),
        m_catExpireTime((spellCategory && categoryDuration) ? (std::chrono::milliseconds(categoryDuration) + clockNow)
                                                            : TimePoint()),
        m_typePermanent(isPermanent), m_itemId(itemId) {}
...
public:
  ...
  bool AddCooldown(TimePoint clockNow, uint32 spellId, uint32 duration, uint32 spellCategory = 0,
                   uint32 categoryDuration = 0, uint32 itemId = 0, bool onHold = false) {
    RemoveBySpellId(spellId);
    auto resultItr = m_spellIdMap.emplace(
        spellId, std::move(std::unique_ptr<CooldownData>(
                     new CooldownData(clockNow, spellId, duration, spellCategory, categoryDuration, itemId, onHold))));
...
struct WorldLocation {
  ...
  WorldLocation(WorldLocation const &loc)
      : mapid(loc.mapid), coord_x(loc.coord_x), coord_y(loc.coord_y), coord_z(loc.coord_z),
        orientation(loc.orientation) {}
  WorldLocation(uint32 mapId, Position const &pos)
      : mapid(mapId), coord_x(pos.x), coord_y(pos.y), coord_z(pos.z), orientation(pos.o) {}
...
struct TempSpawnSettings {
  ...
  TempSpawnSettings() {}
  TempSpawnSettings(WorldObject *spawner, uint32 entry, float x, float y, float z, float ori, TempSpawnType spawnType,
                    uint32 despawnTime, bool activeObject = false, bool setRun = false, uint32 pathId = 0,
                    uint32 faction = 0, uint32 modelId = 0, bool spawnCounting = false, bool forcedOnTop = false,
                    bool spellId = 0, int32 movegen = -1)
      : spawner(spawner), entry(entry), x(x), y(y), z(z), ori(ori), spawnType(spawnType), despawnTime(despawnTime),
        activeObject(activeObject), setRun(setRun), pathId(pathId), faction(faction), modelId(modelId),
        spawnCounting(spawnCounting), forcedOnTop(forcedOnTop), spellId(spellId), movegen(movegen) {}
};
...
MovementFlags const movementFlagsMask =
    MovementFlags(MOVEFLAG_FORWARD | MOVEFLAG_BACKWARD | MOVEFLAG_STRAFE_LEFT | MOVEFLAG_STRAFE_RIGHT |
                  MOVEFLAG_PITCH_UP | MOVEFLAG_PITCH_DOWN | MOVEFLAG_FALLING | MOVEFLAG_FALLINGFAR |
                  MOVEFLAG_ASCENDING | MOVEFLAG_FLYING | MOVEFLAG_SPLINE_ELEVATION);

MovementFlags const movementFlagsIgnoreFlyingMask = MovementFlags(
    MOVEFLAG_FORWARD | MOVEFLAG_BACKWARD | MOVEFLAG_STRAFE_LEFT | MOVEFLAG_STRAFE_RIGHT | MOVEFLAG_PITCH_UP |
    MOVEFLAG_PITCH_DOWN | MOVEFLAG_FALLING | MOVEFLAG_FALLINGFAR | MOVEFLAG_ASCENDING | MOVEFLAG_SPLINE_ELEVATION);
...
public:
  MovementInfo()
      : moveFlags(MOVEFLAG_NONE), moveFlags2(0), ctime(0), stime(0), t_time(0), s_pitch(0.0f), fallTime(0),
        u_unk1(0.0f) {}
...
public:
  ...
  Position const &GetPosition(GenericTransport *transport = nullptr) const {
    if (transport)
      return m_movementInfo.GetTransportPos();
    return m_position;
  }
  ...
  static void GetNearPoint2dAt(const float posX, const float posY, float &x, float &y, float distance2d,
                               float absAngle);
   ...
   */
  inline void GetNearPoint(WorldObject const *searcher, float &x, float &y, float &z, float searcher_bounding_radius,
                           float distance2d, float absAngle, bool isInWater = false) const {
    return GetNearPointAt(GetPositionX(), GetPositionY(), GetPositionZ(), searcher, x, y, z, searcher_bounding_radius,
                          distance2d, absAngle, isInWater);
  }
  void GetNearPointAt(const float posX, const float posY, const float posZ, WorldObject const *searcher, float &x,
                      float &y, float &z, float searcher_bounding_radius, float distance2d, float absAngle,
                      bool isInWater = false) const;
   ...
   * @param distance2d        -           range in which to find a free spot. Default = 0.0f (which usually means the
   * units will have contact)
   * @param angle             -           direction in which to look for a free spot. Default = 0.0f (direction in which
   * 'this' is looking
   * @param obj               -           for whom to look for a spot. Default = nullptr
   */
  inline void GetClosePoint(float &x, float &y, float &z, float bounding_radius, float distance2d = 0.0f,
                            float angle = 0.0f, const WorldObject *obj = nullptr) const {
    // angle calculated from current orientation
    GetNearPoint(obj, x, y, z, bounding_radius, distance2d + GetObjectBoundingRadius() + bounding_radius,
                 GetOrientation() + angle);
  }
  /** Gives a "free" spot for a searcher in contact-range of "this" (including bounding-radius calculation)
   * @param x, y, z           -           position for the found spot
   * @param obj               -           for whom to find a contact position. The position will be searched in
   * direction from 'this' towards 'obj'
   * @param distance2d        -           distance which 'obj' and 'this' should have beetween their bounding radiuses.
   * Default = CONTACT_DISTANCE
   */
  inline void GetContactPoint(const WorldObject *obj, float &x, float &y, float &z,
                              float distance2d = CONTACT_DISTANCE) const {
    // angle to face `obj` to `this` using distance includes size of `obj`
    GetNearPoint(obj, x, y, z, obj->GetObjectBoundingRadius(),
                 distance2d + GetObjectBoundingRadius() + obj->GetObjectBoundingRadius(), GetAngle(obj));
  }
  ...
  void GetRandomPoint(float x, float y, float z, float distance, float &rand_x, float &rand_y, float &rand_z,
  ...
  float GetDistance(const WorldObject *obj, bool is3D = true,
                    DistanceCalculation distcalc = DIST_CALC_BOUNDING_RADIUS) const;
  float GetDistance(float x, float y, float z, DistanceCalculation distcalc = DIST_CALC_BOUNDING_RADIUS,
                    bool transport = false) const;
  ...
  bool GetDistanceOrder(WorldObject const *obj1, WorldObject const *obj2, bool is3D = true,
                        DistanceCalculation distcalc = DIST_CALC_NONE) const;
  ...
  virtual void CleanupsBeforeDelete(); // used in destructor or explicitly before mass creature delete to remove
  ...
  Creature *SummonCreature(uint32 id, float x, float y, float z, float ang, TempSpawnType spwtype, uint32 despwtime,
                           bool asActiveObject = false, bool setRun = false, uint32 pathId = 0, uint32 faction = 0,
  ...
  virtual bool CanAttackSpell(Unit const * /*target*/, SpellEntry const * /*spellInfo*/ = nullptr,
                              bool /*isAOE*/ = false) const {
    return true;
  }
  virtual bool CanAssistSpell(Unit const * /*target*/, SpellEntry const * /*spellInfo*/ = nullptr) const {
    return true;
  }

  int32 CalculateSpellEffectValue(Unit const *target, SpellEntry const *spellProto, SpellEffectIndex effect_index,
                                  int32 const *basePoints = nullptr, bool maximum = false) const;
