...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket &recv_data) {
  ...
  if (!unit) {
    DEBUG_LOG("WORLD: HandleTabardVendorActivateOpcode - %s not found or you can't interact with him.",
...
static void SendTrainerSpellHelper(WorldPacket &data, TrainerSpell const *tSpell, TrainerSpellState state,
                                   float fDiscountMod, bool /*can_learn_primary_prof*/, uint32 reqLevel) {
...
void WorldSession::SendTrainerList(ObjectGuid guid) const {
  ...
  if (cSpells) {
    for (const auto &itr : cSpells->spellList) {
      ...
      if (tSpell->conditionId && !sObjectMgr.IsConditionSatisfied(tSpell->conditionId, GetPlayer(), unit->GetMap(),
                                                                  unit, CONDITION_FROM_TRAINER))
        continue;
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_TRAINER_BUY_SPELL Trainer: %s, learn spell id is: %u",
            guid.GetString().c_str(), spellId);

  Creature *unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
  if (!unit) {
    DEBUG_LOG("WORLD: HandleTrainerBuySpellOpcode - %s not found or you can't interact with him.",
              guid.GetString().c_str());
    return;
  }
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket &recv_data) {
  ...
  if (!pCreature) {
    DEBUG_LOG("WORLD: HandleGossipHelloOpcode - %s not found or you can't interact with him.",
              guid.GetString().c_str());
    return;
  }
...
void WorldSession::HandleSpiritHealerActivateOpcode(WorldPacket &recv_data) {
  ...
  if (!unit) {
    DEBUG_LOG("WORLD: HandleSpiritHealerActivateOpcode - %s not found or you can't interact with him.",
              guid.GetString().c_str());
    return;
  }
...
void WorldSession::SendSpiritResurrect() const {
  ...
  if (corpse)
    corpseGrave = sObjectMgr.GetClosestGraveYard(corpse->GetPositionX(), corpse->GetPositionY(), corpse->GetPositionZ(),
  ...
  if (corpseGrave) {
    WorldSafeLocsEntry const *ghostGrave =
        sObjectMgr.GetClosestGraveYard(_player->GetPositionX(), _player->GetPositionY(), _player->GetPositionZ(),
                                       _player->GetMapId(), _player->GetTeam());
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket &recv_data) {
  ...
  if (!unit) {
    DEBUG_LOG("WORLD: HandleBinderActivateOpcode - %s not found or you can't interact with him.",
...
void WorldSession::HandleListStabledPetsOpcode(WorldPacket &recv_data) {
  ...
  if (!unit) {
    DEBUG_LOG("WORLD: HandleListStabledPetsOpcode - %s not found or you can't interact with him.",
              npcGUID.GetString().c_str());
    return;
  }
...
void WorldSession::SendStablePet(ObjectGuid guid) const {
  ...
  PetSaveMode firstSlot =
      PET_SAVE_FIRST_STABLE_SLOT; // have to be changed to PET_SAVE_AS_CURRENT if pet is currently temp unsummoned
  ...
  if (pet && pet->IsAlive() && pet->getPetType() == HUNTER_PET) {
    ...
    if (_player->GetTemporaryUnsummonedPetNumber()) // temporary unsummon - mount
      ...
      QueryResult *result = CharacterDatabase.PQuery("SELECT owner, id, entry, level, name, loyalty FROM character_pet "
                                                     "WHERE owner = '%u' AND slot = '%u' ORDER BY slot",
  ...
  //                                                     0      1     2   3      4      5        6
  QueryResult *result =
      CharacterDatabase.PQuery("SELECT owner, slot, id, entry, level, loyalty, name FROM character_pet WHERE owner = "
                               "'%u' AND slot >= '%u' AND slot <= '%u' ORDER BY slot",
...
void WorldSession::HandleStablePet(WorldPacket &recv_data) {
  ...
  if (pet) {
    ...
    SpellCastResult loadResult =
        Pet::TryLoadFromDB(_player, 0, 0, _player->GetTemporaryUnsummonedPetNumber() != 0, HUNTER_PET);
  ...
  if (free_slot > 0 && free_slot <= GetPlayer()->m_stableSlots) {
    if (pet)
      ...
      SqlStatement ChangePetSlot = CharacterDatabase.CreateStatement(
          ChangePetSlot_ID, "UPDATE character_pet SET slot = ? WHERE owner = ? AND slot = ? ");
      ChangePetSlot.PExecute(
          free_slot, _player->GetObjectGuid().GetCounter(),
          uint32(_player->GetTemporaryUnsummonedPetNumber() ? PET_SAVE_AS_CURRENT : PET_SAVE_NOT_IN_SLOT));
...
void WorldSession::HandleUnstablePet(WorldPacket &recv_data) {
  ...
  if (!creature_id) {
    sLog.outError("HandleUnstablePet> %s trying to unstable a pet with invalid creature id!",
  ...
  if (pet) {
    ...
    if (loadResult != SPELL_CAST_OK) {
      ...
      SqlStatement ChangePetSlot = CharacterDatabase.CreateStatement(
          ChangePetSlot_ID, "UPDATE character_pet SET slot = ? WHERE owner = ? AND slot = ? ");
      ChangePetSlot.PExecute(
          slot, _player->GetObjectGuid().GetCounter(),
          uint32(_player->GetTemporaryUnsummonedPetNumber() ? PET_SAVE_AS_CURRENT : PET_SAVE_NOT_IN_SLOT));
      CharacterDatabase.CommitTransaction();
      _player->SetTemporaryUnsummonedPetNumber(0);
    }
...
void WorldSession::HandleStableSwapPet(WorldPacket &recv_data) {
  ...
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT slot,entry FROM character_pet WHERE owner = '%u' AND id = '%u'", _player->GetGUIDLow(), pet_number);
  if (!result) {
    SendStableResult(STABLE_ERR_STABLE);
    return;
  }
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recv_data) {
  ...
  if (!unit) {
    DEBUG_LOG("WORLD: HandleRepairItemOpcode - %s not found or you can't interact with him.",
              npcGuid.GetString().c_str());
    return;
  }
