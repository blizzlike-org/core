...
UpdateData::UpdateData() : m_data(1), m_currentIndex(0) { m_data[0].m_buffer = 0; }
...
void UpdateData::Compress(void *dst, uint32 *dst_size, void *src, int src_size) {
  ...
  if (z_res != Z_STREAM_END) {
    sLog.outError("Can't compress update packet (zlib: deflate should report Z_STREAM_END instead %i (%s)", z_res,
                  zError(z_res));
    *dst_size = 0;
    return;
  }
...
WorldPacket UpdateData::BuildPacket(size_t index, bool hasTransport) {
  ...
  ByteBuffer buf(4 + 1 + (m_outOfRangeGUIDs.empty() ? 0 : 1 + 4 + 9 * m_outOfRangeGUIDs.size()) +
                 m_data[index].m_buffer.wpos());
