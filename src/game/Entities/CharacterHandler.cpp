...
bool LoginQueryHolder::Initialize() {
  ...
  // NOTE: all fields in `characters` must be read to prevent lost character data at next save in case wrong DB
  // structure.
  ...
  res &= SetPQuery(
      ...
      "health, power1, power2, power3, power4, power5, exploredZones, equipmentCache, ammoId, knownTitles, actionBars "
      "FROM characters WHERE guid = '%u'",
      m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADGROUP, "SELECT groupId FROM group_member WHERE memberGuid ='%u'",
                   m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADBOUNDINSTANCES,
                   "SELECT id, permanent, map, difficulty, resettime FROM character_instance LEFT JOIN instance ON "
                   "instance = id WHERE guid = '%u'",
                   m_guid.GetCounter());
  res &= SetPQuery(
      PLAYER_LOGIN_QUERY_LOADAURAS,
      "SELECT "
      "caster_guid,item_guid,spell,stackcount,remaincharges,basepoints0,basepoints1,basepoints2,periodictime0,"
      "periodictime1,periodictime2,maxduration,remaintime,effIndexMask FROM character_aura WHERE guid = '%u'",
      m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADSPELLS, "SELECT spell,active,disabled FROM character_spell WHERE guid = '%u'",
                   m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADQUESTSTATUS,
                   "SELECT "
                   "quest,status,rewarded,explored,timer,mobcount1,mobcount2,mobcount3,mobcount4,itemcount1,itemcount2,"
                   "itemcount3,itemcount4 FROM character_queststatus WHERE guid = '%u'",
                   m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADDAILYQUESTSTATUS,
                   "SELECT quest FROM character_queststatus_daily WHERE guid = '%u'", m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADWEEKLYQUESTSTATUS,
                   "SELECT quest FROM character_queststatus_weekly WHERE guid = '%u'", m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMONTHLYQUESTSTATUS,
                   "SELECT quest FROM character_queststatus_monthly WHERE guid = '%u'", m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADREPUTATION,
                   "SELECT faction,standing,flags FROM character_reputation WHERE guid = '%u'", m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADINVENTORY,
                   ...
                   "randomPropertyId, durability, itemTextId, bag, slot, item, item_template FROM character_inventory "
                   "JOIN item_instance ON character_inventory.item = item_instance.guid WHERE character_inventory.guid "
                   "= '%u' ORDER BY bag,slot",
                   m_guid.GetCounter());
  ...
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADSOCIALLIST,
                   "SELECT friend,flags,note FROM character_social WHERE guid = '%u' LIMIT 255", m_guid.GetCounter());
  ...
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADSPELLCOOLDOWNS,
                   "SELECT SpellId, SpellExpireTime, Category, CategoryExpireTime, ItemId FROM "
                   "character_spell_cooldown WHERE guid = '%u'",
                   m_guid.GetCounter());
  ...
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADGUILD, "SELECT guildid,`rank` FROM guild_member WHERE guid = '%u'",
                   m_guid.GetCounter());
  res &= SetPQuery(
  ...
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADBGDATA,
                   ...
                   m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADACCOUNTDATA,
                   "SELECT type, time, data FROM character_account_data WHERE guid='%u'", m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADSKILLS, "SELECT skill, value, max FROM character_skills WHERE guid = '%u'",
                   m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMAILS,
                   "SELECT "
                   "id,messageType,sender,receiver,subject,itemTextId,expire_time,deliver_time,money,cod,checked,"
                   "stationery,mailTemplateId,has_items FROM mail WHERE receiver = '%u' ORDER BY id DESC",
                   m_guid.GetCounter());
  res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMAILEDITEMS,
                   "SELECT itemEntry, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, "
                   "randomPropertyId, durability, itemTextId, mail_id, item_guid, item_template FROM mail_items JOIN "
...
public:
  ...
  void HandlePlayerBotLoginCallback(QueryResult * /*dummy*/, SqlQueryHolder *holder) {
    ...
    WorldSession *botSession =
        new WorldSession(lqh->GetAccountId(), nullptr, SEC_PLAYER, masterSession->GetExpansion(), 0, DEFAULT_LOCALE);
...
void WorldSession::HandleCharEnumOpcode(WorldPacket & /*recv_data*/) {
  ...
  CharacterDatabase.AsyncPQuery(
      &chrHandler, &CharacterHandler::HandleCharEnumCallback, GetAccountId(),
...
void WorldSession::HandleCharCreateOpcode(WorldPacket &recv_data) {
  ...
  if (!classEntry || !raceEntry) {
    ...
    SendPacket(data, true);
    sLog.outError("Account:[%u] attempted to create character of invalid Class (%u) or Race (%u)", GetAccountId(),
                  class_, race_);
    return;
  }
  ...
  if (raceEntry->expansion > GetExpansion()) {
    ...
    sLog.outError("Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", GetExpansion(),
                  GetAccountId(), raceEntry->expansion, race_);
    SendPacket(data, true);
    return;
  }
  ...
  QueryResult *resultacct =
      LoginDatabase.PQuery("SELECT SUM(numchars) FROM realmcharacters WHERE acctid = '%u'", GetAccountId());
  if (resultacct) {
  ...
  QueryResult *result =
      CharacterDatabase.PQuery("SELECT COUNT(guid) FROM characters WHERE account = '%u'", GetAccountId());
  uint8 charcount = 0;
  if (result) {
  ...
  bool AllowTwoSideAccounts =
      !sWorld.IsPvPRealm() || sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_ACCOUNTS) || GetSecurity() > SEC_PLAYER;
  ...
  if (!AllowTwoSideAccounts || skipCinematics == CINEMATICS_SKIP_SAME_RACE) {
    QueryResult *result2 =
        CharacterDatabase.PQuery("SELECT race FROM characters WHERE account = '%u' %s", GetAccountId(),
  ...
  if (!pNewChar->Create(sObjectMgr.GeneratePlayerLowGuid(), name, race_, class_, gender, skin, face, hairStyle,
                        hairColor, facialHair, outfitId)) {
  ...
  DETAIL_LOG("Account: %d (IP: %s) Create Character:[%s] (guid: %u)", GetAccountId(), IP_str.c_str(), name.c_str(),
             pNewChar->GetGUIDLow());
  sLog.outChar("Account: %d (IP: %s) Create Character:[%s] (guid: %u)", GetAccountId(), IP_str.c_str(), name.c_str(),
               pNewChar->GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket &recv_data) {
  ...
  BASIC_LOG("Account: %d (IP: %s) Delete Character:[%s] (guid: %u)", GetAccountId(), IP_str.c_str(), name.c_str(),
            lowguid);
  sLog.outChar("Account: %d (IP: %s) Delete Character:[%s] (guid: %u)", GetAccountId(), IP_str.c_str(), name.c_str(),
               lowguid);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket &recv_data) {
  ...
  if (PlayerLoading()) {
    sLog.outError("HandlePlayerLoginOpcode> Player try to login again while already in loading stage, AccountId = %u",
                  GetAccountId());
  ...
  if (_player) {
    ...
    if (!pCurrChar) {
      sLog.outError("HandlePlayerLoginOpcode> %s try to login a second char, AccountId = %u",
                    _player->GetGuidStr().c_str(), GetAccountId());
      data << (uint8)CHAR_LOGIN_DUPLICATE_CHARACTER;
      SendPacket(data, true);
      return;
    }
    ...
    if (!isLogingOut()) {
      sLog.outError("HandlePlayerLoginOpcode> %s try to login again, AccountId = %u", pCurrChar->GetGuidStr().c_str(),
                    GetAccountId());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder) {
  ...
  if (!pCurrChar->LoadFromDB(playerGuid, holder)) {
    ...
    sLog.outError("HandlePlayerLogin> LoadFromDB failed to load %s, AccountId = %u", pCurrChar->GetGuidStr().c_str(),
                  GetAccountId());
  ...
  // QueryResult *result = CharacterDatabase.PQuery("SELECT guildid,rank FROM guild_member WHERE guid =
  // '%u'",pCurrChar->GetGUIDLow());
  ...
  if (pCurrChar->GetGuildId() != 0) {
    ...
    if (guild) {
      ...
      sLog.outError(
          "Player %s (GUID: %u) marked as member of nonexistent guild (id: %u), removing guild membership for player.",
          pCurrChar->GetName(), pCurrChar->GetGUIDLow(), pCurrChar->GetGuildId());
      pCurrChar->SetInGuild(0);
    }
  }
  ...
  if (lockStatus != AREA_LOCKSTATUS_OK || !pCurrChar->GetMap()->Add(pCurrChar)) {
    ...
    if (!at || lockStatus != AREA_LOCKSTATUS_OK ||
  ...
  if (pCurrChar->m_deathState != ALIVE) {
    ...
    if (pCurrChar->getRace() == RACE_NIGHTELF)
      pCurrChar->CastSpell(pCurrChar, 20584,
                           TRIGGERED_OLD_TRIGGERED); // auras SPELL_AURA_INCREASE_SPEED(+speed in wisp form),
    ...
    pCurrChar->CastSpell(pCurrChar, 8326,
                         TRIGGERED_OLD_TRIGGERED); // auras SPELL_AURA_GHOST, SPELL_AURA_INCREASE_SPEED(why?),
  ...
  std::string IP_str = GetRemoteAddress();
  sLog.outChar("Account: %d (IP: %s) Login Character:[%s] (guid: %u)", GetAccountId(), IP_str.c_str(),
...
void WorldSession::HandlePlayerReconnect() {
  ...
  if (_player->GetGuildId() != 0) {
    ...
    if (guild) {
      ...
      sLog.outError(
          "Player %s (GUID: %u) marked as member of nonexistent guild (id: %u), removing guild membership for player.",
          _player->GetName(), _player->GetGUIDLow(), _player->GetGuildId());
...
void WorldSession::HandleCharRenameOpcode(WorldPacket &recv_data) {
  ...
  CharacterDatabase.AsyncPQuery(&WorldSession::HandleChangePlayerNameOpcodeCallBack, GetAccountId(), newname,
                                "SELECT guid, name FROM characters WHERE guid = %u AND account = %u AND (at_login & "
                                "%u) = %u AND NOT EXISTS (SELECT NULL FROM characters WHERE name = '%s')",
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(QueryResult *result, uint32 accountId, std::string newname) {
  ...
  CharacterDatabase.PExecute("UPDATE characters set name = '%s', at_login = at_login & ~ %u WHERE guid ='%u'",
                             newname.c_str(), uint32(AT_LOGIN_RENAME), guidLow);
  CharacterDatabase.PExecute("DELETE FROM character_declinedname WHERE guid ='%u'", guidLow);
  CharacterDatabase.CommitTransaction();

  sLog.outChar("Account: %d (IP: %s) Character:[%s] (guid:%u) Changed name to: %s", session->GetAccountId(),
               session->GetRemoteAddress().c_str(), oldname.c_str(), guidLow, newname.c_str());
...
void WorldSession::HandleSetPlayerDeclinedNamesOpcode(WorldPacket &recv_data) {
  ...
  CharacterDatabase.PExecute("INSERT INTO character_declinedname (guid, genitive, dative, accusative, instrumental, "
                             "prepositional) VALUES ('%u','%s','%s','%s','%s','%s')",
                             guid.GetCounter(), declinedname.name[0].c_str(), declinedname.name[1].c_str(),
                             declinedname.name[2].c_str(), declinedname.name[3].c_str(), declinedname.name[4].c_str());
  CharacterDatabase.CommitTransaction();
