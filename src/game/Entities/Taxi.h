...
/// Map is a container which describes a full proto-spline made out of taxi node entries in a sequential order for a
/// single game level
...
public:
  explicit Tracker(Player &owner)
      : m_owner(owner), m_state(TRACKER_EMPTY), m_cost(0), m_displayId(0), m_location(nullptr), m_locationIndex(0),
        m_resumeIndex(0), m_debug(false) {}
  ...
  /// Add a taxi flight path to the container by destinations in addition to the last one, returns true if input is
  /// correct
  bool AddRoute(DestID start, DestID end, float discountMulti = 0.0f, bool requireModel = true);
  /// Replace any existing container contents with a chain of flights defined by a vector of destinations, returns true
  /// if input is correct.
  bool AddRoutes(std::vector<DestID> const &destinations, float discountMulti = 0.0f, bool requireModel = true);
  /// Replace any existing container contents with a chain of flights defined by a deque of flight path ids, returns
  /// true if input is correct.
  bool AddRoutes(std::deque<PathID> const &paths, float discountMulti = 0.0f, bool requireModel = true);
  /// Finalize the container and prepare for the tracking of the flight. Used when all routes are added. It will be
  /// impossible to alter the container until the flight ends
  bool Prepare(Index nodeResume = 0);
  /// Clear the container by removing all contents. Can't be performed if taxi flight tracking is currently active,
  /// unless forced
  bool Clear(bool forced = false);

  /// Update tick: notify the container about arrival at the taxi spline node, advances internal node tracker and
  /// provides info about changes. Returns true when current route has changed
