...
enum TypeMask {
  ...
  TYPEMASK_WORLDOBJECT =
      TYPEMASK_UNIT | TYPEMASK_PLAYER | TYPEMASK_GAMEOBJECT | TYPEMASK_DYNAMICOBJECT | TYPEMASK_CORPSE,
};
...
public: // accessors
  ...
  uint32 GetCounter() const {
    return HasEntry() ? uint32(m_guid & uint64(0x0000000000FFFFFF)) : uint32(m_guid & uint64(0x00000000FFFFFFFF));
  }
...
public: // constructors
  ...
  explicit PackedGuid(uint64 const &guid) : m_packedGuid(PACKED_GUID_MIN_BUFFER_SIZE) {
    m_packedGuid.appendPackGUID(guid);
  }
  explicit PackedGuid(ObjectGuid const &guid) : m_packedGuid(PACKED_GUID_MIN_BUFFER_SIZE) {
...
template <HighGuid high> class ObjectGuidGenerator {
public: // constructors
...
namespace std {
template <> class hash<ObjectGuid> {
public:
  size_t operator()(ObjectGuid const &key) const { return hash<uint64>()(key.GetRawValue()); }
};
} // namespace std
