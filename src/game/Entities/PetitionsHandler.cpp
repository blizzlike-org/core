...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket &recv_data) {
  ...
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT petitionguid FROM petition WHERE ownerguid = '%u'  AND type = '%u'", _player->GetGUIDLow(), type);
  ...
  CharacterDatabase.PExecute(
      "INSERT INTO petition (ownerguid, petitionguid, name, type) VALUES ('%u', '%u', '%s', '%u')",
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket &recv_data) {
  ...
  // solve (possible) some strange compile problems with explicit use GUID_LOPART(petitionguid) at some GCC versions
  // (wrong code optimization in compiler?)
  uint32 petitionguid_low = petitionguid.GetCounter();

  QueryResult *result =
      CharacterDatabase.PQuery("SELECT type FROM petition WHERE petitionguid = '%u'", petitionguid_low);
  if (!result) {
...
void WorldSession::HandlePetitionRenameOpcode(WorldPacket &recv_data) {
  ...
  QueryResult *result =
      CharacterDatabase.PQuery("SELECT type FROM petition WHERE petitionguid = '%u'", petitionGuid.GetCounter());
  ...
  CharacterDatabase.PExecute("UPDATE petition SET name = '%s' WHERE petitionguid = '%u'", db_newname.c_str(),
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket &recv_data) {
  ...
  // client doesn't allow to sign petition two times by one character, but not check sign by another character from same
  // account not allow sign another player from already sign player account
  result = CharacterDatabase.PQuery(
      "SELECT playerguid FROM petition_sign WHERE player_account = '%u' AND petitionguid = '%u'", GetAccountId(),
      petitionLowGuid);
...
void WorldSession::HandlePetitionDeclineOpcode(WorldPacket &recv_data) {
  ...
  QueryResult *result =
      CharacterDatabase.PQuery("SELECT ownerguid FROM petition WHERE petitionguid = '%u'", petitionLowGuid);
  if (!result)
    return;
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("OFFER PETITION: type %u petition %s to %s", type, petitionGuid.GetString().c_str(),
  ...
  uint8 signs = 0;
  result = CharacterDatabase.PQuery("SELECT playerguid FROM petition_sign WHERE petitionguid = '%u'",
                                    petitionGuid.GetCounter());
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket &recv_data) {
  ...
  // data
  QueryResult *result = CharacterDatabase.PQuery("SELECT ownerguid, name, type FROM petition WHERE petitionguid = '%u'",
                                                 petitionGuid.GetCounter());
  if (result) {
    Field *fields = result->Fetch();
...
void WorldSession::SendPetitionShowList(ObjectGuid guid) const {
  ...
  if (!pCreature) {
    DEBUG_LOG("WORLD: HandlePetitionShowListOpcode - %s not found or you can't interact with him.",
