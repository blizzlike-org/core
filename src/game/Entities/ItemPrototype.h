...
enum ItemSpelltriggerType {
  ...
  ITEM_SPELLTRIGGER_ON_STORE = 5, // casted at add item to inventory/equip, applied aura removed at remove item, item
...
enum ItemPrototypeFlags {
  ...
  ITEM_FLAG_HAS_LOOT = 0x00000004, // affect only non container items that can be "open" for loot. It or lockid set
                                   // enable for client show "Right click to open". See also ITEM_DYNFLAG_UNLOCKED
  ...
  ITEM_FLAG_NO_USER_DESTROY = 0x00000020, // used for totem. Item can not be destroyed, except by using spell (item can
                                          // be reagent for spell and then allowed)
  ...
  ITEM_FLAG_IS_PROSPECTABLE =
      0x00040000, // item can have prospecting loot (in fact some items expected have empty loot)
  ...
  ITEM_FLAG_NO_DURABILITY_LOSS =
      0x00400000, // Only items of ITEM_SUBCLASS_WEAPON_THROWN have it but not all, so can't be used as in game check
...
const uint32 MaxItemSubclassValues[MAX_ITEM_CLASS] = {
    MAX_ITEM_SUBCLASS_CONSUMABLE, MAX_ITEM_SUBCLASS_CONTAINER,   MAX_ITEM_SUBCLASS_WEAPON,
    MAX_ITEM_SUBCLASS_GEM,        MAX_ITEM_SUBCLASS_ARMOR,       MAX_ITEM_SUBCLASS_REAGENT,
    MAX_ITEM_SUBCLASS_PROJECTILE, MAX_ITEM_SUBCLASS_TRADE_GOODS, MAX_ITEM_SUBCLASS_GENERIC,
...
enum ItemExtraFlags {
  ITEM_EXTRA_REAL_TIME_DURATION = 0x01, // if set and have Duration time, then offline time included in counting, if not
                                        // set then counted only in game time
