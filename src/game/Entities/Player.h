...
enum EnvironmentFlags {
  ...
  ENVIRONMENT_MASK_LIQUID_FLAGS = (ENVIRONMENT_FLAG_UNDERWATER | ENVIRONMENT_MASK_IN_LIQUID |
                                   ENVIRONMENT_FLAG_HIGH_SEA | ENVIRONMENT_FLAG_LIQUID | ENVIRONMENT_FLAG_HIGH_LIQUID),
};
...
struct PlayerSpell {
  ...
  bool disabled : 1;  // first rank has been learned in result talent learn but currently talent unlearned, save max
                      // learned ranks
};
...
struct PlayerInfo {
  ...
  PlayerInfo()
      : mapId(0), areaId(0), positionX(0.f), positionY(0.f), positionZ(0.f), orientation(0.f), displayId_m(0),
        displayId_f(0), levelInfo(nullptr) {}
...
struct PvPInfo {
  PvPInfo()
      : inPvPCombat(false), inPvPEnforcedArea(false), inPvPCapturePoint(false), isPvPFlagCarrier(false),
        timerPvPRemaining(0), timerPvPContestedRemaining(0) {}
...
enum PlayerFieldByte2Flags {
  ...
  PLAYER_FIELD_BYTE2_DETECT_AMORE_0 = 0x02, // SPELL_AURA_DETECT_AMORE, not used as value and maybe not relcted to, but
                                            // used in code as base for mask apply
...
public:
  ...
  MirrorTimer(Type type)
      : m_type(type), m_scale(-1), m_spellId(0), m_status(UNCHANGED), m_active(false), m_frozen(false) {}
...
enum InstanceResetWarningType {
  ...
  RAID_INSTANCE_WARNING_MIN_SOON = 3, // WARNING! %s is scheduled to reset in %d minute(s). Please exit the zone or you
                                      // will be returned to your bind location!
...
enum ReputationSource { REPUTATION_SOURCE_KILL, REPUTATION_SOURCE_QUEST, REPUTATION_SOURCE_SPELL };
...
struct BGData {
  BGData()
      : bgInstanceID(0), bgTypeID(BATTLEGROUND_TYPE_NONE), bgAfkReportedCount(0), bgAfkReportedTimer(0),
...
struct TradeStatusInfo {
  TradeStatusInfo()
      : Status(TRADE_STATUS_BUSY), TraderGuid(), Result(EQUIP_ERR_OK), IsTargetResult(false), ItemLimitCategoryId(0),
        Slot(0) {}
...
public:
  ...
  bool Create(uint32 guidlow, const std::string &name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face,
              uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 outfitId);
  ...
  InventoryResult CanTakeMoreSimilarItems(uint32 entry, uint32 count) const {
    return _CanTakeMoreSimilarItems(entry, count, nullptr);
  }
  InventoryResult CanStoreNewItem(uint8 bag, uint8 slot, ItemPosCountVec &dest, uint32 item, uint32 count,
  ...
  InventoryResult CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec &dest, Item *pItem, bool swap,
  ...
  bool hasWeaponForAttack(WeaponAttackType type) const override {
    return (Unit::hasWeaponForAttack(type) && GetWeaponForAttack(type, true, true));
  }

  Item *ConvertItem(Item *item, uint32 newItemId);

  InventoryResult _CanTakeMoreSimilarItems(uint32 entry, uint32 count, Item *pItem,
                                           uint32 *no_space_count = nullptr) const;
  InventoryResult _CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec &dest, uint32 entry, uint32 count,
                                Item *pItem = nullptr, bool swap = false, uint32 *no_space_count = nullptr) const;
  ...
  // if completed_or_not = 0 (or any other value except 1 or 2) - returns true, if quest is taken and doesn't depend if
  // quest is completed or not if completed_or_not = 1 - returns true, if quest is taken but not completed if
  // completed_or_not = 2 - returns true, if quest is taken and already completed
  ...
  static bool LoadPositionFromDB(ObjectGuid guid, uint32 &mapid, float &x, float &y, float &z, float &o,
                                 bool &in_flight);
  ...
  static void DeleteFromDB(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars = true,
  ...
  void SetSelectionGuid(ObjectGuid guid) override {
  ...
  void SendMailResult(uint32 mailId, MailResponseType mailAction, MailResponseResult mailError, uint32 equipError = 0,
                      uint32 item_guid = 0, uint32 item_count = 0) const;
  ...
  template <class T>
  void ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, Spell const *spell = nullptr, bool finalUse = true);
  ...
  SpellFamily GetSpellClass() const {
    return m_spellClassName;
  } // client function equivalent - says what player can cast
  ...
  void AddResurrectRequest(ObjectGuid casterGuid, SpellEntry const *spellInfo, Position position, uint32 mapId,
  ...
  // Arena Team
  void SetInArenaTeam(uint32 ArenaTeamId, uint8 slot) { SetArenaTeamInfoField(slot, ARENA_TEAM_ID, ArenaTeamId); }
  ...
  std::pair<bool, AreaTrigger const *> CheckAndRevivePlayerOnDungeonEnter(MapEntry const *targetMapEntry,
                                                                          uint32 targetMapId);
  ...
  GetSkillInfo(uint16 id, std::function<bool(SkillRaceClassInfoEntry const &)> filterfunc = nullptr) const;
  ...
  inline uint16 GetSkillValue(uint16 id) const {
    return GetSkill(id, true, true);
  } // skill value + perm. bonus + temp bonus
  ...
  inline uint16 GetSkillMax(uint16 id) const {
    return GetSkill(id, true, true, true);
  } // skill max + perm. bonus + temp bonus
  ...
  void SetDelayedZoneUpdate(bool state, uint32 newZone) {
    m_needsZoneUpdate = state;
    m_newZone = newZone;
  }
  ...
  template <typename T> bool CheckForGroup(T functor) const {
  ...
  int32 CalculateReputationGain(ReputationSource source, int32 rep, int32 maxRep, int32 faction,
                                uint32 creatureOrQuestLevel = 0, bool noAuraBonus = false);
  ...
  void GetHomebindLocation(float &x, float &y, float &z, uint32 &mapId) {
    x = m_homebindX;
    y = m_homebindY;
    z = m_homebindZ;
  ...
  static bool ValidateAppearance(uint8 race, uint8 class_, uint8 gender, uint8 hairID, uint8 hairColor, uint8 faceID,
  ...
  void SendTransferAbortedByLockStatus(MapEntry const *mapEntry, AreaTrigger const *at, AreaLockStatus lockStatus,
                                       uint32 miscRequirement = 0) const;
  ...
  virtual void AddCooldown(SpellEntry const &spellEntry, ItemPrototype const *itemProto = nullptr,
                           bool permanent = false, uint32 forcedDuration = 0) override;
  ...
  template <typename F> void RemoveSomeCooldown(F check) {
...
protected:
  ...
  */
  ...
  void SendMirrorTimerStart(uint32 type, uint32 remaining, uint32 duration, int32 scale, bool paused = false,
                            uint32 spellId = 0);
  ...
  uint32 m_enchantmentFlatMod[MAX_ATTACK]; // TODO: Stat system - incorporate generically, exposes a required hidden
                                           // weapon stat that does not apply when unarmed
...
private:
  ...
  InventoryResult _CanStoreItem_InSpecificSlot(uint8 bag, uint8 slot, ItemPosCountVec &dest,
                                               ItemPrototype const *pProto, uint32 &count, bool swap,
                                               Item *pSrcItem) const;
  InventoryResult _CanStoreItem_InBag(uint8 bag, ItemPosCountVec &dest, ItemPrototype const *pProto, uint32 &count,
                                      bool merge, bool non_specialized, Item *pSrcItem, uint8 skip_bag,
                                      uint8 skip_slot) const;
  InventoryResult _CanStoreItem_InInventorySlots(uint8 slot_begin, uint8 slot_end, ItemPosCountVec &dest,
                                                 ItemPrototype const *pProto, uint32 &count, bool merge, Item *pSrcItem,
  ...
  MirrorTimer m_mirrorTimers[MirrorTimer::NUM_TIMERS] = {MirrorTimer::FATIGUE, MirrorTimer::BREATH,
                                                         MirrorTimer::FEIGNDEATH, MirrorTimer::ENVIRONMENTAL};
...
template <class T>
void Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, Spell const *spell, bool finalUse) {
  SpellEntry const *spellInfo = sSpellTemplate.LookupEntry<SpellEntry>(spellId);
  if (!spellInfo || spellInfo->SpellFamilyName != GetSpellClass() ||
