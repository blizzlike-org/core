...
enum PetType { SUMMON_PET = 0, HUNTER_PET = 1, GUARDIAN_PET = 2, MINI_PET = 3, MAX_PET_TYPE = 4 };
...
enum PetSaveMode {
  ...
  PET_SAVE_LAST_STABLE_SLOT =
      MAX_PET_STABLES, // last in DB stable slot index (including), all higher have same meaning as PET_SAVE_NOT_IN_SLOT
...
enum LoyaltyLevel { REBELLIOUS = 1, UNRULY = 2, SUBMISSIVE = 3, DEPENDABLE = 4, FAITHFUL = 5, BEST_FRIEND = 6 };
...
struct PetSpell {
  uint8
      active; // use instead enum (not good use *uint8* limited enum in case when value in enum not positive in *int8*)
...
public:
  ...
  static SpellCastResult TryLoadFromDB(Unit *owner, uint32 petentry = 0, uint32 petnumber = 0, bool current = false,
                                       PetType mandatoryPetType = MAX_PET_TYPE);
  ...
  bool addSpell(uint32 spell_id, ActiveStates active = ACT_DECIDE, PetSpellState state = PETSPELL_NEW,
  ...
  virtual UnitAI *AI() override {
    if (hasUnitState(UNIT_STAT_POSSESSED) && m_charmInfo->GetAI())
