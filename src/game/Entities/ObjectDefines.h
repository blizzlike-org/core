...
static constexpr float const MAX_VISIBILITY_DISTANCE =
    SIZE_OF_GRIDS; // max distance for visible object show, limited in 533 yards
...
static constexpr float const DEFAULT_VISIBILITY_DISTANCE =
    VISIBILITY_DISTANCE_NORMAL;                                    // default visible distance, 100 yards on continents
...
static constexpr float const DEFAULT_WORLD_OBJECT_SIZE =
    0.388999998569489f; // currently used (correctly?) for any non Unit world objects.
                        // This is actually the bounding_radius, like player/creature from
                        // creature_model_data

static constexpr float const DEFAULT_OBJECT_SCALE =
