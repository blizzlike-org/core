...
std::string Tracker::Save() {
  ...
  // Original numbers format: "src dest dest dest "                   variable amount of destinations in the string,
  // dangling destinations possible, trailing space @ the end) Updated  numbers format: "resumenode:src:dest:dest:dest"
  // strictly formatted, introduced in taxi system rewrite Modified numbers format: "resumenode#pathid#pathid#pathid"
  // strictly formatted, directly uses flights, introduced in an update for redesigned taxi system
  ...
  // Make sure resume node is not the very first one or the very last one - saving and loading those makes no practical
  // sense to us
...
bool Tracker::Load(std::string &string, DestID &destOrphan) {
  ...
  // Read a modified format. All old entries were converted to orphaned taxi node records via DB update for failsafe
  // loading.
    ...
    for (auto j = nodes.begin(); j != nodes.end(); ++j) {
      ...
      if (nodeResume && (*i).pathID == current.pathID) {
        ...
        if ((*j)->index < nodes[nodeResume]->index && (*j)->mapid != nodes[nodeResume]->mapid)
          continue;
        // When resume node reached on the route: calculate resume location index on the spline (which will be equal to
        // the current size on push)
      ...
      if ((*j)->index >= (*i).nodeStart && (*j)->index <= (*i).nodeEnd) {
        if (prev && (*j)->mapid != prev->mapid) {
          ...
          if (sMapStore.LookupEntry((*j)->mapid)) {
            ...
            sLog.outError("TAXI: Malformed flight path node detected and ignored. Node %u on flight path id %u refers "
...
bool Tracker::Trim(Route &first, Route &second) {
  ...
  // We have a fallback custom automatic runtime trimmer. The result will not match retail and can be visually
  // unpleasing at times. For best experience, the taxi shortcuts table needs to be populated with faithful data.
