...
void WorldSession::HandleSplitItemOpcode(WorldPacket &recv_data) {
  ...
  // DEBUG_LOG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u, dstslot = %u, count = %u", srcbag, srcslot,
  // dstbag, dstslot, count);
...
void WorldSession::HandleSwapItem(WorldPacket &recv_data) {
  ...
  // DEBUG_LOG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u, dstslot = %u", srcbag, srcslot, dstbag,
  // dstslot);
  ...
  if (pProto) {
    ...
    for (const auto &Spell : pProto->Spells) {
      ...
      if (spell) {
        ...
        // let the database control the sign here.  negative means that the item should be consumed once the charges are
        // consumed.
...
void WorldSession::HandleSellItemOpcode(WorldPacket &recv_data) {
  ...
  if (!pCreature) {
    DEBUG_LOG("WORLD: HandleSellItemOpcode - %s not found or you can't interact with him.",
  ...
  if (count < pItem->GetCount()) // need split items
  {
    ...
    if (!pNewItem) {
      sLog.outError("WORLD: HandleSellItemOpcode - could not create clone of item %u; count = %u", pItem->GetEntry(),
                    count);
      _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, pCreature, itemGuid, 0);
      return;
    }
...
void WorldSession::HandleBuybackItem(WorldPacket &recv_data) {
  ...
  Creature *pCreature = GetPlayer()->GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);
  if (!pCreature) {
    DEBUG_LOG("WORLD: HandleBuybackItem - %s not found or you can't interact with him.",
              vendorGuid.GetString().c_str());
...
void WorldSession::SendListInventory(ObjectGuid vendorguid) const {
  ...
  if (!pCreature) {
    DEBUG_LOG("WORLD: SendListInventory - %s not found or you can't interact with him.",
              vendorguid.GetString().c_str());
    _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid(), 0);
    return;
  }
    ...
    if (crItem) {
      ...
      if (pProto) {
        if (!_player->IsGameMaster()) {
          ...
          if (crItem->conditionId && !sObjectMgr.IsConditionSatisfied(crItem->conditionId, _player, pCreature->GetMap(),
                                                                      pCreature, CONDITION_FROM_VENDOR))
            continue;
        }
...
void WorldSession::SendItemEnchantTimeUpdate(ObjectGuid playerGuid, ObjectGuid itemGuid, uint32 slot,
...
void WorldSession::HandleItemNameQueryOpcode(WorldPacket &recv_data) {
  ...
  if (ItemPrototype const *pProto = ObjectMgr::GetItemPrototype(itemid)) {
    ...
    if (sItemStore.LookupEntry(itemid))
      sLog.outErrorDb("WORLD: CMSG_ITEM_NAME_QUERY for item %u failed (item listed in Item.dbc but not exist in DB)",
                      itemid);
    else
...
void WorldSession::HandleWrapItemOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, item_slot = %u", gift_bag, gift_slot, item_bag,
            item_slot);
  ...
  CharacterDatabase.PExecute("INSERT INTO character_gifts VALUES ('%u', '%u', '%u', '%u')",
                             item->GetOwnerGuid().GetCounter(), item->GetGUIDLow(), item->GetEntry(),
                             item->GetUInt32Value(ITEM_FIELD_FLAGS));
...
void WorldSession::HandleSocketOpcode(WorldPacket &recv_data) {
  ...
  // if a meta gem is being equipped, all information has to be written to the item before testing if the conditions for
  // the gem are met
  ...
  if (SocketBonusActivated != SocketBonusToBeActivated)         // if there was a change...
  {
    ...
    itemTarget->SetEnchantment(BONUS_ENCHANTMENT_SLOT,
                               (SocketBonusToBeActivated ? itemTarget->GetProto()->socketBonus : 0), 0, 0,
                               _player->GetObjectGuid());
