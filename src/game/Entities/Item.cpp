...
void RemoveItemsSetItem(Player *player, ItemPrototype const *proto) {
  ...
  // can be in case now enough skill requirement for set appling but set has been appliend when skill requirement not
  // enough
  if (!eff)
    return;
...
void Item::UpdateDuration(Player *owner, uint32 diff) {
  ...
  // DEBUG_LOG("Item::UpdateDuration Item (Entry: %u Duration %u Diff %u)", GetEntry(),
...
void Item::SaveToDB() {
  static const char *UPDATE_ITEM = "UPDATE item_instance SET owner_guid = ?, itemEntry = ?, creatorGuid = ?, "
                                   "giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments "
                                   "= ?, randomPropertyId = ?, durability = ?, itemTextId = ? WHERE guid = ?";
  static const char *INSERT_ITEM =
      "REPLACE INTO item_instance (owner_guid, itemEntry, creatorGuid, giftCreatorGuid, count, duration, charges, "
      "flags, enchantments, randomPropertyId, durability, itemTextId, guid) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    if (Player *owner = GetOwner()) {
      ...
      if (m_loot->GetGoldAmount()) {
        SqlStatement stmt = CharacterDatabase.CreateStatement(
            saveGold,
            "INSERT INTO item_loot (guid,owner_guid,itemid,amount,suffix,property) VALUES (?, ?, 0, ?, 0, 0)");
        stmt.PExecute(GetGUIDLow(), owner->GetGUIDLow(), m_loot->GetGoldAmount());
      }

      SqlStatement stmt = CharacterDatabase.CreateStatement(
          saveLoot, "INSERT INTO item_loot (guid,owner_guid,itemid,amount,suffix,property) VALUES (?, ?, ?, ?, ?, ?)");
...
bool Item::LoadFromDB(uint32 guidLow, Field *fields, ObjectGuid ownerGuid) {
  ...
  // itemEntry, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId,
  // durability, itemTextId
  ...
  if (need_save) // normal item changed state set not work at loading
  {
    ...
    SqlStatement stmt = CharacterDatabase.CreateStatement(
        updItem, "UPDATE item_instance SET duration = ?, flags = ?, durability = ? WHERE guid = ?");
...
void Item::LoadLootFromDB(Field *fields) {
  ...
  if (!proto) {
    ...
    sLog.outError("Item::LoadLootFromDB: %s has an unknown item (id: #%u) in item_loot, deleted.",
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id) {
  ...
  if (itemProto->RandomProperty) {
    ...
    if (!random_id) {
      sLog.outErrorDb("Enchantment id #%u used but it doesn't have records in 'ItemRandomProperties.dbc'",
                      randomPropId);
      return 0;
    }
...
void Item::AddToUpdateQueueOf(Player *player) {
  ...
  if (!player) {
    player = GetOwner();
    if (!player) {
      sLog.outError("Item::AddToUpdateQueueOf - %s current owner (%s) not in world!", GetGuidStr().c_str(),
                    GetOwnerGuid().GetString().c_str());
      return;
    }
  }
...
void Item::RemoveFromUpdateQueueOf(Player *player) {
  ...
  if (!player) {
    player = GetOwner();
    if (!player) {
      sLog.outError("Item::RemoveFromUpdateQueueOf - %s current owner (%s) not in world!", GetGuidStr().c_str(),
                    GetOwnerGuid().GetString().c_str());
      return;
    }
  }
...
void Item::SetLootState(ItemLootUpdateState state) {
  ...
  // ITEM_LOOT_NONE -> ITEM_LOOT_NEW -> ITEM_LOOT_UNCHANGED [<-> ITEM_LOOT_CHANGED] -> ITEM_LOOT_REMOVED ->
  // ITEM_LOOT_NONE
  switch (state) {
  case ITEM_LOOT_NONE:
  case ITEM_LOOT_NEW:
