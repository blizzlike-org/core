...
bool ConditionEntry::Meets(WorldObject const *target, Map const *map, WorldObject const *source,
  ...
  DEBUG_LOG("Condition-System: Check condition %u, type %i - called from %s with params target: %s, map %i, source %s",
            m_entry, m_condition, conditionSourceToStr[conditionSourceType],
            target ? target->GetGuidStr().c_str() : "<nullptr>", map ? map->GetId() : -1,
            source ? source->GetGuidStr().c_str() : "<nullptr>");
  ...
  if (!CheckParamRequirements(target, map, source)) {
    sLog.outErrorDb(
        "CONDITION %u type %u used with bad parameters, called from %s, used with target: %s, map %i, source %s",
        m_entry, m_condition, conditionSourceToStr[conditionSourceType],
        target ? target->GetGuidStr().c_str() : "<nullptr>", map ? map->GetId() : -1,
        source ? source->GetGuidStr().c_str() : "<nullptr>");
    return false;
...
bool inline ConditionEntry::Evaluate(WorldObject const *target, Map const *map, WorldObject const *source,
                                     ConditionSource conditionSourceType) const {
  switch (m_condition) {
  case CONDITION_NOT: {
    // Checked on load
  ...
  case CONDITION_AND: {
    ...
    return extraConditionsSatisfied &&
  ...
  case CONDITION_REPUTATION_RANK_MIN: {
    ...
    return faction &&
           static_cast<Player const *>(target)->GetReputationMgr().GetRank(faction) >= ReputationRank(m_value2);
  }
  case CONDITION_TEAM: {
    if (conditionSourceType == CONDITION_FROM_REFERING_LOOT &&
        sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
      return true;
  ...
  case CONDITION_AD_COMMISSION_AURA: {
    ...
    for (const auto &aura : auras)
      if ((aura.second->GetSpellProto()->HasAttribute(SPELL_ATTR_CASTABLE_WHILE_MOUNTED) ||
           aura.second->GetSpellProto()->HasAttribute(SPELL_ATTR_ABILITY)) &&
          aura.second->GetSpellProto()->SpellVisual == 3580)
        return true;
    return false;
  }
  ...
  case CONDITION_CREATURE_IN_RANGE: {
    ...
    MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*target, m_value1, true, false, m_value2,
                                                                               true);
  ...
  case CONDITION_COMPLETED_ENCOUNTER: {
    if (!map)
      map = target ? target->GetMap() : source->GetMap();
    if (!map->IsDungeon()) {
      sLog.outErrorDb("CONDITION_COMPLETED_ENCOUNTER (entry %u) is used outside of a dungeon (on Map %u) by %s",
                      m_entry, target->GetMapId(), target->GetGuidStr().c_str());
      return false;
    }
    ...
    if (dbcEntry1->mapId != map->GetId()) {
      sLog.outErrorDb("CONDITION_COMPLETED_ENCOUNTER (entry %u, DungeonEncounterEntry %u) is used on wrong map (used "
                      "on Map %u) by %s",
                      m_entry, m_value1, target->GetMapId(), target->GetGuidStr().c_str());
      return false;
    }
    return (completedEncounterMask &
            ((dbcEntry1 ? 1 << dbcEntry1->encounterIndex : 0) | (dbcEntry2 ? 1 << dbcEntry2->encounterIndex : 0))) != 0;
  }
  ...
  case CONDITION_DEAD_OR_AWAY: {
    ...
    Player const *player =
        (target && (target->GetTypeId() == TYPEID_PLAYER)) ? static_cast<Player const *>(target) : nullptr;
    switch (m_value1) {
    ...
    case 2: // All players in instance dead or out of range
      ...
      if (!map || !map->Instanceable()) {
        sLog.outErrorDb("CONDITION_DEAD_OR_AWAY %u (Player in instance case) - called from %s without map param or "
                        "from non-instanceable map %i",
                        m_entry, conditionSourceToStr[conditionSourceType], map ? map->GetId() : -1);
        return false;
      }
  ...
  case CONDITION_PVP_SCRIPT: {
    if (target->GetTypeId() != TYPEID_PLAYER) {
      sLog.outErrorDb("CONDITION_PVP_SCRIPT (entry %u) is used on non player target. Target is %s", m_entry,
...
bool ConditionEntry::CheckParamRequirements(WorldObject const *target, Map const *map,
                                            WorldObject const *source) const {
...
bool ConditionEntry::IsValid() const {
  ...
  case CONDITION_NOT: {
    if (m_value1 >= m_entry) {
      sLog.outErrorDb("CONDITION_NOT (entry %u, type %d) has invalid value1 %u, must be lower than entry, skipped",
                      m_entry, m_condition, m_value1);
      return false;
    }
    ConditionEntry const *condition1 = sConditionStorage.LookupEntry<ConditionEntry>(m_value1);
    if (!condition1) {
      sLog.outErrorDb("CONDITION_NOT (entry %u, type %d) has value1 %u without proper condition, skipped", m_entry,
                      m_condition, m_value1);
      return false;
    }
    break;
  }
  ...
  case CONDITION_AND: {
    if (m_value1 >= m_entry) {
      sLog.outErrorDb(
          "CONDITION _AND or _OR (entry %u, type %d) has invalid value1 %u, must be lower than entry, skipped", m_entry,
          m_condition, m_value1);
      return false;
    }
    if (m_value2 >= m_entry) {
      sLog.outErrorDb(
          "CONDITION _AND or _OR (entry %u, type %d) has invalid value2 %u, must be lower than entry, skipped", m_entry,
          m_condition, m_value2);
      return false;
    }
    ConditionEntry const *condition1 = sConditionStorage.LookupEntry<ConditionEntry>(m_value1);
    if (!condition1) {
      sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value1 %u without proper condition, skipped",
                      m_entry, m_condition, m_value1);
      return false;
    }
    ConditionEntry const *condition2 = sConditionStorage.LookupEntry<ConditionEntry>(m_value2);
    if (!condition2) {
      sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value2 %u without proper condition, skipped",
                      m_entry, m_condition, m_value2);
      return false;
    }
    if (m_value3) {
      if (m_value3 >= m_entry) {
        sLog.outErrorDb(
            "CONDITION _AND or _OR (entry %u, type %d) has invalid value3 %u, must be lower than entry, skipped",
            m_entry, m_condition, m_value3);
        return false;
      }
      ConditionEntry const *condition3 = sConditionStorage.LookupEntry<ConditionEntry>(m_value3);
      if (!condition3) {
        sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value3 %u without proper condition, skipped",
                        m_entry, m_condition, m_value3);
        return false;
      }
    }
    if (m_value4) {
      if (m_value4 >= m_entry) {
        sLog.outErrorDb(
            "CONDITION _AND or _OR (entry %u, type %d) has invalid value4 %u, must be lower than entry, skipped",
            m_entry, m_condition, m_value4);
        return false;
      }
      ConditionEntry const *condition4 = sConditionStorage.LookupEntry<ConditionEntry>(m_value4);
      if (!condition4) {
        sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value4 %u without proper condition, skipped",
                        m_entry, m_condition, m_value4);
        return false;
      }
    }
  ...
  case CONDITION_AURA: {
    if (!sSpellTemplate.LookupEntry<SpellEntry>(m_value1)) {
      sLog.outErrorDb("Aura condition (entry %u, type %u) requires to have non existing spell (Id: %d), skipped",
                      m_entry, m_condition, m_value1);
      return false;
    }
    if (m_value2 >= MAX_EFFECT_INDEX) {
      sLog.outErrorDb(
          "Aura condition (entry %u, type %u) requires to have non existing effect index (%u) (must be 0..%u), skipped",
          m_entry, m_condition, m_value2, MAX_EFFECT_INDEX - 1);
      return false;
    }
    break;
  }
  ...
  case CONDITION_ITEM_WITH_BANK: {
    ...
    if (!proto) {
      sLog.outErrorDb("Item condition (entry %u, type %u) requires to have non existing item (%u), skipped", m_entry,
                      m_condition, m_value1);
      return false;
    }
  ...
  case CONDITION_ITEM_EQUIPPED: {
    ...
    if (!proto) {
      sLog.outErrorDb(
          "ItemEquipped condition (entry %u, type %u) requires to have non existing item (%u) equipped, skipped",
          m_entry, m_condition, m_value1);
      return false;
    }
    break;
  ...
  case CONDITION_AREAID: {
    ...
    if (!areaEntry) {
      sLog.outErrorDb("Zone condition (entry %u, type %u) requires to be in non existing area (%u), skipped", m_entry,
                      m_condition, m_value1);
      return false;
    }

    if (m_value2 > 1) {
      sLog.outErrorDb("Zone condition (entry %u, type %u) has invalid argument %u (must be 0..1), skipped", m_entry,
                      m_condition, m_value2);
      return false;
    }
    break;
  ...
  case CONDITION_REPUTATION_RANK_MAX: {
    ...
    if (!factionEntry) {
      sLog.outErrorDb(
          "Reputation condition (entry %u, type %u) requires to have reputation non existing faction (%u), skipped",
          m_entry, m_condition, m_value1);
      return false;
    }

    if (m_value2 >= MAX_REPUTATION_RANK) {
      sLog.outErrorDb("Reputation condition (entry %u, type %u) has invalid rank requirement (value2 = %u) - must be "
                      "between %u and %u, skipped",
                      m_entry, m_condition, m_value2, MIN_REPUTATION_RANK, MAX_REPUTATION_RANK - 1);
      return false;
    }
    break;
  }
  case CONDITION_TEAM: {
    if (m_value1 != ALLIANCE && m_value1 != HORDE) {
      sLog.outErrorDb("Team condition (entry %u, type %u) specifies unknown team (%u), skipped", m_entry, m_condition,
                      m_value1);
      return false;
    }
    break;
  ...
  case CONDITION_SKILL_BELOW: {
    ...
    if (!pSkill) {
      sLog.outErrorDb("Skill condition (entry %u, type %u) specifies non-existing skill (%u), skipped", m_entry,
                      m_condition, m_value1);
      return false;
    }
    if (m_value2 < 1 || m_value2 > sWorld.GetConfigMaxSkillValue()) {
      sLog.outErrorDb("Skill condition (entry %u, type %u) specifies invalid skill value (%u), skipped", m_entry,
                      m_condition, m_value2);
      return false;
    }
    break;
  ...
  case CONDITION_QUEST_NONE: {
    ...
    if (!Quest) {
      sLog.outErrorDb("Quest condition (entry %u, type %u) specifies non-existing quest (%u), skipped", m_entry,
                      m_condition, m_value1);
      return false;
    }

    if (m_value2 && m_condition != CONDITION_QUESTTAKEN)
      sLog.outErrorDb("Quest condition (entry %u, type %u) has useless data in value2 (%u)!", m_entry, m_condition,
                      m_value2);
    break;
  }
  case CONDITION_AD_COMMISSION_AURA: {
    if (m_value1)
      sLog.outErrorDb("Quest condition (entry %u, type %u) has useless data in value1 (%u)!", m_entry, m_condition,
                      m_value1);
    if (m_value2)
      sLog.outErrorDb("Quest condition (entry %u, type %u) has useless data in value2 (%u)!", m_entry, m_condition,
                      m_value2);
  ...
  case CONDITION_ACTIVE_GAME_EVENT: {
    if (!sGameEventMgr.IsValidEvent(m_value1)) {
      sLog.outErrorDb("(Not)Active event condition (entry %u, type %u) requires existing event id (%u), skipped",
                      m_entry, m_condition, m_value1);
      return false;
    }
    break;
  ...
  case CONDITION_RACE_CLASS: {
    ...
    if (m_value1 && !(m_value1 & RACEMASK_ALL_PLAYABLE)) {
      sLog.outErrorDb("Race_class condition (entry %u, type %u) has invalid player class %u, skipped", m_entry,
                      m_condition, m_value1);
      return false;
    }

    if (m_value2 && !(m_value2 & CLASSMASK_ALL_PLAYABLE)) {
      sLog.outErrorDb("Race_class condition (entry %u, type %u) has invalid race mask %u, skipped", m_entry,
                      m_condition, m_value2);
      return false;
    }
    break;
  ...
  case CONDITION_LEVEL: {
    if (!m_value1 || m_value1 > sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL)) {
      sLog.outErrorDb("Level condition (entry %u, type %u)has invalid level %u, skipped", m_entry, m_condition,
                      m_value1);
      return false;
    }

    if (m_value2 > 2) {
      sLog.outErrorDb("Level condition (entry %u, type %u) has invalid argument %u (must be 0..2), skipped", m_entry,
                      m_condition, m_value2);
      return false;
    }
  ...
  case CONDITION_SPELL: {
    if (!sSpellTemplate.LookupEntry<SpellEntry>(m_value1)) {
      sLog.outErrorDb("Spell condition (entry %u, type %u) requires to have non existing spell (Id: %d), skipped",
                      m_entry, m_condition, m_value1);
      return false;
    }

    if (m_value2 > 1) {
      sLog.outErrorDb("Spell condition (entry %u, type %u) has invalid argument %u (must be 0..1), skipped", m_entry,
                      m_condition, m_value2);
      return false;
    }
  ...
  case CONDITION_LEARNABLE_ABILITY: {
    ...
    if (bounds.first == bounds.second) {
      sLog.outErrorDb("Learnable ability conditon (entry %u, type %u) has spell id %u defined, but this spell is not "
                      "listed in SkillLineAbility and can not be used, skipping.",
                      m_entry, m_condition, m_value1);
      return false;
    }
    ...
    if (m_value2) {
      ...
      if (!proto) {
        sLog.outErrorDb("Learnable ability conditon (entry %u, type %u) has item entry %u defined but item does not "
                        "exist, skipping.",
                        m_entry, m_condition, m_value2);
        return false;
      }
  ...
  case CONDITION_CREATURE_IN_RANGE: {
    if (!sCreatureStorage.LookupEntry<CreatureInfo>(m_value1)) {
      sLog.outErrorDb("Creature in range condition (entry %u, type %u) has an invalid value in value1. (Creature %u "
                      "does not exist in the database), skipping.",
                      m_entry, m_condition, m_value1);
      return false;
    }
    if (m_value2 <= 0) {
      sLog.outErrorDb("Creature in range condition (entry %u, type %u) has an invalid value in value2. (Range %u must "
                      "be greater than 0), skipping.",
                      m_entry, m_condition, m_value2);
      return false;
    }
    break;
  ...
  case CONDITION_COMPLETED_ENCOUNTER: {
    ...
    DungeonEncounterEntry const *dbcEntry2 = sDungeonEncounterStore.LookupEntry<DungeonEncounterEntry>(m_value2);
    if (!dbcEntry1) {
      sLog.outErrorDb("Completed Encounter condition (entry %u, type %u) has an unknown DungeonEncounter entry %u "
                      "defined (in value1), skipping.",
                      m_entry, m_condition, m_value1);
      return false;
    }
    if (m_value2 && !dbcEntry2) {
      sLog.outErrorDb("Completed Encounter condition (entry %u, type %u) has an unknown DungeonEncounter entry %u "
                      "defined (in value2), skipping.",
                      m_entry, m_condition, m_value2);
      return false;
    }
    if (dbcEntry2 && dbcEntry1->mapId != dbcEntry2->mapId) {
      sLog.outErrorDb(
          "Completed Encounter condition (entry %u, type %u) has different mapIds for both encounters, skipping.",
          m_entry, m_condition);
      return false;
    }
    break;
  ...
  case CONDITION_LAST_WAYPOINT: {
    if (m_value2 > 2) {
      sLog.outErrorDb("Last Waypoint condition (entry %u, type %u) has an invalid value in value2. (Has %u, supported "
                      "0, 1, or 2), skipping.",
                      m_entry, m_condition, m_value2);
      return false;
    }
  ...
  case CONDITION_GENDER: {
    if (m_value1 >= MAX_GENDER) {
      sLog.outErrorDb("Gender condition (entry %u, type %u) has an invalid value in value1. (Has %u, must be smaller "
                      "than %u), skipping.",
                      m_entry, m_condition, m_value1, MAX_GENDER);
      return false;
    }
    break;
  }
  case CONDITION_DEAD_OR_AWAY: {
    if (m_value1 >= 4) {
      sLog.outErrorDb("Dead condition (entry %u, type %u) has an invalid value in value1. (Has %u, must be smaller "
                      "than 4), skipping.",
                      m_entry, m_condition, m_value1);
      return false;
    }
  ...
  case CONDITION_SPAWN_COUNT: {
    if (!sCreatureStorage.LookupEntry<CreatureInfo>(m_value1)) {
      sLog.outErrorDb("Spawn count condition (entry %u, type %u) has an invalid value in value1. (Creature %u does not "
                      "exist in the database), skipping.",
                      m_entry, m_condition, m_value1);
      return false;
    }
    break;
...
bool IsConditionSatisfied(uint32 conditionId, WorldObject const *target, Map const *map, WorldObject const *source,
                          ConditionSource conditionSourceType) {
