...
struct AreaTrigger {
  ...
  bool IsMinimal() const {
    return requiredLevel == 0 && requiredItem == 0 && requiredItem2 == 0 && heroicKey == 0 && heroicKey2 == 0 &&
           requiredQuest == 0 && requiredQuestHeroic == 0;
  }
  ...
  bool IsLessOrEqualThan(AreaTrigger const *l) const // Expected to have same map
  {
    ...
    return requiredLevel <= l->requiredLevel && requiredItem <= l->requiredItem && requiredItem2 <= l->requiredItem2 &&
           heroicKey <= l->heroicKey && heroicKey2 <= l->heroicKey2 && requiredQuest <= l->requiredQuest &&
           requiredQuestHeroic <= l->requiredQuestHeroic;
  }
};
...
public:
  FindCreatureData(uint32 id, Player *player)
      : i_id(id), i_player(player), i_anyData(nullptr), i_mapData(nullptr), i_mapDist(0.0f), i_spawnedData(nullptr),
        i_spawnedDist(0.0f) {}
...
typedef std::pair<ExclusiveQuestGroupsMap::const_iterator, ExclusiveQuestGroupsMap::const_iterator>
    ExclusiveQuestGroupsMapBounds;
typedef std::pair<ItemRequiredTargetMap::const_iterator, ItemRequiredTargetMap::const_iterator>
...
// We assume the rate is in general the same for all three types below, but chose to keep three for scalability and
// customization
...
struct DungeonEncounter {
  DungeonEncounter(DungeonEncounterEntry const *_dbcEntry, EncounterCreditType _creditType, uint32 _creditEntry,
                   uint32 _lastEncounterDungeon)
      : dbcEntry(_dbcEntry), creditType(_creditType), creditEntry(_creditEntry),
...
#define MAX_INTERNAL_PLAYER_NAME                                                                                       \
  15                        // max server internal player name length ( > MAX_PLAYER_NAME for support declined names )
...
public:
  ...
  std::unordered_map<uint32, std::vector<uint32>> const &GetCreatureSpawnEntry() const {
    return mCreatureSpawnEntryMap;
  }
  ...
  static CreatureModelInfo const *
  GetCreatureModelInfo(uint32 modelid);                             ///< Wrapper for sCreatureModelStorage.LookupEntry
  ...
  static CreatureDataAddon const *
  GetCreatureAddon(uint32 lowguid); ///< Wrapper for sCreatureDataAddonStorage.LookupEntry
  static CreatureDataAddon const *
  GetCreatureTemplateAddon(uint32 entry);                         ///< Wrapper for sCreatureInfoAddonStorage.LookupEntry
  ...
  static CreatureConditionalSpawn const *
  GetCreatureConditionalSpawn(uint32 lowguid); ///< Wrapper for sCreatureConditionalSpawnStore.LookupEntry
  ...
  template <typename Worker> void DoCreatureData(Worker &worker) const {
  ...
  void GetCreatureLocaleStrings(uint32 entry, int32 loc_idx, char const **namePtr,
                                char const **subnamePtr = nullptr) const;
  ...
  QuestgiverGreetingLocale const *GetQuestgiverGreetingLocale(uint32 entry, uint32 type) const {
    ...
    if (itr == m_questgiverGreetingLocaleMap[type].end())
      return nullptr;
    return &itr->second;
  }
  ...
  AreaTriggerLocale const *GetAreaTriggerLocale(uint32 entry) const {
    ...
    if (itr == m_areaTriggerLocaleMap.end())
      return nullptr;
    return &itr->second;
  }
  ...
  template <typename Worker> void DoGOData(Worker &worker) const {
  ...
  inline const char *GetMangosStringForDbcLocale(int32 entry) const {
    return GetMangosString(entry, m_Dbc2StorageLocaleIndex);
  }
  ...
  bool IsVendorItemValid(bool isTemplate, char const *tableName, uint32 vendor_entry, uint32 item_id, uint32 maxcount,
                         uint32 incrtime, uint32 ExtendedCost, uint16 conditionId, Player *pl = nullptr,
...
protected:
  ...
  // guids from reserved range for use in .npc add/.gobject add commands for adding new static spawns (saved in DB) from
  // client.
  ...
  typedef std::pair<CreatureModelRaceMap::const_iterator, CreatureModelRaceMap::const_iterator>
      CreatureModelRaceMapBounds;
...
bool LoadMangosStrings(DatabaseType &db, char const *table, int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID,
                       int32 end_value = std::numeric_limits<int32>::min(), bool extra_content = false);
