...
#define RACEMASK_ALL_PLAYABLE                                                                                          \
  ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_ORC - 1)) | (1 << (RACE_DWARF - 1)) | (1 << (RACE_NIGHTELF - 1)) |            \
   (1 << (RACE_UNDEAD - 1)) | (1 << (RACE_TAUREN - 1)) | (1 << (RACE_GNOME - 1)) | (1 << (RACE_TROLL - 1)) |           \
   (1 << (RACE_BLOODELF - 1)) | (1 << (RACE_DRAENEI - 1)))

// for most cases batter use ChrRace data for team check as more safe, but when need full mask of team can be use this
// defines.
#define RACEMASK_ALLIANCE                                                                                              \
  ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_DWARF - 1)) | (1 << (RACE_NIGHTELF - 1)) | (1 << (RACE_GNOME - 1)) |          \
   (1 << (RACE_DRAENEI - 1)))

#define RACEMASK_HORDE                                                                                                 \
  ((1 << (RACE_ORC - 1)) | (1 << (RACE_UNDEAD - 1)) | (1 << (RACE_TAUREN - 1)) | (1 << (RACE_TROLL - 1)) |             \
   (1 << (RACE_BLOODELF - 1)))
...
#define CLASSMASK_ALL_PLAYABLE                                                                                         \
  ((1 << (CLASS_WARRIOR - 1)) | (1 << (CLASS_PALADIN - 1)) | (1 << (CLASS_HUNTER - 1)) | (1 << (CLASS_ROGUE - 1)) |    \
   (1 << (CLASS_PRIEST - 1)) | (1 << (CLASS_SHAMAN - 1)) | (1 << (CLASS_MAGE - 1)) | (1 << (CLASS_WARLOCK - 1)) |      \
   (1 << (CLASS_DRUID - 1)))
...
enum Stats { STAT_STRENGTH = 0, STAT_AGILITY = 1, STAT_STAMINA = 2, STAT_INTELLECT = 3, STAT_SPIRIT = 4 };
...
enum SpellSchoolMask : uint32 {
  ...
  SPELL_SCHOOL_MASK_SPELL = (SPELL_SCHOOL_MASK_FIRE | SPELL_SCHOOL_MASK_NATURE | SPELL_SCHOOL_MASK_FROST |
...
enum SpellAttributes {
  ...
  SPELL_ATTR_HIDDEN_CLIENTSIDE =
      0x00000080, // 7 Spells with this attribute are not visible in spellbook or aura bar TODO: check usage
  ...
  SPELL_ATTR_TARGET_MAINHAND_ITEM =
      0x00000200, // 9 Client automatically selects item from mainhand slot as a cast target TODO: Implement
  ...
  SPELL_ATTR_DONT_AFFECT_SHEATH_STATE =
      0x00040000, // 18 client won't hide unit weapons in sheath on cast/channel TODO: Implement
  ...
  SPELL_ATTR_DISABLED_WHILE_ACTIVE =
      0x02000000, // 25 Activate and start cooldown after aura fade or remove summoned creature or go
...
enum SpellAttributesEx {
  ...
  SPELL_ATTR_EX_UNAUTOCASTABLE_BY_CHARMED =
      0x00020000, // 17 TODO: Investigate more: SPELL_ATTR_EX_PLAYER_CANT_CAST_CHARMED, likely related to MC
...
enum SpellAttributesEx2 {
  ...
  SPELL_ATTR_EX2_NOT_RESET_AUTO_ACTIONS =
      0x00020000, // 17 suspend weapon timer instead of resetting it, (?Hunters Shot and Stings only have this flag?)
  ...
  SPELL_ATTR_EX2_DAMAGE_REDUCED_SHIELD = 0x00200000,   // 21 for ice blocks, pala immunity buffs, priest absorb shields,
                                                       // but used also for other spells -> not sure!
...
enum SpellAttributesEx3 {
  SPELL_ATTR_EX3_OUT_OF_COMBAT_ATTACK = 0x00000001, // 0 Spell landed counts as hostile action against enemy even if it
                                                    // doesn't trigger combat state, propagates PvP flags
  ...
  SPELL_ATTR_EX3_TRIGGERED_CAN_TRIGGER_SPECIAL =
      0x00000200,                           // 9 Can only proc auras with SPELL_ATTR_EX3_CAN_PROC_FROM_TRIGGERED_SPECIAL
  ...
  SPELL_ATTR_EX3_CAN_PROC_FROM_TRIGGERED_SPECIAL =
      0x04000000, // 26 Auras with this attribute can proc off SPELL_ATTR_EX3_TRIGGERED_CAN_TRIGGER_SPECIAL
...
enum SpellAttributesEx4 {
  ...
  SPELL_ATTR_EX4_CAN_CAST_WHILE_CASTING =
      0x00000080, // 7 In theory, can use this spell while another is channeled/cast/autocast
  SPELL_ATTR_EX4_STACK_DOT_MODIFIER = 0x00000100, // 8 no effect on non DoTs?
  SPELL_ATTR_EX4_TRIGGER_ACTIVATE =
      0x00000200, // 9 initially disabled / trigger activate from event (Execute, Riposte, Deep Freeze end other)
  ...
  SPELL_ATTR_EX4_UNK29 =
      0x20000000, // 29 Related to client selfcheck about dispel in 2.4.3 client but no spells, CC seems to have them
...
enum SpellAttributesEx5 {
  ...
  SPELL_ATTR_EX5_DONT_SHOW_AURA_IF_SELF_CAST =
      0x08000000, // 27 Auras with this attribute are not visible on units that are the caster
  SPELL_ATTR_EX5_DONT_SHOW_AURA_IF_NOT_SELF_CAST =
      0x10000000,                    // 28 Auras with this attribute are not visible on units that are not the caster
  SPELL_ATTR_EX5_UNK29 = 0x20000000, // 29
  SPELL_ATTR_EX5_UNK30 = 0x40000000, // 30
  SPELL_ATTR_EX5_USE_PHYSICAL_HIT_CHANCE = 0x80000000, // 31 Introduced in patch 2.3: Taunt, Growl, etc spells use
                                                       // ability miss calculation (see implementation for details)
};
...
enum SpellAttributesEx6 {
  ...
  SPELL_ATTR_EX6_USE_SPELL_CAST_EVENT = 0x00000040, // 6 Auras with this attribute trigger SPELL_CAST combat log event
                                                    // instead of SPELL_AURA_START (clientside attribute)
  ...
  SPELL_ATTR_EX6_UNK12 = 0x00001000, // 12 for auras SPELL_AURA_TRACK_CREATURES, SPELL_AURA_TRACK_RESOURCES and
                                     // SPELL_AURA_TRACK_STEALTHED select non-stacking tracking spells
  ...
  SPELL_ATTR_EX6_NO_DMG_PERCENT_MODS =
      0x20000000,                    // 29 do not apply damage mods (usually in cases where it has already been applied)
...
static inline Team GetTeamIdByTeamIndex(PvpTeamIndex teamIndex) {
  return teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE;
}
static inline PvpTeamIndex GetTeamIndexByTeamId(Team team) {
  return team == ALLIANCE ? TEAM_INDEX_ALLIANCE : TEAM_INDEX_HORDE;
}
...
uint32 const CREATURE_TYPEMASK_MECHANICAL_OR_ELEMENTAL =
    (1 << (CREATURE_TYPE_MECHANICAL - 1)) | (1 << (CREATURE_TYPE_ELEMENTAL - 1));
...
enum CreatureTypeFlags {
  ...
  CREATURE_TYPEFLAGS_GHOST_VISIBLE = 0x00000002, // Creatures which can _also_ be seen when player is a ghost, used in
                                                 // CanInteract function by client, can't be attacked
  ...
  CREATURE_TYPEFLAGS_UNK7 = 0x00000040, // may be related to attackable / not attackable creatures with spells, used
                                        // together with lua_IsHelpfulSpell/lua_IsHarmfulSpell
  ...
  CREATURE_TYPEFLAGS_CAN_ASSIST =
      0x00001000,                        // Can aid any player (and group) in combat. Typically seen for escorting NPC's
...
enum DiminishingGroup {
  ...
  DIMINISHING_UNSTABLE_AFFLICTION_SILENCE, // The silence from this ability is now subject to diminishing returns. -
                                           // https://wow.gamepedia.com/Patch_2.3.0 Shared Class Specific
...
enum InstanceResetMethod {
  ...
  INSTANCE_RESET_RESPAWN_DELAY // called from reset scheduler for request reset at map unload when map loaded at reset
...
enum ShapeshiftFormFlags {
  SHAPESHIFT_FORM_FLAG_ALLOW_ACTIVITY =
      0x00000001, // Form allows various player activities, which normally cause "You can't X while shapeshifted."
                  // errors (npc/go interaction, item use, etc)
  SHAPESHIFT_FORM_FLAG_UNK2 = 0x00000002,
  SHAPESHIFT_FORM_FLAG_UNK3 = 0x00000004,
  SHAPESHIFT_FORM_FLAG_ALLOW_NPC_INTERACT = 0x00000008, // Form unconditionally allows talking to NPCs while
                                                        // shapeshifted (even if other activities are disabled)
...
enum WorldStateType { WORLD_STATE_REMOVE = 0, WORLD_STATE_ADD = 1 };
