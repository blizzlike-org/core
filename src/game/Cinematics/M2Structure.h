...
#pragma pack(push, 1)
template <typename T> struct M2SplineKey {
  T p0;
  T p1;
  T p2;
};
...
struct M2Header {
  ...
  uint32 GlobalModelFlags; // 0x0001: tilt x, 0x0002: tilt y, 0x0008: add 2 fields in header, 0x0020: load .phys data
                           // (MoP+), 0x0080: has _lod .skin files (MoP?+), 0x0100: is camera related.
