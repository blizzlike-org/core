...
void CinematicMgr::BeginCinematic() {
  ...
  if (itr != sFlyByCameraStore.end()) {
    ...
    if (camitr != m_cinematicCamera->end()) {
      ...
      m_CinematicObject = player->SummonCreature(VISUAL_WAYPOINT, pos.x, pos.y, pos.z, 0.0f, TEMPSPAWN_TIMED_DESPAWN,
...
bool CinematicMgr::UpdateCinematicLocation(uint32 /*diff*/) {
  ...
  Position interPosition = {lastPosition.x + (xDiff * (float(interDiff) / float(timeDiff))),
                            lastPosition.y + (yDiff * (float(interDiff) / float(timeDiff))),
                            lastPosition.z + (zDiff * (float(interDiff) / float(timeDiff))), 0.0f};
