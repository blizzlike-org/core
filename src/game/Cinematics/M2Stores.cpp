...
G3D::Vector3 TranslateLocation(G3D::Vector4 const *DBCPosition, G3D::Vector3 const *basePosition,
...
bool readCamera(M2Camera const *cam, uint32 buffSize, M2Header const *header, CinematicCameraEntry const *dbcentry) {
  ...
  if (cam->target_positions.timestamps.offset_elements + sizeof(M2Array) > buffSize)
    return false;
  uint32 const *targTimestamps =
      reinterpret_cast<uint32 const *>(buffer + cam->target_positions.timestamps.offset_elements);
  M2SplineKey<G3D::Vector3> const *targPositions =
      reinterpret_cast<M2SplineKey<G3D::Vector3> const *>(buffer + cam->target_positions.values.offset_elements);
  ...
  M2SplineKey<G3D::Vector3> const *positions =
      reinterpret_cast<M2SplineKey<G3D::Vector3> const *>(buffer + cam->positions.values.offset_elements);
  ...
  for (uint32 i = 0; i < sCinematicCameraStore.GetNumRows(); ++i) {
    if (CinematicCameraEntry const *dbcentry = sCinematicCameraStore.LookupEntry(i)) {
      ...
      if (header->ofsCameras + sizeof(M2Camera) > static_cast<uint32 const>(fileSize)) {
        sLog.outError("Camera file %s is damaged. Camera references position beyond file end (header)",
                      filename.c_str());
        continue;
      }
      ...
      if (!readCamera(cam, fileSize, header, dbcentry))
        sLog.outError("Camera file %s is damaged. Camera references position beyond file end (camera)",
                      filename.c_str());
    }
  }
  sLog.outString(">> Loaded %u cinematic waypoint sets in %u ms", static_cast<uint32 const>(sFlyByCameraStore.size()),
                 WorldTimer::getMSTimeDiff(oldMSTime, WorldTimer::getMSTime()));
  sLog.outString();
}
