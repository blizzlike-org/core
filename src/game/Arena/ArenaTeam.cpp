...
bool ArenaTeam::Create(ObjectGuid captainGuid, ArenaType type, std::string arenaTeamName) {
  ...
  CharacterDatabase.PExecute(
      ...
      "(arenateamid,name,captainguid,type,BackgroundColor,EmblemStyle,EmblemColor,BorderStyle,BorderColor) "
      "VALUES('%u','%s','%u','%u','%u','%u','%u','%u','%u')",
      m_TeamId, arenaTeamName.c_str(), m_CaptainGuid.GetCounter(), m_Type, m_BackgroundColor, m_EmblemStyle,
      m_EmblemColor, m_BorderStyle, m_BorderColor);
  CharacterDatabase.PExecute("INSERT INTO arena_team_stats (arenateamid, rating, games_week, wins_week, games_season, "
                             "wins_season, `rank`) VALUES "
                             "('%u', '%u', '%u', '%u', '%u', '%u', '%u')",
                             m_TeamId, m_stats.rating, m_stats.games_week, m_stats.wins_week, m_stats.games_season,
                             m_stats.wins_season, m_stats.rank);
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid) {
  ...
  if (pl) {
    ...
    //                                                     0     1
    QueryResult *result =
        CharacterDatabase.PQuery("SELECT name, class FROM characters WHERE guid='%u'", playerGuid.GetCounter());
    if (!result)
      return false;
  ...
  CharacterDatabase.PExecute(
      "INSERT INTO arena_team_member (arenateamid, guid, personal_rating) VALUES ('%u', '%u', '%u')", m_TeamId,
      newmember.guid.GetCounter(), newmember.personal_rating);
...
bool ArenaTeam::LoadMembersFromDB(QueryResult *arenaTeamMembersResult) {
  ...
  do {
    ...
    // prevent crash if db records are broken, when all members in result are already processed and current team hasn't
    // got any members
    if (!fields)
      break;
    ...
    if (arenaTeamId < m_TeamId) {
      ...
      sLog.outErrorDb("ArenaTeam %u does not exist but it has record in arena_team_member table, deleting it!",
                      arenaTeamId);
    ...
    if (newmember.name.empty()) {
      sLog.outErrorDb(
          "ArenaTeam %u has member with empty name - probably player %s doesn't exist, deleting him from memberlist!",
...
void ArenaTeam::SetCaptain(ObjectGuid guid) {
  ...
  // update database
  CharacterDatabase.PExecute("UPDATE arena_team SET captainguid = '%u' WHERE arenateamid = '%u'", guid.GetCounter(),
                             m_TeamId);
...
void ArenaTeam::DelMember(ObjectGuid guid) {
  ...
  CharacterDatabase.PExecute("DELETE FROM arena_team_member WHERE arenateamid = '%u' AND guid = '%u'", GetId(),
                             guid.GetCounter());
}
...
void ArenaTeam::Disband(WorldSession *session) {
  ...
  CharacterDatabase.PExecute(
      ...
      m_TeamId); //< this should be already done by calling DelMember(memberGuids[j]); for each member
...
void ArenaTeam::SetEmblem(uint32 backgroundColor, uint32 emblemStyle, uint32 emblemColor, uint32 borderStyle,
                          uint32 borderColor) {
  ...
  CharacterDatabase.PExecute("UPDATE arena_team SET BackgroundColor='%u', EmblemStyle='%u', EmblemColor='%u', "
                             "BorderStyle='%u', BorderColor='%u' WHERE arenateamid='%u'",
                             m_BackgroundColor, m_EmblemStyle, m_EmblemColor, m_BorderStyle, m_BorderColor, m_TeamId);
}
...
void ArenaTeam::SetStats(uint32 stat_type, uint32 value) {
  ...
  case STAT_TYPE_GAMES_WEEK:
    ...
    CharacterDatabase.PExecute("UPDATE arena_team_stats SET games_week = '%u' WHERE arenateamid = '%u'", value,
                               GetId());
    break;
  ...
  case STAT_TYPE_GAMES_SEASON:
    ...
    CharacterDatabase.PExecute("UPDATE arena_team_stats SET games_season = '%u' WHERE arenateamid = '%u'", value,
                               GetId());
    break;
  case STAT_TYPE_WINS_SEASON:
    m_stats.wins_season = value;
    CharacterDatabase.PExecute("UPDATE arena_team_stats SET wins_season = '%u' WHERE arenateamid = '%u'", value,
                               GetId());
    break;
  case STAT_TYPE_RANK:
    m_stats.rank = value;
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, ObjectGuid guid, char const *str1 /*=nullptr*/,
...
void ArenaTeam::UpdateArenaPointsHelper(std::map<uint32, uint32> &PlayerPoints) {
  ...
  // helper function for arena point distribution (this way, when distributing, no actual calculation is required, just
  // a few comparisons) 10 played games per week is a minimum
  ...
  for (MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr) {
    ...
    // OBSOLETE : CharacterDatabase.PExecute("UPDATE arena_team_member SET points_to_add = '%u' WHERE arenateamid = '%u'
    // AND guid = '%u'", points_to_add, m_TeamId, itr->guid);
...
void ArenaTeam::SaveToDB() {
  ...
  CharacterDatabase.BeginTransaction();
  CharacterDatabase.PExecute("UPDATE arena_team_stats SET rating = '%u',games_week = '%u',games_season = '%u',`rank` = "
                             "'%u',wins_week = '%u',wins_season = '%u' WHERE arenateamid = '%u'",
                             m_stats.rating, m_stats.games_week, m_stats.games_season, m_stats.rank, m_stats.wins_week,
                             m_stats.wins_season, GetId());
  for (MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr) {
    CharacterDatabase.PExecute(
        "UPDATE arena_team_member SET played_week = '%u', wons_week = '%u', played_season = '%u', wons_season = '%u', "
        "personal_rating = '%u' WHERE arenateamid = '%u' AND guid = '%u'",
        itr->games_week, itr->wins_week, itr->games_season, itr->wins_season, itr->personal_rating, m_TeamId,
        itr->guid.GetCounter());
  }
  CharacterDatabase.CommitTransaction();
}
