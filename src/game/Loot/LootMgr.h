...
enum LootItemType { LOOTITEM_TYPE_NORMAL = 1, LOOTITEM_TYPE_QUEST = 2, LOOTITEM_TYPE_CONDITIONNAL = 3 };
...
public:
  ...
  GroupLootRoll()
      : m_rollVoteMap(ROLL_VOTE_MASK_ALL), m_isStarted(false), m_lootItem(nullptr), m_loot(nullptr), m_itemSlot(0),
        m_voteMask(), m_endTime(0) {}
  ~GroupLootRoll();
...
struct LootStoreItem {
  ...
  uint8 maxcount : 8;      // max drop count for the item (mincountOrRef positive) or Ref multiplicator (mincountOrRef
                           // negative)
  uint16 conditionId : 16; // additional loot condition Id

  // Constructor, converting ChanceOrQuestChance -> (chance, needs_quest)
  // displayid is filled in IsValid() which must be called after
  LootStoreItem(uint32 _itemid, float _chanceOrQuestChance, int8 _group, uint16 _conditionId, int32 _mincountOrRef,
                uint8 _maxcount)
      : itemid(_itemid), chance(fabs(_chanceOrQuestChance)), mincountOrRef(_mincountOrRef), group(_group),
        needs_quest(_chanceOrQuestChance < 0), maxcount(_maxcount), conditionId(_conditionId) {}
...
public:
  ...
  void AddItem(uint32 itemid, uint32 count, uint32 randomSuffix,
               int32 randomPropertyId); // used in item.cpp to explicitly load a saved item
...
private:
  Loot()
      : m_lootTarget(nullptr), m_itemTarget(nullptr), m_gold(0), m_maxSlot(0), m_lootType(), m_clientLootType(),
        m_lootMethod(), m_threshold(), m_maxEnchantSkill(0), m_haveItemOverThreshold(false), m_isChecked(false),
        m_isChest(false), m_isChanged(false), m_isFakeLoot(false) {}
  void Clear();
  ...
  bool IsLootOpenedBy(ObjectGuid const &playerGuid) const {
    return m_playersOpened.find(playerGuid) != m_playersOpened.end();
  }
