...
public:
  ...
  void Process(Loot &loot,
               Player const *lootOwner) const; // Rolls an item from the group (if any) and adds the item to the loot
...
private:
  ...
  LootStoreItem const *
  Roll(Loot const &loot,
       Player const *lootOwner) const; // Rolls an item from the group, returns nullptr if all miss their chances
};
...
void LootStore::LoadLootTable() {
  ...
  QueryResult *result = WorldDatabase.PQuery(
      "SELECT entry, item, ChanceOrQuestChance, groupid, mincountOrRef, maxcount, condition_id FROM %s", GetName());
  ...
  if (result) {
    ...
    do {
      ...
      if (maxcount > std::numeric_limits<uint8>::max()) {
        sLog.outErrorDb("Table '%s' entry %u item %u: maxcount value (%u) to large. must be less than %u - skipped",
      ...
      if (conditionId) {
        ...
        if (!condition) {
          sLog.outErrorDb(
              "Table `%s` for entry %u, item %u has condition_id %u that does not exist in `conditions`, ignoring",
              GetName(), entry, item, uint32(conditionId));
          continue;
        }

        if (mincountOrRef < 0 && !ConditionEntry::CanBeUsedWithoutPlayer(conditionId)) {
          sLog.outErrorDb("Table '%s' entry %u mincountOrRef %i < 0 and has condition %u that requires a player and is "
...
void LootStore::ReportUnusedIds(LootIdSet const &ids_set) const {
  ...
  if (!ids_set.empty()) {
    for (uint32 itr : ids_set)
      sLog.outErrorDb("Table '%s' entry %d isn't %s and not referenced from loot, and then useless.", GetName(), itr,
                      GetEntryName());
    sLog.outString();
  }
}
...
bool LootStoreItem::IsValid(LootStore const &store, uint32 entry) const {
  ...
  {
    sLog.outErrorDb("Table '%s' entry %d item %d: group (%u) must be less %u - skipped", store.GetName(), entry, itemid,
                    group, 1 << 7);
    return false;
  }

  if (mincountOrRef == 0) {
    sLog.outErrorDb("Table '%s' entry %d item %d: wrong mincountOrRef (%d) - skipped", store.GetName(), entry, itemid,
                    mincountOrRef);
    return false;
  }
  ...
  if (mincountOrRef > 0) // item (quest or non-quest) entry, maybe grouped
  {
    ...
    if (!proto) {
      sLog.outErrorDb("Table '%s' entry %d item %d: item entry not listed in `item_template` - skipped",
    ...
    {
      sLog.outErrorDb("Table '%s' entry %d item %d: equal-chanced grouped entry, but group not defined - skipped",
                      store.GetName(), entry, itemid);
      return false;
    }
    ...
    {
      sLog.outErrorDb("Table '%s' entry %d item %d: max count (%u) less that min count (%i) - skipped", store.GetName(),
                      entry, itemid, uint32(maxcount), mincountOrRef);
      return false;
    }
  } else // mincountOrRef < 0
  {
    if (needs_quest) {
      sLog.outErrorDb("Table '%s' entry %d item %d: negative chance is given for a reference, skipped", store.GetName(),
                      entry, itemid);
      return false;
    }
    ...
    {
      sLog.outErrorDb(
          "Table '%s' entry %d item %d: zero chance is given for a reference, reference will never be used, skipped",
          store.GetName(), entry, itemid);
      return false;
    }
  }
...
LootSlotType LootItem::GetSlotTypeForSharedLoot(Player const *player, Loot const *loot) const {
  ...
  if (freeForAll)
    return loot->m_lootMethod == NOT_GROUP_TYPE_LOOT
               ? LOOT_SLOT_OWNER
               : LOOT_SLOT_NORMAL; // player have not yet looted a free for all item
  ...
  if (lootItemType == LOOTITEM_TYPE_QUEST || lootItemType == LOOTITEM_TYPE_CONDITIONNAL) {
    ...
    default:
      ...
      // Check if its turn of that player to loot a not party loot. The loot may be released or the item may be passed
      // by currentLooter
...
bool GroupLootRoll::TryToStart(Loot &loot, uint32 itemSlot) {
  if (!m_isStarted) {
    ...
    for (auto itr : m_loot->m_ownerSet) {
      ...
      if (!plr ||
          !m_lootItem->IsAllowed(plr, m_loot)) // check if player meet the condition to be able to roll this item
      {
 ...
 * \param: RollVoteMap::const_iterator& winnerItr > will be different than m_rollCoteMap.end() if winner exist. (Someone
 *voted greed or need) \returns: bool > true if all players voted
 **/
    ...
    for (auto lootItem : m_lootItems) {
      if (player && (lootItem->AllowedForPlayer(player, GetLootTarget(), masterLooter))) {
        ...
        if (playerGuid == m_currentLooterGuid)
          lootItem->currentLooterPass = true; // Some item may not be allowed for current looter, must set this flag to
                                              // avoid item not distributed to other player
      }
    }
  }
...
bool Loot::IsLootedFor(Player const *player) const { return (GetLootStatusFor(player) == 0); }
...
bool Loot::CanLoot(Player const *player) {
  ...
  // if the player is the current looter (his turn to loot under threshold item) or the current looter released the loot
  // then the player can loot
...
void Loot::Release(Player *player) {
  ...
  case HIGHGUID_GAMEOBJECT: {
    ...
    case GAMEOBJECT_TYPE_CHEST: {
      ...
      if (refill) {
        // this vein have can be now refilled, as this is a vein no other player are looting it so no need to send them
        // release
    ...
    for (auto lootItem : m_lootItems) {
      ...
      if (m_roll.find(itemSlot) == m_roll.end() && lootItem->IsAllowed(player, this)) {
        if (!m_roll[itemSlot].TryToStart(*this, itemSlot)) // Create and try to start a roll
          m_roll.erase(m_roll.find(
              itemSlot)); // Cannot start roll so we have to delete it (find will not fail as the item was just created)
      }
    }
  }
...
void Loot::SetGroupLootRight(Player *player) {
  ...
  if (grp && (!m_isChest || static_cast<GameObject *>(m_lootTarget)->GetGOInfo()->chest.groupLootRules)) {
    ...
    do {
      ++itr; // we start by next current looter position in list that way we have directly next looter in result
             // ownerSet (if any exist)
      if (itr == memberList.end())
        itr = memberList
                  .begin(); // reached the end of the list is possible so simply restart from the first element in it
      ...
      if (IsEligibleForLoot(looter, m_lootTarget)) {
        ...
        ownerList.push_back(
            itr->guid); // save this guid to local ordered GuidList (we need to keep it ordered only here)
    ...
    if (m_ownerSet.size() > 1 && m_lootMethod != FREE_FOR_ALL) {
      ...
      if (m_ownerSet.find(currentLooterGuid) == m_ownerSet.end()) {
        // as owner list is filled from NEXT current looter position we can assign first element in list as looter and
        // next element in list as next looter
...
Loot::Loot(Player *player, Creature *creature, LootType type)
  ...
  case LOOT_CORPSE: {
    ...
    sLog.outDebug("Loot::CreateLoot> cannot create corpse loot, FillLoot failed with loot id(%u)!",
                  creatureInfo->LootId);
    creature->SetLootStatus(CREATURE_LOOT_STATUS_LOOTED);
    break;
  }
  case LOOT_PICKPOCKETING: {
    ...
    if (!creatureInfo->PickpocketLootId ||
      ...
      sLog.outError("Loot::CreateLoot> cannot create pickpocket loot, FillLoot failed with loot id(%u)!",
  ...
  case LOOT_SKINNING: {
    ...
    if (!creatureInfo->SkinningLootId ||
      ...
      sLog.outError("Loot::CreateLoot> cannot create skinning loot, FillLoot failed with loot id(%u)!",
  ...
  default:
    sLog.outError("Loot::CreateLoot> Cannot create loot for %s with invalid LootType(%u)",
...
Loot::Loot(Player *player, GameObject *gameObject, LootType type)
  ...
  if ((type != LOOT_FISHINGHOLE &&
    ...
    sLog.outError("Loot::CreateLoot> cannot create game object loot, basic check failed for gameobject %u!",
                  gameObject->GetEntry());
    return;
  }
...
Loot::Loot(Player *player, Corpse *corpse, LootType type)
  ...
  if (!corpse->lootForBody) {
    ...
    if (Player *plr = sObjectAccessor.FindPlayer(corpse->GetOwnerGuid()))
      ...
      pLevel =
          player->getLevel(); // TODO:: not correct, need to save real player level in the corpse data in case of logout
  ...
  if (target->GetSession()) {
    ...
    if (msg == EQUIP_ERR_OK) {
      ...
      if (!m_isChest) {
        ...
        // for chest as the allowed guid should be empty we will add the looter guid so that mean it was looted from
        // target
...
bool Loot::AutoStore(Player *player, bool broadcast /*= false*/, uint32 bag /*= NULL_BAG*/,
                     uint32 slot /*= NULL_SLOT*/) {
  bool result = true;
...
void Loot::GetLootContentFor(Player *player, ByteBuffer &buffer) {
  ...
  for (LootItemList::const_iterator lootItemItr = m_lootItems.begin(); lootItemItr != m_lootItems.end();
    ...
    if (slot_type >= MAX_LOOT_SLOT_TYPE) {
      sLog.outDebug("Item not visible for %s> itemid(%u) in slot (%u)!", player->GetGuidStr().c_str(), lootItem->itemId,
...
void LootTemplate::LootGroup::Verify(LootStore const &lootstore, uint32 id, uint32 group_id) const {
  ...
  if (chance >= 100.0f && !EqualChanced.empty()) {
    sLog.outErrorDb("Table '%s' entry %u group %d has items with chance=0%% but group total chance >= 100%% (%f)",
...
void LootTemplate::Process(Loot &loot, Player const *lootOwner, LootStore const &store, bool rate,
                           uint8 groupId) const {
