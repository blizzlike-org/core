...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPacket &recv_data) {
  ...
  if (lootItem->isBlocked || lootItem->GetSlotTypeForSharedLoot(_player, loot) == MAX_LOOT_SLOT_TYPE) {
    sLog.outDebug("HandleAutostoreLootItemOpcode> %s have no right to loot itemId(%u)", _player->GetGuidStr().c_str(),
                  lootItem->itemId);
    loot->SendReleaseFor(_player);
    return;
  }
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket &recv_data) {
  ...
  if (!target) {
    ...
    sLog.outError("WorldSession::HandleLootMasterGiveOpcode> Cannot retrieve target %s",
  ...
  if (!pLoot) {
    sLog.outError("WorldSession::HandleLootMasterGiveOpcode> Cannot retrieve loot for player %s",
  ...
  if (_player->GetObjectGuid() != pLoot->GetMasterLootGuid()) {
    ...
    sLog.outError("WorldSession::HandleLootMasterGiveOpcode> player %s is not the loot master!",
                  _player->GetGuidStr().c_str());
    return;
  }
  ...
  if (!_player->IsInGroup(target) || !_player->IsInMap(target)) {
    ...
    sLog.outError("WorldSession::HandleLootMasterGiveOpcode> Player %s tried to give an item to ineligible player %s !",
  ...
  if (lootItem->GetSlotTypeForSharedLoot(_player, pLoot) == MAX_LOOT_SLOT_TYPE) {
    sLog.outError("HandleAutostoreLootItemOpcode> %s have no right to loot itemId(%u)", _player->GetGuidStr().c_str(),
                  lootItem->itemId);
    return;
  }
