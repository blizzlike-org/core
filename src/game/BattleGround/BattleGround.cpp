...
public:
  ...
  void operator()(WorldPacket &data, int32 loc_idx) {
    ...
    if (i_args) {
      ...
      ChatHandler::BuildChatPacket(data, i_msgtype, &str[0], LANG_UNIVERSAL, CHAT_TAG_NONE, sourceGuid,
                                   sourceName.c_str());
    } else
      ChatHandler::BuildChatPacket(data, i_msgtype, text, LANG_UNIVERSAL, CHAT_TAG_NONE, sourceGuid, sourceName.c_str(),
                                   sourceGuid, sourceName.c_str());
  }
...
public:
  ...
  void operator()(WorldPacket &data, int32 loc_idx) {
    ...
    if (i_args) {
      ...
      ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_YELL, &str[0], i_language, CHAT_TAG_NONE,
                                   i_source->GetObjectGuid(), i_source->GetName());
    } else
      ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_YELL, text, i_language, CHAT_TAG_NONE,
                                   i_source->GetObjectGuid(), i_source->GetName());
  }
...
public:
  ...
  void operator()(WorldPacket &data, int32 loc_idx) {
    ...
    ChatHandler::BuildChatPacket(data, i_msgtype, str, LANG_UNIVERSAL, CHAT_TAG_NONE, ObjectGuid(), nullptr, guid,
                                 pName);
  }
...
public:
  ...
  void operator()(WorldPacket &data, int32 loc_idx) {
    ...
    ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_YELL, str, i_language, CHAT_TAG_NONE,
    ...
    if (itr != m_players.end()) {
      if (itr->second.offlineRemoveTime <= sWorld.GetGameTime()) {
        ...
        CharacterDatabase.PExecute("UPDATE characters SET at_login = at_login | '%u' WHERE guid = '%u'",
                                   uint32(AT_LOGIN_ADD_BG_DESERTER), itr->first.GetCounter());
...
void BattleGround::EndBattleGround(Team winner) {
  ...
  if (IsBattleGround() && sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_SCORE_STATISTICS)) {
    ...
    SqlStatement stmt = CharacterDatabase.CreateStatement(
        insPvPstatsBattleground,
        "INSERT INTO pvpstats_battlegrounds (id, winner_team, bracket_id, type, date) VALUES (?, ?, ?, ?, NOW())");
  ...
  if (IsArena() && IsRated()) {
    if (winner != TEAM_NONE) {
      ...
      if (winner_arena_team && loser_arena_team) {
        ...
        DEBUG_LOG("--- Winner rating: %u, Loser rating: %u, Winner change: %i, Loser change: %i ---", winner_rating,
                  loser_rating, winner_change, loser_change);
  ...
  for (auto &m_Player : m_players) {
    ...
    if (IsBattleGround() && sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_SCORE_STATISTICS)) {
      ...
      SqlStatement stmt = CharacterDatabase.CreateStatement(
          insPvPstatsPlayer,
          "INSERT INTO pvpstats_players (battleground_id, character_guid, score_killing_blows, score_deaths, "
          "score_honorable_kills, score_bonus_honor, score_damage_done, score_healing_done, attr_1, attr_2, attr_3, "
          "attr_4, attr_5) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    sBattleGroundMgr.BuildBattleGroundStatusPacket(data, this, plr->GetBattleGroundQueueIndex(bgQueueTypeId),
                                                   STATUS_IN_PROGRESS, TIME_TO_AUTOREMOVE, GetStartTime(),
                                                   GetArenaType(), plr->GetBGTeam());
    plr->GetSession()->SendPacket(data);
  }
...
void BattleGround::BlockMovement(Player *player) {
  ...
  // NOTE: control will be automatically reset by client when the player changes the map, so not needed to restore in
  // RemovePlayerAtLeave()
    ...
    if (player) {
      ...
      if (IsArena()) {
        player->RemoveArenaAuras(
            true);                  // removes debuffs / dots etc., we don't want the player to die after porting out
      ...
      if (doSendPacket) {
        WorldPacket data;
        sBattleGroundMgr.BuildBattleGroundStatusPacket(data, this, player->GetBattleGroundQueueIndex(bgQueueTypeId),
                                                       STATUS_NONE, 0, 0, ARENA_TYPE_NONE, TEAM_NONE);
        player->GetSession()->SendPacket(data);
      }

      // this call is important, because player, when joins to battleground, this method is not called, so it must be
      // called when leaving bg
...
void BattleGround::Reset() {
  ...
  if (m_invitedAlliance > 0 || m_invitedHorde > 0)
    sLog.outError("BattleGround system: bad counter, m_InvitedAlliance: %d, m_InvitedHorde: %d", m_invitedAlliance,
                  m_invitedHorde);
...
void BattleGround::StartBattleGround() {
  ...
  // This must be done here, because we need to have already invited some players when first BG::Update() method is
  // executed and it doesn't matter if we call StartBattleGround() more times, because m_BattleGrounds is a map and
...
void BattleGround::PlayerAddedToBgCheckIfBgIsRunning(Player *player) {
  ...
  sBattleGroundMgr.BuildBattleGroundStatusPacket(data, this, player->GetBattleGroundQueueIndex(bgQueueTypeId),
                                                 STATUS_IN_PROGRESS, GetEndTime(), GetStartTime(), GetArenaType(),
                                                 player->GetBGTeam());
  player->GetSession()->SendPacket(data);
}
...
GameObject *BattleGround::GetSingleGameObjectFromStorage(uint32 entry) const {
  ...
  sLog.outError("BattleGround requested gameobject with entry %u, but no gameobject of this entry was created yet, or "
                "it was not stored by battleground script for map %u.",
                entry, GetBgMap()->GetId());
...
Creature *BattleGround::GetSingleCreatureFromStorage(uint32 entry, bool skipDebugLog /*=false*/) const {
  ...
  // Output log, possible reason is not added GO to map, or not yet loaded;
  if (!skipDebugLog)
    script_error_log("BattleGround requested creature with entry %u, but no npc of this entry was created yet, or it "
                     "was not stored by script for map %u.",
                     entry, GetBgMap()->GetId());
