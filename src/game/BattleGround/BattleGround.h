...
enum BattleGroundSounds { SOUND_HORDE_WINS = 8454, SOUND_ALLIANCE_WINS = 8455, SOUND_BG_START = 3439 };
...
enum BattleGroundBracketId // bracketId for level ranges
{
  ...
  BG_BRACKET_ID_LAST =
      6, // so for start level 10 will be 10-19, 20-29, ...  all greater max bg level included in last breaket
};
...
public:
  ...
  virtual void Update(uint32 diff); // must be implemented in BG subclass of BG specific update code, but must in
  ...
  template <class Do> void BroadcastWorker(Do &_do);
  ...
  void SendYell2ToAll(int32 /*entry*/, uint32 /*language*/, Creature const * /*source*/, int32 /*arg1*/,
                      int32 /*arg2*/);
  ...
  uint32 GetAlivePlayersCountByTeam(Team /*team*/) const; // used in arenas to correctly handle death in spirit of
  ...
  static Team GetOtherTeam(Team team) {
    return (team == ALLIANCE || team == HORDE) ? ((team == ALLIANCE) ? HORDE : ALLIANCE) : TEAM_NONE;
  }

  // returns the other team index
  static PvpTeamIndex GetOtherTeamIndex(PvpTeamIndex teamIdx) {
    return teamIdx == TEAM_INDEX_ALLIANCE ? TEAM_INDEX_HORDE : TEAM_INDEX_ALLIANCE;
  }
  ...
  virtual bool IsConditionFulfilled(Player const * /*source*/, uint32 /*conditionId*/,
...
protected:
  // this method is called, when BG cannot spawn its own spirit guide, or something is wrong, It correctly ends
  // BattleGround
  void EndNow();
...
private:
  ...
  bool m_hasBgFreeSlotQueue; // used to make sure that BG is only once inserted into the
  ...
  /* Invited counters are useful for player invitation to BG - do not allow, if BG is started to one faction to have 2
   * more players than another faction */
  /* Invited counters will be changed only when removing already invited player from queue, removing player from
   * battleground and inviting player to BG */
