...
BattleGroundAV::BattleGroundAV()
    : m_honorMapComplete(0), m_repTowerDestruction(0), m_repCaptain(0), m_repBoss(0), m_repOwnedGrave(0),
      m_repOwnedMine(0), m_repSurviveCaptain(0), m_repSurviveTower(0) {
...
void BattleGroundAV::HandleQuestComplete(uint32 questid, Player *player) {
  ...
  case BG_AV_QUEST_ARMOR_SCRAPS_H_REPEAT:
    ...
    reputation = 1;
    if (m_teamQuestStatus[teamIdx][0] == 500 || m_teamQuestStatus[teamIdx][0] == 1000 ||
        m_teamQuestStatus[teamIdx][0] == 1500) // 25,50,75 turn ins
    {
  ...
  case BG_AV_QUEST_GALLON_OF_BLOOD:
    m_teamQuestStatus[teamIdx][4] +=
        4; // there are 2 quests where you can turn in 5 or 1 item.. ( + 4 cause +1 will be done some lines below)
    reputation = 4;
  ...
  case BG_AV_QUEST_COLDTOOTH_SUPPLIES_H:
    ...
    if (m_teamQuestStatus[teamIdx][5] == 28) {
      ...
      if (m_teamQuestStatus[teamIdx][6] == 7)
        DEBUG_LOG("BattleGroundAV: Quest %i completed (need to implement some events here - ground assault ready",
                  questid);
    }
    break;
  ...
  case BG_AV_QUEST_RAM_HIDE:
    ...
    if (m_teamQuestStatus[teamIdx][7] == 25) {
      ...
      if (m_teamQuestStatus[teamIdx][8] == 25)
        DEBUG_LOG("BattleGroundAV: Quest %i completed (need to implement some events here - rider assault ready",
                  questid);
    }
    break;
  ...
  if (reputation)
    RewardReputationToTeam((player->GetTeam() == ALLIANCE) ? BG_AV_FACTION_ID_STORMPIKE : BG_AV_FACTION_ID_FROSTWOLF,
...
void BattleGroundAV::Update(uint32 diff) {
  ...
  // looks for all timers of the nodes and destroy the building (for graveyards the building wont get destroyed, it goes
...
void BattleGroundAV::EndBattleGround(Team winner) {
  ...
  for (uint8 i = 0; i < PVP_TEAM_COUNT; ++i) {
    ...
    DEBUG_LOG("BattleGroundAV: Team %u wins the battleground. Controlled towers: %u bonus towers honor: %u bonus "
              "towers reputation: %u",
              i, m_homeTowersControlled[i], bonusHonor, bonusReputation);
...
void BattleGroundAV::ProcessPlayerDestroyedPoint(AVNodeIds node) {
  ...
  if (isTower) {
    ...
    RewardReputationToTeam((ownerTeam == ALLIANCE) ? BG_AV_FACTION_ID_STORMPIKE : BG_AV_FACTION_ID_STORMPIKE,
    ...
    sObjectMgr.SetGraveYardLinkTeam(avNodeDefaults[node].graveyardId, BG_AV_ZONE_MAIN,
                                    GetTeamIdByTeamIndex(ownerTeamIdx));
  }
}
...
void BattleGroundAV::ChangeMineOwner(AVMineIds mineId, PvpTeamIndex newOwnerTeamIdx) {
  // TODO changing the owner should result in setting respawntime to infinite for current creatures (they should fight
  // the new ones), spawning new mine owners creatures and changing the chest - objects so that the current owning team
  // can use them
  ...
  DoSendYellToTeam(newOwnerTeamIdx, LANG_BG_AV_MINE_TAKEN, 0, true,
                   (mineId == BG_AV_IRONDEEP_MINE_ID) ? LANG_BG_AV_MINE_NORTH : LANG_BG_AV_MINE_SOUTH);
...
void BattleGroundAV::ProcessPlayerDefendsPoint(Player *player, AVNodeIds node) {
  ...
  uint32 newState =
      teamIdx == TEAM_INDEX_ALLIANCE ? avNodeWorldStates[node].worldStateAlly : avNodeWorldStates[node].worldStateHorde;
  ...
  uint32 soundId = isTower                            ? BG_AV_SOUND_BOTH_TOWER_DEFEND
                   : (teamIdx == TEAM_INDEX_ALLIANCE) ? BG_AV_SOUND_ALLIANCE_GOOD
...
void BattleGroundAV::ProcessPlayerAssaultsPoint(Player *player, AVNodeIds node) {
  ...
  uint32 newState = teamIdx == TEAM_INDEX_ALLIANCE ? avNodeWorldStates[node].worldStateAllyGrey
                                                   : avNodeWorldStates[node].worldStateHordeGrey;
  ...
  if (!isTower) {
    // before despawn of GY spirit healer - need to TP players - must be after AssaultNode; opposite team of current
    // player
    if (Creature *spiritHealer = GetClosestCreatureWithEntry(
            player, teamIdx == TEAM_INDEX_ALLIANCE ? BG_NPC_SPIRIT_GUIDE_HORDE : BG_NPC_SPIRIT_GUIDE_ALLIANCE, 100.f))
...
void BattleGroundAV::FillInitialWorldStates(WorldPacket &data, uint32 &count) {
  ...
  // mine world states
  FillInitialWorldState(data, count, avMineWorldStates[BG_AV_IRONDEEP_MINE_ID][m_mineOwner[BG_AV_IRONDEEP_MINE_ID]],
                        WORLD_STATE_ADD);
  FillInitialWorldState(data, count, avMineWorldStates[BG_AV_COLDTOOTH_MINE_ID][m_mineOwner[BG_AV_COLDTOOTH_MINE_ID]],
                        WORLD_STATE_ADD);
}
...
void BattleGroundAV::DoSendYellToTeam(PvpTeamIndex teamIdx, int32 textId, uint8 nodeId, bool useNode /*= true*/,
                                      int32 mineLangId /*= 0*/) {
  // Get the specific herald for each team
  Creature *pHerald = GetSingleCreatureFromStorage(teamIdx == TEAM_INDEX_ALLIANCE ? BG_AV_NPC_STORMPIKE_HERALD
                                                                                  : BG_AV_NPC_FROSTWOLF_HERALD);
  if (!pHerald)
    return;
  ...
  if (useNode) {
    if (mineLangId)
      SendYell2ToAll(textId, LANG_UNIVERSAL, pHerald, (teamIdx == TEAM_INDEX_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE,
                     mineLangId);
    else
      SendYell2ToAll(textId, LANG_UNIVERSAL, pHerald, GetNodeMessageId(AVNodeIds(nodeId)),
...
bool BattleGroundAV::IsConditionFulfilled(Player const * /*source*/, uint32 /*conditionId*/,
