...
BattleGroundEY::BattleGroundEY()
    : m_flagState(), m_towersAlliance(0), m_towersHorde(0), m_honorTicks(0), m_flagRespawnTimer(0),
    ...
    if (eyTowers[i] == go->GetEntry()) {
      for (uint8 j = 0; j < 4; ++j) {
        if (eyTowerEvents[i][j].eventEntry == eventId) {
          ProcessCaptureEvent(go, i, eyTowerEvents[i][j].team, eyTowerEvents[i][j].worldState,
                              eyTowerEvents[i][j].message);
...
void BattleGroundEY::ProcessCaptureEvent(GameObject *go, uint32 towerId, Team team, uint32 newWorldState,
                                         uint32 message) {
  DEBUG_LOG("BattleGroundEY: Process capture point event from gameobject entry %u, captured by team %u", go->GetEntry(),
            team);
  ...
  if (oldTeam == ALLIANCE || oldTeam == HORDE) // only on going to grey
  {
    ...
    Creature *spiritHealer = GetClosestCreatureWithEntry(
        go, oldTeam == ALLIANCE ? BG_NPC_SPIRIT_GUIDE_ALLIANCE : BG_NPC_SPIRIT_GUIDE_HORDE, 100.f);
...
void BattleGroundEY::HandlePlayerDroppedFlag(Player *source) {
  ...
  UpdateWorldState(playerTeam == ALLIANCE ? WORLD_STATE_EY_NETHERSTORM_FLAG_STATE_ALLIANCE
                                          : WORLD_STATE_EY_NETHERSTORM_FLAG_STATE_HORDE,
                   WORLD_STATE_ADD);
  SendMessageToAll(LANG_BG_EY_DROPPED_FLAG,
                   playerTeam == ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE, nullptr);
}
...
void BattleGroundEY::FillInitialWorldStates(WorldPacket &data, uint32 &count) {
  ...
  // tower world states
  FillInitialWorldState(data, count, WORLD_STATE_EY_BLOOD_ELF_TOWER_ALLIANCE,
                        m_towerOwner[NODE_BLOOD_ELF_TOWER] == ALLIANCE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_BLOOD_ELF_TOWER_HORDE, m_towerOwner[NODE_BLOOD_ELF_TOWER] == HORDE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_BLOOD_ELF_TOWER_NEUTRAL,
                        m_towerOwner[NODE_BLOOD_ELF_TOWER] == TEAM_NONE);

  FillInitialWorldState(data, count, WORLD_STATE_EY_FEL_REAVER_RUINS_ALLIANCE,
                        m_towerOwner[NODE_FEL_REAVER_RUINS] == ALLIANCE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_FEL_REAVER_RUINS_HORDE,
                        m_towerOwner[NODE_FEL_REAVER_RUINS] == HORDE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_FEL_REAVER_RUINS_NEUTRAL,
                        m_towerOwner[NODE_FEL_REAVER_RUINS] == TEAM_NONE);
  ...
  FillInitialWorldState(data, count, WORLD_STATE_EY_DRAENEI_RUINS_ALLIANCE,
                        m_towerOwner[NODE_DRAENEI_RUINS] == ALLIANCE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_DRAENEI_RUINS_HORDE, m_towerOwner[NODE_DRAENEI_RUINS] == HORDE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_DRAENEI_RUINS_NEUTRAL,
                        m_towerOwner[NODE_DRAENEI_RUINS] == TEAM_NONE);

  // flag states
  FillInitialWorldState(data, count, WORLD_STATE_EY_NETHERSTORM_FLAG_READY, m_flagState == EY_FLAG_STATE_ON_BASE);
  FillInitialWorldState(data, count, WORLD_STATE_EY_NETHERSTORM_FLAG_STATE_ALLIANCE,
                        m_flagState == EY_FLAG_STATE_ON_ALLIANCE_PLAYER ? 2 : 1);
  FillInitialWorldState(data, count, WORLD_STATE_EY_NETHERSTORM_FLAG_STATE_HORDE,
                        m_flagState == EY_FLAG_STATE_ON_HORDE_PLAYER ? 2 : 1);

  // capture point states
  // if you leave the bg while being in capture point radius - and later join same type of bg the slider would still be
  // displayed because the client caches it
