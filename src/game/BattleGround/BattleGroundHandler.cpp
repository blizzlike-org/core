...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket &recv_data) {
  ...
  if (!sBattlemasterListStore.LookupEntry(receivedBgTypeId)) {
    sLog.outError("Battleground: invalid bgtype (%u) received. possible cheater? player guid %u", receivedBgTypeId,
                  _player->GetGUIDLow());
    return;
  }
  ...
  if (joinAsGroup) {
    ...
    GroupQueueInfo *queueInfo =
        bgQueue.AddGroup(_player, grp, bgTypeId, bgBracketId, ARENA_TYPE_NONE, false, isPremade, 0);
    ...
    for (GroupReference *itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next()) {
      ...
      member->GetSession()->SendPacket(data);
      DEBUG_LOG("Battleground: player joined queue for bg queue type %u bg type %u: GUID %u, NAME %s", bgQueueTypeId,
                bgTypeId, member->GetGUIDLow(), member->GetName());
    }
    DEBUG_LOG("Battleground: group end");
  } else {
    GroupQueueInfo *queueInfo =
        bgQueue.AddGroup(_player, nullptr, bgTypeId, bgBracketId, ARENA_TYPE_NONE, false, isPremade, 0);
    uint32 avgTime = bgQueue.GetAverageQueueWaitTime(queueInfo, _player->GetBattleGroundBracketIdFromLevel(bgTypeId));
    ...
    SendPacket(data);
    DEBUG_LOG("Battleground: player joined queue for bg queue type %u bg type %u: GUID %u, NAME %s", bgQueueTypeId,
              bgTypeId, _player->GetGUIDLow(), _player->GetName());
  }

  sBattleGroundMgr.ScheduleQueueUpdate(0, ARENA_TYPE_NONE, bgQueueTypeId, bgTypeId,
                                       _player->GetBattleGroundBracketIdFromLevel(bgTypeId));
}
...
void WorldSession::HandleBattlefieldPortOpcode(WorldPacket &recv_data) {
  ...
  if (type && !IsArenaTypeValid(ArenaType(type))) {
    sLog.outError("BattlegroundHandler: Invalid CMSG_BATTLEFIELD_PORT received from player (%u), arena type wrong: %u.",
  ...
  // we must use temporary variable, because GroupQueueInfo pointer can be deleted in BattleGroundQueue::RemovePlayer()
  // function
  GroupQueueInfo queueInfo;
  ...
  if (action == 1 && queueInfo.arenaType == ARENA_TYPE_NONE) {
    // if player is trying to enter battleground (not arena!) and he has deserter debuff, we must just remove him from
    // queue
    if (!_player->CanJoinToBattleground()) {
      ...
      DEBUG_LOG("Battleground: player %s (%u) has a deserter debuff, do not port him to battleground!",
                _player->GetName(), _player->GetGUIDLow());
    }
    // if player don't match battleground max level, then do not allow him to enter! (this might happen when player
    // leveled up during his waiting in queue
    if (_player->getLevel() > bg->GetMaxLevel()) {
      sLog.outError("Battleground: Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do "
                    "not port him to battleground!",
  ...
  case 1: // port to battleground
    ...
    sBattleGroundMgr.BuildBattleGroundStatusPacket(data, bg, queueSlot, STATUS_IN_PROGRESS, 0, bg->GetStartTime(),
                                                   bg->GetArenaType(), _player->GetBGTeam());
    _player->GetSession()->SendPacket(data);
    ...
    DEBUG_LOG("Battleground: player %s (%u) joined battle for bg %u, bgtype %u, queue type %u.", _player->GetName(),
              _player->GetGUIDLow(), bg->GetInstanceId(), bg->GetTypeId(), bgQueueTypeId);
  ...
  case 0: // leave queue
    ...
    if (queueInfo.isRated && queueInfo.isInvitedToBgInstanceGuid) {
      ...
      if (at) {
        DEBUG_LOG(
            "UPDATING memberLost's personal arena rating for %s by opponents rating: %u, because he has left queue!",
            _player->GetGuidStr().c_str(), queueInfo.opponentsTeamRating);
    ...
    _player->RemoveBattleGroundQueueId(
        bgQueueTypeId); // must be called this way, because if you move this call to queue->removeplayer, it causes bugs
    ...
    if (queueInfo.arenaType == ARENA_TYPE_NONE)
      sBattleGroundMgr.ScheduleQueueUpdate(queueInfo.arenaTeamRating, queueInfo.arenaType, bgQueueTypeId, bgTypeId,
                                           _player->GetBattleGroundBracketIdFromLevel(bgTypeId));

    SendPacket(data);

    DEBUG_LOG("Battleground: player %s (%u) left queue for bgtype %u, queue type %u.", _player->GetName(),
              _player->GetGUIDLow(), bg->GetTypeId(), bgQueueTypeId);
...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket & /*recv_data*/) {
  ...
  for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i) {
    ...
    if (bgTypeId == _player->GetBattleGroundTypeId() ||
      ...
      // i cannot check any variable from player class because player class doesn't know if player is in 2v2 / 3v3 or
      // 5v5 arena so i must use bg pointer to get that information
    ...
    if (queueInfo.isInvitedToBgInstanceGuid) {
      ...
      uint32 avgTime =
          bgQueue.GetAverageQueueWaitTime(&queueInfo, _player->GetBattleGroundBracketIdFromLevel(bgTypeId));
      // send status in BattleGround Queue
      sBattleGroundMgr.BuildBattleGroundStatusPacket(
          data, bg, i, STATUS_WAIT_QUEUE, avgTime,
          WorldTimer::getMSTimeDiff(queueInfo.joinTime, WorldTimer::getMSTime()), arenaType, TEAM_NONE);
      SendPacket(data);
    }
  }
}
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket &recv_data) {
  ...
  if (!asGroup) {
    ...
    if (!group)
      return;
    uint32 err =
        group->CanJoinBattleGroundQueue(bgTypeId, bgQueueTypeId, arenatype, arenatype, isRated != 0, arenaslot);
    if (err != BG_JOIN_ERR_OK) {
      SendBattleGroundOrArenaJoinError(err);
      return;
    }
  ...
  if (isRated) {
    ...
    // check real arena team existence only here (if it was moved to group->CanJoin .. () then we would have to get it
    // twice)
    ...
    // if avg personal rating is more than 150 points below the teams rating, the team will be queued against an
    // opponent matching or similar to the average personal rating
  ...
  if (asGroup) {
    ...
    if (isRated)
      DEBUG_LOG("Battleground: arena team id %u, leader %s queued with rating %u for type %u",
                _player->GetArenaTeamId(arenaslot), _player->GetName(), arenaRating, arenatype);

    // set arena rated type to show correct minimap arena icon
    bg->SetRated(isRated != 0);

    GroupQueueInfo *ginfo =
        bgQueue.AddGroup(_player, group, bgTypeId, bgBracketId, arenatype, isRated != 0, false, arenaRating, ateamId);
    ...
    for (GroupReference *itr = group->GetFirstMember(); itr != nullptr; itr = itr->next()) {
      ...
      sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(data, bgTypeId);
      member->GetSession()->SendPacket(data);
      DEBUG_LOG("Battleground: player joined queue for arena as group bg queue type %u bg type %u: GUID %u, NAME %s",
                bgQueueTypeId, bgTypeId, member->GetGUIDLow(), member->GetName());
    }
    DEBUG_LOG("Battleground: arena join as group end");
  } else {
    GroupQueueInfo *ginfo =
        bgQueue.AddGroup(_player, nullptr, bgTypeId, bgBracketId, arenatype, isRated != 0, false, arenaRating, ateamId);
    uint32 avgTime = bgQueue.GetAverageQueueWaitTime(ginfo, _player->GetBattleGroundBracketIdFromLevel(bgTypeId));
    uint32 queueSlot = _player->AddBattleGroundQueueId(bgQueueTypeId);
    ...
    sBattleGroundMgr.BuildBattleGroundStatusPacket(data, bg, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, arenatype,
                                                   TEAM_NONE);
    SendPacket(data);
    DEBUG_LOG("Battleground: player joined queue for arena, skirmish, bg queue type %u bg type %u: GUID %u, NAME %s",
              bgQueueTypeId, bgTypeId, _player->GetGUIDLow(), _player->GetName());
  }
  sBattleGroundMgr.ScheduleQueueUpdate(arenaRating, arenatype, bgQueueTypeId, bgTypeId,
                                       _player->GetBattleGroundBracketIdFromLevel(bgTypeId));
}
