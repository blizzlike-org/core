...
enum GMTicketSystemStatus : uint8 { GMTICKET_QUEUE_STATUS_DISABLED = 0, GMTICKET_QUEUE_STATUS_ENABLED = 1 };

enum GMTicketStatus { GMTICKET_STATUS_DBERROR = 0x00, GMTICKET_STATUS_HASTEXT = 0x06, GMTICKET_STATUS_DEFAULT = 0x0A };
...
enum GMTicketStateStatus { GMTICKET_STATUS_UPDATED = 1, GMTICKET_STATUS_CLOSED = 2, GMTICKET_STATUS_SURVEY = 3 };
...
enum GMTicketState : uint8 { GMTICKET_STATE_OPEN = 0, GMTICKET_STATE_CLOSED = 1, GMTICKET_STATE_ABANDONED = 2 };
...
public:
  ...
  inline bool IsSurveyed() const {
    return (m_state == GMTICKET_STATE_CLOSED && (m_statsEscalations || (m_statsChatIn >= 3 || m_statsChatOut >= 3)));
  }
...
public:
  explicit GMSurveyResult(const GMSurveyEntry &entry, uint32 ticketId, const std::map<uint32, uint8> &answers,
...
public:
  ...
  static inline const std::string PrintTicketSummaryLine(GMTicket const &ticket,
                                                         LocaleConstant locale = DEFAULT_LOCALE);

  void PrintTicketList(WorldSession *session, std::ostringstream &output, size_t max,
                       const GMTicketCategoryEntry *category = nullptr, bool online = false) const;

  static bool TicketChatIncoming(GMTicket *ticket, Player *player, WorldSession *gm, const std::string &message,
                                 bool addon = false);
  static bool TicketChatOutgoing(GMTicket *ticket, WorldSession *gm, Player *player, const std::string &message,
                                 bool addon = false);
  ...
  std::pair<bool, bool> HookGMTicketWhisper(Player *sender, const std::string &recepient, Player *character,
  ...
  GMTicket *GetTicketByPlayer(ObjectGuid playerGuid, GMTicketState state = GMTICKET_STATE_OPEN,
  ...
  inline CommandResult Discard(GMTicket *ticket, const std::string &reason, WorldSession *session = nullptr) {
    return Close(ticket, reason, false, session);
  }
  ...
  inline CommandResult Resolve(GMTicket *ticket, const std::string &commentary, WorldSession *session = nullptr) {
    return Close(ticket, commentary, true, session);
  }
  ...
  static inline float GetDaysPassed(time_t now, time_t since) {
    return (now && since ? std::stof(std::to_string((difftime(now, since) / DAY))) : 0);
  }
...
private:
  ...
  std::pair<GMTicket *, std::string> GetTicketByIdTag(const std::string &tag, GMTicketState state = GMTICKET_STATE_OPEN,
                                                      ObjectGuid assigneeGuid = ObjectGuid()) const;
