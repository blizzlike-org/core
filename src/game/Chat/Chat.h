...
public:
   ...
   * \param ObjectGuid const& targetGuid  : Often null, but needed for type *MONSTER* or *BNET* or *BATTLEGROUND* or
   **ACHIEVEMENT* \param char const* targetName        : Often null, but needed for type *MONSTER* or *BNET* or
   **BATTLEGROUND* \param char const* channelName       : Required only for CHAT_MSG_CHANNEL
   **/
  static void BuildChatPacket(WorldPacket &data, ChatMsg msgtype, char const *message,
  ...
  // extraction different type params from args string, all functions update (char** args) to first unparsed tail symbol
  // at return
  ...
  static char *ExtractLinkArg(char **args, char const *const *linkTypes = nullptr, int *foundIdx = nullptr,
  ...
  static char *
  ExtractOptNotLastArg(char **args); // extract name/number/quote/shift-link arg only if more data in args for parse

  char *ExtractKeyFromLink(char **text, char const *linkType, char **something1 = nullptr);
  static char *ExtractKeyFromLink(char **text, char const *const *linkTypes, int *found_idx = nullptr,
  ...
  bool ExtractPlayerTarget(char **args, Player **player, ObjectGuid *player_guid = nullptr,
...
protected:
  ...
  ChatCommandSearchResult FindCommand(ChatCommand *table, char const *&text, ChatCommand *&command,
                                      ChatCommand **parentCommand = nullptr, std::string *cmdNamePtr = nullptr,
                                      bool allAvailable = false, bool exactlyName = false) const;
  ...
  std::string petLink(std::string const &name) const {
    return m_session ? "|cffffffff|Hpet:" + name + "|h[" + name + "]|h|r" : name;
  }
  ...
  void ShowFactionListHelper(FactionEntry const *factionEntry, LocaleConstant loc,
  ...
  bool HandleGoHelper(Player *_player, uint32 mapid, float x, float y, float const *zPtr = nullptr,
  ...
  template <typename T> void ShowNpcOrGoSpawnInformation(uint32 guid);
  template <typename T> std::string PrepareStringNpcOrGoSpawnInformation(uint32 guid) const;
