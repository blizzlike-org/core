...
enum ChatNotify : uint8 {
  ...
  CHAT_THROTTLED_NOTICE = 0x1F,   //+ "[%s] The number of messages that can be sent to this channel is limited, please
                                  //wait to send another message.";
  CHAT_NOT_IN_AREA_NOTICE = 0x20, //+ "[%s] You are not in the correct area for this channel."; -- The user is trying to
                                  //send a chat to a zone specific channel, and they're not physically in that zone.
  CHAT_NOT_IN_LFG_NOTICE = 0x21, //+ "[%s] You must be queued in looking for group before joining this channel."; -- The
                                 //user must be in the looking for group system to join LFG chat channels.
...
class Channel {
  ...
  struct PlayerInfo {
    ...
    void SetFlag(uint8 flag, bool state) {
      if (state)
        flags |= flag;
      else
        flags &= ~flag;
...
public:
  ...
  inline void SetModerator(Player *player, const char *targetName, bool set) {
    SetModeFlags(player, targetName, MEMBER_FLAG_MODERATOR, set);
  }
  inline void SetMute(Player *player, const char *targetName, bool set) {
    SetModeFlags(player, targetName, MEMBER_FLAG_MUTED, set);
  }
  ...
  static void MakeModeChange(WorldPacket &data, const std::string &channel, const ObjectGuid &guid, uint8 oldflags,
                             uint8 newFlags);                                                              //+ 0x0C
  ...
  static void MakePlayerKicked(WorldPacket &data, const std::string &channel, const ObjectGuid &target,
                               const ObjectGuid &source);                //? 0x12
  static void MakeBanned(WorldPacket &data, const std::string &channel); //? 0x13
  static void MakePlayerBanned(WorldPacket &data, const std::string &channel, const ObjectGuid &target,
                               const ObjectGuid &source); //? 0x14
  static void MakePlayerUnbanned(WorldPacket &data, const std::string &channel, const ObjectGuid &target,
                                 const ObjectGuid &source);                                                   //? 0x15
