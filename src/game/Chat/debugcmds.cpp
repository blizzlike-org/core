  ...
  if (CinematicSequencesEntry const *cineSeq = sCinematicSequencesStore.LookupEntry(dwId)) {
    ...
    if (itr != sFlyByCameraStore.end()) {
      ...
      for (FlyByCamera cam : itr->second) {
        PSendSysMessage("%02u - %7ums [%f, %f, %f] Facing %f (%f degrees)", count, cam.timeStamp, cam.locations.x,
                        cam.locations.y, cam.locations.z, cam.locations.w, cam.locations.w * (180 / M_PI));
        count++;
      }
    ...
    for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; ++i) {
      ...
      if (!item->IsBag()) {
        if (item->GetState() == state)
          PSendSysMessage("%s bag: 255 slot: %u owner: %s", item->GetGuidStr().c_str(), uint32(item->GetSlot()),
                          item->GetOwnerGuid().GetString().c_str());
      } else {
        Bag *bag = (Bag *)item;
        for (uint8 j = 0; j < bag->GetBagSize(); ++j) {
          Item *item2 = bag->GetItemByPos(j);
          if (item2 && item2->GetState() == state)
            PSendSysMessage("%s bag: %u slot: %u owner: %s", item2->GetGuidStr().c_str(), uint32(item2->GetBagSlot()),
                            uint32(item2->GetSlot()), item2->GetOwnerGuid().GetString().c_str());
        }
      }
    }
  }
  ...
  if (check_all) {
    ...
    for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; ++i) {
      ...
      if (item->GetOwnerGuid() != player->GetObjectGuid()) {
        PSendSysMessage("%s at slot %u owner (%s) and inventory owner (%s) don't match!", item->GetGuidStr().c_str(),
      ...
      if (Bag *container = item->GetContainer()) {
        PSendSysMessage("%s at slot %u has a container %s from slot %u but shouldnt!", item->GetGuidStr().c_str(),
                        uint32(item->GetSlot()), container->GetGuidStr().c_str(), uint32(container->GetSlot()));
        error = true;
        continue;
      }
      ...
      if (item->IsBag()) {
        ...
        for (uint8 j = 0; j < bag->GetBagSize(); ++j) {
          ...
          if (item2->GetSlot() != j) {
            PSendSysMessage("%s in bag %u at slot %u has an incorrect slot value: %u", item2->GetGuidStr().c_str(),
                            uint32(bag->GetSlot()), uint32(j), uint32(item2->GetSlot()));
            error = true;
            continue;
          }
          ...
          if (item2->IsInUpdateQueue()) {
            ...
            if (updateQueue[qp] != item2) {
              PSendSysMessage(
                  "%s in bag %u at slot %u has a queuepos (%d) that points to %s in the queue (bag %u slot %u)",
    ...
    for (size_t i = 0; i < updateQueue.size(); ++i) {
      ...
      if (item->GetOwnerGuid() != player->GetObjectGuid()) {
        PSendSysMessage("queue(" SIZEFMTD "): %s has the owner (%s) and inventory owner (%s) don't match!", i,
                        item->GetGuidStr().c_str(), item->GetOwnerGuid().GetString().c_str(),
                        player->GetGuidStr().c_str());
        error = true;
        continue;
      }
      ...
      if (item->GetQueuePos() != i) {
        PSendSysMessage("queue(" SIZEFMTD "): %s has queuepos doesn't match it's position in the queue!", i,
      ...
      if (test == nullptr) {
        PSendSysMessage(
            "queue(" SIZEFMTD
            "): %s has incorrect (bag %u slot %u) values, the player doesn't have an item at that position!",
      ...
      if (test != item) {
        PSendSysMessage("queue(" SIZEFMTD "): %s has incorrect (bag %u slot %u) values, the %s is there instead!", i,
                        item->GetGuidStr().c_str(), uint32(item->GetBagSlot()), uint32(item->GetSlot()),
...
bool ChatHandler::HandleDebugSpellModsCommand(char *args) {
  ...
  PSendSysMessage(LANG_YOU_CHANGE_SPELLMODS, opcode == SMSG_SET_FLAT_SPELL_MODIFIER ? "flat" : "pct", spellmodop, value,
                  effidx, GetNameLink(chr).c_str());
...
bool ChatHandler::HandleDebugOverflowCommand(char * /*args*/) {
  std::string name("\360\222\214\245\360\222\221\243\360\222\221\251\360\223\213\215\360\223\213\210\360\223\211\241"
                   "\360\222\214\245\360\222\221\243\360\222\221\251\360\223\213\215\360\223\213\210\360\223\211\241");
