...
bool ChatHandler::HandleGPSCommand(char *args) {
  ...
  PSendSysMessage(LANG_MAP_POSITION, obj->GetMapId(), (mapEntry ? mapEntry->name[GetSessionDbcLocale()] : "<unknown>"),
                  zone_id, (zoneEntry ? zoneEntry->area_name[GetSessionDbcLocale()] : "<unknown>"), area_id,
  ...
  if (VMAP::IVMapManager *vmgr = VMAP::VMapFactory::createOrGetVMapManager())
    PSendSysMessage("Vmap Terrain Height %f", vmgr->getHeight(obj->GetMapId(), obj->GetPositionX(), obj->GetPositionY(),
...
bool ChatHandler::HandleModifyMountCommand(char *args) { return ModifyMountCommandHelper(getSelectedPlayer(), args); }
...
bool ChatHandler::HandleGoZoneXYCommand(char *args) {
  ...
  if (mapEntry->Instanceable()) {
    PSendSysMessage(LANG_INVALID_ZONE_MAP, areaEntry->ID, areaEntry->area_name[GetSessionDbcLocale()], mapEntry->MapID,
                    mapEntry->name[GetSessionDbcLocale()]);
    SetSentErrorMessage(true);
    return false;
  }
