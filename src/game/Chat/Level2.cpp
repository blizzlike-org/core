...
void ChatHandler::ShowTriggerTargetListHelper(uint32 id, AreaTrigger const *at, bool subpart /*= false*/) {
  if (m_session) {
    ...
    PSendSysMessage(LANG_TRIGGER_TARGET_LIST_CHAT, subpart ? " -> " : "", id, id, at->target_mapId, at->target_X,
                    at->target_Y, at->target_Z, dist_buf);
  } else
    PSendSysMessage(LANG_TRIGGER_TARGET_LIST_CONSOLE, subpart ? " -> " : "", id, at->target_mapId, at->target_X,
                    at->target_Y, at->target_Z);
}
...
void ChatHandler::ShowTriggerListHelper(AreaTriggerEntry const *atEntry) {
  ...
  if (m_session) {
    ...
    PSendSysMessage(LANG_TRIGGER_LIST_CHAT, atEntry->id, atEntry->id, atEntry->mapid, atEntry->x, atEntry->y,
                    atEntry->z, dist_buf, tavern, quest);
  } else
    PSendSysMessage(LANG_TRIGGER_LIST_CONSOLE, atEntry->id, atEntry->mapid, atEntry->x, atEntry->y, atEntry->z, tavern,
                    quest);
...
bool ChatHandler::HandleGoCreatureCommand(char *args) {
  ...
  case CREATURE_LINK_RAW: {
    ...
    // Number is invalid - maybe the user specified the mob's name
      ...
      QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM creature, creature_template WHERE creature.id = "
  ...
  if (!data) {
    ...
    // data is nullptr so this mean either the guid/entry provided was wrong or its a summoned creature. Lets give a
    // chance to find a summoned creature.
...
bool ChatHandler::HandleGoObjectCommand(char *args) {
  ...
  case GAMEOBJECT_LINK_RAW: {
    ...
    if (ExtractUInt32(&pParam1, lowguid)) {
      ...
      std::string name = pParam1;
      WorldDatabase.escape_string(name);
      QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM gameobject, gameobject_template WHERE gameobject.id "
...
bool ChatHandler::HandleGameObjectTargetCommand(char *args) {
  ...
  if (*args) {
    ...
    if (ExtractUInt32(&cId, id)) {
      result =
          WorldDatabase.PQuery("SELECT guid, id, position_x, position_y, position_z, orientation, map, (POW(position_x "
                               "- '%f', 2) + POW(position_y - '%f', 2) + POW(position_z - '%f', 2)) AS order_ FROM "
                               "gameobject WHERE map = '%i' AND id = '%u' ORDER BY order_ ASC LIMIT 1",
  ...
  if (target) {
    ...
    if (target->GetGoType() == GAMEOBJECT_TYPE_DOOR)
      PSendSysMessage(LANG_COMMAND_GO_STATUS_DOOR, uint32(target->GetGoState()), uint32(target->GetLootState()),
                      GetOnOffStr(target->IsCollisionEnabled()), goI->door.startOpen ? "open" : "closed");
    else
      PSendSysMessage(LANG_COMMAND_GO_STATUS, uint32(target->GetGoState()), uint32(target->GetLootState()),
                      GetOnOffStr(target->IsCollisionEnabled()));
  }
  return true;
}
...
bool ChatHandler::HandleGameObjectAddCommand(char *args) {
  ...
  if (gInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(gInfo->displayId)) {
    ...
    sLog.outErrorDb("Gameobject (Entry %u GoType: %u) have invalid displayId (%u), not spawned.", id, gInfo->type,
                    gInfo->displayId);
...
bool ChatHandler::HandleGameObjectNearCommand(char *args) {
  ...
  if (result) {
    do {
      ...
      PSendSysMessage(LANG_GO_MIXED_LIST_CHAT, guid, PrepareStringNpcOrGoSpawnInformation<GameObject>(guid).c_str(),
...
bool ChatHandler::HandleGameObjectNearSpawnedCommand(char *args) {
  ...
  MaNGOS::GameObjectInPosRangeCheck go_check(*player, player->GetPositionX(), player->GetPositionY(),
                                             player->GetPositionZ(), distance);
  ...
  for (GameObject *go : gameobjects) {
    ...
    PSendSysMessage(LANG_GO_MIXED_LIST_CHAT, guid.GetCounter(),
                    PrepareStringNpcOrGoSpawnInformation<GameObject>(guid).c_str(), entry, guid, goInfo->name, x, y, z,
                    go->GetMapId());
  }
...
bool ChatHandler::HandleGUIDCommand(char * /*args*/) {
  ...
  PSendSysMessage(
      LANG_OBJECT_GUID,
      (creature->GetObjectGuid().GetString() + " DBGuid: " + std::to_string(creature->GetDbGuid())).c_str());
  return true;
}

void ChatHandler::ShowFactionListHelper(FactionEntry const *factionEntry, LocaleConstant loc,
                                        FactionState const *repState /*= nullptr*/, Player *target /*= nullptr */) {
  std::string name = factionEntry->name[loc];
  // send faction in "id - [faction] rank reputation [visible] [at war] [own team] [unknown] [invisible] [inactive]"
  // format or              "id - [faction] [no reputation]" format
  std::ostringstream ss;
  if (m_session)
    ss << factionEntry->ID << " - |cffffffff|Hfaction:" << factionEntry->ID << "|h[" << name << " " << localeNames[loc]
       << "]|h|r";
  else
...
bool ChatHandler::HandleNpcTempSpawn(char *args) {
  ...
  player->SummonCreature(entry, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
                         player->GetOrientation(), TEMPSPAWN_TIMED_OOC_DESPAWN, timer);
  return true;
}
...
bool ChatHandler::HandleNpcAddVendorItemCommand(char *args) {
  ...
  if (!sObjectMgr.IsVendorItemValid(false, "npc_vendor", vendor_entry, itemId, maxcount, incrtime, extendedcost, 0,
                                    m_session->GetPlayer())) {
    SetSentErrorMessage(true);
    return false;
  }
...
bool ChatHandler::HandleNpcAIInfoCommand(char * /*args*/) {
  ...
  PSendSysMessage(LANG_NPC_AI_NAMES, strAI.empty() ? " - " : strAI.c_str(), cstrAIClass ? cstrAIClass : " - ",
...
bool ChatHandler::HandleNpcFlagCommand(char *args) {
  ...
  WorldDatabase.PExecuteLog("UPDATE creature_template SET NpcFlags = '%u' WHERE entry = '%u'", npcFlags,
                            pCreature->GetEntry());
...
bool ChatHandler::HandleNpcMoveCommand(char *args) {
  ...
  WorldDatabase.PExecuteLog("UPDATE creature SET position_x = '%f', position_y = '%f', position_z = '%f', orientation "
                            "= '%f' WHERE guid = '%u'",
                            x, y, z, o, lowguid);
...
bool ChatHandler::HandleNpcSpawnDistCommand(char *args) {
  ...
  // respawn alive creature to reinitialize everything including movement generator (dead one will do it anyway at
  // respawn)
  ...
  WorldDatabase.PExecuteLog("UPDATE creature SET spawndist=%f, MovementType=%i WHERE guid=%u", option, mtype,
...
bool ChatHandler::HandleNpcSubNameCommand(char * /*args*/) {
  ...
  uint32 idname =
  sObjectMgr.AddCreatureSubName(pCreature->GetName(),args,pCreature->GetUInt32Value(UNIT_FIELD_DISPLAYID));
  pCreature->SetUInt32Value(OBJECT_FIELD_ENTRY, idname);
...
bool ChatHandler::HandlePInfoCommand(char *args) {
  ...
  // get additional information from DB
    ...
    QueryResult *result = CharacterDatabase.PQuery(
        "SELECT totaltime, level, money, account FROM characters WHERE guid = '%u'", target_guid.GetCounter());
    if (!result)
      return false;
  ...
  QueryResult *result = LoginDatabase.PQuery("SELECT username,gmlevel,ip,loginTime FROM account a JOIN account_logons "
                                             "b ON(a.id=b.accountId) WHERE a.id = '%u' ORDER BY loginTime DESC LIMIT 1",
                                             accId);
  if (result) {
    Field *fields = result->Fetch();
  ...
  PSendSysMessage(LANG_PINFO_ACCOUNT, (target ? "" : GetMangosString(LANG_OFFLINE)), nameLink.c_str(),
                  target_guid.GetCounter(), username.c_str(), accId, uint32(security), last_ip.c_str(),
...
/// Helper function
inline Creature *Helper_CreateWaypointFor(Creature *wpOwner, WaypointPathOrigin wpOrigin, int32 pathId, uint32 wpId,
                                          WaypointNode const *wpNode, CreatureInfo const *waypointInfo) {
  ...
  settings.y = wpNode->y;
  settings.z = wpNode->z;
  settings.ori = wpNode->orientation;
...
bool ChatHandler::HandleWpAddCommand(char *args) {
  ...
  if (targetCreature) {
    ...
    if (targetCreature->GetEntry() == VISUAL_WAYPOINT &&
        targetCreature->GetSubtype() == CREATURE_SUBTYPE_TEMPORARY_SUMMON) {
  ...
  if (wpDestination == PATH_NO_PATH) // No Waypoint selected, parse additional params
  {
    ...
    if (wpDestination == PATH_NO_PATH) // No overwrite params. Do best estimate
    {
      if (wpOwner->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
        if (WaypointMovementGenerator<Creature> const *wpMMGen =
                dynamic_cast<WaypointMovementGenerator<Creature> const *>(wpOwner->GetMotionMaster()->GetCurrent()))
          wpMMGen->GetPathInformation(wpPathId, wpDestination);

      // Get information about default path if no current path. If no default path, prepare data dependendy on
      // uniqueness
      if (wpDestination == PATH_NO_PATH &&
          !sWaypointMgr.GetDefaultPath(wpOwner->GetEntry(), wpOwner->GetGUIDLow(), &wpDestination)) {
        wpDestination = PATH_FROM_ENTRY; // Default place to store paths
        if (wpOwner->HasStaticDBSpawnData()) {
          QueryResult *result =
              WorldDatabase.PQuery("SELECT COUNT(id) FROM creature WHERE id = %u", wpOwner->GetEntry());
  ...
  // wpOwner will get a new waypoint inserted into wpPath = GetPathFromOrigin(wpOwner, wpDestination, wpPathId) at
  // wpPointId

  float x, y, z;
  m_session->GetPlayer()->GetPosition(x, y, z);
  if (!sWaypointMgr.AddNode(wpOwner->GetEntry(), wpOwner->GetGUIDLow(), wpPathId, wpPointId, wpDestination, x, y, z,
                            m_session->GetPlayer()->GetOrientation())) {
    PSendSysMessage(LANG_WAYPOINT_NOTCREATED, wpPointId, wpOwner->GetGuidStr().c_str(), wpPathId,
                    WaypointManager::GetOriginString(wpDestination).c_str());
    SetSentErrorMessage(true);
    return false;
  }
  ...
  WaypointPath const *wpPath =
      sWaypointMgr.GetPathFromOrigin(wpOwner->GetEntry(), wpOwner->GetGUIDLow(), wpPathId, wpDestination);
  ...
  PSendSysMessage(LANG_WAYPOINT_ADDED, wpPointId, wpOwner->GetGuidStr().c_str(), wpPathId,
                  WaypointManager::GetOriginString(wpDestination).c_str());
...
bool ChatHandler::HandleWpModifyCommand(char *args) {
  ...
  if ((subCmd != "waittime") && (subCmd != "scriptid") && (subCmd != "orientation") && (subCmd != "del") &&
      (subCmd != "move"))
    return false;
  ...
  WaypointPath const *wpPath =
      sWaypointMgr.GetPathFromOrigin(wpOwner->GetEntry(), wpOwner->GetGUIDLow(), wpPathId, wpSource);
  if (!wpPath) {
    PSendSysMessage(LANG_WAYPOINT_NOTFOUNDPATH, wpOwner->GetGuidStr().c_str(), wpPathId,
  ...
  if (point == wpPath->end()) {
    PSendSysMessage(LANG_WAYPOINT_NOTFOUND, wpId, wpOwner->GetGuidStr().c_str(), wpPathId,
                    WaypointManager::GetOriginString(wpSource).c_str());
    SetSentErrorMessage(true);
    return false;
  }
...
bool ChatHandler::HandleWpShowCommand(char *args) {
  ...
  if (!wpPath || wpPath->empty()) {
    PSendSysMessage(LANG_WAYPOINT_NOTFOUNDPATH, wpOwner->GetGuidStr().c_str(), wpPathId,
                    WaypointManager::GetOriginString(wpOrigin).c_str());
    SetSentErrorMessage(true);
    return false;
  }
  ...
  if (subCmd == "info") {
    ...
    if (point == wpPath->end()) {
      PSendSysMessage(LANG_WAYPOINT_NOTFOUND, wpTarget->GetWaypointId(), wpOwner->GetGuidStr().c_str(), wpPathId,
                      WaypointManager::GetOriginString(wpOrigin).c_str());
      SetSentErrorMessage(true);
      return false;
    }

    PSendSysMessage(LANG_WAYPOINT_INFO_TITLE, wpTarget->GetWaypointId(), wpOwner->GetGuidStr().c_str(), wpPathId,
                    WaypointManager::GetOriginString(wpOrigin).c_str());
...
bool ChatHandler::HandleWpExportCommand(char *args) {
  ...
  WaypointPath const *wpPath =
      sWaypointMgr.GetPathFromOrigin(wpOwner->GetEntry(), wpOwner->GetGUIDLow(), wpPathId, wpOrigin);
  if (!wpPath || wpPath->empty()) {
  ...
  case PATH_FROM_EXTERNAL:
    key = wpOwner->GetEntry();
    key_field = "entry";
    table = sWaypointMgr.GetExternalWPTable();
    break;
  default:
    return false;
  }
  ...
  if (wpOrigin != PATH_FROM_EXTERNAL)
    outfile << "INSERT INTO " << table << " (" << key_field
            << ", point, position_x, position_y, position_z, orientation, waittime, script_id) VALUES\n";
  else
    outfile << "INSERT INTO " << table << " (" << key_field
            << ", point, position_x, position_y, position_z, orientation, waittime) VALUES\n";
...
bool ChatHandler::HandleEventInfoCommand(char *args) {
  ...
  std::string nextStr =
      nextTime >= eventData.start && nextTime < eventData.end ? TimeToTimestampStr(time(nullptr) + delay) : "-";

  std::string occurenceStr = secsToTimeString(eventData.occurence * MINUTE);
  std::string lengthStr = secsToTimeString(eventData.length * MINUTE);

  PSendSysMessage(LANG_EVENT_INFO, event_id, eventData.description.c_str(), activeStr, startTimeStr.c_str(),
...
bool ChatHandler::HandleLookupAccountEmailCommand(char *args) {
  ...
  //                                                 0   1         2        3        4
  QueryResult *result = LoginDatabase.PQuery("SELECT id, username, ip, gmlevel, expansion FROM account a join "
                                             "account_logons b on (a.id=b.accountId) WHERE email " _LIKE_
                                             " " _CONCAT3_("'%%'", "'%s'", "'%%'  ORDER BY loginTime DESC LIMIT 1"),
                                             email.c_str());
...
bool ChatHandler::HandleLookupAccountIpCommand(char *args) {
  ...
  //                                                 0            1         2        3        4
  QueryResult *result =
      LoginDatabase.PQuery("SELECT distinct id, username, ip, gmlevel, expansion FROM account a join account_logons b "
                           "on(a.id=b.accountId) WHERE ip " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'"),
                           ip.c_str());
...
bool ChatHandler::HandleLookupAccountNameCommand(char *args) {
  ...
  //                                                 0   1         2        3        4
  QueryResult *result = LoginDatabase.PQuery("SELECT id, username, ip, gmlevel, expansion FROM account a join "
                                             "account_logons b on (a.id=b.accountId) WHERE username " _LIKE_
                                             " " _CONCAT3_("'%%'", "'%s'", "'%%' ORDER BY loginTime DESC LIMIT 1"),
                                             account.c_str());
...
bool ChatHandler::ShowAccountListHelper(QueryResult *result, uint32 *limit, bool title, bool error) {
  ...
  do {
    ...
    if (m_session)
      PSendSysMessage(LANG_ACCOUNT_LIST_LINE_CHAT, account, fields[1].GetString(), char_name, fields[2].GetString(),
                      fields[3].GetUInt32(), fields[4].GetUInt32());
    else
      PSendSysMessage(LANG_ACCOUNT_LIST_LINE_CONSOLE, account, fields[1].GetString(), char_name, fields[2].GetString(),
                      fields[3].GetUInt32(), fields[4].GetUInt32());
  } while (result->NextRow());
...
bool ChatHandler::HandleLookupPlayerIpCommand(char *args) {
  ...
  QueryResult *result = LoginDatabase.PQuery(
      "SELECT id,username, distinct ip FROM account a join account_logons b on (a.id=b.accountId) WHERE b.ip " _LIKE_
      " " _CONCAT3_("'%%'", "'%s'", "'%%'  ORDER BY loginTime DESC LIMIT 1"),
      ip.c_str());
...
bool ChatHandler::HandleLookupPlayerAccountCommand(char *args) {
  ...
  QueryResult *result = LoginDatabase.PQuery(
      "SELECT id,username FROM account WHERE username " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'"), account.c_str());
...
bool ChatHandler::HandleLookupPlayerEmailCommand(char *args) {
  ...
  QueryResult *result = LoginDatabase.PQuery(
      "SELECT id,username FROM account WHERE email " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'"), email.c_str());
...
bool ChatHandler::LookupPlayerSearchCommand(QueryResult *result, uint32 *limit) {
  ...
  do {
    ...
    QueryResult *chars =
        CharacterDatabase.PQuery("SELECT guid, name, race, class, level FROM characters WHERE account = %u", acc_id);
    if (chars) {
...
void ChatHandler::ShowPoolListHelper(uint16 pool_id) {
  ...
  if (m_session)
    PSendSysMessage(LANG_POOL_ENTRY_LIST_CHAT, uint32(pool_id), uint32(pool_id), pool_template.description.c_str(),
                    pool_template.AutoSpawn ? 1 : 0, pool_template.MaxLimit, sPoolMgr.GetPoolCreatures(pool_id).size(),
                    sPoolMgr.GetPoolGameObjects(pool_id).size(), sPoolMgr.GetPoolPools(pool_id).size());
  else
    PSendSysMessage(LANG_POOL_ENTRY_LIST_CONSOLE, uint32(pool_id), pool_template.description.c_str(),
                    pool_template.AutoSpawn ? 1 : 0, pool_template.MaxLimit, sPoolMgr.GetPoolCreatures(pool_id).size(),
                    sPoolMgr.GetPoolGameObjects(pool_id).size(), sPoolMgr.GetPoolPools(pool_id).size());
}
...
bool ChatHandler::HandlePoolListCommand(char * /*args*/) {
  ...
  if (!mapState->GetMapEntry()->Instanceable()) {
    PSendSysMessage(LANG_POOL_LIST_NON_INSTANCE, mapState->GetMapEntry()->name[GetSessionDbcLocale()],
                    mapState->GetMapId());
...
bool ChatHandler::HandlePoolSpawnsCommand(char *args) {
  ...
  if (!ExtractUint32KeyFromLink(&args, "Hpool", pool_id) && !mapState->GetMapEntry()->Instanceable()) {
    PSendSysMessage(LANG_POOL_SPAWNS_NON_INSTANCE, mapState->GetMapEntry()->name[GetSessionDbcLocale()],
                    mapState->GetMapId());
    SetSentErrorMessage(false);
    return false;
  }
      ...
      if (CreatureData const *data = sObjectMgr.GetCreatureData(crSpawn))
        if (CreatureInfo const *info = ObjectMgr::GetCreatureTemplate(data->id))
          PSendSysMessage(LANG_CREATURE_LIST_CHAT, crSpawn,
                          PrepareStringNpcOrGoSpawnInformation<Creature>(crSpawn).c_str(), crSpawn, info->Name,
                          data->posX, data->posY, data->posZ, data->mapid);
...
bool ChatHandler::HandlePoolInfoCommand(char *args) {
  ...
  if (!mother_pool_id)
    ...
    if (m_session)
      PSendSysMessage(LANG_POOL_INFO_HEADER_CHAT, pool_id, mother_pool_id, mother_pool_id,
                      mother_template.description.c_str(), pool_template.AutoSpawn, pool_template.MaxLimit);
    else
    ...
    for (auto itr : poolCreaturesEx) {
      if (CreatureData const *data = sObjectMgr.GetCreatureData(itr.guid)) {
        if (CreatureInfo const *info = ObjectMgr::GetCreatureTemplate(data->id)) {
          ...
          if (m_session)
            PSendSysMessage(LANG_POOL_CHANCE_CREATURE_LIST_CHAT, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<Creature>(itr.guid).c_str(), itr.guid, info->Name,
                            data->posX, data->posY, data->posZ, data->mapid, itr.chance, active);
          else
            PSendSysMessage(LANG_POOL_CHANCE_CREATURE_LIST_CONSOLE, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<Creature>(itr.guid).c_str(), info->Name, data->posX,
                            data->posY, data->posZ, data->mapid, itr.chance, active);
        }
      }
    }
  }
    ...
    for (auto itr : poolCreaturesEq) {
      if (CreatureData const *data = sObjectMgr.GetCreatureData(itr.guid)) {
        if (CreatureInfo const *info = ObjectMgr::GetCreatureTemplate(data->id)) {
          char const *active = crSpawns && crSpawns->find(itr.guid) != crSpawns->end() ? active_str.c_str() : "";
          if (m_session)
            PSendSysMessage(LANG_POOL_CREATURE_LIST_CHAT, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<Creature>(itr.guid).c_str(), itr.guid, info->Name,
                            data->posX, data->posY, data->posZ, data->mapid, active);
          else
            PSendSysMessage(LANG_POOL_CREATURE_LIST_CONSOLE, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<Creature>(itr.guid).c_str(), info->Name, data->posX,
                            data->posY, data->posZ, data->mapid, active);
        }
      }
    }
  }
    ...
    for (auto itr : poolGameObjectsEx) {
      if (GameObjectData const *data = sObjectMgr.GetGOData(itr.guid)) {
        if (GameObjectInfo const *info = ObjectMgr::GetGameObjectInfo(data->id)) {
          ...
          if (m_session)
            PSendSysMessage(LANG_POOL_CHANCE_GO_LIST_CHAT, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<GameObject>(itr.guid).c_str(), itr.guid, info->name,
                            data->posX, data->posY, data->posZ, data->mapid, itr.chance, active);
          else
            PSendSysMessage(LANG_POOL_CHANCE_GO_LIST_CONSOLE, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<GameObject>(itr.guid).c_str(), info->name, data->posX,
                            data->posY, data->posZ, data->mapid, itr.chance, active);
        }
      }
    }
    ...
    for (auto itr : poolGameObjectsEq) {
      if (GameObjectData const *data = sObjectMgr.GetGOData(itr.guid)) {
        if (GameObjectInfo const *info = ObjectMgr::GetGameObjectInfo(data->id)) {
          char const *active = goSpawns && goSpawns->find(itr.guid) != goSpawns->end() ? active_str.c_str() : "";
          if (m_session)
            PSendSysMessage(LANG_POOL_GO_LIST_CHAT, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<GameObject>(itr.guid).c_str(), itr.guid, info->name,
                            data->posX, data->posY, data->posZ, data->mapid, active);
          else
            PSendSysMessage(LANG_POOL_GO_LIST_CONSOLE, itr.guid,
                            PrepareStringNpcOrGoSpawnInformation<GameObject>(itr.guid).c_str(), info->name, data->posX,
                            data->posY, data->posZ, data->mapid, active);
        }
      }
  ...
  if (!poolPoolsEx.empty()) {
    ...
    for (auto itr : poolPoolsEx) {
      ...
      if (m_session)
        PSendSysMessage(LANG_POOL_CHANCE_POOL_LIST_CHAT, itr.guid, itr.guid, itr_template.description.c_str(),
                        itr_template.AutoSpawn ? 1 : 0, itr_template.MaxLimit,
                        sPoolMgr.GetPoolCreatures(itr.guid).size(), sPoolMgr.GetPoolGameObjects(itr.guid).size(),
                        sPoolMgr.GetPoolPools(itr.guid).size(), itr.chance, active);
      else
        PSendSysMessage(LANG_POOL_CHANCE_POOL_LIST_CONSOLE, itr.guid, itr_template.description.c_str(),
                        itr_template.AutoSpawn ? 1 : 0, itr_template.MaxLimit,
                        sPoolMgr.GetPoolCreatures(itr.guid).size(), sPoolMgr.GetPoolGameObjects(itr.guid).size(),
                        sPoolMgr.GetPoolPools(itr.guid).size(), itr.chance, active);
    }
  ...
  if (!poolPoolsEq.empty()) {
    ...
    for (auto itr : poolPoolsEq) {
      ...
      char const *active = poolSpawns && poolSpawns->find(itr.guid) != poolSpawns->end() ? active_str.c_str() : "";
      if (m_session)
        PSendSysMessage(LANG_POOL_POOL_LIST_CHAT, itr.guid, itr.guid, itr_template.description.c_str(),
                        itr_template.AutoSpawn ? 1 : 0, itr_template.MaxLimit,
                        sPoolMgr.GetPoolCreatures(itr.guid).size(), sPoolMgr.GetPoolGameObjects(itr.guid).size(),
                        sPoolMgr.GetPoolPools(itr.guid).size(), active);
      else
        PSendSysMessage(LANG_POOL_POOL_LIST_CONSOLE, itr.guid, itr_template.description.c_str(),
                        itr_template.AutoSpawn ? 1 : 0, itr_template.MaxLimit,
                        sPoolMgr.GetPoolCreatures(itr.guid).size(), sPoolMgr.GetPoolGameObjects(itr.guid).size(),
                        sPoolMgr.GetPoolPools(itr.guid).size(), active);
    }
  ...
  for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); ++id) {
    ...
    if (titleInfo) {
      ...
      if (loc < MAX_LOCALE) {
        ...
        if (m_session)
          PSendSysMessage(LANG_TITLE_LIST_CHAT, id, titleInfo->bit_index, id, titleNameStr, localeNames[loc], knownStr,
                          activeStr);
        else
          PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, titleNameStr, localeNames[loc], knownStr,
                          activeStr);
  ...
  for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); ++id) {
    ...
    if (titleInfo && target->HasTitle(titleInfo)) {
      ...
      if (m_session)
        ...
        PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, name.c_str(), localeNames[loc], knownStr,
                        activeStr);
    }
  }
  return true;
...
bool ChatHandler::HandleMmapLocCommand(char * /*args*/) {
  ...
  const dtNavMeshQuery *navmeshquery =
      MMAP::MMapFactory::createOrGetMMapManager()->GetNavMeshQuery(player->GetMapId(), player->GetInstanceId());
...
bool ChatHandler::HandleMmapLoadedTilesCommand(char * /*args*/) {
  ...
  const dtNavMeshQuery *navmeshquery =
      MMAP::MMapFactory::createOrGetMMapManager()->GetNavMeshQuery(mapid, m_session->GetPlayer()->GetInstanceId());
  if (!navmesh || !navmeshquery) {
    PSendSysMessage("NavMesh not loaded for current map.");
    return true;
  }
...
bool ChatHandler::HandleMmapStatsCommand(char * /*args*/) {
  ...
  PSendSysMessage(" %u maps loaded with %u tiles overall", manager->getLoadedMapsCount(),
