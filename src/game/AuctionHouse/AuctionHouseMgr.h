...
struct AuctionEntry {
  ...
  uint32 buyout;
  time_t expireTime;
  uint32 bidder;  // current bidder player lowguid, can be 0 if bid generated by server, use 'bid'!=0 for check bid
                  // existance
  ...
  bool UpdateBid(uint32 newbid,
                 Player *newbidder = nullptr); // true if normal bid, false if buyout, bidder==nullptr for generated bid
};
...
public:
  ...
  AuctionEntry *AddAuction(AuctionHouseEntry const *auctionHouseEntry, Item *newItem, uint32 etime, uint32 bid,
                           uint32 buyout = 0, uint32 deposit = 0, Player *pl = nullptr);
...
enum AuctionHouseType { AUCTION_HOUSE_ALLIANCE = 0, AUCTION_HOUSE_HORDE = 1, AUCTION_HOUSE_NEUTRAL = 2 };
