...
void WorldSession::HandleAuctionHelloOpcode(WorldPacket &recv_data) {
  ...
  if (!unit) {
    DEBUG_LOG("WORLD: HandleAuctionHelloOpcode - %s not found or you can't interact with him.",
...
void WorldSession::SendAuctionCommandResult(AuctionEntry *auc, AuctionAction Action, AuctionError ErrorCode,
                                            InventoryResult invError) const {
...
void WorldSession::SendAuctionOwnerNotification(AuctionEntry *auction, bool sold) const {
  ...
  data << uint32(auction->Id);
  data << uint32(
      auction->bid); // if 0, client shows ERR_AUCTION_EXPIRED_S, else ERR_AUCTION_SOLD_S (works only when guid==0)
  data << uint32(auction->GetAuctionOutBid()); // AuctionOutBid?
...
void WorldSession::SendAuctionOutbiddedMail(AuctionEntry *auction) {
  ...
  if (oldBidder || oldBidder_accId) {
    ...
    msgAuctionOutbiddedSubject << auction->itemTemplate << ":" << auction->itemRandomPropertyId << ":"
                               << AUCTION_OUTBIDDED;
...
void WorldSession::SendAuctionCancelledToBidderMail(AuctionEntry *auction) {
  ...
  if (bidder || bidder_accId) {
    ...
    msgAuctionCancelledSubject << auction->itemTemplate << ":" << auction->itemRandomPropertyId << ":"
...
void WorldSession::HandleAuctionSellItem(WorldPacket &recv_data) {
  ...
  if (sAuctionMgr.GetAItem(itemGuid.GetCounter())) {
    sLog.outError("AuctionError, %s is sending %s, but item is already in another auction", pl->GetGuidStr().c_str(),
                  itemGuid.GetString().c_str());
  ...
  if (GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE)) {
    sLog.outCommand(GetAccountId(), "GM %s (Account: %u) create auction: %s (Entry: %u Count: %u)", GetPlayerName(),
                    GetAccountId(), it->GetProto()->Name1, it->GetEntry(), it->GetCount());
  }
  ...
  DETAIL_LOG(
      "selling %s to auctioneer %s with initial bid %u with buyout %u and with time %u (in sec) in auctionhouse %u",
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket &recv_data) {
  ...
  if (!auction || auction->owner != pl->GetGUIDLow()) {
    ...
    sLog.outError("CHEATER : %u, he tried to cancel auction (id: %u) of another player, or auction is nullptr",
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket &recv_data) {
  ...
  if (recv_data.size() != (16 + outbiddedCount * 4)) {
    sLog.outError("Client sent bad opcode!!! with count: %u and size : %u (must be: %u)", outbiddedCount,
                  (uint32)recv_data.size(), (16 + outbiddedCount * 4));
    outbiddedCount = 0;
  }
...
void WorldSession::HandleAuctionListItems(WorldPacket &recv_data) {
  ...
  // DEBUG_LOG("Auctionhouse search %s list from: %u, searchedname: %s, levelmin: %u, levelmax: %u, auctionSlotID: %u,
  // auctionMainCategory: %u, auctionSubCategory: %u, quality: %u, usable: %u",
  //  auctioneerGuid.GetString().c_str(), listfrom, searchedname.c_str(), levelmin, levelmax, auctionSlotID,
  //  auctionMainCategory, auctionSubCategory, quality, usable);
  ...
  BuildListAuctionItems(auctions, data, wsearchedname, listfrom, levelmin, levelmax, usable, auctionSlotID,
                        auctionMainCategory, auctionSubCategory, quality, count, totalcount, isFull != 0);
