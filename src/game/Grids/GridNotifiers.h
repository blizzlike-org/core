...
struct MessageDeliverer {
  ...
  bool i_toSelf;
  MessageDeliverer(Player const &pl, WorldPacket const &msg, bool to_self)
      : i_player(pl), i_message(msg), i_toSelf(to_self) {}
  void Visit(CameraMapType &m);
...
struct MessageDelivererExcept {
  ...
  MessageDelivererExcept(WorldPacket const &msg, Player const *skipped) : i_message(msg), i_skipped_receiver(skipped) {}
...
struct ObjectMessageDistDeliverer {
  ...
  WorldPacket const &i_message;
  float i_dist;
  ObjectMessageDistDeliverer(WorldObject const &obj, WorldPacket const &msg, float dist)
      : i_object(obj), i_message(msg), i_dist(dist) {}
  void Visit(CameraMapType &m);
  template <class SKIP> void Visit(GridRefManager<SKIP> &) {}
};
...
struct DynamicObjectUpdater {
  ...
  DynamicObjectUpdater(DynamicObject &dynobject, Unit *caster, bool positive)
      : i_dynobject(dynobject), i_positive(positive) {
    i_check = caster;
...
template <class Check> struct WorldObjectSearcher {
  WorldObject *&i_object;
  Check &i_check;
...
template <class Do> struct WorldObjectWorker {
  Do const &i_do;
...
template <class Check> struct GameObjectSearcher {
  GameObject *&i_object;
  Check &i_check;
...
template <class Check> struct UnitSearcher {
  Unit *&i_object;
  Check &i_check;
...
template <class Check> struct UnitLastSearcher {
  Unit *&i_object;
  Check &i_check;
...
template <class Check> struct UnitListSearcher {
  UnitList &i_objects;
  Check &i_check;
...
template <class Check> struct CreatureSearcher {
  Creature *&i_object;
  Check &i_check;
...
template <class Do> struct CreatureWorker {
  Do &i_do;
...
template <class Check> struct PlayerSearcher {
  Player *&i_object;
  Check &i_check;
...
template <class Check> struct PlayerListSearcher {
  PlayerList &i_objects;
  Check &i_check;

  PlayerListSearcher(PlayerList &objects, Check &check) : i_objects(objects), i_check(check) {}
...
template <class Do> struct PlayerWorker {
  Do &i_do;
...
template <class Do> struct CameraDistWorker {
...
class SomeCheck
{
    ...
    private:
        SomeObjecType const* i_fobj;                    // Focus object used for check distance from, phase, so place in
world
...
public:
  ...
  bool operator()(GameObject *go) {
    if (go->GetGOInfo()->type == GAMEOBJECT_TYPE_FISHINGHOLE && go->IsSpawned() &&
        i_obj.IsWithinDistInMap(go, i_range) &&
        i_obj.IsWithinDistInMap(go, (float)go->GetGOInfo()->fishinghole.radius)) {
...
// Success at unit in range, range update for next check (this can be used with GameobjectLastSearcher to find nearest
// GO)
...
public:
  AllGameObjectEntriesListInObjectRangeCheck(WorldObject const &obj, std::set<uint32> &entries, float range,
                                             bool is3D = true)
      : i_obj(obj), i_entries(entries), i_range(range), i_is3D(is3D) {}
  WorldObject const &GetFocusObject() const { return i_obj; }
  bool operator()(GameObject *go) {
    if (go->IsSpawned() && i_entries.find(go->GetEntry()) != i_entries.end() &&
        i_obj.IsWithinDistInMap(go, i_range, i_is3D))
      return true;
...
public:
  AllGameObjectEntriesListInPosRangeCheck(float x, float y, float z, std::set<uint32> &entries, float range,
                                          bool is3D = true)
      : i_x(x), i_y(y), i_z(z), i_entries(entries), i_range(range), i_is3D(is3D) {}
  bool operator()(GameObject *go) {
    if (go->IsSpawned() && i_entries.find(go->GetEntry()) != i_entries.end() &&
        go->GetDistance(i_x, i_y, i_z, DIST_CALC_COMBAT_REACH) < i_range)
      return true;
...
// Success at gameobject in range of xyz, range update for next check (this can be use with GameobjectLastSearcher to
// find nearest GO)
...
public:
  ...
  bool operator()(GameObject *go) { return go->IsWithinDist3d(i_x, i_y, i_z, i_range); }
...
public:
  MostHPMissingInRangeCheck(Unit const *obj, float range, float hp, bool onlyInCombat, bool targetSelf)
      : i_obj(obj), i_range(range), i_hp(hp), i_onlyInCombat(onlyInCombat), i_targetSelf(targetSelf) {}
  WorldObject const &GetFocusObject() const { return *i_obj; }
...
public:
  MostHPPercentMissingInRangeCheck(Unit const *obj, float range, float hp, bool onlyInCombat, bool targetSelf)
      : i_obj(obj), i_range(range), i_hp(hp), i_onlyInCombat(onlyInCombat), i_targetSelf(targetSelf) {}
  WorldObject const &GetFocusObject() const { return *i_obj; }
  bool operator()(Unit *u) {
    if (!u->IsAlive() || (i_onlyInCombat && !u->IsInCombat()))
...
public:
  ...
  bool operator()(Unit *u) {
    return u->IsAlive() && u->IsInCombat() && i_obj->CanAssist(u) && i_obj->IsWithinDistInMap(u, i_range) &&
           (u->IsImmobilizedState() || u->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_DECREASE_SPEED) || u->isFrozen() ||
            u->IsCrowdControlled());
  }
...
public:
  FriendlyMissingBuffInRangeInCombatCheck(Unit const *obj, float range, uint32 spellid)
      : i_obj(obj), i_range(range), i_spell(spellid) {}
  Unit const &GetFocusObject() const { return *i_obj; }
  bool operator()(Unit *u) {
    return u->IsAlive() && u->IsInCombat() && i_obj->CanAssist(u) && i_obj->IsWithinDistInMap(u, i_range) &&
           !(u->HasAura(i_spell, EFFECT_INDEX_0) || u->HasAura(i_spell, EFFECT_INDEX_1) ||
             u->HasAura(i_spell, EFFECT_INDEX_2));
  }
...
public:
  FriendlyMissingBuffInRangeNotInCombatCheck(Unit const *obj, float range, uint32 spellid)
      : i_obj(obj), i_range(range), i_spell(spellid) {}
  Unit const &GetFocusObject() const { return *i_obj; }
  bool operator()(Unit *u) {
    return u->IsAlive() && i_obj->CanAssist(u) && i_obj->IsWithinDistInMap(u, i_range) &&
           !(u->HasAura(i_spell, EFFECT_INDEX_0) || u->HasAura(i_spell, EFFECT_INDEX_1) ||
             u->HasAura(i_spell, EFFECT_INDEX_2));
  }
...
public:
  AnyFriendlyUnitInObjectRangeCheck(WorldObject const *obj, SpellEntry const *spellInfo, float range)
...
public:
  AnyFriendlyOrGroupMemberUnitInUnitRangeCheck(Unit const *obj, Group const *group, SpellEntry const *spellInfo,
                                               float range)
...
public:
  ...
  WorldObject const &GetFocusObject() const { return *i_obj; }
  bool operator()(Unit *u) { return u->IsAlive() && i_obj->IsWithinDistInMap(u, i_range); }
...
public:
  AnyUnitFulfillingConditionInRangeCheck(WorldObject const *obj, std::function<bool(Unit *)> functor, float radius,
                                         DistanceCalculation type = DIST_CALC_COMBAT_REACH)
      : i_obj(obj), i_functor(functor), i_range(radius), i_type(type) {}
  WorldObject const &GetFocusObject() const { return *i_obj; }
  bool operator()(Unit *u) { return i_functor(u) && i_obj->GetDistance(u, true, i_type) <= i_range; }
...
public:
  NearestAttackableUnitInObjectRangeCheck(Unit *source, Unit *owner, float range)
      : m_source(source), m_owner(owner), m_range(range) {}
  ...
  bool operator()(Unit *currUnit) {
    if (currUnit->IsAlive() &&
        (m_source->IsAttackedBy(currUnit) || (m_owner && m_owner->IsAttackedBy(currUnit)) ||
         m_source->IsEnemy(currUnit)) &&
        m_source->CanAttack(currUnit) && currUnit->IsVisibleForOrDetect(m_source, m_source, false) &&
...
// Success at unit in range, range update for next check (this can be use with CreatureLastSearcher to find nearest
// creature)
class NearestCreatureEntryWithLiveStateInObjectRangeCheck {
public:
  NearestCreatureEntryWithLiveStateInObjectRangeCheck(WorldObject const &obj, uint32 entry, bool onlyAlive,
                                                      bool onlyDead, float range, bool excludeSelf = false,
                                                      bool is3D = true)
      : i_obj(obj), i_entry(entry), i_range(range * range), i_onlyAlive(onlyAlive), i_onlyDead(onlyDead),
        i_excludeSelf(excludeSelf), i_is3D(is3D), i_foundOutOfRange(false) {}
  WorldObject const &GetFocusObject() const { return i_obj; }
  bool operator()(Creature *u) {
    if (u->GetEntry() == i_entry &&
        ((i_onlyAlive && u->IsAlive()) || (i_onlyDead && u->IsCorpse()) || (!i_onlyAlive && !i_onlyDead)) &&
        (!i_excludeSelf || (&i_obj != u))) {
...
// Success at unit in range, range update for next check (this can be used with CreatureListSearcher to find creatures
// with given entry)
class AllCreatureEntriesWithLiveStateInObjectRangeCheck {
public:
  AllCreatureEntriesWithLiveStateInObjectRangeCheck(WorldObject const &obj, std::set<uint32> &entries, bool alive,
                                                    float range, bool guids = false, bool excludeSelf = false,
                                                    bool is3D = true)
      : i_obj(obj), i_entries(entries), i_range(range), i_guids(guids), i_alive(alive), i_excludeSelf(excludeSelf),
        i_is3D(is3D), i_foundOutOfRange(false) {}
  WorldObject const &GetFocusObject() const { return i_obj; }
  bool operator()(Creature *u) {
    if (i_entries.find(i_guids ? u->GetGUIDLow() : u->GetEntry()) != i_entries.end() &&
        ((i_alive && u->IsAlive()) || (!i_alive && u->IsCorpse())) && (!i_excludeSelf || (&i_obj != u))) {
...
public:
  AllCreaturesOfEntryInRangeCheck(const WorldObject *pObject, uint32 uiEntry, float fMaxRange)
...
public:
  ...
  WorldObject const &GetFocusObject() const { return *i_obj; }
  bool operator()(Player *u) { return u->IsAlive() && i_obj->IsWithinDistInMap(u, i_range); }
...
public:
  AnyPlayerInCapturePointRange(WorldObject const *obj, float range) : i_obj(obj), i_range(range) {}
  WorldObject const &GetFocusObject() const { return *i_obj; }
  bool operator()(Player *u) { return u->CanUseCapturePoint() && i_obj->IsWithinDistInMap(u, i_range); }
...
template <class Builder> class LocalizedPacketDo {
public:
...
// Prepare using Builder localized packets with caching and send to player
template <class Builder> class LocalizedPacketListDo {
public:
