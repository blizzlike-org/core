...
enum FactionFlags {
  ...
  FACTION_FLAG_AT_WAR = 0x02,  // enable AtWar-button in client. player controlled (except opposition team always war
                               // state), Flag only set on initial creation
  FACTION_FLAG_HIDDEN = 0x04,  // hidden faction from reputation pane in client (player can gain reputation, but this
                               // update not sent to client)
  ...
  FACTION_FLAG_TEAM_REPUTATION = 0x80, // faction has own reputation standing despite teaming up sub-factions; spillover
                                       // from subfactions will go this instead of other subfactions
};
...
public: // modifiers
  void SetReputation(FactionEntry const *factionEntry, int32 standing) { SetReputation(factionEntry, standing, false); }
