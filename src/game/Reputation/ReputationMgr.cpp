...
FactionState const *ReputationMgr::GetState(FactionEntry const *factionEntry) const {
  return (factionEntry && factionEntry->HasReputation()) ? GetState(RepListID(factionEntry->reputationListID))
                                                         : nullptr;
}
...
int32 ReputationMgr::GetReputation(uint32 faction_id) const {
  ...
  if (!factionEntry) {
    sLog.outError("ReputationMgr::GetReputation: Can't get reputation of %s for unknown faction (faction id) #%u.",
...
bool ReputationMgr::IsAtWar(uint32 faction_id) const {
  ...
  if (!factionEntry) {
    sLog.outError(
        "ReputationMgr::IsAtWar: Can't get state of 'At War' flag of %s for unknown faction (faction id) #%u.",
        m_player->GetName(), faction_id);
    return false;
  }
    ...
    for (uint32 i = 0; i < MAX_SPILLOVER_FACTIONS; ++i) {
      if (repTemplate->faction[i]) {
        if (m_player->GetReputationRank(repTemplate->faction[i]) <= ReputationRank(repTemplate->faction_rank[i])) {
          ...
          if (SetOneFactionReputation(sFactionStore.LookupEntry<FactionEntry>(repTemplate->faction[i]), spilloverRep,
                                      incremental))
...
void ReputationMgr::LoadFromDB(QueryResult *result) {
  ...
  // QueryResult *result = CharacterDatabase.PQuery("SELECT faction,standing,flags FROM character_reputation WHERE guid
  // = '%u'",GetGUIDLow());
...
void ReputationMgr::SaveToDB() {
  ...
  SqlStatement stmtDel =
      CharacterDatabase.CreateStatement(delRep, "DELETE FROM character_reputation WHERE guid = ? AND faction=?");
  SqlStatement stmtIns = CharacterDatabase.CreateStatement(
      insRep, "INSERT INTO character_reputation (guid,faction,standing,flags) VALUES (?, ?, ?, ?)");
