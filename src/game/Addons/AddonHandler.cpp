...
bool AddonHandler::BuildAddonPacket(WorldPacket &Source, WorldPacket &Target) {
  ...
  unsigned char tdata[256] = {
      };
  ...
  if (uncompress(const_cast<uint8 *>(AddOnPacked.contents()), &AddonRealSize,
                 const_cast<uint8 *>(Source.contents() + CurrentPosition), Source.size() - CurrentPosition) == Z_OK) {
...
/* Code use in 1.10.2 when client not ignore ban state sended for addons. Saved for reference if client switch to use
server ban state information void AddonHandler::BuildAddonPacket(WorldPacket& Source, WorldPacket& Target, uint32
Packetoffset)
{
    ByteBuffer AddOnPacked;
    uLongf AddonRealSize;
    ...
    *Source >> TempValue;                                   // get real size of the packed structure
    ...
    if (!uncompress((uint8*)AddOnPacked.contents(), &AddonRealSize, (uint8*)(*Source).contents() + CurrentPosition,
