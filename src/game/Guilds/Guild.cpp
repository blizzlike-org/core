...
void MemberSlot::SetPNOTE(std::string pnote) {
  ...
  CharacterDatabase.PExecute("UPDATE guild_member SET pnote = '%s' WHERE guid = '%u'", pnote.c_str(),
                             guid.GetCounter());
}
...
void MemberSlot::SetOFFNOTE(std::string offnote) {
  ...
  CharacterDatabase.PExecute("UPDATE guild_member SET offnote = '%s' WHERE guid = '%u'", offnote.c_str(),
                             guid.GetCounter());
}
...
Guild::~Guild() { DeleteGuildBankItems(); }
...
bool Guild::Create(Player *leader, std::string gname) {
  ...
  CharacterDatabase.PExecute(
      "INSERT INTO guild "
      "(guildid,name,leaderguid,info,motd,createdate,EmblemStyle,EmblemColor,BorderStyle,BorderColor,BackgroundColor,"
      "BankMoney) "
      "VALUES('%u','%s','%u', '%s', '%s','" UI64FMTD "','%u','%u','%u','%u','%u','" UI64FMTD "')",
      m_Id, gname.c_str(), m_LeaderGuid.GetCounter(), dbGINFO.c_str(), dbMOTD.c_str(), uint64(now), m_EmblemStyle,
      m_EmblemColor, m_BorderStyle, m_BorderColor, m_BackgroundColor, m_GuildBankMoney);
...
bool Guild::AddMember(ObjectGuid plGuid, uint32 plRank) {
  ...
  if (pl) {
    ...
    QueryResult *result = CharacterDatabase.PQuery(
        "SELECT name,level,class,gender,zone,account FROM characters WHERE guid = '%u'", lowguid);
    if (!result)
    ...
    if (newmember.Level < 1 || newmember.Class < 1 || !((1 << (newmember.Class - 1)) & CLASSMASK_ALL_PLAYABLE)) {
      sLog.outError("%s has a broken data in field `characters` table, cannot add him to guild.",
  ...
  CharacterDatabase.PExecute(
      "INSERT INTO guild_member (guildid,guid,`rank`,pnote,offnote) VALUES ('%u', '%u', '%u','%s','%s')", m_Id, lowguid,
      newmember.RankId, dbPnote.c_str(), dbOFFnote.c_str());
...
bool Guild::LoadRanksFromDB(QueryResult *guildRanksResult) {
  ...
  do {
    ...
    // condition that would be true when all ranks in QueryResult will be processed and guild without ranks is being
    // processed
    if (!fields)
      break;
  ...
  if (broken_ranks) {
    ...
    for (size_t i = 0; i < m_Ranks.size(); ++i) {
      ...
      CharacterDatabase.PExecute("INSERT INTO guild_rank (guildid,rid,rname,rights) VALUES ('%u', '%u', '%s', '%u')",
...
bool Guild::LoadMembersFromDB(QueryResult *guildMembersResult) {
  ...
  do {
    ...
    // this condition will be true when all rows in QueryResult are processed and new guild without members is going to
    // be loaded - prevent crash
    if (!fields)
      break;
    uint32 guildId = fields[0].GetUInt32();
    if (guildId < m_Id) {
    ...
    {
      sLog.outError("%s has a broken data in field `characters`.`data`, deleting him from guild!",
    ...
    {
      sLog.outError("%s has a broken data in field `characters`.`class`, deleting him from guild!",
                    newmember.guid.GetString().c_str());
      CharacterDatabase.PExecute("DELETE FROM guild_member WHERE guid = '%u'", lowguid);
      continue;
    }
 ...
 * @param isDisbanding  Flag set if function called from Guild::Disband, so not need update DB in per-member mode only
 * or leader update
 *
...
void Guild::BroadcastToGuild(WorldSession *session, const std::string &msg, uint32 language) {
  ...
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, CHAT_MSG_GUILD, msg.c_str(), Language(language), player->GetChatTag(),
  ...
  for (MemberList::const_iterator itr = members.begin(); itr != members.end(); ++itr) {
    ...
    if (pl && pl->GetSession() && HasRankRight(pl->GetRank(), GR_RIGHT_GCHATLISTEN) &&
        !pl->GetSocial()->HasIgnore(player->GetObjectGuid()))
...
void Guild::BroadcastToOfficers(WorldSession *session, const std::string &msg, uint32 language) {
  ...
  for (MemberList::const_iterator itr = members.begin(); itr != members.end(); ++itr) {
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, CHAT_MSG_OFFICER, msg.c_str(), Language(language), player->GetChatTag(),
                                 player->GetObjectGuid(), player->GetName());
...
void Guild::CreateRank(std::string name_, uint32 rights) {
  ...
  for (uint32 i = 0; i < uint32(GetPurchasedTabs()); ++i) {
    ...
    CharacterDatabase.PExecute("INSERT INTO guild_bank_right (guildid,TabId,rid) VALUES ('%u','%u','%u')", m_Id, i,
                               new_rank_id);
  }
  // name now can be used for encoding to DB
  CharacterDatabase.escape_string(name_);
  CharacterDatabase.PExecute("INSERT INTO guild_rank (guildid,rid,rname,rights) VALUES ('%u', '%u', '%s', '%u')", m_Id,
                             new_rank_id, name_.c_str(), rights);
}
...
void Guild::SetRankName(uint32 rankId, std::string name_) {
  ...
  // name now can be used for encoding to DB
  CharacterDatabase.escape_string(name_);
  CharacterDatabase.PExecute("UPDATE guild_rank SET rname='%s' WHERE rid='%u' AND guildid='%u'", name_.c_str(), rankId,
                             m_Id);
}
...
void Guild::Roster(WorldSession *session /*= nullptr*/) {
  ...
  WorldPacket data(SMSG_GUILD_ROSTER, (4 + MOTD.length() + 1 + GINFO.length() + 1 + 4 +
                                       m_Ranks.size() * (4 + 4 + GUILD_BANK_MAX_TABS * (4 + 4)) + members.size() * 50));
  ...
  for (RankList::const_iterator ritr = m_Ranks.begin(); ritr != m_Ranks.end(); ++ritr) {
    ...
    data << uint32(
        ritr->BankMoneyPerDay); // count of: withdraw gold(gold/day) Note: in game set gold, in packet set bronze.
    for (int i = 0; i < GUILD_BANK_MAX_TABS; ++i) {
...
void Guild::SetEmblem(uint32 emblemStyle, uint32 emblemColor, uint32 borderStyle, uint32 borderColor,
  ...
  CharacterDatabase.PExecute("UPDATE guild SET EmblemStyle=%u, EmblemColor=%u, BorderStyle=%u, BorderColor=%u, "
                             "BackgroundColor=%u WHERE guildid = %u",
                             m_EmblemStyle, m_EmblemColor, m_BorderStyle, m_BorderColor, m_BackgroundColor, m_Id);
}
...
void Guild::LoadGuildEventLogFromDB() {
  //                                                     0        1          2            3            4        5
  QueryResult *result =
      CharacterDatabase.PQuery("SELECT LogGuid, EventType, PlayerGuid1, PlayerGuid2, NewRank, TimeStamp FROM "
  ...
  do {
    ...
    // There can be a problem if more events have same TimeStamp the ORDER can be broken when fields[0].GetUInt32() ==
    // configCount, but events with same timestamp can appear when there is lag, and we naively suppose that mangos
    // isn't laggy but if problem appears, player will see set of guild events that have same timestamp in bad order
...
void Guild::LogGuildEvent(uint8 EventType, ObjectGuid playerGuid1, ObjectGuid playerGuid2, uint8 newRank) {
  ...
  // Save event to DB
  CharacterDatabase.PExecute("DELETE FROM guild_eventlog WHERE guildid='%u' AND LogGuid='%u'", m_Id,
                             m_GuildEventLogNextGuid);
  CharacterDatabase.PExecute("INSERT INTO guild_eventlog (guildid, LogGuid, EventType, PlayerGuid1, PlayerGuid2, "
                             "NewRank, TimeStamp) VALUES ('%u','%u','%u','%u','%u','%u','" UI64FMTD "')",
                             m_Id, m_GuildEventLogNextGuid, uint32(NewEvent.EventType), NewEvent.PlayerGuid1,
                             NewEvent.PlayerGuid2, uint32(NewEvent.NewRank), NewEvent.TimeStamp);
}
...
void Guild::SetGuildBankTabInfo(uint8 TabId, std::string Name, std::string Icon) {
  ...
  CharacterDatabase.PExecute("UPDATE guild_bank_tab SET TabName='%s',TabIcon='%s' WHERE guildid='%u' AND TabId='%u'",
                             Name.c_str(), Icon.c_str(), m_Id, uint32(TabId));
}
...
void Guild::LoadGuildBankFromDB() {
  //                                                     0      1        2        3
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT TabId, TabName, TabIcon, TabText FROM guild_bank_tab WHERE guildid='%u' ORDER BY TabId", m_Id);
  if (!result) {
    m_TabListMap.clear();
    return;
  }
  ...
  result = CharacterDatabase.PQuery(
      ...
      "randomPropertyId, durability, itemTextId, TabId, SlotId, item_guid, item_entry FROM guild_bank_item JOIN "
      "item_instance ON item_guid = guid WHERE guildid='%u' ORDER BY TabId",
      m_Id);
  if (!result)
    return;
  ...
  do {
    ...
    if (TabId >= GetPurchasedTabs()) {
      sLog.outError("Guild::LoadGuildBankFromDB: Invalid tab for item (GUID: %u id: #%u) in guild bank, skipped.",
                    ItemGuid, ItemEntry);
      continue;
    }

    if (SlotId >= GUILD_BANK_MAX_SLOTS) {
      sLog.outError("Guild::LoadGuildBankFromDB: Invalid slot for item (GUID: %u id: #%u) in guild bank, skipped.",
                    ItemGuid, ItemEntry);
      continue;
    }
    ...
    if (!proto) {
      sLog.outError("Guild::LoadGuildBankFromDB: Unknown item (GUID: %u id: #%u) in guild bank, skipped.", ItemGuid,
                    ItemEntry);
      continue;
    }
    ...
    if (!pItem->LoadFromDB(ItemGuid, fields)) {
      CharacterDatabase.PExecute("DELETE FROM guild_bank_item WHERE guildid='%u' AND TabId='%u' AND SlotId='%u'", m_Id,
                                 uint32(TabId), uint32(SlotId));
...
uint32 Guild::GetMemberSlotWithdrawRem(uint32 LowGuid, uint8 TabId) {
  ...
  if (curTime - member.BankResetTimeTab[TabId] >= 24 * HOUR / MINUTE) {
    ...
    CharacterDatabase.PExecute(
        "UPDATE guild_member SET BankResetTimeTab%u='%u', BankRemSlotsTab%u='%u' WHERE guildid='%u' AND guid='%u'",
...
uint32 Guild::GetMemberMoneyWithdrawRem(uint32 LowGuid) {
  ...
  if (curTime > member.BankResetTimeMoney + 24 * HOUR / MINUTE) {
    ...
    CharacterDatabase.PExecute(
        "UPDATE guild_member SET BankResetTimeMoney='%u', BankRemMoney='%u' WHERE guildid='%u' AND guid='%u'",
...
void Guild::SetBankMoneyPerDay(uint32 rankId, uint32 money) {
  ...
  CharacterDatabase.PExecute("UPDATE guild_rank SET BankMoneyPerDay='%u' WHERE rid='%u' AND guildid='%u'", money,
                             rankId, m_Id);
  CharacterDatabase.PExecute("UPDATE guild_member SET BankResetTimeMoney='0' WHERE guildid='%u' AND `rank`='%u'", m_Id,
                             rankId);
}
...
void Guild::SetBankRightsAndSlots(uint32 rankId, uint8 TabId, uint32 right, uint32 nbSlots, bool db) {
  if (rankId >= m_Ranks.size() || TabId >= GetPurchasedTabs()) {
    ...
    CharacterDatabase.PExecute("DELETE FROM guild_bank_right WHERE guildid='%u' AND rid='%u' AND TabId='%u'", m_Id,
                               rankId, TabId);
    return;
  }
  ...
  if (db) {
    ...
    CharacterDatabase.PExecute("DELETE FROM guild_bank_right WHERE guildid='%u' AND TabId='%u' AND rid='%u'", m_Id,
                               uint32(TabId), rankId);
    CharacterDatabase.PExecute("INSERT INTO guild_bank_right (guildid,TabId,rid,gbright,SlotPerDay) VALUES "
                               "('%u','%u','%u','%u','%u')",
                               m_Id, uint32(TabId), rankId, m_Ranks[rankId].TabRight[TabId],
                               m_Ranks[rankId].TabSlotPerDay[TabId]);
    CharacterDatabase.PExecute("UPDATE guild_member SET BankResetTimeTab%u='0' WHERE guildid='%u' AND `rank`='%u'",
                               uint32(TabId), m_Id, rankId);
  }
}
...
void Guild::LoadGuildBankEventLogFromDB() {
  ...
  for (uint32 tabId = 0; tabId < uint32(GetPurchasedTabs()); ++tabId) {
    //                                                     0        1          2           3            4 5          6
    QueryResult *result = CharacterDatabase.PQuery(
        "SELECT LogGuid, EventType, PlayerGuid, ItemOrMoney, ItemStackCount, DestTabId, TimeStamp FROM "
        "guild_bank_eventlog WHERE guildid='%u' AND TabId='%u' ORDER BY TimeStamp DESC,LogGuid DESC LIMIT %u",
        m_Id, tabId, GUILD_BANK_MAX_LOGS);
    if (!result)
      continue;
    ...
    do {
      ...
      if (NewEvent.isMoneyEvent()) {
        ...
        CharacterDatabase.PExecute(
            "UPDATE guild_bank_eventlog SET TabId='%u' WHERE guildid='%u' AND TabId='%u' AND LogGuid='%u'",
            GUILD_BANK_MONEY_LOGS_TAB, m_Id, tabId, logGuid);
        sLog.outError("GuildBankEventLog ERROR: MoneyEvent LogGuid %u for Guild %u had incorrectly set its TabId to "
                      "%u, correcting it to %u TabId",
                      logGuid, m_Id, tabId, GUILD_BANK_MONEY_LOGS_TAB);
        continue;
      }
      // add event to list
      ...
      if (!isNextLogGuidSet) {
        ...
        // we don't have to do m_GuildBankEventLogNextGuid_Item[tabId] %= configCount; - it will be done when creating
        // new record
        isNextLogGuidSet = true;
      }
    } while (result->NextRow());
  ...
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT LogGuid, EventType, PlayerGuid, ItemOrMoney, ItemStackCount, DestTabId, TimeStamp FROM "
      "guild_bank_eventlog WHERE guildid='%u' AND TabId='%u' ORDER BY TimeStamp DESC,LogGuid DESC LIMIT %u",
      m_Id, GUILD_BANK_MONEY_LOGS_TAB, GUILD_BANK_MAX_LOGS);
  if (!result)
    return;
  ...
  do {
    ...
    if (!isNextMoneyLogGuidSet) {
      ...
      // we don't have to do m_GuildBankEventLogNextGuid_Money %= configCount; - it will be done when creating new
      // record
    ...
    if (!NewEvent.isMoneyEvent())
      sLog.outError("GuildBankEventLog ERROR: MoneyEvent LogGuid %u for Guild %u is not MoneyEvent - ignoring...",
                    fields[0].GetUInt32(), m_Id);
    else
      // add event to list
      // events are ordered from oldest (in beginning) to latest (in the end)
...
void Guild::DisplayGuildBankLogs(WorldSession *session, uint8 TabId) {
  ...
  if (TabId == GUILD_BANK_MAX_TABS) {
    ...
    for (GuildBankEventLog::const_iterator itr = m_GuildBankEventLog_Money.begin();
         itr != m_GuildBankEventLog_Money.end(); ++itr) {
      data << uint8(itr->EventType);
    ...
    for (GuildBankEventLog::const_iterator itr = m_GuildBankEventLog_Item[TabId].begin();
         itr != m_GuildBankEventLog_Item[TabId].end(); ++itr) {
      data << uint8(itr->EventType);
      data << ObjectGuid(HIGHGUID_PLAYER, itr->PlayerGuid);
      data << uint32(itr->ItemOrMoney);
...
void Guild::LogBankEvent(uint8 EventType, uint8 TabId, uint32 PlayerGuidLow, uint32 ItemOrMoney, uint8 ItemStackCount,
                         uint8 DestTabId) {
  // create Event
  GuildBankEventLogEntry NewEvent;
  NewEvent.EventType = EventType;
  ...
  if (NewEvent.isMoneyEvent()) {
    m_GuildBankEventLogNextGuid_Money =
        (m_GuildBankEventLogNextGuid_Money + 1) % sWorld.getConfig(CONFIG_UINT32_GUILD_BANK_EVENT_LOG_COUNT);
    ...
    m_GuildBankEventLogNextGuid_Item[TabId] =
        ((m_GuildBankEventLogNextGuid_Item[TabId]) + 1) % sWorld.getConfig(CONFIG_UINT32_GUILD_BANK_EVENT_LOG_COUNT);
  ...
  CharacterDatabase.PExecute("DELETE FROM guild_bank_eventlog WHERE guildid='%u' AND LogGuid='%u' AND TabId='%u'", m_Id,
                             currentLogGuid, currentTabId);
  ...
  CharacterDatabase.PExecute("INSERT INTO guild_bank_eventlog "
                             ...
                             "TimeStamp) VALUES ('%u','%u','%u','%u','%u','%u','%u','%u','" UI64FMTD "')",
                             m_Id, currentLogGuid, currentTabId, uint32(NewEvent.EventType), NewEvent.PlayerGuid,
                             NewEvent.ItemOrMoney, uint32(NewEvent.ItemStackCount), uint32(NewEvent.DestTabId),
                             NewEvent.TimeStamp);
}
...
void Guild::RemoveItem(uint8 tab, uint8 slot) {
  ...
  CharacterDatabase.PExecute("DELETE FROM guild_bank_item WHERE guildid='%u' AND TabId='%u' AND SlotId='%u'", GetId(),
                             uint32(tab), uint32(slot));
}

InventoryResult Guild::_CanStoreItem_InSpecificSlot(uint8 tab, uint8 slot, GuildItemPosCountVec &dest, uint32 &count,
...
InventoryResult Guild::_CanStoreItem_InTab(uint8 tab, GuildItemPosCountVec &dest, uint32 &count, bool merge,
                                           Item *pSrcItem, uint8 skip_slot) const {
...
InventoryResult Guild::CanStoreItem(uint8 tab, uint8 slot, GuildItemPosCountVec &dest, uint32 count, Item *pItem,
                                    bool swap) const {
  DEBUG_LOG("GUILD STORAGE: CanStoreItem tab = %u, slot = %u, item = %u, count = %u", tab, slot, pItem->GetEntry(),
            count);
...
void Guild::SetGuildBankTabText(uint8 TabId, std::string text) {
  ...
  CharacterDatabase.PExecute("UPDATE guild_bank_tab SET TabText='%s' WHERE guildid='%u' AND TabId='%u'", text.c_str(),
                             m_Id, uint32(TabId));
...
void Guild::SwapItems(Player *pl, uint8 BankTab, uint8 BankTabSlot, uint8 BankTabDst, uint8 BankTabSlotDst,
  ...
  if (SplitedAmount) {
    ...
    if (BankTab != BankTabDst)
      LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTab, pl->GetGUIDLow(), pItemSrc->GetEntry(), SplitedAmount,
                   BankTabDst);
  ...
  } else // non split
  {
    ...
    if (msg == EQUIP_ERR_OK) // merge to
    {
      ...
      if (BankTab != BankTabDst)
        LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTab, pl->GetGUIDLow(), pItemSrc->GetEntry(), pItemSrc->GetCount(),
                     BankTabDst);
    ...
    } else // swap
    {
      ...
      if (BankTab != BankTabDst) {
        LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTab, pl->GetGUIDLow(), pItemSrc->GetEntry(), pItemSrc->GetCount(),
                     BankTabDst);
        LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTabDst, pl->GetGUIDLow(), pItemDst->GetEntry(), pItemDst->GetCount(),
                     BankTab);
      }
...
void Guild::MoveFromBankToChar(Player *pl, uint8 BankTab, uint8 BankTabSlot, uint8 PlayerBag, uint8 PlayerSlot,
                               uint32 SplitedAmount) {
  ...
  } else // Bank -> Char swap with slot (move)
  {
    ...
    if (msg == EQUIP_ERR_OK) // merge case
    {
      ...
      CharacterDatabase.BeginTransaction();
      LogBankEvent(GUILD_BANK_LOG_WITHDRAW_ITEM, BankTab, pl->GetGUIDLow(), pItemBank->GetEntry(),
                   pItemBank->GetCount());
    ...
    } else // Bank <-> Char swap items
    {
      ...
      if (pItemChar) {
        ...
        if (pl->GetSession()->GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE)) {
          sLog.outCommand(pl->GetSession()->GetAccountId(),
                          "GM %s (Account: %u) deposit item: %s (Entry: %d Count: %u) to guild bank (Guild ID: %u )",
                          pl->GetName(), pl->GetSession()->GetAccountId(), pItemChar->GetProto()->Name1,
                          pItemChar->GetEntry(), pItemChar->GetCount(), m_Id);
        }
      }
      ...
      LogBankEvent(GUILD_BANK_LOG_WITHDRAW_ITEM, BankTab, pl->GetGUIDLow(), pItemBank->GetEntry(),
                   pItemBank->GetCount());
      if (pItemChar)
        LogBankEvent(GUILD_BANK_LOG_DEPOSIT_ITEM, BankTab, pl->GetGUIDLow(), pItemChar->GetEntry(),
                     pItemChar->GetCount());
...
void Guild::MoveFromCharToBank(Player *pl, uint8 PlayerBag, uint8 PlayerSlot, uint8 BankTab, uint8 BankTabSlot,
                               uint32 SplitedAmount) {
  Item *pItemBank = GetItem(BankTab, BankTabSlot);
  Item *pItemChar = pl->GetItemByPos(PlayerBag, PlayerSlot);
  ...
  if (SplitedAmount) {
    ...
    if (pl->GetSession()->GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE)) {
      sLog.outCommand(pl->GetSession()->GetAccountId(),
                      "GM %s (Account: %u) deposit item: %s (Entry: %d Count: %u) to guild bank (Guild ID: %u )",
                      pl->GetName(), pl->GetSession()->GetAccountId(), pItemChar->GetProto()->Name1,
                      pItemChar->GetEntry(), SplitedAmount, m_Id);
    }
...
void Guild::BroadcastEvent(GuildEvents event, ObjectGuid guid, char const *str1 /*=nullptr*/,
