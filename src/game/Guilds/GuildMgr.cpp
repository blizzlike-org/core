...
void GuildMgr::AddGuild(Guild *guild) { m_GuildMap[guild->GetId()] = guild; }

void GuildMgr::RemoveGuild(uint32 guildId) { m_GuildMap.erase(guildId); }
...
void GuildMgr::LoadGuilds() {
  ...
  QueryResult *result = CharacterDatabase.Query(
      ...
      //   7               8    9    10         11        12
      "BackgroundColor,info,motd,createdate,BankMoney,(SELECT COUNT(guild_bank_tab.guildid) FROM guild_bank_tab WHERE "
  ...
  QueryResult *guildMembersResult = CharacterDatabase.Query(
      "SELECT guildid,guild_member.guid,`rank`,pnote,offnote,BankResetTimeMoney,BankRemMoney,"
      //   7                 8                9                 10               11                12
      "BankResetTimeTab0,BankRemSlotsTab0,BankResetTimeTab1,BankRemSlotsTab1,BankResetTimeTab2,BankRemSlotsTab2,"
      //   13                14               15                16               17                18
      "BankResetTimeTab3,BankRemSlotsTab3,BankResetTimeTab4,BankRemSlotsTab4,BankResetTimeTab5,BankRemSlotsTab5,"
      //   19               20                21                22                23               24 25
      "characters.name, characters.level, characters.class, characters.gender, characters.zone, "
  ...
  do {
    ...
    if (!newGuild->LoadGuildFromDB(result) || !newGuild->LoadRanksFromDB(guildRanksResult) ||
        !newGuild->LoadMembersFromDB(guildMembersResult) || !newGuild->LoadBankRightsFromDB(guildBankTabRightsResult) ||
  ...
  // you can comment these lines if you don't plan to change CONFIG_UINT32_GUILD_EVENT_LOG_COUNT and
