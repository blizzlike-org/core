...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_STATUS_QUERY - for %s to %s", _player->GetGuidStr().c_str(),
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_HELLO - for %s to %s", _player->GetGuidStr().c_str(),
            guid.GetString().c_str());

  Creature *pCreature = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_NONE);
  if (!pCreature) {
    DEBUG_LOG("WORLD: HandleQuestgiverHelloOpcode - for %s to %s not found or you can't interact with him.",
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_ACCEPT_QUEST - for %s to %s, quest = %u",
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_QUERY_QUEST - for %s to %s, quest = %u",
            _player->GetGuidStr().c_str(), guid.GetString().c_str(), quest);
...
void WorldSession::HandleQuestQueryOpcode(WorldPacket &recv_data) {
  ...
  if (pQuest) {
    ...
    data << uint32(
        pQuest->GetQuestMethod());          // Accepted values: 0, 1 or 2. 0==IsAutoComplete() (skip objectives/details)
    data << int32(pQuest->GetQuestLevel()); // may be -1, static data, in other cases must be used dynamic level:
                                            // Player::GetQuestLevelForPlayer (0 is not known, but assuming this is no
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket &recv_data) {
  ...
  if (reward >= QUEST_REWARD_CHOICES_COUNT) {
    sLog.outError(
        "Error in CMSG_QUESTGIVER_CHOOSE_REWARD - %s tried to get invalid reward (%u) (probably packet hacking)",
        _player->GetGuidStr().c_str(), reward);
    return;
  }
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_CHOOSE_REWARD - for %s to %s, quest = %u, reward = %u",
            _player->GetGuidStr().c_str(), guid.GetString().c_str(), quest, reward);
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket &recv_data) {
  ...
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_REQUEST_REWARD - for %s to %s, quest = %u",
            _player->GetGuidStr().c_str(), guid.GetString().c_str(), quest);
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket &recv_data) {
  ...
  // All ok, continue
  DEBUG_LOG("WORLD: Received opcode CMSG_QUESTGIVER_COMPLETE_QUEST - for %s to %s, quest = %u",
            _player->GetGuidStr().c_str(), guid.GetString().c_str(), quest);

  if (Quest const *pQuest = sObjectMgr.GetQuestTemplate(quest)) {
    if (_player->GetQuestStatus(quest) != QUEST_STATUS_COMPLETE) {
      if (pQuest->IsRepeatable())
        _player->GetPlayerMenu()->SendQuestGiverRequestItems(pQuest, guid, _player->CanCompleteRepeatableQuest(pQuest),
                                                             false);
      else
        _player->GetPlayerMenu()->SendQuestGiverRequestItems(pQuest, guid, _player->CanRewardQuest(pQuest, false),
                                                             false);
    } else
...
void WorldSession::HandlePushQuestToParty(WorldPacket &recvPacket) {
  ...
  if (!_player->CanShareQuest(questId)) {
    sLog.outError("Error in CMSG_PUSHQUESTTOPARTY - %s tried to share invalid quest (%u) (probably packet hacking)",
                  _player->GetGuidStr().c_str(), questId);
    return;
  }
 ...
 * @param defstatus - initial set status (usually it will be called with DIALOG_STATUS_NONE) - must not be
...
uint32 WorldSession::getDialogStatus(const Player *pPlayer, const Object *questgiver, uint32 defstatus) const {
  ...
  for (QuestRelationsMap::const_iterator itr = irbounds.first; itr != irbounds.second; ++itr) {
    ...
    if (status == QUEST_STATUS_COMPLETE && !pPlayer->GetQuestRewardStatus(quest_id))
      dialogStatusNew =
          pQuest->IsRepeatable() && !pQuest->IsDailyOrWeekly() ? DIALOG_STATUS_REWARD_REP : DIALOG_STATUS_REWARD;
    else if (pQuest->IsAutoComplete() && pPlayer->CanTakeQuest(pQuest, false))
      dialogStatusNew = pQuest->IsRepeatable()
                            ? pQuest->IsDailyOrWeekly() ? DIALOG_STATUS_AVAILABLE_REP : DIALOG_STATUS_REWARD_REP
        ...
        if (pPlayer->SatisfyQuestLevel(pQuest, false)) {
          ...
          if (pQuest->IsAutoComplete()) {
            dialogStatusNew = DIALOG_STATUS_REWARD_REP;
                     ...
                     pPlayer->getLevel() <= pPlayer->GetQuestLevelForPlayer(pQuest) + uint32(lowLevelDiff)) {
...
bool WorldSession::CanInteractWithQuestGiver(ObjectGuid guid, char const *descr) const {
  if (guid.IsCreature()) {
    ...
    if (!pCreature) {
      DEBUG_LOG("WORLD: %s - %s cannot interact with %s.", descr, _player->GetGuidStr().c_str(),
                guid.GetString().c_str());
      return false;
    }
    ...
    DEBUG_LOG("WORLD: %s - %s is dead, requested guid was %s", descr, _player->GetGuidStr().c_str(),
              guid.GetString().c_str());
    return false;
  }
