...
OutdoorPvPEP::OutdoorPvPEP() : OutdoorPvP(), m_towersAlliance(0), m_towersHorde(0) {
...
void OutdoorPvPEP::HandlePlayerEnterZone(Player *player, bool isMainZone) {
  ...
  for (auto plaguelandsTowerBuff : plaguelandsTowerBuffs)
    player->RemoveAurasDueToSpell(player->GetTeam() == ALLIANCE ? plaguelandsTowerBuff.spellIdAlliance
...
void OutdoorPvPEP::HandleCreatureCreate(Creature *creature) {
  ...
  case NPC_SPECTRAL_FLIGHT_MASTER:
    ...
    creature->setFaction(m_towerOwner[TOWER_ID_PLAGUEWOOD] == ALLIANCE ? FACTION_FLIGHT_MASTER_ALLIANCE
                                                                       : FACTION_FLIGHT_MASTER_HORDE);
    creature->CastSpell(creature,
                        m_towerOwner[TOWER_ID_PLAGUEWOOD] == ALLIANCE ? SPELL_SPIRIT_PARTICLES_BLUE
...
void OutdoorPvPEP::HandleGameObjectCreate(GameObject *go) {
  ...
  case GO_TOWER_BANNER:
    ...
    if (go->IsWithinDist2d(plaguelandsTowerLocations[TOWER_ID_NORTHPASS][0],
      ...
      InitBanner(go, TOWER_ID_NORTHPASS);
    else if (go->IsWithinDist2d(plaguelandsTowerLocations[TOWER_ID_CROWNGUARD][0],
                                plaguelandsTowerLocations[TOWER_ID_CROWNGUARD][1], 50.0f))
      InitBanner(go, TOWER_ID_CROWNGUARD);
    else if (go->IsWithinDist2d(plaguelandsTowerLocations[TOWER_ID_EASTWALL][0],
                                plaguelandsTowerLocations[TOWER_ID_EASTWALL][1], 50.0f))
      InitBanner(go, TOWER_ID_EASTWALL);
    else if (go->IsWithinDist2d(plaguelandsTowerLocations[TOWER_ID_PLAGUEWOOD][0],
        ...
        if (plaguelandsTowerEvents[i][j].eventEntry == eventId) {
          ...
          if (plaguelandsTowerEvents[i][j].team != m_towerOwner[i]) {
            if (plaguelandsTowerEvents[i][j].defenseMessage)
              sWorld.SendDefenseMessageBroadcastText(ZONE_ID_EASTERN_PLAGUELANDS,
                                                     plaguelandsTowerEvents[i][j].defenseMessage);

            return ProcessCaptureEvent(go, i, plaguelandsTowerEvents[i][j].team,
...
bool OutdoorPvPEP::ProcessCaptureEvent(GameObject *go, uint32 towerId, Team team, uint32 newWorldState) {
  ...
  if (team != TEAM_NONE) {
    ...
    switch (towerId) {
    case TOWER_ID_NORTHPASS:
      RespawnGO(go, m_towerOwner[TOWER_ID_NORTHPASS] == ALLIANCE ? m_lordaeronShrineAlliance : m_lordaeronShrineHorde,
                false);
      break;
    case TOWER_ID_CROWNGUARD:
      sObjectMgr.SetGraveYardLinkTeam(GRAVEYARD_ID_EASTERN_PLAGUE, GRAVEYARD_ZONE_EASTERN_PLAGUE, TEAM_INVALID);
