...
struct SkillExtraItemEntry {
  ...
  SkillExtraItemEntry() : requiredSpecialization(0), additionalCreateChance(0.0f), additionalMaxNum(0) {}
...
// map to store the extra item creation info, the key is the spellId of the creation spell, the mapped value is the
// assigned SkillExtraItemEntry
...
void LoadSkillExtraItemTable() {
  ...
  //                                                 0        1                       2                       3
  QueryResult *result = WorldDatabase.Query("SELECT spellId, requiredSpecialization, additionalCreateChance, "
  ...
  if (result) {
    ...
    do {
      ...
      if (!sSpellTemplate.LookupEntry<SpellEntry>(requiredSpecialization)) {
        sLog.outError("Skill specialization %u have nonexistent required specialization spell id %u in "
      ...
      if (additionalCreateChance <= 0.0f) {
        sLog.outError("Skill specialization %u has too low additional create chance in `skill_extra_item_template`!",
                      spellId);
        continue;
      }
      ...
      if (!additionalMaxNum) {
        sLog.outError("Skill specialization %u has 0 max number of extra items in `skill_extra_item_template`!",
                      spellId);
        continue;
      }
