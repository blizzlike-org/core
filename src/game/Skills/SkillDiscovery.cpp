...
struct SkillDiscoveryEntry {
  ...
  SkillDiscoveryEntry(uint32 _spellId, float _chance) : spellId(_spellId), chance(_chance) {}
};
...
void LoadSkillDiscoveryTable() {
  ...
  do {
    ...
    if (reqSkillOrSpell > 0) // spell case
    {
      ...
      if (!reqSpellEntry) {
        if (reportedReqSpells.find(reqSkillOrSpell) == reportedReqSpells.end()) {
          sLog.outErrorDb(
              "Spell (ID: %u) have nonexistent spell (ID: %i) in `reqSpell` field in `skill_discovery_template` table",
      ...
      if (reqSpellEntry->Mechanic != MECHANIC_DISCOVERY) {
        if (reportedReqSpells.find(reqSkillOrSpell) == reportedReqSpells.end()) {
          sLog.outErrorDb("Spell (ID: %u) not have MECHANIC_DISCOVERY (28) value in Mechanic field in spell.dbc but "
                          "listed for spellId %u (and maybe more) in `skill_discovery_template` table",
    ...
    } else if (reqSkillOrSpell == 0) // skill case
    {
      ...
      if (bounds.first == bounds.second) {
        sLog.outErrorDb("Spell (ID: %u) not listed in `SkillLineAbility.dbc` but listed with `reqSpell`=0 in "
      ...
      sLog.outErrorDb("Spell (ID: %u) have negative value in `reqSpell` field in `skill_discovery_template` table",
                      spellId);
      continue;
    }
  ...
  if (!ssNonDiscoverableEntries.str().empty())
    sLog.outErrorDb("Some items can't be successfully discovered: have in chance field value < 0.000001 in "
                    "`skill_discovery_template` DB table . List:\n%s",
