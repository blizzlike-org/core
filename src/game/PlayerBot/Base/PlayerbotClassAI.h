...
enum JOB_TYPE {
  JOB_MAIN_TANK = 0x01, // for main tank that will need specific heal focus and priority over other party members or
                        // even regular tanks
  ...
  JOB_MANAONLY = 0x40 // for buff checking (NOTE: this means any with powertype mana AND druids (who may be shifted but
                      // still have mana)
};
...
struct heal_priority {
  ...
  // overriding the operator like this is not recommended for general use - however we won't use this struct for
  // anything else
...
public:
  ...
  bool EatDrinkBandage(bool bMana = true, unsigned char foodPercent = 50, unsigned char drinkPercent = 50,
                       unsigned char bandagePercent = 70);
...
protected:
  ...
  CombatManeuverReturns Buff(bool (*BuffHelper)(PlayerbotAI *, uint32, Unit *), uint32 spellId, uint32 type = JOB_ALL,
                             bool mustBeOOC = true);
