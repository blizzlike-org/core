...
void PlayerbotMgr::SetInitialWorldSettings() {
  ...
  if (!botConfig.SetSource(_PLAYERBOT_CONFIG))
    sLog.outError("Playerbot: Unable to open configuration file. Database will be unaccessible. Configuration values "
                  "will use default.");
  else
  ...
  if (botConfig.GetIntDefault("ConfVersion", 0) != PLAYERBOT_CONF_VERSION)
    sLog.outError("Playerbot: Configuration file version doesn't match expected version. Some config variables may be "
                  "wrong or missing.");
}
...
PlayerbotMgr::~PlayerbotMgr() { LogoutAllBots(true); }
...
void PlayerbotMgr::HandleMasterIncomingPacket(const WorldPacket &packet) {
  ...
  case CMSG_OFFER_PETITION: {
    ...
    QueryResult *result =
        CharacterDatabase.PQuery("SELECT * FROM petition_sign WHERE playerguid = '%u' AND petitionguid = '%u'",
                                 player->GetGUIDLow(), petitionLowGuid);
  ...
  case CMSG_ACTIVATETAXI: {
    ...
    // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received CMSG_ACTIVATETAXI from %d to %d", nodes[0],
    // nodes[1]);
  ...
  case CMSG_ACTIVATETAXIEXPRESS: {
    ...
    // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received CMSG_ACTIVATETAXIEXPRESS from %d to %d",
  ...
  case CMSG_QUESTGIVER_ACCEPT_QUEST: {
    ...
    // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received CMSG_QUESTGIVER_ACCEPT_QUEST npc = %s, quest =
    // %u, unk1 = %u", guid.GetString().c_str(), quest, unk1);
    ...
    if (qInfo)
      for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it) {
        ...
        if (bot->GetQuestStatus(quest) == QUEST_STATUS_COMPLETE)
          ...
          bot->GetPlayerbotAI()->TellMaster(
              "I can't take that quest because it requires that I take items, but my bags are full!");
  ...
  case CMSG_QUESTGIVER_COMPLETE_QUEST: {
    ...
    // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received CMSG_QUESTGIVER_COMPLETE_QUEST npc = %s, quest
    // = %u", npcGUID.GetString().c_str(), quest);
  ...
  case CMSG_GOSSIP_HELLO: {
    ...
    for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it) {
      ...
      if (!pCreature) {
        DEBUG_LOG("[PlayerbotMgr]: HandleMasterIncomingPacket - Received  CMSG_GOSSIP_HELLO %s not found or you can't "
                  "interact with him.",
      ...
      GossipMenuItemsMapBounds pMenuItemBounds =
          sObjectMgr.GetGossipMenuItemsMapBounds(pCreature->GetCreatureInfo()->GossipMenuId);
  ...
  case CMSG_SPIRIT_HEALER_ACTIVATE: {
    // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received CMSG_SPIRIT_HEALER_ACTIVATE SpiritHealer is
    // resurrecting the Player %s",m_master->GetName());
    for (PlayerBotMap::iterator itr = m_playerBots.begin(); itr != m_playerBots.end(); ++itr) {
...
void PlayerbotMgr::LogoutPlayerBot(ObjectGuid guid) { m_botToRemove.insert(guid); }
...
void PlayerbotMgr::OnBotLogin(Player *const bot) {
  ...
  *pMSG_MOVE_FALL_LAND << bot->GetMover()->m_movementInfo;
  ...
  if (bot->GetGroup() &&
      (m_master->GetGroup() == nullptr || m_master->GetGroup()->IsMember(bot->GetObjectGuid()) == false))
    bot->RemoveFromGroup();
...
bool ChatHandler::HandlePlayerbotCommand(char *args) {
  ...
  QueryResult *resultlvl =
      CharacterDatabase.PQuery("SELECT level, name, race FROM characters WHERE guid = '%u'", guid.GetCounter());
  if (resultlvl) {
    ...
    if (!(m_session->GetSecurity() > SEC_PLAYER)) {
      if (charlvl > maxlvl) {
        PSendSysMessage("|cffff0000You cannot summon |cffffffff[%s]|cffff0000, it's level is too high.(Current Max:lvl "
                        "|cffffffff%u)",
      ...
      if (!sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP) &&
          m_session->GetPlayer()->GetTeam() != team) {
        PSendSysMessage("|cffff0000You cannot summon |cffffffff[%s]|cffff0000, a member of the enemy side",
                        fields[1].GetString());
        SetSentErrorMessage(true);
        delete resultlvl;
        return false;
