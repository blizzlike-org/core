...
bool PlayerbotClassAI::EatDrinkBandage(bool bMana, unsigned char foodPercent, unsigned char drinkPercent,
                                       unsigned char bandagePercent) {
...
/**
 * Buff()
 * return CombatManeuverReturns Returns RETURN_CONTINUE and buff unit target if it meets criteria to be buffed by bot
 * with spell, else returns RETURN_NO_ACTION_OK
 *
 ...
 * params: mustBeOOC boolean, optional: default: false, will return RETURN_NO_ACTION_OK if set to true and bot is in
 * combat If false is returned, the bot is expected to perform a buff check for the single target version of the group
 * buff.
 *
 */
CombatManeuverReturns PlayerbotClassAI::Buff(bool (*BuffHelper)(PlayerbotAI *, uint32, Unit *), uint32 spellId,
                                             uint32 type, bool mustBeOOC) {
  if (!m_bot.IsAlive() || m_bot.IsInDuel())
    return RETURN_NO_ACTION_ERROR;
  if (mustBeOOC && m_bot.IsInCombat())
    return RETURN_NO_ACTION_ERROR;
  ...
  if (m_bot.GetGroup()) {
    ...
    for (const auto &memberItr : groupSlot) {
      ...
      // If job matches requested job type or (mana target is requested and member is mana user (or shapeshift druid)):
      // buff it
      if (job & type ||
 ...
 * params:singleBuffSpellId uint32 the spell ID of the single target buff equivalent of the group buff like Arcane
 * Intellect for group buff Arcane Brillance If false is returned, the bot is expected to perform a buff check for the
 * single target version of the group buff.
 *
 */
  ...
  if (m_bot.GetGroup()) {
    ...
    for (const auto &memberItr : groupSlot) {
      ...
      if (pet && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) &&
          !(pet->HasAura(groupBuffSpellId, EFFECT_INDEX_0) || pet->HasAura(singleBuffSpellId, EFFECT_INDEX_0)))
        unbuffedTargets++;
    }
...
bool PlayerbotClassAI::FindTargetAndHeal() {
  if (!m_bot.IsAlive() || m_bot.IsInDuel() || !m_ai.IsHealer())
    return false;
 ...
 * If none of the healths are low enough (or multiple valid targets) against these checks, the lowest health is healed.
 * Having a target returned does not guarantee it's worth healing, merely that the target does not have 100% health.
 *
 * return NULL If NULL is returned, no healing is required. At all.
 *
 * Will need extensive re-write for co-operation amongst multiple healers. As it stands, multiple healers would all pick
 * the same 'ideal' healing target.
 */
Player *PlayerbotClassAI::GetHealTarget(JOB_TYPE type, bool onlyPickFromSameGroup) {
  ...
  if (m_bot.GetGroup()) {
    ...
    for (Group::member_citerator itr = groupSlot.begin(); itr != groupSlot.end(); itr++) {
      ...
      if (job & type) {
        uiHealthPercentage =
            int(groupMember->GetMaxHealth() != 0 ? groupMember->GetHealth() * 100 / groupMember->GetMaxHealth() : 0);
  ...
  // Sorts according to type: Main tank first, healers second, then regular tanks, then master followed by DPS, thanks
  // to the order of the TYPE enum
  ...
  while (true) {
    ...
    if (uint32(uCount + i) >= uint32(targets.size()) || !(targets.at(uCount).type & JOB_MAIN_TANK))
      break;
    uCount++;
  }
  ...
  if (x > -1)
    return targets.at(x).p;

  // Try to find a healer in need of healing (if multiple, the lowest health one)
  while (true) {
    if (uint32(uCount + i) >= uint32(targets.size()) || !(targets.at(uCount).type & (JOB_HEAL | JOB_MAIN_HEAL)))
      break;
    uCount++;
  }
  ...
  while (true) {
    if (uint32(uCount + i) >= uint32(targets.size()) || !(targets.at(uCount).type & JOB_TANK))
      break;
    uCount++;
  }
  ...
  if (m_MinHealthPercentMaster != m_MinHealthPercentDPS) {
    while (true) {
      if (uint32(uCount + i) >= uint32(targets.size()) || !(targets.at(uCount).type & JOB_MASTER))
        break;
      uCount++;
    }
  ...
  while (true) {
    if (uint32(uCount + i) >= uint32(targets.size()))
      break;
    uCount++;
  }
...
/**
 * FleeFromAoEIfCan()
 * return boolean Check if the bot can move out of the hostile AoE spell then try to find a proper destination and move
 * towards it The AoE is assumed to be centered on the current bot location (this is the case most of the time)
 *
 * params: spellId uint32 the spell ID of the hostile AoE the bot is supposed to move from. It is used to find the
 * radius of the AoE spell params: pTarget Unit* the creature or gameobject the bot will use to define one of the
 * prefered direction in which to flee
 *
...
/**
 * FleeFromTrapGOIfCan()
 * return boolean Check if the bot can move from a hostile nearby trap, then try to find a proper destination and move
 * towards it
 *
 * params: goEntry uint32 the ID of the hostile trap the bot is supposed to move from. It is used to find the radius of
 * the trap params: pTarget Unit* the creature or gameobject the bot will use to define one of the prefered direction in
 * which to flee
 *
 * return true if bot has found a proper destination, false if none was found
 */
 ...
 * return boolean Check if the bot can move from a creature having a specific aura, then try to find a proper
 * destination and move towards it
 *
 * params: goEntry uint32 the ID of the hostile trap the bot is supposed to move from. It is used to find the radius of
 * the trap params: spellId uint32 the spell ID of the aura the creature is supposed to have (directly or from triggered
 * spell). It is used to find the radius of the aura params: pTarget Unit* the creature or gameobject the bot will use
 * to define one of the prefered direction in which to flee
 *
 * return true if bot has found a proper destination, false if none was found
 */
bool PlayerbotClassAI::FleeFromNpcWithAuraIfCan(uint32 NpcEntry, uint32 spellId, Unit *pTarget) {
  ...
  MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(m_bot, NpcEntry, false, false, radius,
                                                                             true);
  MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreature,
                                                                                                     creature_check);
  ...
  return FleeFromPointIfCan(radius, pTarget, pCreature->GetPositionX(), pCreature->GetPositionY(),
 ...
 *
 * params: radius uint32 the minimal radius (distance) used by the bot to look for a destination from the provided
 * position params: pTarget Unit* the creature or gameobject the bot will use to define one of the prefered direction in
 * which to flee params: x0, y0, z0 float the coordinates of the origin point used to calculate the destination point
 * params: forcedAngle float (optional) when iterating to find a proper point in world to move the bot to, this angle
 * will be prioritly used over other angles if it is provided
 *
 * return true if bot has found a proper destination, false if none was found
 */
bool PlayerbotClassAI::FleeFromPointIfCan(uint32 radius, Unit *pTarget, float x0, float y0, float z0,
  ...
  for (uint8 i = 0; i < 3; i++) {
    ...
    // if an angle was provided, use it instead but only for the first iteration in case this does not lead to a valid
    // point
    if (forcedAngle != 0.0f) {
      angle = forcedAngle;
      forcedAngle = 0.0f;
...
Player *PlayerbotClassAI::GetDispelTarget(DispelType dispelType, JOB_TYPE type, bool bMustBeOOC) {
  if (!m_bot.IsAlive() || m_bot.IsInDuel())
    return nullptr;
  if (bMustBeOOC && m_bot.IsInCombat())
    return nullptr;
...
JOB_TYPE PlayerbotClassAI::GetTargetJob(Player *target) {
  ...
  case CLASS_DRUID:
    if (uSpec == DRUID_SPEC_RESTORATION)
      return JOB_HEAL;
    // Feral can be used for both Tank or DPS... play it safe and assume tank. If not... he best be good at threat
    // management or he'll ravage the healer's mana
...
CombatManeuverReturns PlayerbotClassAI::CastSpellWand(uint32 nextAction, Unit *pTarget, uint32 SHOOT) {
  if (SHOOT > 0 && m_bot.FindCurrentSpellBySpellId(SHOOT) && m_bot.GetWeaponForAttack(RANGED_ATTACK, true, true)) {
    if (nextAction == SHOOT)
      // At this point we're already shooting and are asked to shoot. Don't cause a global cooldown by stopping to
      // shoot! Leave it be.
  ...
  if (nextAction == SHOOT) {
    if (SHOOT > 0 && m_ai.GetCombatStyle() == PlayerbotAI::COMBAT_RANGED && !m_bot.FindCurrentSpellBySpellId(SHOOT) &&
        m_bot.GetWeaponForAttack(RANGED_ATTACK, true, true))
