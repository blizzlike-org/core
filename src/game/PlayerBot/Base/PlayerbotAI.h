...
enum ManaRunesId { DEMONIC_RUNE = 22952, DARK_RUNE = 32905 };
...
enum AutoEquipEnum { AUTOEQUIP_OFF = 0, AUTOEQUIP_ON = 1, AUTOEQUIP_ONCE = 2 };
...
enum CombatManeuverReturns {
  // TODO: RETURN_NO_ACTION_UNKNOWN is not part of ANY_OK or ANY_ERROR. It's also bad form and should be eliminated
  // ASAP.
  ...
  RETURN_FINISHED_FIRST_MOVES =
      0x10,               // Last action of first-combat-maneuver finished, continue onto next-combat-maneuver
...
public:
  ...
  enum CombatOrderType {
    ...
    ORDERS_TEMP_WAIT_TANKAGGRO =
        0x0040, // Wait on tank to build aggro - expect healing to continue, disable setting when tank loses focus
    ORDERS_TEMP_WAIT_OOC =
        0x0080,                  // Wait but only while OOC - wait only - combat will resume healing, dps, tanking, ...
    ...
    ORDERS_MAIN_HEAL =
        0x2000, // concentrate on healing the main tank (will ignore other targets as long as MT needs healing)
  ...
  enum ResistType { SCHOOL_NONE = 0, SCHOOL_FIRE = 1, SCHOOL_NATURE = 2, SCHOOL_FROST = 3, SCHOOL_SHADOW = 4 };

  enum CombatTargetType { TARGET_NORMAL = 0x00, TARGET_THREATEN = 0x01 };
  ...
  enum MovementOrderType { MOVEMENT_NONE = 0x00, MOVEMENT_FOLLOW = 0x01, MOVEMENT_STAY = 0x02 };
  ...
  enum AnnounceFlags { NOTHING = 0x00, INVENTORY_FULL = 0x01, CANT_AFFORD = 0x02, CANT_USE_TOO_FAR = 0x03 };
...
public:
  ...
  // Used by bots to check if their target is neutralized (polymorph, shackle or the like). Useful to avoid breaking
  // crowd control
  ...
  void SetCollectFlag(uint8 flag) {
    if (HasCollectFlag(flag))
      m_collectionFlags &= ~flag;
    else
      m_collectionFlags |= flag;
  }
...
private:
  ...
  void _HandleCommandHelp(const char *szText, Player &fromPlayer) {
    std::string text = szText;
    _HandleCommandHelp(text, fromPlayer);
  }
  void _HandleCommandGM(std::string &text, Player &fromPlayer);
  std::string _HandleCommandHelpHelper(std::string sCommand, std::string sExplain, HELPERLINKABLES reqLink = HL_NONE,
                                       bool bReqLinkMultiples = false, bool bCommandShort = false);
  ...
  void _doSellItem(Item *const item, std::ostringstream &report, std::ostringstream &canSell, uint32 &TotalCost,
                   uint32 &TotalSold);
  ...
  uint32 FISHING, HERB_GATHERING, MINING, SKINNING, ASPECT_OF_THE_MONKEY;
