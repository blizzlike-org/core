...
float rand_float(float low, float high) { return (rand() / (static_cast<float>(RAND_MAX) + 1.0)) * (high - low) + low; }

enum NPCFlags VENDOR_MASK =
    (enum NPCFlags)(UNIT_NPC_FLAG_VENDOR | UNIT_NPC_FLAG_VENDOR_AMMO | UNIT_NPC_FLAG_VENDOR_FOOD |
...
PlayerbotAI::PlayerbotAI(PlayerbotMgr &mgr, Player *const bot, bool debugWhisper)
    : m_AutoEquipToggle(false), m_mgr(mgr), m_bot(bot), m_classAI(0), m_ignoreAIUpdatesUntilTime(CurrentTime()),
      m_combatOrder(ORDERS_NONE), m_ScenarioType(SCENARIO_PVE), m_CurrentlyCastingSpellId(0), m_CraftSpellId(0),
      m_spellIdCommand(0), m_targetGuidCommand(ObjectGuid()), m_taxiMaster(ObjectGuid()),
      m_ignoreNeutralizeEffect(false), m_bDebugCommandChat(false), m_debugWhisper(debugWhisper) {
  // set bot state
...
Player *PlayerbotAI::GetMaster() const { return m_mgr.GetMaster(); }
...
bool PlayerbotAI::CanReachWithSpellAttack(Unit *target) {
  ...
  for (SpellRanges::iterator itr = m_spellRangeMap.begin(); itr != m_spellRangeMap.end(); ++itr) {
    ...
    // DEBUG_LOG("[%s] spell (%s) : dist (%f) < maxrange (%f)",m_bot->GetName(), spellInfo->SpellName[0], dist,
    // maxrange);
...
uint32 PlayerbotAI::initSpell(uint32 spellId) {
  ...
  for (SpellChainMapNext::const_iterator itr = nextMap.lower_bound(spellId); itr != nextMap.upper_bound(spellId);
       ++itr) {
...
void PlayerbotAI::SendNotEquipList(Player & /*player*/) {
  ...
  const std::string descr[] = {"head", "neck",     "shoulders", "body",    "chest",   "waist",    "legs",
...
void PlayerbotAI::AutoUpgradeEquipment() // test for autoequip
{
  ...
  for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++) {
    ...
    if (!pItem2) // no item to compare to see if has stats useful for this bots class/style so check for stats and equip
                 // if possible
    {
  ...
  for (uint8 bag = INVENTORY_SLOT_BAG_START; bag < INVENTORY_SLOT_BAG_END; ++bag) {
    ...
    if (pBag)
      for (uint8 slot = 0; slot < pBag->GetBagSize(); ++slot) {
        ...
        Item *const pItem2 =
            m_bot->GetItemByPos(INVENTORY_SLOT_BAG_0, equipSlot); // do we have anything equipped of this type?
        if (!pItem2) {
          ItemPrototype const *pProto2 = pItem->GetProto();
          if (ItemStatsCount(pProto2) > 0) {
            if (!ItemStatComparison(pProto2, pProto2))
        ...
        // we have an equippable item but something else is equipped so lets send it to the comparison function to see
        // if it's better than we have on
        AutoEquipComparison(pItem, pItem2); // pItem is new item, pItem2 is equipped item.
      }
  }
}
...
void PlayerbotAI::AutoEquipComparison(Item *pItem, Item *pItem2) {
  ...
  case ITEM_CLASS_ARMOR: {
    ...
    if (pProto->ItemLevel == pProto2->ItemLevel && pProto->Quality < pProto2->Quality &&
        pProto->Armor <= pProto2->Armor && m_bot->HasSkill(item_armor_skills[pProto2->SubClass]) &&
    ...
    if (pProto->ItemLevel <= pProto2->ItemLevel && pProto->Armor <= pProto2->Armor &&
        m_bot->HasSkill(item_armor_skills[pProto2->SubClass]) &&
        !m_bot->HasSkill(item_armor_skills[pProto2->SubClass + 1])) // itemlevel + armour + armour class
    {
      // First check to see if this item has stats, and if the bot REALLY wants to lose its old item
      if (ItemStatsCount(pProto2) > 0) {
...
bool PlayerbotAI::ItemStatComparison(const ItemPrototype *pProto, const ItemPrototype *pProto2) {
  ...
  for (int i = 0; i < MAX_ITEM_PROTO_STATS;
       ++i) // item can only have 10 stats. We check each stat slot available for stat and type.
  {
    ...
    // caster stats
    if (itemmod == ITEM_MOD_MANA || itemmod == ITEM_MOD_INTELLECT || itemmod == ITEM_MOD_SPIRIT ||
        itemmod == ITEM_MOD_HIT_SPELL_RATING || itemmod == ITEM_MOD_CRIT_SPELL_RATING ||
        itemmod == ITEM_MOD_HASTE_SPELL_RATING || itemmod2 == ITEM_MOD_MANA || itemmod2 == ITEM_MOD_INTELLECT ||
        itemmod2 == ITEM_MOD_SPIRIT || itemmod2 == ITEM_MOD_HIT_SPELL_RATING ||
    ...
    if (itemmod == ITEM_MOD_HEALTH || itemmod == ITEM_MOD_AGILITY || itemmod == ITEM_MOD_STRENGTH ||
        itemmod == ITEM_MOD_DEFENSE_SKILL_RATING || itemmod == ITEM_MOD_DODGE_RATING ||
        itemmod == ITEM_MOD_PARRY_RATING || itemmod == ITEM_MOD_BLOCK_RATING || itemmod == ITEM_MOD_HIT_MELEE_RATING ||
        itemmod == ITEM_MOD_CRIT_MELEE_RATING || itemmod == ITEM_MOD_HIT_TAKEN_MELEE_RATING ||
        itemmod == ITEM_MOD_HIT_TAKEN_RANGED_RATING || itemmod == ITEM_MOD_HIT_TAKEN_SPELL_RATING ||
        ...
        itemmod == ITEM_MOD_HIT_TAKEN_RATING || itemmod == ITEM_MOD_CRIT_TAKEN_RATING || itemmod2 == ITEM_MOD_HEALTH ||
        itemmod2 == ITEM_MOD_AGILITY || itemmod2 == ITEM_MOD_STRENGTH || itemmod2 == ITEM_MOD_DEFENSE_SKILL_RATING ||
        itemmod2 == ITEM_MOD_DODGE_RATING || itemmod2 == ITEM_MOD_PARRY_RATING || itemmod2 == ITEM_MOD_BLOCK_RATING ||
        itemmod2 == ITEM_MOD_HIT_MELEE_RATING || itemmod2 == ITEM_MOD_CRIT_MELEE_RATING ||
        itemmod2 == ITEM_MOD_HIT_TAKEN_MELEE_RATING || itemmod2 == ITEM_MOD_HIT_TAKEN_RANGED_RATING ||
        itemmod2 == ITEM_MOD_HIT_TAKEN_SPELL_RATING || itemmod2 == ITEM_MOD_CRIT_TAKEN_MELEE_RATING ||
        itemmod2 == ITEM_MOD_CRIT_TAKEN_RANGED_RATING || itemmod2 == ITEM_MOD_CRIT_TAKEN_SPELL_RATING ||
        itemmod2 == ITEM_MOD_HASTE_MELEE_RATING || itemmod2 == ITEM_MOD_HIT_TAKEN_RATING ||
        itemmod2 == ITEM_MOD_CRIT_TAKEN_RATING) {
      switch (isclass) // 1 caster, 2 hybrid, 3 melee
      {
      case 1: {
        if (itemmod > 0) {
    ...
    if (itemmod == ITEM_MOD_HIT_RANGED_RATING || itemmod == ITEM_MOD_CRIT_RANGED_RATING ||
        itemmod == ITEM_MOD_HASTE_RANGED_RATING || itemmod2 == ITEM_MOD_HIT_RANGED_RATING ||
        itemmod2 == ITEM_MOD_CRIT_RANGED_RATING || itemmod2 == ITEM_MOD_HASTE_RANGED_RATING) {
      switch (isclass) // 1 caster, 2 hybrid, 3 melee
      {
      case 1: {
        if (itemmod > 0) {
...
void PlayerbotAI::SendQuestNeedList() {
  ...
  for (BotNeedItem::iterator itr = m_needItemList.begin(); itr != m_needItemList.end(); ++itr) {
    ...
    if (pItemProto) {
      ...
      out << " " << itr->second << "x|cffffffff|Hitem:" << pItemProto->ItemId << ":0:0:0:0:0:0:0"
          << "|h[" << itemName << "]|h|r";
    }
  }
  ...
  for (BotNeedItem::iterator itr = m_needCreatureOrGOList.begin(); itr != m_needCreatureOrGOList.end(); ++itr) {
    ...
    if (m_bot->HasQuestForGO(itr->first)) {
      ...
      if (gInfo) {
        ...
        out << " " << itr->second << "x|cFFFFFF00|Hgameobject_entry:" << itr->first << "|h[" << gameobjectName
            << "]|h|r";
      }
    }
  }
...
bool PlayerbotAI::IsItemUseful(uint32 itemid) {
  const static uint32 item_weapon_skills[MAX_ITEM_SUBCLASS_WEAPON] = {SKILL_AXES,
                                                                      SKILL_2H_AXES,
                                                                      SKILL_BOWS,
                                                                      SKILL_GUNS,
                                                                      SKILL_MACES,
  ...
  case ITEM_CLASS_ARMOR:
    if (pProto->SubClass >= MAX_ITEM_SUBCLASS_ARMOR)
      ...
      return (m_bot->HasSkill(item_armor_skills[pProto->SubClass]) &&
              !m_bot->HasSkill(item_armor_skills[pProto->SubClass + 1]));
    break;
  ...
  case ITEM_CLASS_KEY:
    return true;
  case ITEM_CLASS_GEM:
    if (m_bot->HasSkill(SKILL_BLACKSMITHING) || m_bot->HasSkill(SKILL_ENGINEERING) ||
  ...
  case ITEM_CLASS_TRADE_GOODS:
    ...
    case ITEM_SUBCLASS_METAL_STONE:
      if (m_bot->HasSkill(SKILL_BLACKSMITHING) || m_bot->HasSkill(SKILL_ENGINEERING) || m_bot->HasSkill(SKILL_MINING))
        return true;
      break;
    ...
    case ITEM_SUBCLASS_HERB:
      if (m_bot->HasSkill(SKILL_HERBALISM) || m_bot->HasSkill(SKILL_ALCHEMY))
        return true;
      break;
...
void PlayerbotAI::HandleBotOutgoingPacket(const WorldPacket &packet) {
    ...
    /*    case SMSG_CAST_FAILED:
            {
                ...
                if (result != SPELL_CAST_OK)
                {
                    ...
                    case SPELL_FAILED_BAD_TARGETS:  // 12
                        {
                            // DEBUG_LOG("[%s]bad target / not in front(%u) for spellId (%u) & m_CurrentlyCastingSpellId
       (%u)",m_bot->GetName(),result,spellId,m_CurrentlyCastingSpellId); Spell* const pSpell = GetCurrentSpell(); if
       (pSpell) pSpell->cancel(); break;
                        }
                    ...
                    case SPELL_FAILED_EQUIPPED_ITEM_CLASS:  // 29
                        ...
                        default:
                            DEBUG_LOG ("[%s] SMSG_CAST_FAIL: %s err (%u)",m_bot->GetName(), spellInfo->SpellName[0],
       result); return;
                    }
                }
                TellMaster(out.str().c_str());
  ...
  case SMSG_GUILD_INVITE: {
    ...
    if (!sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        m_bot->GetTeam() != sObjectMgr.GetPlayerTeamByGUID(guild->GetLeaderGuid()))
      return;
  ...
  case SMSG_TRADE_STATUS: {
    ...
    // 1 == TRADE_STATUS_BEGIN_TRADE
      ...
      if (!canObeyCommandFrom(*(m_bot->GetTrader()))) {
        // TODO: Really? What if I give a bot all my junk so it's inventory is full when a nice green/blue/purple comes
        // along?
        SendWhisper("I'm not allowed to trade you any of my items, but you are free to give me money or items.",
                    *(m_bot->GetTrader()));
        return;
      }
  ...
  case SMSG_SPELL_START: {
    ...
    // DEBUG_LOG("castItemGuid (%s) casterItemGuid(%s) spellId (%u) cast_count (%u) castFlags (%u) msTime
    // (%u)",castItemGuid.GetString().c_str(),casterGuid.GetString().c_str(), spellId, cast_count, castFlags, msTime);
    ...
    uint32 CastingTime =
        !IsChanneledSpell(pSpellInfo) ? GetSpellCastTime(pSpellInfo, m_bot) : GetSpellDuration(pSpellInfo);
  ...
  case SMSG_LOOT_RELEASE_RESPONSE: {
    ...
    if (guid == m_lootPrev) {
      ...
      if (c && c->GetCreatureInfo()->SkinningLootId && c->GetLootStatus() != CREATURE_LOOT_STATUS_LOOTED) {
        ...
        if (m_bot->HasSkill(reqSkill) &&
            ((reqSkill != SKILL_SKINNING) || (HasCollectFlag(COLLECT_FLAG_SKIN) && reqSkill == SKILL_SKINNING))) {
...
uint8 PlayerbotAI::GetHealthPercent() const { return GetHealthPercent(*m_bot); }

uint8 PlayerbotAI::GetManaPercent(const Unit &target) const {
  return (static_cast<float>(target.GetPower(POWER_MANA)) / target.GetMaxPower(POWER_MANA)) * 100;
}

uint8 PlayerbotAI::GetManaPercent() const { return GetManaPercent(*m_bot); }

uint8 PlayerbotAI::GetRageAmount(const Unit &target) const { return (static_cast<float>(target.GetPower(POWER_RAGE))); }

uint8 PlayerbotAI::GetRageAmount() const { return GetRageAmount(*m_bot); }

uint8 PlayerbotAI::GetEnergyAmount(const Unit &target) const {
  return (static_cast<float>(target.GetPower(POWER_ENERGY)));
}

uint8 PlayerbotAI::GetEnergyAmount() const { return GetEnergyAmount(*m_bot); }

bool PlayerbotAI::HasAura(uint32 spellId, const Unit &player) const {
  if (spellId <= 0)
    return false;

  for (Unit::SpellAuraHolderMap::const_iterator iter = player.GetSpellAuraHolderMap().begin();
       iter != player.GetSpellAuraHolderMap().end(); ++iter) {
...
bool PlayerbotAI::HasAura(const char *spellName) const { return HasAura(spellName, *m_bot); }
...
Item *PlayerbotAI::FindConsumable(uint32 displayId) const {
  ...
  for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++) {
    ...
    if (pItem) {
      ...
      if ((pItemProto->Class == ITEM_CLASS_CONSUMABLE || pItemProto->Class == ITEM_SUBCLASS_BANDAGE) &&
...
static const uint32 uPriorizedManaPotionIds[12] = {
    ...
    MAJOR_MANA_POTION, MAJOR_REJUVENATION_POTION, SUPERIOR_MANA_POTION, GREATER_MANA_POTION,
    MANA_POTION,       LESSER_MANA_POTION,        MINOR_MANA_POTION,    MINOR_REJUVENATION_POTION};
...
void PlayerbotAI::Attack(Unit *forcedTarget) {
  ...
  if (m_botState != BOTSTATE_COMBAT) {
    ...
    // m_lootCurrent = ObjectGuid(); This was clearing loot target, causing bots to leave corpses unlooted if interupted
    // by combat. Needs testing. using this caused bot to remove current loot target, and add this new threat to the
    // loot list.  Now it remembers the loot target and adds a new one. Bot will still clear the target if the master
    // gets too far away from it.
    m_targetCombat = 0;
...
void PlayerbotAI::DoNextCombatManeuver() {
  ...
  if (!m_targetCombat || m_targetCombat->IsDead() || !m_targetCombat->IsInWorld() ||
      !m_bot->CanAttack(m_targetCombat) || !m_bot->IsInMap(m_targetCombat)) {
    m_bot->AttackStop();
...
void PlayerbotAI::DoCombatMovement() {
  ...
  if (m_combatStyle == COMBAT_MELEE && !m_bot->hasUnitState(UNIT_STAT_CHASE) &&
...
bool PlayerbotAI::IsGroupReady() {
  if (!m_bot)
    return true;
  if (!m_bot->IsAlive() || m_bot->IsInDuel())
    return false; // Let's just say you're otherwise occupied
  if (m_bot->IsInCombat())
    return false;
    ...
    for (Group::member_citerator itr = groupSlot.begin(); itr != groupSlot.end(); itr++) {
      ...
      if (groupMember) {
        if (!groupMember->GetPlayerbotAI() &&
            m_bot->GetPlayerbotAI()->GetClassAI()->GetTargetJob(groupMember) & JOB_TANK)
          return groupMember;
  ...
  for (int qs = 0; qs < MAX_QUEST_LOG_SIZE; ++qs) {
    ...
    for (int i = 0; i < QUEST_OBJECTIVES_COUNT; i++) {
      if (!qInfo->ReqCreatureOrGOCount[i] || (qInfo->ReqCreatureOrGOCount[i] - qData.m_creatureOrGOcount[i]) <= 0)
        continue;
      m_needCreatureOrGOList[qInfo->ReqCreatureOrGOId[i]] =
          (qInfo->ReqCreatureOrGOCount[i] - qData.m_creatureOrGOcount[i]);
    }
  }
}
...
void PlayerbotAI::SetQuestNeedItems() {
  ...
  for (int qs = 0; qs < MAX_QUEST_LOG_SIZE; ++qs) {
    ...
    for (int i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++) {
      ...
      QueryResult *result;
      result =
          WorldDatabase.PQuery("SELECT gt.entry FROM gameobject_template gt "
                               "LEFT JOIN gameobject_loot_template glt ON gt.data1 = glt.entry WHERE glt.item = '%u'",
                               qInfo->ReqItemId[i]);
...
void PlayerbotAI::DoFlight() {
  ...
  if (!npc) {
    DEBUG_LOG("[PlayerbotAI]: DoFlight - %s not found or you can't interact with it.",
              m_taxiMaster.GetString().c_str());
    return;
  }
...
void PlayerbotAI::DoLoot() {
  ...
  if (m_bot->GetDistance(wo) < INTERACTION_DISTANCE) {
      ...
      if (c->HasFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE) && !c->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE)) {
        ...
        *packet << m_lootCurrent;
        ...
        if (m_bot->HasSkill(skillId) &&
            ((skillId != SKILL_SKINNING) || (HasCollectFlag(COLLECT_FLAG_SKIN) && skillId == SKILL_SKINNING))) {
    ...
    if ((m_bot->HasSkill(skillId) && skillId != SKILL_NONE) || (skillId == SKILL_NONE && go)) {
      if (SkillValue < reqSkillValue) {
        TellMaster("My skill is not high enough. It requires %u, but mine is %u.", reqSkillValue, SkillValue);
        skillFailed = true;
      }
      switch (skillId) {
      case SKILL_MINING:
      ...
      case SKILL_HERBALISM:
        ...
        break;
      case SKILL_SKINNING:
        if (c && HasCollectFlag(COLLECT_FLAG_SKIN) && HasTool(TC_SKINNING_KNIFE) &&
            CastSpell(SKINNING, *c) == SPELL_CAST_OK)
          return;
        else
          skillFailed = true;
        break;
        ...
        if (!m_bot->GetQuestRewardStatus(questID)) {
          ...
          if (pQuest->GetRewChoiceItemsCount() == 0) {
            if (m_bot->CanRewardQuest(pQuest, false)) {
              ...
              out << "Quest complete: |cff808080|Hquest:" << questID << ':' << pQuest->GetQuestLevel() << "|h["
                  << questTitle << "]|h|r";
            } else
              out << "|cffff0000Unable to turn quest in:|r |cff808080|Hquest:" << questID << ':'
          ...
          // auto reward quest if one item as reward
            ...
            if (m_bot->CanRewardQuest(pQuest, rewardIdx, false)) {
              ...
              out << "Quest complete: "
                  << " |cff808080|Hquest:" << questID << ':' << pQuest->GetQuestLevel() << "|h[" << questTitle
                  << "]|h|r reward: |cffffffff|Hitem:" << pRewardItem->ItemId << ":0:0:0:0:0:0:0"
                  << "|h[" << itemName << "]|h|r";
            } else
              out << "|cffff0000Unable to turn quest in:|r "
                  << "|cff808080|Hquest:" << questID << ':' << pQuest->GetQuestLevel() << "|h[" << questTitle << "]|h|r"
                  << " reward: |cffffffff|Hitem:" << pRewardItem->ItemId << ":0:0:0:0:0:0:0"
                  << "|h[" << itemName << "]|h|r";
          }
          ...
          // else multiple rewards - let master pick
            ...
            for (uint8 i = 0; i < pQuest->GetRewChoiceItemsCount(); ++i) {
              ...
              std::string itemName = pRewardItem->Name1;
              ItemLocalization(itemName, pRewardItem->ItemId);
              out << "|cffffffff|Hitem:" << pRewardItem->ItemId << ":0:0:0:0:0:0:0"
                  << "|h[" << itemName << "]|h|r";
            }
          }
        }
...
/**
 * IsRegenerating()
 * return boolean Returns true if bot is having at least one aura associated with regenerating mana (drinking) or health
 * (food)
 *
 * params: null
...
bool PlayerbotAI::IsRegenerating() {
  ...
  // or eating (spell category 11, at least for conjured food or season/battle ground items triggering both food and
  // drink auras) or we check is spell is castable while sitting and provide health/power regen like food/drink spells
  // do
    ...
    if (spell->HasAttribute(SPELL_ATTR_CASTABLE_WHILE_SITTING)) {
      for (uint32 i = EFFECT_INDEX_0; i < MAX_EFFECT_INDEX; ++i) {
        if (spell->Effect[i] == SPELL_EFFECT_APPLY_AURA &&
            (spell->EffectApplyAuraName[i] == SPELL_AURA_MOD_POWER_REGEN ||
             spell->EffectApplyAuraName[i] == SPELL_AURA_MOD_REGEN))
          return true;
      }
    }
  }
...
void PlayerbotAI::BotDataRestore() {
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT combat_delay,autoequip FROM playerbot_saved_data WHERE guid = '%u'", m_bot->GetGUIDLow());

  if (!result) {
    sLog.outString();
    sLog.outString(">> [BotDataRestore()] Loaded `playerbot_saved_data`, found no match for guid %u.",
                   m_bot->GetGUIDLow());
    m_DelayAttack = 0;
    return;
  } else {
...
void PlayerbotAI::CombatOrderRestore() {
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT combat_order,primary_target,secondary_target,pname,sname,combat_delay,auto_follow FROM "
      "playerbot_saved_data WHERE guid = '%u'",
      m_bot->GetGUIDLow());

  if (!result) {
    sLog.outString();
    sLog.outString(">> [CombatOrderRestore()] Loaded `playerbot_saved_data`, found no match for guid %u.",
                   m_bot->GetGUIDLow());
  ...
  if (combatOrders & ORDERS_PRIMARY)
    SetCombatOrder(combatOrders, gPrimtarget);
  if (combatOrders & ORDERS_SECONDARY)
    SetCombatOrder(combatOrders, gSectarget);
}
...
void PlayerbotAI::SetCombatOrderByStr(std::string str, Unit *target) {
  ...
  if (str == "tank")
    co = ORDERS_TANK;
  else if (str == "maintank")
    co = ORDERS_MAIN_TANK;
  else if (str == "assist")
    co = ORDERS_ASSIST;
  else if (str == "heal")
    co = ORDERS_HEAL;
  else if (str == "mainheal")
    co = ORDERS_MAIN_HEAL;
  else if (str == "notmainheal")
    co = ORDERS_NOT_MAIN_HEAL;
  else if (str == "protect")
    co = ORDERS_PROTECT;
  else if (str == "passive")
    co = ORDERS_PASSIVE;
  else if (str == "pull")
    co = ORDERS_TEMP_WAIT_TANKAGGRO;
  else if (str == "nodispel")
    co = ORDERS_NODISPEL;
  else if (str == "resistfrost")
    co = ORDERS_RESIST_FROST;
  else if (str == "resistnature")
    co = ORDERS_RESIST_NATURE;
  else if (str == "resistfire")
    co = ORDERS_RESIST_FIRE;
  else if (str == "resistshadow")
    co = ORDERS_RESIST_SHADOW;
  else
    co = ORDERS_RESET;
...
void PlayerbotAI::SetCombatOrder(CombatOrderType co, Unit *target) {
  ...
  case ORDERS_TANK: // 1(01)
  {
    if (m_combatOrder & ORDERS_MAIN_TANK)
      m_combatOrder = (CombatOrderType)((uint32)m_combatOrder &
                                        (uint32)~ORDERS_MAIN_TANK); // ORDERS_TANK and ORDERS_MAIN_TANK exclude one each
                                                                    // other, remove one when the other is set
    break;
  }
  ...
  case ORDERS_HEAL: // 4(100)
  {
    if (m_combatOrder & ORDERS_MAIN_HEAL)
      m_combatOrder = (CombatOrderType)((uint32)m_combatOrder &
                                        (uint32)~ORDERS_MAIN_HEAL); // ORDERS_HEAL and ORDERS_MAIN_HEAL exclude one each
                                                                    // other, remove one when the other is set
    break;
  }
  ...
  case ORDERS_MAIN_TANK: // 1000(1000000000000)
  {
    if (m_combatOrder & ORDERS_TANK)
      m_combatOrder = (CombatOrderType)((uint32)m_combatOrder &
                                        (uint32)~ORDERS_TANK); // ORDERS_TANK and ORDERS_MAIN_TANK exclude one each
                                                               // other, remove one when the other is set
    break;
  }
  case ORDERS_MAIN_HEAL: // 2000(10000000000000)
  {
    if (m_combatOrder & ORDERS_HEAL)
      m_combatOrder = (CombatOrderType)((uint32)m_combatOrder &
                                        (uint32)~ORDERS_HEAL); // ORDERS_HEAL and ORDERS_MAIN_HEAL exclude one each
                                                               // other, remove one when the other is set
    if (m_combatOrder & ORDERS_NOT_MAIN_HEAL)
      m_combatOrder =
          (CombatOrderType)((uint32)m_combatOrder &
                            (uint32)~ORDERS_NOT_MAIN_HEAL); // ORDERS_NOT_MAIN_HEAL and ORDERS_MAIN_HEAL exclude one
                                                            // each other, remove one when the other is set
    break;
  }
  case ORDERS_NOT_MAIN_HEAL: // 4000(100000000000000)
  {
    if (m_combatOrder & ORDERS_MAIN_HEAL)
      m_combatOrder = (CombatOrderType)((uint32)m_combatOrder &
                                        (uint32)~ORDERS_MAIN_HEAL); // ORDERS_NOT_MAIN_HEAL and ORDERS_MAIN_HEAL exclude
                                                                    // one each other, remove one when the other is set
    break;
  }
  case ORDERS_RESET: // FFFF(1111111111111111)
  {
    ...
    m_DelayAttackInit = CurrentTime();
    m_DelayAttack = 0;
    CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET combat_order = 0, primary_target = 0, "
                                     "secondary_target = 0, pname = '',sname = '', combat_delay = 0 WHERE guid = '%u'",
                                     m_bot->GetGUIDLow());
  ...
  if ((co & ORDERS_PRIMARY)) {
    ...
    if (target)
      CharacterDatabase.DirectPExecute(
          ...
          (m_combatOrder & ~ORDERS_TEMP), gTempTarget, gname.c_str(), m_bot->GetGUIDLow());
    else
      CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET combat_order = '%u' WHERE guid = '%u'",
                                       (m_combatOrder & ~ORDERS_TEMP), m_bot->GetGUIDLow());
  } else {
    m_combatOrder = (CombatOrderType)((uint32)m_combatOrder | (uint32)co);
    if (target)
      CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET combat_order = '%u', secondary_target = '%u', "
                                       "sname = '%s' WHERE guid = '%u'",
                                       (m_combatOrder & ~ORDERS_TEMP), gTempTarget, gname.c_str(), m_bot->GetGUIDLow());
    else
      CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET combat_order = '%u' WHERE guid = '%u'",
                                       (m_combatOrder & ~ORDERS_TEMP), m_bot->GetGUIDLow());
...
void PlayerbotAI::Announce(AnnounceFlags msg) {
  ...
  case RACE_HUMAN:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1908) : PlaySound(2032);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1875) : PlaySound(1999);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1924) : PlaySound(2048);
      break;
    default:
      break;
    }
  ...
  case RACE_ORC:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2319) : PlaySound(2374);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2284) : PlaySound(2341);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2335) : PlaySound(2390);
      break;
    default:
      break;
    }
  ...
  case RACE_DWARF:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1630) : PlaySound(1686);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1581) : PlaySound(1654);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1636) : PlaySound(1702);
      break;
    default:
      break;
    }
  ...
  case RACE_NIGHTELF:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2151) : PlaySound(2262);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2118) : PlaySound(2229);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2167) : PlaySound(2278);
      break;
    default:
      break;
    }
  ...
  case RACE_UNDEAD:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2096) : PlaySound(2207);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2054) : PlaySound(2173);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2112) : PlaySound(2223);
      break;
    default:
      break;
    }
  ...
  case RACE_TAUREN:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2463) : PlaySound(2462);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2396) : PlaySound(2397);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(2495) : PlaySound(2494);
      break;
    default:
      break;
    }
  ...
  case RACE_GNOME:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1743) : PlaySound(1798);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1708) : PlaySound(1709);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1759) : PlaySound(1814);
      break;
    default:
      break;
    }
  ...
  case RACE_TROLL:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1853) : PlaySound(1963);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1820) : PlaySound(1930);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(1869) : PlaySound(1993);
      break;
    default:
      break;
    }
  ...
  case RACE_BLOODELF:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(9583) : PlaySound(9584);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(9549) : PlaySound(9550);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(9565) : PlaySound(9566);
      break;
    default:
      break;
    }
  ...
  case RACE_DRAENEI:
    switch (msg) {
    case CANT_AFFORD:
      m_bot->getGender() == GENDER_MALE ? PlaySound(9498) : PlaySound(9499);
      break;
    case INVENTORY_FULL:
      m_bot->getGender() == GENDER_MALE ? PlaySound(9465) : PlaySound(9466);
      break;
    case CANT_USE_TOO_FAR:
      m_bot->getGender() == GENDER_MALE ? PlaySound(9481) : PlaySound(9482);
      break;
    default:
      break;
    }
...
void PlayerbotAI::UpdateAI(const uint32 /*p_time*/) {
  ...
  if (!m_bot->IsAlive()) {
    ...
    if (m_botState == BOTSTATE_DEADRELEASED) {
      ...
      time_t resurrect_time =
          corpse->GetGhostTime() + m_bot->GetCorpseReclaimDelay(corpse->GetType() == CORPSE_RESURRECTABLE_PVP);
      if (resurrect_time > CurrentTime()) {
        SetIgnoreUpdateTime(resurrect_time);
        // DEBUG_LOG ("[PlayerbotAI]: UpdateAI - %s has to wait for %d seconds to revive...", m_bot->GetName(),
        // m_ignoreAIUpdatesUntilTime-CurrentTime() );
        return;
      }
      // resurrect now
  ...
  if (m_bot->IsInWater() &&
      m_bot->GetMap()->GetTerrain()->IsSwimmable(m_bot->GetPositionX(), m_bot->GetPositionY(), m_bot->GetPositionZ(),
                                                 m_bot->GetCollisionHeight())) {
    if (!m_bot->IsSwimming())
  ...
  // unless it is a positive spell then bot is helping a friendly unit and the death of an enemy target should not
  // prevent this make sure any actions that cast a spell set a proper m_ignoreAIUpdatesUntilTime!
  ...
  if (m_botState == BOTSTATE_TAME) {
    ...
    if (m_bot->GetPetGuid() || spell->CheckCast(true) != SPELL_CAST_OK || !pTarget || pTarget->IsDead() ||
        !m_bot->IsInMap(pTarget) ||
        !(((Creature *)pTarget)->GetCreatureInfo()->CreatureTypeFlags & CREATURE_TYPEFLAGS_TAMEABLE)) {
      MovementReset();
      m_bot->SetSelectionGuid(ObjectGuid());
      SetState(BOTSTATE_NORMAL);
      SetIgnoreUpdateTime(0);
  ...
  if (IsInCombat() || (m_botState == BOTSTATE_COMBAT && m_targetCombat) || m_ScenarioType == SCENARIO_PVP_DUEL) {
    ...
    if (!m_bot->IsMounted()) {
      if (!pSpell || !pSpell->IsChannelActive()) {
        // DEBUG_LOG("m_DelayAttackInit (%li) + m_DelayAttack (%u) > time(%li)", m_DelayAttackInit, m_DelayAttack,
        // CurrentTime());
...
void PlayerbotAI::TellMaster(const std::string &text) const { SendWhisper(text, *GetMaster()); }
...
void PlayerbotAI::SendWhisper(const std::string &text, Player &player) const {
  ...
  *packet << text;
  m_bot->GetSession()->QueuePacket(
      std::move(std::unique_ptr<WorldPacket>(packet))); // queue the packet to get around race condition
}
...
SpellCastResult PlayerbotAI::CastSpell(uint32 spellId) {
  ...
  // Power check
  // We use Spell::CheckPower() instead of UnitAI::CanCastSpell() because bots are players and have more requirements
  // than mere units
...
bool PlayerbotAI::CanReceiveSpecificSpell(uint8 spec, Unit *target) const {
  if (IsSpellSpecificUniquePerCaster(SpellSpecific(spec))) {
    ...
    for (it = holders.begin(); it != holders.end(); ++it)
      if ((*it).second->GetCasterGuid() == m_bot->GetObjectGuid() &&
          GetSpellSpecific((*it).second->GetId()) == SpellSpecific(spec))
        return false;
  }
  return true;
}
...
uint8 PlayerbotAI::_findItemSlot(Item *target) {
  ...
  for (uint8 bag = INVENTORY_SLOT_BAG_START; bag < INVENTORY_SLOT_BAG_END; ++bag) // 20 to 23 = 4
  {
    ...
    if (pBag)
      for (uint8 slot = 0; slot < pBag->GetBagSize(); ++slot) {
        // DEBUG_LOG ("[PlayerbotAI]: FindItem - [%s's]bag[%u] slot = %u", m_bot->GetName(), bag, slot);  // 1 to
        // bagsize = ?
  ...
  if (c) {
    ...
    if (loot->GetGoldAmount()) {
      ...
      if (m_mgr.m_confDebugWhisper) {
        ...
        out << "|r|cff009900" << m_bot->GetName() << " loots: "
            << "|h|cffffffff[|r|cff00ff00" << gold << "|r|cfffffc00g|r|cff00ff00" << silver
            << "|r|cffcdcdcds|r|cff00ff00" << copper << "|r|cff993300c"
            << "|h|cffffffff]";
...
void PlayerbotAI::MakeWeaponSkillLink(const SpellEntry *sInfo, std::ostringstream &out, uint32 skillid) {
  int loc = GetMaster()->GetSession()->GetSessionDbcLocale();
  out << "|cff00ffff|Hspell:" << sInfo->Id << "|h[" << sInfo->SpellName[loc] << " : " << m_bot->GetSkillValue(skillid)
      << " /" << m_bot->GetSkillMax(skillid) << "]|h|r";
}
...
void PlayerbotAI::findNearbyGO() {
  ...
  for (BotEntryList::iterator itr = m_collectObjects.begin(); itr != m_collectObjects.end(); ++itr) {
    ...
    MaNGOS::GameObjectEntryInPosRangeCheck go_check(*m_bot, entry, m_bot->GetPositionX(), m_bot->GetPositionY(),
                                                    m_bot->GetPositionZ(), float(m_collectDist));
...
void PlayerbotAI::findNearbyCreature() {
  ...
  TypeContainerVisitor<MaNGOS::CreatureListSearcher<MaNGOS::AnyUnitInObjectRangeCheck>, GridTypeMapContainer> go_visit(
      go_search);
    ...
    for (std::list<enum NPCFlags>::iterator itr = m_findNPC.begin(); itr != m_findNPC.end();) {
      ...
      if (m_bot->GetDistance(wo) < INTERACTION_DISTANCE) {
        ...
        GossipMenuItemsMapBounds pMenuItemBounds =
            sObjectMgr.GetGossipMenuItemsMapBounds(currCreature->GetCreatureInfo()->GossipMenuId);
        ...
        // if canSeeQuests (the default, top level menu) and no menu options exist for this, use options from default
        // options
 ...
 * params:isWorldBoss bool if true, the function will return true only if the target is a worldboss. This allow to
 * enable specific code if the target is a worldboss return false if the target is not elite/rare elite/worldboss or if
 * isWorldBoss was provided as true and that the target is not a worldboss
 *
 */
...
// Check if bot target has one of the following auras: Sap, Polymorph, Shackle Undead, Banish, Seduction, Freezing Trap,
// Hibernate This is used by the AI to prevent bots from attacking crowd control targets
...
/**
 * IsImmuneToSchool()
 * Playerbot wrapper to know if a target is immune or not to a specific damage school. This is used by the AI to prevent
 * using an ability the target is immuned to return bool Returns true if bot's target is a creature with immunity to
 * specified damage school
 *
...
// use item on self
void PlayerbotAI::UseItem(Item *item) { UseItem(item, m_bot); }
...
/**
 * TryEmergency()
 * Playerbot function to select an item that the bot will use to heal itself on low health without waiting for a heal
 * from a healer
 *
...
void PlayerbotAI::EquipItem(Item *src_Item) {
  ...
  DEBUG_LOG("PlayerbotAI::EquipItem: %s in srcbag = %u, srcslot = %u", src_Item->GetProto()->Name1, src_bagIndex,
            src_slot);
...
bool PlayerbotAI::TradeItem(const Item &item, int8 slot) {
  ...
  *packet << (uint8)tradeSlot << (uint8)item.GetBagSlot() << (uint8)item.GetSlot();
  m_bot->GetSession()->QueuePacket(std::move(packet));
  return true;
}
...
void PlayerbotAI::_doSellItem(Item *const item, std::ostringstream &report, std::ostringstream &canSell,
                              uint32 &TotalCost, uint32 &TotalSold) {
  if (!item)
    return;
  ...
  if (SellWhite == 1) // set this with the command 'sell all'
  {
    ...
    if (pProto->SellPrice > 0 && (pProto->Quality == ITEM_QUALITY_NORMAL || pProto->Quality == ITEM_QUALITY_UNCOMMON) &&
        pProto->SubClass != ITEM_SUBCLASS_QUEST) {
      if (pProto->RequiredLevel < (m_bot->getLevel() - m_mgr.gConfigSellLevelDiff) &&
          pProto->SubClass != ITEM_SUBCLASS_WEAPON_MISC && pProto->FoodType == 0) {
        if (pProto->Class == ITEM_CLASS_WEAPON)
          autosell = 1;
        if (pProto->Class == ITEM_CLASS_ARMOR)
          autosell = 1;
      }
      if (pProto->SubClass == ITEM_SUBCLASS_FOOD &&
          (pProto->RequiredLevel < (m_bot->getLevel() - m_mgr.gConfigSellLevelDiff))) {
        autosell = 1;
      }
    }
  }
  ...
  if (autosell ==
      1) // set this switch above and this item gets sold automatically. Only set this for automatic sales e.g junk etc.
  {
    ...
    for (auto i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i) {
      ...
      // if spell charges for this item are negative, it means that the item should be destroyed once the charges are
      // consumed. it also means that the value of this item is relative to how many charges are remaining.
...
bool PlayerbotAI::RemoveAuction(const uint32 auctionid) {
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT houseid,itemguid,item_template,itemowner,buyoutprice,time,buyguid,lastbid,startbid,deposit FROM auction "
      "WHERE id = '%u'",
      auctionid);
...
std::string PlayerbotAI::AuctionResult(std::string subject, std::string body) {
  ...
  case AUCTION_SUCCESSFUL: //= 2,
  {
    ...
    out << "\n( |cff1eff00Deposit:|cffccffff " << Cash(a_info[6]) << " |cffff0000- Tax:|cffccffff " << Cash(a_info[7])
        << " ) |cff1eff00+|cffccffff";
    break;
  }
...
bool PlayerbotAI::AddQuest(const uint32 entry, WorldObject *questgiver) {
  ...
  if (m_bot->GetQuestStatus(entry) == QUEST_STATUS_COMPLETE) {
    ...
    out << "|cffffff00Quest taken "
        << "|cff808080|Hquest:" << entry << ':' << qInfo->GetQuestLevel() << "|h[" << questTitle << "]|h|r";
...
void PlayerbotAI::ListAuctions() {
  ...
  QueryResult *result = CharacterDatabase.PQuery(
      ...
      m_bot->GetObjectGuid().GetCounter());
  if (result) {
    ...
    do {
      ...
      if (expireTime > CurrentTime()) {
        ...
        if (aItem) {
          ...
          if (count > 1)
            report << "|cff00ff00x" << count << "|cffffffff"
                   << "]|h|r";
          else
            report << "]|h|r";
        }
...
void PlayerbotAI::AddAuction(const uint32 itemid, Creature *aCreature) {
  ...
  if (aItem) {
    ...
    uint32 min =
        urand(aItem->GetProto()->SellPrice * aItem->GetCount(), aItem->GetProto()->BuyPrice * aItem->GetCount()) *
        (aItem->GetProto()->Quality + 1);
    uint32 max =
        urand(aItem->GetProto()->SellPrice * aItem->GetCount(), aItem->GetProto()->BuyPrice * aItem->GetCount()) *
        (aItem->GetProto()->Quality + 1);
...
void PlayerbotAI::Buy(Creature *vendor, const uint32 itemid) {
  ...
  for (uint8 vendorslot = 0; vendorslot < numitems; ++vendorslot) {
    VendorItem const *crItem =
        vendorslot < customitems ? vItems->GetItem(vendorslot) : tItems->GetItem(vendorslot - customitems);
...
void PlayerbotAI::HandleCommand(const std::string &text, Player &fromPlayer) {
  ...
  if (text.empty() || text.find("X-Perl") != std::wstring::npos || text.find("HealBot") != std::wstring::npos ||
      text.find("HealComm") != std::wstring::npos || // "HealComm    99990094"
      text.find("LOOT_OPENED") != std::wstring::npos || text.find("CTRA") != std::wstring::npos ||
  ...
  // stats project: 11:30 15/12/10 rev.2 display bot statistics
    ...
    if (!itemIds.empty()) {
      ...
      Object *const pNpc =
          (WorldObject *)m_bot->GetObjectByTypeMask(questRewarderGUID, TYPEMASK_CREATURE_OR_GAMEOBJECT);
      if (!pNpc)
        return;
      ...
      for (uint32 iI = 0; !wasRewarded && iI < questMenu.MenuItemCount(); ++iI) {
        ...
        // if quest is complete, turn it in
        if (status == QUEST_STATUS_COMPLETE && !m_bot->GetQuestRewardStatus(questID) &&
            pQuest->GetRewChoiceItemsCount() > 1 && m_bot->CanRewardQuest(pQuest, false))
...
void PlayerbotAI::_HandleCommandOrders(std::string &text, Player &fromPlayer) {
  if (ExtractCommand("delay", text)) {
    ...
    if (gdelay <= 10) {
      ...
      CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET combat_delay = '%u' WHERE guid = '%u'",
                                       m_DelayAttack, m_bot->GetGUIDLow());
      return;
    } else
    ...
    if (text.empty()) {
      SendWhisper("|cffff0000Syntax error:|cffffffff orders combat <botName> <reset | tank | heal | passive><assist | "
                  "protect [targetPlayer]>",
                  fromPlayer);
      return;
    }

    QueryResult *resultlvl = CharacterDatabase.PQuery("SELECT guid FROM playerbot_saved_data WHERE guid = '%u'",
                                                      m_bot->GetObjectGuid().GetCounter());
    if (!resultlvl)
      CharacterDatabase.DirectPExecute(
          "INSERT INTO playerbot_saved_data "
          "(guid,combat_order,primary_target,secondary_target,pname,sname,combat_delay) VALUES ('%u',0,0,0,'','',0)",
          m_bot->GetObjectGuid().GetCounter());
    else
      delete resultlvl;
    ...
    if (ExtractCommand("protect", text) || ExtractCommand("assist", text)) {
      ...
      if (text.empty() && !targetGUID) {
        SendWhisper("|cffff0000Combat orders protect and assist expect a target either by selection or by giving "
                    "target player in command string!",
                    fromPlayer);
        return;
      }
...
void PlayerbotAI::_HandleCommandFollow(std::string &text, Player &fromPlayer) {
  if (ExtractCommand("auto", text)) // switch to automatic follow distance
  {
    ...
    CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET auto_follow = '%u' WHERE guid = '%u'",
                                     m_FollowAutoGo, m_bot->GetGUIDLow());
  ...
  } else if (ExtractCommand("near", text)) // switch to increment follow distance
  {
    ...
    switch (DistOverRide) {
    case 0:
      msg << "|h|cffff0000" << m_bot->GetDistance(GetMaster()) << "|h|r";
      break; // red
    case 1:
      msg << "|h|cffff8000" << m_bot->GetDistance(GetMaster()) << "|h|r";
      break; // yellow
    case 2:
      msg << "|h|cffe6cc80" << m_bot->GetDistance(GetMaster()) << "|h|r";
      break; // orange
    case 3:
      msg << "|h|cff1eff00" << m_bot->GetDistance(GetMaster()) << "|h|r";
      break; // green
    case 4:
    default:
      msg << "|h|cff0070dd" << m_bot->GetDistance(GetMaster()) << "|h|r";
      break; // blue
    }

    m_FollowAutoGo ? SendWhisper(msg.str(), fromPlayer)
                   : SendWhisper("Automatic Follow Distance is |h|cffff0000OFF|h|r", fromPlayer);
    return;
  } else if (text != "") {
...
void PlayerbotAI::_HandleCommandPull(std::string &text, Player &fromPlayer) {
  ...
  if (ExtractCommand("test", text)) // switch to automatic follow distance
  {
    if (CanPull(fromPlayer))
      SendWhisper("Looks like I am capable of pulling. Ask me 'pull ready' with a target for a more precise check.",
                  fromPlayer);
    return;
  }
  ...
  // TODO: Okay, this one should actually be fixable. InMap should return, but LOS (Line of Sight) should result in
  // moving, well, into LoS.
  if (!m_bot->IsWithinLOSInMap(thingToAttack)) {
  ...
  if (bReadyCheck) {
    SendWhisper(
        "All checks have been passed and I am ready to pull! ... Are you sure you wouldn't like a smaller target?",
        fromPlayer);
    return;
  }
  ...
  /* Technically the tank should wait a bit if/until the HoT has been applied
     ...
     There is no need to take into account that GroupHoTOnTank() may fail due to global cooldown. Either you're prepared
     for a difficult pull in which case it won't fail due to global cooldown, or you're chaining easy pulls in which
     case you don't care.
     */
  /* So have the group wait for the tank to take action (and aggro) - this way it will be easy to see if tank has aggro
     or not without having to worry about tank not being the first to have UpdateAI() called
     */
  ...
  // Set all group members (save this tank) to wait 10 seconds. They will wait until the tank says so, until any
  // non-tank gains aggro or 10 seconds - whichever is shortest
  ...
  //(5) when target is in melee range of tank, wait 2 seconds (healers continue to do group heal checks, all do
  // self-heal checks), then return to normal functioning
}
          ...
          if (res == EQUIP_ERR_OK) {
            ...
            if (m->COD > 0) // if there is COD, take COD money from player and send them to sender by mail
            {
              ...
              if (GetMaster()->GetSession()->GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE)) {
                ...
                sLog.outCommand(GetMaster()->GetSession()->GetAccountId(),
                                "GM %s (Account: %u) receive mail item: %s (Entry: %u Count: %u) and send COD money: "
                                "%u to player: %s (Account: %u)",
                                GetMaster()->GetSession()->GetPlayerName(), GetMaster()->GetSession()->GetAccountId(),
                                item->GetProto()->Name1, item->GetEntry(), item->GetCount(), m->COD,
...
// talent learn [TALENT LINK] ..    -- Learn selected talent [TALENT LINK] from 'talent learn' output (shift click
// icon/link) talent reset                     -- Resets all talents (Must visit a suitable class trainer to reset
// talents)
    ...
    // Handle: List class or profession skills, spells & abilities for selected trainer
      ...
      for (uint32 i = 0; i < 3; ++i) {
        ...
        for (uint32 ts = 0; ts < sTalentStore.GetNumRows(); ++ts) {
          ...
          // fallback if nothing in current locale but locale is supported/found (e.g. spellname not available in loc =
          // 3)
          if (spellInfo->SpellName[loc][0] == '\0') {
            bot_spell_to_learn = spellInfo->SpellName[0];
          }

          out << "|cff4e96f7|Htalent:" << talentInfo->TalentID << ":" << curtalent_maxrank << "|h["
              << bot_spell_to_learn << "]|h|r";
        }
      }
      SendWhisper(out.str(), fromPlayer);
    }
...
void PlayerbotAI::_HandleCommandEquip(std::string &text, Player &fromPlayer) {
  if (ExtractCommand("auto", text)) {
    ...
    CharacterDatabase.DirectPExecute("UPDATE playerbot_saved_data SET autoequip = '%u' WHERE guid = '%u'",
                                     m_AutoEquipToggle, m_bot->GetGUIDLow());
    ...
    m_AutoEquipToggle ? SendWhisper("Auto Equip is |h|cff1eff00ON|h|r", fromPlayer)
...
void PlayerbotAI::_HandleCommandCollect(std::string &text, Player &fromPlayer) {
  while (text.size() > 0) {
    if (ExtractCommand("combat", text))
      ...
      SetCollectFlag(COLLECT_FLAG_PROFESSION);
    else if (ExtractCommand("skin", text) &&
             m_bot->HasSkill(SKILL_SKINNING)) // removes skin even if bot does not have skill
      SetCollectFlag(COLLECT_FLAG_SKIN);
      ...
      if (distance > 0 && distance <= m_mgr.m_confCollectDistanceMax) {
        ...
        oss << "I will now collect items within " << m_mgr.m_confCollectDistanceMax << " yards. " << distance
            << " yards is just too far away.",
            SendWhisper(oss.str(), fromPlayer);
      }
      ...
      std::ostringstream oss;
      oss << "Collect <collectable(s)>: none | distance:<1-" << m_mgr.m_confCollectDistanceMax
          << ">, combat, loot, quest, profession, objects";
    ...
    for (std::list<uint32>::iterator it = m_spellsToLearn.begin(); it != m_spellsToLearn.end(); ++it) {
      ...
      if (SkillLine->categoryId == SKILL_CATEGORY_PROFESSION && *it == SKILL_ENCHANTING) {
        for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j) {
          ...
          if (SkillAbility->skillId == *it && m_bot->HasSpell(SkillAbility->spellId) &&
              SkillAbility->forward_spellid == 0 && ((SkillAbility->classmask & m_bot->getClassMask()) == 0)) {
    ...
    for (int slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot) {
      if (uint32 questId = m_bot->GetQuestSlotQuestId(slot)) {
        ...
        if (m_bot->GetQuestStatus(questId) == QUEST_STATUS_COMPLETE) {
          ...
          comout << " |cFFFFFF00|Hquest:" << questId << ':' << pQuest->GetQuestLevel() << "|h[" << questTitle
                 << "]|h|r";
        } else {
          Item *qitem = FindItem(pQuest->GetSrcItemId());
          if (qitem)
            incomout << " use "
                     << "|cffffffff|Hitem:" << qitem->GetProto()->ItemId << ":0:0:0:0:0:0:0"
                     << "|h[" << qitem->GetProto()->Name1 << "]|h|r"
                     << " on ";
          hasIncompleteQuests = true;
          incomout << " |cFFFFFF00|Hquest:" << questId << ':' << pQuest->GetQuestLevel() << "|h[" << questTitle
                   << "]|h|r";
        }
      }
    }
  ...
  if (ExtractCommand("abandon", text)) {
    ...
    *packet << pet->GetObjectGuid();
    ...
    for (PetSpellMap::iterator itr = pet->m_spells.begin(); itr != pet->m_spells.end(); ++itr) {
      ...
      if (IsPositiveSpell(spellId))
        posOut << " |" << color << "|Hspell:" << spellId << "|h[" << pSpellInfo->SpellName[loc] << "]|h|r";
      else
        negOut << " |" << color << "|Hspell:" << spellId << "|h[" << pSpellInfo->SpellName[loc] << "]|h|r";
    }
...
void PlayerbotAI::_HandleCommandSurvey(std::string & /*text*/, Player &fromPlayer) {
  ...
  result = WorldDatabase.PQuery(
      "SELECT gameobject.guid, id, position_x, position_y, position_z, map, "
      "(POW(position_x - %f, 2) + POW(position_y - %f, 2) + POW(position_z - %f, 2)) AS order_ FROM gameobject "
      "LEFT OUTER JOIN game_event_gameobject on gameobject.guid=game_event_gameobject.guid WHERE map = '%i' %s ORDER "
      "BY order_ ASC LIMIT 10",
      m_bot->GetPositionX(), m_bot->GetPositionY(), m_bot->GetPositionZ(), m_bot->GetMapId(),
  ...
  if (result) {
    do {
      ...
      detectout << "|cFFFFFF00|Hfound:" << guid << ":" << entry << ":"
...
// skill learn                     -- List available class or profession (Primary or Secondary) skills, spells &
// abilities from selected trainer. skill learn all                 -- Learn all skills and spells available from
// selected trainer. skill learn [HLINK][HLINK] ..   -- Learn selected skill and spells, from selected trainer ([HLINK]
// from skill learn). skill unlearn [HLINK][HLINK] .. -- Unlearn selected primary profession skill(s) and all associated
// spells ([HLINK] from skill)
void PlayerbotAI::_HandleCommandSkill(std::string &text, Player &fromPlayer) {
  ...
  if (ExtractCommand("learn", text)) {
    ...
    // Handle: Learning class or profession (primary or secondary) skill & spell(s) for selected trainer, skill learn
    // [HLINK][HLINK][HLINK].. ([HLINK] from skill train)
    if (text.size() > 0) {
      ...
      ReloadAI();
      if (totalSpellLearnt == 0)
        msg.clear();
      else
        msg << "\n";
      msg << "Total of " << totalSpellLearnt << " spell";
      if (totalSpellLearnt != 1)
        msg << "s";
      msg << " learnt, ";
  ...
  // Handle: Unlearning selected primary profession skill(s) and all associated spells, skill unlearn [HLINK][HLINK]..
  // ([HLINK] from skill)
      ...
      if (IsPrimaryProfessionSkill(*it))
        for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j) {
          ...
          if (skillLine->skillId == *it && skillLine->learnOnGetSkill == 0) {
            uint32 SpellId;
            m_bot->HasSpell(skillLine->forward_spellid) ? SpellId = skillLine->forward_spellid
                                                        : SpellId = skillLine->spellId;
            ...
            if (m_bot->GetSkillValue(*it) <= rank[sSpellMgr.GetSpellRank(SpellId)] && m_bot->HasSpell(SpellId)) {
              // DEBUG_LOG ("[PlayerbotAI]: HandleCommand - skill (%u)(%u)(%u):",skillLine->spellId,
              // rank[sSpellMgr.GetSpellRank(skillLine->spellId)], m_bot->GetSkillValue(*it));
...
void PlayerbotAI::_HandleCommandStats(std::string &text, Player &fromPlayer) {
  ...
  out << totalfree << " |h|cffffffff bag slots,|h"
      << " |cff00ff00";
...
void PlayerbotAI::_HandleCommandGM(std::string &text, Player &fromPlayer) {
  // Check should happen OUTSIDE this function, but this is account security we're talking about, so let's be doubly
  // sure
    ...
    if (ExtractCommand("combat", text)) {
      for (AttackerInfoList::iterator i = m_attackerInfo.begin(); i != m_attackerInfo.end(); ++i)
        DEBUG_LOG("[Attacker]:{ %s, victim:%s, threat:%.2f, highest-threat:%.2f, count:%d }",
                  i->second.attacker->GetName(), i->second.victim->GetName(), i->second.threat, i->second.threat2,
                  i->second.count);
      DEBUG_LOG("[Attacker]:{ };");
    } else if (ExtractCommand("loot", text)) {
...
void PlayerbotAI::_HandleCommandHelp(std::string &text, Player &fromPlayer) {
  ...
  if (ExtractCommand("help", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "help",
            "Lists all the things you can order me to do... But it's up to me whether to follow your orders... Or not.")
            .c_str());
    return;
  }
  ...
  if (bMainHelp || ExtractCommand("attack", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "attack", "Attack the selected target. Which would, of course, require a valid target.", HL_TARGET)
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("pull", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("pull", "Pull the target in a coordinated party/raid manner.", HL_TARGET).c_str());
  ...
  if (bMainHelp || ExtractCommand("pull", text)) {
    SendWhisper(_HandleCommandHelpHelper("pull", "Pull the target in a coordinated party/raid manner.", HL_TARGET),
                fromPlayer);

    if (!bMainHelp) {
      SendWhisper(_HandleCommandHelpHelper("pull test", "I'll tell you if I could pull at all. Can be used anywhere."),
                  fromPlayer);
      SendWhisper(_HandleCommandHelpHelper(
                      "pull ready",
                      "I'll tell you if I'm ready to pull *right now*. To be used on location with valid target."),
                  fromPlayer);
      if (!text.empty())
  ...
  if (bMainHelp || ExtractCommand("neutralize", text)) {
    SendWhisper(_HandleCommandHelpHelper("neutralize|neutral",
                                         "The bot will try to put its master's target out of combat with crowd control "
                                         "abilities like polymorph, banish, hibernate, shackles and the like.",
                                         HL_TARGET),
                fromPlayer);
  ...
  if (bMainHelp || ExtractCommand("follow", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("follow",
                                 "I will follow you - this also revives me if dead and teleports me if I'm far away.")
            .c_str());
    ch.SendSysMessage(_HandleCommandHelpHelper("follow auto", "Toggles Automatic Follow Distance (ON/OFF).").c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper("follow info", "I will show my Automatic Follow Distance, toggle status (ON/OFF).")
            .c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper("follow far", "I will follow at a father distance away from you.").c_str());
    ch.SendSysMessage(_HandleCommandHelpHelper("follow near", "I will follow at a closer distance to you.").c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper("follow reset", "I will reset my follow distance to its original state.").c_str());
  ...
  if (bMainHelp || ExtractCommand("assist", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("assist", "I will assist the character listed, attacking as they attack.", HL_NAME)
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("craft", text)) {
    ...
    if (!bMainHelp) {
      ...
      ch.SendSysMessage(
          _HandleCommandHelpHelper("craft < blacksmithing | b >", "List all learnt blacksmith recipes").c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper("craft < cooking | c >", "List all learnt cooking recipes").c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("craft < engineering | e >", "List all learnt engineering recipes").c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper("craft < firstaid | f >", "List all learnt firstaid recipes").c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("craft < jewelcrafting | j >", "List all learnt jewelcrafting recipes").c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("craft < leatherworking | l >", "List all learnt leatherworking recipes").c_str());
      ...
      ch.SendSysMessage(
          _HandleCommandHelpHelper("craft < tailoring | t >", "List all learnt tailoring recipes").c_str());
      if (!text.empty())
        ch.SendSysMessage(sInvalidSubcommand.c_str());
      return;
    }
  ...
  if (bMainHelp || ExtractCommand("process", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("process < disenchant | d >", "Disenchants a green coloured [ITEM] or better", HL_ITEM)
            .c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper("process < prospect | p >", "Searches 5 metal ore [ITEM] for precious gems", HL_ITEM)
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("enchant", text)) {
    ...
    ch.SendSysMessage(_HandleCommandHelpHelper("enchant [SPELL]",
                                               "Enchants selected tradable [ITEM] either equipped or in bag", HL_ITEM)
                          .c_str());
  ...
  if (bMainHelp || ExtractCommand("use", text)) {
    ...
    if (!bMainHelp) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper("use [ITEM]", "I will use the first linked item on a selected TARGET.", HL_TARGET)
              .c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper(
                            "use [ITEM]", "I will use the first linked item on an equipped linked item.", HL_ITEM)
                            .c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper(
                            "use [ITEM]", "I will use the first linked item on a linked gameobject.", HL_GAMEOBJECT)
                            .c_str());
  ...
  if (bMainHelp || ExtractCommand("equip", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("equip auto",
                                 "I will automatically equip items I acquire if they are better than what I'm wearing. "
                                 "Acts as toggle (ON/OFF) if used without subcommand. Fashion sense not included.",
                                 HL_ITEM, true)
            .c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper("equip info", "I will tell you my equip auto toggle status (ON/OFF).").c_str());
    ...
    if (!bMainHelp || ExtractCommand("auto", text)) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper("equip auto on", "Turns auto equip ON, also does an immediate check (like once).")
              .c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper("equip auto off", "Turns auto equip OFF.").c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("equip auto once", "Runs auto equip once, then turns it off.").c_str());
  ...
  if (bMainHelp || ExtractCommand("reset", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "reset",
            "I will reset all my states, orders, loot list, talent spec, ... Hey, that's kind of like memory loss.")
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("stats", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "stats", "This will inform you of my wealth, free bag slots and estimated equipment repair costs.")
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("find", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("find", "I will find said game object, walk right up to it, and wait.", HL_GAMEOBJECT)
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("get", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("get", "I will get said game object and return to your side.", HL_GAMEOBJECT).c_str());
  ...
  if (bMainHelp || ExtractCommand("quest", text)) {
    ...
    if (!bMainHelp) {
      ...
      ch.SendSysMessage(
          _HandleCommandHelpHelper("quest drop", "Removes this quest from my quest log.", HL_QUEST).c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper("quest end", "Turns in my completed quests.").c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("quest list", "Lists the quests offered to me by this target.").c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper("quest report",
                                                 "This will give you a full report of all the items, creatures or "
                                                 "gameobjects I still need to finish my quests.",
                                                 HL_QUEST)
                            .c_str());
  ...
  if (bMainHelp || ExtractCommand("orders", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("orders", "Shows you my orders. Free will is overrated, right?").c_str());

    if (!bMainHelp) {
      SendWhisper(_HandleCommandHelpHelper("orders combat <tank | heal | assist | protect | reset> [targetPlayer]",
                                           "Sets general orders I should follow. Assist and Protect require a target."),
                  fromPlayer);
      SendWhisper(_HandleCommandHelpHelper("orders delay <0-10>", "Activates a delay before I start fighting."),
                  fromPlayer);
      SendWhisper(_HandleCommandHelpHelper("orders resume", "Resume combat orders to what they were before logout."),
                  fromPlayer);

      // Catches all valid subcommands, also placeholders for potential future sub-subcommands
      if (ExtractCommand("combat", text, true)) {
        SendWhisper(_HandleCommandHelpHelper("orders combat tank",
                                             "Order me to tank. Best used on paladins, warriors or druids."),
                    fromPlayer);
        SendWhisper(_HandleCommandHelpHelper("orders combat heal",
                                             "Order me to heal. Best used on shamans, priests, druids or paladins."),
                    fromPlayer);
        SendWhisper(_HandleCommandHelpHelper("orders combat assist",
                                             "Assist the linked target focusing our killing power.", HL_TARGET),
                    fromPlayer);
        SendWhisper(_HandleCommandHelpHelper(
                        "orders combat protect",
                        "Protect the listed target, attempting to keep aggro away from the target.", HL_TARGET),
                    fromPlayer);
        SendWhisper(_HandleCommandHelpHelper("orders combat reset",
                                             "Resets my combat orders as though you'd never given me any at all."),
                    fromPlayer);
  ...
  if (bMainHelp || ExtractCommand("pet", text)) {
    if (bMainHelp)
      ch.SendSysMessage(
          _HandleCommandHelpHelper("pet", "Helps command my pet. Must always be used with a subcommand.").c_str());
    else if (text.empty()) // not "help" AND "help pet"
      ch.SendSysMessage(
          _HandleCommandHelpHelper(
              "pet",
              "This by itself is not a valid command. Just so you know. To be used with a subcommand, such as...")
              .c_str());

    if (!bMainHelp) {
      ch.SendSysMessage(_HandleCommandHelpHelper("pet abandon", "Abandons active hunter pet.").c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("pet cast",
                                   "Has my pet cast this spell. May require a treat. Or at least ask nicely.", HL_SPELL)
              .c_str());
      ...
      if (ExtractCommand("spells", text)) {
        ...
        ch.SendSysMessage(_HandleCommandHelpHelper("aggressive", "sets it so my precious attacks everything in sight.",
                                                   HL_NONE, false, true)
                              .c_str());
        ch.SendSysMessage(_HandleCommandHelpHelper(
                              "defensive",
                              "sets it so it automatically attacks anything that attacks me, or anything I attack.",
                              HL_NONE, false, true)
                              .c_str());
        ch.SendSysMessage(_HandleCommandHelpHelper("passive",
                                                   "makes it so my pet won't attack anything unless directly told to.",
                                                   HL_NONE, false, true)
                              .c_str());
  ...
  if (bMainHelp || ExtractCommand("collect", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("collect", "Tells you what my current collect status is. Also lists possible options.")
            .c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "collect",
            "Sets what I collect. Obviously the 'none' option should be used alone, but all the others can be mixed.",
            HL_OPTION, true)
            .c_str());
  ...
  if (bMainHelp || ExtractCommand("sell", text)) {
    ...
    ch.SendSysMessage(
        _HandleCommandHelpHelper("sell all", "The next time you sell, I'll sell all my low level white items.")
            .c_str());
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "sell all", "This command must be called each time before you sell, OR I won't auto sell white items.")
            .c_str());
    if (!bMainHelp) {
      if (!text.empty())
        ch.SendSysMessage(sInvalidSubcommand.c_str());
  ...
  if (bMainHelp || ExtractCommand("drop", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("drop", "Drops the linked item(s). Permanently.", HL_ITEM, true).c_str());
  ...
  if (bMainHelp || ExtractCommand("auction", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "auction",
            "Lists all my active auctions. With pretty little links and such. Hi hi hi... I'm gonna be sooo rich!")
            .c_str());

    if (!bMainHelp) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper(
              "auction add", "Adds the item to my 'auction off later' list. I have a lot of lists, you see...", HL_ITEM)
              .c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("auction remove",
                                   "Adds the item to my 'Don't auction after all' list. Hope it hasn't sold by then!",
                                   HL_AUCTION)
              .c_str());
  ...
  if (bMainHelp || ExtractCommand("repair", text)) {
    ...
    if (!bMainHelp) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper("repair", "Has me find an armorer and repair the items you listed.", HL_ITEM)
              .c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper(
              "repair all",
              "Has me find an armorer and repair all my items, be they equipped or just taking up bagspace.")
              .c_str());
  ...
  if (bMainHelp || ExtractCommand("talent", text)) {
    ...
    msg =
        _HandleCommandHelpHelper("talent learn", "Lists linked talents, that can be learnt with unspent talent points");
    ...
    if (!bMainHelp) {
      ...
      ch.SendSysMessage(_HandleCommandHelpHelper("talent reset",
                                                 "Resets my talents. Assuming I have the appropriate amount of "
                                                 "sparkly gold, shiny silver, and... unrusted copper.")
                            .c_str());
  ...
  if (bMainHelp || ExtractCommand("bank", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("bank", "Gives you my bank balance. I thought that was private.").c_str());

    if (!bMainHelp) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper("bank deposit", "Deposits the listed items in my bank.", HL_ITEM, true).c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("bank withdraw", "Withdraw the listed items from my bank.", HL_ITEM, true).c_str());
  ...
  if (bMainHelp || ExtractCommand("skill", text)) {
    ...
    if (!bMainHelp) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper("skill learn",
                                   "Lists the things this trainer can teach me. If you've targeted a trainer, that is.",
                                   HL_TARGET)
              .c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("skill learn", "Have me learn this skill from the selected trainer.", HL_SKILL)
              .c_str());
      ch.SendSysMessage(_HandleCommandHelpHelper(
                            "skill unlearn",
                            "Unlearn the linked (primary) profession and everything that goes with it.", HL_PROFESSION)
                            .c_str());

      // Catches all valid subcommands, also placeholders for potential future sub-subcommands
      if (ExtractCommand("learn", text)) {
        ch.SendSysMessage(
            _HandleCommandHelpHelper("skill learn all", "Learn everything this trainer can teach me.", HL_TARGET)
                .c_str());
        if (ExtractCommand("all", text)) {
        }
      } else if (ExtractCommand("unlearn", text)) {
  ...
  if (fromPlayer.GetSession()->GetSecurity() > SEC_PLAYER && (bMainHelp || ExtractCommand("gm", text))) {
    ...
    if (!bMainHelp) {
      ...
      ch.SendSysMessage(
          _HandleCommandHelpHelper("gm chat",
                                   "Outputs all commands the bot receives - including those it feels it can't obey.")
              .c_str());
  ...
  if (bMainHelp || ExtractCommand("mail", text)) {
    ch.SendSysMessage(
        _HandleCommandHelpHelper("mail inbox |cFFFFFF00|h[Mailbox]|h|r", "Lists all bot mail from selected [Mailbox]")
            .c_str());

    if (!bMainHelp) {
      ch.SendSysMessage(
          _HandleCommandHelpHelper("mail getcash", "Gets money from all selected [Mailid]..", HL_MAIL, true).c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("mail getitem", "Gets items from all selected [Mailid]..", HL_MAIL, true).c_str());
      ch.SendSysMessage(
          _HandleCommandHelpHelper("mail delete", "Delete all selected [Mailid]..", HL_MAIL, true).c_str());
  ...
  if (bMainHelp)
    ch.SendSysMessage(
        _HandleCommandHelpHelper(
            "help", "Gives you this listing of main commands... But then, you know that already don't you.")
            .c_str());

  if (!text.empty())
    ch.SendSysMessage("Either that is not a valid command, or someone forgot to add it to my help journal. I mean "
                      "seriously, they can't expect me to remember *all* this stuff, can they?");
}

std::string PlayerbotAI::_HandleCommandHelpHelper(std::string sCommand, std::string sExplain, HELPERLINKABLES reqLink,
                                                  bool bReqLinkMultiples, bool bCommandShort) {
  if (sCommand.empty()) {
  ...
  if (reqLink != HL_NONE) {
    if (reqLink == HL_PROFESSION) {
      ...
      if (bReqLinkMultiples)
        DEBUG_LOG("[PlayerbotAI] _HandleCommandHelpHelper: sCommand \"pet\" with bReqLinkMultiples \"true\". ... Why? "
                  "Bug, surely.");
