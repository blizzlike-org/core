...
class PlayerbotAI;
PlayerbotWarlockAI::PlayerbotWarlockAI(Player &master, Player &bot, PlayerbotAI &ai)
...
CombatManeuverReturns PlayerbotWarlockAI::DoFirstCombatManeuverPVP(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotWarlockAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  // Voidwalker sacrifice gives shield - but you lose the pet (and it's DPS/tank) - use only as last resort for your own
  // health!
  if (m_ai.GetHealthPercent() < 20 && pet && pet->GetEntry() == DEMON_VOIDWALKER && SACRIFICE &&
      !m_bot.HasAura(SACRIFICE))
    m_ai.CastPetSpell(SACRIFICE);
        ...
        if (pet) {
          ...
          case DEMON_SUCCUBUS:
            if (creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_HUMANOID)
              if (SEDUCTION && !newTarget->HasAura(SEDUCTION) &&
                  m_ai.CastPetSpell(SEDUCTION, newTarget) == SPELL_CAST_OK)
                return RETURN_NO_ACTION_OK;
          }
        }
        // if aggroed mob is a demon or an elemental: banish it
        if (creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_DEMON ||
            creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_ELEMENTAL) {
  ...
  if (!m_ai.IsElite(pTarget, true) && pTarget->GetHealthPercent() < HPThreshold &&
      (shardCount < MAX_SHARD_COUNT && freeSpace > 0)) {
    if (SHADOWBURN && m_ai.In_Reach(pTarget, SHADOWBURN) && !pTarget->HasAura(SHADOWBURN) &&
        m_bot.IsSpellReady(SHADOWBURN) && CastSpell(SHADOWBURN, pTarget))
      return RETURN_CONTINUE;

    // Do not cast Drain Soul if Shadowburn is active on target
    if (DRAIN_SOUL && m_ai.In_Reach(pTarget, DRAIN_SOUL) && !pTarget->HasAura(DRAIN_SOUL) &&
        !pTarget->HasAura(SHADOWBURN) && CastSpell(DRAIN_SOUL, pTarget)) {
      m_ai.SetIgnoreUpdateTime(15);
      return RETURN_CONTINUE;
    }
  }

  if (pet && DARK_PACT && (100 * pet->GetPower(POWER_MANA) / pet->GetMaxPower(POWER_MANA)) > 10 &&
      m_ai.GetManaPercent() <= 20)
  ...
  if (spec) {
    ...
    case WARLOCK_SPEC_AFFLICTION:
      if (CORRUPTION && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
          m_ai.In_Reach(pTarget, CORRUPTION) && !pTarget->HasAura(CORRUPTION) && CastSpell(CORRUPTION, pTarget))
        return RETURN_CONTINUE;
      if (FIRE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget, FIRE) &&
          !pTarget->HasAura(FIRE) && CastSpell(FIRE, pTarget))
        return RETURN_CONTINUE;
      if (SIPHON_LIFE > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
          m_ai.In_Reach(pTarget, SIPHON_LIFE) && !pTarget->HasAura(SIPHON_LIFE) && CastSpell(SIPHON_LIFE, pTarget))
        return RETURN_CONTINUE;
      if (SHADOW_BOLT && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
          m_ai.In_Reach(pTarget, SHADOW_BOLT) && CastSpell(SHADOW_BOLT, pTarget))
        return RETURN_CONTINUE;
      break;
    ...
    case WARLOCK_SPEC_DEMONOLOGY:
      ...
      if (FIRE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget, FIRE) &&
          !pTarget->HasAura(FIRE) && CastSpell(FIRE, pTarget))
        return RETURN_CONTINUE;
      if (INCINERATE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
          m_ai.In_Reach(pTarget, INCINERATE) && pTarget->HasAura(FIRE) && CastSpell(INCINERATE, pTarget))
        return RETURN_CONTINUE;
      if (SHADOW_BOLT && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
          m_ai.In_Reach(pTarget, SHADOW_BOLT) && CastSpell(SHADOW_BOLT, pTarget))
        return RETURN_CONTINUE;
    ...
    case WARLOCK_SPEC_DESTRUCTION:
      if (SHADOWBURN && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
          pTarget->GetHealthPercent() < (HPThreshold / 2.0) && m_ai.In_Reach(pTarget, SHADOWBURN) &&
          !pTarget->HasAura(SHADOWBURN) && CastSpell(SHADOWBURN, pTarget))
        return RETURN_CONTINUE;
      if (CORRUPTION && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
          m_ai.In_Reach(pTarget, CORRUPTION) && !pTarget->HasAura(CORRUPTION) && CastSpell(CORRUPTION, pTarget))
        return RETURN_CONTINUE;
      if (FIRE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget, FIRE) &&
          !pTarget->HasAura(FIRE) && CastSpell(FIRE, pTarget))
        return RETURN_CONTINUE;
      if (CONFLAGRATE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
          m_ai.In_Reach(pTarget, CONFLAGRATE) && pTarget->HasAura(FIRE) && m_bot.IsSpellReady(CONFLAGRATE) &&
          CastSpell(CONFLAGRATE, pTarget))
        return RETURN_CONTINUE;
      if (INCINERATE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
          m_ai.In_Reach(pTarget, INCINERATE) && pTarget->HasAura(FIRE) && CastSpell(INCINERATE, pTarget))
        return RETURN_CONTINUE;
  ...
  if (FIRE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget, FIRE) &&
    ...
    return RETURN_CONTINUE;
  if (SHADOW_BOLT && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
      m_ai.In_Reach(pTarget, SHADOW_BOLT))
...
bool PlayerbotWarlockAI::CheckCurse(Unit *target) {
  ...
  if (target->GetTypeId() == TYPEID_UNIT) {
    ...
    if (creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_HUMANOID && target->GetHealthPercent() < 20 &&
      ...
      } else if (CURSE_OF_RECKLESSNESS && m_ai.In_Reach(target, CURSE_OF_RECKLESSNESS) &&
                 !target->HasAura(CURSE_OF_RECKLESSNESS) && !target->HasAura(CURSE_OF_EXHAUSTION) &&
                 CastSpell(CURSE_OF_RECKLESSNESS, target)) {
  ...
  // Enemy elite mages have low health but can cast dangerous spells: group safety before bot DPS OR if facing mana user
  // player
...
void PlayerbotWarlockAI::DoNonCombatActions() {
  ...
  // hp/mana check
  if (pet && DARK_PACT && (100 * pet->GetPower(POWER_MANA) / pet->GetMaxPower(POWER_MANA)) > 40 &&
      m_ai.GetManaPercent() <= 60)
    if (m_ai.CastSpell(DARK_PACT, m_bot) == SPELL_CAST_OK)
      return;
  ...
  if (weapon && !offweapon && weapon->GetProto()->SubClass != ITEM_SUBCLASS_WEAPON_STAFF &&
    ...
    if (spellstone_count == 0 && firestone_count == 0) {
      if ((CREATE_SPELLSTONE && shardCount > 0 && m_ai.CastSpell(CREATE_SPELLSTONE) == SPELL_CAST_OK) ||
          (CREATE_SPELLSTONE == 0 && CREATE_FIRESTONE > 0 && shardCount > 0 &&
           m_ai.CastSpell(CREATE_FIRESTONE) == SPELL_CAST_OK))
        return;
    } else if (stone) {
  ...
  // Heal Voidwalker
  if (pet && pet->GetEntry() == DEMON_VOIDWALKER && CONSUME_SHADOWS && pet->GetHealthPercent() < 75 &&
      !pet->HasAura(CONSUME_SHADOWS))
  ...
  // Check demon buffs
  if (pet && pet->GetEntry() == DEMON_IMP && BLOOD_PACT && !m_bot.HasAura(BLOOD_PACT) &&
      m_ai.CastPetSpell(BLOOD_PACT) == SPELL_CAST_OK)
    return;

  if (pet && pet->GetEntry() == DEMON_FELHUNTER && FEL_INTELLIGENCE && !m_bot.HasAura(FEL_INTELLIGENCE) &&
      m_ai.CastPetSpell(FEL_INTELLIGENCE))
    return;
