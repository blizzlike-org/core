...
CombatManeuverReturns PlayerbotPriestAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/) {
  if (m_ai.IsHealer()) {
    ...
    // This is cast on a target, which activates (and switches to another target within the group) upon
    // receiving+healing damage Mana efficient even at one use
    if (healTarget && PRAYER_OF_MENDING > 0 && m_ai.In_Reach(healTarget, PRAYER_OF_MENDING) &&
        !healTarget->HasAura(PRAYER_OF_MENDING, EFFECT_INDEX_0) &&
        CastSpell(PRAYER_OF_MENDING, healTarget) & RETURN_CONTINUE)
...
CombatManeuverReturns PlayerbotPriestAI::DoFirstCombatManeuverPVP(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotPriestAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  // switch to melee if in melee range AND can't shoot OR have no ranged (wand) equipped AND is not healer
           ...
           (SHOOT == 0 || !m_bot.GetWeaponForAttack(RANGED_ATTACK, true, true)) && !m_ai.IsHealer())
  ...
  if (FEAR_WARD > 0 && m_bot.IsSpellReady(FEAR_WARD)) {
    // Buff tank first
    if (mainTank) {
      if (m_ai.In_Reach(mainTank, FEAR_WARD) && !mainTank->HasAura(FEAR_WARD, EFFECT_INDEX_0) &&
          CastSpell(FEAR_WARD, mainTank))
        return RETURN_CONTINUE;
    }
    // Else try to buff master
    if (m_ai.In_Reach(&m_master, FEAR_WARD) && !m_master.HasAura(FEAR_WARD, EFFECT_INDEX_0) &&
  ...
  if (newTarget && !PlayerbotAI::IsNeutralized(newTarget)) // TODO: && party has a tank
  {
    ...
    if (m_ai.GetHealthPercent() < 35 && POWER_WORD_SHIELD > 0 && !m_bot.HasAura(POWER_WORD_SHIELD, EFFECT_INDEX_0) &&
        !m_bot.HasAura(WEAKNED_SOUL, EFFECT_INDEX_0)) {
    ...
    if (m_ai.GetHealthPercent() < 35 && DESPERATE_PRAYER > 0 && m_ai.In_Reach(&m_bot, DESPERATE_PRAYER) &&
        CastSpell(DESPERATE_PRAYER, &m_bot) & RETURN_CONTINUE) {
    ...
    if (ELUNES_GRACE && !m_bot.HasAura(ELUNES_GRACE, EFFECT_INDEX_0) && m_bot.IsSpellReady(ELUNES_GRACE) &&
        CastSpell(ELUNES_GRACE, &m_bot))
      return RETURN_CONTINUE;
    ...
    if (meleeReach) {
      ...
      // Have threat, can't quickly lower it. 3 options remain: Stop attacking, lowlevel damage (wand), keep on keeping
      // on.
  ...
  if (m_ai.IsHealer()) {
    ...
    if (SHADOW_WORD_PAIN > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
        m_ai.In_Reach(pTarget, SHADOW_WORD_PAIN) && !pTarget->HasAura(SHADOW_WORD_PAIN, EFFECT_INDEX_0) &&
  ...
  case PRIEST_SPEC_HOLY:
    if (HOLY_FIRE > 0 && m_ai.In_Reach(pTarget, HOLY_FIRE) && !pTarget->HasAura(HOLY_FIRE, EFFECT_INDEX_0) &&
  ...
  case PRIEST_SPEC_SHADOW:
    if (DEVOURING_PLAGUE > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
        m_ai.In_Reach(pTarget, DEVOURING_PLAGUE) && !pTarget->HasAura(DEVOURING_PLAGUE, EFFECT_INDEX_0) &&
        CastSpell(DEVOURING_PLAGUE, pTarget))
      return RETURN_CONTINUE;
    if (VAMPIRIC_TOUCH > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
        m_ai.In_Reach(pTarget, VAMPIRIC_TOUCH) && !pTarget->HasAura(VAMPIRIC_TOUCH, EFFECT_INDEX_0) &&
        CastSpell(VAMPIRIC_TOUCH, pTarget))
      return RETURN_CONTINUE;
    if (SHADOW_WORD_PAIN > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
        m_ai.In_Reach(pTarget, SHADOW_WORD_PAIN) && !pTarget->HasAura(SHADOW_WORD_PAIN, EFFECT_INDEX_0) &&
        CastSpell(SHADOW_WORD_PAIN, pTarget))
      return RETURN_CONTINUE;
    if (MIND_BLAST > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
        m_ai.In_Reach(pTarget, MIND_BLAST) && (m_bot.IsSpellReady(MIND_BLAST)) && CastSpell(MIND_BLAST, pTarget))
      return RETURN_CONTINUE;
    if (MIND_FLAY > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
        m_ai.In_Reach(pTarget, MIND_FLAY) && CastSpell(MIND_FLAY, pTarget)) {
      m_ai.SetIgnoreUpdateTime(3);
      return RETURN_CONTINUE;
    }
    if (SHADOWFIEND > 0 && m_ai.In_Reach(pTarget, SHADOWFIEND) && !m_bot.GetPet() && CastSpell(SHADOWFIEND))
      return RETURN_CONTINUE;
    if (SHADOWFORM == 0 && MIND_FLAY == 0 && SMITE > 0 && m_ai.In_Reach(pTarget, SMITE) &&
        CastSpell(SMITE, pTarget)) // low levels
      return RETURN_CONTINUE;
    break;

  case PRIEST_SPEC_DISCIPLINE:
    if (POWER_INFUSION > 0 && m_ai.In_Reach(&m_master, POWER_INFUSION) &&
        CastSpell(POWER_INFUSION, &m_master)) // TODO: just master?
      return RETURN_CONTINUE;
    if (INNER_FOCUS > 0 && m_ai.In_Reach(&m_bot, INNER_FOCUS) && !m_bot.HasAura(INNER_FOCUS, EFFECT_INDEX_0) &&
        CastSpell(INNER_FOCUS, &m_bot))
      return RETURN_CONTINUE;
    if (SMITE > 0 && m_ai.In_Reach(pTarget, SMITE) && CastSpell(SMITE, pTarget))
      return RETURN_CONTINUE;
    break;
...
CombatManeuverReturns PlayerbotPriestAI::DoNextCombatManeuverPVP(Unit *pTarget) {
  ...
  case PlayerbotAI::SCENARIO_PVP_DUEL:
    // TODO: spec tweaking
    if (m_ai.HasAura(SCREAM, *pTarget) && m_ai.GetHealthPercent() < 60 && HEAL && m_ai.In_Reach(pTarget, HEAL) &&
        CastSpell(HEAL) & RETURN_ANY_OK)
      return RETURN_CONTINUE;

    if (SHADOW_WORD_PAIN && m_ai.In_Reach(pTarget, SHADOW_WORD_PAIN) &&
        CastSpell(SHADOW_WORD_PAIN) & RETURN_ANY_OK) // TODO: Check whether enemy has it active yet
      return RETURN_CONTINUE;

    if (m_ai.GetHealthPercent() < 80 && RENEW && m_ai.In_Reach(pTarget, RENEW) &&
        CastSpell(RENEW) & RETURN_ANY_OK) // TODO: Check whether you have renew active on you
      return RETURN_CONTINUE;

    if (SCREAM && m_ai.In_Reach(pTarget, SCREAM) && CastSpell(SCREAM) & RETURN_ANY_OK) // TODO: Check for cooldown
      return RETURN_CONTINUE;

    if (MIND_BLAST && m_ai.In_Reach(pTarget, MIND_BLAST) &&
        CastSpell(MIND_BLAST) & RETURN_ANY_OK) // TODO: Check for cooldown
      return RETURN_CONTINUE;

    if (m_ai.GetHealthPercent() < 50 && GREATER_HEAL && m_ai.In_Reach(pTarget, GREATER_HEAL) &&
        CastSpell(GREATER_HEAL) & RETURN_ANY_OK)
      return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotPriestAI::HealPlayer(Player *target) {
  ...
  if ((GetTargetJob(target) == JOB_TANK || GetTargetJob(target) == JOB_MAIN_TANK) &&
      m_bot.GetPlayerbotAI()->GetMovementOrder() != PlayerbotAI::MOVEMENT_STAY && !m_ai.In_Reach(target, FLASH_HEAL)) {
  ...
  if (m_ai.IsInCombat() && (hp < 50 || m_ai.GetManaPercent() < 40))
    if (INNER_FOCUS > 0 && m_bot.IsSpellReady(INNER_FOCUS) && !m_bot.HasAura(INNER_FOCUS, EFFECT_INDEX_0) &&
        CastSpell(INNER_FOCUS, &m_bot))
      return RETURN_CONTINUE;

  if (hp < 25 && POWER_WORD_SHIELD > 0 && m_ai.IsInCombat() && m_ai.In_Reach(target, POWER_WORD_SHIELD) &&
      !m_bot.HasAura(POWER_WORD_SHIELD, EFFECT_INDEX_0) && !target->HasAura(WEAKNED_SOUL, EFFECT_INDEX_0) &&
      m_ai.CastSpell(POWER_WORD_SHIELD, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  if (hp < 35 && FLASH_HEAL > 0 && m_ai.In_Reach(target, FLASH_HEAL) &&
      m_ai.CastSpell(FLASH_HEAL, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  if (hp < 50 && GREATER_HEAL > 0 && m_ai.In_Reach(target, GREATER_HEAL) &&
      m_ai.CastSpell(GREATER_HEAL, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  // Heals target AND self for equal amount
  if (hp < 60 && hpSelf < 80 && BINDING_HEAL > 0 && m_ai.In_Reach(target, BINDING_HEAL) &&
      m_ai.CastSpell(BINDING_HEAL, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  if (hp < 60 && PRAYER_OF_MENDING > 0 && m_ai.In_Reach(target, PRAYER_OF_MENDING) &&
      !target->HasAura(PRAYER_OF_MENDING, EFFECT_INDEX_0) && CastSpell(PRAYER_OF_MENDING, target))
    return RETURN_FINISHED_FIRST_MOVES;
  if (hp < 70 && HEAL > 0 && m_ai.In_Reach(target, HEAL) && m_ai.CastSpell(HEAL, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  if (hp < 90 && RENEW > 0 && m_ai.In_Reach(target, RENEW) && !target->HasAura(RENEW) &&
      m_ai.CastSpell(RENEW, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotPriestAI::ResurrectPlayer(Player *target) {
  ...
  if (RESURRECTION > 0 && m_ai.In_Reach(target, RESURRECTION) &&
      m_ai.CastSpell(RESURRECTION, *target) == SPELL_CAST_OK) {
...
void PlayerbotPriestAI::DoNonCombatActions() {
  ...
  if (m_ai.IsHealer()) {
    ...
    // TODO: In a group/raid with a healer you'd want this bot to focus on DPS (it's not specced/geared for healing
    // either)
  ...
  if (NeedGroupBuff(PRAYER_OF_SHADOW_PROTECTION, SHADOW_PROTECTION) && m_ai.HasSpellReagents(PRAYER_OF_FORTITUDE)) {
    if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_SHADOW &&
        Buff(&PlayerbotPriestAI::BuffHelper, PRAYER_OF_SHADOW_PROTECTION) & RETURN_CONTINUE)
      return;
  } else if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_SHADOW &&
             Buff(&PlayerbotPriestAI::BuffHelper, SHADOW_PROTECTION) & RETURN_CONTINUE)
    return;
