...
private:
  ...
  CombatManeuverReturns CastSpell(uint32 nextAction, Unit *pTarget = nullptr) {
    return CastSpellNoRanged(nextAction, pTarget);
  }
  ...
  uint32 CAT_FORM, BEAR_FORM, DIRE_BEAR_FORM, MOONKIN_FORM, TREE_OF_LIFE, TRAVEL_FORM;

  // druid cat attacks
  uint32 CLAW, COWER, TIGERS_FURY, RAKE, RIP, SHRED, FEROCIOUS_BITE, MAIM, MANGLE, MANGLE_CAT;

  // druid bear/dire bear attacks & buffs
  uint32 BASH, MAUL, SWIPE, DEMORALIZING_ROAR, CHALLENGING_ROAR, GROWL, ENRAGE, FAERIE_FIRE_FERAL, MANGLE_BEAR,
      LACERATE;

  // druid caster DPS attacks & debuffs
  uint32 MOONFIRE, ROOTS, WRATH, OMEN_OF_CLARITY, STARFIRE, INSECT_SWARM, FAERIE_FIRE, FORCE_OF_NATURE, HURRICANE,
      HIBERNATE;

  // druid buffs
  uint32 MARK_OF_THE_WILD, GIFT_OF_THE_WILD, THORNS, INNERVATE, NATURES_SWIFTNESS, BARKSKIN;

  // druid heals
  uint32 LIFEBLOOM, REJUVENATION, REGROWTH, HEALING_TOUCH, SWIFTMEND, TRANQUILITY, REBIRTH, REMOVE_CURSE, CURE_POISON,
      ABOLISH_POISON;
