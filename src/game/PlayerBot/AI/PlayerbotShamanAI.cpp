...
CombatManeuverReturns PlayerbotShamanAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotShamanAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  case SHAMAN_SPEC_ELEMENTAL:
    ...
    /*if (FROST_SHOCK > 0 && !pTarget->HasAura(FROST_SHOCK, EFFECT_INDEX_0) && m_ai.CastSpell(FROST_SHOCK, *pTarget) ==
       SPELL_CAST_OK) return RETURN_CONTINUE;*/
...
CombatManeuverReturns PlayerbotShamanAI::HealPlayer(Player *target) {
  ...
  if (target->GetHealthPercent() < 30 && HEALING_WAVE > 0 && m_ai.CastSpell(HEALING_WAVE, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  if (target->GetHealthPercent() < 50 && LESSER_HEALING_WAVE > 0 &&
      m_ai.CastSpell(LESSER_HEALING_WAVE, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotShamanAI::ResurrectPlayer(Player *target) {
  ...
  if (ANCESTRAL_SPIRIT > 0 && m_ai.In_Reach(target, ANCESTRAL_SPIRIT) &&
      m_ai.CastSpell(ANCESTRAL_SPIRIT, *target) == SPELL_CAST_OK) {
...
void PlayerbotShamanAI::DropTotems() {
  ...
  if ((fire == nullptr) || (m_bot.GetDistance(fire) > 30)) {
    if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FROST && FROST_RESISTANCE_TOTEM > 0 &&
        m_ai.CastSpell(FROST_RESISTANCE_TOTEM) == SPELL_CAST_OK)
      return;
    ...
    // If the spec didn't take totem of wrath, use flametongue
             ...
             m_ai.CastSpell(FLAMETONGUE_TOTEM) == SPELL_CAST_OK)
      return;
  }
  ...
  if ((air == nullptr) || (m_bot.GetDistance(air) > 30)) {
    if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_NATURE && NATURE_RESISTANCE_TOTEM > 0 &&
        m_ai.CastSpell(NATURE_RESISTANCE_TOTEM) == SPELL_CAST_OK)
      return;
    else if (spec == SHAMAN_SPEC_ENHANCEMENT) {
      if (WIND_FURY_TOTEM > 0 /*&& !m_bot.HasAura(IMPROVED_ICY_TALONS)*/ &&
          m_ai.CastSpell(WIND_FURY_TOTEM) == SPELL_CAST_OK)
        return;
    } else {
  ...
  if ((water == nullptr) || (m_bot.GetDistance(water) > 30)) {
    if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FIRE && FIRE_RESISTANCE_TOTEM > 0 &&
        m_ai.CastSpell(FIRE_RESISTANCE_TOTEM) == SPELL_CAST_OK)
      return;
  ...
  /*if (EARTH_ELEMENTAL_TOTEM > 0 && m_ai.CastSpell(EARTH_ELEMENTAL_TOTEM) == SPELL_CAST_OK)
      return RETURN_CONTINUE;*/
  /*if (EARTHBIND_TOTEM > 0 && !pTarget->HasAura(EARTHBIND_TOTEM, EFFECT_INDEX_0) &&
     !m_bot.HasAura(STRENGTH_OF_EARTH_TOTEM, EFFECT_INDEX_0) && m_ai.CastSpell(EARTHBIND_TOTEM) == SPELL_CAST_OK)
      return RETURN_CONTINUE;*/
  ...
  /*if (FIRE_NOVA_TOTEM > 0 && m_ai.CastSpell(FIRE_NOVA_TOTEM) == SPELL_CAST_OK)
      return RETURN_CONTINUE;*/
  /*if (GROUNDING_TOTEM > 0 && !m_bot.HasAura(GROUNDING_TOTEM, EFFECT_INDEX_0) && !m_bot.HasAura(WRATH_OF_AIR_TOTEM,
     EFFECT_INDEX_0) && !m_bot.HasAura(WIND_FURY_TOTEM, EFFECT_INDEX_0) && m_ai.CastSpell(GROUNDING_TOTEM) ==
     SPELL_CAST_OK) return RETURN_CONTINUE;*/
  /*if (MAGMA_TOTEM > 0 && (!m_bot.HasAura(TOTEM_OF_WRATH, EFFECT_INDEX_0)) && m_ai.CastSpell(MAGMA_TOTEM)) ==
     SPELL_CAST_OK) return RETURN_CONTINUE;*/
  /*if (SEARING_TOTEM > 0 && !pTarget->HasAura(SEARING_TOTEM, EFFECT_INDEX_0) && !m_bot.HasAura(TOTEM_OF_WRATH,
     EFFECT_INDEX_0) && m_ai.CastSpell(SEARING_TOTEM) == SPELL_CAST_OK) return RETURN_CONTINUE;*/
  /*if (STONECLAW_TOTEM > 0 && m_ai.GetHealthPercent() < 51 && !pTarget->HasAura(STONECLAW_TOTEM, EFFECT_INDEX_0) &&
     !pTarget->HasAura(EARTHBIND_TOTEM, EFFECT_INDEX_0) && !m_bot.HasAura(STRENGTH_OF_EARTH_TOTEM, EFFECT_INDEX_0) &&
     m_ai.CastSpell(STONECLAW_TOTEM) == SPELL_CAST_OK) return RETURN_CONTINUE;*/
  /*if (STONESKIN_TOTEM > 0 && !m_bot.HasAura(STONESKIN_TOTEM, EFFECT_INDEX_0) &&
     !m_bot.HasAura(STRENGTH_OF_EARTH_TOTEM, EFFECT_INDEX_0) && m_ai.CastSpell(STONESKIN_TOTEM) == SPELL_CAST_OK)
      return RETURN_CONTINUE;*/
...
void PlayerbotShamanAI::DoNonCombatActions() {
  ...
  /*
         ...
         if (ROCKBITER_WEAPON > 0)
              (!m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
     !m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) &&
     m_ai.CastSpell(ROCKBITER_WEAPON,*m_bot) == SPELL_CAST_OK); else if (WINDFURY_WEAPON > 0)
              (!m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) &&
     !m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) &&
     m_ai.CastSpell(WINDFURY_WEAPON,*m_bot) == SPELL_CAST_OK); else if (FLAMETONGUE_WEAPON > 0)
              (!m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
     !m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) &&
     m_ai.CastSpell(FLAMETONGUE_WEAPON,*m_bot) == SPELL_CAST_OK); else if (FROSTBRAND_WEAPON > 0)
              (!m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
     !m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) && !m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) &&
     m_ai.CastSpell(FROSTBRAND_WEAPON,*m_bot) == SPELL_CAST_OK);
   */
  // Mainhand
  Item *weapon;
