...
PlayerbotPaladinAI::PlayerbotPaladinAI(Player &master, Player &bot, PlayerbotAI &ai)
...
CombatManeuverReturns PlayerbotPaladinAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }

CombatManeuverReturns PlayerbotPaladinAI::DoFirstCombatManeuverPVP(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotPaladinAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  if (m_ai.GetHealthPercent() < 8) {
    if (DIVINE_SHIELD > 0 && m_bot.IsSpellReady(DIVINE_SHIELD) && !m_bot.HasAura(DIVINE_SHIELD, EFFECT_INDEX_0) &&
        !m_bot.HasAura(DIVINE_PROTECTION, EFFECT_INDEX_0) && !m_bot.HasAura(FORBEARANCE, EFFECT_INDEX_0) &&
        m_ai.CastSpell(DIVINE_SHIELD, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;

    if (DIVINE_PROTECTION > 0 && !(m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_TANK) &&
        m_bot.IsSpellReady(DIVINE_PROTECTION) && !m_bot.HasAura(DIVINE_SHIELD, EFFECT_INDEX_0) &&
        !m_bot.HasAura(DIVINE_PROTECTION, EFFECT_INDEX_0) && !m_bot.HasAura(FORBEARANCE, EFFECT_INDEX_0) &&
        m_ai.CastSpell(DIVINE_PROTECTION, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
  }
  ...
  if (newTarget && !(m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_TANK) &&
      !m_ai.IsNeutralized(newTarget)) // TODO: && party has a tank
  {
    // Aggroed by an elite
    if (m_ai.IsElite(newTarget)) {
      // Try to stun the mob
      if (HAMMER_OF_JUSTICE > 0 && m_ai.In_Reach(newTarget, HAMMER_OF_JUSTICE) &&
          m_bot.IsSpellReady(HAMMER_OF_JUSTICE) && !newTarget->HasAura(HAMMER_OF_JUSTICE) &&
          m_ai.CastSpell(HAMMER_OF_JUSTICE, *newTarget) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
      ...
      if (m_ai.GetHealthPercent() < 15) {
        ...
        if (DIVINE_PROTECTION > 0 && m_bot.IsSpellReady(DIVINE_PROTECTION) &&
            !m_bot.HasAura(DIVINE_SHIELD, EFFECT_INDEX_0) && !m_bot.HasAura(DIVINE_PROTECTION, EFFECT_INDEX_0) &&
            !m_bot.HasAura(FORBEARANCE, EFFECT_INDEX_0) && m_ai.CastSpell(DIVINE_PROTECTION, m_bot) == SPELL_CAST_OK)
          return RETURN_CONTINUE;
      }
  ...
  case PALADIN_SPEC_RETRIBUTION:
    if (HAMMER_OF_WRATH > 0 && pTarget->GetHealth() < pTarget->GetMaxHealth() * 0.20 &&
        m_ai.CastSpell(HAMMER_OF_WRATH, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (CRUSADER_STRIKE > 0 && m_bot.IsSpellReady(CRUSADER_STRIKE) &&
        m_ai.CastSpell(CRUSADER_STRIKE, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
  ...
  case PALADIN_SPEC_PROTECTION:
    ...
    if (HOLY_SHIELD > 0 && !m_bot.HasAura(HOLY_SHIELD) && m_ai.CastSpell(HOLY_SHIELD, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (AVENGERS_SHIELD > 0 && m_bot.IsSpellReady(AVENGERS_SHIELD) &&
        m_ai.CastSpell(AVENGERS_SHIELD, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (JUDGEMENT > 0 && m_ai.CastSpell(JUDGEMENT, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    return RETURN_NO_ACTION_OK;
...
CombatManeuverReturns PlayerbotPaladinAI::HealPlayer(Player *target) {
  ...
  if (hp < 10 && LAY_ON_HANDS && m_bot.IsSpellReady(LAY_ON_HANDS) && m_ai.In_Reach(target, LAY_ON_HANDS) &&
      m_ai.CastSpell(LAY_ON_HANDS, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  ...
  if (BLESSING_OF_PROTECTION > 0 &&
      ...
      !target->HasAura(FORBEARANCE, EFFECT_INDEX_0) && !target->HasAura(BLESSING_OF_PROTECTION, EFFECT_INDEX_0) &&
      !target->HasAura(DIVINE_PROTECTION, EFFECT_INDEX_0) && !target->HasAura(DIVINE_SHIELD, EFFECT_INDEX_0) &&
  ...
  if (hp < 40 && HOLY_LIGHT > 0 && m_ai.In_Reach(target, HOLY_LIGHT) &&
      m_ai.CastSpell(HOLY_LIGHT, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;

  if (hp < 60 && HOLY_SHOCK > 0 && m_ai.In_Reach(target, HOLY_SHOCK) &&
      m_ai.CastSpell(HOLY_SHOCK, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;

  if (hp < 80 && FLASH_OF_LIGHT > 0 && m_ai.In_Reach(target, FLASH_OF_LIGHT) &&
      m_ai.CastSpell(FLASH_OF_LIGHT, *target) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
...
void PlayerbotPaladinAI::CheckAuras() {
  ...
  if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FROST) {
    ...
    if (!m_bot.HasAura(SHADOW_RESISTANCE_AURA) && SHADOW_RESISTANCE_AURA > 0 &&
        !m_bot.HasAura(SHADOW_RESISTANCE_AURA)) // /*&& !m_bot.HasAura(PRAYER_OF_SHADOW_PROTECTION)*/ /*&&
                                                // !m_bot.HasAura(PRAYER_OF_SHADOW_PROTECTION)*/
...
bool PlayerbotPaladinAI::CheckSealAndJudgement(Unit *target) {
  ...
  if (target->GetTypeId() == TYPEID_UNIT) {
    ...
    if (creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_HUMANOID && target->GetHealthPercent() < 20 &&
        !creature->IsWorldBoss()) {
      if (SEAL_OF_JUSTICE > 0 && !m_bot.HasAura(SEAL_OF_JUSTICE, EFFECT_INDEX_0) &&
          m_ai.CastSpell(SEAL_OF_JUSTICE, m_bot) == SPELL_CAST_OK) {
  ...
  if (m_CurrentSeal > 0 && m_bot.HasAura(m_CurrentSeal, EFFECT_INDEX_0) && m_CurrentJudgement > 0 &&
      target->HasAura(m_CurrentJudgement, EFFECT_INDEX_0))
    return false;
  ...
  if (m_CurrentJudgement == 0 && m_CurrentSeal > 0 && m_bot.HasAura(m_CurrentSeal, EFFECT_INDEX_0)) {
    if (JUDGEMENT > 0 && m_bot.IsSpellReady(JUDGEMENT) && m_ai.In_Reach(target, JUDGEMENT) &&
        m_ai.CastSpell(JUDGEMENT, *target) == SPELL_CAST_OK) {
  ...
  if (m_ai.IsHealer())
    spec = PALADIN_SPEC_HOLY;
  if (m_ai.IsTank())
    spec = PALADIN_SPEC_PROTECTION;
  if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_ASSIST)
    spec = PALADIN_SPEC_RETRIBUTION;
  ...
  return m_CurrentSeal > 0 && !m_bot.HasAura(m_CurrentSeal, EFFECT_INDEX_0) &&
         m_ai.CastSpell(m_CurrentSeal, m_bot) == SPELL_CAST_OK;
}
...
void PlayerbotPaladinAI::DoNonCombatActions() {
  ...
  // buff group
  if (Buff(&PlayerbotPaladinAI::BuffHelper, 1) &
      RETURN_CONTINUE) // Paladin's BuffHelper takes care of choosing the specific Blessing so just pass along a
                       // non-zero value
    return;
...
/**
 * BuffHelper
 * BuffHelper is a static function, takes an AI, spellId (ignored for paladin) and a target and attempts to buff them as
 * well as their pets as best as possible.
 *
...
bool PlayerbotPaladinAI::BuffHelper(PlayerbotAI *ai, uint32 spellId, Unit *target) {
  ...
  if (pet && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) && ai->HasSpellReagents(petBigSpellId) &&
      ai->Buff(petBigSpellId, pet) == SPELL_CAST_OK)
    return true;
  if (ai->HasSpellReagents(bigSpellId) && ai->Buff(bigSpellId, target) == SPELL_CAST_OK)
    return true;
  return (pet && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) && ai->Buff(petSpellId, pet) == SPELL_CAST_OK) ||
         ai->Buff(spellId, target) == SPELL_CAST_OK;
}

// Match up with "Pull()" below
bool PlayerbotPaladinAI::CanPull() { return EXORCISM > 0 && m_bot.IsSpellReady(EXORCISM); }

// Match up with "CanPull()" above
bool PlayerbotPaladinAI::Pull() { return EXORCISM > 0 && m_ai.CastSpell(EXORCISM) == SPELL_CAST_OK; }

bool PlayerbotPaladinAI::CastHoTOnTank() {
  if (!m_ai.IsHealer())
    return false;

  // Paladin: Sheath of Light (with talents), Flash of Light (with Infusion of Light talent and only on a target with
  // the Sacred Shield buff),
