...
CombatManeuverReturns PlayerbotHunterAI::DoFirstCombatManeuver(Unit *pTarget) {
  ...
  // There are NPCs in BGs and Open World PvP, so don't filter this on PvP scenarios (of course if PvP targets anyone
  // but tank, all bets are off anyway) Wait until the tank says so, until any non-tank gains aggro or X seconds -
...
CombatManeuverReturns PlayerbotHunterAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }

bool PlayerbotHunterAI::HasPet(Player *bot) {
  QueryResult *result =
      CharacterDatabase.PQuery("SELECT * FROM character_pet WHERE owner = '%u' AND (slot = '%u' OR slot = '%u')",
                               bot->GetGUIDLow(), PET_SAVE_AS_CURRENT, PET_SAVE_NOT_IN_SLOT);

  return result != nullptr; // hunter either has no pet or stabled
} // end HasPet

CombatManeuverReturns PlayerbotHunterAI::DoFirstCombatManeuverPVP(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotHunterAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  if (pet && PET_MEND > 0 && pet->IsAlive() && pet->GetHealthPercent() < 50 && pVictim != &m_bot &&
      !pet->HasAura(PET_MEND, EFFECT_INDEX_0) && m_ai.CastSpell(PET_MEND, m_bot) == SPELL_CAST_OK) {
    m_ai.TellMaster("healing pet.");
    return RETURN_CONTINUE;
  } else if (pet && INTIMIDATION > 0 && pVictim == pet && !pet->HasAura(INTIMIDATION, EFFECT_INDEX_0) &&
             m_ai.CastSpell(INTIMIDATION, m_bot) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  ...
  */
  ...
  if (TRANQUILIZING_SHOT > 0 && IsTargetEnraged(pTarget) && m_bot.IsSpellReady(TRANQUILIZING_SHOT) &&
      m_ai.CastSpell(TRANQUILIZING_SHOT, *pTarget) == SPELL_CAST_OK) {
    m_ai.TellMaster("Casting TRANQUILIZING SHOT onto %s", pTarget->GetName());
    return RETURN_CONTINUE;
  }

  // Used to determine if this bot has highest threat
  Unit *newTarget = m_ai.FindAttacker(
      (PlayerbotAI::ATTACKERINFOTYPE)(PlayerbotAI::AIT_VICTIMSELF | PlayerbotAI::AIT_HIGHESTTHREAT), &m_bot);
  // Aggro management
  if (newTarget && !PlayerbotAI::IsNeutralized(newTarget)) // TODO: && party has a tank
  {
    ...
    if (m_ai.IsElite(newTarget)) {
      // Try to disengage
      if (DISENGAGE > 0 && m_bot.IsSpellReady(DISENGAGE) && m_ai.In_Reach(newTarget, DISENGAGE) &&
          m_ai.CastSpell(DISENGAGE, *newTarget) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
      // Increase dodge and parry chance
      if (DETERRENCE > 0 && m_bot.IsSpellReady(DETERRENCE) && !m_bot.HasAura(DETERRENCE, EFFECT_INDEX_0) &&
          m_ai.CastSpell(DETERRENCE, m_bot) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
      // Else feign death if low on health or attacked by a worldboss
      if (FEIGN_DEATH > 0 && (m_ai.GetHealthPercent() <= 20 || m_ai.IsElite(pTarget, true)) &&
          m_bot.IsSpellReady(FEIGN_DEATH) && !m_bot.HasAura(FEIGN_DEATH, EFFECT_INDEX_0) &&
          m_ai.CastSpell(FEIGN_DEATH, m_bot) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
    }
  }

  // Distance management: avoid to be in the dead zone where neither melee nor range can be used: keep distance whenever
  // possible If not in range: come closer Do not do it if passive or stay orders.
  if (!m_ai.In_Reach(pTarget, AUTO_SHOT) && !(m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_PASSIVE) &&
  ...
  if (m_ai.GetCombatStyle() == PlayerbotAI::COMBAT_RANGED) {
    // Debuff target
    if (HUNTERS_MARK > 0 && m_ai.In_Reach(pTarget, HUNTERS_MARK) && !pTarget->HasAura(HUNTERS_MARK, EFFECT_INDEX_0) &&
        m_ai.CastSpell(HUNTERS_MARK, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    // Buff self
    if (RAPID_FIRE > 0 && !m_bot.HasAura(RAPID_FIRE, EFFECT_INDEX_0) && m_bot.IsSpellReady(RAPID_FIRE) &&
        m_ai.CastSpell(RAPID_FIRE, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (ARCANE_SHOT > 0 && m_bot.IsSpellReady(ARCANE_SHOT) && m_ai.In_Range(pTarget, ARCANE_SHOT) &&
        m_ai.CastSpell(ARCANE_SHOT, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    ...
    if (pTarget->GetPower(POWER_MANA) > 0 && !m_ai.IsElite(pTarget, true)) {
      if (VIPER_STING > 0 && m_ai.In_Range(pTarget, VIPER_STING) && !pTarget->HasAura(VIPER_STING, EFFECT_INDEX_0) &&
          m_ai.CastSpell(VIPER_STING, *pTarget) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
    }
    // Serpent sting for everyone else
    else {
      if (SERPENT_STING > 0 && m_ai.In_Range(pTarget, SERPENT_STING) &&
          !pTarget->HasAura(SERPENT_STING, EFFECT_INDEX_0) && m_ai.CastSpell(SERPENT_STING, *pTarget) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
    }
    if (CONCUSSIVE_SHOT > 0 && m_ai.In_Range(pTarget, CONCUSSIVE_SHOT) &&
        !pTarget->HasAura(CONCUSSIVE_SHOT, EFFECT_INDEX_0) &&
        m_ai.CastSpell(CONCUSSIVE_SHOT, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (BLACK_ARROW > 0 && m_ai.In_Range(pTarget, BLACK_ARROW) && !pTarget->HasAura(BLACK_ARROW, EFFECT_INDEX_0) &&
        m_ai.CastSpell(BLACK_ARROW, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (AIMED_SHOT > 0 && m_bot.IsSpellReady(AIMED_SHOT) && m_ai.In_Range(pTarget, AIMED_SHOT) &&
        m_ai.CastSpell(AIMED_SHOT, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;

    //       if (MULTI_SHOT > 0 && m_bot.IsSpellReady(MULTI_SHOT) && m_ai.In_Range(pTarget,MULTI_SHOT) &&
    //       m_ai.GetAttackerCount() >= 3 && m_ai.CastSpell(MULTI_SHOT, *pTarget) == SPELL_CAST_OK)
    //            return RETURN_CONTINUE;
    //       if (VOLLEY > 0 && m_ai.In_Range(pTarget,VOLLEY) && m_ai.GetAttackerCount() >= 3 && m_ai.CastSpell(VOLLEY,
    //       *pTarget) == SPELL_CAST_OK)
    //           return RETURN_CONTINUE;
    ...
    if (MONGOOSE_BITE > 0 &&
        m_bot.RollMeleeOutcomeAgainst(pTarget, BASE_ATTACK, SPELL_SCHOOL_MASK_NORMAL) == MELEE_HIT_DODGE &&
        m_ai.CastSpell(MONGOOSE_BITE, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (RAPTOR_STRIKE > 0 && m_bot.IsSpellReady(RAPTOR_STRIKE) && m_ai.In_Reach(pTarget, RAPTOR_STRIKE) &&
        m_ai.CastSpell(RAPTOR_STRIKE, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (EXPLOSIVE_TRAP > 0 && !pTarget->HasAura(EXPLOSIVE_TRAP, EFFECT_INDEX_0) &&
        !pTarget->HasAura(IMMOLATION_TRAP, EFFECT_INDEX_0) && !pTarget->HasAura(FROST_TRAP, EFFECT_INDEX_0) &&
        m_ai.CastSpell(EXPLOSIVE_TRAP, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (WING_CLIP > 0 && m_bot.IsSpellReady(WING_CLIP) && m_ai.In_Reach(pTarget, WING_CLIP) &&
        !pTarget->HasAura(WING_CLIP, EFFECT_INDEX_0) && m_ai.CastSpell(WING_CLIP, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (IMMOLATION_TRAP > 0 && !pTarget->HasAura(IMMOLATION_TRAP, EFFECT_INDEX_0) &&
        !pTarget->HasAura(EXPLOSIVE_TRAP, EFFECT_INDEX_0) && !pTarget->HasAura(FROST_TRAP, EFFECT_INDEX_0) &&
        m_ai.CastSpell(IMMOLATION_TRAP, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (FROST_TRAP > 0 && !pTarget->HasAura(FROST_TRAP, EFFECT_INDEX_0) &&
        !pTarget->HasAura(IMMOLATION_TRAP, EFFECT_INDEX_0) && !pTarget->HasAura(EXPLOSIVE_TRAP, EFFECT_INDEX_0) &&
        m_ai.CastSpell(FROST_TRAP, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    else if (m_bot.getRace() == RACE_DRAENEI && m_ai.GetHealthPercent() < 25 &&
             !m_bot.HasAura(GIFT_OF_THE_NAARU, EFFECT_INDEX_0) &&
             m_ai.CastSpell(GIFT_OF_THE_NAARU, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    else if (pet && pet->IsAlive() && MISDIRECTION > 0 && pVictim == &m_bot &&
             !m_bot.HasAura(MISDIRECTION, EFFECT_INDEX_0) && m_ai.CastSpell(MISDIRECTION, *pet) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    //        if (m_bot.getRace() == RACE_TAUREN && !pTarget->HasAura(WAR_STOMP, EFFECT_INDEX_0) &&
    //        m_ai.CastSpell(WAR_STOMP, *pTarget) == SPELL_CAST_OK)
    //            return RETURN_CONTINUE;
    //        else if (m_bot.getRace() == RACE_DWARF && m_bot.HasAuraState(AURA_STATE_DEADLY_POISON) &&
    //        m_ai.CastSpell(STONEFORM, *m_bot) == SPELL_CAST_OK)
    //            return RETURN_CONTINUE;

    /*else if(FREEZING_TRAP > 0 && !pTarget->HasAura(FREEZING_TRAP, EFFECT_INDEX_0) && !pTarget->HasAura(ARCANE_TRAP,
       EFFECT_INDEX_0) && !pTarget->HasAura(EXPLOSIVE_TRAP, EFFECT_INDEX_0) && !pTarget->HasAura(BEAR_TRAP,
       EFFECT_INDEX_0) && !pTarget->HasAura(IMMOLATION_TRAP, EFFECT_INDEX_0) && !pTarget->HasAura(FROST_TRAP,
       EFFECT_INDEX_0) && m_ai.CastSpell(FREEZING_TRAP,*pTarget) == SPELL_CAST_OK) out << " > Freezing Trap"; // this
       can trap your bots too
    */
  }
...
void PlayerbotHunterAI::DoNonCombatActions() {
  ...
  if (PET_SUMMON > 0 && !m_petSummonFailed && m_bot.GetPetGuid()) {
    ...
    if (!pet) // Hunter has pet but it is not found: dismissed or dead and corpse was removed
    {
      ...
      if (PET_MEND > 0 && pet->IsAlive() && !pet->HasAura(PET_MEND, EFFECT_INDEX_0) &&
          m_ai.CastSpell(PET_MEND, m_bot) == SPELL_CAST_OK)
        m_ai.TellMaster("healing pet.");
          ...
          for (uint8 slot = 0; slot < pBag->GetBagSize(); ++slot) {
            ...
            if (pItem) {
              ...
              if (pet->HaveInDiet(pItemProto)) // is pItem in pets diet
              {
                ...
                uint32 count = 1; // number of items used
                int32 benefit =
                    pet->GetCurrentFoodBenefitLevel(pItemProto->ItemLevel) * 15; // nutritional value of food
                m_bot.DestroyItemCount(pItem, count, true);                      // remove item from inventory
                m_bot.CastCustomSpell(&m_bot, PET_FEED, &benefit, nullptr, nullptr,
  ...
  if (SHADOWMELD > 0 && !m_bot.isMovingOrTurning() && !m_bot.IsMounted() &&
