...
CombatManeuverReturns PlayerbotMageAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotMageAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  else if (m_ai.GetCombatStyle() != PlayerbotAI::COMBAT_MELEE && meleeReach &&
  ...
  if (newTarget && !PlayerbotAI::IsNeutralized(newTarget)) // Bot has aggro and the mob is not already crowd controled
  {
    if (newTarget->GetHealthPercent() > 25) {
      ...
      if (m_ai.IsElite(newTarget)) {
        // If the attacker is a beast or humanoid, let's the bot give it a form more suited to the low intellect of
        // something fool enough to attack a mage No need to check if target is a player as IsElite() returns false for
        // players
        Creature *creature = (Creature *)newTarget;
        if (creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_HUMANOID ||
            creature->GetCreatureInfo()->CreatureType == CREATURE_TYPE_BEAST) {
          if (POLYMORPH > 0 && !PlayerbotAI::IsImmuneToSchool(newTarget, SPELL_SCHOOL_MASK_ARCANE) &&
              CastSpell(POLYMORPH, newTarget))
            return RETURN_CONTINUE;
        }

        // Things are getting dire: cast Ice block
        if (ICE_BLOCK > 0 && m_bot.IsSpellReady(ICE_BLOCK) && m_ai.GetHealthPercent() < 30 &&
            !m_bot.HasAura(ICE_BLOCK, EFFECT_INDEX_0) && m_ai.CastSpell(ICE_BLOCK) == SPELL_CAST_OK)
          return RETURN_CONTINUE;

        // Cast Ice Barrier if health starts to goes low
        if (ICE_BARRIER > 0 && m_bot.IsSpellReady(ICE_BARRIER) && m_ai.GetHealthPercent() < 50 &&
            !m_bot.HasAura(ICE_BARRIER) && m_ai.SelfBuff(ICE_BARRIER) == SPELL_CAST_OK)
          return RETURN_CONTINUE;

        // Have threat, can't quickly lower it. 3 options remain: Stop attacking, lowlevel damage (wand), keep on
        // keeping on.
        return CastSpell(SHOOT, pTarget);
      } else // not elite
      {
        // Cast mana shield if no shield is already up
        if (MANA_SHIELD > 0 && m_ai.GetHealthPercent() < 70 && !m_bot.HasAura(MANA_SHIELD) &&
            !m_bot.HasAura(ICE_BARRIER) && m_ai.SelfBuff(MANA_SHIELD) == SPELL_CAST_OK)
          return RETURN_CONTINUE;
      }
    }
  }
  ...
  if (EVOCATION && m_ai.GetManaPercent() <= 10 && m_bot.IsSpellReady(EVOCATION) && !newTarget &&
      m_ai.SelfBuff(EVOCATION) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  ...
  if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FROST && FROST_WARD && m_bot.IsSpellReady(FROST_WARD) &&
      m_ai.SelfBuff(FROST_WARD) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
  if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FIRE && FIRE_WARD && m_bot.IsSpellReady(FIRE_WARD) &&
      m_ai.SelfBuff(FIRE_WARD) == SPELL_CAST_OK)
    return RETURN_CONTINUE;

  if (COUNTERSPELL > 0 && m_bot.IsSpellReady(COUNTERSPELL) && pTarget->IsNonMeleeSpellCasted(true) &&
      CastSpell(COUNTERSPELL, pTarget))
    return RETURN_CONTINUE;
  ...
  if (m_bot.HasAura(CLEARCASTING_1) && ARCANE_MISSILES > 0 &&
      !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_ARCANE) && CastSpell(ARCANE_MISSILES, pTarget)) {
  ...
  case MAGE_SPEC_FROST:
    if (ICY_VEINS > 0 && m_ai.In_Reach(&m_bot, ICY_VEINS) && !m_bot.HasAura(ICY_VEINS, EFFECT_INDEX_0) &&
        CastSpell(ICY_VEINS, &m_bot))
      return RETURN_CONTINUE;
    if (COLD_SNAP && m_bot.IsSpellReady(COLD_SNAP) && CheckFrostCooldowns() > 2 &&
        m_ai.SelfBuff(COLD_SNAP) == SPELL_CAST_OK) // Clear frost spell cooldowns if bot has more than 2 active
      return RETURN_CONTINUE;
    if (CONE_OF_COLD > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
        m_bot.IsSpellReady(CONE_OF_COLD) && meleeReach) {
    ...
    if (FROSTBOLT > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
        m_ai.In_Reach(pTarget, FROSTBOLT) && !pTarget->HasAura(FROSTBOLT, EFFECT_INDEX_0) &&
        CastSpell(FROSTBOLT, pTarget))
      return RETURN_CONTINUE;
    if (FROST_NOVA > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
        m_bot.IsSpellReady(FROST_NOVA) && meleeReach && !pTarget->HasAura(FROST_NOVA, EFFECT_INDEX_0) &&
        CastSpell(FROST_NOVA, pTarget))
      return RETURN_CONTINUE;
    if (ICE_LANCE > 0 && !m_ai.IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
        m_ai.In_Reach(pTarget, ICE_LANCE) && CastSpell(ICE_LANCE, pTarget))
      return RETURN_CONTINUE;
    ...
    if (FROSTBOLT > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
        m_ai.In_Reach(pTarget, FROSTBOLT))
      return CastSpell(FROSTBOLT, pTarget);
    /*
    if (BLIZZARD > 0 && !m_ai.IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) && m_ai.In_Reach(pTarget,BLIZZARD) &&
    m_ai.GetAttackerCount() >= 5 && CastSpell(BLIZZARD, pTarget))
    {
  ...
  case MAGE_SPEC_FIRE:
    if (COMBUSTION > 0 && m_ai.SelfBuff(COMBUSTION) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (BLAST_WAVE > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
        m_ai.GetAttackerCount() >= 3 && meleeReach && CastSpell(BLAST_WAVE, pTarget))
      return RETURN_CONTINUE;
    ...
    if (FIREBALL > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
        !pTarget->HasAura(FIREBALL, EFFECT_INDEX_1) && CastSpell(FIREBALL, pTarget))
      return RETURN_CONTINUE;
    // 3 stacks of Scorch and fireball DoT: use fire blast if available
    if (FIRE_BLAST > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
        m_bot.IsSpellReady(FIRE_BLAST) && CastSpell(FIRE_BLAST, pTarget))
      return RETURN_CONTINUE;
    if (DRAGONS_BREATH > 0 && !m_ai.IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && meleeReach &&
        CastSpell(DRAGONS_BREATH, pTarget))
      return RETURN_CONTINUE;
    ...
    if (FIREBALL > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
        m_ai.In_Reach(pTarget, FIREBALL))
      return CastSpell(FIREBALL, pTarget);
    /*
    if (FLAMESTRIKE > 0 && !m_ai.IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget,FLAMESTRIKE)
    && CastSpell(FLAMESTRIKE, pTarget)) return RETURN_CONTINUE;
    */
    break;

  case MAGE_SPEC_ARCANE:
    if (ARCANE_POWER > 0 && m_bot.IsSpellReady(ARCANE_POWER) && m_ai.IsElite(pTarget) &&
        m_ai.CastSpell(ARCANE_POWER) ==
            SPELL_CAST_OK) // Do not waste Arcane Power on normal NPCs as the bot is likely in a group
      return RETURN_CONTINUE;
    if (PRESENCE_OF_MIND > 0 && !m_bot.HasAura(PRESENCE_OF_MIND) && m_bot.IsSpellReady(PRESENCE_OF_MIND) &&
        m_ai.IsElite(pTarget) && m_ai.SelfBuff(PRESENCE_OF_MIND) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    ...
    if (PRESENCE_OF_MIND && m_bot.HasAura(PRESENCE_OF_MIND)) {
      ...
      if (PYROBLAST > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
          CastSpell(PYROBLAST, pTarget))
        return RETURN_CONTINUE;
      if (FIREBALL > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
          CastSpell(FIREBALL, pTarget))
        return RETURN_CONTINUE;
    }
    if (ARCANE_EXPLOSION > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_ARCANE) &&
        m_ai.GetAttackerCount() >= 3 && meleeReach && CastSpell(ARCANE_EXPLOSION, pTarget))
      return RETURN_CONTINUE;
    // Default arcane spec actions (yes, two fire spells)
    if (FIRE_BLAST > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
        m_bot.IsSpellReady(FIRE_BLAST) && CastSpell(FIRE_BLAST, pTarget))
      return RETURN_CONTINUE;
    if (SLOW > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_ARCANE) && m_ai.In_Reach(pTarget, SLOW) &&
        !pTarget->HasAura(SLOW, EFFECT_INDEX_0) && CastSpell(SLOW, pTarget))
      return RETURN_CONTINUE;
    if (ARCANE_BLAST > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_ARCANE) &&
        m_ai.In_Reach(pTarget, ARCANE_BLAST) && CastSpell(ARCANE_BLAST, pTarget))
      return RETURN_CONTINUE;
    ...
    if (ARCANE_MISSILES > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_ARCANE) &&
  ...
  if (FROSTBOLT > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
    ...
    return RETURN_CONTINUE;
  if (FIREBALL > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
      m_ai.In_Reach(pTarget, FIREBALL) && CastSpell(FIREBALL, pTarget)) // Very low levels
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotMageAI::DoNextCombatManeuverPVP(Unit *pTarget) {
  if (FIREBALL && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget, FIREBALL) &&
      m_ai.CastSpell(FIREBALL) == SPELL_CAST_OK)
    return RETURN_CONTINUE;
...
static const uint32 uPriorizedManaGemIds[5] = {MANA_EMERALD_DISPLAYID, MANA_RUBY_DISPLAYID, MANA_CITRINE_DISPLAYID,
                                               MANA_AGATE_DISPLAYID, MANA_JADE_DISPLAYID};
