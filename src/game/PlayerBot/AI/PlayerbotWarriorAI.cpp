...
class PlayerbotAI;
PlayerbotWarriorAI::PlayerbotWarriorAI(Player &master, Player &bot, PlayerbotAI &ai)
...
CombatManeuverReturns PlayerbotWarriorAI::DoFirstCombatManeuverPVE(Unit *pTarget) {
  ...
  if (BERSERKER_STANCE && spec == WARRIOR_SPEC_FURY && (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_ASSIST)) {
    ...
    if (!m_bot.HasAura(DEFENSIVE_STANCE, EFFECT_INDEX_0) && m_ai.CastSpell(DEFENSIVE_STANCE) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    else if (TAUNT > 0 && m_bot.HasAura(DEFENSIVE_STANCE, EFFECT_INDEX_0) &&
             m_ai.CastSpell(TAUNT, *pTarget) == SPELL_CAST_OK)
      return RETURN_FINISHED_FIRST_MOVES;
  }
...
CombatManeuverReturns PlayerbotWarriorAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  if (canBeSlowed &&
      (m_bot.HasAura(BATTLE_STANCE, EFFECT_INDEX_0) || m_bot.HasAura(BERSERKER_STANCE, EFFECT_INDEX_0)) &&
      pTarget->GetHealthPercent() < 20) {
    if (HAMSTRING > 0 && !pTarget->HasAura(HAMSTRING, EFFECT_INDEX_0) &&
        m_ai.CastSpell(HAMSTRING, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
  }
  ...
  if (spec == WARRIOR_SPEC_FURY && (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_ASSIST)) {
    ...
    if (DEATH_WISH > 0 && !m_bot.HasAura(DEATH_WISH, EFFECT_INDEX_0) && m_bot.IsSpellReady(DEATH_WISH) &&
        m_ai.CastSpell(DEATH_WISH, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    ...
    if (RETALIATION > 0 && pVictim == &m_bot && m_ai.GetAttackerCount() >= 2 && m_bot.IsSpellReady(RETALIATION) &&
        !m_bot.HasAura(RETALIATION, EFFECT_INDEX_0) && m_ai.CastSpell(RETALIATION, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    ...
    if (REND > 0 && !pTarget->HasAura(REND, EFFECT_INDEX_0) && m_ai.CastSpell(REND, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (MORTAL_STRIKE > 0 && m_bot.IsSpellReady(MORTAL_STRIKE) &&
        m_ai.CastSpell(MORTAL_STRIKE, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    ...
    if (!newTarget) {
      ...
      if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_TANK && TAUNT > 0 && m_bot.IsSpellReady(TAUNT) &&
          m_ai.CastSpell(TAUNT, *pTarget) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
    }

    // If tank is on the verge of dying but "I DON'T WANT TO DIE !!! :'-(("
    // TODO: should behaviour (or treshold) be different between elite and normal mobs? We don't want bots to burn such
    // precious cooldown needlessly
    if (m_bot.GetHealthPercent() < 10) {
      // Cast Last Stand first because it has lower cooldown
      if (LAST_STAND > 0 && !m_bot.HasAura(LAST_STAND, EFFECT_INDEX_0) &&
          m_ai.CastSpell(LAST_STAND, m_bot) == SPELL_CAST_OK) {
        m_ai.TellMaster("I'm using LAST STAND");
        return RETURN_CONTINUE;
      }

      // Cast Shield Wall only if Last Stand is on cooldown and not active
      if (SHIELD_WALL > 0 && (!m_bot.IsSpellReady(LAST_STAND) || LAST_STAND == 0) &&
          !m_bot.HasAura(LAST_STAND, EFFECT_INDEX_0) && !m_bot.HasAura(SHIELD_WALL, EFFECT_INDEX_0) &&
          m_ai.CastSpell(SHIELD_WALL, m_bot) == SPELL_CAST_OK) {
    ...
    if (pTarget->IsNonMeleeSpellCasted(true)) {
      ...
      if (PUMMEL > 0 && m_ai.CastSpell(PUMMEL, *pTarget) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
      if (SPELL_REFLECTION > 0 && !m_bot.HasAura(SPELL_REFLECTION, EFFECT_INDEX_0) &&
          m_ai.CastSpell(SPELL_REFLECTION, m_bot) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
    }
    ...
    if (REVENGE > 0 && m_bot.IsSpellReady(REVENGE)) {
      uint8 base = pTarget->RollMeleeOutcomeAgainst(&m_bot, BASE_ATTACK, SPELL_SCHOOL_MASK_NORMAL);
      uint8 off = pTarget->RollMeleeOutcomeAgainst(&m_bot, OFF_ATTACK, SPELL_SCHOOL_MASK_NORMAL);
      if (base == MELEE_HIT_PARRY || base == MELEE_HIT_DODGE || base == MELEE_HIT_BLOCK || off == MELEE_HIT_PARRY ||
          off == MELEE_HIT_DODGE || off == MELEE_HIT_BLOCK)
    ...
    if (SUNDER_ARMOR > 0) {
      if (!pTarget->HasAura(SUNDER_ARMOR) &&
          m_ai.CastSpell(SUNDER_ARMOR, *pTarget) == SPELL_CAST_OK) // no stacks: cast it
        return RETURN_CONTINUE;
      else {
    ...
    if (DEMORALIZING_SHOUT > 0 && !pTarget->HasAura(DEMORALIZING_SHOUT, EFFECT_INDEX_0) &&
        m_ai.CastSpell(DEMORALIZING_SHOUT, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    ...
    if (SHIELD_BLOCK > 0 && m_ai.IsElite(pTarget) && !m_bot.HasAura(SHIELD_BLOCK, EFFECT_INDEX_0) &&
        m_ai.CastSpell(SHIELD_BLOCK, m_bot) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
    if (CONCUSSION_BLOW > 0 && m_bot.IsSpellReady(CONCUSSION_BLOW) &&
        m_ai.CastSpell(CONCUSSION_BLOW, *pTarget) == SPELL_CAST_OK)
      return RETURN_CONTINUE;
  ...
  /*    case WarriorBattle:
          if (SWEEPING_STRIKES > 0 && m_ai.GetAttackerCount() >= 2 && !m_bot.HasAura(SWEEPING_STRIKES, EFFECT_INDEX_0)
     && m_ai.CastSpell(SWEEPING_STRIKES, *m_bot) == SPELL_CAST_OK) return RETURN_CONTINUE; if (INTIMIDATING_SHOUT > 0 &&
     m_ai.GetAttackerCount() > 5 && m_ai.CastSpell(INTIMIDATING_SHOUT, *pTarget) == SPELL_CAST_OK) return
     RETURN_CONTINUE; if (CHALLENGING_SHOUT > 0 && pVictim != m_bot && m_ai.GetHealthPercent() > 25 &&
     !pTarget->HasAura(MOCKING_BLOW, EFFECT_INDEX_0) && !pTarget->HasAura(CHALLENGING_SHOUT, EFFECT_INDEX_0) &&
     m_ai.CastSpell(CHALLENGING_SHOUT, *pTarget) == SPELL_CAST_OK) return RETURN_CONTINUE; if (CLEAVE > 0 &&
     m_ai.CastSpell(CLEAVE, *pTarget) == SPELL_CAST_OK) return RETURN_CONTINUE; if (PIERCING_HOWL > 0 && &&
     m_ai.GetAttackerCount() >= 3 && !pTarget->HasAura(WAR_STOMP, EFFECT_INDEX_0) && !pTarget->HasAura(PIERCING_HOWL,
     EFFECT_INDEX_0) && !pTarget->HasAura(SHOCKWAVE, EFFECT_INDEX_0) && !pTarget->HasAura(CONCUSSION_BLOW,
     EFFECT_INDEX_0) && m_ai.CastSpell(PIERCING_HOWL, *pTarget) == SPELL_CAST_OK) return RETURN_CONTINUE; if
     (MOCKING_BLOW > 0 && pVictim != m_bot && m_ai.GetHealthPercent() > 25 && !pTarget->HasAura(MOCKING_BLOW,
     EFFECT_INDEX_0) && !pTarget->HasAura(CHALLENGING_SHOUT, EFFECT_INDEX_0) && m_ai.CastSpell(MOCKING_BLOW, *pTarget)
     == SPELL_CAST_OK) return RETURN_CONTINUE; if (m_bot.getRace() == RACE_TAUREN && !pTarget->HasAura(WAR_STOMP,
     EFFECT_INDEX_0) && !pTarget->HasAura(PIERCING_HOWL, EFFECT_INDEX_0) && !pTarget->HasAura(SHOCKWAVE, EFFECT_INDEX_0)
     && !pTarget->HasAura(CONCUSSION_BLOW, EFFECT_INDEX_0) && m_ai.CastSpell(WAR_STOMP, *pTarget) == SPELL_CAST_OK)
              return RETURN_CONTINUE;
          if (m_bot.getRace() == RACE_UNDEAD && m_bot.HasAuraType(SPELL_AURA_MOD_FEAR) ||
     m_bot.HasAuraType(SPELL_AURA_MOD_CHARM) && m_ai.CastSpell(WILL_OF_THE_FORSAKEN, *m_bot) == SPELL_CAST_OK) return
     RETURN_CONTINUE; if (m_bot.getRace() == RACE_DWARF && m_bot.HasAuraState(AURA_STATE_DEADLY_POISON) &&
     m_ai.CastSpell(STONEFORM, *m_bot) == SPELL_CAST_OK) return RETURN_CONTINUE; if (m_bot.getRace() == RACE_GNOME &&
     m_bot.hasUnitState(UNIT_STAT_STUNNED) || m_bot.HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED) &&
     m_ai.CastSpell(ESCAPE_ARTIST, *m_bot) == SPELL_CAST_OK) return RETURN_CONTINUE; if (m_bot.getRace() == RACE_ORC &&
     !m_bot.HasAura(BLOOD_FURY, EFFECT_INDEX_0) && m_ai.CastSpell(BLOOD_FURY, *m_bot) == SPELL_CAST_OK) return
     RETURN_CONTINUE; if (m_bot.getRace() == RACE_TROLL && !m_bot.HasAura(BERSERKING, EFFECT_INDEX_0) &&
     m_ai.CastSpell(BERSERKING, *m_bot) == SPELL_CAST_OK) return RETURN_CONTINUE; if (m_bot.getRace() == RACE_DRAENEI &&
     m_ai.GetHealthPercent() < 25 && !m_bot.HasAura(GIFT_OF_THE_NAARU, EFFECT_INDEX_0) &&
     m_ai.CastSpell(GIFT_OF_THE_NAARU, *m_bot) == SPELL_CAST_OK) return RETURN_CONTINUE; break;

      case WarriorDefensive:
          if (DISARM > 0 && !pTarget->HasAura(DISARM, EFFECT_INDEX_0) && m_ai.CastSpell(DISARM, *pTarget) ==
     SPELL_CAST_OK) return RETURN_CONTINUE; if (SHIELD_BLOCK > 0 && !m_bot.HasAura(SHIELD_BLOCK, EFFECT_INDEX_0) &&
     m_ai.CastSpell(SHIELD_BLOCK, *m_bot) == SPELL_CAST_OK) return RETURN_CONTINUE; if (SHIELD_WALL > 0 &&
     !m_bot.HasAura(SHIELD_WALL, EFFECT_INDEX_0) && m_ai.CastSpell(SHIELD_WALL, *m_bot) == SPELL_CAST_OK) return
     RETURN_CONTINUE; break;*/
...
void PlayerbotWarriorAI::DoNonCombatActions() {
  ...
  if (weapon &&
      (weapon->GetProto()->InventoryType == INVTYPE_WEAPONOFFHAND ||
       weapon->GetProto()->InventoryType == INVTYPE_WEAPONMAINHAND) &&
...
bool PlayerbotWarriorAI::CanPull() { return m_bot.GetUInt32Value(PLAYER_AMMO_ID) != 0; }
