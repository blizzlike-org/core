...
CombatManeuverReturns PlayerbotDruidAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }

CombatManeuverReturns PlayerbotDruidAI::DoFirstCombatManeuverPVP(Unit * /*pTarget*/) { return RETURN_NO_ACTION_OK; }
...
CombatManeuverReturns PlayerbotDruidAI::DoNextCombatManeuverPVE(Unit *pTarget) {
  ...
  if (newTarget && !(m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_TANK) &&
      !PlayerbotAI::IsNeutralized(newTarget)) // TODO: && party has a tank
  {
  ...
  case DRUID_SPEC_BALANCE:
    ...
    /*if (BASH > 0 && !pTarget->HasAura(BASH, EFFECT_INDEX_0) && DruidSpellCombat < 5 && CastSpell(BASH, pTarget))
        return RETURN_CONTINUE;
    if (CHALLENGING_ROAR > 0 && pVictim != m_bot && !pTarget->HasAura(CHALLENGING_ROAR, EFFECT_INDEX_0) &&
    !pTarget->HasAura(GROWL, EFFECT_INDEX_0) && CastSpell(CHALLENGING_ROAR, pTarget)) return RETURN_CONTINUE; if (ROOTS
    > 0 && !pTarget->HasAura(ROOTS, EFFECT_INDEX_0) && CastSpell(ROOTS, pTarget)) return RETURN_CONTINUE; if (HURRICANE
    > 0 && ai->In_Reach(target,HURRICANE) && m_ai.GetAttackerCount() >= 5 && CastSpell(HURRICANE, pTarget))
    {
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverBear(Unit *pTarget) {
  if (!m_bot.HasAura((DIRE_BEAR_FORM > 0 ? DIRE_BEAR_FORM : BEAR_FORM)))
  ...
  if (FAERIE_FIRE_FERAL > 0 && m_ai.In_Reach(pTarget, FAERIE_FIRE_FERAL) &&
      !pTarget->HasAura(FAERIE_FIRE_FERAL, EFFECT_INDEX_0))
  ...
  if (DEMORALIZING_ROAR > 0 && !pTarget->HasAura(DEMORALIZING_ROAR, EFFECT_INDEX_0) &&
      CastSpell(DEMORALIZING_ROAR, pTarget))
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverCat(Unit *pTarget) {
  ...
  if (FAERIE_FIRE_FERAL > 0 && m_ai.In_Reach(pTarget, FAERIE_FIRE_FERAL) &&
      !pTarget->HasAura(FAERIE_FIRE_FERAL, EFFECT_INDEX_0) && CastSpell(FAERIE_FIRE_FERAL, pTarget))
    return RETURN_CONTINUE;

  if (TIGERS_FURY > 0 && m_bot.IsSpellReady(TIGERS_FURY) && !m_bot.HasAura(TIGERS_FURY, EFFECT_INDEX_0) &&
      CastSpell(TIGERS_FURY))
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverSpellDPS(Unit *pTarget) {
  ...
  if (FAERIE_FIRE > 0 && m_ai.In_Reach(pTarget, FAERIE_FIRE) && !pTarget->HasAura(FAERIE_FIRE, EFFECT_INDEX_0) &&
      CastSpell(FAERIE_FIRE, pTarget))
    return RETURN_CONTINUE;

  if (INSECT_SWARM > 0 && m_ai.In_Reach(pTarget, INSECT_SWARM) && !pTarget->HasAura(INSECT_SWARM, EFFECT_INDEX_0) &&
      CastSpell(INSECT_SWARM, pTarget))
    return RETURN_CONTINUE;
  ...
  if (MOONFIRE > 0 && m_ai.In_Reach(pTarget, MOONFIRE) && !pTarget->HasAura(MOONFIRE, EFFECT_INDEX_0) &&
      CastSpell(MOONFIRE, pTarget))
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotDruidAI::HealPlayer(Player *target) {
  ...
  if (((GetTargetJob(target) == JOB_TANK || GetTargetJob(target) == JOB_MAIN_TANK) && hp < 10) ||
    ...
    if (HEALING_TOUCH > 0 && m_bot.HasAura(NATURES_SWIFTNESS, EFFECT_INDEX_0) && m_ai.In_Reach(target, HEALING_TOUCH) &&
        CastSpell(HEALING_TOUCH, target))
      return RETURN_CONTINUE;
    ...
    if (SWIFTMEND > 0 && m_bot.IsSpellReady(SWIFTMEND) && m_ai.In_Reach(target, SWIFTMEND) &&
        (target->HasAura(REJUVENATION) || target->HasAura(REGROWTH)) && CastSpell(SWIFTMEND, target))
      return RETURN_CONTINUE;
  }
  ...
  if (((GetTargetJob(target) == JOB_TANK || GetTargetJob(target) == JOB_MAIN_TANK) && hp < 15) ||
    ...
    if (REGROWTH > 0 && m_ai.In_Reach(target, REGROWTH) && !target->HasAura(REGROWTH) && CastSpell(REGROWTH, target))
      return RETURN_CONTINUE;
    if (REJUVENATION > 0 && m_ai.In_Reach(target, REJUVENATION) && target->HasAura(REGROWTH) &&
        !target->HasAura(REJUVENATION) && CastSpell(REJUVENATION, target))
      return RETURN_CONTINUE;
    if (SWIFTMEND > 0 && m_bot.IsSpellReady(SWIFTMEND) && m_ai.In_Reach(target, SWIFTMEND) &&
        (target->HasAura(REJUVENATION) || target->HasAura(REGROWTH)) && CastSpell(SWIFTMEND, target))
      return RETURN_CONTINUE;
  ...
  if (hp < 65 && LIFEBLOOM > 0 && m_ai.In_Reach(target, LIFEBLOOM) && !target->HasAura(LIFEBLOOM) &&
      CastSpell(LIFEBLOOM, target))
    return RETURN_CONTINUE;

  if (hp < 80 && REJUVENATION > 0 && m_ai.In_Reach(target, REJUVENATION) && !target->HasAura(REJUVENATION) &&
      CastSpell(REJUVENATION, target))
    return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotDruidAI::ResurrectPlayer(Player *target) {
  ...
  if (REBIRTH > 0 && m_ai.In_Reach(target, REBIRTH) && m_bot.IsSpellReady(REBIRTH) &&
      m_ai.CastSpell(REBIRTH, *target) == SPELL_CAST_OK) {
...
void PlayerbotDruidAI::DoNonCombatActions() {
  ...
  if (NeedGroupBuff(GIFT_OF_THE_WILD, MARK_OF_THE_WILD) && m_ai.HasSpellReagents(GIFT_OF_THE_WILD)) {
    ...
    return;
  if (Buff(&PlayerbotDruidAI::BuffHelper, THORNS, (m_bot.GetGroup() ? JOB_TANK | JOB_MAIN_TANK : JOB_ALL)) &
      RETURN_CONTINUE)
    return;
  ...
  if (INNERVATE > 0 && m_ai.In_Reach(&m_bot, INNERVATE) && !m_bot.HasAura(INNERVATE) && m_ai.GetManaPercent() <= 20 &&
...
bool PlayerbotDruidAI::BuffHelper(PlayerbotAI *ai, uint32 spellId, Unit *target) {
  ...
  if (pet && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) &&
      ai->Buff(spellId, pet, &(PlayerbotDruidAI::GoBuffForm)) == SPELL_CAST_OK)
    return true;
...
bool PlayerbotDruidAI::CanPull() { return BEAR_FORM > 0 && FAERIE_FIRE_FERAL; }

// Match up with "CanPull()" above
bool PlayerbotDruidAI::Pull() { return BEAR_FORM > 0 && (CastSpell(FAERIE_FIRE_FERAL) & RETURN_CONTINUE); }

bool PlayerbotDruidAI::CastHoTOnTank() {
  if ((PlayerbotAI::ORDERS_HEAL & m_ai.GetCombatOrder()) == 0)
    return false;
