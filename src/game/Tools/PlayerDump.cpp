...
void PlayerDumpWriter::DumpTableContent(std::string &dump, uint32 guid, char const *tableFrom, char const *tableTo,
                                        DumpTableType type) {
...
std::string PlayerDumpWriter::GetDump(uint32 guid) {
  ...
  dump += "IMPORTANT NOTE: This sql queries not created for apply directly, use '.pdump load' command in console or "
          "client chat instead.\n";
  ...
  if (result) {
    ...
    if (!reqName.empty()) {
      ...
      sLog.outError(
          "Table 'character_db_version' not have revision guard field, revision guard query not added to pdump.");
