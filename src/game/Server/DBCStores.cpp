...
static bool LoadDBC_assert_print(uint32 fsize, uint32 rsize, const std::string &filename) {
  sLog.outError("Size of '%s' setted by format string (%u) not equal size of C++ structure (%u).", filename.c_str(),
                fsize, rsize);
...
template <class T>
inline void LoadDBC(uint32 &availableDbcLocales, BarGoLink &bar, StoreProblemList &errlist, DBCStorage<T> &storage,
                    const std::string &dbc_path, const std::string &filename) {
  // compatibility format and C++ structure sizes
  MANGOS_ASSERT(DBCFileLoader::GetFormatRecordSize(storage.GetFormat()) == sizeof(T) ||
  ...
  if (storage.Load(dbc_filename.c_str())) {
    ...
    if (f) {
      char buf[100];
      snprintf(buf, 100, " (exist, but have %u fields instead " SIZEFMTD ") Wrong client version DBC file?",
...
void LoadDBCStores(const std::string &dataPath) {
  ...
  LoadDBC(availableDbcLocales, bar, bad_dbc_files, sCharacterFacialHairStylesStore, dbcPath,
  ...
  for (uint32 i = 0; i < sCharSectionsStore.GetNumRows(); ++i)
    if (CharSectionsEntry const *entry = sCharSectionsStore.LookupEntry(i))
      if (entry->Race && ((1 << (entry->Race - 1)) & RACEMASK_ALL_PLAYABLE) != 0) // ignore Nonplayable races
        sCharSectionMap.emplace(uint8(entry->BaseSection) | (uint8(entry->Gender) << 8) | (uint8(entry->Race) << 16),
                                entry);
  ...
  LoadDBC(availableDbcLocales, bar, bad_dbc_files, sCreatureDisplayInfoExtraStore, dbcPath,
  ...
  LoadDBC(availableDbcLocales, bar, bad_dbc_files, sGtChanceToMeleeCritBaseStore, dbcPath,
  ...
  LoadDBC(availableDbcLocales, bar, bad_dbc_files, sGtChanceToSpellCritBaseStore, dbcPath,
  ...
  // LoadDBC(availableDbcLocales,bar,bad_dbc_files,sGtOCTRegenMPStore,        dbcPath,"gtOCTRegenMP.dbc");       -- not
  // used currently
  ...
  // LoadDBC(availableDbcLocales,bar,bad_dbc_files,sItemDisplayInfoStore,     dbcPath,"ItemDisplayInfo.dbc");     -- not
  // used currently LoadDBC(availableDbcLocales,bar,bad_dbc_files,sItemCondExtCostsStore,
  ...
  for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j) {
    ...
    if (spellInfo &&
        (spellInfo->Attributes &
         (SPELL_ATTR_ABILITY | SPELL_ATTR_PASSIVE | SPELL_ATTR_HIDDEN_CLIENTSIDE | SPELL_ATTR_HIDE_IN_COMBAT_LOG)) ==
            (SPELL_ATTR_ABILITY | SPELL_ATTR_PASSIVE | SPELL_ATTR_HIDDEN_CLIENTSIDE | SPELL_ATTR_HIDE_IN_COMBAT_LOG)) {
  ...
  //                sLog.outErrorDb("Spell ID %u found in spell item enchant %u does not exist.",
  //                enchantEntry->spellid[k], i);
  //    }
  //}
  LoadDBC(availableDbcLocales, bar, bad_dbc_files, sSpellItemEnchantmentConditionStore, dbcPath,
  ...
  if (bad_dbc_files.size() >= DBCFilesCount) {
    sLog.outError("\nIncorrect DataDir value in mangosd.conf or ALL required *.dbc files (%d) not found by path: %sdbc",
  ...
  if (!sSkillLineAbilityStore.LookupEntry(17514) || !sMapStore.LookupEntry(598) ||
      !sGemPropertiesStore.LookupEntry(1127) || !sItemExtendedCostStore.LookupEntry(2425) ||
      !sCharTitlesStore.LookupEntry(71) || !sAreaStore.LookupEntry(1768)) {
    sLog.outError("\nYou have _outdated_ DBC files. Please re-extract DBC files for one from client build: %s",
...
CharSectionsEntry const *GetCharSectionEntry(uint8 race, CharSectionType genType, uint8 gender, uint8 type,
                                             uint8 color) {
  ...
  for (auto itr = eqr.first; itr != eqr.second; ++itr) {
    if (itr->second->VariationIndex == type && itr->second->ColorIndex == color &&
        !itr->second->HasFlag(SECTION_FLAG_UNAVAILABLE))
      return itr->second;
  }
...
bool MapCoordinateVsZoneCheck(float x, float y, uint32 mapid, uint32 zone) {
  ...
  if (WmaEntry && mapid == WmaEntry->map_id && x >= WmaEntry->x2 && x <= WmaEntry->x1 && y >= WmaEntry->y2 &&
      y <= WmaEntry->y1)
    return true;
  return false;
}
...
bool IsPointInAreaTriggerZone(AreaTriggerEntry const *atEntry, uint32 mapid, float x, float y, float z, float delta) {
  ...
  if (atEntry->radius > 0) {
    ...
    float dist2 =
        (x - atEntry->x) * (x - atEntry->x) + (y - atEntry->y) * (y - atEntry->y) + (z - atEntry->z) * (z - atEntry->z);
...
uint32 GetCreatureModelRace(uint32 model_id) {
  ...
  CreatureDisplayInfoExtraEntry const *extraEntry =
      sCreatureDisplayInfoExtraStore.LookupEntry(displayEntry->ExtendedDisplayInfoID);
