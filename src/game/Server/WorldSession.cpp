...
WorldSession::WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec, uint8 expansion, time_t mute_time,
                           LocaleConstant locale)
    : LookingForGroup_auto_join(false), LookingForGroup_auto_add(true), m_muteTime(mute_time), _player(nullptr),
      m_Socket(sock ? sock->shared<WorldSocket>() : nullptr), m_requestSocket(nullptr),
      m_sessionState(WORLD_SESSION_STATE_CREATED), _security(sec), _accountId(id), m_expansion(expansion),
      m_orderCounter(0), _logoutTime(0), m_playerSave(true), m_inQueue(false), m_playerLoading(false),
      m_playerLogout(false), m_playerRecentlyLogout(false), m_sessionDbcLocale(sWorld.GetAvailableDbcLocale(locale)),
      m_sessionDbLocaleIndex(sObjectMgr.GetStorageLocaleIndexFor(locale)), m_latency(0),
      m_tutorialState(TUTORIALDATA_UNCHANGED), m_timeSyncClockDeltaQueue(6), m_timeSyncClockDelta(0),
      m_pendingTimeSyncRequests(), m_timeSyncNextCounter(0), m_timeSyncTimer(0) {}
...
void WorldSession::SendPacket(WorldPacket const &packet, bool forcedSend /*= false*/) const {
  ...
  if ((cur_time - lastTime) < 60) {
    ...
    DETAIL_LOG("Send all time packets count: " UI64FMTD " bytes: " UI64FMTD
               ...
               sendPacketCount, sendPacketBytes, float(sendPacketCount) / fullTime, float(sendPacketBytes) / fullTime,
               uint32(fullTime));
    DETAIL_LOG("Send last min packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f",
               sendLastPacketCount, sendLastPacketBytes, float(sendLastPacketCount) / minTime,
...
void WorldSession::LogUnexpectedOpcode(WorldPacket const &packet, const char *reason) const {
  sLog.outError("SESSION: received unexpected opcode %s (0x%.4X) %s", packet.GetOpcodeName(), packet.GetOpcode(),
                reason);
}
...
void WorldSession::LogUnprocessedTail(WorldPacket const &packet) const {
  sLog.outError("SESSION: opcode %s (0x%.4X) have unprocessed tail data (read stop at " SIZEFMTD " from " SIZEFMTD ")",
                packet.GetOpcodeName(), packet.GetOpcode(), packet.rpos(), packet.wpos());
}

void WorldSession::ProcessByteBufferException(WorldPacket const &packet) {
  sLog.outError("WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, "
                "accountid=%i.",
  ...
  if (sWorld.getConfig(CONFIG_BOOL_KICK_PLAYER_ON_BAD_PACKET)) {
    DETAIL_LOG("Disconnecting session [account id %u / address %s] for badly formatted packet.", GetAccountId(),
               GetRemoteAddress().c_str());
    ObjectGuid guid = _player->GetObjectGuid();
    GetMessager().AddMessage([guid](WorldSession *world) -> void { ObjectAccessor::KickPlayer(guid); });
  }
}
  ...
  while (m_Socket && !m_Socket->IsClosed() && !recvQueueCopy.empty()) {
    ...
    try {
      ...
      case STATUS_LOGGEDIN:
        if (!_player) {
          // skip STATUS_LOGGEDIN opcode unexpected errors if player logout sometime ago - this can be network lag
          // delayed packets
      ...
      case STATUS_AUTHED:
        ...
        break;
      case STATUS_NEVER:
        sLog.outError("SESSION: received not allowed opcode %s (0x%.4X)", packet->GetOpcodeName(), packet->GetOpcode());
        break;
      case STATUS_UNHANDLED:
        DEBUG_LOG("SESSION: received not handled opcode %s (0x%.4X)", packet->GetOpcodeName(), packet->GetOpcode());
        break;
      default:
        sLog.outError("SESSION: received wrong-status-req opcode %s (0x%.4X)", packet->GetOpcodeName(),
                      packet->GetOpcode());
        break;
      }
...
void WorldSession::LogoutPlayer() {
  ...
  if (_player) {
    ...
    sLog.outChar("Account: %d (IP: %s) Logout Character:[%s] (guid: %u)", GetAccountId(), GetRemoteAddress().c_str(),
    ...
    // no point resetting online in character table here as Player::SaveToDB() will set it to 1 since player has not
    // been removed from world at this stage No SQL injection as AccountID is uint32
    ...
    stmt.PExecute(guid);
#else
    ///- Since each account can only have one online character at any given time, ensure all characters for active
    /// account are marked as offline
...
void WorldSession::Handle_EarlyProccess(WorldPacket &recvPacket) {
  sLog.outError("SESSION: received opcode %s (0x%.4X) that must be processed in WorldSocket::OnRead",
                recvPacket.GetOpcodeName(), recvPacket.GetOpcode());
}

void WorldSession::Handle_ServerSide(WorldPacket &recvPacket) {
  sLog.outError("SESSION: received server-side opcode %s (0x%.4X)", recvPacket.GetOpcodeName(), recvPacket.GetOpcode());
}

void WorldSession::Handle_Deprecated(WorldPacket &recvPacket) {
  sLog.outError("SESSION: received deprecated opcode %s (0x%.4X)", recvPacket.GetOpcodeName(), recvPacket.GetOpcode());
}

void WorldSession::SendAuthWaitQue(uint32 position) const {
  // these SMSG_AUTH_RESPONSE structure should be used only after at least one WorldSession::SendAuthOk or
  // WorldSession::SendQueued was sent
  if (position == 0) {
...
void WorldSession::SetAccountData(AccountDataType type, time_t time_, const std::string &data) {
  if ((1 << type) & GLOBAL_CACHE_MASK) {
    ...
    SqlStatement stmt =
        CharacterDatabase.CreateStatement(delId, "DELETE FROM character_account_data WHERE guid=? AND type=?");
...
void WorldSession::LoadTutorialsData() {
  ...
  QueryResult *result = CharacterDatabase.PQuery(
      "SELECT tut0,tut1,tut2,tut3,tut4,tut5,tut6,tut7 FROM character_tutorial WHERE account = '%u'", GetAccountId());
...
void WorldSession::SaveTutorialsData() {
  ...
  case TUTORIALDATA_CHANGED: {
    SqlStatement stmt =
        CharacterDatabase.CreateStatement(updTutorial, "UPDATE character_tutorial SET tut0=?, tut1=?, tut2=?, tut3=?, "
  ...
  case TUTORIALDATA_NEW: {
    SqlStatement stmt = CharacterDatabase.CreateStatement(
        insTutorial, "INSERT INTO character_tutorial (account,tut0,tut1,tut2,tut3,tut4,tut5,tut6,tut7) VALUES (?, ?, "
                     "?, ?, ?, ?, ?, ?, ?)");
