...
enum TutorialDataState { TUTORIALDATA_UNCHANGED = 0, TUTORIALDATA_CHANGED = 1, TUTORIALDATA_NEW = 2 };
...
public:
  WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec, uint8 expansion, time_t mute_time,
               LocaleConstant locale);
  ~WorldSession();

  // Set this session have no attached socket but keep it alive for short period of time to permit a possible
  // reconnection
  void SetOffline();
  void SetOnline();
  ...
  bool ShouldDisconnect(time_t currTime) { return (_logoutTime > 0 && currTime >= _logoutTime + 60); }
  ...
  void SendAuctionCommandResult(AuctionEntry *auc, AuctionAction Action, AuctionError ErrorCode,
                                InventoryResult invError = EQUIP_ERR_OK) const;
  ...
  void BuildListAuctionItems(std::vector<AuctionEntry *> const &auctions, WorldPacket &data,
                             std::wstring const &searchedname, uint32 listfrom, uint32 levelmin, uint32 levelmax,
                             uint32 usable, uint32 inventoryType, uint32 itemClass, uint32 itemSubClass, uint32 quality,
                             uint32 &count, uint32 &totalcount, bool isFull) const;
  ...
  void SendArenaTeamCommandResult(uint32 team_action, const std::string &team, const std::string &player,
                                  uint32 error_id) const;
...
private:
  ...
  boost::circular_buffer<std::pair<int64, uint32>>
      m_timeSyncClockDeltaQueue; // first member: clockDelta. Second member: latency of the packet exchange that was
                                 // used to compute that clockDelta.
  int64 m_timeSyncClockDelta;
  void ComputeNewClockDelta();

  std::map<uint32, uint32>
      m_pendingTimeSyncRequests; // key: counter. value: server time when packet with that counter was sent.
  ...
  std::deque<CharacterNameQueryResponse>
      m_offlineNameResponses; // for responses to name queries made when not logged in
