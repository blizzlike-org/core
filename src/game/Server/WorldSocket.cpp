...
std::deque<uint32> WorldSocket::GetOpcodeHistory() { return m_opcodeHistory; }

WorldSocket::WorldSocket(boost::asio::io_service &service, std::function<void(Socket *)> closeHandler)
    : Socket(service, std::move(closeHandler)), m_lastPingTime(std::chrono::system_clock::time_point::min()),
      m_overSpeedPings(0), m_existingHeader(), m_useExistingHeader(false), m_session(nullptr), m_seed(urand()) {}
...
void WorldSocket::SendPacket(const WorldPacket &pct, bool immediate) {
  ...
  // encrypt thread unsafe due to being executed from map contexts frequently - TODO: move to post service context in
  // future
  ...
  if (pct.size() > 0)
    Write(reinterpret_cast<const char *>(&header), sizeof(header), reinterpret_cast<const char *>(pct.contents()),
          pct.size());
  else
...
bool WorldSocket::ProcessIncomingData() {
  ...
  if ((header.size < 4) || (header.size > 0x2800) || (header.cmd >= NUM_MSG_TYPES)) {
    sLog.outError("WorldSocket::ProcessIncomingData: client sent malformed packet size = %u , cmd = %u", header.size,
                  header.cmd);
  ...
  try {
    ...
    sLog.outError("WorldSocket::ProcessIncomingData ByteBufferException occured while parsing an instant handled "
...
bool WorldSocket::HandleAuthSession(WorldPacket &recvPacket) {
  ...
  DEBUG_LOG("WorldSocket::HandleAuthSession: client build %u, account %s, clientseed %X", ClientBuild, account.c_str(),
            clientSeed);
  ...
  QueryResult *banresult = LoginDatabase.PQuery(
      "SELECT 1 FROM account_banned WHERE account_id = %u AND active = 1 AND (expires_at > UNIX_TIMESTAMP() OR "
  ...
  DEBUG_LOG("WorldSocket::HandleAuthSession: Client '%s' authenticated successfully from %s.", account.c_str(),
            address.c_str());
  ...
  SqlStatement stmt = LoginDatabase.CreateStatement(
      updAccount, "INSERT INTO account_logons(accountId,ip,loginTime,loginSource) VALUES(?,?,NOW(),?)");
