...
struct CharStartOutfitEntry {
  // uint32 Id;                                           // 0        m_ID
  uint32 RaceClassGender; // 1        m_raceID m_classID m_sexID m_outfitID (UNIT_FIELD_BYTES_0 & 0x00FFFFFF) comparable
                          // (0 byte = race, 1 byte = class, 2 byte = gender)
  ...
  // uint32 Unknown1;                                     // 38, unique values (index-like with gaps ordered in other
  // way as ids) uint32 Unknown2;                                     // 39 uint32 Unknown3; // 40
};
...
struct CreatureDisplayInfoExtraEntry {
  ...
  // uint32    Equipment[11];                             // 8-18     m_NPCItemDisplay equipped static items
  // EQUIPMENT_SLOT_HEAD..EQUIPMENT_SLOT_HANDS, client show its by self uint32    CanEquip; // 19       m_flags 0..1 Can
  // equip additional things when used for players char*                                                // 20 m_BakeName
  // CreatureDisplayExtra-*.blp
};
...
struct FactionEntry {
  ...
  uint32 spilloverMaxRankIn;  // 21       m_parentFactionCap[2] The highest rank the faction will profit from incoming
                              // spillover
  uint32 spilloverRank_unk;   // 22       It does not seem to be the max standing at which a faction outputs spillover
                              // ...so no idea
...
{
    uint32      ID;                                         // 0        m_ID
    uint32      maskID;                                     // 1        m_maskID       index of the bit to check for in
the faction group mask char*       internalName;                               // 2        m_internalName
...
struct LiquidTypeEntry {
  uint32 Id;       // 0
  uint32 LiquidId; // 1        1: Coilfang Raid - Water; 23: Water; 29: Ocean; 35: Magma; 41: Slime; 47: Naxxramas -
                   // Slime; 65: Hyjal Past - Water.
...
struct MapEntry {
  ...
  // chat*     unknownText1                               // 66-81 unknown empty text fields, possible normal Intro
  // text. 82 text flags chat*     heroicIntroText                            // 83-98 heroic mode requirement text 99
  // text flags chat*     unknownText2                               // 100-115 unknown empty text fields 116 text flags
  int32 ghost_entrance_map; // 117      m_corpseMapID map_id of entrance map in ghost mode (continent always and in most
  ...
  bool IsContinent() const { return MapID == 0 || MapID == 1 || MapID == 530; }
};
...
struct SpellEntry {
  ...
  int32 EffectBasePoints[MAX_EFFECT_INDEX];          // 80-82    m_effectBasePoints (don't must be used in spell/auras
                                                     // explicitly, must be used cached Spell::m_currentBasePoints)
  ...
  bool IsFitToFamilyMask(uint64 familyFlags) const { return SpellFamilyFlags.IsFitToFamilyMask(familyFlags); }
  ...
  bool IsFitToFamilyMask(ClassFamilyMask const &mask) const { return SpellFamilyFlags.IsFitToFamilyMask(mask); }
...
struct TalentEntry {
  ...
  // uint32  needAddInSpellBook;                          // 19       m_flags also need disable higest ranks on reset
  // talent tree
...
struct TotemCategoryEntry {
  ...
  uint32 categoryMask; // 19       m_totemCategoryMask (compatibility mask for same type: different for totems,
                       // compatible from high to low for rods)
};
...
struct WorldMapAreaEntry {
  ...
  int32 virtual_map_id; // 8        m_displayMapID -1 (map_id have correct map) other: virtual map where zone show
                        // (map_id - where zone in fact internally)
};
...
struct ItemCategorySpellPair {
  ...
  bool operator<(ItemCategorySpellPair const &pair) const {
    return spellId == pair.spellId ? itemId < pair.itemId : spellId < pair.spellId;
  }
};
