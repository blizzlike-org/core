...
struct PoolObject {
  ...
  template <typename T>
  void CheckEventLinkAndReport(uint32 poolId, int16 event_id, std::map<uint32, int16> const &creature2event,
...
public:
  ...
  template <typename T> bool IsSpawnedObject(uint32 db_guid_or_pool_id) const;

  uint32 GetSpawnedObjects(uint32 pool_id) const;

  template <typename T> void AddSpawn(uint32 db_guid_or_pool_id, uint32 pool_id);

  template <typename T> void RemoveSpawn(uint32 db_guid_or_pool_id, uint32 pool_id);
...
public:
  ...
  void CheckEventLinkAndReport(int16 event_id, std::map<uint32, int16> const &creature2event,
                               std::map<uint32, int16> const &go2event) const;
...
public:
  ...
  template <typename T> uint16 IsPartOfAPool(uint32 db_guid_or_pool_id) const;

  // Method that tell if the creature/gameobject/pool is part of top level pool and return the pool id if yes
  template <typename T> uint16 IsPartOfTopPool(uint32 db_guid_or_pool_id) const {
  ...
  template <typename T> void SetExcludeObject(uint16 pool_id, uint32 db_guid_or_pool_id, bool state);

  bool CheckPool(uint16 pool_id) const;
  void CheckEventLinkAndReport(uint16 pool_id, int16 event_id, std::map<uint32, int16> const &creature2event,
                               std::map<uint32, int16> const &go2event) const;
  ...
  template <typename T> void UpdatePool(MapPersistentState &mapState, uint16 pool_id, uint32 db_guid_or_pool_id = 0);
  ...
  // used for calling from global systems when need initialize spawn pool state in appropriate (possible) map persistent
  // state
  void InitSpawnPool(MapPersistentState &mapState, uint16 pool_id);

  template <typename T> void UpdatePoolInMaps(uint16 pool_id, uint32 db_guid_or_pool_id = 0);
...
template <> inline uint16 PoolManager::IsPartOfAPool<Creature>(uint32 db_guid) const {
...
template <> inline uint16 PoolManager::IsPartOfAPool<GameObject>(uint32 db_guid) const {
...
template <> inline uint16 PoolManager::IsPartOfAPool<Pool>(uint32 pool_id) const {
