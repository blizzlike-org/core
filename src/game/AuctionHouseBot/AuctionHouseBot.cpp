...
void AuctionHouseBot::Initialize() {
  ...
  if (m_chanceSell > 0 || m_chanceBuy > 0) {
    ...
    FillUintVectorFromQuery(
        "SELECT entry FROM creature_template WHERE `rank` = 0 AND entry IN (SELECT entry FROM creature_loot_template)",
        m_creatureLootNormalTemplates);
    FillUintVectorFromQuery(
        "SELECT entry FROM creature_template WHERE `rank` = 1 AND entry IN (SELECT entry FROM creature_loot_template)",
        m_creatureLootEliteTemplates);
    FillUintVectorFromQuery(
        "SELECT entry FROM creature_template WHERE `rank` = 2 AND entry IN (SELECT entry FROM creature_loot_template)",
        m_creatureLootRareEliteTemplates);
    FillUintVectorFromQuery(
        "SELECT entry FROM creature_template WHERE `rank` = 3 AND entry IN (SELECT entry FROM creature_loot_template)",
        m_creatureLootWorldBossTemplates);
    FillUintVectorFromQuery(
        "SELECT entry FROM creature_template WHERE `rank` = 4 AND entry IN (SELECT entry FROM creature_loot_template)",
        m_creatureLootRareTemplates);
    ...
    FillUintVectorFromQuery(
        "SELECT DISTINCT entry FROM gameobject_loot_template WHERE entry IN (SELECT data1 FROM gameobject_template "
        "WHERE entry IN (SELECT id FROM gameobject WHERE state = 1 AND spawntimesecsmax > 0))",
        m_gameobjectLootTemplates);
    ...
    FillUintVectorFromQuery("SELECT entry FROM item_template WHERE entry IN (SELECT EffectItemType1 FROM "
                            "spell_template WHERE attributes & 32 AND attributes & 65536)",
                            m_professionItems);
    ...
    if (m_auctionBidMin > m_auctionBidMax) {
      sLog.outError("AHBot error: AuctionHouseBot.Bid.Min must be less or equal to AuctionHouseBot.Bid.Max. Setting "
                    "Bid.Min equal to Bid.Max.");
    ...
    if (m_auctionTimeMin > m_auctionTimeMax) {
      sLog.outError("AHBot error: AuctionHouseBot.Time.Min must be less or equal to AuctionHouseBot.Time.Max. Setting "
                    "Time.Min equal to Time.Max.");
    ...
    // overridden items
    QueryResult *result =
        CharacterDatabase.PQuery("SELECT item, value, add_chance, min_amount, max_amount FROM ahbot_items");
    if (result) {
      do {
...
void AuctionHouseBot::Update() {
  ...
  if (m_houseAction < MAX_AUCTION_HOUSE_TYPE && urand(0, 99) < m_chanceSell) {
    ...
    AddLootToItemMap(&LootTemplates_Creature, m_creatureLootNormalConfig, m_creatureLootNormalTemplates,
                     itemMap); // normal creature loot
    AddLootToItemMap(&LootTemplates_Creature, m_creatureLootEliteConfig, m_creatureLootEliteTemplates,
                     itemMap); // elite creature loot
    AddLootToItemMap(&LootTemplates_Creature, m_creatureLootRareEliteConfig, m_creatureLootRareEliteTemplates,
                     itemMap); // rare elite creature loot
    AddLootToItemMap(&LootTemplates_Creature, m_creatureLootWorldBossConfig, m_creatureLootWorldBossTemplates,
                     itemMap); // world boss creature loot
    AddLootToItemMap(&LootTemplates_Creature, m_creatureLootRareConfig, m_creatureLootRareTemplates,
                     itemMap); // rare creature loot

    AddLootToItemMap(&LootTemplates_Disenchant, m_disenchantLootConfig, m_disenchantLootTemplates,
                     itemMap);                                                                      // disenchant loot
    AddLootToItemMap(&LootTemplates_Fishing, m_fishingLootConfig, m_fishingLootTemplates, itemMap); // fishing loot
    AddLootToItemMap(&LootTemplates_Gameobject, m_gameobjectLootConfig, m_gameobjectLootTemplates,
                     itemMap); // gameobject loot
    ...
    if (m_professionItemsConfig[1] > 0 && m_professionItemsConfig[3] > 0 && m_professionItems.size() > 0) {
      int32 maxTemplates =
          m_professionItemsConfig[0] < 0
              ? urand(0, m_professionItemsConfig[1] - m_professionItemsConfig[0]) + m_professionItemsConfig[0]
              : urand(m_professionItemsConfig[0], m_professionItemsConfig[1]);
      if (maxTemplates > 0) {
        for (int32 templateCounter = 0; templateCounter < maxTemplates; ++templateCounter) {
          ...
          if (!prototype || prototype->Quality == 0 || urand(0, (1 << (prototype->Quality - 1)) - 1) > 0)
            continue; // make it decreasingly likely that crafted items of higher quality is added to the auction house
                      // (white: 100%, green: 50%, blue: 25%, purple: 12.5%, ...)
          uint32 count = (uint32)round((uint64)prototype->GetMaxStackSize() *
                                       urand(m_professionItemsConfig[2], m_professionItemsConfig[3]) / 100.0);
          if (count <= 0)
            count = 1;
          itemMap[item] += count;
        }
    ...
    for (auto &itemData : m_itemData) {
      if (itemData.second.AddChance > 0) // replace normal loot sources with custom chance of adding item
        itemMap[itemData.first] =
            urand(0, 99) < itemData.second.AddChance ? urand(itemData.second.MinAmount, itemData.second.MaxAmount) : 0;
    }
    ...
    for (auto &itemEntry : itemMap) {
      ...
      uint32 itemValue =
          ValueWithVariance(iterator != m_itemData.end() ? iterator->second.Value : CalculateBuyoutPrice(prototype));
      for (uint32 stackCounter = 0; stackCounter < itemEntry.second; stackCounter += prototype->GetMaxStackSize()) {
        uint32 count = itemEntry.second - stackCounter > prototype->GetMaxStackSize() ? prototype->GetMaxStackSize()
                                                                                      : itemEntry.second - stackCounter;
        ...
        if (item)
          auctionHouse->AddAuction(sAuctionHouseStore.LookupEntry(houseType == AUCTION_HOUSE_ALLIANCE
                                                                      ? 1
                                                                      : (houseType == AUCTION_HOUSE_HORDE ? 6 : 7)),
                                   item, urand(m_auctionTimeMin, m_auctionTimeMax) * HOUR, bidPrice, buyoutPrice);
      }
    }
    ...
    for (AuctionHouseObject::AuctionEntryMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr) {
      ...
      if (auction->buyout > 0 && buyItemCheck > auction->buyout)
        buyoutAuctions.push_back(
            auction); // can't buyout item here as that modifies the AuctionEntryMap, invalidating the iterator
...
void AuctionHouseBot::Rebuild(bool all) {
  ...
  for (uint32 i = 0; i < MAX_AUCTION_HOUSE_TYPE; ++i) {
    AuctionHouseObject::AuctionEntryMapBounds bounds =
        sAuctionMgr.GetAuctionsMap(AuctionHouseType(i))->GetAuctionsBounds();
    for (AuctionHouseObject::AuctionEntryMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr) {
...
void AuctionHouseBot::SetItemData(uint32 item, AuctionHouseBotItemData &itemData, bool reset) {
  ...
  stmt = CharacterDatabase.CreateStatement(
      addItem, "INSERT INTO ahbot_items (item, value, add_chance, min_amount, max_amount) VALUES (?, ?, ?, ?, ?)");
  stmt.addUInt32(item);
...
AuctionHouseBotItemData AuctionHouseBot::GetItemData(uint32 item) {
  ...
  // item data not overridden, set MinAmount/MaxAmount to 0 (those values can normally never be 0) as a means to alert
  // requester that item data is not overridden
  AuctionHouseBotItemData itemData;
  ItemPrototype const *prototype = ObjectMgr::GetItemPrototype(item);
...
uint32 AuctionHouseBot::GetMinMaxConfig(const char *config, uint32 minValue, uint32 maxValue, uint32 defaultValue) {
  ...
  if (field < minValue) {
    sLog.outError("AHBot error: %s must be between %u and %u. Setting value to %u.", config, minValue, maxValue,
                  defaultValue);
    field = defaultValue;
...
void AuctionHouseBot::ParseLootConfig(char const *fieldname, std::vector<int32> &lootConfig) {
  ...
  if (lootConfig.size() > 4)
    sLog.outError(
        "AHBot error: Too many values specified for field %s (%lu), 4 values required. Additional values ignored.",
        fieldname, lootConfig.size());
  else if (lootConfig.size() < 4) {
    sLog.outError(
        "AHBot error: Too few values specified for field %s (%lu), 4 values required. Setting 0 for remaining values.",
        fieldname, lootConfig.size());
  ...
  for (uint32 index = 1; index < 4; ++index) {
    if (lootConfig[index] < 0) {
      sLog.outError("AHBot error: %s value (%d) for field %s should not be a negative number, setting value to 0.",
                    (index == 1 ? "Second" : (index == 2 ? "Third" : "Fourth")), lootConfig[1], fieldname);
      lootConfig[index] = 0;
    }
  }
  if (lootConfig[0] > lootConfig[1]) {
    sLog.outError("AHBot error: First value (%d) must be less than or equal to second value (%d) for field %s. Setting "
                  "first value to second value.",
                  lootConfig[0], lootConfig[1], fieldname);
    lootConfig[0] = lootConfig[1];
  }
  if (lootConfig[2] > lootConfig[3]) {
    sLog.outError("AHBot error: Third value (%d) must be less than or equal to fourth value (%d) for field %s. Setting "
                  "third value to fourth value.",
                  lootConfig[2], lootConfig[3], fieldname);
...
void AuctionHouseBot::AddLootToItemMap(LootStore *store, std::vector<int32> &lootConfig,
                                       std::vector<uint32> &lootTemplates,
                                       std::unordered_map<uint32, uint32> &itemMap) {
  if (lootConfig[1] <= 0 || lootConfig[3] <= 0 || lootTemplates.size() <= 0)
    return;
  int32 maxTemplates =
      lootConfig[0] < 0 ? urand(0, lootConfig[1] - lootConfig[0]) + lootConfig[0] : urand(lootConfig[0], lootConfig[1]);
  if (maxTemplates <= 0)
    return;
  for (int32 templateCounter = 0; templateCounter < maxTemplates; ++templateCounter) {
...
uint32 AuctionHouseBot::CalculateBuyoutPrice(ItemPrototype const *prototype) {
  ...
  // test whether we have a buy price or if buy price greatly exceed sell price (causes item to be valued too high,
  // notably arrows/shells do this) if using sell price then price must be multiplied by 4 if white or gray item, 5 if
  // green or better to match expected buy price
  ...
  buyoutPrice *= (m_vendorValue && m_vendorItems.find(prototype->ItemId) != m_vendorItems.end()
                      ? 100
                      : m_itemValue[prototype->Quality][prototype->Class]);
