...
static void AttemptJoin(Player *_player) {
  ...
  for (HashMapHolder<Player>::MapType::const_iterator iter = players.begin(); iter != players.end(); ++iter) {
    ...
    if (!plr->m_lookingForGroup.more.canAutoJoin() ||
        !_player->m_lookingForGroup.HaveInSlot(plr->m_lookingForGroup.more))
      continue;
...
static void AttemptAddMore(Player *_player) {
  ...
  for (HashMapHolder<Player>::MapType::const_iterator iter = players.begin(); iter != players.end(); ++iter) {
    ...
    // skip non auto-join or empty slots, or non compatible slots
    if (!plr->GetSession()->LookingForGroup_auto_join ||
        !plr->m_lookingForGroup.HaveInSlot(_player->m_lookingForGroup.more))
      continue;
