...
public:
  ...
  bool IsEvadeRegen() const {
    return (m_evadeTimer > 0 && m_evadeTimer <= 5000) || m_evadeState;
  }                                                // Only regen after 5 seconds, or when in permanent evade
  ...
  void SetLeashingCheck(std::function<bool(Unit *, float x, float y, float z)> check) {
    m_leashingCheck = check;
  } // if check evals as true - evade
private:
  Unit *m_owner;
