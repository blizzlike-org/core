          ...
          if (check) {
            ...
            // if timer ran out and we are far away from last refresh pos, evade
              ...
              if (m_owner->GetVictim()->GetDistance2d(m_lastRefreshPos.GetPositionX(),
                                                      m_lastRefreshPos.GetPositionY()) > 30.0f)
...
void CombatManager::SetEvadeState(EvadeState state) {
  ...
  if (!m_owner->HasCharmer())
    m_owner->CallForAllControlledUnits(SetEvadeHelper(state),
                                       CONTROLLED_PET | CONTROLLED_TOTEMS | CONTROLLED_GUARDIANS | CONTROLLED_CHARM);
}

void CombatManager::TriggerCombatTimer(Unit *target) {
  if (target->CanHaveThreatList()) // From attackers PoV if we have victim and victim can have a threat list then set
                                   // his timer
