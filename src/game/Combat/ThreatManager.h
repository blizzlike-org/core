...
public:
  static float CalcThreat(Unit *hatedUnit, Unit *hatingUnit, float threat, bool crit, SpellSchoolMask schoolMask,
...
public:
  ...
  bool operator==(const HostileReference &hostileReference) const {
    return hostileReference.getUnitGuid() == getUnitGuid();
  }
...
public:
  ...
  void addThreat(Unit *victim, float threat) {
    addThreat(victim, threat, false, SPELL_SCHOOL_MASK_NONE, nullptr, false);
  }
  ...
  void modifyThreatPercent(Unit *victim,
                           int32 threatPercent); // -101 removes whole ref, -100 sets threat to 0, rest modifies it
  void modifyAllThreatPercent(int32 threatPercent);
  ...
  void
  setCurrentVictimByTarget(Unit *target); // Used in SPELL_EFFECT_ATTACK_ME to set the current target to the taunter
