...
MapPersistentState::MapPersistentState(uint16 MapId, uint32 InstanceId, Difficulty difficulty)
    : m_instanceid(InstanceId), m_mapid(MapId), m_difficulty(difficulty), m_usedByMap(nullptr) {}

MapPersistentState::~MapPersistentState() {}

MapEntry const *MapPersistentState::GetMapEntry() const { return sMapStore.LookupEntry(m_mapid); }
...
void MapPersistentState::SaveCreatureRespawnTime(uint32 loguid, time_t t) {
  ...
  SqlStatement stmt =
      CharacterDatabase.CreateStatement(delSpawnTime, "DELETE FROM creature_respawn WHERE guid = ? AND instance = ?");
...
void MapPersistentState::SaveGORespawnTime(uint32 loguid, time_t t) {
  ...
  SqlStatement stmt =
      CharacterDatabase.CreateStatement(delSpawnTime, "DELETE FROM gameobject_respawn WHERE guid = ? AND instance = ?");
  stmt.PExecute(loguid, m_instanceid);
...
bool WorldPersistentState::CanBeUnload() const {
  ...
  // Note: non instanceable Map never unload until server shutdown and in result for loaded non-instanceable maps map
  // persistent states also not unloaded
  //       but for proper work pool systems with shared pools state for non-instanceable maps need
  //       load persistent map states for any non-instanceable maps before Map loading and make sure that it never
  //       unloaded
...
DungeonPersistentState::DungeonPersistentState(uint16 MapId, uint32 InstanceId, Difficulty difficulty, time_t resetTime,
                                               bool canReset, uint32 completedEncountersMask)
    : MapPersistentState(MapId, InstanceId, difficulty), m_resetTime(resetTime), m_canReset(canReset),
      m_completedEncountersMask(completedEncountersMask) {}
...
void DungeonPersistentState::SaveToDB() {
  ...
  CharacterDatabase.PExecute("INSERT INTO instance VALUES ('%u', '%u', '" UI64FMTD "', '%u', '%u', '%s')",
                             GetInstanceId(), GetMapId(), (uint64)GetResetTimeForDB(), GetDifficulty(),
                             GetCompletedEncountersMask(), data.c_str());
}
...
void DungeonPersistentState::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry) {
  ...
  for (DungeonEncounterMap::const_iterator iter = bounds.first; iter != bounds.second; ++iter) {
    ...
    if (iter->second->creditType == type && Difficulty(dbcEntry->Difficulty) == GetDifficulty() &&
        dbcEntry->mapId == GetMapId()) {
      m_completedEncountersMask |= 1 << dbcEntry->encounterIndex;

      CharacterDatabase.PExecute("UPDATE instance SET encountersMask = '%u' WHERE id = '%u'", m_completedEncountersMask,
                                 GetInstanceId());

      DEBUG_LOG("DungeonPersistentState: Dungeon %s (Id %u) completed encounter %s", GetMap()->GetMapName(),
                GetInstanceId(), dbcEntry->encounterName[sWorld.GetDefaultDbcLocale()]);
      if (/*uint32 dungeonId =*/iter->second->lastEncounterDungeon) {
        DEBUG_LOG("DungeonPersistentState:: Dungeon %s (Instance-Id %u) completed last encounter %s",
                  GetMap()->GetMapName(), GetInstanceId(), dbcEntry->encounterName[sWorld.GetDefaultDbcLocale()]);
  ...
  if (result) {
    ...
    if (result) {
      do {
        ...
        if (itr != InstResetTime.end() && itr->second.second != resettime) {
          CharacterDatabase.DirectPExecute("UPDATE instance SET resettime = '" UI64FMTD "' WHERE id = '%u'",
    ...
    for (ResetTimeMapType::iterator itr = InstResetTime.begin(); itr != InstResetTime.end(); ++itr)
      if (itr->second.second > now)
        ScheduleReset(true, itr->second.second,
  ...
  if (result) {
    do {
      ...
      if (oldresettime != newresettime)
        CharacterDatabase.DirectPExecute("UPDATE instance_reset SET resettime = '" UI64FMTD "' WHERE mapid = '%u'",
                                         newresettime, mapid);
  ...
  if (add)
    ...
    if (itr == range.second) {
      ...
      if (itr == m_resetTimeQueue.end())
        sLog.outError(
            "DungeonResetScheduler::ScheduleReset: cannot cancel the reset, the event(%d,%d,%d) was not found!",
...
*/
MapPersistentState *MapPersistentStateManager::AddPersistentState(MapEntry const *mapEntry, uint32 instanceId,
    ...
    if (!resetTime) {
      ...
      if (mapEntry->map_type == MAP_RAID || difficulty > DUNGEON_DIFFICULTY_NORMAL)
        ...
        m_Scheduler.ScheduleReset(true, resetTime,
                                  DungeonResetEvent(RESET_EVENT_NORMAL_DUNGEON, mapEntry->MapID, instanceId));
      }
    }
  }

  DEBUG_LOG("MapPersistentStateManager::AddPersistentState: mapid = %d, instanceid = %d, reset time = '" UI64FMTD
            "', canRset = %u",
            mapEntry->MapID, instanceId, uint64(resetTime), canReset ? 1 : 0);

  MapPersistentState *state;
  if (mapEntry->IsDungeon()) {
    DungeonPersistentState *dungeonState = new DungeonPersistentState(mapEntry->MapID, instanceId, difficulty,
                                                                      resetTime, canReset, completedEncountersMask);
    if (!load)
    ...
    if (itr != m_instanceSaveByInstanceId.end()) {
      ...
      if (itr->second->GetMapEntry()->IsDungeon())
        if (time_t resettime = ((DungeonPersistentState *)itr->second)->GetResetTimeForDB())
          CharacterDatabase.PExecute("UPDATE instance SET resettime = '" UI64FMTD "' WHERE id = '%u'",
...
void MapPersistentStateManager::_DelHelper(DatabaseType &db, const char *fields, const char *table,
...
void MapPersistentStateManager::CleanupInstances() {
  ...
  _DelHelper(CharacterDatabase, "character_instance.guid, instance", "character_instance",
             "LEFT JOIN characters ON character_instance.guid = characters.guid WHERE characters.guid IS NULL");
  _DelHelper(
      CharacterDatabase, "group_instance.leaderGuid, instance", "group_instance",
      "LEFT JOIN characters ON group_instance.leaderGuid = characters.guid LEFT JOIN `groups` ON "
      "group_instance.leaderGuid = `groups`.leaderGuid WHERE characters.guid IS NULL OR `groups`.leaderGuid IS NULL");

  // clean instances that do not have any players or groups bound to them
  _DelHelper(
      CharacterDatabase, "id, map, difficulty", "instance",
      "LEFT JOIN character_instance ON character_instance.instance = id LEFT JOIN group_instance ON "
      "group_instance.instance = id WHERE character_instance.instance IS NULL AND group_instance.instance IS NULL");

  // clean invalid instance references in other tables
  _DelHelper(CharacterDatabase, "character_instance.guid, instance", "character_instance",
             "LEFT JOIN instance ON character_instance.instance = instance.id WHERE instance.id IS NULL");
  _DelHelper(CharacterDatabase, "group_instance.leaderGuid, instance", "group_instance",
             "LEFT JOIN instance ON group_instance.instance = instance.id WHERE instance.id IS NULL");

  // clean unused respawn data
  CharacterDatabase.Execute(
      "DELETE FROM creature_respawn WHERE instance <> 0 AND instance NOT IN (SELECT id FROM instance)");
  CharacterDatabase.Execute(
      "DELETE FROM gameobject_respawn WHERE instance <> 0 AND instance NOT IN (SELECT id FROM instance)");
...
void MapPersistentStateManager::PackInstances() const {
  ...
  for (uint32 i : InstanceSet) {
    if (i != InstanceNumber) {
      CharacterDatabase.BeginTransaction();
      // remap instance id
      CharacterDatabase.PExecute("UPDATE creature_respawn SET instance = '%u' WHERE instance = '%u'", InstanceNumber,
                                 i);
      CharacterDatabase.PExecute("UPDATE gameobject_respawn SET instance = '%u' WHERE instance = '%u'", InstanceNumber,
                                 i);
      CharacterDatabase.PExecute("UPDATE corpse SET instance = '%u' WHERE instance = '%u'", InstanceNumber, i);
      CharacterDatabase.PExecute("UPDATE character_instance SET instance = '%u' WHERE instance = '%u'", InstanceNumber,
                                 i);
...
void MapPersistentStateManager::_ResetOrWarnAll(uint32 mapid, bool warn, uint32 timeLeft) {
  ...
  if (!warn) {
    ...
    // note that we must build a list of states to unbind and then unbind them in two steps.  this is because the
    // unbinding may trigger the modification of the collection, which would invalidate the iterator and cause a crash.
    for (auto &itr : m_instanceSaveByInstanceId)
    ...
    CharacterDatabase.BeginTransaction();
    CharacterDatabase.PExecute("DELETE FROM character_instance USING character_instance LEFT JOIN instance ON "
                               "character_instance.instance = id WHERE map = '%u'",
                               mapid);
    CharacterDatabase.PExecute("DELETE FROM group_instance USING group_instance LEFT JOIN instance ON "
                               "group_instance.instance = id WHERE map = '%u'",
                               mapid);
    ...
    // update it in the DB
    CharacterDatabase.PExecute("UPDATE instance_reset SET resettime = '" UI64FMTD "' WHERE mapid = '%u'",
...
void MapPersistentStateManager::_CleanupExpiredInstancesAtTime(time_t t) {
  _DelHelper(CharacterDatabase, "id, map, difficulty", "instance",
             ...
             "' AND instance.resettime > '0') OR (NOT instance_reset.resettime IS NULL AND instance_reset.resettime < "
             "'" UI64FMTD "')",
             (uint64)t, (uint64)t);
}
...
void MapPersistentStateManager::LoadCreatureRespawnTimes() {
  ...
  //                                                    0     1            2    3         4           5          6
  QueryResult *result =
      CharacterDatabase.Query("SELECT guid, respawntime, map, instance, difficulty, resettime, encountersMask FROM "
                              "creature_respawn LEFT JOIN instance ON instance = id");
  if (!result) {
    BarGoLink bar(1);
    bar.step();
  ...
  do {
    ...
    MapPersistentState *state = AddPersistentState(mapEntry, instanceId, Difficulty(difficulty), resetTime,
                                                   mapEntry->IsDungeon(), true, completedEncounters);
    if (!state)
      continue;
