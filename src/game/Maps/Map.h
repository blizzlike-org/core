...
public:
  ...
  // _currently_ spawnmode == difficulty, but this can be changes later, so use appropriate spawnmode/difficult
  // functions for simplify later code support regular difficulty = continent/dungeon normal/raid normal difficulty
  ...
  enum ScriptExecutionParam {
    ...
    SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE =
        0x01, // Start Script only if not yet started (uniqueness identified by id and source)
    SCRIPT_EXEC_PARAM_UNIQUE_BY_TARGET =
        0x02, // Start Script only if not yet started (uniqueness identified by id and target)
    SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE_TARGET =
        0x03, // Start Script only if not yet started (uniqueness identified by id, source and target)
  };
  ...
  WorldObject *
  GetWorldObject(ObjectGuid guid); // only use if sure that need objects at current map, specially for player case
  ...
  void MonsterYellToMap(CreatureInfo const *cinfo, int32 textId, ChatMsg chatMsg, Language language, Unit const *target,
                        uint32 senderLowGuid = 0) const;
  ...
  bool GetHitPosition(float srcX, float srcY, float srcZ, float &destX, float &destY, float &destZ,
...
private:
  ...
  template <class T> void AddToGrid(T *, NGridType *, Cell const &);

  template <class T> void RemoveFromGrid(T *, NGridType *, Cell const &);
