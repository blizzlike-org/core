...
#define INVALID_HEIGHT                                                                                                 \
  -100000.0f // for check, must be equal to VMAP_INVALID_HEIGHT, real value for unknown height is
             // VMAP_INVALID_HEIGHT_VALUE
#define INVALID_HEIGHT_VALUE                                                                                           \
  -200000.0f // for return, must be equal to VMAP_INVALID_HEIGHT_VALUE, check value for unknown height is
             // VMAP_INVALID_HEIGHT
...
template <const unsigned int LIMIT> struct CoordPair {
...
namespace MaNGOS {
template <class RET_TYPE, int CENTER_VAL> inline RET_TYPE Compute(float x, float y, float center_offset, float size) {
...
inline bool IsValidMapCoord(float c) { return std::isfinite(c) && (std::fabs(c) <= MAP_HALFSIZE - 0.5); }

inline bool IsValidMapCoord(float x, float y) { return IsValidMapCoord(x) && IsValidMapCoord(y); }

inline bool IsValidMapCoord(float x, float y, float z) { return IsValidMapCoord(x, y) && std::isfinite(z); }

inline bool IsValidMapCoord(float x, float y, float z, float o) { return IsValidMapCoord(x, y, z) && std::isfinite(o); }
} // namespace MaNGOS
#endif
