...
public:
  ...
  GridMapLiquidStatus getLiquidStatus(float x, float y, float z, uint8 ReqLiquidType,
                                      GridMapLiquidData *data = nullptr);
};

template <typename Countable> class Referencable {
public:
...
public:
  ...
  float GetWaterOrGroundLevel(float x, float y, float z, float &groundZ, bool swim = false,
...
class TerrainManager
    : public MaNGOS::Singleton<TerrainManager, MaNGOS::ClassLevelLockable<TerrainManager, std::mutex>> {
