...
public:
  ...
  template <typename T> bool IsSpawnedPoolObject(uint32 db_guid_or_pool_id) {
...
private:
  ...
  // persistent data
  RespawnTimes m_creatureRespawnTimes; // lock MapPersistentState from unload, for example for temporary bound dungeon
                                       // unload delay
  RespawnTimes
      m_goRespawnTimes; // lock MapPersistentState from unload, for example for temporary bound dungeon unload delay
...
private:
  ...
  uint32 m_completedEncountersMask; // completed encounter mask, bit indexes are DungeonEncounter.dbc boss numbers, used
                                    // for packets
};
...
class MapPersistentStateManager
    : public MaNGOS::Singleton<MapPersistentStateManager,
                               MaNGOS::ClassLevelLockable<MapPersistentStateManager, std::mutex>> {
...
public: // common for all MapPersistentState (sub)classes
  ...
  // auto select appropriate MapPersistentState (sub)class by MapEntry, and autoselect appropriate way store (by
  // instance/map id) always return != nullptr
  MapPersistentState *AddPersistentState(MapEntry const *mapEntry, uint32 instanceId, Difficulty difficulty,
  ...
  template <typename Do> void DoForAllStatesWithMapId(uint32 mapId, Do &_do);
...
template <typename Do> inline void MapPersistentStateManager::DoForAllStatesWithMapId(uint32 mapId, Do &_do) {
