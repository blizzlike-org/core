...
uint32 Map::GetCurrentMSTime() const { return World::GetCurrentMSTime(); }

TimePoint Map::GetCurrentClockTime() const { return World::GetCurrentClockTime(); }

uint32 Map::GetCurrentDiff() const { return World::GetCurrentDiff(); }

GenericTransport *Map::GetTransport(ObjectGuid guid) {
  // elevators also cause the client to send MOVEFLAG_ONTRANSPORT - just unmount if the guid can be found in the
  // transport list
...
void Map::AddTransport(Transport *transport) { m_transports.insert(transport); }
...
Map::Map(uint32 id, time_t expiry, uint32 InstanceId, uint8 SpawnMode)
    : i_mapEntry(sMapStore.LookupEntry(id)), i_spawnMode(SpawnMode), i_id(id), i_InstanceId(InstanceId),
      m_unloadTimer(0), m_VisibleDistance(DEFAULT_VISIBILITY_DISTANCE), m_persistentState(nullptr),
      m_activeNonPlayersIter(m_activeNonPlayers.end()), m_onEventNotifiedIter(m_onEventNotifiedObjects.end()),
      i_gridExpiry(expiry), m_TerrainData(sTerrainMgr.LoadTerrain(id)), i_data(nullptr), i_script_id(0),
      m_transportsIterator(m_transports.begin()) {
...
void Map::Initialize(bool loadInstanceData /*= true*/) {
  ...
  m_persistentState =
      sMapPersistentStateMgr.AddPersistentState(i_mapEntry, GetInstanceId(), GetDifficulty(), 0, IsDungeon(), false);
...
void Map::EnsureGridLoadedAtEnter(const Cell &cell, Player *player) {
  ...
  if (EnsureGridLoaded(cell)) {
    ...
    if (player) {
      DEBUG_FILTER_LOG(LOG_FILTER_PLAYER_MOVES, "Player %s enter cell[%u,%u] triggers of loading grid[%u,%u] on map %u",
                       player->GetName(), cell.CellX(), cell.CellY(), cell.GridX(), cell.GridY(), i_id);
    } else {
      DEBUG_FILTER_LOG(LOG_FILTER_PLAYER_MOVES, "Active object nearby triggers of loading grid [%u,%u] on map %u",
...
template <class T> void Map::Add(T *obj) {
  ...
  if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP) {
    sLog.outError("Map::Add: Object (GUID: %u TypeId: %u) have invalid coordinates X:%f Y:%f grid cell [%u:%u]",
                  obj->GetGUIDLow(), obj->GetTypeId(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
    return;
  }
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_CREATURE_MOVES, "%s enters grid[%u,%u]", obj->GetGuidStr().c_str(), cell.GridX(),
                   cell.GridY());
...
void Map::MessageBroadcast(Player const *player, WorldPacket const &msg, bool to_self) {
  ...
  if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP) {
    sLog.outError("Map::MessageBroadcast: Player (GUID: %u) have invalid coordinates X:%f Y:%f grid cell [%u:%u]",
                  player->GetGUIDLow(), player->GetPositionX(), player->GetPositionY(), p.x_coord, p.y_coord);
    return;
  }
...
void Map::MessageBroadcast(WorldObject const *obj, WorldPacket const &msg) {
  ...
  if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP) {
    sLog.outError(
        "Map::MessageBroadcast: Object (GUID: %u TypeId: %u) have invalid coordinates X:%f Y:%f grid cell [%u:%u]",
        obj->GetGUIDLow(), obj->GetTypeId(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
    return;
  }
...
void Map::MessageDistBroadcast(Player const *player, WorldPacket const &msg, float dist, bool to_self,
                               bool own_team_only) {
  CellPair p = MaNGOS::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
...
void Map::ExecuteDistWorker(WorldObject const *obj, float dist, std::function<void(Player *)> const &worker) {
  ...
  if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP) {
    sLog.outError(
        "Map::ExecuteDistWorker: Object (GUID: %u TypeId: %u) have invalid coordinates X:%f Y:%f grid cell [%u:%u]",
        obj->GetGUIDLow(), obj->GetTypeId(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
    return;
  }
...
void Map::Update(const uint32 &t_diff) {
...
#ifdef BUILD_METRICS
  metric::duration<std::chrono::milliseconds> meas(
      "map.update", {{"map_id", std::to_string(i_id)}, {"instance_id", std::to_string(i_InstanceId)}});
#endif
  ...
  // Don't unload grids if it's battleground, since we may have manually added GOs,creatures, those doesn't load from DB
  // at grid re-load ! This isn't really bother us, since as soon as we have instanced BG-s, the whole map unloads as
  // the BG gets ended
...
void Map::Remove(Player *player, bool remove) {
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_PLAYER_MOVES, "Remove player %s from grid[%u,%u]", player->GetName(), cell.GridX(),
                   cell.GridY());
  NGridType *grid = getNGrid(cell.GridX(), cell.GridY());
  MANGOS_ASSERT(grid != nullptr);
...
template <class T> void Map::Remove(T *obj, bool remove) {
  CellPair p = MaNGOS::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
  if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP) {
    sLog.outError("Map::Remove: Object (GUID: %u TypeId:%u) have invalid coordinates X:%f Y:%f grid cell [%u:%u]",
                  obj->GetGUIDLow(), obj->GetTypeId(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
    return;
  }
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_CREATURE_MOVES, "Remove %s from grid[%u,%u]", obj->GetGuidStr().c_str(),
  ...
  UpdateObjectVisibility(obj, cell, p); // i think will be better to call this function while object still in grid, this
                                        // changes nothing but logically is better(as for me)
...
void Map::PlayerRelocation(Player *player, float x, float y, float z, float orientation) {
  ...
  if (old_cell.DiffGrid(new_cell) || old_cell.DiffCell(new_cell)) {
    DEBUG_FILTER_LOG(LOG_FILTER_PLAYER_MOVES, "Player %s relocation grid[%u,%u]cell[%u,%u]->grid[%u,%u]cell[%u,%u]",
                     player->GetName(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(),
...
void Map::CreatureRelocation(Creature *creature, float x, float y, float z, float ang) {
  ...
  // creature coordinates will be updated and notifiers send
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_CREATURE_MOVES,
                     "Creature (GUID: %u Entry: %u ) can't be move to unloaded respawn grid.", creature->GetGUIDLow(),
                     creature->GetEntry());
  }
}
  ...
  if (old_cell.DiffGrid(new_cell)) {
    if (!go->isActiveObject() && !loaded(new_cell.gridPair())) {
      DEBUG_FILTER_LOG(
          ...
          "Creature (GUID: %u Entry: %u) attempt move from grid[%u,%u]cell[%u,%u] to unloaded grid[%u,%u]cell[%u,%u].",
          go->GetGUIDLow(), go->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(),
          new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
      return;
    }
...
bool Map::CreatureCellRelocation(Creature *c, const Cell &new_cell) {
  ...
  if (old_cell != new_cell) {
    DEBUG_FILTER_LOG(LOG_FILTER_CREATURE_MOVES,
                     ...
                     c->GetGUIDLow(), c->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(),
                     old_cell.CellY(), new_cell.CellX(), new_cell.CellY());
    NGridType *oldGrid = getNGrid(old_cell.GridX(), old_cell.GridY());
    NGridType *newGrid = getNGrid(new_cell.GridX(), new_cell.GridY());
...
bool Map::CreatureRespawnRelocation(Creature *c) {
  ...
  DEBUG_FILTER_LOG(
      LOG_FILTER_CREATURE_MOVES,
      "Creature (GUID: %u Entry: %u) will moved from grid[%u,%u]cell[%u,%u] to respawn grid[%u,%u]cell[%u,%u].",
      c->GetGUIDLow(), c->GetEntry(), c->GetCurrentCell().GridX(), c->GetCurrentCell().GridY(),
      c->GetCurrentCell().CellX(), c->GetCurrentCell().CellY(), resp_cell.GridX(), resp_cell.GridY(), resp_cell.CellX(),
      resp_cell.CellY());
...
void Map::SendInitSelf(Player *player) const {
  ...
  // build other passengers at transport also (they always visible and marked as visible and will not send at visibility
  // update at add to map
  if (GenericTransport *transport = player->GetTransport()) {
...
void Map::AddToOnEventNotified(WorldObject *obj) { m_onEventNotifiedObjects.insert(obj); }
...
void Map::CreateInstanceData(bool load) {
  ...
  if (load) {
    ...
    if (result) {
      ...
      if (data) {
        DEBUG_LOG("Loading instance data for `%s` (Map: %u Instance: %u)", sScriptDevAIMgr.GetScriptName(i_script_id),
                  GetId(), i_InstanceId);
        i_data->Load(data);
      }
      delete result;
    } else {
      // for non-instanceable map always add data to table if not found, later code expected that for map in `word`
      // exist always after load
      if (!Instanceable())
...
DungeonMap::DungeonMap(uint32 id, time_t expiry, uint32 InstanceId, uint8 SpawnMode)
    : Map(id, expiry, InstanceId, SpawnMode), m_resetAfterUnload(false), m_unloadWhenEmpty(false), m_team(TEAM_NONE) {
...
bool DungeonMap::Add(Player *player) {
  ...
  if (playerBind && playerBind->perm) {
    ...
    if (playerBind->state != GetPersistanceState()) {
      sLog.outError(
          "DungeonMap::Add: player %s(%d) is permanently bound to instance %d,%d,%d,%d,%d,%d but he is being put in "
          "instance %d,%d,%d,%d,%d,%d",
          player->GetName(), player->GetGUIDLow(), playerBind->state->GetMapId(), playerBind->state->GetInstanceId(),
          playerBind->state->GetDifficulty(), playerBind->state->GetPlayerCount(), playerBind->state->GetGroupCount(),
          playerBind->state->CanReset(), GetPersistanceState()->GetMapId(), GetPersistanceState()->GetInstanceId(),
    ...
    if (pGroup) {
      ...
      if (playerBind) {
        sLog.outError(
            "DungeonMap::Add: %s is being put in instance %d,%d,%d,%d,%d,%d but he is in group (Id: %d) and is bound "
            "to instance %d,%d,%d,%d,%d,%d!",
            player->GetGuidStr().c_str(), GetPersistentState()->GetMapId(), GetPersistentState()->GetInstanceId(),
            GetPersistanceState()->GetDifficulty(), GetPersistanceState()->GetPlayerCount(),
            GetPersistanceState()->GetGroupCount(), GetPersistanceState()->CanReset(), pGroup->GetId(),
        ...
        if (groupBind)
          sLog.outError("DungeonMap::Add: the group (Id: %d) is bound to instance %d,%d,%d,%d,%d,%d", pGroup->GetId(),
                        groupBind->state->GetMapId(), groupBind->state->GetInstanceId(),
                        groupBind->state->GetDifficulty(), groupBind->state->GetPlayerCount(),
                        groupBind->state->GetGroupCount(), groupBind->state->CanReset());
      ...
      if (!groupBind)
        ...
        if (groupBind->state != GetPersistentState()) {
          sLog.outError("DungeonMap::Add: %s is being put in instance %d,%d,%d but he is in group (Id: %d) which is "
                        "bound to instance %d,%d,%d!",
                        player->GetGuidStr().c_str(), GetPersistentState()->GetMapId(),
                        GetPersistentState()->GetInstanceId(), GetPersistentState()->GetDifficulty(), pGroup->GetId(),
                        groupBind->state->GetMapId(), groupBind->state->GetInstanceId(),
                        groupBind->state->GetDifficulty());

          sLog.outError("MapSave players: %d, group count: %d", GetPersistanceState()->GetPlayerCount(),
                        GetPersistanceState()->GetGroupCount());

          if (groupBind->state)
            sLog.outError("GroupBind save players: %d, group count: %d", groupBind->state->GetPlayerCount(),
  ...
  DETAIL_LOG("MAP: Player '%s' is entering instance '%u' of map '%s'", player->GetName(), GetInstanceId(),
             GetMapName());
...
void DungeonMap::Update(const uint32 &t_diff) { Map::Update(t_diff); }

void DungeonMap::Remove(Player *player, bool remove) {
  DETAIL_LOG("MAP: Removing player '%s' from instance '%u' of map '%s' before relocating to other map",
...
void DungeonMap::SetResetSchedule(bool on) {
  ...
  if (!HavePlayers() && !IsRaidOrHeroicDungeon()) {
    ...
    sMapPersistentStateMgr.GetScheduler().ScheduleReset(
        on, resetTime, DungeonResetEvent(RESET_EVENT_NORMAL_DUNGEON, GetId(), GetInstanceId()));
  }
}
...
void BattleGroundMap::Initialize(bool) { Map::Initialize(false); }
...
void BattleGroundMap::Remove(Player *player, bool remove) {
  DETAIL_FILTER_LOG(LOG_FILTER_PLAYER_MOVES,
                    "MAP: Removing player '%s' from bg '%u' of map '%s' before relocating to other map",
                    player->GetName(), GetInstanceId(), GetMapName());
  Map::Remove(player, remove);
}
...
bool Map::ScriptsStart(ScriptMapMapName const &scripts, uint32 id, Object *source, Object *target,
                       ScriptExecutionParam execParams /*=SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE_TARGET*/) {
  ...
  {
    for (ScriptScheduleMap::const_iterator searchItr = m_scriptSchedule.begin(); searchItr != m_scriptSchedule.end();
         ++searchItr) {
      if (searchItr->second.IsSameScript(
              scripts.first, id, execParams & SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE ? sourceGuid : ObjectGuid(),
              execParams & SCRIPT_EXEC_PARAM_UNIQUE_BY_TARGET ? targetGuid : ObjectGuid(), ownerGuid)) {
        DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                          "DB-SCRIPTS: Process table `%s` id %u. Skip script as script already started for source %s, "
                          "target %s - ScriptsStartParams %u",
                          scripts.first, id, sourceGuid.GetString().c_str(), targetGuid.GetString().c_str(),
                          execParams);
        return true;
      }
    }
 ...
 */
Creature *Map::GetCreature(ObjectGuid guid) { return m_objectsStore.find<Creature>(guid, (Creature *)nullptr); }
 ...
 */
Pet *Map::GetPet(ObjectGuid guid) { return m_objectsStore.find<Pet>(guid, (Pet *)nullptr); }
 ...
 */
GameObject *Map::GetGameObject(ObjectGuid guid) { return m_objectsStore.find<GameObject>(guid, (GameObject *)nullptr); }
...
public:
  StaticMonsterChatBuilder(CreatureInfo const *cInfo, ChatMsg msgtype, int32 textId, Language language,
 ...
 */
void Map::MonsterYellToMap(ObjectGuid guid, int32 textId, ChatMsg chatMsg, Language language,
                           Unit const *target) const {
  if (guid.IsAnyTypeCreature()) {
    CreatureInfo const *cInfo = ObjectMgr::GetCreatureTemplate(guid.GetEntry());
    if (!cInfo) {
      sLog.outError("Map::MonsterYellToMap: Called for nonexistent creature entry in guid: %s",
 ...
 */
void Map::MonsterYellToMap(CreatureInfo const *cinfo, int32 textId, ChatMsg chatMsg, Language language,
                           Unit const *target, uint32 senderLowGuid /*= 0*/) const {
 ...
 */
bool Map::GetHitPosition(float srcX, float srcY, float srcZ, float &destX, float &destY, float &destZ,
                         float modifyDist) const {
  // at first check all static objects
  float tempX, tempY, tempZ = 0.0f;
  bool result0 = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(GetId(), srcX, srcY, srcZ, destX, destY,
                                                                              destZ, tempX, tempY, tempZ, modifyDist);
  if (result0) {
    // DEBUG_LOG("Map::GetHitPosition vmaps corrects gained with static objects! new dest coords are X:%f Y:%f Z:%f",
    // destX, destY, destZ);
    destX = tempX;
    destY = tempY;
    destZ = tempZ;
  ...
  if (result1) {
    // DEBUG_LOG("Map::GetHitPosition vmaps corrects gained with dynamic objects! new dest coords are X:%f Y:%f Z:%f",
    // destX, destY, destZ);
    destX = tempX;
    destY = tempY;
    destZ = tempZ;
...
bool Map::GetHeightInRange(float x, float y, float &z, float maxSearchDist /*= 4.0f*/) const {
  ...
  if (GridMap *gmap =
          const_cast<TerrainInfo *>(m_TerrainData)->GetGrid(x, y)) // TODO:: find a way to remove that const_cast
  ...
  if (diffVmaps < maxSearchDist) {
    if (diffMaps < maxSearchDist) {
      // well we simply have to take the highest as normally there we cannot be on top of cavern is maxSearchDist is not
      // too big
...
float Map::GetHeight(float x, float y, float z, bool swim) const {
  float staticHeight =
      m_TerrainData->GetHeightStatic(x, y, z, true, (swim ? DEFAULT_WATER_SEARCH : DEFAULT_HEIGHT_SEARCH));
...
void Map::InsertGameObjectModel(const GameObjectModel &mdl) { m_dyn_tree.insert(mdl); }

void Map::RemoveGameObjectModel(const GameObjectModel &mdl) { m_dyn_tree.remove(mdl); }

bool Map::ContainsGameObjectModel(const GameObjectModel &mdl) const { return m_dyn_tree.contains(mdl); }
...
bool Map::GetRandomPointUnderWater(float &x, float &y, float &z, float radius, GridMapLiquidData &liquid_status,
  ...
  if (ground > INVALID_HEIGHT) // GetHeight can fail
  {
    ...
    float liquidLevel =
        liquid_status.level - 2.0f; // just to make the generated point is in water and not on surface or a bit above
...
void Map::OnEventHappened(uint16 event_id, bool activate, bool resume) {
  ...
  for (m_onEventNotifiedIter = m_onEventNotifiedObjects.begin();
       m_onEventNotifiedIter != m_onEventNotifiedObjects.end(); ++m_onEventNotifiedIter)
    if ((*m_onEventNotifiedIter)->IsInWorld())
      (*m_onEventNotifiedIter)->OnEventHappened(event_id, activate, resume);
}

uint32 Map::SpawnedCountForEntry(uint32 entry) { return m_spawnedCount[entry].size(); }

void Map::AddToSpawnCount(const ObjectGuid &guid) { m_spawnedCount[guid.GetEntry()].insert(guid); }

void Map::RemoveFromSpawnCount(const ObjectGuid &guid) { m_spawnedCount[guid.GetEntry()].erase(guid); }
