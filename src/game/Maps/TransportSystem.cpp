...
TransportBase::~TransportBase() { MANGOS_ASSERT(m_passengers.empty()); }

// Update every now and then (after some change of transporter's position)
// This is used to calculate global positions (which don't have to be exact, they are only required for some server-side
// calculations
