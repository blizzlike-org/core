...
enum ScriptCommand // resSource, resTarget are the resulting Source/ Target after buddy search is done
{
  ...
  SCRIPT_COMMAND_QUEST_EXPLORED =
      7, // one from source or target must be Player, another GO/Creature, datalong=quest_id, datalong2=distance or 0
  SCRIPT_COMMAND_KILL_CREDIT = 8, // source or target with Player, datalong = creature entry (or 0 for target-entry),
  ...
  SCRIPT_COMMAND_TEMP_SPAWN_CREATURE = 10, // source = any, datalong=creature entry, datalong2=despawn_delay,
                                           // datalong3=pathId, text1 - setRun - 1, text2 - faction ID, text3 - modelId
  ...
  SCRIPT_COMMAND_ACTIVATE_OBJECT = 13, // source = unit, target=GO; data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL send
                                       // gameobject custom anim, datalong = animId
  ...
  SCRIPT_COMMAND_PLAY_SOUND =
      16, // resSource = WorldObject, target=any/player, datalong (sound_id), datalong2 (bitmask: 0/1=target-player,
          // 0/2=with distance dependent, 0/4=map wide, 0/8=zone wide; so 1|2 = 3 is target with distance dependent)
  ...
  SCRIPT_COMMAND_MOVEMENT = 20,     // resSource = Creature. datalong = MovementType (0:idle, 1:random or 2:waypoint),
                                    // datalong2 = wander-distance/pathId, datalong3 = timer
  ...
  SCRIPT_COMMAND_TERMINATE_COND = 34, // datalong = condition_id, datalong2 = if != 0 then quest_id of quest that will
                                      // be failed for player's group if the script is terminated
  ...
  // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL also set TargetGuid of resSource to resTarget. In this case resTarget
  // MUST be Creature/ Player datalong != 0 Reset TargetGuid, Reset orientation
  ...
  // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL Obtain a random point around resTarget in direction of
  // resTarget->GetOrientation + orientation for resTarget == resSource and orientation == 0 this will mean resSource
  // moving forward
  ...
  SCRIPT_COMMAND_RESPAWN =
      41, // resSource = Creature. Requires SCRIPT_FLAG_BUDDY_IS_DESPAWNED to find dead or despawned targets
...
#define MAX_TEXT_ID                                                                                                    \
  4 // used for SCRIPT_COMMAND_TALK, SCRIPT_COMMAND_EMOTE, SCRIPT_COMMAND_CAST_SPELL, SCRIPT_COMMAND_TERMINATE_SCRIPT
...
struct ScriptInfo {
  ...
  ScriptInfo()
      : id(0), delay(0), command(0), buddyEntry(0), searchRadiusOrGuid(0), data_flags(0), x(0), y(0), z(0), o(0),
        condition_id(0) {
  ...
  bool IsDeadOrDespawnedBuddy() const { return (data_flags & SCRIPT_FLAG_BUDDY_IS_DESPAWNED) != 0; }
...
class ScriptAction {
public:
  ScriptAction(const char *_table, Map *_map, ObjectGuid _sourceGuid, ObjectGuid _targetGuid, ObjectGuid _ownerGuid,
               ScriptInfo const *_script)
      : m_table(_table), m_map(_map), m_sourceGuid(_sourceGuid), m_targetGuid(_targetGuid), m_ownerGuid(_ownerGuid),
        m_script(_script) {}
  ...
  bool IsSameScript(const char *table, uint32 id, ObjectGuid sourceGuid, ObjectGuid targetGuid,
                    ObjectGuid ownerGuid) const {
    return table == m_table && id == GetId() && (sourceGuid == m_sourceGuid || !sourceGuid) &&
           (targetGuid == m_targetGuid || !targetGuid) && (ownerGuid == m_ownerGuid || !ownerGuid);
  }
...
private:
  ...
  bool GetScriptProcessTargets(WorldObject *pOrigSource, WorldObject *pOrigTarget, WorldObject *&pFinalSource,
                               WorldObject *&pFinalTarget) const;
...
public:
  ...
  bool CheckScriptStringTemplateId(uint32 id) const {
    return m_scriptTemplates[STRING_TEMPLATE].find(id) != m_scriptTemplates[STRING_TEMPLATE].end();
  }
  bool CheckScriptRelayTemplateId(uint32 id) const {
    return m_scriptTemplates[RELAY_TEMPLATE].find(id) != m_scriptTemplates[RELAY_TEMPLATE].end();
  }
  typedef std::pair<int32, uint32> ScriptTemplatePair;
  typedef std::vector<ScriptTemplatePair> ScriptTemplateVector;
  void GetScriptStringTemplate(uint32 id, ScriptTemplateVector &stringTemplate) {
    stringTemplate = m_scriptTemplates[STRING_TEMPLATE][id];
  }
  void GetScriptRelayTemplate(uint32 id, ScriptTemplateVector &stringTemplate) {
    stringTemplate = m_scriptTemplates[RELAY_TEMPLATE][id];
  }
