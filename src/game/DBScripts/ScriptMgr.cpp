...
uint8 GetSpellStartDBScriptPriority(SpellEntry const *spellinfo, SpellEffectIndex effIdx) {
  ...
  if (spellinfo->Effect[effIdx] == SPELL_EFFECT_TRIGGER_SPELL &&
      !sSpellTemplate.LookupEntry<SpellEntry>(spellinfo->EffectTriggerSpell[effIdx]))
    return 5;
...
// Priorize: SCRIPT_EFFECT before DUMMY before Non-Existing triggered spell, for same priority the first effect with the
// priority triggers
...
void ScriptMgr::LoadScripts(ScriptMapMapName &scripts, const char *tablename) {
  ...
  QueryResult *result = WorldDatabase.PQuery(
      "SELECT id, delay, command, datalong, datalong2, datalong3, buddy_entry, search_radius, data_flags, dataint, "
      "dataint2, dataint3, dataint4, x, y, z, o, condition_id FROM %s ORDER BY priority",
      tablename);
  ...
  do {
    ...
    if (tmp.condition_id && !sConditionStorage.LookupEntry<ConditionEntry>(tmp.condition_id)) {
      sLog.outErrorDb("Table `%s` has condition_id = %u in command %u for script id %u, but this condition does not "
                      "exist, skipping.",
                      tablename, tmp.condition_id, tmp.command, tmp.id);
      continue;
    }
    ...
    if (tmp.buddyEntry && !(tmp.data_flags & SCRIPT_FLAG_BUDDY_BY_GUID)) {
      if (tmp.IsCreatureBuddy() && !ObjectMgr::GetCreatureTemplate(tmp.buddyEntry)) {
        sLog.outErrorDb("Table `%s` has buddyEntry = %u in command %u for script id %u, but this creature_template "
                        "does not exist, skipping.",
                        tablename, tmp.buddyEntry, tmp.command, tmp.id);
        continue;
      }
      if (!tmp.IsCreatureBuddy() && !ObjectMgr::GetGameObjectInfo(tmp.buddyEntry)) {
        sLog.outErrorDb("Table `%s` has buddyEntry = %u in command %u for script id %u, but this gameobject_template "
                        "does not exist, skipping.",
                        tablename, tmp.buddyEntry, tmp.command, tmp.id);
        continue;
      }
      if (!tmp.searchRadiusOrGuid) {
        sLog.outErrorDb("Table `%s` has searchRadius = 0 in command %u for script id %u for buddy %u, skipping.",
    ...
    if (tmp.data_flags) // Check flags
    {
      if (tmp.data_flags & ~MAX_SCRIPT_FLAG_VALID) {
        sLog.outErrorDb("Table `%s` has invalid data_flags %u in command %u for script id %u, skipping.", tablename,
                        tmp.data_flags, tmp.command, tmp.id);
        continue;
      }
      ...
      if ((tmp.data_flags & SCRIPT_FLAG_BUDDY_AS_TARGET) != 0 && (tmp.data_flags & SCRIPT_FLAG_BUDDY_BY_POOL) == 0 &&
          !tmp.buddyEntry) {
        sLog.outErrorDb("Table `%s` has buddy required in data_flags %u in command %u for script id %u, but no buddy "
                        "defined, skipping.",
      ...
      if (tmp.data_flags & SCRIPT_FLAG_BUDDY_BY_GUID) // Check guid
      {
        if (tmp.IsCreatureBuddy()) {
          ...
          if (!data) {
            sLog.outErrorDb("Table `%s` has buddy defined by guid (SCRIPT_FLAG_BUDDY_BY_GUID %u set) but no npc "
                            "spawned with guid %u, skipping.",
                            tablename, SCRIPT_FLAG_BUDDY_BY_GUID, tmp.searchRadiusOrGuid);
            continue;
          }
          if (data->id != tmp.buddyEntry) {
            sLog.outErrorDb("Table `%s` has buddy defined by guid (SCRIPT_FLAG_BUDDY_BY_GUID %u set) but spawned npc "
                            "with guid %u has entry %u, expected buddy_entry is %u, skipping.",
                            tablename, SCRIPT_FLAG_BUDDY_BY_GUID, tmp.searchRadiusOrGuid, data->id, tmp.buddyEntry);
            continue;
          }
        } else {
          GameObjectData const *data = sObjectMgr.GetGOData(tmp.searchRadiusOrGuid);
          if (!data) {
            sLog.outErrorDb("Table `%s` has go-buddy defined by guid (SCRIPT_FLAG_BUDDY_BY_GUID %u set) but no go "
                            "spawned with guid %u, skipping.",
                            tablename, SCRIPT_FLAG_BUDDY_BY_GUID, tmp.searchRadiusOrGuid);
            continue;
          }
          if (data->id != tmp.buddyEntry) {
            sLog.outErrorDb("Table `%s` has go-buddy defined by guid (SCRIPT_FLAG_BUDDY_BY_GUID %u set) but spawned go "
                            "with guid %u has entry %u, expected buddy_entry is %u, skipping.",
                            tablename, SCRIPT_FLAG_BUDDY_BY_GUID, tmp.searchRadiusOrGuid, data->id, tmp.buddyEntry);
            continue;
          }
        ...
        if (tmp.IsCreatureBuddy()) {
          ...
          if (pool.isEmpty()) {
            sLog.outErrorDb("Table `%s` has go-buddy defined by pool (SCRIPT_FLAG_BUDDY_BY_POOL %u set) but pool %u is "
                            "empty, skipping.",
                            tablename, tmp.data_flags, tmp.searchRadiusOrGuid);
            continue;
          }
        } else {
      ...
      if (!tmp.talk.stringTemplateId) // template checked later after loading strings
      {
        ...
        for (int i = 0; i < MAX_TEXT_ID; ++i) {
          if (tmp.textId[i] && (tmp.textId[i] < MIN_DB_SCRIPT_STRING_ID || tmp.textId[i] >= MAX_DB_SCRIPT_STRING_ID)) {
            sLog.outErrorDb("Table `%s` has out of range text_id%u (dataint = %i expected %u-%u) in "
                            "SCRIPT_COMMAND_TALK for script id %u",
                            tablename, i + 1, tmp.textId[i], MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID, tmp.id);
          }
        }
      }
    ...
    case SCRIPT_COMMAND_EMOTE: // 1
    {
      ...
      for (int i = 0; i < MAX_TEXT_ID; ++i) {
        if (tmp.textId[i] && !sEmotesStore.LookupEntry(tmp.textId[i])) {
          sLog.outErrorDb("Table `%s` has invalid emote id (text_id%u = %u) in SCRIPT_COMMAND_EMOTE for script id %u",
                          tablename, i + 1, tmp.textId[i], tmp.id);
        }
      }
      break;
    }
    ...
    case SCRIPT_COMMAND_TELEPORT_TO: // 6
    {
      if (!sMapStore.LookupEntry(tmp.teleportTo.mapId)) {
        sLog.outErrorDb("Table `%s` has invalid map (Id: %u) in SCRIPT_COMMAND_TELEPORT_TO for script id %u", tablename,
                        tmp.teleportTo.mapId, tmp.id);
        continue;
      }

      if (!MaNGOS::IsValidMapCoord(tmp.x, tmp.y, tmp.z, tmp.o)) {
        sLog.outErrorDb(
            "Table `%s` has invalid coordinates (X: %f Y: %f) in SCRIPT_COMMAND_TELEPORT_TO for script id %u",
    ...
    case SCRIPT_COMMAND_QUEST_EXPLORED: // 7
    {
      ...
      if (!quest->HasSpecialFlag(QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT)) {
        sLog.outErrorDb("Table `%s` has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id "
                        "%u, but quest not have flag QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT in quest flags. Script "
                        "command or quest flags wrong. Quest modified to require objective.",
                        tablename, tmp.questExplored.questId, tmp.id);
      ...
      if (float(tmp.questExplored.distance) > DEFAULT_VISIBILITY_DISTANCE) {
        sLog.outErrorDb("Table `%s` has too large distance (%u) for exploring objective complete in `datalong2` in "
      ...
      if (tmp.questExplored.distance && float(tmp.questExplored.distance) > DEFAULT_VISIBILITY_DISTANCE) {
        sLog.outErrorDb("Table `%s` has too large distance (%u) for exploring objective complete in `datalong2` in "
                        "SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, max distance is %f or 0 for "
      ...
      if (tmp.questExplored.distance && float(tmp.questExplored.distance) < INTERACTION_DISTANCE) {
        sLog.outErrorDb("Table `%s` has too small distance (%u) for exploring objective complete in `datalong2` in "
                        "SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, min distance is %f or 0 for "
                        "disable distance check",
    ...
    case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT: // 9
    {
      ...
      if (!tmp.GetGOGuid()) {
        if (!tmp.buddyEntry) {
          sLog.outErrorDb(
              "Table `%s` has no gameobject nor buddy defined in SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id %u",
              tablename, tmp.id);
          continue;
        }
      ...
      if (!info) {
        sLog.outErrorDb("Table `%s` has gameobject with invalid entry (GUID: %u Entry: %u) in "
      ...
      if (info->type == GAMEOBJECT_TYPE_FISHINGNODE || info->type == GAMEOBJECT_TYPE_FISHINGHOLE ||
        ...
        sLog.outErrorDb("Table `%s` have gameobject type (%u) unsupported by command SCRIPT_COMMAND_RESPAWN_GAMEOBJECT "
                        "for script id %u",
                        tablename, info->type, tmp.id);
        continue;
      }
      break;
    }
    case SCRIPT_COMMAND_TEMP_SPAWN_CREATURE: // 10
    {
      if (!MaNGOS::IsValidMapCoord(tmp.x, tmp.y, tmp.z, tmp.o)) {
        sLog.outErrorDb(
            "Table `%s` has invalid coordinates (X: %f Y: %f) in SCRIPT_COMMAND_TEMP_SPAWN_CREATURE for script id %u",
            tablename, tmp.x, tmp.y, tmp.id);
        continue;
      }
      ...
      if (!ObjectMgr::GetCreatureTemplate(tmp.summonCreature.creatureEntry)) {
        sLog.outErrorDb(
            "Table `%s` has invalid creature (Entry: %u) in SCRIPT_COMMAND_TEMP_SPAWN_CREATURE for script id %u",
    ...
    case SCRIPT_COMMAND_CLOSE_DOOR: // 12
    {
      ...
      if (!tmp.GetGOGuid()) {
        if (!tmp.buddyEntry) {
          sLog.outErrorDb(
              "Table `%s` has no gameobject nor buddy defined in %s for script id %u", tablename,
              (tmp.command == SCRIPT_COMMAND_OPEN_DOOR ? "SCRIPT_COMMAND_OPEN_DOOR" : "SCRIPT_COMMAND_CLOSE_DOOR"),
              tmp.id);
          continue;
        }
        goEntry = tmp.buddyEntry;
        ...
        if (!data) {
          sLog.outErrorDb(
              "Table `%s` has invalid gameobject (GUID: %u) in %s for script id %u", tablename, tmp.GetGOGuid(),
              (tmp.command == SCRIPT_COMMAND_OPEN_DOOR ? "SCRIPT_COMMAND_OPEN_DOOR" : "SCRIPT_COMMAND_CLOSE_DOOR"),
              tmp.id);
          continue;
        }
      ...
      if (!info) {
        sLog.outErrorDb(
            "Table `%s` has gameobject with invalid entry (GUID: %u Entry: %u) in %s for script id %u", tablename,
            tmp.GetGOGuid(), goEntry,
            (tmp.command == SCRIPT_COMMAND_OPEN_DOOR ? "SCRIPT_COMMAND_OPEN_DOOR" : "SCRIPT_COMMAND_CLOSE_DOOR"),
            tmp.id);
        continue;
      }
      ...
      if (info->type != GAMEOBJECT_TYPE_DOOR) {
        sLog.outErrorDb(
            "Table `%s` has gameobject type (%u) non supported by command %s for script id %u", tablename, info->id,
            (tmp.command == SCRIPT_COMMAND_OPEN_DOOR ? "SCRIPT_COMMAND_OPEN_DOOR" : "SCRIPT_COMMAND_CLOSE_DOOR"),
            tmp.id);
        continue;
      }
    ...
    case SCRIPT_COMMAND_REMOVE_AURA: // 14
    {
      if (!sSpellTemplate.LookupEntry<SpellEntry>(tmp.removeAura.spellId)) {
        sLog.outErrorDb("Table `%s` using nonexistent spell (id: %u) in SCRIPT_COMMAND_REMOVE_AURA or "
    ...
    case SCRIPT_COMMAND_CAST_SPELL: // 15
    {
      ...
      for (uint8 i = 0; i < MAX_TEXT_ID; ++i) {
        if (tmp.textId[i] && !sSpellTemplate.LookupEntry<SpellEntry>(uint32(tmp.textId[i]))) {
          sLog.outErrorDb(
              "Table `%s` using nonexistent spell (id: %u) in SCRIPT_COMMAND_CAST_SPELL for script id %u, dataint%u",
    ...
    case SCRIPT_COMMAND_PLAY_SOUND: // 16
    {
      ...
      if (tmp.playSound.flags & ~(1 | 2 | 4 | 8))
        sLog.outErrorDb("Table `%s` using unsupported sound flags (datalong2: %u) in SCRIPT_COMMAND_PLAY_SOUND for "
                        "script id %u, unsupported flags will be ignored",
                        tablename, tmp.playSound.flags, tmp.id);
      if ((tmp.playSound.flags & (1 | 2)) > 0 && (tmp.playSound.flags & (4 | 8)) > 0)
        sLog.outErrorDb("Table `%s` uses sound flags (datalong2: %u) in SCRIPT_COMMAND_PLAY_SOUND for script id %u, "
                        "combining (1|2) with (4|8) makes no sense",
                        tablename, tmp.playSound.flags, tmp.id);
      break;
    }
    case SCRIPT_COMMAND_CREATE_ITEM: // 17
    {
      ...
      if (!tmp.createItem.amount) {
        sLog.outErrorDb("Table `%s` SCRIPT_COMMAND_CREATE_ITEM but amount is %u for script id %u", tablename,
                        tmp.createItem.amount, tmp.id);
        continue;
      }
      break;
    }
    ...
    {
      sLog.outErrorDb("Table `%s` use unsupported SCRIPT_COMMAND_PLAY_MOVIE for script id %u", tablename, tmp.id);
      continue;
    }
    case SCRIPT_COMMAND_MOVEMENT: // 20
    {
      if (tmp.movement.movementType >= MAX_DB_MOTION_TYPE) {
        sLog.outErrorDb("Table `%s` SCRIPT_COMMAND_MOVEMENT has invalid MovementType %u for script id %u", tablename,
                        tmp.movement.movementType, tmp.id);
        continue;
      }
    ...
    case SCRIPT_COMMAND_SET_FACTION: // 22
    {
      if (tmp.faction.factionId && !sFactionTemplateStore.LookupEntry(tmp.faction.factionId)) {
        sLog.outErrorDb("Table `%s` has datalong = %u in SCRIPT_COMMAND_SET_FACTION for script id %u, but this "
    ...
    case SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL: // 23
    {
      if (tmp.data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL) {
        if (tmp.morph.creatureOrModelEntry && !sCreatureDisplayInfoStore.LookupEntry(tmp.morph.creatureOrModelEntry)) {
          sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL for script id %u, "
    ...
    case SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL: // 24
    {
      if (tmp.data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL) {
        if (tmp.mount.creatureOrModelEntry && !sCreatureDisplayInfoStore.LookupEntry(tmp.mount.creatureOrModelEntry)) {
          sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL for script id %u, "
                          "but this model does not exist.",
    ...
    case SCRIPT_COMMAND_GO_LOCK_STATE: // 27
    {
      if ( // lock(0x01) and unlock(0x02) together
        ...
        sLog.outErrorDb(
            "Table `%s` has invalid lock state (datalong = %u) in SCRIPT_COMMAND_GO_LOCK_STATE for script id %u.",
    ...
    case SCRIPT_COMMAND_STAND_STATE: // 28
    {
      if (tmp.standState.stand_state >= MAX_UNIT_STAND_STATE) {
        sLog.outErrorDb(
            "Table `%s` has invalid stand state (datalong = %u) in SCRIPT_COMMAND_STAND_STATE for script id %u",
    ...
    case SCRIPT_COMMAND_MODIFY_NPC_FLAGS: // 29
    {
      if (tmp.npcFlag.change_flag > 2)
        sLog.outErrorDb(
            "Table `%s` has invalid change flag (datalong2 = %u) in SCRIPT_COMMAND_MODIFY_NPC_FLAGS for script id %u",
            tablename, tmp.npcFlag.change_flag, tmp.id);
      break;
    }
    case SCRIPT_COMMAND_SEND_TAXI_PATH: // 30
    {
      if (!sTaxiPathStore.LookupEntry(tmp.sendTaxiPath.taxiPathId)) {
        sLog.outErrorDb("Table `%s` has datalong = %u in SCRIPT_COMMAND_SEND_TAXI_PATH for script id %u, but this taxi "
                        "path does not exist.",
                        tablename, tmp.sendTaxiPath.taxiPathId, tmp.id);
        continue;
      }
      ...
      if (!sLog.HasLogFilter(LOG_FILTER_DB_STRICTED_CHECK)) {
        ...
        if (taxiSpell) {
          sLog.outErrorDb("Table `%s` has datalong = %u in SCRIPT_COMMAND_SEND_TAXI_PATH for script id %u, but this "
                          "taxi path can be triggered by spell %u.",
                          tablename, tmp.sendTaxiPath.taxiPathId, tmp.id, taxiSpell);
          continue;
        }
      }
      break;
    ...
    case SCRIPT_COMMAND_TERMINATE_SCRIPT: // 31
    {
      if (tmp.terminateScript.npcEntry && !ObjectMgr::GetCreatureTemplate(tmp.terminateScript.npcEntry)) {
        sLog.outErrorDb("Table `%s` has npc entry = '%u' in SCRIPT_COMMAND_TERMINATE_SCRIPT for script id %u, but this "
                        "npc entry does not exist.",
                        tablename, tmp.terminateScript.npcEntry, tmp.id);
        continue;
      }
      if (tmp.terminateScript.poolId && tmp.terminateScript.poolId > sPoolMgr.GetMaxPoolId()) {
        sLog.outErrorDb("Table `%s` has pool id = '%u' in SCRIPT_COMMAND_TERMINATE_SCRIPT for script id %u, but this "
                        "pool id does not exist.",
                        tablename, tmp.terminateScript.poolId, tmp.id);
        continue;
      }
      break;
    }
    ...
    case SCRIPT_COMMAND_TERMINATE_COND: // 34
    {
      if (!sConditionStorage.LookupEntry<ConditionEntry>(tmp.terminateCond.conditionId)) {
        sLog.outErrorDb("Table `%s` has datalong = %u in SCRIPT_COMMAND_TERMINATE_COND for script id %u, but this "
                        "condition_id does not exist.",
                        tablename, tmp.terminateCond.conditionId, tmp.id);
        continue;
      }
      if (tmp.terminateCond.failQuest && !sObjectMgr.GetQuestTemplate(tmp.terminateCond.failQuest)) {
        sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_TERMINATE_COND for script id %u, but this "
                        "questId does not exist.",
                        tablename, tmp.terminateCond.failQuest, tmp.id);
        continue;
      }
      break;
    }
    ...
    case SCRIPT_COMMAND_MOVE_DYNAMIC: // 37
    {
      if (tmp.moveDynamic.maxDist < tmp.moveDynamic.minDist) {
        sLog.outErrorDb("Table `%s` has invalid min-dist (datalong2 = %u) less than max-dist (datalong = %u) in "
                        "SCRIPT_COMMAND_MOVE_DYNAMIC for script id %u",
                        tablename, tmp.moveDynamic.minDist, tmp.moveDynamic.maxDist, tmp.id);
        continue;
      }
      break;
    }
    case SCRIPT_COMMAND_SEND_MAIL: // 38
    {
      if (!sMailTemplateStore.LookupEntry(tmp.sendMail.mailTemplateId)) {
        sLog.outErrorDb(
            "Table `%s` has invalid mailTemplateId (datalong = %u) in SCRIPT_COMMAND_SEND_MAIL for script id %u",
            tablename, tmp.sendMail.mailTemplateId, tmp.id);
        continue;
      }
      if (tmp.sendMail.altSender && !ObjectMgr::GetCreatureTemplate(tmp.sendMail.altSender)) {
        sLog.outErrorDb(
            "Table `%s` has invalid alternativeSender (datalong2 = %u) in SCRIPT_COMMAND_SEND_MAIL for script id %u",
            tablename, tmp.sendMail.altSender, tmp.id);
        continue;
      }
      break;
    }
    ...
    case SCRIPT_COMMAND_SET_EQUIPMENT_SLOTS: // 42
    {
      if (tmp.textId[0] < 0 || tmp.textId[1] < 0 || tmp.textId[2] < 0) {
        sLog.outErrorDb("Table `%s` has invalid equipment slot (dataint = %u, dataint2 = %u, dataint3 = %u) in "
                        "SCRIPT_COMMAND_SET_EQUIPMENT_SLOTS for script id %u",
                        tablename, tmp.textId[0], tmp.textId[1], tmp.textId[2], tmp.id);
        continue;
      }
      break;
    }
    ...
    case SCRIPT_COMMAND_UPDATE_TEMPLATE: // 44
    {
      if (!sCreatureStorage.LookupEntry<CreatureInfo>(tmp.updateTemplate.newTemplate)) {
        sLog.outErrorDb(
            "Table `%s` uses nonexistent creature entry %u in SCRIPT_COMMAND_UPDATE_TEMPLATE for script id %u.",
            tablename, tmp.updateTemplate.newTemplate, tmp.id);
        continue;
      }
      break;
    }
    case SCRIPT_COMMAND_START_RELAY_SCRIPT: // 45
    {
      if (strcmp(scripts.first, "dbscripts_on_relay") != 0) // Relay scripts are done first and checked after load
      {
        if (tmp.relayScript.relayId) {
          if (sRelayScripts.second.find(tmp.relayScript.relayId) == sRelayScripts.second.end()) {
            sLog.outErrorDb("Table `dbscripts_on_relay` uses nonexistent relay ID %u in "
                            "SCRIPT_COMMAND_START_RELAY_SCRIPT for script id %u.",
                            tmp.relayScript.relayId, tmp.id);
            continue;
          }
        }
      }
      if (tmp.relayScript.templateId) {
        if (!sScriptMgr.CheckScriptRelayTemplateId(tmp.relayScript.templateId)) {
          sLog.outErrorDb("Table `dbscripts_on_relay` uses nonexistent dbscript_random_template ID %u in "
                          "SCRIPT_COMMAND_START_RELAY_SCRIPT for script id %u.",
                          tmp.relayScript.relayId, tmp.id);
          continue;
        }
    ...
    case SCRIPT_COMMAND_CAST_CUSTOM_SPELL: // 46
    {
      if (!sSpellTemplate.LookupEntry<SpellEntry>(tmp.castSpell.spellId)) {
        sLog.outErrorDb(
            "Table `%s` using nonexistent spell (id: %u) in SCRIPT_COMMAND_CAST_CUSTOM_SPELL for script id %u",
            tablename, tmp.castSpell.spellId, tmp.id);
        continue;
      }
      if (tmp.textId[0] == 0 && tmp.textId[1] == 0 && tmp.textId[2] == 0) {
        sLog.outErrorDb("Table `%s` has invalid BP values (dataint = %u, dataint2 = %u, dataint3 = %u) in "
                        "SCRIPT_COMMAND_CAST_CUSTOM_SPELL for script id %u. At least one field has to be populated.",
                        tablename, tmp.textId[0], tmp.textId[1], tmp.textId[2], tmp.id);
        continue;
      }
      break;
    ...
    case SCRIPT_COMMAND_INTERRUPT_SPELL: // 47
    {
      if (tmp.interruptSpell.currentSpellType >= CURRENT_MAX_SPELL) {
        sLog.outErrorDb(
            "Table `%s` uses invalid current spell type %u (must be smaller or equal to %u) for script id %u.",
            tablename, tmp.interruptSpell.currentSpellType, CURRENT_MAX_SPELL - 1, tmp.id);
        continue;
      }
      break;
    }
    case SCRIPT_COMMAND_MODIFY_UNIT_FLAGS: // 48
    {
      if (tmp.unitFlag.change_flag > 2)
        sLog.outErrorDb(
            "Table `%s` has invalid change flag (datalong2 = %u) in SCRIPT_COMMAND_MODIFY_UNIT_FLAGS for script id %u",
            tablename, tmp.unitFlag.change_flag, tmp.id);
      break;
    }
    default: {
...
void ScriptMgr::LoadGameObjectScripts() {
  ...
  // check ids
  for (ScriptMapMap::const_iterator itr = sGameObjectScripts.second.begin(); itr != sGameObjectScripts.second.end();
       ++itr) {
...
void ScriptMgr::LoadGameObjectTemplateScripts() {
  ...
  // check ids
  for (ScriptMapMap::const_iterator itr = sGameObjectTemplateScripts.second.begin();
       itr != sGameObjectTemplateScripts.second.end(); ++itr) {
    if (!sObjectMgr.GetGameObjectInfo(itr->first))
      sLog.outErrorDb("Table `dbscripts_on_go_template_use` has not existing gameobject (Entry: %u) as script id",
                      itr->first);
  }
}
...
void ScriptMgr::LoadQuestEndScripts() {
  ...
  // check ids
  for (ScriptMapMap::const_iterator itr = sQuestEndScripts.second.begin(); itr != sQuestEndScripts.second.end();
       ++itr) {
...
void ScriptMgr::LoadQuestStartScripts() {
  ...
  // check ids
  for (ScriptMapMap::const_iterator itr = sQuestStartScripts.second.begin(); itr != sQuestStartScripts.second.end();
       ++itr) {
    if (!sObjectMgr.GetQuestTemplate(itr->first))
...
void ScriptMgr::LoadEventScripts() {
  ...
  for (ScriptMapMap::const_iterator itr = sEventScripts.second.begin(); itr != sEventScripts.second.end(); ++itr) {
    ...
    if (itr2 == eventIds.end())
      sLog.outErrorDb("Table `dbscripts_on_event` has script (Id: %u) not referring to any fitting gameobject_template "
                      "or any spell effect %u or path taxi node data",
...
void ScriptMgr::LoadCreatureDeathScripts() {
  ...
  // check ids
  for (ScriptMapMap::const_iterator itr = sCreatureDeathScripts.second.begin();
       itr != sCreatureDeathScripts.second.end(); ++itr) {
    if (!sObjectMgr.GetCreatureTemplate(itr->first))
      sLog.outErrorDb("Table `dbscripts_on_creature_death` has not existing creature (Entry: %u) as script id",
                      itr->first);
  }
}
      ...
      if (data.second.command == SCRIPT_COMMAND_START_RELAY_SCRIPT) {
        ...
        if (data.second.relayScript.relayId) {
          if (sRelayScripts.second.find(data.second.relayScript.relayId) == sRelayScripts.second.end()) {
            sLog.outErrorDb("Table `dbscripts_on_relay` uses nonexistent relay ID %u in "
                            "SCRIPT_COMMAND_START_RELAY_SCRIPT for script id %u.",
                            data.second.relayScript.relayId, data.second.id);
            hasErrored = true;
          }
        }
...
void ScriptMgr::LoadDbScriptStrings() {
  sObjectMgr.LoadMangosStrings(WorldDatabase, "dbscript_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID,
                               true);
  ...
  if (result) {
    do {
      ...
      if (type < MAX_TYPE) {
        ...
        sLog.outErrorDb("Table `dbscript_random_templates` entry (%u) uses invalid type (%u). Won't be used.", id,
                        type);
    } while (result->NextRow());
  ...
  for (auto &templateData : m_scriptTemplates[RELAY_TEMPLATE])
    for (auto &data : templateData.second)
      if (data.first && sRelayScripts.second.find(data.first) == sRelayScripts.second.end())
        sLog.outErrorDb("Table `dbscript_random_templates` entry (%u) uses nonexistent relay ID (%u).",
      ...
      if (itrM->second.command == SCRIPT_COMMAND_TALK) {
        for (int i : itrM->second.textId) {
          if (i && !sObjectMgr.GetMangosStringLocale(i))
            sLog.outErrorDb("Table `dbscript_string` is missing string id %u, used in database script table %s id %u.",
                            i, scripts.first, itrMM->first);
        ...
        if (itrM->second.talk.stringTemplateId) {
          ...
          for (auto &data : vector) {
            if (!sObjectMgr.GetMangosStringLocale(data.first))
              sLog.outErrorDb("Table `dbscript_string` is missing string id %d, used in database script template table "
                              "dbscript_string_template id %u.",
                              data.first, itrM->second.talk.stringTemplateId);
          }
        }
      }
    }
...
bool ScriptAction::GetScriptCommandObject(const ObjectGuid guid, bool includeItem, Object *&resultObject) const {
  ...
  default:
    sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u with unsupported guid %s, skipping", m_table,
                    m_script->id, m_script->command, guid.GetString().c_str());
    return false;
  }
...
bool ScriptAction::GetScriptProcessTargets(WorldObject *pOrigSource, WorldObject *pOrigTarget,
                                           WorldObject *&pFinalSource, WorldObject *&pFinalTarget) const {
  WorldObject *pBuddy = nullptr;

  if (m_script->buddyEntry || (m_script->data_flags & SCRIPT_FLAG_BUDDY_BY_POOL) != 0 ||
      (m_script->data_flags & SCRIPT_FLAG_BUDDY_BY_GUID) != 0) {
    if (m_script->data_flags & SCRIPT_FLAG_BUDDY_BY_GUID) {
      if (m_script->IsCreatureBuddy()) {
        ...
        if (pBuddy && ((Creature *)pBuddy)->IsAlive() == m_script->IsDeadOrDespawnedBuddy()) {
          if (m_script->command != SCRIPT_COMMAND_TERMINATE_SCRIPT) {
            sLog.outError(" DB-SCRIPTS: Process table `%s` id %u, command %u has buddy %u by guid %u but buddy is "
                          "dead, skipping.",
                          m_table, m_script->id, m_script->command, pBuddy->GetEntry(), m_script->searchRadiusOrGuid);
            return false;
          } else
            pBuddy = nullptr;
        }
      ...
      if (!pBuddy && m_script->command != SCRIPT_COMMAND_TERMINATE_SCRIPT) {
        DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                          " DB-SCRIPTS: Process table `%s` id %u, command %u has buddy by guid %u not loaded in map %u "
                          "(data-flags %u), skipping.",
                          m_table, m_script->id, m_script->command, m_script->searchRadiusOrGuid, m_map->GetId(),
                          m_script->data_flags);
        return false;
      }
      ...
      if (!pBuddy) {
        sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u has buddy %u by pool id %u and no creature "
                        "found in map %u (data-flags %u), skipping.",
                        m_table, m_script->id, m_script->command, m_script->buddyEntry, m_script->searchRadiusOrGuid,
                        m_map->GetId(), m_script->data_flags);
        return false;
      }
    } else // Buddy by entry
    {
      if (!pOrigSource && !pOrigTarget) {
        sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u called without buddy %u, but no source for "
                        "search available, skipping.",
      ...
      if (m_script->IsCreatureBuddy()) {
        ...
        if (m_script->IsDeadOrDespawnedBuddy()) {
          MaNGOS::AllCreaturesOfEntryInRangeCheck u_check(pSearcher, m_script->buddyEntry,
                                                          m_script->searchRadiusOrGuid);
          ...
          MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(
              *pSearcher, m_script->buddyEntry, true, false, m_script->searchRadiusOrGuid, true);
          MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(
              pCreatureBuddy, u_check);
        ...
        if (!pBuddy && pSearcher->GetEntry() == m_script->buddyEntry) {
          sLog.outErrorDb(" DB-SCRIPTS: WARNING: Process table `%s` id %u, command %u has no OTHER buddy %u found - "
                          "maybe you need to update the script?",
                          m_table, m_script->id, m_script->command, m_script->buddyEntry);
          pBuddy = pSearcher;
        }
      } else {
        ...
        MaNGOS::NearestGameObjectEntryInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry,
                                                                 m_script->searchRadiusOrGuid);
      ...
      if (!pBuddy && m_script->command != SCRIPT_COMMAND_TERMINATE_SCRIPT) {
        sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u has buddy %u not found in range %u of "
                        "searcher %s (data-flags %u), skipping.",
                        m_table, m_script->id, m_script->command, m_script->buddyEntry, m_script->searchRadiusOrGuid,
...
bool ScriptAction::LogIfNotCreature(WorldObject *pWorldObject) const {
  if (!pWorldObject || pWorldObject->GetTypeId() != TYPEID_UNIT) {
    sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u call for non-creature, skipping.", m_table,
                    m_script->id, m_script->command);
    return true;
  }
  return false;
}
bool ScriptAction::LogIfNotUnit(WorldObject *pWorldObject) const {
  if (!pWorldObject || !pWorldObject->isType(TYPEMASK_UNIT)) {
    sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u call for non-unit, skipping.", m_table,
                    m_script->id, m_script->command);
    return true;
  }
  return false;
...
bool ScriptAction::LogIfNotGameObject(WorldObject *pWorldObject) const {
  if (!pWorldObject || pWorldObject->GetTypeId() != TYPEID_GAMEOBJECT) {
    sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u call for non-gameobject, skipping.", m_table,
                    m_script->id, m_script->command);
    return true;
  }
  return false;
...
bool ScriptAction::LogIfNotPlayer(WorldObject *pWorldObject) const {
  if (!pWorldObject || pWorldObject->GetTypeId() != TYPEID_PLAYER) {
    sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u call for non-player, skipping.", m_table,
                    m_script->id, m_script->command);
    return true;
  }
  return false;
...
bool ScriptAction::HandleScriptStep() {
...

  {
    ...
    DETAIL_FILTER_LOG(
        LOG_FILTER_DB_SCRIPT,
        "DB-SCRIPTS: Process table `%s` id %u, command %u for source %s (%sin world), target %s (%sin world)", m_table,
        m_script->id, m_script->command, m_sourceGuid.GetString().c_str(), source ? "" : "not ",
  ...
  if (m_script->condition_id &&
      !sObjectMgr.IsConditionSatisfied(m_script->condition_id, pTarget, m_map, pSource, CONDITION_FROM_DBSCRIPTS))
    return false;
  ...
  {
    if (!pSource) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u found no worldobject as source, skipping.",
    ...
    if (!DoDisplayText(pSource, textId, unitTarget))
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, could not display text %i properly", m_table,
                      m_script->id, textId);
    break;
  }
  case SCRIPT_COMMAND_EMOTE: // 1
  ...
  case SCRIPT_COMMAND_FIELD_SET: // 2
    if (!pSourceOrItem) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u call for nullptr object.", m_table,
                      m_script->id, m_script->command);
      break;
    }
    if (m_script->setField.fieldId <= OBJECT_FIELD_ENTRY ||
        m_script->setField.fieldId >= pSourceOrItem->GetValuesCount()) {
      sLog.outErrorDb(
          " DB-SCRIPTS: Process table `%s` id %u, command %u call for wrong field %u (max count: %u) in %s.", m_table,
          m_script->id, m_script->command, m_script->setField.fieldId, pSourceOrItem->GetValuesCount(),
          pSourceOrItem->GetGuidStr().c_str());
      break;
    }
  ...
  case SCRIPT_COMMAND_MOVE_TO: // 3
  {
    ...
    if (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL) {
      ((Unit *)pSource)
          ->NearTeleportTo(m_script->x, m_script->y, m_script->z,
                           m_script->o != 0.0f ? m_script->o : ((Unit *)pSource)->GetOrientation());
      break;
    }
    ...
    if (m_script->moveTo.travelSpeed)
      ((Unit *)pSource)
          ->GetMotionMaster()
          ->MoveCharge(m_script->x, m_script->y, m_script->z, m_script->moveTo.travelSpeed * 0.01f, 0);
    else {
  ...
  case SCRIPT_COMMAND_FLAG_SET: // 4
    ...
    if (m_script->setFlag.fieldId <= OBJECT_FIELD_ENTRY ||
        m_script->setFlag.fieldId >= pSourceOrItem->GetValuesCount()) {
      sLog.outErrorDb("SCRIPT_COMMAND_FLAG_SET (script id %u) call for wrong field %u (max count: %u) in %s.",
                      m_script->id, m_script->setFlag.fieldId, pSourceOrItem->GetValuesCount(),
                      pSourceOrItem->GetGuidStr().c_str());
      break;
    }
  ...
  case SCRIPT_COMMAND_FLAG_REMOVE: // 5
    ...
    if (m_script->removeFlag.fieldId <= OBJECT_FIELD_ENTRY ||
        m_script->removeFlag.fieldId >= pSourceOrItem->GetValuesCount()) {
      sLog.outErrorDb("SCRIPT_COMMAND_FLAG_REMOVE (script id %u) call for wrong field %u (max count: %u) in %s.",
                      m_script->id, m_script->removeFlag.fieldId, pSourceOrItem->GetValuesCount(),
                      pSourceOrItem->GetGuidStr().c_str());
      break;
    }
  ...
  case SCRIPT_COMMAND_QUEST_EXPLORED: // 7
  {
    ...
    if (m_script->questExplored.distance != 0 && !pWorldObject) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u called without source worldobject, skipping.",
                      m_table, m_script->id, m_script->command);
      break;
    }
    ...
    if (pWorldObject && pWorldObject->GetTypeId() == TYPEID_UNIT && !((Creature *)pWorldObject)->IsAlive())
      failQuest = true;
             ...
             !pWorldObject->IsWithinDistInMap(pPlayer, float(m_script->questExplored.distance)))
      failQuest = true;
  ...
  case SCRIPT_COMMAND_KILL_CREDIT: // 8
  {
    ...
    WorldObject *pRewardSource = pSource && pSource->GetTypeId() == TYPEID_UNIT
                                     ? pSource
                                     : (pTarget && pTarget->GetTypeId() == TYPEID_UNIT ? pTarget : nullptr);
    ...
    if (!creatureEntry) {
      if (pRewardSource)
        ...
        sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u called for dynamic killcredit without "
                        "creature partner, skipping.",
                        m_table, m_script->id, m_script->command);
        break;
      }
    }
    ...
    if (m_script->killCredit.isGroupCredit) {
      ...
      if (pSearcher != pRewardSource)
        sLog.outDebug(" DB-SCRIPTS: Process table `%s` id %u, SCRIPT_COMMAND_KILL_CREDIT called for groupCredit "
                      "without creature as searcher, script might need adjustment.",
                      m_table, m_script->id);
  ...
  case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT: // 9
  {
    ...
    if (!pGo) {
      sLog.outErrorDb(
          " DB-SCRIPTS: Process table `%s` id %u, command %u failed for gameobject(guid: %u, buddyEntry: %u).", m_table,
          m_script->id, m_script->command, m_script->respawnGo.goGuid, m_script->buddyEntry);
      break;
    }

    if (pGo->GetGoType() == GAMEOBJECT_TYPE_FISHINGNODE || pGo->GetGoType() == GAMEOBJECT_TYPE_DOOR) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u can not be used with gameobject of type %u "
                      "(guid: %u, buddyEntry: %u).",
                      m_table, m_script->id, m_script->command, uint32(pGo->GetGoType()), m_script->respawnGo.goGuid,
                      m_script->buddyEntry);
      break;
    }
  ...
  case SCRIPT_COMMAND_TEMP_SPAWN_CREATURE: // 10
  {
    ...
    TempSpawnSettings settings(
        pSource, m_script->summonCreature.creatureEntry, x, y, z, o,
        m_script->summonCreature.despawnDelay ? TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN : TEMPSPAWN_DEAD_DESPAWN,
        m_script->summonCreature.despawnDelay, (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL) != 0, run,
        m_script->summonCreature.pathId, factionId, modelId);
    settings.spawnDataEntry = m_script->textId[3];

    Creature *pCreature = WorldObject::SummonCreature(settings, pSource->GetMap());
    if (!pCreature) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u failed for creature (entry: %u).", m_table,
                      m_script->id, m_script->command, m_script->summonCreature.creatureEntry);
      break;
    }
  ...
  case SCRIPT_COMMAND_CLOSE_DOOR: // 12
  {
    ...
    if (!pDoor) {
      sLog.outErrorDb(
          " DB-SCRIPTS: Process table `%s` id %u, command %u failed for gameobject(guid: %u, buddyEntry: %u).", m_table,
          m_script->id, m_script->command, m_script->changeDoor.goGuid, m_script->buddyEntry);
      break;
    }

    if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u failed for non-door(GoType: %u).", m_table,
                      m_script->id, m_script->command, pDoor->GetGoType());
      break;
    }
  ...
  case SCRIPT_COMMAND_CAST_SPELL: // 15
  {
    ...
    if (pSource && pSource->GetTypeId() == TYPEID_GAMEOBJECT) {
      ...
      ((Unit *)pTarget)
          ->CastSpell(((Unit *)pTarget), spell, TRIGGERED_OLD_TRIGGERED | TRIGGERED_DO_NOT_PROC, nullptr, nullptr,
                      pSource->GetObjectGuid());
      break;
    }
  ...
  {
    if (!pSource) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u could not find proper source", m_table,
                      m_script->id, m_script->command);
      break;
    }
    ...
    if (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL)
      ...
      if (m_script->playSound.flags & 2)
        ...
        m_map->PlayDirectSoundToMap(m_script->playSound.soundId,
                                    m_script->playSound.flags & 8 ? pSource->GetZoneId() : 0);
      else
  ...
  case SCRIPT_COMMAND_CREATE_ITEM: // 17
  {
    ...
    if (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL)
      ...
      if (Item *pItem =
              pPlayer->StoreNewItemInInventorySlot(m_script->createItem.itemEntry, m_script->createItem.amount))
  ...
  case SCRIPT_COMMAND_DESPAWN_SELF: // 18
  {
    ...
    if (pTarget && pTarget->GetTypeId() != TYPEID_UNIT && pSource && pSource->GetTypeId() == TYPEID_UNIT) {
      sLog.outErrorDb("DB-SCRIPTS: Process table `%s` id %u, command %u target must be creature, but (only) source is, "
                      "use data_flags to fix",
                      m_table, m_script->id, m_script->command);
      pTarget = pSource;
    }
  ...
  case SCRIPT_COMMAND_MOVEMENT: // 20
  {
    ...
    if (source->IsInCombat()) {
      DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                        " DB-SCRIPTS: Process table `%s` id %u, SCRIPT_COMMAND_MOVEMENT called for movement change to "
                        "%u with source guid %s but source is in combat and may lead to wrong behaviour: skipping.",
                        m_table, m_script->id, m_script->movement.movementType, pSource->GetGuidStr().c_str());
      break;
    }
    ...
    case RANDOM_MOTION_TYPE:
      if (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL)
        source->GetMotionMaster()->MoveRandomAroundPoint(
            pSource->GetPositionX(), pSource->GetPositionY(), pSource->GetPositionZ(),
            float(m_script->movement.wanderORpathId), 0.f, m_script->movement.timer);
      else {
        ...
        source->GetMotionMaster()->MoveRandomAroundPoint(respX, respY, respZ, wander_distance, 0.f,
                                                         m_script->movement.timer);
      }
      break;
  ...
  case SCRIPT_COMMAND_ATTACK_START: // 26
  {
    ...
    if (!pAttacker->CanAttackNow(unitTarget)) {
      sLog.outErrorDb(
          " DB-SCRIPTS: Process table `%s` id %u, command %u attacker can not attack (Attacker: %s, Target: %s)",
          m_table, m_script->id, m_script->command, pAttacker->GetGuidStr().c_str(), unitTarget->GetGuidStr().c_str());
      break;
    }
  ...
  case SCRIPT_COMMAND_MODIFY_NPC_FLAGS: // 29
  {
    ...
    // Toggle Flags
      ...
      sLog.outErrorDb(" DB-SCRIPTS: Unexpected value %u used for script id %u, command %u.", m_script->npcFlag.flag,
                      m_script->id, m_script->command);
  ...
  case SCRIPT_COMMAND_TERMINATE_SCRIPT: // 31
  {
    if (!pSource && (!pTarget && (m_script->data_flags & SCRIPT_FLAG_BUDDY_BY_GUID) == 0)) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u call for nullptr, skipping.", m_table,
                      m_script->id, m_script->command);
      return true;
    }
    ...
    if (m_script->terminateScript.npcEntry || m_script->terminateScript.poolId ||
        (m_script->data_flags & SCRIPT_FLAG_BUDDY_BY_GUID)) {
      Creature *pCreatureBuddy = nullptr;
      WorldObject *pSearcher = pSource ? pSource : pTarget;
      if (!pSearcher) {
        sLog.outError("DB - SCRIPTS: Process table `%s` id %u, command %u no pSource provided", m_table, m_script->id,
                      m_script->command);
        return false;
      }
      ...
      if (m_script->data_flags & SCRIPT_FLAG_BUDDY_BY_GUID) {
        ...
        MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(
            *pSearcher, m_script->terminateScript.npcEntry, true, false, m_script->terminateScript.searchDist, true);
        MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(
            pCreatureBuddy, u_check);
        Cell::VisitGridObjects(pSearcher, searcher, m_script->terminateScript.searchDist);
      } else {
        // poolId is provided
        // this is supposed to work only on situation where only one of the provided creature in pool can be alive at a
        // time
        ...
        for (auto objItr : equalChancedObjectList) {
          ...
          if (Creature *buddy = m_map->GetCreature(cData->GetObjectGuid(objItr.guid))) {
            ...
            if (buddy->IsAlive() &&
                (!m_script->terminateScript.searchDist ||
                 pSearcher->IsWithinDist3d(buddy->GetPositionX(), buddy->GetPositionY(), buddy->GetPositionZ(),
                                           m_script->terminateScript.searchDist))) {
      ...
      if (!(m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL) && !pCreatureBuddy) {
        if (m_script->terminateScript.npcEntry)
          DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                            "DB-SCRIPTS: Process table `%s` id %u, terminate further steps of this script! (as "
                            "searched npc entry(%u) was not found alive)",
                            m_table, m_script->id, m_script->terminateScript.npcEntry);
        else
          DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                            "DB-SCRIPTS: Process table `%s` id %u, terminate further steps of this script! (as no npc "
                            "in pool id(%u) was found alive)",
                            m_table, m_script->id, m_script->terminateScript.poolId);
        result = true;
      } else if (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL && pCreatureBuddy) {
        if (m_script->terminateScript.npcEntry)
          DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                            "DB-SCRIPTS: Process table `%s` id %u, terminate further steps of this script! (as "
                            "searched npc entry(%u) was found alive)",
                            m_table, m_script->id, m_script->terminateScript.npcEntry);
        else
          DETAIL_FILTER_LOG(LOG_FILTER_DB_SCRIPT,
                            "DB-SCRIPTS: Process table `%s` id %u, terminate further steps of this script! (as "
                            "searched npc in pool id(%u) was found alive)",
                            m_table, m_script->id, m_script->terminateScript.poolId);
        result = true;
      }
    } else
    ...
    if (result) // Terminate further steps of this script
    {
      if (m_script->textId[0] && !LogIfNotCreature(pSource)) {
        ...
        if (cSource->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
          (static_cast<WaypointMovementGenerator<Creature> *>(cSource->GetMotionMaster()->top()))
              ->AddToWaypointPauseTime(m_script->textId[0]);
      }
  ...
  case SCRIPT_COMMAND_RESERVED_1: // 33
  {
    sLog.outError(" DB-SCRIPTS: Process table `%s` id %u, command %u not supported.", m_table, m_script->id,
                  m_script->command);
    break;
  }
  case SCRIPT_COMMAND_TERMINATE_COND: {
    ...
    if (m_script->data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL)
      terminateResult = !sObjectMgr.IsConditionSatisfied(m_script->terminateCond.conditionId, player, m_map, second,
  ...
  case SCRIPT_COMMAND_SEND_AI_EVENT: // 35
  {
    ...
    if (m_script->sendAIEvent.radius)
      ((Creature *)pSource)
          ->AI()
          ->SendAIEventAround(AIEventType(m_script->sendAIEvent.eventType), (Unit *)pTarget, 0,
                              float(m_script->sendAIEvent.radius));
  ...
  case SCRIPT_COMMAND_SET_FACING: // 36
  {
    ...
    if (!pTarget) {
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command _SET_FACING (%u): No target found.", m_table,
                      m_script->id, m_script->command);
      return false;
    }
    if (m_script->setFacing.resetFacing) {
      ...
      if (pCSource->GetMotionMaster()->empty() ||
          !pCSource->GetMotionMaster()->top()->GetResetPosition(*pCSource, x, y, z, o))
  ...
  case SCRIPT_COMMAND_MOVE_DYNAMIC: // 37
  {
    ...
    if (source->IsInCombat()) {
      sLog.outDebug(" DB-SCRIPTS: Process table `%s` id %u, SCRIPT_COMMAND_MOVE_DYNAMIC called for source guid %s but "
                    "source is in combat and may lead to wrong behaviour: skipping.",
                    m_table, m_script->id, pSource->GetGuidStr().c_str());
      break;
    }
    ...
    {
      if (pTarget == pSource) {
        sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, _MOVE_DYNAMIC called with maxDist == 0, but "
                        "resultingSource == resultingTarget (== %s)",
                        m_table, m_script->id, pSource->GetGuidStr().c_str());
        break;
      }
    ...
    } else // Calculate position
    {
      ...
      pSource->GetRandomPoint(pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(),
                              m_script->moveDynamic.maxDist, x, y, z, m_script->moveDynamic.minDist,
                              (orientation == 0.0f ? nullptr : &orientation));
  ...
  case SCRIPT_COMMAND_SEND_MAIL: // 38
  {
    ...
    MailDraft(m_script->sendMail.mailTemplateId)
        .SendMailTo(static_cast<Player *>(pTarget), sender, MAIL_CHECK_MASK_HAS_BODY, deliverDelay);
    break;
  }
  case SCRIPT_COMMAND_SET_HOVER: // 39
  {
  ...
  case SCRIPT_COMMAND_RESET_GO: // 43
  {
    ...
    case GAMEOBJECT_TYPE_BUTTON:
      ...
      break;
    default:
      sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u failed for gameobject(buddyEntry: %u). "
                      "Gameobject is not a door or button",
                      m_table, m_script->id, m_script->command, m_script->buddyEntry);
      break;
    }
    break;
  ...
  case SCRIPT_COMMAND_UPDATE_TEMPLATE: // 44
  {
    ...
    if (pCSource->GetEntry() != m_script->updateTemplate.newTemplate)
      ...
      sLog.outErrorDb(
          " DB-SCRIPTS: Process table `%s` id %u, command %u failed. Source already has specified creature entry.",
          m_table, m_script->id, m_script->command);
    break;
  }
  case SCRIPT_COMMAND_START_RELAY_SCRIPT: // 45
  ...
  case SCRIPT_COMMAND_CAST_CUSTOM_SPELL: // 46
  {
    ...
    ((Unit *)pSource)
        ->CastCustomSpell((Unit *)pTarget, m_script->castCustomSpell.spellId, &m_script->textId[0],
                          &m_script->textId[1], &m_script->textId[2], m_script->castCustomSpell.castFlags);
    break;
  }
  case SCRIPT_COMMAND_INTERRUPT_SPELL: // 47
  {
  ...
  case SCRIPT_COMMAND_MODIFY_UNIT_FLAGS: // 48
  {
    ...
    // Toggle Flags
      ...
      sLog.outErrorDb(" DB-SCRIPTS: Unexpected value %u used for script id %u, command %u.", m_script->unitFlag.flag,
                      m_script->id, m_script->command);

    break;
  }
  default:
    sLog.outErrorDb(" DB-SCRIPTS: Process table `%s` id %u, command %u unknown command used.", m_table, m_script->id,
                    m_script->command);
    break;
  }
...
int32 ScriptMgr::GetRandomScriptStringFromTemplate(uint32 id) { return GetRandomScriptTemplateId(id, STRING_TEMPLATE); }

int32 ScriptMgr::GetRandomRelayDbscriptFromTemplate(uint32 id) { return GetRandomScriptTemplateId(id, RELAY_TEMPLATE); }
