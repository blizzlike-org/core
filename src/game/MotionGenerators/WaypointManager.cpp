...
char const *waypointOriginTables[] = {
    "", "creature_movement", "creature_movement_template", "DND", "creature_movement_reference",
};
...
void CheckDbscript(WaypointNode &node, uint32 entry, uint32 point, std::set<uint32> &movementScriptSet,
                   std::string const &tablename) {
  auto iter = sCreatureMovementScripts.second.find(node.script_id);
  if (iter == sCreatureMovementScripts.second.end()) {
    sLog.outErrorDb("Table %s for entry %u, point %u have script_id %u that does not exist in "
                    "`dbscripts_on_creature_movement`, ignoring",
                    tablename.data(), entry, point, node.script_id);
    return;
  } else if (!node.delay) {
    auto &data = *iter;
    bool delay = false;
    for (auto &item : data.second) {
      ...
      if (item.second.command == SCRIPT_COMMAND_DESPAWN_SELF && item.second.delay == 0 &&
          item.second.despawn.despawnDelay == 0) {
        delay = true;
        sLog.outErrorDb("Table %s entry %u point %u has no delay and no delay despawn script. Adding delay to point.",
                        tablename.data(), entry, point);
        break;
      } else if (item.second.command == SCRIPT_COMMAND_MOVEMENT) {
        delay = true;
        sLog.outErrorDb("Table %s entry %u point %u has no delay but changes movegen. Adding delay to point.",
                        tablename.data(), entry, point);
        break;
              ...
              if (iter != sRelayScripts.second.end()) {
                ...
                for (auto &item : data.second) {
                  ...
                  if (item.second.command == SCRIPT_COMMAND_DESPAWN_SELF && item.second.delay == 0 &&
                    ...
                    delay = true;
                    sLog.outErrorDb(
                        "Table %s entry %u point %u has no delay and no delay despawn script. Adding delay to point.",
                        tablename.data(), entry, point);
                    break;
                  } else if (item.second.command == SCRIPT_COMMAND_MOVEMENT) {
                    delay = true;
                    sLog.outErrorDb(
                        "Table %s entry %u point %u has no delay but changes movegen. Adding delay to point.",
                        tablename.data(), entry, point);
                    break;
                  }
                }
...
void WaypointManager::Load() {
  ...
  for (ScriptMapMap::const_iterator itr = sCreatureMovementScripts.second.begin();
  ...
  if (!result) {
    ...
    //                                   0   1      2           3           4           5            6         7
    result = WorldDatabase.Query("SELECT id, point, position_x, position_y, position_z, orientation, waittime, "
                                 "script_id FROM creature_movement");
    ...
    do {
      ...
      if (!cData) {
        sLog.outErrorDb("Table creature_movement contain path for creature guid %u, but this creature guid does not "
                        "exist. Skipping.",
                        id);
        continue;
      }
      ...
      if (!MaNGOS::IsValidMapCoord(node.x, node.y, node.z, node.orientation)) {
        ...
        if (result1)
          ...
          sLog.outErrorDb("Waypoint path %d, have invalid coordinates in his waypoint %d (X: %f, Y: %f).", id, point,
                          node.x, node.y);
        ...
        WorldDatabase.PExecute("UPDATE creature_movement SET position_x = '%f', position_y = '%f', position_z = '%f' "
                               "WHERE id = '%u' AND point = '%u'",
                               node.x, node.y, node.z, id, point);
      }
    ...
    if (!creatureNoMoveType.empty()) {
      for (uint32 itr : creatureNoMoveType) {
        ...
        ERROR_DB_STRICT_LOG("Table creature_movement has waypoint for creature guid %u (entry %u), but MovementType is "
                            "not WAYPOINT_MOTION_TYPE(2). Make sure that this is actually used in a script!",
                            itr, cData->id);

        if (cInfo->MovementType == WAYPOINT_MOTION_TYPE)
          sLog.outErrorDb("Table creature_template for this entry(%u) guid(%u) has MovementType "
                          "WAYPOINT_MOTION_TYPE(2), did you intend to use creature_movement_template ?",
                          cData->id, itr);
      }
    }

    sLog.outString(">> Loaded %u paths, %u nodes and %u behaviors from waypoints", total_paths, total_nodes,
                   total_behaviors);
    sLog.outString();
  ...
  if (!result) {
    ...
    //                                   0      1       2      3           4           5           6            7
    result = WorldDatabase.Query("SELECT entry, pathId, point, position_x, position_y, position_z, orientation, "
                                 "waittime, script_id FROM creature_movement_template");
    ...
    do {
      ...
      if (point == 0) {
        ...
        sLog.outErrorDb("Table `creature_movement_template` has invalid point 0 for entry %u in path %u. Skipping.`",
                        entry, pathId);
      }
      ...
      // prevent using invalid coordinates
      if (!MaNGOS::IsValidMapCoord(node.x, node.y, node.z, node.orientation)) {
        sLog.outErrorDb("Table creature_movement_template for entry %u (point %u) are using invalid coordinates "
        ...
        sLog.outErrorDb("Table creature_movement_template for entry %u (point %u) are auto corrected to normalized "
                        "position_x=%f, position_y=%f",
                        entry, point, node.x, node.y);

        WorldDatabase.PExecute("UPDATE creature_movement_template SET position_x = '%f', position_y = '%f' WHERE entry "
                               "= %u AND point = %u AND pathId = %u",
                               node.x, node.y, entry, point, pathId);
      }
    ...
    sLog.outString(">> Loaded %u path templates with %u nodes and %u behaviors from waypoint templates", total_paths,
                   total_nodes, total_behaviors);
    sLog.outString();
  }
...
bool WaypointManager::AddExternalNode(uint32 entry, int32 pathId, uint32 pointId, float x, float y, float z, float o,
                                      uint32 waittime, uint32 scriptId) {
  ...
  if (!MaNGOS::IsValidMapCoord(x, y, z, o)) {
    sLog.outErrorScriptLib(
        "WaypointManager::AddExternalNode: (Npc-Entry %u, PathId %i, PointId %u) Invalid coordinates", entry, pathId,
        pointId);
    return false;
  }
...
/// - Insert at a certain point, if pointId == 0 insert last. In this case pointId will be changed to the id to which
/// the node was added
WaypointNode const *WaypointManager::AddNode(uint32 entry, uint32 dbGuid, uint32 pathId, uint32 &pointId,
                                             WaypointPathOrigin wpDest, float x, float y, float z, float orientation) {
  ...
  for (WaypointPath::reverse_iterator rItr = path.rbegin(); rItr != path.rend() && rItr->first > pointId; ++rItr) {
    if (rItr->first <= nextPoint) {
      if (wpDest == PATH_FROM_ENTRY)
        WorldDatabase.PExecuteLog("UPDATE %s SET point=point+1 WHERE %s=%u AND point=%u AND pathId=%u", table,
                                  key_field, key, rItr->first - 1, pathId);
      else
        WorldDatabase.PExecuteLog("UPDATE %s SET point=point+1 WHERE %s=%u AND point=%u", table, key_field, key,
                                  rItr->first - 1);
    }
  }
  ...
  if (wpDest == PATH_FROM_ENTRY)
    WorldDatabase.PExecuteLog(
        "INSERT INTO %s (%s,pathId,point,position_x,position_y,position_z,orientation) VALUES (%u,%u,%u, %f,%f,%f, %f)",
        table, key_field, pathId, key, pointId, x, y, z, orientation);
  else
    WorldDatabase.PExecuteLog(
        "INSERT INTO %s (%s,point,position_x,position_y,position_z,orientation) VALUES (%u,%u, %f,%f,%f, %f)", table,
        key_field, key, pointId, x, y, z, orientation);

  return &path[pointId];
}

void WaypointManager::DeleteNode(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId,
  ...
  if (wpOrigin == PATH_FROM_ENTRY)
    WorldDatabase.PExecuteLog("DELETE FROM %s WHERE %s=%u AND point=%u AND pathId=%u", table, key_field, key, point,
                              pathId);
  else
...
void WaypointManager::SetNodePosition(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId,
                                      WaypointPathOrigin wpOrigin, float x, float y, float z) {
  // Support only normal movement tables
  if (wpOrigin == PATH_FROM_EXTERNAL)
    return;
  ...
  if (wpOrigin == PATH_FROM_ENTRY)
    WorldDatabase.PExecuteLog(
        "UPDATE %s SET position_x=%f, position_y=%f, position_z=%f WHERE %s=%u AND point=%u AND pathId=%u", table, x, y,
        z, key_field, key, point, pathId);
  else
    WorldDatabase.PExecuteLog("UPDATE %s SET position_x=%f, position_y=%f, position_z=%f WHERE %s=%u AND point=%u",
                              table, x, y, z, key_field, key, point);
...
void WaypointManager::SetNodeWaittime(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId,
                                      WaypointPathOrigin wpOrigin, uint32 waittime) {
  // Support only normal movement tables
  if (wpOrigin == PATH_FROM_EXTERNAL)
    return;
  ...
  uint32 const key = wpOrigin == PATH_FROM_GUID ? dbGuid : entry;
  if (wpOrigin == PATH_FROM_ENTRY)
    WorldDatabase.PExecuteLog("UPDATE %s SET waittime=%u WHERE %s=%u AND point=%u AND pathId=%u", table, waittime,
                              key_field, key, point, pathId);
  else
    WorldDatabase.PExecuteLog("UPDATE %s SET waittime=%u WHERE %s=%u AND point=%u", table, waittime, key_field, key,
                              point);
...
void WaypointManager::SetNodeOrientation(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId,
                                         WaypointPathOrigin wpOrigin, float orientation) {
  // Support only normal movement tables
  if (wpOrigin == PATH_FROM_EXTERNAL)
    return;
  ...
  uint32 const key = wpOrigin == PATH_FROM_GUID ? dbGuid : entry;
  if (wpOrigin == PATH_FROM_ENTRY)
    WorldDatabase.PExecuteLog("UPDATE %s SET orientation=%f WHERE %s=%u AND point=%u AND pathId=%u", table, orientation,
                              key_field, key, point, pathId);
  else
    WorldDatabase.PExecuteLog("UPDATE %s SET orientation=%f WHERE %s=%u AND point=%u", table, orientation, key_field,
                              key, point);
...
bool WaypointManager::SetNodeScriptId(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId,
                                      WaypointPathOrigin wpOrigin, uint32 scriptId) {
  // Support only normal movement tables
  if (wpOrigin == PATH_FROM_EXTERNAL)
    return false;
  ...
  if (wpOrigin == PATH_FROM_ENTRY)
    WorldDatabase.PExecuteLog("UPDATE %s SET script_id=%u WHERE %s=%u AND point=%u AND pathId=%u", table, scriptId,
                              key_field, key, point, pathId);
  else
    WorldDatabase.PExecuteLog("UPDATE %s SET script_id=%u WHERE %s=%u AND point=%u", table, scriptId, key_field, key,
                              point);
