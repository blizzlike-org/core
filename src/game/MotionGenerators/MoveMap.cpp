...
uint32 MMapManager::packTileID(int32 x, int32 y) const { return uint32(x << 16 | y); }
...
bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y) {
  ...
  if (fileHeader.mmapVersion != MMAP_VERSION) {
    sLog.outError("MMAP:loadMap: %03u%02i%02i.mmtile was built with generator v%i, expected v%i", mapId, x, y,
  ...
  ++loadedTiles;
  DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:loadMap: Loaded mmtile %03i[%02i,%02i] into %03i[%02i,%02i]", mapId, x,
                   y, mapId, header->x, header->y);
  return true;
}
...
bool MMapManager::loadGameObject(uint32 displayId) {
  ...
  if (fileHeader.mmapVersion != MMAP_VERSION) {
    sLog.outError("MMAP:loadGameObject: %s was built with generator v%i, expected v%i", fileName,
                  fileHeader.mmapVersion, MMAP_VERSION);
    fclose(file);
    return false;
  }
...
bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y) {
  ...
  if (loadedMMaps.find(mapId) == loadedMMaps.end()) {
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING,
                     "MMAP:unloadMap: Asked to unload not loaded navmesh map. %03u%02i%02i.mmtile", mapId, x, y);
    return false;
  }
  ...
  if (mmap->mmapLoadedTiles.find(packedGridPos) == mmap->mmapLoadedTiles.end()) {
    // file may not exist, therefore not loaded
    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING,
                     "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %03u%02i%02i.mmtile", mapId, x, y);
    return false;
  }
  ...
  if (dtStatusFailed(dtResult)) {
    ...
    --loadedTiles;
    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:unloadMap: Unloaded mmtile %03i[%02i,%02i] from %03i", mapId, x, y,
                     mapId);
    return true;
  }
...
bool MMapManager::unloadMapInstance(uint32 mapId, uint32 instanceId) {
  ...
  if (loadedMMaps.find(mapId) == loadedMMaps.end()) {
    // file may not exist, therefore not loaded
    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:unloadMapInstance: Asked to unload not loaded navmesh map %03u",
                     mapId);
    return false;
  }
  ...
  if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end()) {
    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING,
                     "MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId %03u instanceId %u",
                     mapId, instanceId);
    return false;
  }
  ...
  DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:unloadMapInstance: Unloaded mapId %03u instanceId %u", mapId,
                   instanceId);
...
dtNavMeshQuery const *MMapManager::GetNavMeshQuery(uint32 mapId, uint32 instanceId) {
  ...
  if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end()) {
    ...
    if (dtStatusFailed(dtResult)) {
      dtFreeNavMeshQuery(query);
      sLog.outError("MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %03u instanceId %u", mapId,
                    instanceId);
      return nullptr;
    }

    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING,
                     "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId %03u instanceId %u", mapId, instanceId);
  ...
  if (mmap->navMeshGOQueries.find(threadId) == mmap->navMeshGOQueries.end()) {
    ...
    if (mmap->navMeshGOQueries.find(threadId) == mmap->navMeshGOQueries.end()) {
      ...
      if (dtStatusFailed(query->init(mmap->navMesh, 2048))) {
        dtFreeNavMeshQuery(query);
        sLog.outError("MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for displayid %03u tid %s", displayId,
                      ss.str().data());
        return nullptr;
      }
