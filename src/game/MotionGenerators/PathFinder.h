...
public:
  ...
  bool calculate(float destX, float destY, float destZ, bool forceDest = false,
                 bool straightLine = false); // transfers coorddinates from global to local space if on transport - use
                                             // other func if coords are already in transport space
  ...
  void setPathLengthLimit(float distance) {
    m_pointPathLimit = std::min<uint32>(uint32(distance / SMOOTH_PATH_STEP_SIZE * 1.25f), MAX_POINT_PATH_LENGTH);
  };
...
private:
  ...
  dtPolyRef getPathPolyByPosition(const dtPolyRef *polyPath, uint32 polyPathSize, const float *point,
  ...
  bool getSteerTarget(const float *startPos, const float *endPos, float minTargetDist, const dtPolyRef *path,
                      uint32 pathSize, float *steerPos, unsigned char &steerPosFlag, dtPolyRef &steerPosRef) const;
