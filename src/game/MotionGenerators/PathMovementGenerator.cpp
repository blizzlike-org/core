...
AbstractPathMovementGenerator::AbstractPathMovementGenerator(const WaypointPath *path, int32 offset /* = 0*/,
  ...
  if (!path)
    return;
  // NOTE:
  // Data in WaypointManager can be removed at run time, we cant really reference it and have to make a copy just in
  // case Possibly remake storage in WaypointManager with smart pointers so we can reference it instead
  m_path = *path;
...
void AbstractPathMovementGenerator::Initialize(Unit &unit) {
  ...
  if (m_path.empty()) {
    sLog.outError("AbstractPathMovementGenerator::Initialize Path empty for unit name %s entry %u counter %u.",
                  unit.GetName(), unit.GetEntry(), unit.GetGUIDLow());
    return;
  }
...
void AbstractPathMovementGenerator::Interrupt(Unit &unit) { unit.InterruptMoving(); }

void AbstractPathMovementGenerator::Reset(Unit &unit) { Initialize(unit); }
...
FixedPathMovementGenerator::FixedPathMovementGenerator(Unit &creature, int32 pathId, WaypointPathOrigin wpOrigin,
                                                       ...
                                                       int32 offset, bool cyclic)
    : AbstractPathMovementGenerator(
          (pathId || wpOrigin != PATH_NO_PATH
               ? sWaypointMgr.GetPathFromOrigin(creature.GetEntry(), creature.GetGUIDLow(), pathId,
                                                (wpOrigin == PATH_NO_PATH && pathId ? PATH_FROM_ENTRY : wpOrigin))
               : sWaypointMgr.GetDefaultPath(creature.GetEntry(), creature.GetGUIDLow())),
          offset, cyclic),
...
FixedPathMovementGenerator::FixedPathMovementGenerator(Creature &creature)
    : AbstractPathMovementGenerator(
          (creature.GetMotionMaster()->GetPathId()
               ? sWaypointMgr.GetPathFromOrigin(creature.GetEntry(), creature.GetGUIDLow(),
