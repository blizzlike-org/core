...
template <class T, class P> class PathMovementBase {
public:
...
template <class T> class WaypointMovementGenerator;
...
public:
  WaypointMovementGenerator(Creature &)
      : i_nextMoveTime(0), m_scriptTime(0), m_lastReachedWaypoint(0), m_pathId(0), m_pathDuration(0), m_PathOrigin() {}
  ...
  void InitializeWaypointPath(Creature &u, int32 pathId, WaypointPathOrigin wpSource, uint32 initialDelay,
  ...
  void GetPathInformation(uint32 &pathId, WaypointPathOrigin &wpOrigin) const {
    pathId = m_pathId;
