...
public:
  ...
  WaypointNode const *AddNode(uint32 entry, uint32 dbGuid, uint32 pathId, uint32 &pointId, WaypointPathOrigin wpDest,
  ...
  void SetNodePosition(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId, WaypointPathOrigin wpOrigin, float x,
                       float y, float z);
  void SetNodeWaittime(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId, WaypointPathOrigin wpOrigin,
                       uint32 waittime);
  void SetNodeOrientation(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId, WaypointPathOrigin wpOrigin,
                          float orientation);
  bool SetNodeScriptId(uint32 entry, uint32 dbGuid, uint32 point, uint32 pathId, WaypointPathOrigin wpOrigin,
                       uint32 scriptId);
...
private:
  ...
  WaypointPathMap *getMapForPathType(WaypointPathOrigin origin) {
    ...
    case PATH_FROM_GUID:
      return &m_pathMap;
    case PATH_FROM_ENTRY:
      return &m_pathTemplateMap;
    case PATH_FROM_EXTERNAL:
      return &m_externalPathTemplateMap;
    default:
      return nullptr;
    }
  }
...
bool AddWaypointFromExternal(uint32 entry, uint32 pathId, uint32 pointId, float x, float y, float z, float o,
                             uint32 waittime);
