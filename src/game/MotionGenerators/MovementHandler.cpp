...
void WorldSession::HandleMoveWorldportAckOpcode() {
  ...
  if (mEntry->IsBattleGroundOrArena()) {
    ...
    if (!map) {
      DETAIL_LOG(
          "WorldSession::HandleMoveWorldportAckOpcode: %s was teleported far to nonexisten battleground instance "
      ...
      if (!GetPlayer()->TeleportTo(old_loc)) {
        DETAIL_LOG("WorldSession::HandleMoveWorldportAckOpcode: %s cannot be ported to his previous place, teleporting "
                   "him to his homebind place...",
  ...
  if (GetPlayer()->m_teleportTransport) {
    found = false;
    GetPlayer()->m_movementInfo.t_pos =
        Position(loc.coord_x, loc.coord_y, loc.coord_z,
                 loc.orientation); // when teleporting onto transport, position is local coords
...
void WorldSession::HandleMovementOpcodes(WorldPacket &recv_data) {
  ...
  if (plMover && plMover->IsLaunched() &&
      (recv_data.GetOpcode() == MSG_MOVE_FALL_LAND || recv_data.GetOpcode() == MSG_MOVE_START_SWIM))
...
void WorldSession::HandleForceSpeedChangeAckOpcodes(WorldPacket &recv_data) {
  ...
  static char const *move_type_name[MAX_MOVE_TYPE] = {"Walk",     "Run",      "RunBack", "Swim",
                                                      "SwimBack", "TurnRate", "Flight",  "FlightBack"};
  ...
  if (!_player->GetTransport() && fabs(_player->GetSpeed(move_type) - newspeed) > 0.01f) {
    ...
    {
      BASIC_LOG("Player %s from account id %u kicked for incorrect speed (must be %f instead %f)", _player->GetName(),
                _player->GetSession()->GetAccountId(), _player->GetSpeed(move_type), newspeed);
...
void WorldSession::HandleMoveNotActiveMoverOpcode(WorldPacket &recv_data) {
  ...
  if (_player->IsClientControlled() && _player->GetMover() && _player->GetMover()->GetObjectGuid() == old_mover_guid) {
    sLog.outError("HandleMoveNotActiveMover: incorrect mover guid: mover is %s and should be %s instead of %s",
                  _player->GetMover()->GetGuidStr().c_str(), _player->GetGuidStr().c_str(),
...
bool WorldSession::VerifyMovementInfo(MovementInfo const &movementInfo) const {
  if (!MaNGOS::IsValidMapCoord(movementInfo.GetPos().x, movementInfo.GetPos().y, movementInfo.GetPos().z,
                               movementInfo.GetPos().o))
    return false;

  if (movementInfo.HasMovementFlag(MOVEFLAG_ONTRANSPORT)) {
    // transports size limited
    // (also received at zeppelin/lift leave by some reason with t_* as absolute in continent coordinates, can be safely
    // skipped)
    if (movementInfo.GetTransportPos().x > 50 || movementInfo.GetTransportPos().y > 50 ||
        movementInfo.GetTransportPos().z > 100)
      return false;

    if (!MaNGOS::IsValidMapCoord(movementInfo.GetPos().x + movementInfo.GetTransportPos().x,
...
void WorldSession::HandleMoverRelocation(MovementInfo &movementInfo) {
  ...
  {
    if (mover->IsInWorld())
      mover->GetMap()->CreatureRelocation((Creature *)mover, movementInfo.GetPos().x, movementInfo.GetPos().y,
...
void WorldSession::HandleTimeSyncResp(WorldPacket &recvData) {
  ...
  // time it took for the request to travel to the client, for the client to process it and reply and for response to
  // travel back to the server. we are going to make 2 assumptions: 1) we assume that the request processing time equals
  // 0. 2) we assume that the packet took as much time to travel from server to client than it took to travel from
  // client to server.
  ...
  where
  serverTime: time that was displayed on the clock of the SERVER at the moment when the client processed the
  SMSG_TIME_SYNC_REQUEST packet. clientTime:  time that was displayed on the clock of the CLIENT at the moment when the
  client processed the SMSG_TIME_SYNC_REQUEST packet. Once clockDelta has been computed, we can compute the time of an
  event on server clock when we know the time of that same event on the client clock, using the following relation:
...
void WorldSession::ComputeNewClockDelta() {
  ...
  // https://web.archive.org/web/20180430214420/http://www.mine-control.com/zack/timesync/timesync.html to reduce the
  // skew induced by dropped TCP packets that get resent.
