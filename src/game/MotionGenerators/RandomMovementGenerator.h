...
public:
  explicit AbstractRandomMovementGenerator(uint32 stateActive, uint32 stateMotion, uint32 delayMin, uint32 delayMax,
                                           uint32 movesMax = 1)
      : i_x(0.0f), i_y(0.0f), i_z(0.0f), i_radius(0.0f), i_verticalZ(0.0f), i_pathLength(0.0f), i_walk(true),
        i_nextMoveTimer(0), i_nextMoveCount(1), i_nextMoveCountMax(movesMax), i_nextMoveDelayMin(delayMin),
        i_nextMoveDelayMax(delayMax), i_stateActive(stateActive), i_stateMotion(stateMotion) {}
