...
protected:
  TargetedMovementGeneratorMedium(Unit &target, float offset, float angle)
      : TargetedMovementGeneratorBase(target), i_recheckDistance(0), i_offset(offset), i_angle(angle),
        i_speedChanged(false), i_targetReached(false), i_faceTarget(true), i_path(nullptr) {}
...
Fanning - likewise after 2 seconds, all mobs spread out at a given target - has to be coordinated with all targets which
attack same victim
...
public:
  ChaseMovementGenerator(Unit &target, float offset, float angle, bool moveFurther = true, bool walk = false,
      ...
      : TargetedMovementGeneratorMedium<Unit, ChaseMovementGenerator>(target, offset, angle),
        m_moveFurther(moveFurther), m_walk(walk), m_combat(combat), m_reachable(true), m_fanningEnabled(true),
...
public:
  FollowMovementGenerator(Unit &target, float offset, float angle, bool main, bool possess, bool alwaysBoost = false)
      : TargetedMovementGeneratorMedium<Unit, FollowMovementGenerator>(target, offset, angle), m_main(main),
        m_possess(possess), m_boost(alwaysBoost), m_targetMoving(false), m_targetFaced(false) {
