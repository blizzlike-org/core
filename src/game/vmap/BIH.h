...
public:
  ...
  void intersectRay(const Ray &r, RayCallback &intersectCallback, float &maxDist, bool stopAtFirst = false,
                    bool ignoreM2Model = false) const {
  ...
  template <typename IsectCallback> void intersectPoint(const Vector3 &p, IsectCallback &intersectCallback) const {
...
protected:
  ...
  public:
    BuildStats()
        : numNodes(0), numLeaves(0), sumObjects(0), minObjects(0x0FFFFFFF), maxObjects(0xFFFFFFFF), sumDepth(0),
          minDepth(0x0FFFFFFF), maxDepth(0xFFFFFFFF), numBVH2(0) {
  ...
  void subdivide(int left, int right, std::vector<uint32> &tempTree, buildData &dat, AABound &gridBox, AABound &nodeBox,
