...
StaticMapTree::StaticMapTree(uint32 mapID, const std::string &basePath)
  ...
  if (iBasePath.length() > 0 &&
      (iBasePath[iBasePath.length() - 1] != '/' && iBasePath[iBasePath.length() - 1] != '\\')) {
    iBasePath.append("/");
  }
}
...
bool StaticMapTree::getIntersectionTime(const G3D::Ray &pRay, float &pMaxDist, bool pStopAtFirstHit,
...
bool StaticMapTree::getObjectHitPos(const Vector3 &pPos1, const Vector3 &pPos2, Vector3 &pResultHitPos,
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2 *vm) {
  ...
  if (tile == iLoadedTiles.end()) {
    // ERROR_LOG("StaticMapTree::UnloadMapTile(): Trying to unload non-loaded tile. Map:%u X:%u Y:%u", iMapID, tileX,
    // tileY);
    return;
  }
