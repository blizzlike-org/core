...
template <class T, class BoundsFunc = BoundsTrait<T>> class BIHWrap {
  template <class RayCallback> struct MDLCallback {
    const T *const *objects;
    RayCallback &cb;
    uint32 objectsSize;

    MDLCallback(RayCallback &callback, const T *const *objects_array, uint32 objSize)
        : objects(objects_array), cb(callback), objectsSize(objSize) {}
...
public:
  ...
  template <typename IsectCallback> void intersectPoint(const Vector3 &p, IsectCallback &intersectCallback) {
    balance();
